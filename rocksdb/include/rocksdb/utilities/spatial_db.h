...
class FeatureSet {
	...
	class iterator {
	    public:
		/* implicit */ iterator(const map::const_iterator itr)
			: itr_(itr)
		{
		}
		iterator &operator++()
...
template <typename T> struct BoundingBox {
	...
	{
		return !(min_x > a.max_x || min_y > a.max_y ||
			 a.min_x > max_x || a.min_y > max_y);
	}
};
...
struct SpatialIndexOptions {
	...
	SpatialIndexOptions(const std::string &_name,
			    const BoundingBox<double> &_bbox,
			    uint32_t _tile_bits)
...
class SpatialDB : public StackableDB {
	...
	static Status
	Create(const SpatialDBOptions &options, const std::string &name,
	...
	static Status Open(const SpatialDBOptions &options,
