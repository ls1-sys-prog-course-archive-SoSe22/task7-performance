...
class LDBCommand {
	...
	static LDBCommand *InitFromCmdLineArgs(
		...
		const std::function<LDBCommand *(const ParsedParams &)>
   ...
   */
	...
	BuildCmdLineOptions(std::vector<std::string> options);
	...
	bool
	ParseStringOption(const std::map<std::string, std::string> &options,
   ...
   */
	bool
	ParseBooleanOption(const std::map<std::string, std::string> &options,
...
class LDBCommandRunner {
	...
	static void
	RunCommand(int argc, char **argv, Options options,
		   const LDBOptions &ldb_options,
		   const std::vector<ColumnFamilyDescriptor> *column_families);
};
