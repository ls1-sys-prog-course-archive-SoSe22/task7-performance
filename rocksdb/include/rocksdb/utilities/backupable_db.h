...
struct BackupableDBOptions {
	...
	explicit BackupableDBOptions(
		...
		: backup_dir(_backup_dir), backup_env(_backup_env),
		  share_table_files(_share_table_files), info_log(_info_log),
		  sync(_sync), destroy_old_data(_destroy_old_data),
		  ...
		  callback_trigger_interval_size(
			  _callback_trigger_interval_size),
...
struct BackupInfo {
	...
	BackupInfo(BackupID _backup_id, int64_t _timestamp, uint64_t _size,
		...
		: backup_id(_backup_id), timestamp(_timestamp), size(_size),
		  number_files(_number_files), app_metadata(_app_metadata)
	{
	}
};
...
class BackupEngineReadOnly {
	...
	virtual void
	GetCorruptedBackups(std::vector<BackupID> *corrupt_backup_ids) = 0;
