...
struct ExternalSstFileInfo {
	...
	ExternalSstFileInfo(const std::string &_file_path,
			    ...
			    int32_t _version)
		: file_path(_file_path), smallest_key(_smallest_key),
		  largest_key(_largest_key), sequence_number(_sequence_number),
		  file_size(_file_size), num_entries(_num_entries),
		  version(_version)
	{
	}
...
class SstFileWriter {
	...
	SstFileWriter(const EnvOptions &env_options, const Options &options,
		...
		: SstFileWriter(env_options, options, options.comparator,
	...
	ROCKSDB_DEPRECATED_FUNC Status Add(const Slice &user_key,
					   const Slice &value);
