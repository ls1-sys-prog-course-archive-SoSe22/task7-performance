...
template <int A, int B> struct constexpr_max {
...
struct ThreadStatus {
	...
	static const int kNumOperationProperties =
		constexpr_max<NUM_COMPACTION_PROPERTIES,
	...
	ThreadStatus(const uint64_t _id, const ThreadType _thread_type,
		...
		: thread_id(_id), thread_type(_thread_type), db_name(_db_name),
		  cf_name(_cf_name), operation_type(_operation_type),
		  op_elapsed_micros(_op_elapsed_micros),
		  operation_stage(_operation_stage), state_type(_state_type)
	{
	...
	GetOperationPropertyName(OperationType op_type, int i);

	// Translate the "i"th property of the specified operation given
	// a property value.
	static std::map<std::string, uint64_t>
	InterpretOperationProperties(OperationType op_type,
