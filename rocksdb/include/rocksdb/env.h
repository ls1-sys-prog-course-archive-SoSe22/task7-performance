...
class Env {
	...
	virtual EnvOptions
	OptimizeForLogRead(const EnvOptions &env_options) const;
	...
	// files.
	virtual EnvOptions
	OptimizeForManifestRead(const EnvOptions &env_options) const;
	...
	virtual EnvOptions
	OptimizeForManifestWrite(const EnvOptions &env_options) const;
...
class WritableFile {
    public:
	WritableFile()
		: last_preallocated_block_(0), preallocation_block_size_(0),
...
class WritableFileWrapper : public WritableFile {
	...
	{
		target_->GetPreallocationStatus(block_size,
