...
class DB {
	...
	static Status
	Open(const DBOptions &db_options, const std::string &name,
	     const std::vector<ColumnFamilyDescriptor> &column_families,
	     std::vector<ColumnFamilyHandle *> *handles, DB **dbptr);
	...
	static Status
	ListColumnFamilies(const DBOptions &db_options, const std::string &name,
	...
	virtual Status
	DestroyColumnFamilyHandle(ColumnFamilyHandle *column_family);
	...
	virtual bool KeyMayExist(const ReadOptions &options, const Slice &key,
				 ...
				 bool *value_found = nullptr)
	{
		return KeyMayExist(options, DefaultColumnFamily(), key, value,
				   value_found);
	}
	...
	virtual void GetApproximateMemTableStats(const Range &range,
						 ...
						 uint64_t *const size)
	{
		GetApproximateMemTableStats(DefaultColumnFamily(), range, count,
					    size);
	}

	// Deprecated versions of GetApproximateSizes
	ROCKSDB_DEPRECATED_FUNC virtual void
			    ...
			    bool include_memtable)
	{
		...
		GetApproximateSizes(column_family, range, n, sizes,
				    include_flags);
	}
	...
	ROCKSDB_DEPRECATED_FUNC virtual Status
	...
	virtual Status
	SetOptions(ColumnFamilyHandle * /*column_family*/,
	...
	virtual Status
	SetDBOptions(const std::unordered_map<std::string, std::string>
			     &new_options) = 0;
	...
	//
	virtual Status
	EnableAutoCompaction(const std::vector<ColumnFamilyHandle *>
...
#undef DeleteFile
	...
	// and end key
	virtual void
	GetLiveFilesMetaData(std::vector<LiveFileMetaData> * /*metadata*/)
	{
	}
	...
	virtual Status
	IngestExternalFile(const std::vector<std::string> &external_files,
	...
	ROCKSDB_DEPRECATED_FUNC virtual Status
	AddFile(const std::vector<std::string> &file_path_list,
		bool move_file = false, bool skip_snapshot_check = false)
	{
		IngestExternalFileOptions ifo;
		ifo.move_files = move_file;
		...
		ifo.allow_global_seqno = false;
		ifo.allow_blocking_flush = false;
		return IngestExternalFile(DefaultColumnFamily(), file_path_list,
					  ifo);
	}
		...
		bool skip_snapshot_check = false)
	{
		...
		ifo.allow_global_seqno = false;
		ifo.allow_blocking_flush = false;
		return IngestExternalFile(DefaultColumnFamily(), { file_path },
					  ifo);
	}
	...
	ROCKSDB_DEPRECATED_FUNC virtual Status
	AddFile(const std::vector<ExternalSstFileInfo> &file_info_list,
		bool move_file = false, bool skip_snapshot_check = false)
	{
		std::vector<std::string> external_files;
		for (const ExternalSstFileInfo &file_info : file_info_list) {
		...
		bool skip_snapshot_check = false)
	{
		...
		ifo.allow_global_seqno = false;
		ifo.allow_blocking_flush = false;
		return IngestExternalFile(column_family,
					  { file_info->file_path }, ifo);
	}
	...
	virtual Status
	GetPropertiesOfAllTables(TablePropertiesCollection *props)
	{
