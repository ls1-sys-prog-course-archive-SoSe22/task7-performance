		  ...
		  const OffsetGap &blacklist)
{
	...
	for (char *ptr = start_ptr + offset; ptr < start_ptr + total_size;
	     ptr++) {
		if (*ptr == kSpecialChar) {
			total_unset_bytes_base++;
		}
...
TEST_F(OptionsSettableTest, BlockBasedTableOptionsAllFieldsSettable)
{
	...
	FillWithSpecialChar(bbto_ptr, sizeof(BlockBasedTableOptions),
			    kBbtoBlacklist);
	...
	*bbto = BlockBasedTableOptions();
	int unset_bytes_base = NumUnsetBytes(
		bbto_ptr, sizeof(BlockBasedTableOptions), kBbtoBlacklist);
...
TEST_F(OptionsSettableTest, DBOptionsAllFieldsSettable)
{
	const OffsetGap kDBOptionsBlacklist = {
		...
		{ offsetof(struct DBOptions, info_log),
		  sizeof(std::shared_ptr<Logger>) },
		{ offsetof(struct DBOptions, statistics),
		  sizeof(std::shared_ptr<Statistics>) },
		{ offsetof(struct DBOptions, db_paths),
		...
		{ offsetof(struct DBOptions, row_cache),
		  sizeof(std::shared_ptr<Cache>) },
		{ offsetof(struct DBOptions, wal_filter),
	...
	FillWithSpecialChar(options_ptr, sizeof(DBOptions),
			    kDBOptionsBlacklist);
	// It based on the behavior of compiler that padding bytes are not changed
	// when copying the struct. It's prone to failure when compiler behavior
	// changes. We verify there is unset bytes to detect the case.
	*options = DBOptions();
	int unset_bytes_base = NumUnsetBytes(options_ptr, sizeof(DBOptions),
					     kDBOptionsBlacklist);
	ASSERT_GT(unset_bytes_base, 0);
	options->~DBOptions();
	...
	FillWithSpecialChar(new_options_ptr, sizeof(DBOptions),
			    kDBOptionsBlacklist);
	...
	ASSERT_EQ(unset_bytes_base,
		  NumUnsetBytes(new_options_ptr, sizeof(DBOptions),
				kDBOptionsBlacklist));
...
TEST_F(OptionsSettableTest, ColumnFamilyOptionsAllFieldsSettable)
{
	...
	const OffsetGap kColumnFamilyOptionsBlacklist = {
		{ offset_of(&ColumnFamilyOptions::inplace_callback),
		  sizeof(UpdateStatus(*)(char *, uint32_t *, Slice,
					 std::string *)) },
