...
ImmutableCFOptions::ImmutableCFOptions(const ImmutableDBOptions &db_options,
	...
	: compaction_style(cf_options.compaction_style),
	  ...
	  statistics(db_options.statistics.get()), env(db_options.env),
	  ...
	  listeners(db_options.listeners), row_cache(db_options.row_cache),
					     ...
					     CompactionStyle compaction_style)
{
	...
	for (int i = 0; i < num_levels; ++i) {
		if (i == 0 && compaction_style == kCompactionStyleUniversal) {
			...
			max_file_size[i] = MultiplyCheckOverflow(
				max_file_size[i - 1],
