			     ...
			     const std::string &file_name, Env *env)
{
	...
	for (size_t i = 0; i < cf_opts.size(); ++i) {
		...
		s = GetStringFromColumnFamilyOptions(&options_file_content,
						     cf_opts[i], "\n  ");
		if (!s.ok()) {
			writable->Close();
			return s;
					  ...
					  const int line_num)
{
	*section = kOptionSectionUnknown;
	...
	if (arg_start_pos != std::string::npos &&
	    arg_start_pos != arg_end_pos) {
		*title = TrimAndRemoveComment(line.substr(1, arg_start_pos - 1),
					      true);
		*argument = UnescapeOptionString(line.substr(
			arg_start_pos + 1, arg_end_pos - arg_start_pos - 1));
	} else {
		*title = TrimAndRemoveComment(line.substr(1, line.size() - 2),
					      true);
		*argument = "";
	}
	for (int i = 0; i < kOptionSectionUnknown; ++i) {
		if (title->find(opt_section_titles[i]) == 0) {
			if (i == kOptionSectionVersion ||
			    i == kOptionSectionDBOptions ||
			    i == kOptionSectionCFOptions) {
				if (title->size() ==
				    opt_section_titles[i].size()) {
					// if true, then it indicats equal
					*section =
						static_cast<OptionSection>(i);
					return CheckSection(*section, *argument,
							    line_num);
				}
...
{
	return Status::InvalidArgument("[RocksDBOptionsParser Error] ",
				       message + " (at line " +
					       ToString(line_num) + ")");
}
					    ...
					    const int line_num)
{
	...
	*value = TrimAndRemoveComment(
		line.substr(eq_pos + 1, line.size() - eq_pos - 1));
	if (name->empty()) {
		 ...
		 std::string *output, bool *has_data, Status *result)
{
	...
	while (!has_complete_line) {
		...
		if (!has_complete_line) {
			...
			if (*has_data) {
				*result = seq_file->Read(kBufferSize,
							 &input_slice, buffer);
			}
				   ...
				   bool ignore_unknown_options)
{
	...
	for (int line_num = 1;
		...
		if (IsSection(line)) {
			...
			s = ParseSection(&section, &title, &argument, line,
					 line_num);
			if (!s.ok()) {
				return s;
			}
						...
						int *version)
{
	...
	for (size_t i = 0; i < ver_string.size(); ++i) {
		if (ver_string[i] == '.') {
			...
			current_number =
				current_number * 10 + (ver_string[i] - '0');
			current_digit_count++;
		} else {
			snprintf(
				buffer, sizeof(buffer) - 1,
	...
	bool ignore_unknown_options)
{
	...
	if (section == kOptionSectionDBOptions) {
		...
		s = GetColumnFamilyOptionsFromMap(ColumnFamilyOptions(),
		...
		for (const auto pair : opt_map) {
			if (pair.first == "rocksdb_version") {
				s = ParseVersionNumber(pair.first, pair.second,
						       3, db_version);
				if (!s.ok()) {
					return s;
				}
		     ...
		     const std::unordered_map<std::string, std::string> *opt_map)
{
	...
	case OptionType::kDouble:
		return AreEqualDoubles(
			*reinterpret_cast<const double *>(offset1),
	...
	case OptionType::kAccessHint:
		return (*reinterpret_cast<const DBOptions::AccessHint *>(
				offset1) ==
	...
	default:
		if (type_info.verification == OptionVerificationType::kByName ||
			...
			std::string value1;
			bool result = SerializeSingleOptionHelper(
				offset1, type_info.type, &value1);
			if (result == false) {
				return false;
			}
			if (opt_map == nullptr) {
	...
	OptionsSanityCheckLevel sanity_check_level)
{
	for (auto pair : db_options_type_info) {
		...
		if (DBOptionSanityCheckLevel(pair.first) <=
		    sanity_check_level) {
			if (!AreEqualOptions(
				    reinterpret_cast<const char *>(&base_opt),
	...
	OptionsSanityCheckLevel sanity_check_level)
{
	for (auto &pair : cf_options_type_info) {
		...
		if (CFOptionSanityCheckLevel(pair.first) <=
		    sanity_check_level) {
			if (!AreEqualOptions(
				    reinterpret_cast<const char *>(&base_opt),
				    reinterpret_cast<const char *>(
	...
	OptionsSanityCheckLevel sanity_check_level)
{
	...
	for (auto &pair : block_based_table_type_info) {
		...
		if (BBTOptionSanityCheckLevel(pair.first) <=
		    sanity_check_level) {
			if (!AreEqualOptions(
				    reinterpret_cast<const char *>(&base_opt),
