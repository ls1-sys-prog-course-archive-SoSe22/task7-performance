...
void ImmutableDBOptions::Dump(Logger *log) const
{
	...
	ROCKS_LOG_HEADER(log,
			 ...
			 rate_limiter.get());
	Header(log, "    Options.sst_file_manager.rate_bytes_per_sec: %" PRIi64,
	       sst_file_manager ?
			     sst_file_manager->GetDeleteRateBytesPerSecond() :
			     0);
	ROCKS_LOG_HEADER(
		log,
...
MutableDBOptions::MutableDBOptions()
	: max_background_jobs(2), base_background_compactions(-1),
	  max_background_compactions(-1), avoid_flush_during_shutdown(false),
	  delayed_write_rate(2 * 1024U * 1024U), max_total_wal_size(0),
	  delete_obsolete_files_period_micros(6ULL * 60 * 60 * 1000000),
	  stats_dump_period_sec(600), max_open_files(-1)
{
}
