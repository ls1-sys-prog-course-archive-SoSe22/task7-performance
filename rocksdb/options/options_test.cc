...
TEST_F(OptionsTest, GetOptionsFromMapTest)
{
	...
	ASSERT_EQ(new_cf_opt.compression_per_level[8],
		  kZSTDNotFinalCompression);
	...
	ASSERT_EQ(new_cf_opt.max_bytes_for_level_multiplier_additional.size(),
		  3U);
	...
	ASSERT_NOK(GetColumnFamilyOptionsFromMap(base_cf_opt, cf_options_map,
						 &new_cf_opt));
	ASSERT_OK(
		RocksDBOptionsParser::VerifyCFOptions(base_cf_opt, new_cf_opt));
	...
	ASSERT_OK(
		GetDBOptionsFromMap(base_db_opt, db_options_map, &new_db_opt));
	...
	ASSERT_NOK(
		GetDBOptionsFromMap(base_db_opt, db_options_map, &new_db_opt));
	ASSERT_OK(
		RocksDBOptionsParser::VerifyDBOptions(base_db_opt, new_db_opt));
...
TEST_F(OptionsTest, GetColumnFamilyOptionsFromStringTest)
{
	...
	ASSERT_OK(
		GetColumnFamilyOptionsFromString(base_cf_opt, "", &new_cf_opt));
	ASSERT_OK(GetColumnFamilyOptionsFromString(
		base_cf_opt, "write_buffer_size=5", &new_cf_opt));
	...
	// Units (t)
	ASSERT_OK(GetColumnFamilyOptionsFromString(
		base_cf_opt, "write_buffer_size=20t;arena_block_size=21T",
		&new_cf_opt));
...
TEST_F(OptionsTest, GetMemTableRepFactoryFromString)
{
	...
	ASSERT_OK(
		GetMemTableRepFactoryFromString("skip_list", &new_mem_factory));
	ASSERT_OK(GetMemTableRepFactoryFromString("skip_list:16",
						  &new_mem_factory));
	...
	ASSERT_OK(GetMemTableRepFactoryFromString("prefix_hash",
						  &new_mem_factory));
	ASSERT_OK(GetMemTableRepFactoryFromString("prefix_hash:1000",
						  &new_mem_factory));
	ASSERT_EQ(std::string(new_mem_factory->Name()),
	...
	ASSERT_OK(GetMemTableRepFactoryFromString("vector:1024",
						  &new_mem_factory));
	...
	ASSERT_OK(GetMemTableRepFactoryFromString("cuckoo:1024",
						  &new_mem_factory));
	ASSERT_EQ(std::string(new_mem_factory->Name()), "HashCuckooRepFactory");

	ASSERT_NOK(GetMemTableRepFactoryFromString("bad_factory",
						   &new_mem_factory));
}
#endif // !ROCKSDB_LITE
...
TEST_F(OptionsTest, GetOptionsFromStringTest)
{
	...
	ASSERT_EQ(new_options.bottommost_compression,
	...
	ASSERT_TRUE(
		new_block_based_table_options.cache_index_and_filter_blocks);
...
TEST_F(OptionsTest, DBOptionsSerialization)
{
	...
	ASSERT_OK(GetStringFromDBOptions(&base_options_file_content,
					 base_options));
	...
	ASSERT_OK(GetDBOptionsFromString(DBOptions(), base_options_file_content,
					 &new_options));
	ASSERT_OK(RocksDBOptionsParser::VerifyDBOptions(base_options,
							new_options));
}
...
TEST_F(OptionsTest, ColumnFamilyOptionsSerialization)
{
	...
	std::string base_options_file_content;
	ASSERT_OK(GetStringFromColumnFamilyOptions(&base_options_file_content,
						   base_opt));
	...
	for (std::string base : bases) {
		for (int rand_seed = 301; rand_seed < 401; rand_seed++) {
			...
			for (int attempt = 0; attempt < 10; attempt++) {
				...
				size_t pos = static_cast<size_t>(rnd.Uniform(
...
TEST_F(OptionsTest, GetStringFromCompressionType)
{
	...
	ASSERT_OK(
		GetStringFromCompressionType(&res, kDisableCompressionOption));
	...
	ASSERT_NOK(GetStringFromCompressionType(
		&res, static_cast<CompressionType>(-10)));
}
#endif // !ROCKSDB_LITE
...
TEST_F(OptionsTest, ConvertOptionsTest)
{
	...
	ASSERT_EQ(converted_opt.create_if_missing,
		  leveldb_opt.create_if_missing);
	...
	ASSERT_EQ(converted_opt.write_buffer_size,
		  leveldb_opt.write_buffer_size);
...
TEST_F(OptionsParserTest, Comment)
{
	...
	ASSERT_OK(RocksDBOptionsParser::VerifyDBOptions(*parser.db_opt(),
							db_opt));
...
TEST_F(OptionsParserTest, ParseVersion)
{
	...
	for (auto iv : invalid_versions) {
		snprintf(buffer, kLength - 1, file_template.c_str(),
			 iv.c_str());
...
TEST_F(OptionsParserTest, DumpAndParse)
{
	...
	for (int c = 0; c < num_cf; ++c) {
		...
		if (c < 4) {
			cf_opt.prefix_extractor.reset(
				test::RandomSliceTransform(&rnd, c));
		}
		if (c < 3) {
			cf_opt.table_factory.reset(
				test::RandomTableFactory(&rnd, c));
		}
	...
	ASSERT_OK(RocksDBOptionsParser::VerifyRocksDBOptionsFromFile(
		base_db_opt, cf_names, base_cf_opts, kOptionsFileName,
		env_.get()));
...
TEST_F(OptionsParserTest, DifferentDefault)
{
...

	{
		...
		ASSERT_EQ(10 * 1048576,
			  old_default_opts.max_bytes_for_level_base);
		ASSERT_EQ(5000, old_default_opts.max_open_files);
		ASSERT_EQ(2 * 1024U * 1024U,
			  old_default_opts.delayed_write_rate);
	...
	}
	{
		...
		ASSERT_EQ(2 * 1048576,
			  old_default_cf_opts.target_file_size_base);
		ASSERT_EQ(4 << 20, old_default_cf_opts.write_buffer_size);
		ASSERT_EQ(2 * 1048576,
			  old_default_cf_opts.target_file_size_base);
		ASSERT_EQ(0, old_default_cf_opts
...
class OptionsSanityCheckTest : public OptionsParserTest {
				    ...
				    OptionsSanityCheckLevel level)
	{
		return RocksDBOptionsParser::VerifyRocksDBOptionsFromFile(
			DBOptions(), { "default" }, { cf_opts },
			kOptionsFileName, env_.get(), level);
	}
	...
	Status PersistCFOptions(const ColumnFamilyOptions &cf_opts)
	{
		...
		return PersistRocksDBOptions(DBOptions(), { "default" },
					     { cf_opts }, kOptionsFileName,
					     env_.get());
	}
...
TEST_F(OptionsSanityCheckTest, SanityCheck)
{
	...
	// prefix_extractor
	{
		...
		ASSERT_OK(SanityCheckCFOptions(opts,
	...
	// table_factory
	{
		for (int tb = 0; tb <= 2; ++tb) {
			...
			opts.table_factory.reset(
				test::RandomTableFactory(&rnd, tb));
			ASSERT_NOK(SanityCheckCFOptions(
				opts, kSanityLevelLooselyCompatible));
			ASSERT_OK(SanityCheckCFOptions(opts, kSanityLevelNone));
	...
	// merge_operator
	{
		for (int test = 0; test < 5; ++test) {
			...
			opts.merge_operator.reset(
				test::RandomMergeOperator(&rnd));
			ASSERT_NOK(SanityCheckCFOptions(
				opts, kSanityLevelLooselyCompatible));
			ASSERT_OK(SanityCheckCFOptions(opts, kSanityLevelNone));
	...
	// compaction_filter
	{
		for (int test = 0; test < 5; ++test) {
			...
			opts.compaction_filter =
				test::RandomCompactionFilter(&rnd);
			ASSERT_NOK(SanityCheckCFOptions(
				opts, kSanityLevelExactMatch));
...
TEST_F(OptionsParserTest, EscapeOptionString)
{
	...
	ASSERT_EQ(
		RocksDBOptionsParser::TrimAndRemoveComment(
