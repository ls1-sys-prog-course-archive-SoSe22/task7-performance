...
class PosixEnv : public Env {
					 ...
					 const EnvOptions &options) override
	{
		...
		if (options.use_direct_reads && !options.use_mmap_reads) {
			...
			if (fcntl(fd, F_NOCACHE, 1) == -1) {
				close(fd);
				return IOError("While fcntl NoCache", fname,
					       errno);
			}
#endif
		} else {
			...
			if (file == nullptr) {
				close(fd);
				return IOError(
					"While opening file for sequentially read",
					fname, errno);
			}
		}
		result->reset(
			new PosixSequentialFile(fname, file, fd, options));
		return Status::OK();
	}
					   ...
					   const EnvOptions &options) override
	{
...
#if !defined(OS_MACOSX) && !defined(OS_OPENBSD) && !defined(OS_SOLARIS)
			flags |= O_DIRECT;
			TEST_SYNC_POINT_CALLBACK("NewRandomAccessFile:O_DIRECT",
						 &flags);
#endif
		}
		...
		if (fd < 0) {
			return IOError("While open a file for random read",
				       fname, errno);
		}
		SetFD_CLOEXEC(fd, &options);
		...
		if (options.use_mmap_reads && sizeof(void *) >= 8) {
			...
			if (s.ok()) {
				...
				if (base != MAP_FAILED) {
					...
					s = IOError("while mmap file for read",
						    fname, errno);
				}
			}
			close(fd);
			...
			result->reset(
				new PosixRandomAccessFile(fname, fd, options));
		}
		return s;
	}
					...
					bool reopen = false)
	{
		...
		int fd = -1;
		int flags =
			(reopen) ? (O_CREAT | O_APPEND) : (O_CREAT | O_TRUNC);
		...
		if (options.use_direct_writes && !options.use_mmap_writes) {
			...
			flags |= O_DIRECT;
#endif
			TEST_SYNC_POINT_CALLBACK("NewWritableFile:O_DIRECT",
						 &flags);
		...
		if (fd < 0) {
			s = IOError("While open a file for appending", fname,
				    errno);
			return s;
		}
		SetFD_CLOEXEC(fd, &options);
		...
		if (options.use_mmap_writes && !forceMmapOff_) {
			...
			if (fcntl(fd, F_NOCACHE, 1) == -1) {
				close(fd);
				s = IOError(
					"While fcntl NoCache an opened file for appending",
					fname, errno);
				return s;
			}
			...
			if (directio(fd, DIRECTIO_ON) == -1) {
				if (errno !=
				    ENOTTY) { // ZFS filesystems don't support DIRECTIO_ON
					close(fd);
					s = IOError("While calling directio()",
						    fname, errno);
					return s;
				}
			}
#endif
			result->reset(
				new PosixWritableFile(fname, fd, options));
		} else {
			// disable mmap writes
			EnvOptions no_mmap_writes_options = options;
			no_mmap_writes_options.use_mmap_writes = false;
			result->reset(new PosixWritableFile(
				fname, fd, no_mmap_writes_options));
		}
		return s;
	}
					 ...
					 const EnvOptions &options) override
	{
		...
		if (fd < 0) {
			s = IOError("while reopen file for write", fname,
				    errno);
			return s;
		}
		...
		if (rename(old_fname.c_str(), fname.c_str()) != 0) {
			s = IOError("while rename file to " + fname, old_fname,
				    errno);
			close(fd);
			return s;
		}
		...
		if (options.use_mmap_writes && !forceMmapOff_) {
			...
			if (fcntl(fd, F_NOCACHE, 1) == -1) {
				close(fd);
				s = IOError(
					"while fcntl NoCache for reopened file for append",
					fname, errno);
				return s;
			}
#elif defined(OS_SOLARIS)
				       ...
				       const EnvOptions &options) override
	{
		...
		while (fd < 0) {
			...
			if (fd < 0) {
				...
				return IOError(
					"While open file for random read/write",
					fname, errno);
			}
		}
	...
	virtual Status FileExists(const std::string &fname) override
	{
		...
		default:
			...
			return Status::IOError(
				"Unexpected error(" + ToString(result) +
	...
	virtual Status CreateDirIfMissing(const std::string &name) override
	{
		...
		if (mkdir(name.c_str(), 0755) != 0) {
			if (errno != EEXIST) {
				result = IOError("While mkdir if missing", name,
						 errno);
			} else if (!DirExists(
					   name)) { // Check that name is actually a
				// directory.
				  ...
				  const std::string &target) override
	{
		...
		if (rename(src.c_str(), target.c_str()) != 0) {
			result = IOError("While renaming a file to " + target,
					 src, errno);
		}
		return result;
	}
				...
				const std::string &target) override
	{
		...
		if (link(src.c_str(), target.c_str()) != 0) {
			...
			result = IOError("while link file to " + target, src,
					 errno);
		}
		return result;
	}
				...
				FileLock **lock) override
	{
		*lock = nullptr;
		...
		if (fd < 0) {
			result = IOError("while open a file for lock", fname,
					 errno);
	...
	virtual Status UnlockFile(FileLock *lock) override
	{
		PosixFileLock *my_lock =
			reinterpret_cast<PosixFileLock *>(lock);
		Status result;
		if (LockOrUnlock(my_lock->filename, my_lock->fd_, false) ==
		    -1) {
	...
	virtual Status GetTestDirectory(std::string *result) override
	{
		...
		if (env && env[0] != '\0') {
			*result = env;
			...
			char buf[100];
			snprintf(buf, sizeof(buf), "/tmp/rocksdbtest-%d",
				 int(geteuid()));
			*result = buf;
		}
				 ...
				 shared_ptr<Logger> *result) override
	{
		...
		if (f == nullptr) {
			result->reset();
			return IOError("when fopen a file for new logger",
				       fname, errno);
		} else {
			int fd = fileno(f);
			...
			SetFD_CLOEXEC(fd, nullptr);
			result->reset(
				new PosixLogger(f, &PosixEnv::gettid, this));
			return Status::OK();
		}
	}
	...
	// Allow increasing the number of worker threads.
	virtual void IncBackgroundThreadsIfNeeded(int num,
	...
	EnvOptions
	OptimizeForLogWrite(const EnvOptions &env_options,
	...
	EnvOptions
	OptimizeForManifestWrite(const EnvOptions &env_options) const override
	{
		EnvOptions optimized = env_options;
		optimized.use_mmap_writes = false;
		optimized.use_direct_writes = false;
...
PosixEnv::PosixEnv()
	: checkedDiskForMmap_(false), forceMmapOff_(false),
	  page_size_(getpagesize()), thread_pools_(Priority::TOTAL)
{
	ThreadPoolImpl::PthreadCall("mutex_init",
...
void PosixEnv::StartThread(void (*function)(void *arg), void *arg)
{
	...
	state->arg = arg;
	ThreadPoolImpl::PthreadCall("start thread",
