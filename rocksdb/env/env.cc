				      ...
				      std::vector<FileAttributes> *result)
{
	...
	for (size_t i = 0; i < child_fnames.size(); ++i) {
		...
		if (!(s = GetFileSize(path, &(*result)[result_size].size_bytes))
			     .ok()) {
...
{
	if (info_log &&
	    info_log->GetInfoLogLevel() <= InfoLogLevel::INFO_LEVEL) {
		va_list ap;
		va_start(ap, format);
...
{
	if (info_log &&
	    info_log->GetInfoLogLevel() <= InfoLogLevel::DEBUG_LEVEL) {
		va_list ap;
		va_start(ap, format);
...
{
	if (info_log &&
	    info_log->GetInfoLogLevel() <= InfoLogLevel::WARN_LEVEL) {
		va_list ap;
		va_start(ap, format);
...
{
	if (info_log &&
	    info_log->GetInfoLogLevel() <= InfoLogLevel::ERROR_LEVEL) {
		va_list ap;
		va_start(ap, format);
...
{
	if (info_log &&
	    info_log->GetInfoLogLevel() <= InfoLogLevel::FATAL_LEVEL) {
		va_list ap;
		va_start(ap, format);
...
EnvOptions
Env::OptimizeForCompactionTableWrite(const EnvOptions &env_options,
...
EnvOptions
Env::OptimizeForCompactionTableRead(const EnvOptions &env_options,
				    const ImmutableDBOptions &db_options) const
{
	EnvOptions optimized_env_options(env_options);
