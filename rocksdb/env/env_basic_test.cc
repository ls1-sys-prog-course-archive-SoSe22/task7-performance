...
class NormalizingEnvWrapper : public EnvWrapper {
	...
	{
		Status status =
			EnvWrapper::GetChildrenFileAttributes(dir, result);
		if (status.ok()) {
			result->erase(
...
std::vector<Env *> GetCustomEnvs()
{
	...
	if (!init) {
		...
		if (uri != nullptr) {
			custom_env =
				NewCustomObject<Env>(uri, &custom_env_guard);
		}
	}
...
TEST_P(EnvBasicTestWithParam, Basics)
{
	...
	ASSERT_TRUE(!env_->GetFileSize(test_dir_ + "/non_existent", &file_size)
			     .ok());
	ASSERT_OK(env_->GetChildren(test_dir_, &children));
	ASSERT_EQ(0U, children.size());

	// Create a file.
	ASSERT_OK(env_->NewWritableFile(test_dir_ + "/f", &writable_file,
					soptions_));
	...
	// Write to the file.
	ASSERT_OK(env_->NewWritableFile(test_dir_ + "/f1", &writable_file,
					soptions_));
	ASSERT_OK(writable_file->Append("abc"));
	ASSERT_OK(writable_file->Close());
	writable_file.reset();
	ASSERT_OK(env_->NewWritableFile(test_dir_ + "/f2", &writable_file,
					soptions_));
	ASSERT_OK(writable_file->Close());
	writable_file.reset();
	...
	ASSERT_TRUE(
		!env_->RenameFile(test_dir_ + "/non_existent", test_dir_ + "/g")
			 .ok());
	...
	ASSERT_TRUE(!env_->NewSequentialFile(test_dir_ + "/non_existent",
					     &seq_file, soptions_)
			     .ok());
	...
	ASSERT_OK(env_->GetChildren(test_dir_, &children));
	ASSERT_EQ(0U, children.size());
	ASSERT_TRUE(env_->GetChildren(test_dir_ + "/non_existent", &children)
			    .IsNotFound());
}
...
TEST_P(EnvBasicTestWithParam, ReadWrite)
{
	...
	// Read sequentially.
	ASSERT_OK(env_->NewSequentialFile(test_dir_ + "/f", &seq_file,
					  soptions_));
	...
	ASSERT_OK(seq_file->Read(1000, &result,
				 scratch)); // Try reading past EOF.
	...
	// Random reads.
	ASSERT_OK(env_->NewRandomAccessFile(test_dir_ + "/f", &rand_file,
					    soptions_));
...
{
	unique_ptr<WritableFile> writable_file;
	ASSERT_OK(env_->NewWritableFile(test_dir_ + "/b", &writable_file,
					soptions_));
...
TEST_P(EnvMoreTestWithParam, GetChildren)
{
	...
	ASSERT_TRUE(
		!env_->GetChildrenFileAttributes(test_dir_, &childAttr).ok());

	// if dir is a file, returns IOError
	ASSERT_OK(env_->CreateDir(test_dir_));
	unique_ptr<WritableFile> writable_file;
	ASSERT_OK(env_->NewWritableFile(test_dir_ + "/file", &writable_file,
					soptions_));
	ASSERT_OK(writable_file->Close());
	writable_file.reset();
