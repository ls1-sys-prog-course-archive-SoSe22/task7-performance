...
class ChrootEnv : public EnvWrapper {
		...
		: EnvWrapper(base_env)
	{
		...
		char *real_chroot_dir =
			realpath(chroot_dir.c_str(), resolvedName);
#else
			  ...
			  const EnvOptions &options) override
	{
		...
		return EnvWrapper::NewSequentialFile(status_and_enc_path.second,
						     result, options);
	}
					   ...
					   const EnvOptions &options) override
	{
		...
		return EnvWrapper::NewRandomAccessFile(
			status_and_enc_path.second, result, options);
	}
				       ...
				       const EnvOptions &options) override
	{
		...
		return EnvWrapper::NewWritableFile(status_and_enc_path.second,
						   result, options);
	}
					 ...
					 const EnvOptions &options) override
	{
		...
		return EnvWrapper::ReuseWritableFile(
			status_and_old_enc_path.second,
			status_and_old_enc_path.second, result, options);
	}
				       ...
				       const EnvOptions &options) override
	{
		...
		return EnvWrapper::NewRandomRWFile(status_and_enc_path.second,
						   result, options);
	}
				    ...
				    unique_ptr<Directory> *result) override
	{
		...
		return EnvWrapper::NewDirectory(status_and_enc_path.second,
						result);
	}
				   ...
				   std::vector<std::string> *result) override
	{
		...
		return EnvWrapper::GetChildren(status_and_enc_path.second,
					       result);
	}
				  ...
				  std::vector<FileAttributes> *result) override
	{
		...
		return EnvWrapper::GetChildrenFileAttributes(
			status_and_enc_path.second, result);
	}
	...
	virtual Status CreateDirIfMissing(const std::string &dirname) override
	{
		...
		return EnvWrapper::CreateDirIfMissing(
			status_and_enc_path.second);
	}
				   ...
				   uint64_t *file_size) override
	{
		...
		return EnvWrapper::GetFileSize(status_and_enc_path.second,
					       file_size);
	}
					       ...
					       uint64_t *file_mtime) override
	{
		...
		return EnvWrapper::GetFileModificationTime(
			status_and_enc_path.second, file_mtime);
	}
	...
	virtual Status GetTestDirectory(std::string *path) override
	{
		...
		char buf[256];
		snprintf(buf, sizeof(buf), "/rocksdbtest-%d",
				 ...
				 shared_ptr<Logger> *result) override
	{
		...
		return EnvWrapper::NewLogger(status_and_enc_path.second,
					     result);
	}
				       ...
				       std::string *output_path) override
	{
		...
		return EnvWrapper::GetAbsolutePath(status_and_enc_path.second,
						   output_path);
	}
	...
	EncodePathWithNewBasename(const std::string &path)
	{
		...
		auto status_and_enc_path =
			EncodePath(path.substr(0, base_sep + 1));
