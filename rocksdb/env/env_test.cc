...
std::unique_ptr<char, Deleter> NewAligned(const size_t size, const char ch)
{
	char *ptr = nullptr;
#ifdef OS_WIN
	if (!(ptr = reinterpret_cast<char *>(
		      _aligned_malloc(size, kPageSize)))) {
		return std::unique_ptr<char, Deleter>(nullptr,
						      Deleter(_aligned_free));
	}
	std::unique_ptr<char, Deleter> uptr(ptr, Deleter(_aligned_free));
#else
	if (posix_memalign(reinterpret_cast<void **>(&ptr), kPageSize, size) !=
	    0) {
...
TEST_P(EnvPosixTestWithParam, UnSchedule)
{
	...
	env_->Schedule(&test::SleepingBackgroundTask::DoSleepTask,
		       &sleeping_task1, Env::Priority::LOW, &sleeping_task1);
	...
	ASSERT_TRUE(!sleeping_task.IsSleeping() &&
		    !sleeping_task1.IsSleeping());
	WaitThreadPoolsEmpty();
}
...
TEST_P(EnvPosixTestWithParam, TwoPools)
{
	...
	class CB {
		...
		CB(const std::string &pool_name, int pool_size,
		   port::Mutex *trigger_mu, port::CondVar *trigger_cv,
		   bool *_should_start)
			: mu_(), num_running_(0), num_finished_(0),
			  pool_size_(pool_size), pool_name_(pool_name),
			  trigger_mu_(trigger_mu), trigger_cv_(trigger_cv),
	...
	env_->IncBackgroundThreadsIfNeeded(kLowPoolSize - 1,
					   Env::Priority::LOW);
	env_->IncBackgroundThreadsIfNeeded(kHighPoolSize + 1,
					   Env::Priority::HIGH);
...
TEST_P(EnvPosixTestWithParam, DecreaseNumBgThreads)
{
	...
	env_->Schedule(&test::SleepingBackgroundTask::DoSleepTask, &tasks[9],
		       Env::Priority::HIGH);
	Env::Default()->SleepForMicroseconds(kDelayMicros);
	ASSERT_GT(env_->GetThreadPoolQueueLen(Env::Priority::HIGH),
		  (unsigned int)0);
...
#define rmdir _rmdir
		...
		dir_ = dir_buf;
#else
		std::list<std::string> candidate_dir_list = { "/var/tmp",
							      "/tmp" };
		...
		if (tmp &&
		    strlen(tmp) + strlen(fmt) - 2 + 1 <= sizeof dir_buf) {
...
TEST_F(EnvPosixTest, PositionedAppend)
{
	...
	ASSERT_OK(env_->NewWritableFile(ift.name() + "/f", &writable_file,
					options));
	...
	ASSERT_OK(
		env_->NewSequentialFile(ift.name() + "/f", &seq_file, options));
...
TEST_P(EnvPosixTestWithParam, AllocateTest)
{
	if (env_ == Env::Default()) {
		...
		std::string fname_test_fallocate =
			ift.name() + "/preallocate_testfile_2";
		int fd = -1;
		do {
			fd = open(fname_test_fallocate.c_str(),
		...
		ASSERT_GE((f_stat.st_size + kPageSize + kBlockSize - 1) /
				  kBlockSize,
	...
	if (env_ == Env::Default()) {
		...
		for (const std::string fname : fnames) {
			...
			ASSERT_OK(env_->NewRandomAccessFile(fname, &file,
							    soptions));
			size_t id_size =
				file->GetUniqueId(temp_id, MAX_ID_SIZE);
			ASSERT_TRUE(id_size > 0);
...
TEST_P(EnvPosixTestWithParam, RandomAccessUniqueIDDeletes)
{
	if (env_ == Env::Default()) {
		...
		for (int i = 0; i < 1000; ++i) {
			...
			{
				unique_ptr<RandomAccessFile> file;
				ASSERT_OK(env_->NewRandomAccessFile(
					fname, &file, soptions));
				size_t id_size =
					file->GetUniqueId(temp_id, MAX_ID_SIZE);
				ASSERT_TRUE(id_size > 0);
...
TEST_P(EnvPosixTestWithParam, InvalidateCache)
{
	...
	!defined(OS_AIX)
		...
		if (file->use_direct_io()) {
			ASSERT_OK(file->PositionedRead(0, kSectorSize, &result,
						       scratch.get()));
		} else {
			ASSERT_OK(file->Read(kSectorSize, &result,
					     scratch.get()));
		}
		ASSERT_EQ(memcmp(scratch.get(), data.get(), kSectorSize), 0);
		ASSERT_OK(file->InvalidateCache(0, 11));
...
class TestLogger : public Logger {
	...
	virtual void Logv(const char *format, va_list ap) override
	{
		...
		std::fill_n(new_format, sizeof(new_format), '2');
		{
			...
			int n = vsnprintf(new_format, sizeof(new_format) - 1,
					  format, backup_ap);
...
// When we have n == -1 there is not a terminating zero expected
			...
			if (new_format[0] == '[') {
				...
				ASSERT_TRUE(
					n <=
					56 + (512 - static_cast<int>(sizeof(
							    struct timeval))));
			} else {
				ASSERT_TRUE(
					n <=
					48 + (512 - static_cast<int>(sizeof(
							    struct timeval))));
			}
			va_end(backup_ap);
		}
...
TEST_P(EnvPosixTestWithParam, LogBufferTest)
{
	...
	ROCKS_LOG_BUFFER(&log_buffer, "x%sx%sx%sx", bytes200, bytes200,
			 bytes200);
	...
	virtual void Logv(const char *format, va_list ap) override
	{
		...
		std::fill_n(new_format, sizeof(new_format), '2');
		{
			...
			int n = vsnprintf(new_format, sizeof(new_format) - 1,
					  format, backup_ap);
			// 48 bytes for extra information + bytes allocated
			ASSERT_TRUE(n <= 48 + static_cast<int>(
						      max_log_size_ -
...
TEST_P(EnvPosixTestWithParam, LogBufferMaxSizeTest)
{
	...
	for (size_t max_log_size = 256; max_log_size <= 1024;
		...
		test_logger.SetInfoLogLevel(InfoLogLevel::INFO_LEVEL);
		LogBuffer log_buffer(InfoLogLevel::INFO_LEVEL, &test_logger);
		ROCKS_LOG_BUFFER_MAX_SZ(&log_buffer, max_log_size, "%s",
					bytes9000);
		log_buffer.FlushBufferToLog();
	}
}
...
TEST_P(EnvPosixTestWithParam, Preallocation)
{
	...
	// Write an entire preallocation block, make sure we increased by two.
	{
		...
		srcfile->Append(buf);
		srcfile->GetPreallocationStatus(&block_size,
...
TEST_P(EnvPosixTestWithParam, ConsistentChildrenAttributes)
{
	...
	ASSERT_OK(env_->GetChildrenFileAttributes(test::TmpDir(env_),
						  &file_attrs));
	for (int i = 0; i < kNumChildren; ++i) {
		std::ostringstream oss;
		oss << "testfile_" << i;
		...
		auto file_attrs_iter =
			std::find_if(file_attrs.begin(), file_attrs.end(),
...
TEST_P(EnvPosixTestWithParam, WritableFileWrapper)
{
	class Base : public WritableFile {
		...
		void
		GetPreallocationStatus(size_t *block_size,
	...
	class Wrapper : public WritableFileWrapper {
	    public:
		explicit Wrapper(WritableFile *target)
...
class RandomRWFileWithMirrorString {
    public:
	explicit RandomRWFileWithMirrorString(RandomRWFile *_file)
		: file_(_file)
	{
	}
	...
	void Read(size_t offset = 0, size_t n = 1000000)
	{
		...
		if (offset < file_mirror_.size()) {
			size_t str_res_sz =
				std::min(file_mirror_.size() - offset, n);
			str_res =
				Slice(file_mirror_.data() + offset, str_res_sz);
		...
		ASSERT_EQ(str_res.ToString(), file_res.ToString())
			<< offset << " " << n;
	}

	void SetFile(RandomRWFile *_file)
	{
		file_ = _file;
	}
...
TEST_P(EnvPosixTestWithParam, PosixRandomRWFileRandomized)
{
	...
	for (int i = 0; i < 10000; i++) {
		...
		if (i % 500 == 0) {
			...
			ASSERT_OK(env_->NewRandomRWFile(path, &file,
							EnvOptions()));
