...
class HdfsReadableFile : virtual public SequentialFile,
		...
		: fileSys_(fileSys), filename_(fname), hfile_(nullptr)
	{
		ROCKS_LOG_DEBUG(mylog,
				...
				filename_.c_str());
		hfile_ = hdfsOpenFile(fileSys_, filename_.c_str(), O_RDONLY, 0,
				      0, 0);
		ROCKS_LOG_DEBUG(
			mylog,
	...
	virtual Status Read(size_t n, Slice *result, char *scratch)
	{
		...
		while (remaining_bytes > 0) {
			bytes_read = hdfsRead(fileSys_, hfile_, buffer,
					      remaining_bytes);
			if (bytes_read <= 0) {
				break;
			}
	...
	tOffset fileSize()
	{
		ROCKS_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile fileSize %s\n",
				filename_.c_str());
		hdfsFileInfo *pFileInfo =
			hdfsGetPathInfo(fileSys_, filename_.c_str());
		tOffset size = 0L;
		if (pFileInfo != nullptr) {
			size = pFileInfo->mSize;
			hdfsFreeFileInfo(pFileInfo, 1);
		} else {
			throw HdfsFatalException("fileSize on unknown file " +
						 filename_);
		}
		return size;
	}
...
class HdfsWritableFile : public WritableFile {
		...
		: fileSys_(fileSys), filename_(fname), hfile_(nullptr)
	{
		ROCKS_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile opening %s\n",
				filename_.c_str());
		hfile_ = hdfsOpenFile(fileSys_, filename_.c_str(), O_WRONLY, 0,
				      0, 0);
...
Status HdfsEnv::NewLogger(const std::string &fname, shared_ptr<Logger> *result)
{
	HdfsWritableFile *f = new HdfsWritableFile(fileSys_, fname);
	if (f == nullptr || !f->isValid()) {
		delete f;
