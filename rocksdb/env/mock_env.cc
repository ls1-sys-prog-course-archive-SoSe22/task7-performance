...
class MemFile {
	...
	explicit MemFile(Env *env, const std::string &fn,
		...
		: env_(env), fn_(fn), refs_(0), is_lock_file_(_is_lock_file),
		  locked_(false), size_(0), modified_time_(Now()),
		  rnd_(static_cast<uint32_t>(MurmurHash(
			  fn.data(), static_cast<int>(fn.size()), 0))),
		  fsynced_bytes_(0)
	{
	}
	...
	void CorruptBuffer()
	{
		...
		uint64_t start = fsynced_bytes_ +
				 rnd_.Uniform(static_cast<int>(buffered_bytes));
	...
	virtual Status Append(const Slice &data) override
	{
		...
		while (bytes_written < data.size()) {
			...
			Status s = file_->Append(
				Slice(data.data() + bytes_written, bytes));
			if (!s.ok()) {
				return s;
			}
			bytes_written += bytes;
...
class TestMemLogger : public Logger {
	...
	TestMemLogger(std::unique_ptr<WritableFile> f, Env *env,
		...
		: Logger(log_level), file_(std::move(f)), log_size_(0),
	...
	virtual void Logv(const char *format, va_list ap) override
	{
		...
		for (int iter = 0; iter < 2; iter++) {
			...
			uint64_t now_micros =
				static_cast<uint64_t>(now_tv.tv_sec) * 1000000 +
				now_tv.tv_usec;
...
Status MockEnv::NewLogger(const std::string &fname, shared_ptr<Logger> *result)
{
	auto fn = NormalizePath(fname);
	MutexLock lock(&mutex_);
	auto iter = file_map_.find(fn);
