 ...
 */
template <class OS_TYPE> class FbsonJsonParserT {
    public:
	...
	bool parse(const char *pch, unsigned int len,
	...
	bool parseDecimal(std::istream &in, int sign)
	{
		...
		while (in.good() && !strchr(kJsonDelim, ch)) {
			if (ch >= '0' && ch <= '9') {
				...
				return parseDouble(in, static_cast<double>(val),
						   precision, sign);
			} else {
