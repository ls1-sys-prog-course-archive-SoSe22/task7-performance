...
class FbsonOutStream : public std::ostream {
	...
	explicit FbsonOutStream(uint32_t capacity = 1024)
		: std::ostream(nullptr), head_(nullptr), size_(0),
		  capacity_(capacity), alloc_(true)
	{
		if (capacity_ == 0) {
			capacity_ = 1024;
		}
	...
	FbsonOutStream(char *buffer, uint32_t capacity)
		: std::ostream(nullptr), head_(buffer), size_(0),
		  capacity_(capacity), alloc_(false)
	{
	...
	void write(int64_t l)
	{
		...
		int len = snprintf(head_ + size_, MAX_INT64_DIGITS + 1,
				   "%" PRIi64, l);
		assert(len > 0);
		size_ += len;
	}
	...
	void write(double d)
	{
		...
		int len = snprintf(head_ + size_, MAX_DOUBLE_DIGITS + 1,
				   "%.15g", d);
		assert(len > 0);
		size_ += len;
	}
