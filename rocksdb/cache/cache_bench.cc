...
class SharedState {
	...
	explicit SharedState(CacheBench *cache_bench)
		: cv_(&mu_), num_threads_(FLAGS_threads), num_initialized_(0),
		  start_(false), num_done_(0), cache_bench_(cache_bench)
	{
	}
...
class CacheBench {
	...
	CacheBench() : num_threads_(FLAGS_threads)
	{
		if (FLAGS_use_clock_cache) {
			cache_ = NewClockCache(FLAGS_cache_size,
	...
	bool Run()
	{
		...
		}
		{
			...
			uint32_t qps = static_cast<uint32_t>(
				static_cast<double>(FLAGS_threads *
