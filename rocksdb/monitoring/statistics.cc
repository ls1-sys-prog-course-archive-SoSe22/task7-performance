...
uint64_t StatisticsImpl::getTickerCountLocked(uint32_t tickerType) const
{
	...
	uint64_t res = 0;
	for (size_t core_idx = 0; core_idx < per_core_stats_.Size();
	     ++core_idx) {
		res += per_core_stats_.AccessAtCore(core_idx)
			       ->tickers_[tickerType];
	}
	return res;
}
...
StatisticsImpl::getHistogramImplLocked(uint32_t histogramType) const
{
	...
	for (size_t core_idx = 0; core_idx < per_core_stats_.Size();
	     ++core_idx) {
		res_hist->Merge(per_core_stats_.AccessAtCore(core_idx)
					->histograms_[histogramType]);
	}
	return res_hist;
}
...
void StatisticsImpl::setTickerCountLocked(uint32_t tickerType, uint64_t count)
{
	...
	for (size_t core_idx = 0; core_idx < per_core_stats_.Size();
	     ++core_idx) {
		if (core_idx == 0) {
			per_core_stats_.AccessAtCore(core_idx)
				->tickers_[tickerType] = count;
		} else {
			per_core_stats_.AccessAtCore(core_idx)
				->tickers_[tickerType] = 0;
		}
	}
}
...
uint64_t StatisticsImpl::getAndResetTickerCount(uint32_t tickerType)
{
	uint64_t sum = 0;
	{
		...
		for (size_t core_idx = 0; core_idx < per_core_stats_.Size();
		     ++core_idx) {
			sum += per_core_stats_.AccessAtCore(core_idx)
				       ->tickers_[tickerType]
	...
	for (const auto &h : HistogramsNameMap) {
		if (h.first < HISTOGRAM_ENUM_MAX || enable_internal_stats_) {
			...
			snprintf(
				...
				hData.percentile95, hData.percentile99,
				hData.max);
			res.append(buffer);
		}
	}
