...
HistogramWindowingImpl::HistogramWindowingImpl(uint64_t num_windows,
	...
	: num_windows_(num_windows), micros_per_window_(micros_per_window),
...
bool HistogramWindowingImpl::Empty() const
{
	return stats_.Empty();
}
...
void HistogramWindowingImpl::Merge(const HistogramWindowingImpl &other)
{
	...
	for (unsigned int i = 0; i < std::min(num_windows_, other.num_windows_);
		...
		uint64_t other_window_index =
			(other_cur_window + other.num_windows_ - i) %
			other.num_windows_;

		window_stats_[window_index].Merge(
			other.window_stats_[other_window_index]);
	}
}
...
void HistogramWindowingImpl::SwapHistoryBucket()
{
	...
	// swap, if needed.
	if (mutex_.try_lock()) {
		last_swap_time_.store(env_->NowMicros(),
		...
		if (!stats_to_drop.Empty()) {
			...
			if (stats_.min() == stats_to_drop.min()) {
				...
				for (unsigned int i = 0; i < num_windows_;
				     i++) {
					if (i != next_window) {
						uint64_t m =
							window_stats_[i].min();
			...
			if (stats_.max() == stats_to_drop.max()) {
				...
				stats_.max_.store(new_max,
						  std::memory_order_relaxed);
			}

			stats_.num_.fetch_sub(stats_to_drop.num(),
					      std::memory_order_relaxed);
			stats_.sum_.fetch_sub(stats_to_drop.sum(),
					      std::memory_order_relaxed);
			stats_.sum_squares_.fetch_sub(
				stats_to_drop.sum_squares(),
				std::memory_order_relaxed);
