...
struct ConstantColumnFamilyInfo {
    ...
    public:
	ConstantColumnFamilyInfo(const void *_db_key,
...
struct ThreadStatusData {
	...
	std::atomic<uint64_t>
		op_properties[ThreadStatus::kNumOperationProperties];
...
class ThreadStatusUpdater {
	...
	SetThreadOperationStage(const ThreadStatus::OperationStage stage);
	...
	std::unordered_map<const void *,
			   std::unique_ptr<ConstantColumnFamilyInfo> >
		cf_info_map_;
