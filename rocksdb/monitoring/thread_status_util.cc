				      ...
				      ThreadStatus::ThreadType thread_type)
{
	...
	thread_updater_local_cache_->RegisterThread(thread_type,
						    env->GetThreadID());
}
...
void ThreadStatusUtil::SetThreadOperation(ThreadStatus::OperationType op)
{
	...
	if (op != ThreadStatus::OP_UNKNOWN) {
		...
		thread_updater_local_cache_->SetOperationStartTime(
			current_time);
	} else {
...
ThreadStatusUtil::SetThreadOperationStage(ThreadStatus::OperationStage stage)
{
	if (thread_updater_local_cache_ == nullptr) {
		// thread_updater_local_cache_ must be set in SetColumnFamily
		// or other ThreadStatusUtil functions.
...
void ThreadStatusUtil::IncreaseThreadOperationProperty(int code, uint64_t delta)
{
	if (thread_updater_local_cache_ == nullptr) {
		// thread_updater_local_cache_ must be set in SetColumnFamily
		// or other ThreadStatusUtil functions.
	...
	thread_updater_local_cache_->IncreaseThreadOperationProperty(code,
								     delta);
}
...
{
	ThreadStatusUpdater *thread_updater =
