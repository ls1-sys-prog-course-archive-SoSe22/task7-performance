...
void HistogramStat::Merge(const HistogramStat &other)
{
	...
	for (unsigned int b = 0; b < num_buckets_; b++) {
		buckets_[b].fetch_add(other.bucket_at(b),
...
double HistogramStat::Percentile(double p) const
{
	...
	for (unsigned int b = 0; b < num_buckets_; b++) {
		...
		if (cumulative_sum >= threshold) {
			...
			double r =
				left_point + (right_point - left_point) * pos;
...
double HistogramStat::StandardDeviation() const
{
	...
	if (cur_num == 0)
		return 0;
	double variance = static_cast<double>(cur_sum_squares * cur_num -
					      cur_sum * cur_sum) /
...
std::string HistogramStat::ToString() const
{
	...
	for (unsigned int b = 0; b < num_buckets_; b++) {
		...
		size_t marks =
			static_cast<size_t>(mult * bucket_value / 5 + 0.5);
		r.append(marks, '#');
		r.push_back('\n');
	}
	return r;
