...
class StatisticsImpl : public Statistics {
	...
	virtual void setTickerCount(uint32_t ticker_type,
	...
	virtual void measureTime(uint32_t histogram_type,
	...
	static_assert(sizeof(StatisticsData) % 64 == 0,
		      "Expected 64-byte aligned");

	CoreLocalArray<StatisticsData> per_core_stats_;

	uint64_t getTickerCountLocked(uint32_t ticker_type) const;
	std::unique_ptr<HistogramImpl>
	getHistogramImplLocked(uint32_t histogram_type) const;
