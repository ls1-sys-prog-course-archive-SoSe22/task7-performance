...
void gettimeofday(struct timeval *tv, struct timezone * /* tz */)
{
	...
	microseconds usNow(duration_cast<microseconds>(
	...
	tv->tv_usec = static_cast<long>(
		usNow.count() - duration_cast<microseconds>(secNow).count());
}
...
bool CondVar::TimedWait(uint64_t abs_time_us)
{
	...
	microseconds usNow(duration_cast<microseconds>(
		system_clock::now().time_since_epoch()));
	microseconds relTimeUs = (usAbsTime > usNow) ? (usAbsTime - usNow) :
							     microseconds::zero();
...
JEMALLOC_ATTR(used) static const void(WINAPI *p_rocksdb_init_jemalloc)(void) =
	rocksdb::port::InitializeJemalloc;
