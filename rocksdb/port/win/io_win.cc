...
SSIZE_T pread(HANDLE hFile, char *src, size_t numBytes, uint64_t offset)
{
	...
	if (FALSE == ReadFile(hFile, src, static_cast<DWORD>(numBytes),
...
Status ftruncate(const std::string &filename, HANDLE hFile, uint64_t toSize)
{
	...
	if (!SetFileInformationByHandle(hFile, FileEndOfFileInfo, &end_of_file,
		...
		auto lastError = GetLastError();
		status = IOErrorFromWindowsError(
			"Failed to Set end of file: " + filename, lastError);
	}
...
WinMmapReadableFile::WinMmapReadableFile(const std::string &fileName,
					 ...
					 size_t length)
	: WinFileData(fileName, hFile, false /* use_direct_io */), hMap_(hMap),
	  mapped_region_(mapped_region), length_(length)
{
}
				 ...
				 char *scratch) const
{
	...
	*result = Slice(reinterpret_cast<const char *>(mapped_region_) + offset,
			n);
	return s;
}
...
Status WinMmapFile::MapNewRegion()
{
	...
	if (hMap_ == NULL || reserved_size_ > mapping_size_) {
		...
		hMap_ = CreateFileMappingA(
			...
			mappingSize
				.HighPart, // Enable mapping the whole file but the actual
		...
		if (NULL == hMap_) {
			return IOErrorFromWindowsError(
				"WindowsMmapFile failed to create file mapping for: " +
					filename_,
				GetLastError());
		}
	...
	mapped_begin_ = reinterpret_cast<char *>(
		MapViewOfFileEx(hMap_, FILE_MAP_WRITE, offset.HighPart,
...
WinMmapFile::WinMmapFile(const std::string &fname, HANDLE hFile,
	...
	: WinFileData(fname, hFile, false), hMap_(NULL), page_size_(page_size),
	  allocation_granularity_(allocation_granularity), reserved_size_(0),
	  mapping_size_(0), view_size_(0), mapped_begin_(nullptr),
	  mapped_end_(nullptr), dst_(nullptr), last_sync_(nullptr),
	  file_offset_(0), pending_sync_(false)
{
	...
	const size_t viewSize =
		32 *
		1024; // 32Kb similar to the Windows File Cache in buffered mode
...
Status WinMmapFile::Close()
{
	...
	if (NULL != hMap_) {
		...
		if (!ret && s.ok()) {
			...
			s = IOErrorFromWindowsError(
				"Failed to Close mapping for file: " +
					filename_,
				lastError);
		}
	...
	if (hFile_ != NULL) {
		...
		if (!ret && s.ok()) {
			auto lastError = GetLastError();
			s = IOErrorFromWindowsError(
				"Failed to close file map handle: " + filename_,
				lastError);
		}
	}
...
Status WinMmapFile::Sync()
{
	...
	if (dst_ > last_sync_) {
		...
		size_t page_begin = TruncateToPageBoundary(
			page_size_, last_sync_ - mapped_begin_);
		size_t page_end = TruncateToPageBoundary(
			page_size_, dst_ - mapped_begin_ - 1);
		...
		if (!::FlushViewOfFile(mapped_begin_ + page_begin,
			...
			s = IOErrorFromWindowsError(
				"Failed to FlushViewOfFile: " + filename_,
				GetLastError());
		} else {
			last_sync_ = dst_;
		}
...
Status WinMmapFile::Fsync()
{
	...
	if (s.ok() && pending_sync_) {
		if (!::FlushFileBuffers(hFile_)) {
			s = IOErrorFromWindowsError(
				"Failed to FlushFileBuffers: " + filename_,
				GetLastError());
		}
		pending_sync_ = false;
	}
...
Status WinSequentialFile::Read(size_t n, Slice *result, char *scratch)
{
	...
	if (n > UINT_MAX) {
		return IOErrorFromWindowsError(filename_,
					 ...
					 Slice *result, char *scratch)
{
	...
	// it is a highly unlikely case.
	if (n > UINT_MAX) {
		return IOErrorFromWindowsError(GetName(),
					       ERROR_INVALID_PARAMETER);
	}
...
Status WinSequentialFile::Skip(uint64_t n)
{
	...
	LARGE_INTEGER li;
	li.QuadPart =
		static_cast<int64_t>(n); //cast is safe due to the check above
...
inline SSIZE_T
WinRandomAccessImpl::PositionedReadInternal(char *src, size_t numBytes,
					    uint64_t offset) const
{
					    ...
					    Slice *result, char *scratch) const
{
	...
	if (file_base_->use_direct_io()) {
		if (!IsAligned(alignment_, scratch)) {
			return Status::InvalidArgument(
				"WinRandomAccessImpl::ReadImpl: scratch is not properly aligned");
		}
	}
	...
	if (r > 0) {
		...
		// of the file as OK.
		if (lastError != ERROR_HANDLE_EOF) {
			s = IOErrorFromWindowsError(file_base_->GetName(),
						    lastError);
		}
	}
...
inline WinWritableImpl::WinWritableImpl(WinFileData *file_data,
					size_t alignment)
	: file_data_(file_data), alignment_(alignment), next_write_offset_(0),
	  reservedsize_(0)
{
	...
	LARGE_INTEGER pos;
	pos.QuadPart = 0;
	BOOL ret = SetFilePointerEx(file_data_->GetFileHandle(), zero_move,
				    &pos, FILE_CURRENT);
...
inline Status WinWritableImpl::AppendImpl(const Slice &data)
{
	Status s;
...
inline Status WinWritableImpl::PositionedAppendImpl(const Slice &data,
						    uint64_t offset)
{
	if (file_data_->use_direct_io()) {
	...
	if (ret < 0) {
		auto lastError = GetLastError();
		s = IOErrorFromWindowsError("Failed to pwrite for: " +
						    file_data_->GetName(),
					    lastError);
	} else {
...
inline Status WinWritableImpl::CloseImpl()
{
	Status s;
	...
	if (!file_data_->CloseFile()) {
		auto lastError = GetLastError();
		s = IOErrorFromWindowsError("CloseHandle failed for: " +
						    file_data_->GetName(),
					    lastError);
	}
	return s;
...
inline Status WinWritableImpl::SyncImpl()
{
	Status s;
	// Calls flush buffers
...
inline Status WinWritableImpl::AllocateImpl(uint64_t offset, uint64_t len)
{
	Status status;
