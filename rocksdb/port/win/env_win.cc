...
WinEnvIO::WinEnvIO(Env *hosted_env)
	: hosted_env_(hosted_env), page_size_(4 * 1012),
	  allocation_granularity_(page_size_), perf_counter_frequency_(0),
				   ...
				   const EnvOptions &options)
{
	...
	if (INVALID_HANDLE_VALUE == hFile) {
		...
		s = IOErrorFromWindowsError(
			"Failed to open NewSequentialFile" + fname, lastError);
	} else {
				     ...
				     const EnvOptions &options)
{
	...
	if (options.use_mmap_reads && sizeof(void *) >= 8) {
		...
		if (s.ok()) {
			...
			if (fileSize == 0) {
				return IOError(
					"NewRandomAccessFile failed to map empty file: " +
						fname,
					EINVAL);
			}
			...
			if (!hMap) {
				auto lastError = GetLastError();
				return IOErrorFromWindowsError(
					"Failed to create file mapping for NewRandomAccessFile: " +
						fname,
					lastError);
			}
			...
			if (!mapped_region) {
				auto lastError = GetLastError();
				return IOErrorFromWindowsError(
					"Failed to MapViewOfFile for NewRandomAccessFile: " +
						fname,
					lastError);
			}

			result->reset(new WinMmapReadableFile(
				fname, hFile, hMap, mapped_region, fileSize));

			mapGuard.release();
			fileGuard.release();
		}
	} else {
		result->reset(new WinRandomAccessFile(fname, hFile, page_size_,
						      options));
		fileGuard.release();
	}
	return s;
...
Status WinEnvIO::OpenWritableFile(const std::string &fname,
				  ...
				  const EnvOptions &options, bool reopen)
{
	...
	if (reopen) {
		...
		if (!ret) {
			auto lastError = GetLastError();
			return IOErrorFromWindowsError(
				"Failed to create a ReopenWritableFile move to the end: " +
					fname,
				lastError);
		}
	}
				 ...
				 const EnvOptions &options)
{
	...
	if (INVALID_HANDLE_VALUE == hFile) {
		auto lastError = GetLastError();
		return IOErrorFromWindowsError(
			"NewRandomRWFile failed to Create/Open: " + fname,
			lastError);
	}
...
Status WinEnvIO::GetFileSize(const std::string &fname, uint64_t *size)
{
	...
	if (GetFileAttributesExA(fname.c_str(), GetFileExInfoStandard,
				 &attrs)) {
		...
		*size = file_size.QuadPart;
		...
		auto lastError = GetLastError();
		s = IOErrorFromWindowsError("Can not get size for: " + fname,
					    lastError);
	}
	return s;
}
					 ...
					 uint64_t *file_mtime)
{
	...
	if (GetFileAttributesExA(fname.c_str(), GetFileExInfoStandard,
		...
		*file_mtime = FileTimeToUnixTime(attrs.ftLastWriteTime);
		...
		auto lastError = GetLastError();
		s = IOErrorFromWindowsError(
			"Can not get file modification time for: " + fname,
			lastError);
		*file_mtime = 0;
	}
...
Status WinEnvIO::RenameFile(const std::string &src, const std::string &target)
{
	...
	if (!MoveFileExA(src.c_str(), target.c_str(),
...
Status WinEnvIO::LockFile(const std::string &lockFname, FileLock **lock)
{
	...
	*lock = NULL;
	...
	{
		IOSTATS_TIMER_GUARD(open_nanos);
		hFile = CreateFileA(lockFname.c_str(),
			   ...
			   std::shared_ptr<Logger> *result)
{
	...
	HANDLE hFile = 0;
	{
		...
		hFile = CreateFileA(
			...
			FILE_SHARE_READ |
				FILE_SHARE_DELETE, // In RocksDb log files are
	...
	if (INVALID_HANDLE_VALUE == hFile) {
		auto lastError = GetLastError();
		s = IOErrorFromWindowsError("Failed to open LogFile" + fname,
					    lastError);
	} else {
		{
		...
		result->reset(new WinLogger(&WinEnvThreads::gettid, hosted_env_,
					    hFile));
	}
	return s;
}
...
EnvOptions
WinEnvIO::OptimizeForManifestWrite(const EnvOptions &env_options) const
{
	EnvOptions optimized = env_options;
	optimized.use_mmap_writes = false;
	optimized.use_direct_writes = false;
...
{
	WIN32_FILE_ATTRIBUTE_DATA attrs;
	if (GetFileAttributesExA(dname.c_str(), GetFileExInfoStandard,
				 &attrs)) {
...
void WinEnvThreads::StartThread(void (*function)(void *arg), void *arg)
{
	...
	state->arg = arg;
	try {
		rocksdb::port::WindowsThread th(&StartThreadWrapper,
						state.get());
		state.release();
...
{
	return winenv_threads_.Schedule(function, arg, pri, tag,
					unschedFunction);
}
...
EnvOptions WinEnv::OptimizeForManifestWrite(const EnvOptions &env_options) const
{
