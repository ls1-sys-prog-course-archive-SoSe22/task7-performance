...
class DateTieredTest : public testing::Test {
			      ...
			      bool read_only = false)
	{
		...
		ASSERT_OK(DateTieredDB::Open(options_, dbname_, &date_tiered_db,
					     ttl, column_family_interval,
					     read_only));
		date_tiered_db_.reset(date_tiered_db);
	}

	void CloseDateTieredDB()
	{
		date_tiered_db_.reset(nullptr);
	}
	...
	Status AppendTimestamp(std::string *key)
	{
		...
		if (port::kLittleEndian) {
			for (int i = 0; i < bytes_to_fill; ++i) {
				ts[i] = (timestamp_value >>
					 ((bytes_to_fill - i - 1) << 3)) &
					0xFF;
			}
		} else {
			memcpy(ts, static_cast<void *>(&timestamp_value),
			       bytes_to_fill);
		}
		key->append(ts, 8);
		return Status::OK();
...
TEST_F(DateTieredTest, KeyMayExistTest)
{
	...
	for (auto &kv : map_insert) {
		std::string value;
		ASSERT_TRUE(
			date_tiered_db_->KeyMayExist(ropts, kv.first, &value));
		ASSERT_EQ(value, kv.second);
	}
}
...
{
	fprintf(stderr,
		"SKIPPED as DateTieredDB is not supported in ROCKSDB_LITE\n");
	return 0;
}
