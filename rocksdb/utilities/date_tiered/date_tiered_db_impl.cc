...
DateTieredDBImpl::DateTieredDBImpl(
	...
	: db_(db), cf_options_(ColumnFamilyOptions(options)),
	  ioptions_(ImmutableCFOptions(options)), ttl_(ttl),
	...
	if (!s.ok()) {
		...
		for (auto name : column_family_names) {
			descriptors.emplace_back(
				ColumnFamilyDescriptor(name, cf_options));
		}

		// Open database
		if (read_only) {
			s = DB::OpenForReadOnly(db_options, dbname, descriptors,
						&handles, &db);
		} else {
			s = DB::Open(db_options, dbname, descriptors, &handles,
				     &db);
		}
	}

	if (s.ok()) {
		*dbptr = new DateTieredDBImpl(db, options, descriptors, handles,
					      ttl, column_family_interval);
	}
	return s;
}
...
Status DateTieredDBImpl::CreateColumnFamily(ColumnFamilyHandle **column_family)
{
	int64_t curtime;
	Status s;
	mutex_.AssertHeld();
	...
	if (handle_map_.empty()) {
		...
		new_timebound =
			latest_timebound_ + ((curtime - latest_timebound_) /
						     column_family_interval_ +
					     1) * column_family_interval_;
	}
	...
	if (s.ok()) {
		handle_map_.insert(
			std::make_pair(new_timebound, *column_family));
	}
	return s;
}
...
Status DateTieredDBImpl::Write(const WriteOptions &opts, WriteBatch *updates)
{
	class Handler : public WriteBatch::Handler {
		...
		{
			WriteBatchInternal::Put(&updates_ttl, column_family_id,
						key, value);
			return Status::OK();
		}
