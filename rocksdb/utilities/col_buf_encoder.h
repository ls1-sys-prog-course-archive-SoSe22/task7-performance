...
class ColBufEncoder {
	...
	static ColBufEncoder *
	NewColBufEncoder(const ColDeclaration &col_declaration);
...
class FixedLengthColBufEncoder : public ColBufEncoder {
	...
	explicit FixedLengthColBufEncoder(
		size_t size,
		ColCompressionType col_compression_type = kColNoCompression,
		bool nullable = false, bool big_endian = false)
		: size_(size), col_compression_type_(col_compression_type),
		  nullable_(nullable), big_endian_(big_endian), last_val_(0),
		  run_length_(-1), run_val_(0)
	{
	}
...
class VariableChunkColBufEncoder : public VariableLengthColBufEncoder {
	...
	size_t Append(const char *buf) override;
	void Finish() override;
	explicit VariableChunkColBufEncoder(
...
struct ColDeclaration {
	explicit ColDeclaration(
		...
		size_t _size = 0, bool _nullable = false,
		bool _big_endian = false)
		: col_type(_col_type),
		  col_compression_type(_col_compression_type), size(_size),
		  nullable(_nullable), big_endian(_big_endian)
	{
	}
	std::string col_type;
	ColCompressionType col_compression_type;
...
struct KVPairColDeclarations {
	...
	KVPairColDeclarations(
		std::vector<ColDeclaration> *_key_col_declarations,
...
struct KVPairColBufEncoders {
	...
	explicit KVPairColBufEncoders(const KVPairColDeclarations &kvp_cd)
	{
		for (auto kcd : *kvp_cd.key_col_declarations) {
			key_col_bufs.emplace_back(std::move(
				ColBufEncoder::NewColBufEncoder(kcd)));
		}
		for (auto vcd : *kvp_cd.value_col_declarations) {
			value_col_bufs.emplace_back(std::move(
				ColBufEncoder::NewColBufEncoder(vcd)));
		}
		value_checksum_buf.reset(ColBufEncoder::NewColBufEncoder(
			*kvp_cd.value_checksum_declaration));
	}
