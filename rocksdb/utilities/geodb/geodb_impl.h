...
class GeoDBImpl : public GeoDB {
	...
	static std::string PositionToQuad(const GeoPosition &pos,
					  int levelOfDetail);
	...
	static GeoPosition PixelToPosition(const Pixel &pixel,
					   int levelOfDetail);
	...
	static double distance(double lat1, double lon1, double lat2,
			       double lon2);
	...
	{
		return displaceLatLon(in.latitude, in.longitude, -radius,
				      -radius);
	}
