...
inline std::string
GetSpatialIndexColumnFamilyName(const std::string &spatial_index_name)
{
...
void FeatureSet::Serialize(std::string *output) const
{
	for (const auto &iter : map_) {
		...
		case Variant::kBool:
			output->push_back(
				static_cast<char>(iter.second.get_bool()));
			break;
		case Variant::kInt:
		...
		case Variant::kString:
			PutLengthPrefixedSlice(output,
					       iter.second.get_string());
			break;
		default:
			assert(false);
		}
...
bool FeatureSet::Deserialize(const Slice &input)
{
	...
	while (s.size()) {
		...
		case Variant::kBool: {
			...
			map_.insert({ key.ToString(),
				      Variant(static_cast<bool>(s[0])) });
			s.remove_prefix(1);
			break;
		}
		case Variant::kInt: {
...
class ValueGetterFromIterator : public ValueGetter {
    public:
	explicit ValueGetterFromIterator(Iterator *iterator)
		: iterator_(iterator)
	{
	}
	...
	virtual bool Get(uint64_t id) override
	{
		...
		if (!iterator_->Valid() ||
		    iterator_->key() != Slice(encoded_id)) {
			status_ = Status::Corruption("Index inconsistency");
			return false;
		}
...
class SpatialIndexCursor : public Cursor {
	...
	SpatialIndexCursor(Iterator *spatial_iterator,
			   ValueGetter *value_getter,
			   const BoundingBox<uint64_t> &tile_bbox,
			   uint32_t tile_bits)
		: value_getter_(value_getter), valid_(true)
	{
		...
		for (uint64_t x = tile_bbox.min_x; x <= tile_bbox.max_x; ++x) {
			for (uint64_t y = tile_bbox.min_y; y <= tile_bbox.max_y;
			     ++y) {
				quad_keys.push_back(
					GetQuadKeyFromTile(x, y, tile_bits));
			}
		}
		...
		for (auto quad_key : quad_keys) {
			...
			while (CheckQuadKey(spatial_iterator, slice_quad_key)) {
				...
				bool ok = GetFixed64BigEndian(
					Slice(spatial_iterator->key().data() +
						      sizeof(uint64_t),
					      sizeof(uint64_t)),
					&id);
				if (!ok) {
	...
	bool CheckQuadKey(Iterator *spatial_iterator, const Slice &quad_key)
	{
		...
		if (spatial_iterator->key().size() != 2 * sizeof(uint64_t)) {
			status_ =
				Status::Corruption("Invalid spatial index key");
			valid_ = false;
			return false;
		}
	...
	void ExtractData()
	{
		...
		if (valid_) {
			...
			if (!GetLengthPrefixedSlice(&data, &current_blob_) ||
				...
				status_ = Status::Corruption(
					"Primary key column family corruption");
				valid_ = false;
			}
		}
	}
...
class SpatialDBImpl : public SpatialDB {
	...
	SpatialDBImpl(DB *db, ColumnFamilyHandle *data_column_family,
		      const std::vector<std::pair<SpatialIndexOptions,
						  ColumnFamilyHandle *> >
			      &spatial_indexes,
		      uint64_t next_id, bool read_only)
		: SpatialDB(db), data_column_family_(data_column_family),
		  next_id_(next_id), read_only_(read_only)
	{
	...
	virtual Status
	Insert(const WriteOptions &write_options,
	       ...
	       const std::vector<std::string> &spatial_indexes) override
	{
		...
		if (spatial_indexes.size() == 0) {
			return Status::InvalidArgument(
				"Spatial indexes can't be empty");
		}
		...
		for (const auto &si : spatial_indexes) {
			...
			BoundingBox<uint64_t> tile_bbox =
				GetTileBoundingBox(spatial_index, bbox);

			for (uint64_t x = tile_bbox.min_x; x <= tile_bbox.max_x;
			     ++x) {
				for (uint64_t y = tile_bbox.min_y;
				     y <= tile_bbox.max_y; ++y) {
					...
					batch.Put(itr->second.column_family,
						  key, Slice());
					if (batch.GetDataSize() >=
					    kWriteOutEveryBytes) {
						Status s = Write(write_options,
								 &batch);
						batch.Clear();
						if (!s.ok()) {
							return s;
	...
	virtual Status Compact(int num_threads) override
	{
		...
		for (auto cfh : column_families) {
			threads.emplace_back([&, cfh] {
				...
				if (t.ok()) {
					t = CompactRange(CompactRangeOptions(),
							 cfh, nullptr, nullptr);
				}
			      ...
			      const std::string &spatial_index) override
	{
		...
		if (read_only_) {
			spatial_iterator = NewIterator(
				read_options, itr->second.column_family);
			value_getter = new ValueGetterFromDB(
				this, data_column_family_);
		} else {
			std::vector<Iterator *> iterators;
			Status s = NewIterators(read_options,
						{ data_column_family_,
						  itr->second.column_family },
						&iterators);
			if (!s.ok()) {
			...
			value_getter =
				new ValueGetterFromIterator(iterators[0]);
		}
		return new SpatialIndexCursor(spatial_iterator, value_getter,
					      GetTileBoundingBox(si, bbox),
					      si.tile_bits);
	}
...
DBOptions GetDBOptionsFromSpatialDBOptions(const SpatialDBOptions &options)
{
	...
	db_options.env->SetBackgroundThreads(
		db_options.max_background_compactions, Env::LOW);
					   ...
					   std::shared_ptr<Cache> block_cache)
{
	...
	column_family_options.max_bytes_for_level_base =
		256 * 1024 * 1024; // 256MB
	...
	for (int i = 0; i < column_family_options.num_levels; ++i) {
		if (i < 2) {
			column_family_options.compression_per_level[i] =
				kNoCompression;
		} else {
			column_family_options.compression_per_level[i] =
				kLZ4Compression;
...
ColumnFamilyOptions
OptimizeOptionsForDataColumnFamily(ColumnFamilyOptions options,
				   std::shared_ptr<Cache> block_cache)
{
...
class MetadataStorage {
	...
	Status AddIndex(const SpatialIndexOptions &index)
	{
		...
		return db_->Put(WriteOptions(), cf_,
				GetSpatialIndexColumnFamilyName(index.name),
				encoded_index);
	}
	...
	Status GetIndex(const std::string &name, SpatialIndexOptions *dst)
	{
		...
		return ok ? Status::OK() :
				  Status::Corruption("Index encoding corrupted");
	}
...
Status
SpatialDB::Create(const SpatialDBOptions &options, const std::string &name,
		  const std::vector<SpatialIndexOptions> &spatial_indexes)
{
	...
	column_families.push_back(ColumnFamilyDescriptor(
		...
		OptimizeOptionsForDataColumnFamily(column_family_options,
						   block_cache)));
	column_families.push_back(ColumnFamilyDescriptor(
		kMetadataColumnFamilyName, column_family_options));
	...
	DB *base_db;
	Status s =
		DB::Open(db_options, name, column_families, &handles, &base_db);
	if (!s.ok()) {
		return s;
	}
		       ...
		       SpatialDB **db, bool read_only)
{
	...
	for (const auto &cf_name : existing_column_families) {
		...
		if (GetSpatialIndexName(cf_name, &spatial_index)) {
			spatial_indexes.emplace_back(spatial_index.data(),
						     spatial_index.size());
		}
	}
	...
	for (const auto &index : spatial_indexes) {
		column_families.emplace_back(
			GetSpatialIndexColumnFamilyName(index),
			column_family_options);
	}
	std::vector<ColumnFamilyHandle *> handles;
	DB *base_db;
	if (read_only) {
		s = DB::OpenForReadOnly(db_options, name, column_families,
					&handles, &base_db);
	} else {
		s = DB::Open(db_options, name, column_families, &handles,
			     &base_db);
	}
	if (!s.ok()) {
		return s;
	...
	std::vector<std::pair<SpatialIndexOptions, ColumnFamilyHandle *> >
		index_cf;
	...
	if (s.ok()) {
		// find next_id
		Iterator *iter =
			base_db->NewIterator(ReadOptions(), handles[0]);
		iter->SeekToLast();
		if (iter->Valid()) {
			uint64_t last_id = 0;
			if (!GetFixed64BigEndian(iter->key(), &last_id)) {
				s = Status::Corruption(
					"Invalid key in data column family");
			} else {
				next_id = last_id + 1;
			}
		}
