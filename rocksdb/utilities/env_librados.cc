...
#define LOG_DEBUG(...)                                                         \
	do {                                                                   \
		printf("[%ld:%s:%i:%s]", syscall(SYS_gettid), __FILE__,        \
...
class LibradosWritableFile : public WritableFile {
	...
	std::mutex
		_mutex; // used to protect modification of all following variables
   ...
   */
	...
	LibradosWritableFile(librados::IoCtx *io_ctx, std::string fid,
		...
		: _io_ctx(io_ctx), _fid(fid), _hint(hint), _env(env), _buffer(),
		  _buffer_size(0), _file_size(0)
	{
   ...
   */
	Status Truncate(uint64_t size)
	{
		LOG_DEBUG("[IN]%lld|%lld|%lld\n", (long long)size,
			  (long long)_file_size, (long long)_buffer_size);
		int r = 0;
   ...
   */
	Status Flush()
	{
		...
		std::lock_guard<std::mutex> lock(_mutex);
		r = _io_ctx->aio_append(_fid, write_completion, _buffer,
					_buffer_size);
...
class LibradosFileLock : public FileLock {
	...
	LibradosFileLock(librados::IoCtx *io_ctx, const std::string obj_name)
		: _io_ctx(io_ctx), _obj_name(obj_name), _lock_name("lock_name"),
		  _cookie("cookie")
	{
...
EnvLibrados::EnvLibrados(const std::string &client_name,
	...
	: EnvWrapper(Env::Default()), _client_name(client_name),
	  _cluster_name(cluster_name), _flags(flags), _db_name(db_name),
	  _config_path(config_path), _db_pool_name(db_pool), _wal_dir(wal_dir),
	  _wal_pool_name(wal_pool), _write_buffer_size(write_buffer_size)
{
	int ret = 0;
	...
	if (ret < 0) { // let's handle any error that might have come back
		std::cerr << "couldn't initialize rados! error " << ret
			  << std::endl;
		ret = EXIT_FAILURE;
		goto out;
	}
				      ...
				      const EnvOptions &options)
{
	...
	do {
		...
		result->reset(new LibradosSequentialFile(_GetIoctx(fpath), fid,
							 fpath));
		s = Status::OK();
	} while (0);
 ...
 */
Status
EnvLibrados::NewRandomAccessFile(const std::string &fname,
				 ...
				 const EnvOptions &options)
{
	...
	do {
		...
		result->reset(new LibradosRandomAccessFile(_GetIoctx(fpath),
							   fid, fpath));
		s = Status::OK();
	} while (0);
				    ...
				    const EnvOptions &options)
{
	...
	do {
		...
		result->reset(new LibradosWritableFile(_GetIoctx(fpath), fid,
						       fpath, this));
		s = Status::OK();
	} while (0);
				      ...
				      const EnvOptions &options)
{
	...
	do {
		...
		result->reset(new LibradosWritableFile(
			_GetIoctx(dst_fpath), src_fid, dst_fpath, this));
	} while (0);
