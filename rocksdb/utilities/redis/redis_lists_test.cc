...
TEST_F(RedisListsTest, SimpleTest)
{
	...
	std::vector<std::string> result =
		redis.Range("k1", 0, 2); // Get the list
...
TEST_F(RedisListsTest, SetTest)
{
	...
	ASSERT_EQ(redis.Set("k1", 7, "off-by-one-error in positive index"),
		  false);
	ASSERT_EQ(redis.Set("k1", 43892, "big random index should fail"),
		  false);
	ASSERT_EQ(redis.Set("k1", -21391, "large negative index should fail"),
		  false);
...
TEST_F(RedisListsTest, InsertPushSetTest)
{
	...
	redis.InsertBefore("k1", "z",
			   "newbegin"); // InsertBefore beginning of list
	...
	redis.InsertBefore("k1", "a",
			   "sad"); // [sad, a, happy, z, a, aftera, ...]
	ASSERT_TRUE(redis.Index("k1", 0, &tempv));
	ASSERT_EQ(tempv, "sad");
	ASSERT_TRUE(redis.Index("k1", 2, &tempv));
	ASSERT_EQ(tempv, "happy");
	...
	redis.InsertAfter("k1", "a",
			  "zz"); // [sad, a, zz, happy, z, a, aftera, ...]
	ASSERT_TRUE(redis.Index("k1", 2, &tempv));
	ASSERT_EQ(tempv, "zz");
	ASSERT_TRUE(redis.Index("k1", 6, &tempv));
	ASSERT_EQ(tempv, "aftera");
	ASSERT_TRUE(redis.Set("k1", 1,
			      "nota")); // [sad, nota, zz, happy, z, a, ...]
	redis.InsertBefore("k1", "a",
			   "ba"); // [sad, nota, zz, happy, z, ba, a, ...]
	ASSERT_TRUE(redis.Index("k1", 4, &tempv));
	ASSERT_EQ(tempv, "z");
	ASSERT_TRUE(redis.Index("k1", 5, &tempv));
	ASSERT_EQ(tempv, "ba");
	...
	lengthCheck =
		redis.Length("k1"); // Ensure that the length doesn't change
	ASSERT_EQ(lengthCheck, 10);
	ASSERT_EQ(redis.InsertBefore("k1", "non-exist", "randval"),
		  lengthCheck);
	ASSERT_EQ(redis.InsertAfter("k1", "nothing", "a"), lengthCheck);
	ASSERT_EQ(redis.InsertAfter("randKey", "randVal", "ranValue"),
		  0); // Empty
	ASSERT_EQ(redis.Length("k1"),
		  lengthCheck); // The length should not change
...
TEST_F(RedisListsTest, RemoveTest)
{
	...
	// Try removing ALL!
	numRemoved =
		redis.Remove("k1", 0, "newbegin"); // REMOVE 0 will remove all!
...
TEST_F(RedisListsTest, PersistenceMultiKeyTest)
{
	...
	// Block one: populate a single key in the database
	{
		...
		redis.PushRight("k1", "x");
		redis.InsertBefore("k1", "z",
				   "newbegin"); // InsertBefore start of list
		redis.InsertAfter("k1", "x",
				  "newend"); // InsertAfter end of list
		redis.InsertAfter("k1", "a", "aftera");
		redis.PushRight("k1", "a");
		redis.PushRight("k1", "a");
...
int manual_redis_test(bool destructive)
{
	RedisLists redis(RedisListsTest::kDefaultDbName,
			 RedisListsTest::options, destructive);
	...
	while (true) {
		...
		if (command == "LINSERT") {
			...
			MakeUpper(&t);
			if (t == "BEFORE") {
				std::cout << redis.InsertBefore(k, p, v)
					  << std::endl;
			} else if (t == "AFTER") {
				std::cout << redis.InsertAfter(k, p, v)
					  << std::endl;
			}
