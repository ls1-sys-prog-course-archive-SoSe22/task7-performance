...
class RedisListIterator {
	...
	explicit RedisListIterator(const std::string &list_data)
		: data_(list_data.data()),
		  ...
		  cur_byte_(0), cur_elem_(0), cur_elem_length_(0), length_(0),
		  result_()
	{
		...
		if (num_bytes_ < sizeof(length_)) {
			ThrowError(
				"Corrupt header."); // Will break control flow
		}
		...
		//   if possible.
		if (length_ > 0) {
			if (cur_byte_ + sizeof(cur_elem_length_) <=
			    num_bytes_) {
				cur_elem_length_ =
					DecodeFixed32(data_ + cur_byte_);
			} else {
	...
	void InitializeResult()
	{
		...
		result_.resize(sizeof(uint32_t),
			       0); // Put a block of 0's as the header
	}
	...
	void WriteCurrentElement()
	{
		...
		result_.insert(result_.end(), data_ + cur_byte_,
			       data_ + cur_byte_ + sizeof(uint32_t) +
				       cur_elem_length_);
	}
	...
	void CheckErrors()
	{
		...
		if ((cur_elem_ > length_) || // Bad index
		    ...
		    (cur_byte_ + cur_elem_length_ >
		     num_bytes_) || // Item too large
		    (cur_byte_ == num_bytes_ &&
		     cur_elem_ != length_) || // Too many items
		    (cur_elem_ == length_ &&
		     cur_byte_ != num_bytes_)) { // Too many bytes
