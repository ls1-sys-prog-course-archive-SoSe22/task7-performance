...
class ColumnAwareEncodingTestWithSize
	...
	static std::vector<size_t> GetValues()
	{
		return { 4, 8 };
	}
};
...
TEST_P(ColumnAwareEncodingTestWithSize, NoCompressionEncodeDecode)
{
	...
	std::unique_ptr<ColBufEncoder> col_buf_encoder(
		new FixedLengthColBufEncoder(col_size, kColNoCompression, false,
					     true));
	std::string str_buf;
	...
	if (col_size == 8) {
		expected_encoded_val = port::kLittleEndian ?
						     0x0807060504030201 :
						     0x0102030405060708;
	} else if (col_size == 4) {
	...
	std::unique_ptr<ColBufDecoder> col_buf_decoder(
		new FixedLengthColBufDecoder(col_size, kColNoCompression, false,
					     true));
	...
	for (int i = 0; i < row_count; ++i) {
		encoded_data_ptr += col_buf_decoder->Decode(encoded_data_ptr,
							    &decoded_data);
	}
...
TEST_P(ColumnAwareEncodingTestWithSize, DeltaEncodeDecode)
{
	...
	int row_count = 4;
	std::unique_ptr<ColBufEncoder> col_buf_encoder(
		new FixedLengthColBufEncoder(col_size, kColDeltaVarint, false,
					     true));
	std::string str_buf;
	uint64_t base_val1 =
	...
	for (int i = 0; i < row_count / 2; ++i) {
		str_buf =
			std::string(reinterpret_cast<char *>(&val1), col_size);
		str_buf_ptr = str_buf.c_str();
		col_buf_encoder->Append(str_buf_ptr);

		str_buf =
			std::string(reinterpret_cast<char *>(&val2), col_size);
		str_buf_ptr = str_buf.c_str();
		col_buf_encoder->Append(str_buf_ptr);
	}
	col_buf_encoder->Finish();
	...
	std::unique_ptr<ColBufDecoder> col_buf_decoder(
		new FixedLengthColBufDecoder(col_size, kColDeltaVarint, false,
					     true));
	char *decoded_data = new char[100];
	char *decoded_data_base = decoded_data;
	const char *encoded_data_ptr = encoded_data.c_str();
