				      ...
				      bool *value_changed) const
{
	...
	if (options_.ignore_value == false) {
		...
		lua_pushlstring(lua_state, existing_value.data(),
				existing_value.size());
		num_input_values = 3;
		num_return_values = 3;
	} else {
	...
	if ((error_no = lua_pcall(lua_state, num_input_values,
				  num_return_values, 0)) != 0) {
		LogLuaError("[Lua] Error(%d) in Filter function --- %s",
	...
	if (!lua_isboolean(lua_state, kIndexIsFiltered)) {
		LogLuaError("[Lua] Error in Filter function -- "
	...
	if (!has_error) {
		...
		if (options_.ignore_value == false) {
			*value_changed =
				lua_toboolean(lua_state, kIndexValueChanged);
			if (*value_changed) {
				const char *new_value_buf =
					lua_tostring(lua_state, kIndexNewValue);
				const size_t new_value_size =
					lua_strlen(lua_state, kIndexNewValue);
				...
				new_value->assign(new_value_buf,
						  new_value_size);
			}
		} else {
			*value_changed = false;
...
const char *RocksLuaCompactionFilter::Name() const
{
	...
	if (!lua_isstring(lua_state, -1)) {
		...
		const size_t name_size __attribute__((unused)) =
