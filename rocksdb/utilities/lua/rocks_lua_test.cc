...
class RocksLuaTest : public testing::Test {
			...
			*output_factory = nullptr)
	{
		...
		auto factory = std::make_shared<
			rocksdb::lua::RocksLuaCompactionFilterFactory>(lua_opt);
		...
		options_.target_file_size_base =
			(kKeySize + kValueSize) * kKeysPerFlush;
...
TEST_F(RocksLuaTest, GetName)
{
	...
	lua_opt.lua_script =
		std::string("function Filter(level, key, existing_value)\n"
			    "  return false, false, \"\"\n"
			    "end\n"
			    "\n"
			    "function FilterMergeOperand(level, key, operand)\n"
...
TEST_F(RocksLuaTest, ConditionallyChangeAndFilterValue)
{
	...
	lua_opt.lua_script = "function Filter(level, key, existing_value)\n"
...
TEST_F(RocksLuaTest, DynamicChangeScript)
{
	...
	factory->SetScript("function Filter(level, key, existing_value)\n"
			   "  return true, false, \"\"\n"
			   "end\n"
			   "\n"
			   "function FilterMergeOperand(level, key, operand)\n"
