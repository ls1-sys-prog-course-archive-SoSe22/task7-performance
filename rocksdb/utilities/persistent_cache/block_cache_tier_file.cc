...
struct CacheRecordHeader {
	...
	CacheRecordHeader(const uint32_t magic, const uint32_t key_size,
		...
		: magic_(magic), crc_(0), key_size_(key_size),
		  val_size_(val_size)
	{
	}
...
struct CacheRecord {
	...
	{
		return static_cast<uint32_t>(sizeof(CacheRecordHeader) +
					     key.size() + val.size());
	}
...
uint32_t CacheRecord::ComputeCRC() const
{
	...
	tmp.crc_ = 0;
	crc = crc32c::Extend(crc, reinterpret_cast<const char *>(&tmp),
			     sizeof(tmp));
...
bool RandomAccessCacheFile::OpenImpl(const bool enable_direct_reads)
{
	...
	std::unique_ptr<RandomAccessFile> file;
	Status status = NewRandomAccessCacheFile(env_, Path(), &file,
						 enable_direct_reads);
	if (!status.ok()) {
		Error(log_, "Error opening random access file %s. %s",
				...
				const bool enable_direct_reads)
{
	...
	if (s.ok()) {
		ROCKS_LOG_WARN(log_, "File %s already exists. %s",
			       Path().c_str(), s.ToString().c_str());
	}

	s = NewWritableCacheFile(env_, Path(), &file_);
	if (!s.ok()) {
		ROCKS_LOG_WARN(log_, "Unable to create file %s. %s",
			       Path().c_str(), s.ToString().c_str());
		return false;
	}
...
bool WriteableCacheFile::Append(const Slice &key, const Slice &val, LBA *lba)
{
	...
	if (!ExpandBuffer(rec_size)) {
		...
		ROCKS_LOG_DEBUG(log_, "Error expanding buffers. size=%d",
				rec_size);
		return false;
	}
				    ...
				    char *scratch)
{
	...
	if (!ReadBuffer(lba, scratch)) {
		Error(log_, "Error reading from buffer. cache=%d off=%d",
		      cache_id_, lba.off_);
		return false;
	}
