...
Status BlockCacheTier::Open()
{
	...
	status = NewCacheFile();
	if (!status.ok()) {
		Error(opt_.log, "Error creating new file %s. %s",
		      opt_.path.c_str(), status.ToString().c_str());
		return status;
	}
...
Status BlockCacheTier::CleanupCacheFolder(const std::string &folder)
{
	...
	if (!status.ok()) {
		Error(opt_.log, "Error getting files for %s. %s",
	...
	for (auto file : files) {
		if (IsCacheFile(file)) {
			...
			if (!status.ok()) {
				Error(opt_.log, "Error deleting file %s. %s",
				      file.c_str(), status.ToString().c_str());
				return status;
			}
		} else {
			ROCKS_LOG_DEBUG(opt_.log, "Skipping file %s",
					file.c_str());
		}
	}
	return Status::OK();
			      ...
			      const size_t size)
{
	...
	if (opt_.pipeline_writes) {
		...
		insert_ops_.Push(InsertOp(key.ToString(),
...
void BlockCacheTier::InsertMain()
{
	while (true) {
		...
		size_t retry = 0;
		Status s;
		while ((s = InsertImpl(Slice(op.key_), Slice(op.data_)))
			       .IsTryAgain()) {
			if (retry > kMaxRetry) {
				break;
			}
...
Status BlockCacheTier::NewCacheFile()
{
	...
	std::unique_ptr<WriteableCacheFile> f(new WriteableCacheFile(
		...
		writer_cache_id_, opt_.cache_file_size, opt_.log));

	bool status =
		f->Create(opt_.enable_direct_writes, opt_.enable_direct_reads);
	if (!status) {
