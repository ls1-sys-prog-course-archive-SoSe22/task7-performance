...
std::unique_ptr<PersistentCacheTier> NewBlockCache()
{
	...
	PersistentCacheConfig opt(Env::Default(), FLAGS_path, FLAGS_cache_size,
				  log);
...
class CacheTierBenchmark {
	...
	void InsertKey(const uint64_t key)
	{
		...
		while (true) {
			Status status = cache_->Insert(block_key, block.get(),
						       FLAGS_iosize);
			if (status.ok()) {
				break;
			}
	...
	void ReadKey(const uint64_t val)
	{
		...
		if (!FLAGS_benchmark) {
			...
			assert(memcmp(block.get(), expected_block.get(),
				      FLAGS_iosize) == 0);
		}
	}
	...
	std::atomic<uint64_t> read_key_limit_{
		0
	}; // data can be read safely upto
