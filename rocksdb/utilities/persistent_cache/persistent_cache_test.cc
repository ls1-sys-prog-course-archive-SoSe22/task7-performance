...
std::unique_ptr<PersistentTieredCache>
NewTieredCache(const size_t mem_size, const PersistentCacheConfig &opt)
{
	std::unique_ptr<PersistentTieredCache> tcache(
		new PersistentTieredCache());
	// create primary tier
	assert(mem_size);
	auto pcache =
		std::shared_ptr<PersistentCacheTier>(new VolatileCacheTier(
			/*is_compressed*/ true, mem_size));
	tcache->AddTier(pcache);
	// create secondary tier
	auto scache =
		std::shared_ptr<PersistentCacheTier>(new BlockCacheTier(opt));
...
// create block cache
std::unique_ptr<PersistentCacheTier>
NewBlockCache(Env *env, const std::string &path,
	      ...
	      const bool enable_direct_writes = false)
{
	...
	opt.max_write_pipeline_backlog_size =
...
std::unique_ptr<PersistentTieredCache>
NewTieredCache(Env *env, const std::string &path,
...
TEST_F(PersistentCacheTierTest, VolatileCacheInsert)
{
	for (auto nthreads : { 1, 5 }) {
		for (auto max_keys : { 10 * 1024 * kStressFactor,
...
TEST_F(PersistentCacheTierTest, VolatileCacheInsertWithEviction)
{
	for (auto nthreads : { 1, 5 }) {
		for (auto max_keys : { 1 * 1024 * 1024 * kStressFactor }) {
			cache_ = std::make_shared<VolatileCacheTier>(
				/*compressed=*/true,
				/*size=*/static_cast<size_t>(1 * 1024 * 1024 *
							     kStressFactor));
			RunInsertTestWithEviction(
				nthreads, static_cast<size_t>(max_keys));
		}
	}
}
	...
	for (auto nthreads : { 1, 5 }) {
		for (auto max_keys : { 1 * 1024 * 1024 * kStressFactor }) {
			cache_ = NewBlockCache(
				Env::Default(), path_,
				/*max_size=*/
				static_cast<size_t>(200 * 1024 * 1024 *
						    kStressFactor));
			RunInsertTestWithEviction(
				nthreads, static_cast<size_t>(max_keys));
		}
...
std::shared_ptr<PersistentCacheTier>
MakeVolatileCache(const std::string & /*dbname*/)
{
...
std::shared_ptr<PersistentCacheTier> MakeTieredCache(const std::string &dbname)
{
	...
	const size_t max_keys = 100 * 1024, const size_t max_usecase = 5)
{
	...
	for (size_t iter = 0; iter < max_usecase; iter++) {
		Options options;
		options.write_buffer_size = static_cast<size_t>(
		...
		auto page_hit =
			TestGetTickerCount(options, PERSISTENT_CACHE_HIT);
		auto page_miss =
			TestGetTickerCount(options, PERSISTENT_CACHE_MISS);
