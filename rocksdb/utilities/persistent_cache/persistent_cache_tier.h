...
struct PersistentCacheConfig {
	explicit PersistentCacheConfig(
		Env *const _env, const std::string &_path,
		const uint64_t _cache_size, const std::shared_ptr<Logger> &_log,
	...
	Status ValidateSettings() const
	{
		...
		if (cache_size < cache_file_size ||
		    ...
		    write_buffer_size * write_buffer_count() <
			    2 * cache_file_size) {
		...
		if (!writer_qdepth ||
		    writer_dispatch_size > write_buffer_size ||
	...
	size_t write_buffer_count() const
	{
		...
		return static_cast<size_t>((writer_qdepth + 1.2) *
	...
	MakePersistentCacheConfig(const std::string &path, const uint64_t size,
...
class PersistentCacheTier : public PersistentCache {
	...
	virtual Status Lookup(const Slice &page_key,
