...
class PersistentCacheTierTest : public testing::Test {
	...
	void Verify(const size_t nthreads = 1,
		    const bool eviction_enabled = false)
	{
		...
		key_ = 0;
		// spawn threads
		auto fn = std::bind(&PersistentCacheTierTest::VerifyImpl, this,
				    eviction_enabled);
		auto threads = SpawnThreads(nthreads, fn);
		// join with threads
		Join(std::move(threads));
	...
	void VerifyImpl(const bool eviction_enabled = false)
	{
		...
		while (true) {
			...
			if (eviction_enabled) {
				if (!cache_->Lookup(key, &block, &block_size)
					     .ok()) {
	...
	void RunInsertTestWithEviction(const size_t nthreads,
...
class PersistentCacheDBTest : public DBTestBase {
	...
	// test template
	void
	RunTest(const std::function<std::shared_ptr<PersistentCacheTier>(bool)>
			&new_pcache,
