...
struct LogicalBlockAddress {
	...
	explicit LogicalBlockAddress(const uint32_t cache_id,
				     const uint32_t off, const uint16_t size)
...
class BlockCacheFile : public LRUElement<BlockCacheFile> {
	...
	explicit BlockCacheFile(Env *const env, const std::string &dir,
		...
		: LRUElement<BlockCacheFile>(), env_(env), dir_(dir),
		  cache_id_(cache_id)
	{
	}
	...
	std::list<BlockInfo *> &block_infos()
	{
		return block_infos_;
	}
	...
	std::list<BlockInfo *>
		block_infos_; // List of index entries mapping to the
		// file content
};
...
class RandomAccessCacheFile : public BlockCacheFile {
	...
	bool Read(const LBA &lba, Slice *key, Slice *block,
...
class WriteableCacheFile : public RandomAccessCacheFile {
    public:
	explicit WriteableCacheFile(Env *const env,
				    CacheWriteBufferAllocator *alloc,
				    Writer *writer, const std::string &dir,
				    const uint32_t cache_id,
				    const uint32_t max_size,
				    const std::shared_ptr<Logger> &log)
		: RandomAccessCacheFile(env, dir, cache_id, log), alloc_(alloc),
		  writer_(writer), max_size_(max_size)
	{
	}
	...
	// create file on disk
	bool Create(const bool enable_direct_writes,
		  ...
		  char *scratch) override
	{
		...
		if (closed) {
			...
			return RandomAccessCacheFile::Read(lba, key, block,
							   scratch);
		}
	...
	bool ReadBuffer(const LBA &lba, Slice *key, Slice *block,
			char *scratch);
...
class ThreadedWriter : public Writer {
	...
	struct IO {
		...
		explicit IO(WritableFile *const file,
			    CacheWriteBuffer *const buf,
			    const uint64_t file_off,
			    const std::function<void()> callback)
			: file_(file), buf_(buf), file_off_(file_off),
			  callback_(callback)
		{
		}
	...
	explicit ThreadedWriter(PersistentCacheTier *const cache,
