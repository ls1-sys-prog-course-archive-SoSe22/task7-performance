...
class BlockCacheTier : public PersistentCacheTier {
	...
	explicit BlockCacheTier(const PersistentCacheConfig &opt)
		: opt_(opt), insert_ops_(opt_.max_write_pipeline_backlog_size),
		  buffer_allocator_(opt.write_buffer_size,
				    opt.write_buffer_count()),
	...
	{
		return opt_.is_compressed;
	}

	std::string GetPrintableOptions() const override
	{
		return opt_.ToString();
	}
