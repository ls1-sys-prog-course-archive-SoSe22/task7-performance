...
class TtlTest : public testing::Test {
	...
	void OpenTtlWithTestCompaction(int32_t ttl)
	{
		options_.compaction_filter_factory =
			std::shared_ptr<CompactionFilterFactory>(
				new TestFilterFactory(kSampleSize_,
						      kNewValue_));
		OpenTtl(ttl);
	}
	...
	void MakePutWriteBatch(const BatchOperation *batch_ops, int64_t num_ops)
	{
		...
		WriteBatch batch;
		kv_it_ = kvmap_.begin();
		for (int64_t i = 0; i < num_ops && kv_it_ != kvmap_.end();
		     i++, ++kv_it_) {
			switch (batch_ops[i]) {
			case OP_PUT:
	...
	void PutValues(int64_t start_pos_map, int64_t num_entries,
		       bool flush = true, ColumnFamilyHandle *cf = nullptr)
	{
		ASSERT_TRUE(db_ttl_);
		...
		for (int64_t i = 0; kv_it_ != kvmap_.end() && i < num_entries;
			...
			ASSERT_OK(cf == nullptr ?
						db_ttl_->Put(wopts, kv_it_->first,
						       kv_it_->second) :
						db_ttl_->Put(wopts, cf, kv_it_->first,
						       kv_it_->second));
		}
		// Put a mock kv at the end because CompactionFilter doesn't delete last key
		ASSERT_OK(cf == nullptr ?
					db_ttl_->Put(wopts, "keymock", "valuemock") :
					db_ttl_->Put(wopts, cf, "keymock",
					       "valuemock"));
		if (flush) {
			if (cf == nullptr) {
				db_ttl_->Flush(flush_opts);
	...
	{
		if (cf == nullptr) {
			db_ttl_->CompactRange(CompactRangeOptions(), nullptr,
					      nullptr);
		} else {
			db_ttl_->CompactRange(CompactRangeOptions(), cf,
					      nullptr, nullptr);
		}
	}
	...
	void SimpleKeyMayExistCheck()
	{
		...
		std::string val;
		for (auto &kv : kvmap_) {
			bool ret = db_ttl_->KeyMayExist(ropts, kv.first, &val,
							&value_found);
			       ...
			       ColumnFamilyHandle *cf = nullptr)
	{
		...
		std::string v;
		for (int64_t i = 0; kv_it_ != kvmap_.end() && i < span;
		     i++, ++kv_it_) {
			Status s =
				(cf == nullptr) ?
					      db_ttl_->Get(ropts, kv_it_->first, &v) :
					      db_ttl_->Get(ropts, cf, kv_it_->first,
						     &v);
			if (s.ok() != check) {
				...
				if (test_compaction_change &&
				    v.compare(kNewValue_) != 0) {
					fprintf(stderr,
						" value for key=%s present in database is %s but "
						" should be %s\n",
						kv_it_->first.c_str(),
						v.c_str(), kNewValue_.c_str());
					ASSERT_TRUE(false);
				} else if (!test_compaction_change &&
					   v.compare(kv_it_->second) != 0) {
					fprintf(stderr,
						" value for key=%s present in database is %s but "
						" should be %s\n",
						kv_it_->first.c_str(),
				   ...
				   bool check = true)
	{
		...
		if (!check) {
			if (dbiter->Valid()) {
				ASSERT_NE(
					dbiter->value().compare(kv_it_->second),
					0);
			}
		} else { // dbiter should have found out kvmap_[st_pos]
			for (int64_t i = st_pos;
	...
	class TestFilter : public CompactionFilter {
	    public:
		TestFilter(const int64_t kSampleSize,
			   const std::string &kNewValue)
				    ...
				    bool *value_changed) const override
		{
			...
			if (pos != std::string::npos) {
				auto key_substr = key_string.substr(
					pos, key.size() - pos);
#ifndef CYGWIN
				num_key_end = std::stoi(key_substr);
#else
				num_key_end =
					std::strtol(key_substr.c_str(), 0, 10);
#endif
	...
	class TestFilterFactory : public CompactionFilterFactory {
	    public:
		TestFilterFactory(const int64_t kSampleSize,
				  const std::string &kNewValue)
			: kSampleSize_(kSampleSize), kNewValue_(kNewValue)
		{
		}
...
TEST_F(TtlTest, NoEffect)
{
	...
	OpenTtl(-1);
	PutValues(boundary2,
		  kSampleSize_ - boundary2); //T=3: Set3 never deleted
...
TEST_F(TtlTest, ResetTimestamp)
{
	...
	SleepCompactCheck(2, 0,
			  kSampleSize_); // T=4: Set1 should still be there
	CloseTtl();
}
...
TEST_F(TtlTest, IterAbsentAfterTTL)
{
	...
	SleepCompactCheckIter(2, 0, kSampleSize_,
...
TEST_F(TtlTest, CompactionFilter)
{
	...
	SleepCompactCheck(0, 2 * partition, partition, true,
			  true); // Part changed
	CloseTtl();
}
...
TEST_F(TtlTest, ColumnFamiliesTest)
{
	...
	ASSERT_OK(db_ttl_->CreateColumnFamilyWithTtl(
		options, "ttl_column_family_2", &new_handle, 2));
