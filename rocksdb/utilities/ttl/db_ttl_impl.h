...
class DBWithTTLImpl : public DBWithTTL {
	...
	static const int32_t kMinTimestamp =
		1368146402; // 05/09/2013:5:40PM GMT-8

	static const int32_t kMaxTimestamp =
...
class TtlIterator : public Iterator {
	...
	{
		return DecodeFixed32(iter_->value().data() +
				     iter_->value().size() -
...
class TtlCompactionFilter : public CompactionFilter {
	...
	TtlCompactionFilter(int32_t ttl, Env *env,
		...
		: ttl_(ttl), env_(env), user_comp_filter_(user_comp_filter),
			  ...
			  std::move(user_comp_filter_from_factory))
	{
		...
		if (!user_comp_filter_) {
			user_comp_filter_ =
				user_comp_filter_from_factory_.get();
		}
	}
			    ...
			    bool *value_changed) const override
	{
		...
		Slice old_val_without_ts(old_val.data(),
					 old_val.size() -
						 DBWithTTLImpl::kTSLength);
		if (user_comp_filter_->Filter(level, key, old_val_without_ts,
					      new_val, value_changed)) {
			return true;
		}
		if (*value_changed) {
			new_val->append(old_val.data() + old_val.size() -
						DBWithTTLImpl::kTSLength,
					DBWithTTLImpl::kTSLength);
		}
		return false;
...
class TtlMergeOperator : public MergeOperator {
				 ...
				 MergeOperationOutput *merge_out) const override
	{
		...
		bool good = true;
		MergeOperationOutput user_merge_out(
			merge_out->new_value, merge_out->existing_operand);
		if (merge_in.existing_value) {
			Slice existing_value_without_ts(
			...
			good = user_merge_op_->FullMergeV2(
				MergeOperationInput(merge_in.key, nullptr,
						    operands_without_ts,
						    merge_in.logger),
				&user_merge_out);
		}
				       ...
				       Logger *logger) const override
	{
		...
		assert(new_value);
		if (!user_merge_op_->PartialMergeMulti(key, operands_without_ts,
						       new_value, logger)) {
			return false;
		}
