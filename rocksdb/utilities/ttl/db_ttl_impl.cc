				    ...
				    Env *env)
{
	if (options->compaction_filter) {
		options->compaction_filter = new TtlCompactionFilter(
			ttl, env, options->compaction_filter);
	} else {
		       ...
		       DBWithTTL **dbptr, int32_t ttl, bool read_only)
{
	...
	Status s = DBWithTTL::Open(db_options, dbname, column_families,
				   &handles, dbptr, { ttl }, read_only);
	if (s.ok()) {
...
Status
DBWithTTL::Open(const DBOptions &db_options, const std::string &dbname,
		...
		std::vector<int32_t> ttls, bool read_only)
{
	...
	for (size_t i = 0; i < column_families_sanitized.size(); ++i) {
		DBWithTTLImpl::SanitizeOptions(
			...
			db_options.env == nullptr ? Env::Default() :
							  db_options.env);
	}
	DB *db;
	...
	Status st;
	if (read_only) {
		st = DB::OpenForReadOnly(db_options, dbname,
					 column_families_sanitized, handles,
					 &db);
	} else {
		st = DB::Open(db_options, dbname, column_families_sanitized,
			      handles, &db);
	}
	if (st.ok()) {
...
Status
DBWithTTLImpl::CreateColumnFamilyWithTtl(const ColumnFamilyOptions &options,
					 ...
					 ColumnFamilyHandle **handle, int ttl)
{
	...
	return DBWithTTL::CreateColumnFamily(sanitized_options,
...
{
	return CreateColumnFamilyWithTtl(options, column_family_name, handle,
					 0);
}
...
std::vector<Status>
DBWithTTLImpl::MultiGet(const ReadOptions &options,
...
bool DBWithTTLImpl::KeyMayExist(const ReadOptions &options,
				...
				bool *value_found)
{
	bool ret = db_->KeyMayExist(options, column_family, key, value,
				    value_found);
	...
	class Handler : public WriteBatch::Handler {
				     ...
				     const Slice &value) override
		{
			...
			if (!st.ok()) {
				...
				WriteBatchInternal::Put(&updates_ttl,
				       ...
				       const Slice &value) override
		{
			...
			if (!st.ok()) {
				...
				WriteBatchInternal::Merge(&updates_ttl,
							  column_family_id, key,
							  value_with_ts);
			}
			return Status::OK();
		...
		{
			WriteBatchInternal::Delete(&updates_ttl,
						   column_family_id, key);
			return Status::OK();
		}
