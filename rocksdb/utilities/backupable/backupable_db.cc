...
void BackupableDBOptions::Dump(Logger *logger) const
{
	ROCKS_LOG_INFO(logger, "               Options.backup_dir: %s",
		       backup_dir.c_str());
	ROCKS_LOG_INFO(logger, "               Options.backup_env: %p",
		       backup_env);
	ROCKS_LOG_INFO(logger, "        Options.share_table_files: %d",
		       static_cast<int>(share_table_files));
	ROCKS_LOG_INFO(logger, "                 Options.info_log: %p",
		       info_log);
...
class BackupEngineImpl : public BackupEngine {
	...
	Status CreateNewBackupWithMetadata(
		...
		std::function<void()> progress_callback = []() {}) override;
	...
	void
	GetCorruptedBackups(std::vector<BackupID> *corrupt_backup_ids) override;
	Status RestoreDBFromBackup(BackupID backup_id,
	...
	struct FileInfo {
		FileInfo(const std::string &fname, uint64_t sz,
			 uint32_t checksum)
			: refs(0), filename(fname), size(sz),
	...
	class BackupMeta {
		...
		const std::string &GetAppMetadata() const
		{
		...
		Status
		LoadFromFile(const std::string &backup_dir,
		...
		std::string GetInfoString()
		{
			...
			for (const auto &file : files_) {
				AppendHumanBytes(file->size, human_size,
						 sizeof(human_size));
				ss << file->filename << ", size " << human_size
				   << ", refs " << file->refs << std::endl;
			}
			return ss.str();
		}
		...
		static const size_t max_backup_meta_file_size_ =
			10 * 1024 * 1024; // 10MB
	}; // BackupMeta

	inline std::string
	GetAbsolutePath(const std::string &relative_path = "") const
	{
	...
	inline std::string
	GetSharedFileWithChecksumRel(const std::string &file = "",
				     bool tmp = false) const
	{
		assert(file.size() == 0 || file[0] != '/');
		return GetSharedChecksumDirRel() + "/" + file +
		       (tmp ? ".tmp" : "");
	}
	inline std::string
	GetSharedFileWithChecksum(const std::string &file,
				  const uint32_t checksum_value,
	...
	inline std::string
	GetFileFromChecksumFile(const std::string &file) const
	{
		assert(file.size() == 0 || file[0] != '/');
		std::string file_copy = file;
	...
	Status CopyOrCreateFile(
		...
		bool sync, RateLimiter *rate_limiter, uint64_t *size = nullptr,
	...
	struct CopyOrCreateWorkItem {
		...
		CopyOrCreateWorkItem &
		operator=(const CopyOrCreateWorkItem &) = delete;
		...
		CopyOrCreateWorkItem &
		operator=(CopyOrCreateWorkItem &&o) ROCKSDB_NOEXCEPT
		{
		...
		CopyOrCreateWorkItem(
			...
			: src_path(std::move(_src_path)),
			  ...
			  contents(std::move(_contents)), src_env(_src_env),
			  dst_env(_dst_env), sync(_sync),
			  rate_limiter(_rate_limiter), size_limit(_size_limit),
	...
	struct BackupAfterCopyOrCreateWorkItem {
		...
		BackupAfterCopyOrCreateWorkItem &
		operator=(BackupAfterCopyOrCreateWorkItem &&o) ROCKSDB_NOEXCEPT
		{
			result = std::move(o.result);
			shared = o.shared;
	...
	struct RestoreAfterCopyOrCreateWorkItem {
		...
		RestoreAfterCopyOrCreateWorkItem(
			std::future<CopyOrCreateResult> &&_result,
		...
		RestoreAfterCopyOrCreateWorkItem &
		operator=(RestoreAfterCopyOrCreateWorkItem &&o) ROCKSDB_NOEXCEPT
		{
			result = std::move(o.result);
	...
	Status AddBackupFileWorkItem(
		...
		std::vector<BackupAfterCopyOrCreateWorkItem>
			&backup_items_to_finish,
		BackupID backup_id, bool shared, const std::string &src_dir,
		const std::string &fname, // starts with "/"
		RateLimiter *rate_limiter, uint64_t size_bytes,
	...
	static const size_t kDefaultCopyFileBufferSize =
		5 * 1024 * 1024LL; // 5MB
...
BackupEngineImpl::BackupEngineImpl(Env *db_env,
				   ...
				   bool read_only)
	: initialized_(false), stop_backup_(false), options_(options),
	  db_env_(db_env),
...
Status BackupEngineImpl::Initialize()
{
	...
	if (!read_only_) {
		...
		for (const auto &d : directories) {
			...
			if (s.ok()) {
				s = backup_env_->NewDirectory(d.first,
							      d.second);
			}
			if (!s.ok()) {
				return s;
	...
	{
		auto s = backup_env_->GetChildren(GetBackupMetaDir(),
						  &backup_meta_files);
		if (s.IsNotFound()) {
			return Status::NotFound(GetBackupMetaDir() +
						" is missing");
		} else if (!s.ok()) {
			return s;
		}
	...
	for (auto &file : backup_meta_files) {
		...
		ROCKS_LOG_INFO(options_.info_log, "Detected backup %s",
			       file.c_str());
		...
		if (backup_id == 0 || file != rocksdb::ToString(backup_id)) {
			if (!read_only_) {
				...
				auto s = backup_env_->DeleteFile(
					GetBackupMetaDir() + "/" + file);
				ROCKS_LOG_INFO(
					options_.info_log,
	...
	if (options_.destroy_old_data) { // Destroy old data
		...
		for (const auto &rel_dir :
			...
			InsertPathnameToSizeBytes(abs_dir, backup_env_,
						  &abs_path_to_size);
		}
		...
		for (auto backup_iter = backups_.rbegin();
		     ...
		     valid_backups_to_open > 0;
		     ++backup_iter) {
			InsertPathnameToSizeBytes(
				GetAbsolutePath(
					GetPrivateFileRel(backup_iter->first)),
				backup_env_, &abs_path_to_size);
			Status s = backup_iter->second->LoadFromFile(
			...
			if (s.IsCorruption()) {
				...
				ROCKS_LOG_INFO(
					options_.info_log,
					"Loading backup %" PRIu32 " OK:\n%s",
					backup_iter->first,
					backup_iter->second->GetInfoString()
						.c_str());
				latest_backup_id_ = std::max(
		...
		if (options_.max_valid_backups_to_open == 0) {
			...
			num_unopened_backups = std::max(
				0, static_cast<int>(backups_.size()) -
					   options_.max_valid_backups_to_open);
		}
	...
	ROCKS_LOG_INFO(options_.info_log, "Latest backup is %u",
		       latest_backup_id_);
		...
		threads_.emplace_back([this]() {
			...
			while (files_to_copy_or_create_.read(work_item)) {
				...
				result.status = CopyOrCreateFile(
					work_item.src_path, work_item.dst_path,
					work_item.contents, work_item.src_env,
					work_item.dst_env, work_item.sync,
					work_item.rate_limiter, &result.size,
	...
	std::function<void()> progress_callback)
{
	...
	if (s.ok()) {
		...
		s = checkpoint.CreateCustomCheckpoint(
			...
			[&](const std::string &src_dirname,
			    const std::string &fname, FileType) {
			...
			[&](const std::string &src_dirname,
				...
				Log(options_.info_log, "add file for backup %s",
				    fname.c_str());
				uint64_t size_bytes = 0;
				Status st;
				if (type == kTableFile) {
					st = db_env_->GetFileSize(src_dirname +
									  fname,
								  &size_bytes);
				}
				if (st.ok()) {
			...
			[&](const std::string &fname,
			    const std::string &contents, FileType) {
				Log(options_.info_log, "add file for backup %s",
				    fname.c_str());
				return AddBackupFileWorkItem(
					live_dst_paths, backup_items_to_finish,
					new_backup_id, false /* shared */,
					"" /* src_dir */, fname, rate_limiter,
					contents.size(), 0 /* size_limit */,
					false /* shared_checksum */,
					progress_callback, contents);
			} /* create_file_cb */,
			&sequence_number,
			flush_before_backup ? 0 : port::kMaxUint64);
		if (s.ok()) {
	...
	for (auto &item : backup_items_to_finish) {
		...
		if (item_status.ok() && item.shared && item.needed_to_copy) {
			item_status = item.backup_env->RenameFile(
				item.dst_path_tmp, item.dst_path);
		}
		if (item_status.ok()) {
			item_status = new_backup.get()->AddFile(
				std::make_shared<FileInfo>(
					item.dst_relative, result.size,
					result.checksum_value));
		}
		if (!item_status.ok()) {
			s = item_status;
	...
	if (s.ok()) {
		...
		s = backup_env_->RenameFile(
			GetAbsolutePath(
				GetPrivateFileRel(new_backup_id, true)), // tmp
			GetAbsolutePath(
				GetPrivateFileRel(new_backup_id, false)));
	}
	...
	if (s.ok() && options_.sync) {
		...
		backup_env_->NewDirectory(GetAbsolutePath(GetPrivateFileRel(
						  new_backup_id, false)),
	...
	ROCKS_LOG_INFO(options_.info_log,
		       "Backup time: %" PRIu64 " microseconds", backup_time);
	ROCKS_LOG_INFO(options_.info_log, "Backup speed: %.3f MB/s",
		       backup_speed);
	ROCKS_LOG_INFO(options_.info_log, "Backup Statistics %s",
		       backup_statistics_.ToString().c_str());
	return s;
...
Status BackupEngineImpl::DeleteBackup(BackupID backup_id)
{
	...
	for (auto &itr : backuped_file_infos_) {
		if (itr.second->refs == 0) {
			Status s = backup_env_->DeleteFile(
				GetAbsolutePath(itr.first));
	...
	const std::string &wal_dir, const RestoreOptions &restore_options)
{
	...
	if (restore_options.keep_log_files) {
		...
		db_env_->GetChildren(archive_dir,
				     &archive_files); // ignore errors
	...
	for (const auto &file_info : backup->GetFiles()) {
		...
		CopyOrCreateWorkItem copy_or_create_work_item(
			GetAbsolutePath(file), dst, "" /* contents */,
			backup_env_, db_env_, false, rate_limiter,
			0 /* size_limit */);
		RestoreAfterCopyOrCreateWorkItem after_copy_or_create_work_item(
			copy_or_create_work_item.result.get_future(),
			file_info->checksum_value);
		files_to_copy_or_create_.write(
			std::move(copy_or_create_work_item));
...
Status BackupEngineImpl::VerifyBackup(BackupID backup_id)
{
	...
	for (const auto &rel_dir :
	     { GetPrivateFileRel(backup_id), GetSharedFileRel(),
	       GetSharedFileWithChecksumRel() }) {
		const auto abs_dir = GetAbsolutePath(rel_dir);
		InsertPathnameToSizeBytes(abs_dir, backup_env_,
	...
	for (const auto &file_info : backup->GetFiles()) {
		...
		if (curr_abs_path_to_size.find(abs_path) ==
		    curr_abs_path_to_size.end()) {
...
Status BackupEngineImpl::CopyOrCreateFile(
	const std::string &src, const std::string &dst,
	const std::string &contents, Env *src_env, Env *dst_env, bool sync,
	...
	uint64_t size_limit, std::function<void()> progress_callback)
{
	...
	do {
		...
		if (checksum_value != nullptr) {
			*checksum_value = crc32c::Extend(
				*checksum_value, data.data(), data.size());
		}
	...
	} while (s.ok() && contents.empty() && data.size() > 0 &&
		 size_limit > 0);
...
Status BackupEngineImpl::AddBackupFileWorkItem(
	...
	std::vector<BackupAfterCopyOrCreateWorkItem> &backup_items_to_finish,
	BackupID backup_id, bool shared, const std::string &src_dir,
	const std::string &fname, RateLimiter *rate_limiter,
	uint64_t size_bytes, uint64_t size_limit, bool shared_checksum,
	std::function<void()> progress_callback, const std::string &contents)
{
	...
	if (shared && shared_checksum) {
		...
		if (size_bytes == port::kMaxUint64) {
			return Status::NotFound("File missing: " + src_dir +
						fname);
		}
		dst_relative = GetSharedFileWithChecksum(
			dst_relative, checksum_value, size_bytes);
		dst_relative_tmp =
			GetSharedFileWithChecksumRel(dst_relative, true);
		dst_relative =
			GetSharedFileWithChecksumRel(dst_relative, false);
	} else if (shared) {
		dst_relative_tmp = GetSharedFileRel(dst_relative, true);
		dst_relative = GetSharedFileRel(dst_relative, false);
	} else {
		dst_relative_tmp =
			GetPrivateFileRel(backup_id, true, dst_relative);
		dst_relative =
			GetPrivateFileRel(backup_id, false, dst_relative);
	}
	...
	if (!contents.empty()) {
		...
		if (shared_checksum) {
			...
			ROCKS_LOG_INFO(options_.info_log,
				       "%s already present, calculate checksum",
				       fname.c_str());
			s = CalculateChecksum(src_dir + fname, db_env_,
	...
	if (!contents.empty() || need_to_copy) {
		ROCKS_LOG_INFO(options_.info_log, "Copying %s to %s",
			       fname.c_str(), dst_path_tmp.c_str());
		CopyOrCreateWorkItem copy_or_create_work_item(
			src_dir.empty() ? "" : src_dir + fname, dst_path_tmp,
			contents, db_env_, backup_env_, options_.sync,
			rate_limiter, size_limit, progress_callback);
		BackupAfterCopyOrCreateWorkItem after_copy_or_create_work_item(
			copy_or_create_work_item.result.get_future(), shared,
			need_to_copy, backup_env_, dst_path_tmp, dst_path,
			dst_relative);
		files_to_copy_or_create_.write(
			std::move(copy_or_create_work_item));
		...
		BackupAfterCopyOrCreateWorkItem after_copy_or_create_work_item(
			promise_result.get_future(), shared, need_to_copy,
	...
	std::unordered_map<std::string, uint64_t> *result)
{
	...
	for (const auto &file_attrs : files_attrs) {
		result->emplace(dir + (slash_needed ? "/" : "") +
					file_attrs.name,
...
Status BackupEngineImpl::GarbageCollect()
{
	...
	if (options_.share_table_files) {
		...
		std::vector<std::string> shared_children;
		{
			...
			if (s.ok()) {
				s = backup_env_->GetChildren(shared_path,
							     &shared_children);
			} else if (s.IsNotFound()) {
				s = Status::OK();
			}
	...
	{
		auto s = backup_env_->GetChildren(
			GetAbsolutePath(GetPrivateDirRel()), &private_children);
		if (!s.ok()) {
			return s;
		}
	}
	for (auto &child : private_children) {
		...
		for (auto &subchild : subchildren) {
			Status s = backup_env_->DeleteFile(full_private_path +
							   subchild);
			ROCKS_LOG_INFO(options_.info_log, "Deleting %s -- %s",
...
Status
BackupEngineImpl::BackupMeta::AddFile(std::shared_ptr<FileInfo> file_info)
{
	auto itr = file_infos_->find(file_info->filename);
	if (itr == file_infos_->end()) {
		auto ret =
			file_infos_->insert({ file_info->filename, file_info });
		if (ret.second) {
			itr = ret.first;
			itr->second->refs = 1;
		} else {
	...
	const std::unordered_map<std::string, uint64_t> &abs_path_to_size)
{
	...
	s = env_->NewSequentialFile(meta_filename_, &backup_meta_file,
				    EnvOptions());
	if (!s.ok()) {
		return s;
	}
	...
	Slice data;
	s = backup_meta_reader->Read(max_backup_meta_file_size_, &data,
				     buf.get());
	...
	if (data.starts_with(kMetaDataPrefix)) {
		...
		bool decode_success =
			hex_encoded_metadata.DecodeHex(&app_metadata_);
		if (!decode_success) {
			return Status::Corruption(
	...
	for (uint32_t i = 0; s.ok() && i < num_files; ++i) {
		...
		if (line.empty()) {
			return Status::Corruption(
				"File checksum is missing for " + filename +
				" in " + meta_filename_);
		}
		...
		if (line.starts_with(checksum_prefix)) {
			...
			return Status::Corruption("Unknown checksum type for " +
						  filename + " in " +
						  meta_filename_);
		}

		files.emplace_back(
			new FileInfo(filename, size, checksum_value));
	}
...
Status BackupEngineImpl::BackupMeta::StoreToFile(bool sync)
{
	...
	if (!app_metadata_.empty()) {
		...
		if (hex_encoded_metadata.size() + kMetaDataPrefix.size() + 1 >
		    buf_size - len) {
			return Status::Corruption(
				"Buffer too small to fit backup metadata");
		}
		memcpy(buf.get() + len, kMetaDataPrefix.data(),
		       kMetaDataPrefix.size());
	...
	len += snprintf(buf.get() + len, buf_size - len,
			"%" ROCKSDB_PRIszt "\n", files_.size());
	for (const auto &file : files_) {
...
class BackupEngineReadOnlyImpl : public BackupEngineReadOnly {
    public:
	BackupEngineReadOnlyImpl(Env *db_env,
				 const BackupableDBOptions &options)
	...
	// latest backup comes last.
	virtual void
	GetBackupInfo(std::vector<BackupInfo> *backup_info) override
	{
	...
	virtual Status
	RestoreDBFromBackup(BackupID backup_id, const std::string &db_dir,
			    const std::string &wal_dir,
			    const RestoreOptions &restore_options =
				    RestoreOptions()) override
	{
	...
	virtual Status
	RestoreDBFromLatestBackup(const std::string &db_dir,
				  const std::string &wal_dir,
				  const RestoreOptions &restore_options =
					  RestoreOptions()) override
	{
		return backup_engine_->RestoreDBFromLatestBackup(
			db_dir, wal_dir, restore_options);
	}
