...
class DummyDB : public StackableDB {
	...
	virtual Status GetSortedWalFiles(VectorLogPtr &files) override
	{
		...
		for (size_t i = 0; i < files.size(); ++i) {
			files[i].reset(new DummyLogFile(wal_files_[i].first,
							wal_files_[i].second));
		}
		return Status::OK();
	}
...
class TestEnv : public EnvWrapper {
				 ...
				 const EnvOptions &options) override
	{
		...
		if (dummy_sequential_file_) {
			r->reset(new TestEnv::DummySequentialFile(
				dummy_sequential_file_fail_reads_));
			return Status::OK();
		} else {
	...
	{
		MutexLock l(&mutex_);
		std::sort(should_have_written.begin(),
			  should_have_written.end());
	...
	void
	SetDummySequentialFileFailReads(bool dummy_sequential_file_fail_reads)
	{
		MutexLock l(&mutex_);
		dummy_sequential_file_fail_reads_ =
			dummy_sequential_file_fail_reads;
	}
	...
	void
	SetFilenamesForMockedAttrs(const std::vector<std::string> &filenames)
	{
		filenames_for_mocked_attrs_ = filenames;
	}
	Status
	GetChildrenFileAttributes(const std::string &dir,
				  std::vector<Env::FileAttributes> *r) override
	{
		if (filenames_for_mocked_attrs_.size() > 0) {
			for (const auto &filename :
			     filenames_for_mocked_attrs_) {
				r->push_back({ dir + filename,
					       10 /* size_bytes */ });
			}
			return Status::OK();
		}
	...
	Status GetFileSize(const std::string &path,
			   uint64_t *size_bytes) override
	{
		if (filenames_for_mocked_attrs_.size() > 0) {
			auto fname = path.substr(path.find_last_of('/'));
			auto filename_iter =
				std::find(filenames_for_mocked_attrs_.begin(),
					  filenames_for_mocked_attrs_.end(),
					  fname);
			if (filename_iter !=
			    filenames_for_mocked_attrs_.end()) {
				*size_bytes = 10;
...
class FileManager : public EnvWrapper {
				       ...
				       const std::string &data)
	{
		...
		while (true) {
			int i = rnd_.Next() % children.size();
			if (children[i] != "." && children[i] != "..") {
				return WriteToFile(dir + "/" + children[i],
						   data);
			}
		}
		// should never get here
	...
	Status CorruptFile(const std::string &fname, uint64_t bytes_to_corrupt)
	{
		...
		for (uint64_t i = 0; i < bytes_to_corrupt; ++i) {
			...
			file_contents[rnd_.Next() % file_contents.size()] =
				tmp[0];
		}
...
class BackupableDBTest : public testing::Test {
	...
	BackupableDBTest()
	{
		...
		std::string backup_chroot =
			test::TmpDir() + "/backupable_db_backup";
		...
		backup_chroot_env_.reset(
			NewChrootEnv(Env::Default(), backup_chroot));
		...
		backupable_options_.reset(new BackupableDBOptions(
			backupdir_, test_backup_env_.get(), true, logger_.get(),
			true));
		...
		bool share_with_checksums = false)
	{
		backupable_options_->share_files_with_checksum =
			share_with_checksums;
		OpenDBAndBackupEngine(destroy_old_data, dummy,
				      share_with_checksums);
	}

	void OpenDBAndBackupEngine(bool destroy_old_data = false,
				   bool dummy = false,
				   bool share_table_files = true)
	{
		...
		ASSERT_OK(BackupEngine::Open(test_db_env_.get(),
					     *backupable_options_,
					     &backup_engine));
				     ...
				     bool keep_log_files = false)
	{
		...
		if (backup_id > 0) {
			ASSERT_OK(backup_engine_->RestoreDBFromBackup(
				backup_id, dbname_, dbname_, restore_options));
		} else {
...
TEST_P(BackupableDBTestWithParam, OfflineIntegrationTest)
{
	...
	for (int iter = 0; iter < 2; ++iter) {
		...
		for (int i = 0; i < 5; ++i) {
			...
			int fill_up_to =
				std::min(keys_iteration * (i + 1), max_key);
			...
			if (i >= 3) { // test purge old backups
				...
				ASSERT_OK(
					backup_engine_->PurgeOldBackups(5 - i));
			}
...
TEST_P(BackupableDBTestWithParam, OnlineIntegrationTest)
{
	...
	for (int i = 1; i <= 5; ++i) {
		if (i == 2) {
			// we deleted backup 2
			Status s = backup_engine_->RestoreDBFromBackup(
				2, dbname_, dbname_);
			ASSERT_TRUE(!s.ok());
		} else {
...
TEST_F(BackupableDBTest, NoDoubleCopy)
{
	...
	dummy_db_->live_files_ = { "/00010.sst", "/00011.sst", "/CURRENT",
				   "/MANIFEST-01" };
	dummy_db_->wal_files_ = { { "/00011.log", true },
				  { "/00012.log", false } };
	...
	uint64_t size;
	test_backup_env_->GetFileSize(backupdir_ + "/private/2/MANIFEST-01",
				      &size);
...
TEST_F(BackupableDBTest, CorruptionsTest)
{
	...
	ASSERT_TRUE(!s.ok());
	CloseBackupEngine();
	ASSERT_OK(file_manager_->DeleteRandomFileInDir(backupdir_ +
						       "/private/4"));
	...
	ASSERT_OK(
		file_manager_->CorruptChecksum(backupdir_ + "/meta/3", false));
	...
	ASSERT_OK(
		backup_engine_->CreateNewBackup(db_.get(), !!(rnd.Next() % 2)));
	CloseDBAndBackupEngine();
...
TEST_F(BackupableDBTest, InterruptCreationTest)
{
	...
	ASSERT_OK(
		backup_chroot_env_->FileExists(backupdir_ + "/private/1.tmp/"));
...
TEST_F(BackupableDBTest, BackupOptions)
{
	...
	for (int i = 1; i < 5; i++) {
		...
		db_.reset(OpenDB());
		ASSERT_OK(backup_engine_->CreateNewBackup(db_.get(), true));
		rocksdb::GetLatestOptionsFileName(db_->GetName(), options_.env,
						  &name);
		ASSERT_OK(file_manager_->FileExists(OptionsPath(backupdir_, i) +
						    name));
		backup_chroot_env_->GetChildren(OptionsPath(backupdir_, i),
						&filenames);
...
TEST_F(BackupableDBTest, NoDeleteWithReadOnly)
{
	...
	CloseDBAndBackupEngine();
	ASSERT_OK(
		file_manager_->WriteToFile(backupdir_ + "/LATEST_BACKUP", "4"));
...
TEST_F(BackupableDBTest, NoShareTableFiles)
{
	...
	for (int i = 0; i < 5; ++i) {
		FillDB(db_.get(), keys_iteration * i, keys_iteration * (i + 1));
		ASSERT_OK(
			backup_engine_->CreateNewBackup(db_.get(), !!(i % 2)));
	}
	CloseDBAndBackupEngine();
	...
	for (bool shared_checksum : { false, true }) {
		if (shared_checksum) {
			OpenDBAndBackupEngineShareWithChecksum(
				...
				true /* share_table_files */,
				true /* share_with_checksums */);
		} else {
			OpenDBAndBackupEngine();
		}
		CloseDBAndBackupEngine();
		...
		CloseDBAndBackupEngine();
		ASSERT_EQ(Status::NotFound(),
			  file_manager_->FileExists(shared_tmp));
		ASSERT_EQ(Status::NotFound(),
			  file_manager_->FileExists(private_tmp_file));
		ASSERT_EQ(Status::NotFound(),
			  file_manager_->FileExists(private_tmp_dir));
	}
}
...
TEST_F(BackupableDBTest, RateLimiting)
{
	...
	for (bool makeThrottler : { false, true }) {
		if (makeThrottler) {
			backupable_options_->backup_rate_limiter =
				backupThrottler;
			backupable_options_->restore_rate_limiter =
				restoreThrottler;
		}
		...
		for (int iter = 0; iter < 2; ++iter) {
			for (const auto &limit : limits) {
				...
				if (makeThrottler) {
					backupThrottler->SetBytesPerSecond(
						limit.first);
					restoreThrottler->SetBytesPerSecond(
						limit.second);
				} else {
				...
				backupable_options_->max_background_operations =
					(iter == 0) ? 1 : 10;
				...
				ASSERT_OK(backup_engine_->CreateNewBackup(
					db_.get(), false));
				auto backup_time = db_chroot_env_->NowMicros() -
						   start_backup;
				auto rate_limited_backup_time =
					(bytes_written * kMicrosPerSec) /
					limit.first;
				ASSERT_GT(backup_time,
					  0.8 * rate_limited_backup_time);

				CloseDBAndBackupEngine();

				OpenBackupEngine();
				auto start_restore =
					db_chroot_env_->NowMicros();
				ASSERT_OK(backup_engine_
				...
				auto rate_limited_restore_time =
					(bytes_written * kMicrosPerSec) /
					limit.second;
				ASSERT_GT(restore_time,
					  0.8 * rate_limited_restore_time);
...
TEST_F(BackupableDBTest, ReadOnlyBackupEngine)
{
	...
	BackupEngineReadOnly *read_only_backup_engine;
	ASSERT_OK(BackupEngineReadOnly::Open(db_chroot_env_.get(),
					     *backupable_options_,
					     &read_only_backup_engine));
	std::vector<BackupInfo> backup_info;
	read_only_backup_engine->GetBackupInfo(&backup_info);
...
TEST_F(BackupableDBTest, EnvFailures)
{
	...
	// get children failure
	{
		...
		ASSERT_NOK(BackupEngine::Open(test_db_env_.get(),
					      *backupable_options_,
					      &backup_engine));
	...
	// Read from meta-file failure
	{
		...
		FillDB(db_.get(), 0, 100);
		ASSERT_TRUE(
			backup_engine_->CreateNewBackup(db_.get(), true).ok());
		CloseDBAndBackupEngine();
...
TEST_F(BackupableDBTest, ChangeManifestDuringBackupCreation)
{
	...
	std::string prev_manifest_path = DescriptorFileName(
		dbname_, db_impl->TEST_Current_Manifest_FileNo());
	FillDB(db_.get(), 0, 100);
	ASSERT_OK(db_chroot_env_->FileExists(prev_manifest_path));
	ASSERT_OK(db_->Flush(FlushOptions()));
	ASSERT_TRUE(
		db_chroot_env_->FileExists(prev_manifest_path).IsNotFound());
...
TEST_F(BackupableDBTest, Issue921Test)
{
	...
	ASSERT_OK(BackupEngine::Open(backup_chroot_env_.get(),
				     *backupable_options_, &backup_engine));
...
TEST_F(BackupableDBTest, BackupWithMetadata)
{
	...
	for (int i = 0; i < 5; ++i) {
		...
		FillDB(db_.get(), keys_iteration * i, keys_iteration * (i + 1));
		ASSERT_OK(backup_engine_->CreateNewBackupWithMetadata(
			db_.get(), metadata, true));
	}
	CloseDBAndBackupEngine();
...
TEST_F(BackupableDBTest, BinaryMetadata)
{
	...
	ASSERT_OK(backup_engine_->CreateNewBackupWithMetadata(db_.get(),
							      binaryMetadata));
	CloseDBAndBackupEngine();
...
TEST_F(BackupableDBTest, MetadataTooLarge)
{
	...
	ASSERT_NOK(backup_engine_->CreateNewBackupWithMetadata(db_.get(),
							       largeMetadata));
	CloseDBAndBackupEngine();
	DestroyDB(dbname_, options_);
}
...
TEST_F(BackupableDBTest, LimitBackupsOpened)
{
	...
	for (int i = 1; i <= 4; ++i) {
		...
		ASSERT_OK(backup_engine_->CreateNewBackup(db_.get(), true));
		if (i == 3) {
			ASSERT_OK(file_manager_->CorruptFile(
				backupdir_ + "/meta/3", 3));
		}
	}
	CloseDBAndBackupEngine();
...
{
	fprintf(stderr,
		"SKIPPED as BackupableDB is not supported in ROCKSDB_LITE\n");
	return 0;
}
