...
TEST_F(DocumentDBTest, ComplexQueryTest)
{
	...
	{
		std::unique_ptr<JSONDocument> query(Parse(
			"{'progress': {'$gt': 50.0}, '$index': 'progress'}"));
		ASSERT_OK(db_->Remove(ReadOptions(), WriteOptions(), *query));
	}
	...
	{
		std::unique_ptr<JSONDocument> query(
			Parse("{'job_name': 'white'}"));
		std::unique_ptr<JSONDocument> update(
			Parse("{'$set': {'priority': 10}}"));
		ASSERT_OK(db_->Update(ReadOptions(), WriteOptions(), *query,
				      *update));
	}
	...
	{
		std::unique_ptr<JSONDocument> query(Parse(
			"[{'$filter': {'priority': {'$lt': 0}, '$index': 'priority'}}]"));
		std::unique_ptr<Cursor> cursor(
			db_->Query(ReadOptions(), *query));
	...
	{
		std::unique_ptr<JSONDocument> query(Parse(
			"[{'$filter': {'priority': {'$gt': 4}, '$index': 'priority'}}]"));
		std::unique_ptr<Cursor> cursor(
			db_->Query(ReadOptions(), *query));
		ASSERT_OK(cursor->status());
...
{
	fprintf(stderr,
		"SKIPPED as DocumentDB is not supported in ROCKSDB_LITE\n");
	return 0;
}
