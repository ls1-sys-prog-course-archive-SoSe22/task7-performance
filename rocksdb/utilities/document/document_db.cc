...
int DocumentCompare(const JSONDocument &lhs, const JSONDocument &rhs)
{
	...
	case JSONDocument::kBool:
		return static_cast<int>(lhs.GetBool()) -
		       static_cast<int>(rhs.GetBool());
...
class Filter {
	...
	struct Interval {
		...
		Interval()
			: upper_bound(), lower_bound(), upper_inclusive(false),
			  lower_inclusive(false)
		{
		}
		Interval(const JSONDocument &ub, const JSONDocument &lb,
			 bool ui, bool li)
			: upper_bound(ub), lower_bound(lb), upper_inclusive(ui),
			  lower_inclusive(li)
		{
		}
...
void Filter::Interval::UpdateUpperBound(const JSONDocument &ub, bool inclusive)
{
...
void Filter::Interval::UpdateLowerBound(const JSONDocument &lb, bool inclusive)
{
...
Filter *Filter::ParseFilter(const JSONDocument &filter)
{
	...
	for (const auto &items : f->filter_.Items()) {
		...
		if (items.second.IsObject()) {
			...
			f->intervals_.insert(
				{ items.first,
				  Interval(items.second, items.second, true,
					   true) });
		}
	}
...
bool Filter::SatisfiesFilter(const JSONDocument &document) const
{
	for (const auto &interval : intervals_) {
		...
		if (!interval.second.upper_bound.IsNull()) {
			if (value.type() !=
			    interval.second.upper_bound.type()) {
				// no cross-type queries yet
				// TODO(icanadi) do this at least for numbers!
				return false;
			}
			int cmp = DocumentCompare(interval.second.upper_bound,
						  value);
			if (cmp < 0 ||
			    (cmp == 0 &&
			     interval.second.upper_inclusive == false)) {
		...
		if (!interval.second.lower_bound.IsNull()) {
			if (value.type() !=
			    interval.second.lower_bound.type()) {
				// no cross-type queries yet
				return false;
			}
			int cmp = DocumentCompare(interval.second.lower_bound,
						  value);
			if (cmp > 0 ||
			    (cmp == 0 &&
			     interval.second.lower_inclusive == false)) {
...
class Index {
	...
	// failure.
	static Index *
	CreateIndexFromDescription(const JSONDocument &description,
...
class IndexKey {
	...
	explicit IndexKey(const Slice &slice)
	{
		...
		uint32_t primary_key_offset = DecodeFixed32(
			slice.data() + slice.size() - sizeof(uint32_t));
	...
	IndexKey(const Slice &secondary_key, const Slice &primary_key)
		: ok_(true)
	{
	...
	SliceParts GetSliceParts()
	{
		uint32_t primary_key_offset =
			static_cast<uint32_t>(parts_[0].size());
...
class SimpleSortedIndex : public Index {
				 ...
				 std::string *key) const override
	{
		if (!document.Contains(field_)) {
			if (!EncodeJSONPrimitive(
				    JSONDocument(JSONDocument::kNull), key)) {
				assert(false);
			}
		} else {
	...
	// REQUIRES: UsefulIndex(filter) == true
	virtual bool
	ShouldContinueLooking(const Filter &filter, const Slice &secondary_key,
			      Index::Direction direction) const override
	{
		...
		if (direction == kForwards) {
			...
			if (!EncodeJSONPrimitive(interval->upper_bound,
						 &encoded_upper_bound)) {
				// uhm...?
			...
			int compare = secondary_key.compare(
				Slice(encoded_upper_bound));
			...
			if (!EncodeJSONPrimitive(interval->lower_bound,
						 &encoded_lower_bound)) {
				// uhm...?
				// TODO(icanadi) store encoded upper and lower bounds in Filter*?
				assert(false);
...
class CursorWithFilterIndexed : public Cursor {
	...
	CursorWithFilterIndexed(Iterator *primary_index_iter,
				Iterator *secondary_index_iter,
				const Index *index, const Filter *filter)
		: primary_index_iter_(primary_index_iter),
		  secondary_index_iter_(secondary_index_iter), index_(index),
		  filter_(filter), valid_(true), current_json_document_(nullptr)
	{
		assert(filter_.get() != nullptr);
		direction_ = index->Position(*filter_.get(),
					     secondary_index_iter_.get());
		UpdateIndexKey();
	...
	void AdvanceUntilSatisfies()
	{
		...
		while (secondary_index_iter_->Valid() &&
		       index_->ShouldContinueLooking(
			       *filter_.get(), index_key_.GetSecondaryKey(),
			       direction_)) {
	...
	bool UpdateJSONDocument()
	{
		...
		current_json_document_.reset(JSONDocument::Deserialize(
			primary_index_iter_->value()));
	...
	void UpdateIndexKey()
	{
		if (secondary_index_iter_->Valid()) {
			...
			if (!index_key_.ok()) {
				status_ =
					Status::Corruption("Invalid index key");
				valid_ = false;
			}
		}
	}
	...
	void UpdateCurrentJSON()
	{
		if (Valid()) {
			current_json_document_.reset(
				JSONDocument::Deserialize(iter_->value()));
			if (current_json_document_.get() == nullptr) {
				status_ = Status::Corruption(
					"JSON deserialization failed");
			}
...
class CursorWithFilter : public Cursor {
	...
	void SeekToNextSatisfies()
	{
		for (; base_cursor_->Valid(); base_cursor_->Next()) {
			if (filter_->SatisfiesFilter(
				    base_cursor_->document())) {
				break;
			}
		}
	}
...
class DocumentDBImpl : public DocumentDB {
	...
	DocumentDBImpl(
		...
		const std::vector<std::pair<Index *, ColumnFamilyHandle *> >
			&indexes,
	...
	{
		auto index_obj = Index::CreateIndexFromDescription(
			*index.description, index.name);
		if (index_obj == nullptr) {
			return Status::InvalidArgument(
				"Failed parsing index description");
		}

		ColumnFamilyHandle *cf_handle;
		Status s = CreateColumnFamily(
			ColumnFamilyOptions(rocksdb_options_),
		...
		std::unique_ptr<CursorFromIterator> cursor(
			new CursorFromIterator(DocumentDB::NewIterator(
				ReadOptions(), primary_key_column_family_)));

		WriteBatch batch;
		for (; cursor->Valid(); cursor->Next()) {
			std::string secondary_index_key;
			index_obj->GetIndexKey(cursor->document(),
					       &secondary_index_key);
			IndexKey index_key(Slice(secondary_index_key),
					   cursor->key());
			batch.Put(cf_handle, index_key.GetSliceParts(),
				  SliceParts());
		}
...

		{
			...
			name_to_index_.insert(
				{ index.name,
				  IndexColumnFamily(index_obj, cf_handle) });
		}
			      ...
			      const JSONDocument &document) override
	{
		...
		PinnableSlice value;
		Status s = DocumentDB::Get(ReadOptions(),
					   primary_key_column_family_,
					   primary_key_slice, &value);
		if (!s.IsNotFound()) {
			return s.ok() ? Status::InvalidArgument(
		...
		for (const auto &iter : name_to_index_) {
			std::string secondary_index_key;
			iter.second.index->GetIndexKey(document,
						       &secondary_index_key);
			IndexKey index_key(Slice(secondary_index_key),
					   primary_key_slice);
			batch.Put(iter.second.column_family,
				  index_key.GetSliceParts(), SliceParts());
		}
			      ...
			      const JSONDocument &query) override
	{
		...
		WriteBatch batch;
		for (; cursor->status().ok() && cursor->Valid();
		     cursor->Next()) {
			...
			auto primary_key = document[kPrimaryKey];
			if (primary_key.IsNull() || (!primary_key.IsString() &&
						     !primary_key.IsInt64())) {
				return Status::Corruption(
					"Document corruption");
			}
			...
			std::string primary_key_encoded;
			if (!EncodeJSONPrimitive(primary_key,
						 &primary_key_encoded)) {
				// previous call should be guaranteed to pass because of all primary_key
				// conditions checked before
				assert(false);
			...
			Slice primary_key_slice(primary_key_encoded);
			batch.Delete(primary_key_column_family_,
				     primary_key_slice);

			for (const auto &iter : name_to_index_) {
				std::string secondary_index_key;
				iter.second.index->GetIndexKey(
					document, &secondary_index_key);
				IndexKey index_key(Slice(secondary_index_key),
						   primary_key_slice);
				batch.Delete(iter.second.column_family,
					     index_key.GetSliceParts());
			}
		}
		...
		for (; cursor->status().ok() && cursor->Valid();
			...
			for (const auto &update : updates.Items()) {
				if (update.first == "$set") {
					...
					for (const auto &itr :
						...
						if (itr.first == kPrimaryKey) {
							return Status::NotSupported(
								"Please don't change primary key");
						}
						res = builder.WriteKeyValue(
							itr.first, itr.second);
						assert(res);
					}
					res = builder.WriteEndObject();
					assert(res);
					JSONDocument update_document =
						builder.GetJSONDocument();
					builder.Reset();
					res = builder.WriteStartObject();
					assert(res);
					for (const auto &itr :
					     new_document.Items()) {
						if (update_document.Contains(
							    itr.first)) {
							res = builder.WriteKeyValue(
								itr.first,
								update_document
					...
					return Status::InvalidArgument(
						"Can't understand update command");
				}
			}
			...
			if (!new_document.Contains(kPrimaryKey)) {
				return Status::Corruption(
					"Corrupted document -- primary key missing");
			}
			...
			for (const auto &iter : name_to_index_) {
				...
				iter.second.index->GetIndexKey(old_document,
							       &old_key);
				iter.second.index->GetIndexKey(new_document,
							       &new_key);
				if (old_key == new_key) {
				...
				IndexKey old_index_key(Slice(old_key),
						       primary_key_slice);
				IndexKey new_index_key(Slice(new_key),
						       primary_key_slice);

				batch.Delete(iter.second.column_family,
					     old_index_key.GetSliceParts());
				batch.Put(iter.second.column_family,
					  new_index_key.GetSliceParts(),
					  SliceParts());
			}
		}
			      ...
			      const JSONDocument &query) override
	{
		...
		if (!query.IsArray()) {
			return new CursorError(Status::InvalidArgument(
				"Query has to be an array"));
		}
		...
		for (size_t i = 0; i < query.Count(); ++i) {
			...
			if (command.first == "$filter") {
				cursor = ConstructFilterCursor(
					read_options, cursor, command.second);
			} else {
		...
		if (cursor == nullptr) {
			cursor = new CursorFromIterator(DocumentDB::NewIterator(
				      ...
				      const JSONDocument &query)
	{
		...
		if (filter.get() == nullptr) {
			return new CursorError(
				Status::InvalidArgument("Invalid query"));
		}
		...
		if (cursor == nullptr) {
			...
			if (index_column_family != nullptr &&
			    index_column_family->index->UsefulIndex(
				    *filter.get())) {
				std::vector<Iterator *> iterators;
				Status s = DocumentDB::NewIterators(
					read_options,
					{ primary_key_column_family_,
					  index_column_family->column_family },
					&iterators);
				if (!s.ok()) {
					delete cursor;
				...
				return new CursorWithFilterIndexed(
	...
	struct IndexColumnFamily {
		IndexColumnFamily(Index *_index,
...
Options GetRocksDBOptionsFromOptions(const DocumentDBOptions &options)
{
	...
	rocksdb_options.max_background_compactions =
			...
			DocumentDB **db, bool read_only)
{
	...
	for (const auto &index : indexes) {
		column_families.emplace_back(
			InternalSecondaryIndexName(index.name),
			ColumnFamilyOptions(rocksdb_options));
	}
	...
	Status s;
	if (read_only) {
		s = DB::OpenForReadOnly(DBOptions(rocksdb_options), name,
					column_families, &handles, &base_db);
	} else {
		s = DB::Open(DBOptions(rocksdb_options), name, column_families,
			     &handles, &base_db);
	}
	if (!s.ok()) {
		return s;
	...
	std::vector<std::pair<Index *, ColumnFamilyHandle *> > index_cf(
		indexes.size());
	assert(handles.size() == indexes.size() + 1);
	for (size_t i = 0; i < indexes.size(); ++i) {
		auto index = Index::CreateIndexFromDescription(
			*indexes[i].description, indexes[i].name);
		index_cf[i] = { index, handles[i + 1] };
	}
	*db = new DocumentDBImpl(base_db, handles[0], index_cf,
				 rocksdb_options);
	return Status::OK();
}
