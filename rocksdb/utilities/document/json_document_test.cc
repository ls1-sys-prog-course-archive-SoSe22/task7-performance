...
class JSONDocumentTest : public testing::Test {
	...
	void AssertSampleJSON(const JSONDocument &json)
	{
		...
		AssertField(json["properties"]["flags"][2], "tag",
			    std::string("no"));
		AssertField(json["properties"]["flags"][2],
			    std::string("status"));
		AssertField(json["properties"], "age", 110.5e-4);
		AssertField(json["properties"], "depth",
...
TEST_F(JSONDocumentTest, OperatorEqualsTest)
{
	...
	unique_ptr<JSONDocument> arrayWithInt8Doc(
	...
	ASSERT_TRUE((*arrayWithInt8Doc)[0] ==
		    JSONDocument(static_cast<int64_t>(8)));

	unique_ptr<JSONDocument> arrayWithInt16Doc(
...
{
	unique_ptr<JSONDocument> parsedArray(JSONDocument::ParseJSON(
		"[1, [123, \"a\", \"b\"], {\"b\":\"c\"}]"));
...
{
	fprintf(stderr,
		"SKIPPED as JSONDocument is not supported in ROCKSDB_LITE\n");
	return 0;
}
