		      ...
		      Func f)
{
	...
	memcpy(buf, writer.getOutput()->getBuffer(),
		...
		std::bind(
			[](fbson::FbsonWriter &writer,
			   const std::string &str) -> uint32_t {
				...
				return bytesWritten +
				       static_cast<uint32_t>(str.empty());
			},
			_1, s));
}
...
void CreateArray(std::unique_ptr<char[]> *data, fbson::FbsonValue **value)
{
	...
	memcpy(data->get(), writer.getOutput()->getBuffer(),
	       writer.getOutput()->getSize());
	*value = reinterpret_cast<fbson::FbsonDocument *>(data->get())
			 ->getValue();
}
...
JSONDocument::JSONDocument(Type _type)
{
	...
	case kBool:
		InitJSONDocument(&data_, &value_,
				 std::bind(&fbson::FbsonWriter::writeBool, _1,
					   false));
		break;
	case kDouble:
		InitJSONDocument(&data_, &value_,
				 std::bind(&fbson::FbsonWriter::writeDouble, _1,
					   0.));
		break;
	case kArray:
	...
	case kInt64:
		InitJSONDocument(&data_, &value_,
				 std::bind(&fbson::FbsonWriter::writeInt64, _1,
					   0));
		break;
	case kString:
...
JSONDocument::const_item_iterator::const_item_iterator(Impl *impl) : it_(impl)
{
}
...
JSONDocument::ItemsIteratorGenerator::begin() const
{
	return const_item_iterator(
		new const_item_iterator::Impl(object_.begin()));
}
...
{
	return const_item_iterator(
		new const_item_iterator::Impl(object_.end()));
}
