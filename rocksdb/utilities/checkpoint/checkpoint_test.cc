...
TEST_F(CheckpointTest, CheckpointCF)
{
	...
	CreateAndReopenWithCF({ "one", "two", "three", "four", "five" },
			      options);
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
	...
	for (size_t i = 0; i < cfs.size(); ++i) {
		column_families.push_back(
			ColumnFamilyDescriptor(cfs[i], options));
	}
...
TEST_F(CheckpointTest, CurrentFileModifiedWhileCheckpointing2PC)
{
	...
	for (int i = 1; i <= 100000; i++) {
		Transaction *tx =
			txdb->BeginTransaction(write_options, txn_options);
	...
	for (auto &file : files) {
		...
		if (ParseFileName(file, &num, &type, &log_type) &&
		    type == kLogFile) {
			num_log_files++;
		}
	}
	...
	ASSERT_EQ(value, "bar");
	ASSERT_OK(
		snapshotDB->Get(read_options, cf_handles[1], "foocfa", &value));
