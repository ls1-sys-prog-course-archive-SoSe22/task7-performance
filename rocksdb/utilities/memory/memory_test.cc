...
class MemoryTest : public testing::Test {
	...
	void UpdateUsagesHistory(const std::vector<DB *> &dbs)
	{
		...
		for (int i = 0; i < MemoryUtil::kNumUsageTypes; ++i) {
			usage_history_[i].push_back(
				usage_by_type[static_cast<MemoryUtil::UsageType>(
					i)]);
		}
	}
		...
		std::unordered_set<const Cache *> *cache_set)
	{
		...
		if (bbtf != nullptr) {
			...
			cache_set->insert(
				bbt_opts.block_cache_compressed.get());
		}
	}
			      ...
			      std::unordered_set<const Cache *> *cache_set)
	{
		...
		for (auto *db : dbs) {
			...
			if (db_impl != nullptr) {
				ASSERT_OK(
					db_impl->TEST_GetAllImmutableCFOptions(
						&iopts_map));
			}
			for (auto pair : iopts_map) {
				GetCachePointersFromTableFactory(
					pair.second->table_factory, cache_set);
			}
		}
	}
		...
		std::map<MemoryUtil::UsageType, uint64_t> *usage_by_type)
	{
		...
		return MemoryUtil::GetApproximateMemoryUsageByType(
...
TEST_F(MemoryTest, SharedBlockCacheTotal)
{
	...
	for (int p = 0; p < opt.min_write_buffer_number_to_merge / 2; ++p) {
		for (int i = 0; i < kNumDBs; ++i) {
			for (int j = 0; j < 100; ++j) {
				keys_by_db[i].emplace_back(
					RandomString(kKeySize));
				dbs[i]->Put(WriteOptions(),
					    keys_by_db[i].back(),
	...
	for (size_t i = 1;
		...
		ASSERT_EQ(
			usage_history_[MemoryUtil::kTableReadersTotal][i],
...
TEST_F(MemoryTest, MemTableAndTableReadersTotal)
{
	...
	for (size_t i = 1;
		...
		ASSERT_GT(
			usage_history_[MemoryUtil::kMemTableUnFlushed][i],
	...
	size_t usage_check_point =
		usage_history_[MemoryUtil::kMemTableTotal].size();
	...
	for (int i = 0; i < kNumDBs; ++i) {
		...
		for (int j = 0; j < 100; ++j) {
			std::string value;
			dbs[i]->Get(ReadOptions(), RandomString(kKeySize),
				    &value);
		}
