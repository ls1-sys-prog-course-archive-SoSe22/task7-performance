...
class ColumnAwareEncodingExp {
	...
	static void Run(const std::string &sst_file)
	{
		...
		KVPairColDeclarations kvp_cd(key_col_declarations,
		...
		if (!FLAGS_dump_file.empty()) {
			...
			reader.DumpDataColumns(FLAGS_dump_file, kvp_cd,
					       kv_pair_blocks);
			return;
		}
		...
		CompressionType compression_type =
			compressions[FLAGS_compression_type];
		...
		if (CompressionTypeSupported(compression_type)) {
			...
			if (!FLAGS_encoded_file.empty()) {
				env->NewWritableFile(FLAGS_encoded_file,
						     &encoded_out_file,
						     env_options);
			}
			...
			if (FLAGS_format == "col") {
				reader.EncodeBlocks(
					kvp_cd, encoded_out_file.get(),
					compression_type, kv_pair_blocks,
					&encoded_blocks, FLAGS_stat);
			} else { // row format
				reader.EncodeBlocksToRowFormat(
					encoded_out_file.get(),
					compression_type, kv_pair_blocks,
					&encoded_blocks);
			}
			...
			if (decode) {
				...
				if (FLAGS_format == "col") {
					reader.DecodeBlocks(
						kvp_cd, decoded_out_file.get(),
						&encoded_blocks);
				} else {
					reader.DecodeBlocksFromRowFormat(
						decoded_out_file.get(),
						&encoded_blocks);
				}
				uint64_t decode_time =
					sw.ElapsedNanosSafe(true /* reset */);
