...
const std::string &ColBufEncoder::GetData()
{
	return buffer_;
}

ColBufEncoder *
ColBufEncoder::NewColBufEncoder(const ColDeclaration &col_declaration)
{
	if (col_declaration.col_type == "FixedLength") {
		...
		return new VariableChunkColBufEncoder(
			col_declaration.col_compression_type);
	} else if (col_declaration.col_type == "LongFixedLength") {
		return new LongFixedLengthColBufEncoder(
	...
	if (IsRunLength(col_compression_type_)) {
		if (run_length_ == -1) {
			...
			if (col_compression_type_ == kColRle) {
				buffer_.append(
					reinterpret_cast<char *>(&run_val_),
					size_);
			} else {
		...
		if (col_compression_type_ == kColNoCompression) {
			buffer_.append(reinterpret_cast<char *>(&write_val),
				       size_);
		} else {
...
void FixedLengthColBufEncoder::Finish()
{
	...
	if (IsRunLength(col_compression_type_)) {
		...
		if (col_compression_type_ == kColRle) {
			buffer_.append(reinterpret_cast<char *>(&run_val_),
				       size_);
		} else {
			PutVarint64(&buffer_, run_val_);
		}
...
size_t VariableChunkColBufEncoder::Append(const char *buf)
{
	...
	while (mark == 0xFF) {
		...
		if (col_compression_type_ == kColDict) {
			...
			tmp_buffer.append(reinterpret_cast<char *>(&val),
					  chunk_size);
		}
		length += chunk_size;
	}
