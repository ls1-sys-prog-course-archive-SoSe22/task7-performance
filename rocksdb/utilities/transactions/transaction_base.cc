...
TransactionBaseImpl::TransactionBaseImpl(DB *db,
	...
	: db_(db), dbimpl_(reinterpret_cast<DBImpl *>(db)),
	  write_options_(write_options),
	  cmp_(GetColumnFamilyUserComparator(db->DefaultColumnFamily())),
	  start_time_(db_->GetEnv()->NowMicros()),
	  write_batch_(cmp_, 0, true, 0), indexing_enabled_(true)
{
...
void TransactionBaseImpl::SetSnapshot()
{
	const Snapshot *snapshot =
		dbimpl_->GetSnapshotForWriteConflictBoundary();
...
void TransactionBaseImpl::SetSnapshotInternal(const Snapshot *snapshot)
{
	...
	snapshot_.reset(snapshot,
			std::bind(&TransactionBaseImpl::ReleaseSnapshot, this,
				  std::placeholders::_1, db_));
...
Status TransactionBaseImpl::RollbackToSavePoint()
{
	if (save_points_ != nullptr && save_points_->size() > 0) {
		...
		TransactionBaseImpl::SavePoint &save_point =
			save_points_->top();
		...
		for (const auto &key_map_iter : key_map) {
			...
			for (const auto &key_iter : keys) {
				...
				uint32_t num_writes =
					key_iter.second.num_writes;

				auto tracked_keys_iter =
					cf_tracked_keys.find(key);
				assert(tracked_keys_iter !=
				       cf_tracked_keys.end());

				// Decrement the total reads/writes of this key by the number of
				// reads/writes done since the last SavePoint.
				if (num_reads > 0) {
					assert(tracked_keys_iter->second
						       .num_reads >= num_reads);
					tracked_keys_iter->second.num_reads -=
						num_reads;
				}
				if (num_writes > 0) {
					assert(tracked_keys_iter->second
...
{
	return write_batch_.GetFromBatchAndDB(db_, read_options, column_family,
					      key, value);
}
	...
	const std::vector<Slice> &keys, std::vector<std::string> *values)
{
	...
	for (size_t i = 0; i < num_keys; ++i) {
		...
		stat_list[i] =
			Get(read_options, column_family[i], keys[i], value);
	}
				   ...
				   bool exclusive)
{
	...
	if (save_points_ != nullptr && !save_points_->empty()) {
		...
		TrackKey(&save_points_->top().new_keys_, cfh_id, key, seq,
			 read_only, exclusive);
	}
}
				   ...
				   bool read_only, bool exclusive)
{
	...
	if (iter == cf_key_map.end()) {
		auto result =
			cf_key_map.insert({ key, TransactionKeyMapInfo(seq) });
		iter = result.first;
	...
	if (save_points_ != nullptr && !save_points_->empty()) {
		...
		for (const auto &key_map_iter : save_points_->top().new_keys_) {
			...
			for (const auto &key_iter : keys) {
				...
				assert(total_key_info->second.num_reads >=
				       num_reads);
				assert(total_key_info->second.num_writes >=
				       num_writes);

				if (total_key_info->second.num_reads ==
					    num_reads &&
				    total_key_info->second.num_writes ==
					    num_writes) {
					// All the reads/writes to this key were done in the last savepoint.
					bool read_only = (num_writes == 0);
					TrackKey(result, column_family_id, key,
						 key_iter.second.seq, read_only,
						 key_iter.second.exclusive);
				}
			}
		}
					   ...
					   const Slice &key)
{
	...
	if (save_points_ != nullptr && !save_points_->empty()) {
		...
		auto &cf_savepoint_keys =
			save_points_->top().new_keys_[column_family_id];
...
Status TransactionBaseImpl::RebuildFromWriteBatch(WriteBatch *src_batch)
{
	struct IndexedWriteBatchBuilder : public WriteBatch::Handler {
		...
		IndexedWriteBatchBuilder(Transaction *txn, DBImpl *db)
			: txn_(txn), db_(db)
		{
			assert(dynamic_cast<TransactionBaseImpl *>(txn_) !=
			       nullptr);
		}
		...
		{
			return txn_->Put(db_->GetColumnFamilyHandle(cf), key,
					 val);
		}

		Status DeleteCF(uint32_t cf, const Slice &key) override
		{
			return txn_->Delete(db_->GetColumnFamilyHandle(cf),
					    key);
		}

		Status SingleDeleteCF(uint32_t cf, const Slice &key) override
		{
			return txn_->SingleDelete(
				db_->GetColumnFamilyHandle(cf), key);
		}

		Status MergeCF(uint32_t cf, const Slice &key,
			       const Slice &val) override
		{
			return txn_->Merge(db_->GetColumnFamilyHandle(cf), key,
					   val);
		}
