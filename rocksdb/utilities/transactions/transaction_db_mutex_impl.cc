...
{
	return std::shared_ptr<TransactionDBMutex>(
...
{
	return std::shared_ptr<TransactionDBCondVar>(
...
{
	auto mutex_impl =
		reinterpret_cast<TransactionDBMutexImpl *>(mutex.get());
...
Status
TransactionDBCondVarImpl::WaitFor(std::shared_ptr<TransactionDBMutex> mutex,
