...
TransactionImpl::TransactionImpl(TransactionDB *txn_db,
	...
	: TransactionBaseImpl(txn_db->GetRootDB(), write_options),
	  txn_db_impl_(nullptr), txn_id_(0), waiting_cf_id_(0),
	  waiting_key_(nullptr), expiration_time_(0), lock_timeout_(0),
	  deadlock_detect_(false), deadlock_detect_depth_(0)
{
...
void TransactionImpl::Initialize(const TransactionOptions &txn_options)
{
	...
	if (lock_timeout_ < 0) {
		...
		lock_timeout_ = txn_db_impl_->GetTxnDBOptions()
...
Status TransactionImpl::CommitBatch(WriteBatch *batch)
{
	...
	if (IsExpired()) {
		...
		can_commit = std::atomic_compare_exchange_strong(
...
Status TransactionImpl::Prepare()
{
	...
	if (expiration_time_ > 0) {
		...
		TransactionState expected = STARTED;
		can_prepare = std::atomic_compare_exchange_strong(
			&txn_state_, &expected, AWAITING_PREPARE);
	} else if (txn_state_ == STARTED) {
	...
	if (can_prepare) {
		...
		WriteBatchInternal::MarkEndPrepare(
			GetWriteBatch()->GetWriteBatch(), name_);
		...
		s = Status::InvalidArgument(
			"Transaction has already been rolledback.");
	} else {
		s = Status::InvalidArgument(
			"Transaction is not in state for commit.");
	}
...
Status TransactionImpl::RollbackToSavePoint()
{
	if (txn_state_ != STARTED) {
		return Status::InvalidArgument(
			"Transaction is beyond state for rollback.");
	}
				  ...
				  TransactionKeyMap *keys_to_unlock)
{
	...
	for (const auto &cf_iter : handler.keys_) {
		...
		for (const auto &key_iter : cfh_keys) {
			...
			TrackKey(keys_to_unlock, cfh_id, std::move(key),
				...
				bool exclusive, bool untracked)
{
	...
	if (untracked || snapshot_ == nullptr) {
		...
		if (s.ok()) {
			s = ValidateSnapshot(column_family, key, current_seqno,
					     &new_seqno);
					 ...
					 SequenceNumber *new_seqno)
{
	...
	*new_seqno = seq;
	...
	return TransactionUtil::CheckKeyForConflicts(
		db_impl, cfh, key.ToString(), snapshot_->GetSequenceNumber(),
		false /* cache_only */);
}
...
void TransactionImpl::UnlockGetForUpdate(ColumnFamilyHandle *column_family,
					 const Slice &key)
{
	txn_db_impl_->UnLock(this, GetColumnFamilyID(column_family),
			     key.ToString());
}
...
Status TransactionImpl::SetName(const TransactionName &name)
{
	...
	if (txn_state_ == STARTED) {
		if (name_.length()) {
			s = Status::InvalidArgument(
				"Transaction has already been named.");
		} else if (txn_db_impl_->GetTransactionByName(name) !=
			   nullptr) {
			s = Status::InvalidArgument(
