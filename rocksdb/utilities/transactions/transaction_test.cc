...
class TransactionTest : public ::testing::TestWithParam<bool> {
	...
	Status OpenWithStackableDB()
	{
		...
		Status s = DB::Open(options_copy, dbname, column_families,
				    &handles, &root_db);
		if (s.ok()) {
...
TEST_P(TransactionTest, WaitingTxn)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		...
		[&](void *arg) {
			std::string key;
			uint32_t cf_id;
			std::vector<TransactionID> wait =
				txn2->GetWaitingTxns(&cf_id, &key);
			ASSERT_EQ(key, "foo");
...
TEST_P(TransactionTest, SharedLocks)
{
	...
	// correctly.
	std::vector<TransactionID> expected_txns = { txn1->GetID(),
						     txn2->GetID(),
						     txn3->GetID() };
...
TEST_P(TransactionTest, DeadlockCycleShared)
{
	...
	for (uint32_t i = 15; i < 31; i++) {
		auto s = txns[i]->GetForUpdate(read_options, "0", nullptr,
					       true /* exclusive */);
...
TEST_P(TransactionTest, DeadlockCycle)
{
	...
	for (uint32_t len = 2; len < kMaxCycleLength; len++) {
		...
		for (uint32_t i = 0; i < len; i++) {
			txns[i] = db->BeginTransaction(write_options,
						       txn_options);
			ASSERT_TRUE(txns[i]);
			auto s = txns[i]->GetForUpdate(read_options,
						       ToString(i), nullptr);
		...
		for (uint32_t i = 0; i < len - 1; i++) {
			std::function<void()> blocking_thread = [&, i] {
				auto s = txns[i]->GetForUpdate(
					read_options, ToString(i + 1), nullptr);
				ASSERT_OK(s);
				txns[i]->Rollback();
				delete txns[i];
			};
		...
		auto s =
			txns[len - 1]->GetForUpdate(read_options, "0", nullptr);
		ASSERT_TRUE(s.IsDeadlock());
...
TEST_P(TransactionTest, TwoPhaseMultiThreadTest)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"WriteThread::JoinBatchGroup:Wait", [&](void *arg) {
			auto *writer =
				reinterpret_cast<WriteThread::Writer *>(arg);

			if (writer->ShouldWriteToWAL()) {
				t_wait_on_prepare.fetch_add(1);
				// wait for friends
				while (t_wait_on_prepare.load() <
				       NUM_TXN_THREADS) {
	...
	for (size_t n = 0; n < num_tests; n++) {
		...
		case 1:
			options.table_factory.reset(
				new mock::MockTableFactory());
			break;
		case 2: {
			PlainTableOptions pt_opts;
			pt_opts.hash_table_ratio = 0;
			options.table_factory.reset(
				NewPlainTableFactory(pt_opts));
			break;
		}
		}
		...
		for (int i = 0; i < 1000; i++) {
			s = db->Put(write_options, std::to_string(i),
				    test::CompressibleString(&rnd, 0.8, 100,
							     &value));
			ASSERT_OK(s);
		}
...
TEST_P(TransactionTest, ColumnFamiliesTest)
{
	...
	std::vector<ColumnFamilyHandle *> multiget_cfh = {
		handles[1], handles[2], handles[0], handles[2]
	};
...
TEST_P(TransactionTest, PredicateManyPreceders)
{
	...
	std::vector<Status> results = txn1->MultiGetForUpdate(
		read_options1, multiget_keys, &multiget_values);
	...
	results = txn1->MultiGetForUpdate(read_options1, multiget_keys,
					  &multiget_values);
	ASSERT_TRUE(results[1].IsNotFound());
...
TEST_P(TransactionTest, ToggleAutoCompactionTest)
{
	...
	auto cfh_default =
		reinterpret_cast<ColumnFamilyHandleImpl *>(handles[0]);
...
TEST_P(TransactionTest, TransactionStressTest)
{
	...
	std::function<void()> call_inserter = [&] {
		ASSERT_OK(TransactionStressTestInserter(
			db, num_transactions_per_thread, num_sets,
			num_keys_per_set));
	};
