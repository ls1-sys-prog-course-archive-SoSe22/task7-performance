...
struct LockInfo {
	...
	LockInfo(const LockInfo &lock_info)
		: exclusive(lock_info.exclusive), txn_ids(lock_info.txn_ids),
...
void UnrefLockMapsCache(void *ptr)
{
	...
	auto lock_maps_cache = static_cast<
		std::unordered_map<uint32_t, std::shared_ptr<LockMap> > *>(ptr);
...
TransactionLockMgr::TransactionLockMgr(
	...
	: txn_db_impl_(nullptr), default_num_stripes_(default_num_stripes),
...
std::shared_ptr<LockMap>
TransactionLockMgr::GetLockMap(uint32_t column_family_id)
{
	...
	if (!expired && lock_info.expiration_time > 0) {
		...
		*expire_time = lock_info.expiration_time;
		...
		for (auto id : lock_info.txn_ids) {
			...
			bool success =
				txn_db_impl_->TryStealingExpiredTransactionLocks(
					id);
			if (!success) {
				expired = false;
				break;
				   ...
				   bool exclusive)
{
	...
	if (lock_map == nullptr) {
		...
		snprintf(msg, sizeof(msg),
			 "Column family id not found: %" PRIu32,
			 column_family_id);
	...
	return AcquireWithTimeout(txn, lock_map, stripe, column_family_id, key,
				  env, timeout, lock_info);
}
	...
	if (!result.ok() && timeout != 0) {
		...
		do {
			...
			if (expire_time_hint > 0 &&
			    (timeout < 0 ||
			     (timeout > 0 && expire_time_hint < end_time))) {
			...
			if (wait_ids.size() != 0) {
				...
				txn->SetWaitingTxn(wait_ids, column_family_id,
						   &key);
			}
			...
			if (cv_end_time < 0) {
				// Wait indefinitely
				result = stripe->stripe_cv->Wait(
					stripe->stripe_mutex);
			} else {
				uint64_t now = env->NowMicros();
				if (static_cast<uint64_t>(cv_end_time) > now) {
					result = stripe->stripe_cv->WaitFor(
						stripe->stripe_mutex,
						cv_end_time - now);
				}
			}
			...
			if (result.ok() || result.IsTimedOut()) {
				result = AcquireLocked(lock_map, stripe, key,
						       env, lock_info,
						       &expire_time_hint,
						       &wait_ids);
			}
	...
	const TransactionImpl *txn, const autovector<TransactionID> &wait_ids)
{
	...
	for (int tail = 0, head = 0; head < txn->GetDeadlockDetectDepth();
	     head++) {
		int i = 0;
		if (next_ids) {
	...
	if (stripe->keys.find(key) != stripe->keys.end()) {
		...
		if (lock_info.exclusive || txn_lock_info.exclusive) {
			if (lock_info.txn_ids.size() == 1 &&
				...
				if (IsLockExpired(txn_lock_info.txn_ids[0],
						  lock_info, env,
						  expire_time)) {
					// lock is expired, can steal it
					lock_info.txn_ids =
						txn_lock_info.txn_ids;
					lock_info.exclusive =
		...
		// Check lock limit
		if (max_num_locks_ > 0 &&
		    lock_map->lock_cnt.load(std::memory_order_acquire) >=
			    max_num_locks_) {
				...
				const TransactionKeyMap *key_map, Env *env)
{
	for (auto &key_map_iter : *key_map) {
		...
		std::shared_ptr<LockMap> lock_map_ptr =
			GetLockMap(column_family_id);
		LockMap *lock_map = lock_map_ptr.get();
		...
		for (auto &stripe_iter : keys_by_stripe) {
			...
			assert(lock_map->lock_map_stripes_.size() > stripe_num);
			LockMapStripe *stripe =
				lock_map->lock_map_stripes_.at(stripe_num);
