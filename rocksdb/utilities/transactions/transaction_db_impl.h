...
class TransactionDBImpl : public TransactionDB {
	...
	Status
	Initialize(const std::vector<size_t> &compaction_enabled_cf_indices,
	...
	Status TryLock(TransactionImpl *txn, uint32_t cfh_id,
		       const std::string &key, bool exclusive);

	void UnLock(TransactionImpl *txn, const TransactionKeyMap *keys);
	void UnLock(TransactionImpl *txn, uint32_t cfh_id,
		    const std::string &key);

	void AddColumnFamily(const ColumnFamilyHandle *handle);

	static TransactionDBOptions
	ValidateTxnDBOptions(const TransactionDBOptions &txn_db_options);
	...
	void
	GetAllPreparedTransactions(std::vector<Transaction *> *trans) override;
