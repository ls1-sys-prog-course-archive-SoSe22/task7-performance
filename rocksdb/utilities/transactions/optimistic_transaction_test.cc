...
class OptimisticTransactionTest : public testing::Test {
	...
	{
		Status s =
			OptimisticTransactionDB::Open(options, dbname, &txn_db);
		assert(s.ok());
...
TEST_F(OptimisticTransactionTest, ColumnFamiliesTest)
{
	...
	s = OptimisticTransactionDB::Open(options, dbname, column_families,
					  &handles, &txn_db);
	ASSERT_OK(s);
	assert(txn_db != nullptr);
	db = txn_db->GetBaseDB();
	...
	txn->Put(handles[2], SliceParts(&key_slice, 1),
	...
	txn2->Put(handles[1], SliceParts(key_slices, 3),
		  SliceParts(&value_slice, 1));

	txn2->Delete(handles[2], "XXX");
	txn2->Delete(handles[1], "XXX");
	s = txn2->GetForUpdate(snapshot_read_options, handles[1], "AAA",
			       &value);
	ASSERT_TRUE(s.IsNotFound());
					       ...
					       const size_t num_keys_per_set)
{
	...
	for (size_t t = 0; t < num_transactions; t++) {
		bool success =
			inserter.OptimisticTransactionDBInsert(db, txn_options);
		if (!success) {
			// unexpected failure
...
TEST_F(OptimisticTransactionTest, OptimisticTransactionStressTest)
{
	...
	std::function<void()> call_inserter = [&] {
		ASSERT_OK(OptimisticTransactionStressTestInserter(
			txn_db, num_transactions_per_thread, num_sets,
			num_keys_per_set));
	};
