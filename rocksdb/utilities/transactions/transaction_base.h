...
class TransactionBaseImpl : public Transaction {
	...
	// MergeUntracked.
	virtual Status TryLock(ColumnFamilyHandle *column_family,
	...
	Status GetForUpdate(const ReadOptions &options, const Slice &key,
			    std::string *value, bool exclusive) override
	{
		return GetForUpdate(options, db_->DefaultColumnFamily(), key,
				    value, exclusive);
	}
	...
	virtual void SetLockTimeout(int64_t timeout) override
	{ /* Do nothing */
	}
	...
	void SetSnapshotOnNextOperation(std::shared_ptr<TransactionNotifier>
						notifier = nullptr) override;
	...
	void TrackKey(uint32_t cfh_id, const std::string &key,
		      SequenceNumber seqno, bool readonly, bool exclusive);
	...
	struct SavePoint {
		...
		SavePoint(std::shared_ptr<const Snapshot> snapshot,
			...
			: snapshot_(snapshot),
			  ...
			  num_puts_(num_puts), num_deletes_(num_deletes),
