...
TransactionDBImpl::TransactionDBImpl(DB *db,
	...
	: TransactionDB(db), db_impl_(dynamic_cast<DBImpl *>(db)),
	  txn_db_options_(txn_db_options),
	  lock_mgr_(this, txn_db_options_.num_stripes,
		    txn_db_options.max_num_locks,
...
TransactionDBImpl::TransactionDBImpl(StackableDB *db,
				     const TransactionDBOptions &txn_db_options)
	: TransactionDB(db), db_impl_(dynamic_cast<DBImpl *>(db->GetRootDB())),
	  txn_db_options_(txn_db_options),
	  lock_mgr_(this, txn_db_options_.num_stripes,
		    txn_db_options.max_num_locks,
		    txn_db_options_.custom_mutex_factory ?
	...
	const std::vector<ColumnFamilyHandle *> &handles)
{
	...
	compaction_enabled_cf_handles.reserve(
		compaction_enabled_cf_indices.size());
	...
	for (auto it = rtrxs.begin(); it != rtrxs.end(); it++) {
		...
		Transaction *real_trx =
			BeginTransaction(w_options, t_options, nullptr);
		assert(real_trx);
...
Transaction *
TransactionDBImpl::BeginTransaction(const WriteOptions &write_options,
	...
	std::vector<ColumnFamilyHandle *> *handles, TransactionDB **dbptr)
{
	...
	std::vector<ColumnFamilyDescriptor> column_families_copy =
		column_families;
	std::vector<size_t> compaction_enabled_cf_indices;
	DBOptions db_options_2pc = db_options;
	PrepareWrap(&db_options_2pc, &column_families_copy,
		    &compaction_enabled_cf_indices);
	s = DB::Open(db_options_2pc, dbname, column_families_copy, handles,
		     &db);
	if (s.ok()) {
		s = WrapDB(db, txn_db_options, compaction_enabled_cf_indices,
			   *handles, dbptr);
	}
	return s;
}
...
Status
TransactionDBImpl::CreateColumnFamily(const ColumnFamilyOptions &options,
...
Transaction *
TransactionDBImpl::BeginInternalTransaction(const WriteOptions &options)
{
...
bool TransactionDBImpl::TryStealingExpiredTransactionLocks(TransactionID tx_id)
{
	std::lock_guard<std::mutex> lock(map_mutex_);
...
Transaction *
TransactionDBImpl::GetTransactionByName(const TransactionName &name)
{
