...
struct BlobDBOptions {
	...
	// evicted.
	std::function<void(const ColumnFamilyHandle *, const Slice &,
			   const Slice &)>
		gc_evict_cb_fn;
...
class BlobDB : public StackableDB {
	...
	{
		return PutWithTTL(options, DefaultColumnFamily(), key, value,
				  ttl);
	}
	...
	{
		return PutUntil(options, DefaultColumnFamily(), key, value,
				expiration);
	}
	...
	static Status
	Open(const DBOptions &db_options, const BlobDBOptions &bdb_options,
	     const std::string &dbname,
	     const std::vector<ColumnFamilyDescriptor> &column_families,
	     std::vector<ColumnFamilyHandle *> *handles, BlobDB **blob_db,
	     bool no_base_db = false);
