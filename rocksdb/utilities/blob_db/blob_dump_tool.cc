...
{
	fprintf(stdout,
		"Read record with offset 0x%" PRIx64 " (%" PRIu64 "):\n",
		*offset, *offset);
	Slice slice;
	...
	uint32_t header_crc = crc32c::Extend(
		0, slice.data(), slice.size() - 2 * sizeof(uint32_t));
	*offset += BlobLogRecord::kHeaderSize;
	s = Read(*offset, key_size + blob_size + BlobLogRecord::kFooterSize,
		 &slice);
	if (!s.ok()) {
		return s;
	}
	...
	uint32_t blob_crc =
		crc32c::Extend(0, slice.data() + key_size, blob_size);
	...
	if (show_key != DisplayType::kNone) {
		...
		if (show_blob != DisplayType::kNone) {
			fprintf(stdout, "  blob       : ");
			DumpSlice(Slice(slice.data() + key_size, blob_size),
				  show_blob);
		}
	}
	Slice footer_slice(slice.data() + record.GetKeySize() +
				   record.GetBlobSize(),
...
template <class T> std::string BlobDumpTool::GetString(std::pair<T, T> p)
{
