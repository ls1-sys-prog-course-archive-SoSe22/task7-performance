...
Status Reader::ReadHeader(BlobLogHeader *header)
{
	...
	assert(next_byte_ == 0);
	Status status = file_->Read(BlobLogHeader::kHeaderSize, &buffer_,
				    GetReadBuffer());
			  ...
			  WALRecoveryMode wal_recovery_mode)
{
	...
	Status status = file_->Read(BlobLogRecord::kHeaderSize, &buffer_,
				    GetReadBuffer());
	next_byte_ += buffer_.size();
	if (!status.ok())
		return status;
	...
	case kReadHdrFooter:
		...
		if (buffer_.size() != BlobLogRecord::kFooterSize) {
			return Status::IOError(
				"EOF reached before record footer");
		}
	...
	case kReadHdrKeyFooter:
		...
		header_crc = crc32c::Extend(header_crc, record->key_.data(),
					    record->GetKeySize());
		header_crc = crc32c::Mask(header_crc);
		if (header_crc != record->header_cksum_) {
			return Status::Corruption(
				"Record Checksum mismatch: header_cksum");
		}
		...
		if (buffer_.size() != BlobLogRecord::kFooterSize) {
			return Status::IOError(
				"EOF reached during footer read");
		}
	...
	case kReadHdrKeyBlobFooter:
		...
		blob_crc = crc32c::Extend(blob_crc, record->blob_.data(),
					  record->blob_.size());
		...
		if (buffer_.size() != BlobLogRecord::kFooterSize) {
			return Status::IOError(
				"EOF reached during blob footer read");
		}
