...
class BlobLogHeader {
	...
	void set_ts_guess(const tsrange_t &ts)
	{
		ts_guess_.reset(new tsrange_t(ts));
	}
...
class BlobLogFooter {
	...
	const snrange_t &GetSNRange() const
	{
		return sn_range_;
	}
...
class BlobLogRecord {
	...
	char *GetKeyBuffer()
	{
		return &(key_buffer_[0]);
	}

	char *GetBlobBuffer()
	{
		return &(blob_buffer_[0]);
	}
	...
	uint32_t footer_checksum() const
	{
		return footer_cksum_;
	}
