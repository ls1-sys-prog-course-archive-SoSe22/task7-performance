...
class Writer {
	...
	static void ConstructBlobHeader(std::string *headerbuf,
	...
	Status EmitPhysicalRecord(const std::string &headerbuf,
				  const Slice &key, const Slice &val,
				  uint64_t *key_offset, uint64_t *blob_offset);
	...
	{
		return block_offset_ > next_sync_offset_;
	}

	void Sync();

	void ResetSyncPointer()
	{
