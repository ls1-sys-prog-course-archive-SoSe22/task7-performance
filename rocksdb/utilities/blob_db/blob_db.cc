...
typedef std::shared_ptr<EvictAllVersionsCompactionListener> CompactionListener_t;
			   ...
			   Options *changed_options)
{
	...
	*blob_db = nullptr;
	...
	ReconcileWalFilter_t rw_filter =
		std::make_shared<BlobReconcileWalFilter>();
		    ...
		    const std::string &dbname, BlobDB **blob_db)
{
	*blob_db = nullptr;
	...
	Status s = BlobDB::Open(db_options, bdb_options, dbname,
				column_families, &handles, blob_db);
	if (s.ok()) {
...
Status BlobDB::Open(const DBOptions &db_options,
		    ...
		    std::vector<ColumnFamilyHandle *> *handles,
		    BlobDB **blob_db, bool no_base_db)
{
	*blob_db = nullptr;
...
BlobDBOptions::BlobDBOptions()
	: blob_dir("blob_dir"), path_relative(true), is_fifo(false),
	  blob_dir_size(1000ULL * 1024ULL * 1024ULL * 1024ULL),
	  ttl_range_secs(3600), min_blob_size(512), bytes_per_sync(0),
	  blob_file_size(256 * 1024 * 1024), num_concurrent_simple_blobs(4),
	  default_ttl_extractor(false), compression(kNoCompression)
{
}
