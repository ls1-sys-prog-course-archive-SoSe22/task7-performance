...
{
BlobFile::BlobFile()
	: parent_(nullptr), file_number_(0), blob_count_(0), gc_epoch_(-1),
	  file_size_(0), deleted_count_(0), deleted_size_(0), closed_(false),
	  can_be_deleted_(false), gc_once_after_open_(false),
	  ttl_range_(std::make_pair(0, 0)), time_range_(std::make_pair(0, 0)),
	  sn_range_(std::make_pair(0, 0)), last_access_(-1), last_fsync_(0),
	  header_valid_(false)
{
}

BlobFile::BlobFile(const BlobDBImpl *p, const std::string &bdir, uint64_t fn)
	: parent_(p), path_to_dir_(bdir), file_number_(fn), blob_count_(0),
	  gc_epoch_(-1), file_size_(0), deleted_count_(0), deleted_size_(0),
	  closed_(false), can_be_deleted_(false), gc_once_after_open_(false),
	  ttl_range_(std::make_pair(0, 0)), time_range_(std::make_pair(0, 0)),
	  sn_range_(std::make_pair(0, 0)), last_access_(-1), last_fsync_(0),
	  header_valid_(false)
{
...
BlobFile::OpenSequentialReader(Env *env, const DBOptions &db_options,
			       const EnvOptions &env_options) const
{
	...
	std::shared_ptr<Reader> log_reader = std::make_shared<Reader>(
		db_options.info_log, std::move(sfile_reader));
...
Status BlobFile::WriteFooterAndCloseLocked()
{
	...
	if (HasTTL())
		footer.set_ttl_range(ttl_range_);

	footer.sn_range_ = sn_range_;
	if (HasTimestamp())
		footer.set_time_range(time_range_);
...
{
	if (file_size_ <
	    (BlobLogHeader::kHeaderSize + BlobLogFooter::kFooterSize)) {
	...
	Status s = ra_file_reader_->Read(
		footer_offset, BlobLogFooter::kFooterSize, &result, scratch);
	if (!s.ok())
		return s;
...
BlobFile::GetOrOpenRandomAccessReader(Env *env, const EnvOptions &env_options,
				      bool *fresh_open)
{
	*fresh_open = false;
	last_access_ = std::chrono::system_clock::to_time_t(
	...
	ra_file_reader_ =
		std::make_shared<RandomAccessFileReader>(std::move(rfile));
	*fresh_open = true;
	return ra_file_reader_;
}
