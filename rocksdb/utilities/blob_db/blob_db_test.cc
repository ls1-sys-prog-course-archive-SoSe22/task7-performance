...
class BlobDBTest : public testing::Test {
	...
	BlobDBTest()
		: dbname_(test::TmpDir() + "/blob_db_test"), blob_db_(nullptr)
	{
		    ...
		    Options options = Options())
	{
		...
		ASSERT_OK(
			BlobDB::Open(options, bdb_options, dbname_, &blob_db_));
	...
	void
	PutRandomWithTTL(const std::string &key, int32_t ttl, Random *rnd,
			 std::map<std::string, std::string> *data = nullptr)
	{
		int len = rnd->Next() % kMaxBlobSize + 1;
		std::string value = test::RandomHumanReadableString(rnd, len);
		ASSERT_OK(blob_db_->PutWithTTL(WriteOptions(), Slice(key),
					       Slice(value), ttl));
		if (data != nullptr) {
			(*data)[key] = value;
		}
	...
	void InsertBlobs()
	{
		...
		for (size_t i = 0; i < 100000; i++) {
			...
			PutRandomWithTTL("key" + ToString(i % 500), ttl, &rnd,
					 nullptr);
		}
...
TEST_F(BlobDBTest, WriteBatch)
{
	...
	for (size_t i = 0; i < 100; i++) {
		...
		for (size_t j = 0; j < 10; j++) {
			PutRandomToWriteBatch("key" + ToString(j * 100 + i),
					      &rnd, &batch, &data);
		}
...
TEST_F(BlobDBTest, Compression)
{
	...
	for (int i = 0; i < 100; i++) {
		WriteBatch batch;
		for (size_t j = 0; j < 10; j++) {
			PutRandomToWriteBatch("write-batch-key" +
						      ToString(j * 100 + i),
					      &rnd, &batch, &data);
		}
		blob_db_->Write(WriteOptions(), &batch);
...
TEST_F(BlobDBTest, SequenceNumber)
{
	...
	for (int i = 0; i < 100; i++) {
		...
		ASSERT_OK(blob_db_impl->TEST_GetSequenceNumber(
			key, &actual_sequence));
	...
	for (int i = 0; i < 100; i++) {
		...
		for (size_t k = 0; k < batch_size; k++) {
			std::string value =
				test::RandomHumanReadableString(&rnd, 1000);
			ASSERT_OK(batch.Put("key" + ToString(i) + "-" +
						    ToString(k),
					    value));
		}
		ASSERT_OK(blob_db_->Write(WriteOptions(), &batch));
