...
class BlobReconcileWalFilter : public WalFilter {
    public:
	virtual WalFilter::WalProcessingOption
...
class EvictAllVersionsCompactionListener : public EventListener {
	...
	class InternalListener : public CompactionEventListener {
	    ...
	    public:
		virtual void
		OnCompaction(int level, const Slice &key,
...
class BlobDBImpl : public BlobDB {
...
#ifndef NDEBUG
	Status TEST_GetSequenceNumber(const Slice &key,
	...
	Status
	GetAllLogFiles(std::set<std::pair<uint64_t, std::string> > *file_nums);
	...
	GetOrOpenRandomAccessReader(const std::shared_ptr<BlobFile> &bfile,
	...
	CheckOrCreateWriterLocked(const std::shared_ptr<BlobFile> &bfile);
	...
	void FilterSubsetOfFiles(
		...
		uint64_t epoch, uint64_t last_id, size_t files_to_collect);
	...
	std::multiset<std::shared_ptr<BlobFile>, blobf_compare_ttl>
		open_blob_files_;
...
class BlobFile {
	...
	BlobFile(const BlobDBImpl *parent, const std::string &bdir,
		 uint64_t fnum);
	...
	std::shared_ptr<Writer> GetWriter() const
	{
		return log_writer_;
	}
	...
	OpenSequentialReader(Env *env, const DBOptions &db_options,
	...
	std::shared_ptr<RandomAccessFileReader>
	GetOrOpenRandomAccessReader(Env *env, const EnvOptions &env_options,
				    bool *fresh_open);
	...
	void set_time_range(const tsrange_t &tr)
	{
		time_range_ = tr;
	}
	...
	void SetSNRange(const snrange_t &snr)
	{
		sn_range_ = snr;
	}

	// The following functions are atomic, and don't need locks
	void SetFileSize(uint64_t fs)
	{
		file_size_ = fs;
	}

	void SetBlobCount(uint64_t bc)
	{
		blob_count_ = bc;
	}
...
class BlobDBIterator : public Iterator {
    public:
	explicit BlobDBIterator(Iterator *iter,
				ColumnFamilyHandle *column_family,
				BlobDBImpl *impl)
