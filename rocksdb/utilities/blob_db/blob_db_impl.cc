...
void extendSN(rocksdb::blob_db::snrange_t *sn_range, rocksdb::SequenceNumber sn)
{
...
struct GCStats {
	...
	GCStats()
		: blob_count(0), num_deletes(0), deleted_size(0), num_relocs(0),
		  succ_deletes_lsm(0), succ_relocs(0)
	{
	}
};
...
class BlobHandle {
	...
	void set_filenumber(uint64_t fn)
	{
		file_number_ = fn;
	}
	...
	void set_compression(CompressionType t)
	{
		compression_ = t;
	}
		...
		if (s.ok()) {
			if (impl_->debug_level_ >= 3)
				Log(InfoLogLevel::INFO_LEVEL,
				    ...
				    handle.filenumber(), handle.offset(),
				    handle.size());

			impl_->override_vals_q_.enqueue(
				{ handle.filenumber(), key.size(),
...
BlobDBImpl::BlobDBImpl(const std::string &dbname,
	...
	: BlobDB(nullptr), db_impl_(nullptr), myenv_(db_options.env),
	  wo_set_(false), bdb_options_(blob_db_options),
	  db_options_(db_options), env_options_(db_options), dir_change_(false),
	  next_file_number_(1), epoch_of_(0), shutdown_(false),
	  current_epoch_(0), open_file_count_(0), last_period_write_(0),
	  last_period_ampl_(0), total_periods_write_(0), total_periods_ampl_(0),
	  total_blob_space_(0), open_p1_done_(false), debug_level_(0)
{
...
BlobDBOptions BlobDBImpl::GetBlobDBOptions() const
{
	return bdb_options_;
}
...
BlobDBImpl::BlobDBImpl(DB *db, const BlobDBOptions &blob_db_options)
	: BlobDB(db), db_impl_(dynamic_cast<DBImpl *>(db)),
	  opt_db_(new OptimisticTransactionDBImpl(db, false)), wo_set_(false),
	  bdb_options_(blob_db_options), db_options_(db->GetOptions()),
	  env_options_(db_->GetOptions()), dir_change_(false),
	  next_file_number_(1), epoch_of_(0), shutdown_(false),
	  current_epoch_(0), open_file_count_(0), last_period_write_(0),
	  last_period_ampl_(0), total_periods_write_(0), total_periods_ampl_(0),
	  total_blob_space_(0)
...
void BlobDBImpl::StartBackgroundTasks()
{
	...
	tqueue_.add(bdb_options_.reclaim_of_period_millisecs,
		    std::bind(&BlobDBImpl::ReclaimOpenFiles, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.gc_check_period_millisecs,
		    std::bind(&BlobDBImpl::RunGC, this, std::placeholders::_1));
	tqueue_.add(bdb_options_.deletion_check_period_millisecs,
		    std::bind(&BlobDBImpl::EvictDeletions, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.deletion_check_period_millisecs,
		    std::bind(&BlobDBImpl::EvictCompacted, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.delete_obsf_period_millisecs,
		    std::bind(&BlobDBImpl::DeleteObsFiles, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.sanity_check_period_millisecs,
		    std::bind(&BlobDBImpl::SanityCheck, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.wa_stats_period_millisecs,
		    std::bind(&BlobDBImpl::WaStats, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.fsync_files_period_millisecs,
		    std::bind(&BlobDBImpl::FsyncFiles, this,
			      std::placeholders::_1));
	tqueue_.add(bdb_options_.check_seqf_period_millisecs,
		    std::bind(&BlobDBImpl::CheckSeqFiles, this,
			      std::placeholders::_1));
}
...
Status BlobDBImpl::OpenAllFiles()
{
	...
	for (auto f_iter : file_nums) {
		...
		if (debug_level_ >= 1)
			Log(InfoLogLevel::INFO_LEVEL, db_options_.info_log,
			    "Blob File open: %s size: %" PRIu64, bfpath.c_str(),
			    size_bytes);

		std::shared_ptr<BlobFile> bfptr = std::make_shared<BlobFile>(
			this, blob_dir_, f_iter.first);
		...
		std::shared_ptr<RandomAccessFileReader> ra_reader =
			GetOrOpenRandomAccessReader(bfptr, myenv_,
						    env_options_);
		...
		if (s1.ok()) {
			...
			Log(InfoLogLevel::INFO_LEVEL, db_options_.info_log,
			    ...
			    sn_range.first, sn_range.second,
			    bfptr->HasTimestamp(), bfptr->HasTTL());

			if (bfptr->HasTTL()) {
				ttl_range.second = std::max(
					ttl_range.second,
					ttl_range.first +
						(uint32_t)bdb_options_
							.ttl_range_secs);
				...
				if (ttl_range.second < epoch_now) {
					Status fstatus =
						CreateWriterLocked(bfptr);
					if (fstatus.ok())
						fstatus =
							bfptr->WriteFooterAndCloseLocked();
					if (!fstatus.ok()) {
						Log(InfoLogLevel::ERROR_LEVEL,
						    db_options_.info_log,
						    "Failed to close Blob File: %s status: '%s'. Skipped",
						    bfpath.c_str(),
						    fstatus.ToString().c_str());
						continue;
					} else {
						Log(InfoLogLevel::ERROR_LEVEL,
						    db_options_.info_log,
						    "Blob File Closed: %s now: %d ttl_range: (%d, %d)",
						    bfpath.c_str(), epoch_now,
						    ttl_range.first,
						    ttl_range.second);
					}
				} else {
...
void BlobDBImpl::CloseRandomAccessLocked(const std::shared_ptr<BlobFile> &bfile)
{
...
BlobDBImpl::GetOrOpenRandomAccessReader(const std::shared_ptr<BlobFile> &bfile,
					Env *env, const EnvOptions &env_options)
{
	bool fresh_open = false;
	auto rar = bfile->GetOrOpenRandomAccessReader(env, env_options,
						      &fresh_open);
	if (fresh_open)
		open_file_count_++;
	return rar;
...
Status BlobDBImpl::CreateWriterLocked(const std::shared_ptr<BlobFile> &bfile)
{
	...
	if (debug_level_ >= 2 && boffset) {
		Log(InfoLogLevel::DEBUG_LEVEL, db_options_.info_log,
		    "Open blob file: %s with offset: %d", fpath.c_str(),
		    boffset);
	}
	...
	bfile->log_writer_ = std::make_shared<Writer>(
		std::move(fwriter), bfile->file_number_,
...
std::shared_ptr<BlobFile>
BlobDBImpl::FindBlobFileLocked(uint32_t expiration) const
{
	...
	if (citr.first == open_blob_files_.end()) {
		...
		return (check->ttl_range_.second < expiration) ? nullptr :
								       check;
	}
...
BlobDBImpl::CheckOrCreateWriterLocked(const std::shared_ptr<BlobFile> &bfile)
{
...
std::shared_ptr<BlobFile> BlobDBImpl::SelectBlobFile()
{
	...
	{
		ReadLock rl(&mutex_);
		if (open_simple_files_.size() ==
		    bdb_options_.num_concurrent_simple_blobs)
			return open_simple_files_
				[val % bdb_options_.num_concurrent_simple_blobs];
	}
...
std::shared_ptr<BlobFile> BlobDBImpl::SelectBlobFileTTL(uint32_t expiration)
{
	...
	uint32_t exp_low = (expiration / bdb_options_.ttl_range_secs) *
			   bdb_options_.ttl_range_secs;
				    ...
				    int32_t *ttl_val)
{
	...
	*ttl_val = -1;
	...
	std::string ttl_exp(
		value.data() + value.size() - BlobDB::kTTLSuffixLength, 4);
	if (ttl_exp != "ttl:")
		return false;
...
Status BlobDBImpl::Write(const WriteOptions &opts, WriteBatch *updates)
{
	class BlobInserter : public WriteBatch::Handler {
				     ...
				     const Slice &value_unc) override
		{
			...
			if (impl_->bdb_options_.extract_ttl_fn) {
				impl_->bdb_options_.extract_ttl_fn(
					value_unc, &newval, &ttl_val);
			} else {
				newval = value_unc;
			}
			...
			if (ttl_val != -1) {
				std::time_t cur_t =
					std::chrono::system_clock::to_time_t(
						std::chrono::system_clock::now());
				expiration =
					ttl_val + static_cast<int32_t>(cur_t);
			}
			std::shared_ptr<BlobFile> bfile =
				(ttl_val != -1) ?
					      ...
					      ((last_file_) ?
						       last_file_ :
						       impl_->SelectBlobFile());
			if (last_file_ && last_file_ != bfile) {
				batch_rewrite_status_ =
					Status::NotFound("too many blob files");
				return batch_rewrite_status_;
			}

			if (!bfile) {
				batch_rewrite_status_ =
					Status::NotFound("blob file not found");
				return batch_rewrite_status_;
			}
			...
			Slice value = impl_->GetCompressedSlice(
				value_unc, &compression_output);

			std::string headerbuf;
			Writer::ConstructBlobHeader(&headerbuf, key, value,
						    expiration, -1);
			std::string index_entry;
			Status st = impl_->AppendBlob(bfile, headerbuf, key,
						      value, &index_entry);
			if (st.ok()) {
			...
			if (!st.ok()) {
				...
				WriteBatchInternal::Put(&updates_blob_,
		...
		{
			WriteBatchInternal::Delete(&updates_blob_,
						   column_family_id, key);
			sequence_++;
			return Status::OK();
		}
					      ...
					      const Slice & /*key*/) override
		{
			batch_rewrite_status_ = Status::NotSupported(
				"Not supported operation in blob db.");
			return batch_rewrite_status_;
		}

		virtual Status MergeCF(uint32_t /*column_family_id*/,
				       const Slice & /*key*/,
	...
	class DeleteBookkeeper : public WriteBatch::Handler {
	    public:
		explicit DeleteBookkeeper(BlobDBImpl *impl,
					...
					const Slice &key) override
		{
			ColumnFamilyHandle *cfh =
				impl_->db_impl_->GetColumnFamilyHandleUnlocked(
					column_family_id);

			impl_->delete_keys_q_.enqueue(
				{ cfh, key.ToString(), sequence_ });
			sequence_++;
			return Status::OK();
		}
			    ...
			    const Slice &value_unc, int32_t expiration)
{
	...
	if (debug_level_ >= 3)
		Log(InfoLogLevel::DEBUG_LEVEL, db_options_.info_log,
		    ">Adding KEY FILE: %s: KEY: %s VALSZ: %d",
...
std::vector<Status>
BlobDBImpl::MultiGet(const ReadOptions &options,
		     ...
		     std::vector<std::string> *values)
{
	...
	auto statuses =
		db_->MultiGet(options, column_family, keys, &values_lsm);
	...
	for (size_t i = 0; i < keys.size(); ++i) {
		...
		Status s =
			CommonGet(cfd, keys[i], values_lsm[i], &((*values)[i]));
		statuses[i] = s;
	}
	return statuses;
}
			     ...
			     SequenceNumber *sequence)
{
	...
	// valid offset.
	if (handle.offset() < (BlobLogHeader::kHeaderSize +
			       BlobLogRecord::kHeaderSize + key.size())) {
		if (debug_level_ >= 2) {
			Log(InfoLogLevel::ERROR_LEVEL, db_options_.info_log,
			    "Invalid blob handle file_number: %" PRIu64
			    " blob_offset: %" PRIu64 " blob_size: %" PRIu64
			    " key: %s",
			    handle.filenumber(), handle.offset(), handle.size(),
			    key.data());
		}
		return Status::NotFound(
			"Blob Not Found, although found in LSM");
	}
	...
	std::shared_ptr<BlobFile> bfile;
	{
		...
		if (hitr == blob_files_.end()) {
			return Status::NotFound(
				"Blob Not Found as blob file missing");
		}
	...
	if (value != nullptr) {
		...
		Slice blob_value;
		s = reader->Read(handle.offset(), handle.size(), &blob_value,
				 buffer);
		if (!s.ok() || blob_value.size() != handle.size()) {
			if (debug_level_ >= 2) {
				Log(InfoLogLevel::ERROR_LEVEL,
				    ...
				    " blob_size: %" PRIu64
				    " read: %d key: %s status: '%s'",
				    bfile->PathName().c_str(), handle.offset(),
				    handle.size(),
				    static_cast<int>(blob_value.size()),
				    key.data(), s.ToString().c_str());
			}
			return Status::NotFound(
				"Blob Not Found as couldnt retrieve Blob");
		}
		...
		if (!s.ok() || !GetFixed32(&crc_slice, &crc_exp)) {
			if (debug_level_ >= 2) {
				Log(InfoLogLevel::ERROR_LEVEL,
				    ...
				    " key: %s status: '%s'",
				    bfile->PathName().c_str(), handle.offset(),
				    handle.size(), key.data(),
				    s.ToString().c_str());
			}
			return Status::NotFound(
				"Blob Not Found as couldnt retrieve CRC");
		}

		uint32_t crc =
			crc32c::Extend(0, blob_value.data(), blob_value.size());
		...
		if (bdb_options_.compression != kNoCompression) {
			...
			s = UncompressBlockContentsForCompressionType(
				...
				bdb_options_.compression, *(cfd->ioptions()));
	...
	if (sequence != nullptr) {
		...
		s = reader->Read(handle.offset() + handle.size(),
				 BlobLogRecord::kFooterSize, &footer_slice,
				 buffer);
		if (!s.ok()) {
			return s;
		}
...
Slice BlobDBIterator::value() const
{
	...
	Status s = db_impl_->CommonGet(cfd, iter_->key(),
				       index_entry.ToString(false), &vpart_);
	return Slice(vpart_);
}
...
std::pair<bool, int64_t> BlobDBImpl::SanityCheck(bool aborted)
{
	...
	for (auto bfile_pair : blob_files_) {
		...
		Log(InfoLogLevel::INFO_LEVEL, db_options_.info_log,
		    "Blob File %s %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64
		    " %d",
		    bfile->PathName().c_str(), bfile->GetFileSize(),
		    bfile->BlobCount(), bfile->deleted_count_,
		    bfile->deleted_size_,
...
BlobDBImpl::CloseSeqWrite(std::shared_ptr<BlobFile> bfile, bool aborted)
{
	{
		WriteLock wl(&mutex_);
	...
	while (override_vals_q_.dequeue(&packet)) {
		...
		if (debug_level_ >= 3)
			Log(InfoLogLevel::INFO_LEVEL, db_options_.info_log,
			    ...
			    packet.dsn_, packet.file_number_,
			    packet.blob_offset_, packet.blob_size_, succ);
	}
	return std::make_pair(true, -1);
}
...
std::pair<bool, int64_t> BlobDBImpl::EvictDeletions(bool aborted)
{
	...
	while (delete_keys_q_.dequeue(&dpacket)) {
		if (last_cfh != dpacket.cfh_) {
			if (!range_del_agg) {
				auto cfhi = reinterpret_cast<
					ColumnFamilyHandleImpl *>(dpacket.cfh_);
				auto cfd = cfhi->cfd();
				range_del_agg.reset(new RangeDelAggregator(
			...
			iter.set(db_impl_->NewInternalIterator(
				&arena, range_del_agg.get(), dpacket.cfh_));
		...
		while (iter->Valid()) {
			if (!bwc->Equal(ExtractUserKey(iter->key()),
					ExtractUserKey(eslice)))
				break;
				      ...
				      GCStats *gcstats)
{
	...
	while (reader->ReadRecord(&record, shallow).ok()) {
		...
		if (no_relocation_ttl || (has_ttl && tt > record.GetTTL())) {
			...
			if (sv == nullptr) {
				Status result = Status::InvalidArgument(
					"Could not access column family 0");
				return result;
			}
			Status s1 = db_impl_->GetLatestSequenceForKey(
				sv, record.Key(), false, &seq,
				&found_record_for_key);
			if (s1.IsNotFound() ||
			    (!found_record_for_key || seq != record.GetSN())) {
				del_this = true;
			}
		...
		if (del_this) {
			...
			Transaction *txn =
				static_cast<OptimisticTransactionDB *>(
					opt_db_.get())
		...
		if (!newfile) {
			...
			dir_change_.store(true);
			blob_files_.insert(std::make_pair(
				newfile->BlobFileNumber(), newfile));
		}
		...
		newfile->blob_count_++;
		newfile->file_size_ +=
			BlobLogRecord::kHeaderSize + record.Key().size() +
			record.Blob().size() + BlobLogRecord::kFooterSize;

		Transaction *txn =
			static_cast<OptimisticTransactionDB *>(opt_db_.get())
				->BeginTransaction(write_options_);
			      ...
			      uint64_t last_id, std::string *reason)
{
	if (bfile->HasTTL()) {
		...
		if (bdb_options_.ttl_range_secs <
			...
			*reason =
				"has ttl but partial expiration not turned on";
			return false;
		}

		ReadLock lockbfile_r(&bfile->mutex_);
		bool ret = ((bfile->deleted_size_ * 100.0 /
			     bfile->file_size_.load()) >
...
std::pair<bool, int64_t> BlobDBImpl::RemoveTimerQ(TimerQueue *tq, bool aborted)
{
	WriteLock wl(&mutex_);
...
std::pair<bool, int64_t>
BlobDBImpl::CallbackEvicts(TimerQueue *tq, std::shared_ptr<BlobFile> bfile,
			   bool aborted)
{
	if (aborted)
		return std::make_pair(false, -1);
	...
	if (succ) {
		Log(InfoLogLevel::DEBUG_LEVEL, db_options_.info_log,
		    "Eviction callbacks completed %s",
		    bfile->PathName().c_str());
	}
	...
	uint64_t last_id, size_t files_to_collect)
{
	...
	for (auto bfile : blob_files) {
		if (files_processed >= files_to_collect)
			break;
		...
		if (bfile->gc_epoch_ != -1 &&
		    (uint64_t)bfile->gc_epoch_ != epoch) {
			continue;
		}
	...
	if (!obsoletes.empty()) {
		...
		for (auto bfile : obsoletes) {
			...
			if (!evict_cb) {
				...
				tq->add(0,
					std::bind(&BlobDBImpl::CallbackEvicts,
						  this,
						  (last_file) ? tq.get() :
								      nullptr,
						  bfile,
		     ...
		     const BlobDBOptions &bdb_options)
{
	...
	Status status;
	std::string blobdir;
	blobdir = (bdb_options.path_relative) ?
				dbname + "/" + bdb_options.blob_dir :
