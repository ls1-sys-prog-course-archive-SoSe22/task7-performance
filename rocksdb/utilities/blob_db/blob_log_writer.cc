...
Writer::Writer(unique_ptr<WritableFileWriter> &&dest, uint64_t log_number,
	...
	: dest_(std::move(dest)), log_number_(log_number),
	  block_offset_(boffset), bytes_per_sync_(bpsync), next_sync_offset_(0),
	  use_fsync_(use_fs), last_elem_type_(kEtNone)
{
				 ...
				 const Slice &val, int32_t ttl, int64_t ts)
{
	...
	header_crc = crc32c::Extend(header_crc, headerbuf->c_str(),
				    headerbuf->size());
