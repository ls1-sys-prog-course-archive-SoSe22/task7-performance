...
ColumnAwareEncodingReader::ColumnAwareEncodingReader(
	...
	: file_name_(file_path), ioptions_(options_),
...
void ColumnAwareEncodingReader::InitTableReader(const std::string &file_path)
{
	...
	shared_ptr<BlockBasedTableFactory> block_table_factory =
		std::dynamic_pointer_cast<BlockBasedTableFactory>(
			options_.table_factory);

	std::unique_ptr<TableReader> table_reader;
	block_table_factory->NewTableReader(
		TableReaderOptions(ioptions_, soptions_, internal_comparator_,
				   /*skip_filters=*/false),
		std::move(file_), file_size, &table_reader,
		/*enable_prefetch=*/false);

	table_reader_.reset(
		dynamic_cast<BlockBasedTable *>(table_reader.release()));
}
	...
	WritableFile *out_file, const std::vector<std::string> *blocks)
{
	...
	for (auto &block : *blocks) {
		...
		CompressionType type = (CompressionType)
			slice_final_with_bit[slice_final_with_bit.size() - 1];
		if (type != kNoCompression) {
			UncompressBlockContents(slice_final_with_bit.c_str(),
						slice_final_with_bit.size() - 1,
		...
		for (size_t i = 0; i < num_key_columns; ++i) {
			...
			key_content_ptr[i] +=
				key_col_bufs[i]->Init(key_content_ptr[i]);
			size_t offset;
		...
		for (size_t i = 0; i < num_value_columns; ++i) {
			...
			value_content_ptr[i] +=
				value_col_bufs[i]->Init(value_content_ptr[i]);
			size_t offset;
			offset = DecodeFixed64(header_content_ptr);
			header_content_ptr += sizeof(size_t);
			col_content_ptr += offset;
		...
		checksum_content_ptr +=
			value_checksum_buf->Init(checksum_content_ptr);

		// Decode block
		char *decoded_content = decoded_content_base;
		for (size_t j = 0; j < num_kv_pairs; ++j) {
			for (size_t i = 0; i < num_key_columns; ++i) {
				key_content_ptr[i] += key_col_bufs[i]->Decode(
					key_content_ptr[i], &decoded_content);
			}
			for (size_t i = 0; i < num_value_columns; ++i) {
				value_content_ptr[i] +=
					value_col_bufs[i]->Decode(
						value_content_ptr[i],
						&decoded_content);
			}
			checksum_content_ptr += value_checksum_buf->Decode(
				checksum_content_ptr, &decoded_content);
		}
	...
	for (auto &block : *blocks) {
		...
		if (type != kNoCompression) {
			UncompressBlockContents(slice_final_with_bit.c_str(),
						...
						&contents, format_version,
						compression_dict, ioptions);
			decoded_content = std::string(contents.data.data(),
						      contents.data.size());
		} else {
		       ...
		       CompressionType *type, std::string *compressed_output)
{
	...
	Slice compression_dict;
	*slice_final = CompressBlock(output_content, compression_opts, type,
	...
	for (auto &kv_pairs : kv_pair_blocks) {
		...
		for (auto &kv_pair : kv_pairs) {
			...
			if (counter >= block_restart_interval) {
				...
				const size_t min_length = std::min(
	...
	std::vector<std::string> *blocks, bool print_column_stat)
{
	...
	for (auto &kv_pairs : kv_pair_blocks) {
		...
		if (print_column_stat) {
			for (size_t i = 0; i < key_col_bufs.size(); ++i) {
				...
				auto type = compression_type;
				std::string compressed_output;
				CompressDataBlock(key_col_bufs[i]->GetData(),
						  &slice_final, &type,
						  &compressed_output);
				out_file->Append(slice_final);
			...
			for (size_t i = 0; i < value_col_bufs.size(); ++i) {
				...
				auto type = compression_type;
				std::string compressed_output;
				CompressDataBlock(value_col_bufs[i]->GetData(),
						  &slice_final, &type,
						  &compressed_output);
				out_file->Append(slice_final);
			...
			auto type = compression_type;
			std::string compressed_output;
			CompressDataBlock(value_checksum_buf->GetData(),
					  &slice_final, &type,
					  &compressed_output);
			out_file->Append(slice_final);
			...
			// Add a bit in the end for decoding
			std::string slice_final_with_bit(
				slice_final.data(), slice_final.size() + 1);
			slice_final_with_bit[slice_final.size()] =
				static_cast<char>(type);
			blocks->push_back(
				std::string(slice_final_with_bit.data(),
					    slice_final_with_bit.size()));
		}
	}
	...
	if (print_column_stat) {
		...
		for (size_t i = 0; i < key_col_sizes.size(); ++i)
			printf("Key col %" ROCKSDB_PRIszt
			       " size: %" ROCKSDB_PRIszt " percentage %lf%%\n",
			       i, key_col_sizes[i],
			       100.0 * key_col_sizes[i] / total_size);
		for (size_t i = 0; i < value_col_sizes.size(); ++i)
			printf("Value col %" ROCKSDB_PRIszt
			       " size: %" ROCKSDB_PRIszt " percentage %lf%%\n",
			       i, value_col_sizes[i],
			       100.0 * value_col_sizes[i] / total_size);
		printf("Value checksum size: %" ROCKSDB_PRIszt
		       " percentage %lf%%\n",
		       value_checksum_size,
	...
	ColDeclaration **value_checksum_declaration)
{
	*key_col_declarations = new std::vector<ColDeclaration>{
		ColDeclaration("FixedLength", ColCompressionType::kColRleVarint,
			       4, false, true),
		ColDeclaration("FixedLength",
			       ColCompressionType::kColRleDeltaVarint, 8, false,
			       true),
		ColDeclaration("FixedLength",
			       ColCompressionType::kColDeltaVarint, 8, false,
			       true),
		ColDeclaration("FixedLength",
			       ColCompressionType::kColDeltaVarint, 8, false,
			       true),
	...
	*value_col_declarations = new std::vector<ColDeclaration>{
		...
		ColDeclaration("FixedLength",
			       ColCompressionType::kColDeltaVarint, 4),
		ColDeclaration("FixedLength", ColCompressionType::kColRleVarint,
			       8)
	};
	*value_checksum_declaration =
		new ColDeclaration("LongFixedLength",
				   ColCompressionType::kColNoCompression, 9,
				   true /* nullable */);
}
	...
	ColDeclaration **value_checksum_declaration)
{
	*key_col_declarations = new std::vector<ColDeclaration>{
		...
		ColDeclaration("FixedLength", ColCompressionType::kColRle, 1),
		ColDeclaration("FixedLength",
			       ColCompressionType::kColDeltaVarint, 4, false,
			       true),
		ColDeclaration("FixedLength",
			       ColCompressionType::kColDeltaVarint, 8, false,
			       true),
		ColDeclaration("FixedLength", ColCompressionType::kColRleVarint,
			       8, false, true),
		ColDeclaration("VariableChunk",
			       ColCompressionType::kColNoCompression),
		ColDeclaration("FixedLength", ColCompressionType::kColRleVarint,
			       8)
	};
	*value_col_declarations = new std::vector<ColDeclaration>();
