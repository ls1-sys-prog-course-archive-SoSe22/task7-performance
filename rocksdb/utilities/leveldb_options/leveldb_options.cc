...
LevelDBOptions::LevelDBOptions()
	: comparator(BytewiseComparator()), create_if_missing(false),
	  ...
	  info_log(nullptr), write_buffer_size(4 << 20), max_open_files(1000),
	  block_cache(nullptr), block_size(4096), block_restart_interval(16),
...
Options ConvertOptions(const LevelDBOptions &leveldb_options)
{
	...
	table_options.block_restart_interval =
		leveldb_options.block_restart_interval;
