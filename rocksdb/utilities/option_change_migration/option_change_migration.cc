	...
	if (old_opts.num_levels <= new_opts.num_levels ||
		...
		bool need_compact = false;
		{
			...
			if (!metadata.levels.empty() &&
			    metadata.levels.back().level >=
				    new_opts.num_levels) {
				need_compact = true;
			}
		}
		if (need_compact) {
			return CompactToLevel(old_opts, dbname,
					      new_opts.num_levels - 1, true);
		}
		return Status::OK();
	}
}
			  ...
			  const Options &new_opts)
{
	if (!new_opts.level_compaction_dynamic_level_bytes) {
		...
		if (old_opts.num_levels == 1) {
			...
			return CompactToLevel(new_opts, dbname,
					      new_opts.num_levels - 1, false);
		} else {
			Options opts = old_opts;
			opts.target_file_size_base =
				new_opts.target_file_size_base;
			return CompactToLevel(opts, dbname,
					      new_opts.num_levels - 1, true);
		}
	}
}
