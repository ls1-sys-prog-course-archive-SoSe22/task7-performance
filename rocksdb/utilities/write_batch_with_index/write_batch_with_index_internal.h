...
struct WriteBatchIndexEntry {
	WriteBatchIndexEntry(size_t o, uint32_t c, size_t ko, size_t ksz)
		: offset(o), column_family(c), key_offset(ko), key_size(ksz),
		  search_key(nullptr)
	{
	}
	WriteBatchIndexEntry(const Slice *sk, uint32_t c)
		: offset(0), column_family(c), key_offset(0), key_size(0),
		  search_key(sk)
	{
	}
...
class ReadableWriteBatch : public WriteBatch {
	...
	Status GetEntryFromDataOffset(size_t data_offset, WriteType *type,
				      Slice *Key, Slice *value, Slice *blob,
				      Slice *xid) const;
};
...
class WriteBatchEntryComparator {
	...
	WriteBatchEntryComparator(const Comparator *_default_comparator,
		...
		: default_comparator_(_default_comparator),
