				     ...
				     WriteBatchWithIndex *batch)
{
	...
	// Iterator all keys
	{
		...
		iter->SeekToLast();
		for (auto pair = data_map.rbegin(); pair != data_map.rend();
		     ++pair) {
			for (auto v = pair->second.rbegin();
			     v != pair->second.rend(); v++) {
				ASSERT_OK(iter->status());
				ASSERT_TRUE(iter->Valid());
				auto write_entry = iter->Entry();
				ASSERT_EQ(pair->first,
	...
	// Iterator all indexes
	{
		...
		iter->SeekToLast();
		for (auto pair = index_map.rbegin(); pair != index_map.rend();
		     ++pair) {
			for (auto v = pair->second.rbegin();
			     v != pair->second.rend(); v++) {
				ASSERT_OK(iter->status());
...
TEST_F(WriteBatchWithIndexTest, TestRandomIteraratorWithBase)
{
	...
	for (int rand_seed = 301; rand_seed < 366; rand_seed++) {
		...
		for (int i = 0; i < 128; i++) {
			...
			case 2: {
				// Seek to random key
				auto key_idx = rnd.Uniform(static_cast<int>(
					source_strings.size()));
				 ...
				 ColumnFamilyHandle *column_family)
{
	...
	if (column_family == nullptr) {
		...
		iter = batch->NewIteratorWithBase(column_family,
						  new KVIter(base_map));
	}
...
TEST_F(WriteBatchWithIndexTest, SavePointTest)
{
	...
	ASSERT_EQ("PUT(A):a,PUT(A):aa,PUT(B):b,",
		  PrintContents(&batch, nullptr));
