						  ...
						  Slice *xid) const
{
	...
	if (data_offset > GetDataSize()) {
		return Status::InvalidArgument(
			"data offset exceed write batch size");
	}
	Slice input =
		Slice(rep_.data() + data_offset, rep_.size() - data_offset);
	char tag;
	uint32_t column_family;
	Status s = ReadRecordFromWriteBatch(&input, &tag, &column_family, Key,
					    value, blob, xid);
	...
	Status *s)
{
	...
	*s = Status::OK();
	...
	std::unique_ptr<WBWIIterator> iter = std::unique_ptr<WBWIIterator>(
	...
	while (iter->Valid()) {
		...
		case kMergeRecord: {
			result = WriteBatchWithIndexInternal::Result::
				kMergeInProgress;
		...
		default: {
			...
			(*s) = Status::Corruption(
				"Unexpected entry in WriteBatchWithIndex:",
				ToString(entry.type));
			break;
		}
		}
		...
		if (result == WriteBatchWithIndexInternal::Result::
				      kMergeInProgress &&
		    overwrite_key == true) {
			...
			if (merge_context->GetNumOperands() > 0) {
				...
				if (column_family != nullptr) {
					...
					*s = Status::InvalidArgument(
						"Must provide a column_family");
					result = WriteBatchWithIndexInternal::
						Result::kError;
					return result;
				}
				...
				Logger *logger =
					immuable_db_options.info_log.get();
				...
				if (result == WriteBatchWithIndexInternal::
						      Result::kFound) { // PUT
					value->assign(entry_value.data(),
						      entry_value.size());
				}
			}
		}
