...
class EnvLibradosTest : public testing::Test {
	...
	~EnvLibradosTest()
	{
		...
		do {
			ret = rados.init(
				"admin"); // just use the client.admin keyring
			if (ret <
			    0) { // let's handle any error that might have come back
				std::cerr << "couldn't initialize rados! error "
					  << ret << std::endl;
				ret = EXIT_FAILURE;
				break;
			}
       ...
       */
			...
			if (ret < 0) {
				std::cerr
					<< "couldn't connect to cluster! error "
					<< ret << std::endl;
				ret = EXIT_FAILURE;
				break;
			}
       ...
       */
			...
			if (delete_ret < 0) {
				// be careful not to
				std::cerr
					<< "We failed to delete our test pool!"
					<< db_pool << delete_ret << std::endl;
				ret = EXIT_FAILURE;
			}
		} while (0);
	}
...
TEST_F(EnvLibradosTest, Misc)
{
	...
	unique_ptr<WritableFile> writable_file;
	ASSERT_TRUE(
		!env_->NewWritableFile("/a/b", &writable_file, soptions_).ok());
...
TEST_F(EnvLibradosTest, DBBulkLoadKeysInRandomOrder)
{
	...
	int bulk_size = 1 << 15;
	Timer timer(false);
	std::cout << "Test size : loop(" << max_loop << "); bulk_size("
		  << bulk_size << ")" << std::endl;
	/**********************************
            use default env
  ***********************************/
...
class EnvLibradosMutipoolTest : public testing::Test {
	...
	{
		env_ = new EnvLibrados(client_name, cluster_name, flags,
				       db_name, config, db_pool, wal_dir,
				       wal_pool, write_buffer_size);
	}
	~EnvLibradosMutipoolTest()
	{
		delete env_;
...
TEST_F(EnvLibradosMutipoolTest, Basics)
{
	...
	for (size_t i = 0; i < v.size(); ++i) {
		...
		ASSERT_EQ(Status::NotFound(),
			  env_->FileExists(dir_non_existent.c_str()));
		ASSERT_TRUE(
			!env_->GetFileSize(dir_non_existent.c_str(), &file_size)
				 .ok());
		ASSERT_OK(env_->GetChildren(dir.c_str(), &children));
		ASSERT_EQ(0U, children.size());

		// Create a file.
		ASSERT_OK(env_->NewWritableFile(dir_f.c_str(), &writable_file,
						soptions_));
		writable_file.reset();
		...
		unique_ptr<SequentialFile> seq_file;
		unique_ptr<RandomAccessFile> rand_file;
		ASSERT_TRUE(!env_->NewSequentialFile(dir_non_existent.c_str(),
						     &seq_file, soptions_)
				     .ok());
		ASSERT_TRUE(!seq_file);
