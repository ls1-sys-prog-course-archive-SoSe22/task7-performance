...
class SimCacheImpl : public SimCache {
	...
	{
		std::string res;
		res.append("SimCache MISSes: " +
			   std::to_string(get_miss_counter()) + "\n");
		res.append("SimCache HITs:    " +
			   std::to_string(get_hit_counter()) + "\n");
		char buff[350];
		auto lookups = get_miss_counter() + get_hit_counter();
		snprintf(buff, sizeof(buff), "SimCache HITRATE: %.2f%%\n",
			 (lookups == 0 ? 0 :
					       get_hit_counter() * 100.0f / lookups));
		res.append(buff);
		return res;
	}
	...
	void inc_hit_counter()
	{
		hit_times_.fetch_add(1, std::memory_order_relaxed);
	}
};
				      ...
				      size_t sim_capacity, int num_shard_bits)
{
	...
	return std::make_shared<SimCacheImpl>(cache, sim_capacity,
					      num_shard_bits);
}
