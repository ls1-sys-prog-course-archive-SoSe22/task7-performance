...
ColBufDecoder *
ColBufDecoder::NewColBufDecoder(const ColDeclaration &col_declaration)
{
	if (col_declaration.col_type == "FixedLength") {
		return new FixedLengthColBufDecoder(
			col_declaration.size,
			col_declaration.col_compression_type,
		...
		return new VariableChunkColBufDecoder(
			col_declaration.col_compression_type);
	} else if (col_declaration.col_type == "LongFixedLength") {
		return new LongFixedLengthColBufDecoder(
			col_declaration.size, col_declaration.nullable);
	}
...
size_t FixedLengthColBufDecoder::Init(const char *src)
{
	...
	if (col_compression_type_ == kColDict ||
		...
		for (uint64_t i = 0; i < dict_size; ++i) {
			...
			dict_key = EncodeFixed64WithEndian(dict_key,
							   big_endian_, size_);
...
size_t FixedLengthColBufDecoder::Decode(const char *src, char **dest)
{
	...
	if (IsRunLength(col_compression_type_)) {
		if (remain_runs_ == 0) {
			...
			if (col_compression_type_ != kColRleDeltaVarint &&
			    col_compression_type_ != kColRleDict) {
				run_val_ = EncodeFixed64WithEndian(
					run_val_, big_endian_, size_);
			}
		}
		read_val = run_val_;
	} else {
		if (col_compression_type_ == kColNoCompression) {
			...
			const char *q =
				GetVarint64Ptr(src, src_limit, &read_val);
			assert(q != nullptr);
			src = q;
		}
		if (col_compression_type_ != kColDeltaVarint &&
		    col_compression_type_ != kColDict) {
			read_val = EncodeFixed64WithEndian(read_val,
							   big_endian_, size_);
		}
	}
	...
	if (col_compression_type_ == kColDeltaVarint ||
		...
		write_val =
			EncodeFixed64WithEndian(write_val, big_endian_, size_);
		last_val_ = tmp;
