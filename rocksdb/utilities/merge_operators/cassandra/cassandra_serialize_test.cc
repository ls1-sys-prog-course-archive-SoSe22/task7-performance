...
TEST(SerializeTest, SerializeI64)
{
	...
	EXPECT_EQ(std::string({ '\x00', '\x00', '\x00', '\x00', '\x00', '\x00',
				'\x00', '\x00' }),
		  dest);
	...
	dest.clear();
	Serialize<int64_t>(-1, &dest);
	EXPECT_EQ(std::string({ '\xff', '\xff', '\xff', '\xff', '\xff', '\xff',
				'\xff', '\xff' }),
		  dest);

	dest.clear();
	Serialize<int64_t>(9223372036854775807, &dest);
	EXPECT_EQ(std::string({ '\x7f', '\xff', '\xff', '\xff', '\xff', '\xff',
				'\xff', '\xff' }),
		  dest);

	dest.clear();
	Serialize<int64_t>(-9223372036854775807, &dest);
	EXPECT_EQ(std::string({ '\x80', '\x00', '\x00', '\x00', '\x00', '\x00',
				'\x00', '\x01' }),
		  dest);
}
...
TEST(SerializeTest, DeserializeI64)
{
	...
	EXPECT_EQ(-9223372036854775807,
		  Deserialize<int64_t>(dest.c_str(), offset));
