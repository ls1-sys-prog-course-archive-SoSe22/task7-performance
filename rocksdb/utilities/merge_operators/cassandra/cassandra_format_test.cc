...
TEST(ColumnTest, Column)
{
	...
	EXPECT_TRUE(std::memcmp(data, dest.c_str() + offset, sizeof(data)) ==
		    0);
	...
	EXPECT_TRUE(std::memcmp(dest.c_str(), dest.c_str() + c.Size(),
				c.Size()) == 0);
	...
	EXPECT_TRUE(std::memcmp(dest.c_str() + c.Size(),
				dest.c_str() + c.Size() * 2, c.Size()) == 0);
}
...
TEST(RowValueTest, RowTombstone)
{
	...
	EXPECT_TRUE(std::memcmp(dest.c_str(), dest.c_str() + r.Size(),
				r.Size()) == 0);
}
...
TEST(RowValueTest, RowWithColumns)
{
	...
	int32_t e_ttl = 3600;
	columns.push_back(std::unique_ptr<ExpiringColumn>(new ExpiringColumn(
		ColumnTypeMask::EXPIRATION_MASK, e_index, e_timestamp,
	...
	offset += sizeof(int32_t);
	EXPECT_TRUE(std::memcmp(e_data, dest.c_str() + offset,
				sizeof(e_data)) == 0);
	...
	offset += sizeof(int32_t);
	EXPECT_TRUE(std::memcmp(c_data, dest.c_str() + offset,
				sizeof(c_data)) == 0);
