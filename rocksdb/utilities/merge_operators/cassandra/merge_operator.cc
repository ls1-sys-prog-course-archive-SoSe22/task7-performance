	...
	MergeOperationOutput *merge_out) const
{
	...
	for (auto &operand : merge_in.operand_list) {
		row_values.push_back(
			RowValue::Deserialize(operand.data(), operand.size()));
	}
...
std::shared_ptr<MergeOperator> MergeOperators::CreateCassandraMergeOperator()
{
	return std::make_shared<
		rocksdb::cassandra::CassandraValueMergeOperator>();
}
