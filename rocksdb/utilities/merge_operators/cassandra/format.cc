...
ColumnBase::ColumnBase(int8_t mask, int8_t index) : mask_(mask), index_(index)
{
}
...
std::unique_ptr<Column> Column::Deserialize(const char *src, std::size_t offset)
{
	int8_t mask = rocksdb::cassandra::Deserialize<int8_t>(src, offset);
	offset += sizeof(mask);
...
std::unique_ptr<ExpiringColumn> ExpiringColumn::Deserialize(const char *src,
							    std::size_t offset)
{
	int8_t mask = rocksdb::cassandra::Deserialize<int8_t>(src, offset);
	offset += sizeof(mask);
...
RowValue RowValue::Deserialize(const char *src, std::size_t size)
{
	...
	while (offset < size) {
		...
		assert(offset <= size);
		last_modified_time =
			std::max(last_modified_time, c->Timestamp());
...
RowValue RowValue::Merge(std::vector<RowValue> &&values)
{
	...
	for (auto &value : values) {
		...
		for (auto &column : value.columns_) {
			...
			if (merged_columns.find(index) ==
			    merged_columns.end()) {
				merged_columns[index] = std::move(column);
			} else {
				if (column->Timestamp() >
				    merged_columns[index]->Timestamp()) {
	...
	for (auto &pair : merged_columns) {
		...
		last_modified_time =
			std::max(last_modified_time, pair.second->Timestamp());
