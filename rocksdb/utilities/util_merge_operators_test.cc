...
class UtilMergeOperatorTest : public testing::Test {
				...
				std::string key = "")
	{
		...
		std::vector<Slice> operands_slice(operands.begin(),
						  operands.end());
				...
				std::string key = "")
	{
		...
		const MergeOperator::MergeOperationInput merge_in(
			key, nullptr, operands_slice, nullptr);
		MergeOperator::MergeOperationOutput merge_out(result,
							      result_operand);
		merge_operator_->FullMergeV2(merge_in, &merge_out);
				 ...
				 std::string key = "")
	{
		...
		merge_operator_->PartialMerge(key, left, right, &result,
					      nullptr);
		return result;
	}
	...
	{
		std::string result;
		std::deque<Slice> operands_slice(operands.begin(),
						 operands.end());

		merge_operator_->PartialMergeMulti(key, operands_slice, &result,
						   nullptr);
		return result;
	}
