...
template <typename Key, class Comparator>
inline void SkipList<Key, Comparator>::Iterator::SeekForPrev(const Key &target)
{
	Seek(target);
	if (!Valid()) {
		SeekToLast();
...
SkipList<Key, Comparator>::SkipList(const Comparator cmp, Allocator *allocator,
	...
	: kMaxHeight_(max_height), kBranching_(branching_factor),
	  kScaledInverseBranching_((Random::kMaxNext + 1) / kBranching_),
	  compare_(cmp), allocator_(allocator),
	  head_(NewNode(0 /* any key will do */, max_height)), max_height_(1),
	  prev_height_(1)
{
	assert(max_height > 0 &&
...
void SkipList<Key, Comparator>::Insert(const Key &key)
{
	...
	if (!KeyIsAfterNode(key, prev_[0]->NoBarrier_Next(0)) &&
		...
		assert(prev_[0] != head_ ||
		       (prev_height_ == 1 && GetMaxHeight() == 1));
	...
	assert(prev_[0]->Next(0) == nullptr ||
	       !Equal(key, prev_[0]->Next(0)->key));
