...
struct CuckooStep {
	...
	CuckooStep(int bucket_id, int prev_step_id, int depth)
		: bucket_id_(bucket_id), prev_step_id_(prev_step_id),
		  depth_(depth)
	{
	}
};
...
class HashCuckooRep : public MemTableRep {
	...
	unsigned int GetHash(const Slice &slice, const int hash_func_id) const
	{
		...
		static const int
			kMurmurHashSeeds[HashCuckooRepFactory::kMaxHashCount] = {
				545609244,  1769731426, 763324157, 13099088,
				592422103,  1899789565, 248369300, 1984183468,
				1613664382, 1491157517
			};
	...
	virtual MemTableRep::Iterator *GetIterator(Arena *arena) override
	{
		...
		for (unsigned int bid = 0; bid < bucket_count_; ++bid) {
			const char *bucket = cuckoo_array_[bid].load(
				std::memory_order_relaxed);
			if (bucket != nullptr) {
			...
			bool (*callback_func)(void *arg, const char *entry))
{
	...
	for (unsigned int hid = 0; hid < hash_function_count_; ++hid) {
		const char *bucket = cuckoo_array_[GetHash(user_key, hid)].load(
...
void HashCuckooRep::Insert(KeyHandle handle)
{
	...
	if (FindCuckooPath(key, user_key, cuckoo_path_, &cuckoo_path_length,
		...
		if (backup_table_.get() == nullptr) {
			...
			backup_table_.reset(factory.CreateMemTableRep(
				compare_, allocator_, nullptr, nullptr));
	...
	for (size_t i = 1; i < cuckoo_path_length; ++i) {
		...
		cuckoo_array_[kicked_out_bid].store(
			cuckoo_array_[current_bid].load(
				std::memory_order_relaxed),
...
bool HashCuckooRep::Contains(const char *internal_key) const
{
	...
	for (unsigned int hid = 0; hid < hash_function_count_; ++hid) {
		const char *stored_key =
			cuckoo_array_[GetHash(user_key, hid)].load(
				std::memory_order_acquire);
				...
				int bucket_ids[], const int initial_hash_id)
{
	...
	for (unsigned int hid = initial_hash_id; hid < hash_function_count_;
	     ++hid) {
		...
		const char *stored_key = cuckoo_array_[bucket_ids[hid]].load(
			std::memory_order_relaxed);
		if (stored_key == nullptr) {
	...
	if (cuckoo_bucket_id != -1) {
		cuckoo_array_[cuckoo_bucket_id].store(
			const_cast<char *>(internal_key),
			std::memory_order_release);
		return true;
	}
	...
	*cuckoo_path_length = 0;
	...
	while (step_buffer_.HasNewWrite()) {
		...
		Slice bucket_user_key =
			UserKey(cuckoo_array_[step.bucket_id_].load(
				std::memory_order_relaxed));
		...
		for (unsigned int hid = start_hid + 1;
		     hid < hash_function_count_; ++hid) {
			...
			if (cuckoo_array_[next_step.bucket_id_].load(
				...
				cuckoo_path[(*cuckoo_path_length)++] =
					next_step.bucket_id_;
				int depth;
				for (depth = step.depth_;
				     depth > 0 &&
				     step_id != CuckooStep::kNullStep;
				     depth--) {
					const CuckooStep &prev_step =
						step_buffer_.steps_[step_id];
					cuckoo_path[(*cuckoo_path_length)++] =
						prev_step.bucket_id_;
...
HashCuckooRep::Iterator::Iterator(
	std::shared_ptr<std::vector<const char *> > bucket,
	const KeyComparator &compare)
	: bucket_(bucket), cit_(bucket_->end()), compare_(compare),
	  sorted_(false)
{
}
	...
	const SliceTransform *transform, Logger *logger)
{
	...
	size_t bucket_count = static_cast<size_t>(
		(write_buffer_size_ / (average_data_size_ + pointer_size)) /
			kFullness +
		1);
	...
	return new HashCuckooRep(
		...
		static_cast<size_t>((average_data_size_ + pointer_size) /
				    kFullness));
}
