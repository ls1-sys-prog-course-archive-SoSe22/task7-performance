...
class BenchmarkThread {
	...
	explicit BenchmarkThread(MemTableRep *table, KeyGenerator *key_gen,
				 ...
				 uint64_t *read_hits)
		: table_(table), key_gen_(key_gen),
		  bytes_written_(bytes_written), bytes_read_(bytes_read),
		  sequence_(sequence), num_ops_(num_ops), read_hits_(read_hits)
	{
	}
...
class FillBenchmarkThread : public BenchmarkThread {
	...
	FillBenchmarkThread(MemTableRep *table, KeyGenerator *key_gen,
			    ...
			    uint64_t *sequence, uint64_t num_ops,
			    uint64_t *read_hits)
		: BenchmarkThread(table, key_gen, bytes_written, bytes_read,
...
class ConcurrentFillBenchmarkThread : public FillBenchmarkThread {
	...
	ConcurrentFillBenchmarkThread(MemTableRep *table, KeyGenerator *key_gen,
		...
		: FillBenchmarkThread(table, key_gen, bytes_written, bytes_read,
				      sequence, num_ops, read_hits)
	{
...
class ReadBenchmarkThread : public BenchmarkThread {
	...
	{
		CallbackVerifyArgs *callback_args =
			static_cast<CallbackVerifyArgs *>(arg);
...
class ConcurrentReadBenchmarkThread : public ReadBenchmarkThread {
	...
	ConcurrentReadBenchmarkThread(MemTableRep *table, KeyGenerator *key_gen,
				      ...
				      uint64_t num_ops, uint64_t *read_hits,
				      std::atomic_int *threads_done)
		: ReadBenchmarkThread(table, key_gen, bytes_written, bytes_read,
				      sequence, num_ops, read_hits)
	{
		threads_done_ = threads_done;
	}
...
class SeqConcurrentReadBenchmarkThread : public SeqReadBenchmarkThread {
    public:
	SeqConcurrentReadBenchmarkThread(MemTableRep *table,
					 ...
					 uint64_t *read_hits,
					 std::atomic_int *threads_done)
		: SeqReadBenchmarkThread(table, key_gen, bytes_written,
					 bytes_read, sequence, num_ops,
					 read_hits)
	{
		threads_done_ = threads_done;
	}
...
class Benchmark {
	...
	explicit Benchmark(MemTableRep *table, KeyGenerator *key_gen,
		...
		: table_(table), key_gen_(key_gen), sequence_(sequence),
	...
	virtual void Run()
	{
		...
		RunThreads(&threads, &bytes_written, &bytes_read, true,
			   &read_hits);
		auto elapsed_time =
			static_cast<double>(timer.ElapsedNanos() / 1000);
		std::cout << "Elapsed time: " << static_cast<int>(elapsed_time)
			  << " us" << std::endl;

		if (bytes_written > 0) {
			auto MiB_written =
				static_cast<double>(bytes_written) / (1 << 20);
			auto write_throughput =
				MiB_written / (elapsed_time / 1000000);
			std::cout << "Total bytes written: " << MiB_written
				  << " MiB" << std::endl;
			std::cout << "Write throughput: " << write_throughput
				  << " MiB/s" << std::endl;
			auto us_per_op =
				elapsed_time / num_write_ops_per_thread_;
			std::cout << "write us/op: " << us_per_op << std::endl;
		}
		if (bytes_read > 0) {
			auto MiB_read =
				static_cast<double>(bytes_read) / (1 << 20);
			auto read_throughput =
				MiB_read / (elapsed_time / 1000000);
			std::cout << "Total bytes read: " << MiB_read << " MiB"
				  << std::endl;
			std::cout << "Read throughput: " << read_throughput
				  << " MiB/s" << std::endl;
			auto us_per_op =
				elapsed_time / num_read_ops_per_thread_;
...
class FillBenchmark : public Benchmark {
	...
	{
		FillBenchmarkThread(table_, key_gen_, bytes_written, bytes_read,
				    sequence_, num_write_ops_per_thread_,
				    read_hits)();
	}
};
...
class ReadBenchmark : public Benchmark {
		...
		: Benchmark(table, key_gen, sequence, FLAGS_num_threads)
	{
		num_read_ops_per_thread_ =
			FLAGS_num_operations / FLAGS_num_threads;
	}
			...
			bool write, uint64_t *read_hits) override
	{
		for (int i = 0; i < FLAGS_num_threads; ++i) {
			threads->emplace_back(ReadBenchmarkThread(
				table_, key_gen_, bytes_written, bytes_read,
				sequence_, num_read_ops_per_thread_,
				read_hits));
		}
...
template <class ReadThreadType> class ReadWriteBenchmark : public Benchmark {
    public:
	explicit ReadWriteBenchmark(MemTableRep *table, KeyGenerator *key_gen,
				    uint64_t *sequence)
		: Benchmark(table, key_gen, sequence, FLAGS_num_threads)
	{
		num_read_ops_per_thread_ = FLAGS_num_threads <= 1 ?
							 0 :
							 (FLAGS_num_operations /
						    (FLAGS_num_threads - 1));
		num_write_ops_per_thread_ = FLAGS_num_operations;
			...
			bool write, uint64_t *read_hits) override
	{
		...
		for (int i = 1; i < FLAGS_num_threads; ++i) {
			threads->emplace_back(ReadThreadType(
				table_, key_gen_, bytes_written, bytes_read,
...
int main(int argc, char **argv)
{
	...
	if (FLAGS_memtablerep == "skiplist") {
		...
		factory.reset(rocksdb::NewHashLinkListRepFactory(
			...
			FLAGS_if_log_bucket_dist_when_flash,
	...
	auto createMemtableRep = [&] {
		sequence = 0;
		return factory->CreateMemTableRep(
			key_comp, &arena, options.prefix_extractor.get(),
	...
	while (benchmarks != nullptr) {
		...
		if (name == rocksdb::Slice("fillseq")) {
			...
			key_gen.reset(new rocksdb::KeyGenerator(
				&rng, rocksdb::SEQUENTIAL,
				FLAGS_num_operations));
			benchmark.reset(new rocksdb::SeqReadBenchmark(
			...
			std::cout << "WARNING: skipping unknown benchmark '"
				  << name.ToString() << std::endl;
			continue;
		}
