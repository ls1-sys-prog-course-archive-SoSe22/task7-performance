...
template <class Comparator> class InlineSkipList {
	...
	void Insert(const char *key, Splice *splice,
	...
	void FindSpliceForLevel(const char *key, Node *before, Node *after,
						       ...
						       int recompute_level)
{
	...
	for (int i = recompute_level - 1; i >= 0; --i) {
		FindSpliceForLevel(key, splice->prev_[i + 1],
	...
	if (UseCAS) {
		for (int i = 0; i < height; ++i) {
			while (true) {
				...
				FindSpliceForLevel(key, splice->prev_[i],
						   nullptr, i,
						   &splice->prev_[i],
						   &splice->next_[i]);
