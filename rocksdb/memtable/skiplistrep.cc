...
class SkipListRep : public MemTableRep {
	...
	explicit SkipListRep(const MemTableRep::KeyComparator &compare,
		...
		: MemTableRep(allocator), skip_list_(compare, allocator),
		  cmp_(compare), transform_(transform), lookahead_(lookahead)
	{
	}
				       ...
				       const Slice &end_ikey) override
	{
		...
		uint64_t end_count =
			skip_list_.EstimateCount(EncodeKey(&tmp, end_ikey));
		return (end_count >= start_count) ? (end_count - start_count) :
							  0;
	}
	...
	class LookaheadIterator : public MemTableRep::Iterator {
		...
		virtual void Next() override
		{
			...
			if (prev_.Valid()) {
				...
				if (k1.compare(k2) == 0) {
					...
					auto t1 =
						rep_.transform_->Transform(k1);
					auto t2 =
						rep_.transform_->Transform(k2);
				  ...
				  const char *memtable_key) override
		{
			...
			if (prev_.Valid() &&
			    rep_.cmp_(encoded_key, prev_.key()) >= 0) {
	...
	GetIterator(Arena *arena = nullptr) override
	{
		if (lookahead_ > 0) {
			void *mem =
				arena ? arena->AllocateAligned(sizeof(
