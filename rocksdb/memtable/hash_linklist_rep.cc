...
struct BucketHeader {
	...
	void IncNumEntries()
	{
		...
		num_entries.store(GetNumEntries() + 1,
...
struct SkipListBucketHeader {
	...
	explicit SkipListBucketHeader(const MemTableRep::KeyComparator &cmp,
		...
		: Counting_header(
			  this, // Pointing to itself to indicate header type.
			  count),
...
class HashLinkListRep : public MemTableRep {
	...
	virtual MemTableRep::Iterator *
	GetIterator(Arena *arena = nullptr) override;

	virtual MemTableRep::Iterator *
	GetDynamicPrefixIterator(Arena *arena = nullptr) override;
	...
	SkipListBucketHeader *
	GetSkipListBucketHeader(Pointer *first_next_pointer) const;
	...
	{
		return MurmurHash(slice.data(), static_cast<int>(slice.size()),
				  0) %
		       bucket_size_;
	}
	...
	class FullListIterator : public MemTableRep::Iterator {
	    public:
		explicit FullListIterator(MemtableSkipList *list,
					  Allocator *allocator)
	...
	class LinkListIterator : public MemTableRep::Iterator {
	    public:
		explicit LinkListIterator(
			const HashLinkListRep *const hash_link_list_rep,
			Node *head)
			: hash_link_list_rep_(hash_link_list_rep), head_(head),
			  node_(nullptr)
		{
		}
		...
		virtual void Seek(const Slice &internal_key,
				  const char *memtable_key) override
		{
			node_ = hash_link_list_rep_->FindGreaterOrEqualInBucket(
				head_, internal_key);
		}
	...
	class DynamicIterator : public HashLinkListRep::LinkListIterator {
		...
		explicit DynamicIterator(HashLinkListRep &memtable_rep)
			: HashLinkListRep::LinkListIterator(&memtable_rep,
							    nullptr),
			...
			if (skip_list_header != nullptr) {
				...
				if (!skip_list_iter_) {
					skip_list_iter_.reset(
						new MemtableSkipList::Iterator(
							&skip_list_header
								 ->skip_list));
				} else {
					skip_list_iter_->SetList(
						&skip_list_header->skip_list);
				}
				if (memtable_key != nullptr) {
					...
					skip_list_iter_->Seek(
						encoded_key.GetUserKey().data());
				}
			} else {
				// The bucket is organized as a linked list
				skip_list_iter_.reset();
				Reset(memtable_rep_.GetLinkListFirstNode(
					bucket));
				HashLinkListRep::LinkListIterator::Seek(
					k, memtable_key);
			}
		}
...
HashLinkListRep::HashLinkListRep(const MemTableRep::KeyComparator &compare,
	...
	: MemTableRep(allocator), bucket_size_(bucket_size),
	  ...
	  transform_(transform), compare_(compare), logger_(logger),
...
SkipListBucketHeader *
HashLinkListRep::GetSkipListBucketHeader(Pointer *first_next_pointer) const
{
	...
	// Counting header
	BucketHeader *header =
		reinterpret_cast<BucketHeader *>(first_next_pointer);
...
void HashLinkListRep::Insert(KeyHandle handle)
{
	...
	if (first_next_pointer->load(std::memory_order_relaxed) == nullptr) {
		...
		if (header->IsSkipListBucket()) {
			...
			auto *skip_list_bucket_header =
				reinterpret_cast<SkipListBucketHeader *>(
					header);
	...
	if (header->GetNumEntries() == threshold_use_skiplist_) {
		...
		LinkListIterator bucket_iter(
			this, reinterpret_cast<Node *>(first_next_pointer->load(
				      std::memory_order_relaxed)));
		auto mem = allocator_->AllocateAligned(
			sizeof(SkipListBucketHeader));
		SkipListBucketHeader *new_skip_list_header = new (mem)
			SkipListBucketHeader(compare_, allocator_,
					     header->GetNumEntries() + 1);
		...
		if (prev) {
			...
			header->next.store(static_cast<void *>(x),
...
bool HashLinkListRep::Contains(const char *key) const
{
	...
	SkipListBucketHeader *skip_list_header =
		GetSkipListBucketHeader(bucket);
	if (skip_list_header != nullptr) {
		return skip_list_header->skip_list.Contains(key);
	} else {
		return LinkListContains(GetLinkListFirstNode(bucket),
					internal_key);
	}
}
...
MemTableRep::Iterator *HashLinkListRep::GetIterator(Arena *alloc_arena)
{
	...
	for (size_t i = 0; i < bucket_size_; ++i) {
		...
		if (bucket != nullptr) {
			auto *skip_list_header =
				GetSkipListBucketHeader(bucket);
			if (skip_list_header != nullptr) {
				// Is a skip list
				MemtableSkipList::Iterator itr(
					&skip_list_header->skip_list);
				...
				auto *link_list_head =
					GetLinkListFirstNode(bucket);
				if (link_list_head != nullptr) {
					LinkListIterator itr(this,
							     link_list_head);
	...
	if (if_log_bucket_dist_when_flash_ && logger_ != nullptr) {
		Info(logger_,
		     "hashLinkedList Entry distribution among buckets: %s",
	...
	if (alloc_arena == nullptr) {
		...
		auto mem =
			alloc_arena->AllocateAligned(sizeof(FullListIterator));
		return new (mem) FullListIterator(list, new_arena);
	}
}

MemTableRep::Iterator *
HashLinkListRep::GetDynamicPrefixIterator(Arena *alloc_arena)
{
	if (alloc_arena == nullptr) {
		return new DynamicIterator(*this);
	} else {
		auto mem =
			alloc_arena->AllocateAligned(sizeof(DynamicIterator));
		return new (mem) DynamicIterator(*this);
	}
}

bool HashLinkListRep::LinkListContains(Node *head, const Slice &user_key) const
{
...
NewHashLinkListRepFactory(size_t bucket_count, size_t huge_page_tlb_size,
			  ...
			  uint32_t threshold_use_skiplist)
{
	return new HashLinkListRepFactory(bucket_count, threshold_use_skiplist,
					  huge_page_tlb_size,
