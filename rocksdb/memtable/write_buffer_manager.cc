...
struct WriteBufferManager::CacheRep {
		...
		: cache_(cache), cache_allocated_size_(0)
	{
		...
		memcpy(cache_key_, static_cast<const void *>(this),
		       pointer_size);
	}
	...
	Slice GetNextCacheKey()
	{
		...
		char *end = EncodeVarint64(cache_key_ + kCacheKeyPrefix,
					   next_cache_key_id_++);
...
WriteBufferManager::WriteBufferManager(size_t _buffer_size,
	...
	: buffer_size_(_buffer_size), mutable_limit_(buffer_size_ * 7 / 8),
	  memory_used_(0), memory_active_(0), cache_rep_(nullptr)
{
#ifndef ROCKSDB_LITE
	if (cache) {
...
void WriteBufferManager::ReserveMemWithCache(size_t mem)
{
	...
	size_t new_mem_used =
		memory_used_.load(std::memory_order_relaxed) + mem;
	memory_used_.store(new_mem_used, std::memory_order_relaxed);
	while (new_mem_used > cache_rep_->cache_allocated_size_) {
		// Expand size by at least 1MB.
		// Add a dummy record to the cache
		Cache::Handle *handle;
		cache_rep_->cache_->Insert(cache_rep_->GetNextCacheKey(),
					   nullptr, kSizeDummyEntry, nullptr,
					   &handle);
...
void WriteBufferManager::FreeMemWithCache(size_t mem)
{
	...
	if (new_mem_used < cache_rep_->cache_allocated_size_ / 4 * 3 &&
		...
		cache_rep_->cache_->Release(cache_rep_->dummy_handles_.back(),
					    true);
