...
class HashSkipListRep : public MemTableRep {
	...
	typedef SkipList<const char *, const MemTableRep::KeyComparator &>
		Bucket;
	...
	class Iterator : public MemTableRep::Iterator {
		...
		explicit Iterator(Bucket *list, bool own_list = true,
				  Arena *arena = nullptr)
			: list_(list), iter_(list), own_list_(own_list),
			  arena_(arena)
		{
		}
...
HashSkipListRep::HashSkipListRep(const MemTableRep::KeyComparator &compare,
	...
	: MemTableRep(allocator), bucket_size_(bucket_size),
	  ...
	  transform_(transform), compare_(compare), allocator_(allocator)
{
	auto mem = allocator->AllocateAligned(sizeof(std::atomic<void *>) *
					      bucket_size);
...
HashSkipListRep::Bucket *
HashSkipListRep::GetInitializedBucket(const Slice &transformed)
{
	...
	if (bucket == nullptr) {
		...
		bucket = new (addr)
			Bucket(compare_, allocator_, skiplist_height_,
...
MemTableRepFactory *NewHashSkipListRepFactory(size_t bucket_count,
