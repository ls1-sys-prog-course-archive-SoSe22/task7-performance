	...
	jboolean jfull_scan_mode, jboolean jstore_index_in_file)
{
	...
	options.encoding_type =
		static_cast<rocksdb::EncodingType>(jencoding_type);
...
jlong Java_org_rocksdb_BlockBasedTableConfig_newTableFactoryHandle(
	...
	jlong block_cache_size, jint block_cache_num_shardbits,
	jlong block_size, jint block_size_deviation,
	...
	jbyte jindex_type, jint jformat_version)
{
	...
	if (!no_block_cache && block_cache_size > 0) {
		if (block_cache_num_shardbits > 0) {
			options.block_cache = rocksdb::NewLRUCache(
				block_cache_size, block_cache_num_shardbits);
		} else {
			options.block_cache =
	...
	return reinterpret_cast<jlong>(
		rocksdb::NewBlockBasedTableFactory(options));
}
