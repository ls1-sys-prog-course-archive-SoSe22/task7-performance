...
jlong rocksdb_open_helper(
	...
	std::function<rocksdb::Status(const rocksdb::Options &,
				      const std::string &, rocksdb::DB **)>
		open_fn)
{
							  ...
							  jstring jdb_path)
{
	return rocksdb_open_helper(env, jopt_handle, jdb_path,
				   (rocksdb::Status(*)(const rocksdb::Options &,
						       const std::string &,
						       rocksdb::DB **)) &
					   rocksdb::DB::Open);
}
	...
	JNIEnv *env, jclass jcls, jlong jopt_handle, jstring jdb_path)
{
	return rocksdb_open_helper(
		...
		[](const rocksdb::Options &options, const std::string &db_path,
		   rocksdb::DB **db) {
			return rocksdb::DB::OpenForReadOnly(options, db_path,
							    db);
		});
}
		...
		open_fn)
{
	...
	rocksdb::JniUtil::byteStrings<std::string>(
		...
		[&jco, &column_families](size_t idx, std::string cf_name) {
			...
			column_families.push_back(
				rocksdb::ColumnFamilyDescriptor(cf_name,
								*cf_options));
		},
		&has_exception);
	...
	rocksdb::DB *db = nullptr;
	rocksdb::Status s =
		open_fn(*opt, db_path, column_families, &handles, &db);
	...
	if (s.ok()) {
		const jsize resultsLen =
			1 + len_cols; //db handle + column family handles
					       ...
					       jint jval_off, jint jval_len)
{
	...
	rocksdb_put_helper(env, db, default_write_options, nullptr, jkey,
						...
						jint jval_off, jint jval_len)
{
	...
	rocksdb_put_helper(env, db, *write_options, nullptr, jkey, jkey_off,
	...
	jint jval_off, jint jval_len, jlong jcf_handle)
{
	...
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	if (cf_handle != nullptr) {
		rocksdb_put_helper(env, db, *write_options, cf_handle, jkey,
				   jkey_off, jkey_len, jval, jval_off,
				   jval_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
			env, rocksdb::Status::InvalidArgument(
 ...
 * Signature: (JJJ)V
 */
void Java_org_rocksdb_RocksDB_write0(JNIEnv *env, jobject jdb, jlong jdb_handle,
				     jlong jwrite_options_handle,
				     jlong jwb_handle)
{
	auto *db = reinterpret_cast<rocksdb::DB *>(jdb_handle);
 ...
 * Signature: (JJJ)V
 */
void Java_org_rocksdb_RocksDB_write1(JNIEnv *env, jobject jdb, jlong jdb_handle,
				     jlong jwrite_options_handle,
				     jlong jwbwi_handle)
{
	auto *db = reinterpret_cast<rocksdb::DB *>(jdb_handle);
	auto *write_options = reinterpret_cast<rocksdb::WriteOptions *>(
		jwrite_options_handle);
	auto *wbwi =
		reinterpret_cast<rocksdb::WriteBatchWithIndex *>(jwbwi_handle);
			      ...
			      jobject jstring_builder, bool *has_exception)
{
	...
	if (cf_handle != nullptr) {
		...
		keyMayExist = db->KeyMayExist(read_opt, key_slice, &value,
					      &value_found);
	}
	...
	jint jkey_off, jint jkey_len, jobject jstring_builder)
{
	...
	return key_may_exist_helper(env, db, rocksdb::ReadOptions(), nullptr,
	...
	jbyteArray jkey, jint jkey_off, jint jkey_len, jobject jstring_builder)
{
	...
	bool has_exception = false;
	return key_may_exist_helper(env, db, read_options, nullptr, jkey,
						...
						jint jkey_len)
{
	return rocksdb_get_helper(env,
				  ...
				  rocksdb::ReadOptions(), nullptr, jkey,
				  jkey_off, jkey_len);
}
						 ...
						 jlong jcf_handle)
{
	...
	auto cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	if (cf_handle != nullptr) {
		return rocksdb_get_helper(env, db_handle,
					  rocksdb::ReadOptions(), cf_handle,
					  jkey, jkey_off, jkey_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
			env, rocksdb::Status::InvalidArgument(
						 ...
						 jint jkey_len)
{
	return rocksdb_get_helper(
		...
		*reinterpret_cast<rocksdb::ReadOptions *>(jropt_handle),
		nullptr, jkey, jkey_off, jkey_len);
}
			...
			bool *has_exception)
{
	...
	rocksdb::Status s;
	if (column_family_handle != nullptr) {
		s = db->Get(read_options, column_family_handle, key_slice,
			    &cvalue);
	} else {
		// backwards compatibility
			      ...
			      jlongArray jcolumn_family_handles)
{
	...
	if (jcolumn_family_handles != nullptr) {
		const jsize len_cols =
			env->GetArrayLength(jcolumn_family_handles);

		jlong *jcfh = env->GetLongArrayElements(jcolumn_family_handles,
							nullptr);
		if (jcfh == nullptr) {
			// exception thrown: OutOfMemoryError
			return nullptr;
		...
		for (jsize i = 0; i < len_cols; i++) {
			auto *cf_handle =
				reinterpret_cast<rocksdb::ColumnFamilyHandle *>(
					jcfh[i]);
			cf_handles.push_back(cf_handle);
		}
		env->ReleaseLongArrayElements(jcolumn_family_handles, jcfh,
					      JNI_ABORT);
	}
	...
	for (jsize i = 0; i < len_keys; i++) {
		...
		if (env->ExceptionCheck()) {
			// exception thrown: ArrayIndexOutOfBoundsException
			env->ReleaseIntArrayElements(jkey_lens, jkey_len,
						     JNI_ABORT);
			env->ReleaseIntArrayElements(jkey_offs, jkey_off,
						     JNI_ABORT);
	...
	// prepare the results
	jobjectArray jresults = rocksdb::ByteJni::new2dByteArray(
	...
	// add to the jresults
	for (std::vector<rocksdb::Status>::size_type i = 0; i != s.size();
	     i++) {
		if (s[i].ok()) {
			std::string *value = &values[i];
			const jsize jvalue_len =
			...
			env->SetObjectArrayElement(
				jresults, static_cast<jsize>(i), jentry_value);
			if (env->ExceptionCheck()) {
				// exception thrown: ArrayIndexOutOfBoundsException
				env->DeleteLocalRef(jentry_value);
				return nullptr;
	...
	jintArray jkey_offs, jintArray jkey_lens)
{
	return multi_get_helper(env, jdb,
				reinterpret_cast<rocksdb::DB *>(jdb_handle),
				rocksdb::ReadOptions(), jkeys, jkey_offs,
				jkey_lens, nullptr);
}
						...
						jint jval_off, jint jval_len)
{
	...
	return rocksdb_get_helper(
		env, reinterpret_cast<rocksdb::DB *>(jdb_handle),
		*reinterpret_cast<rocksdb::ReadOptions *>(jropt_handle),
		nullptr, jkey, jkey_off, jkey_len, jval, jval_off, jval_len,
		&has_exception);
}
					     ...
					     jint jkey_off, jint jkey_len)
{
	...
	static const rocksdb::WriteOptions default_write_options =
		rocksdb::WriteOptions();
	rocksdb_delete_helper(env, db, default_write_options, nullptr, jkey,
			      jkey_off, jkey_len);
}
 ...
 * Signature: (JJ[BIIJ)V
 */
void Java_org_rocksdb_RocksDB_delete__JJ_3BIIJ(JNIEnv *env, jobject jdb,
					       jlong jdb_handle,
					       jlong jwrite_options,
					       jbyteArray jkey, jint jkey_off,
					       jint jkey_len, jlong jcf_handle)
{
	...
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	if (cf_handle != nullptr) {
		rocksdb_delete_helper(env, db, *write_options, cf_handle, jkey,
				      jkey_off, jkey_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
			env, rocksdb::Status::InvalidArgument(
	...
	jbyteArray jkey, jint jkey_len, jlong jcf_handle)
{
	...
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	if (cf_handle != nullptr) {
		rocksdb_single_delete_helper(env, db, *write_options, cf_handle,
					     jkey, jkey_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
			env, rocksdb::Status::InvalidArgument(
				 ...
				 jint jend_key_off, jint jend_key_len)
{
	...
	rocksdb::Slice end_key_slice(reinterpret_cast<char *>(end_key),
				     jend_key_len);

	rocksdb::Status s = db->DeleteRange(write_options, cf_handle,
					    begin_key_slice, end_key_slice);
	...
	jint jend_key_off, jint jend_key_len, jlong jcf_handle)
{
	...
	if (cf_handle != nullptr) {
		rocksdb_delete_range_helper(env, db, default_write_options,
					    cf_handle, jbegin_key,
					    jbegin_key_off, jbegin_key_len,
					    jend_key, jend_key_off,
					    jend_key_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
			env, rocksdb::Status::InvalidArgument(
	...
	jbyteArray jend_key, jint jend_key_off, jint jend_key_len)
{
	...
	auto *write_options =
		reinterpret_cast<rocksdb::WriteOptions *>(jwrite_options);
	rocksdb_delete_range_helper(env, db, *write_options, nullptr,
				    jbegin_key, jbegin_key_off, jbegin_key_len,
				    jend_key, jend_key_off, jend_key_len);
}
	...
	jlong jcf_handle)
{
	...
	if (cf_handle != nullptr) {
		rocksdb_delete_range_helper(env, db, *write_options, cf_handle,
					    jbegin_key, jbegin_key_off,
					    jbegin_key_len, jend_key,
						 ...
						 jint jval_off, jint jval_len)
{
	...
	rocksdb_merge_helper(env, db, default_write_options, nullptr, jkey,
			     jkey_off, jkey_len, jval, jval_off, jval_len);
}
	...
	jint jval_len, jlong jcf_handle)
{
	...
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	if (cf_handle != nullptr) {
		rocksdb_merge_helper(env, db, default_write_options, cf_handle,
				     jkey, jkey_off, jkey_len, jval, jval_off,
				     jval_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
	...
	jint jval_off, jint jval_len, jlong jcf_handle)
{
	...
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	if (cf_handle != nullptr) {
		rocksdb_merge_helper(env, db, *write_options, cf_handle, jkey,
				     jkey_off, jkey_len, jval, jval_off,
				     jval_len);
	} else {
		rocksdb::RocksDBExceptionJni::ThrowNew(
 ...
 * Signature: (J)V
 */
void Java_org_rocksdb_RocksDB_disposeInternal(JNIEnv *env, jobject java_db,
					      jlong jhandle)
{
...
jlong Java_org_rocksdb_RocksDB_iteratorCF__JJ(JNIEnv *env, jobject jdb,
					      jlong db_handle, jlong jcf_handle)
{
	auto *db = reinterpret_cast<rocksdb::DB *>(db_handle);
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
 ...
 * Signature: (J)J
 */
jlong Java_org_rocksdb_RocksDB_getDefaultColumnFamily(JNIEnv *env, jobject jobj,
						      jlong jdb_handle)
{
	auto *db_handle = reinterpret_cast<rocksdb::DB *>(jdb_handle);
						  ...
						  jlong jcolumn_options)
{
	...
	auto *db_handle = reinterpret_cast<rocksdb::DB *>(jdb_handle);
	auto *cfOptions = reinterpret_cast<rocksdb::ColumnFamilyOptions *>(
		jcolumn_options);
	...
	jstring jproperty, jint jproperty_len)
{
	...
	std::string property_value;
	bool retCode =
		db->GetProperty(cf_handle, property_slice, &property_value);
	env->ReleaseStringUTFChars(jproperty, property);
	...
	jstring jproperty, jint jproperty_len)
{
	...
	bool retCode =
		db->GetIntProperty(cf_handle, property_slice, &property_value);
	env->ReleaseStringUTFChars(jproperty, property);
				 ...
				 jint jtarget_path_id)
{
	...
	if (cf_handle != nullptr) {
		s = db->CompactRange(compact_options, cf_handle, nullptr,
				     nullptr);
	} else {
		// backwards compatibility
				 ...
				 jint jtarget_path_id)
{
	...
	const rocksdb::Slice begin_slice(reinterpret_cast<char *>(begin),
					 jbegin_len);
	...
	compact_options.target_path_id = static_cast<uint32_t>(jtarget_path_id);
	if (cf_handle != nullptr) {
		s = db->CompactRange(compact_options, cf_handle, &begin_slice,
				     &end_slice);
	} else {
		// backwards compatibility
 ...
 * Signature: (J)V
 */
void Java_org_rocksdb_RocksDB_continueBackgroundWork(JNIEnv *env, jobject jobj,
						     jlong jdb_handle)
{
	auto *db = reinterpret_cast<rocksdb::DB *>(jdb_handle);
 ...
 * Signature: (J)V
 */
jlong Java_org_rocksdb_RocksDB_getLatestSequenceNumber(JNIEnv *env, jobject jdb,
						       jlong jdb_handle)
{
	auto *db = reinterpret_cast<rocksdb::DB *>(jdb_handle);
