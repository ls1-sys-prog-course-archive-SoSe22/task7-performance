...
{
	auto *sptr_rate_limiter = new std::shared_ptr<rocksdb::RateLimiter>(
 ...
 * Signature: (J)V
 */
void Java_org_rocksdb_RateLimiter_disposeInternal(JNIEnv *env, jobject jobj,
						  jlong jhandle)
{
	auto *handle =
 ...
 * Signature: (JJ)V
 */
void Java_org_rocksdb_RateLimiter_setBytesPerSecond(JNIEnv *env, jobject jobj,
						    jlong handle,
 ...
 * Signature: (J)J
 */
jlong Java_org_rocksdb_RateLimiter_getTotalRequests(JNIEnv *env, jobject jobj,
						    jlong handle)
{
	return reinterpret_cast<std::shared_ptr<rocksdb::RateLimiter> *>(handle)
		->get()
