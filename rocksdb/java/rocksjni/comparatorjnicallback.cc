	  ...
	  mtx_findShortestSeparator(new port::Mutex(copt->use_adaptive_mutex))
{
	...
	jstring jsName =
		(jstring)env->CallObjectMethod(m_jComparator, jNameMethodId);
...
int BaseComparatorJniCallback::Compare(const Slice &a, const Slice &b) const
{
	...
	jint result = env->CallIntMethod(m_jComparator, m_jCompareMethodId,
					 m_jSliceA, m_jSliceB);
...
void BaseComparatorJniCallback::FindShortestSeparator(std::string *start,
						      const Slice &limit) const
{
	...
	bool pending_exception = AbstractSliceJni::setHandle(env, m_jSliceLimit,
							     &limit, JNI_FALSE);
	if (pending_exception) {
		if (env->ExceptionCheck()) {
			// exception thrown from setHandle or descendant
	...
	if (jsResultStart != nullptr) {
		...
		jboolean has_exception = JNI_FALSE;
		std::string result = JniUtil::copyString(
			env, jsResultStart,
...
void BaseComparatorJniCallback::FindShortSuccessor(std::string *key) const
{
	...
	jstring jsResultKey = (jstring)env->CallObjectMethod(
		m_jComparator, m_jFindShortSuccessorMethodId, jsKey);
	...
	if (jsResultKey != nullptr) {
		...
		jboolean has_exception = JNI_FALSE;
		std::string result =
			JniUtil::copyString(env, jsResultKey, &has_exception);
		if (has_exception == JNI_TRUE) {
			if (env->ExceptionCheck()) {
				env->ExceptionDescribe(); // print out exception to stderr
			}
