 ...
 * Signature: ()J
 */
jlong Java_org_rocksdb_WriteBatchWithIndex_newWriteBatchWithIndex__(JNIEnv *env,
								    jclass jcls)
{
...
{
	auto *wbwi = new rocksdb::WriteBatchWithIndex(
		reinterpret_cast<rocksdb::Comparator *>(
 ...
 * Signature: (J)I
 */
jint Java_org_rocksdb_WriteBatchWithIndex_count0(JNIEnv *env, jobject jobj,
						 jlong jwbwi_handle)
{
	auto *wbwi =
	...
	jlong jcf_handle)
{
	...
	auto put = [&wbwi, &cf_handle](rocksdb::Slice key,
				       rocksdb::Slice value) {
	...
	jlong jcf_handle)
{
	...
	auto *cf_handle =
		reinterpret_cast<rocksdb::ColumnFamilyHandle *>(jcf_handle);
	assert(cf_handle != nullptr);
	auto merge = [&wbwi, &cf_handle](rocksdb::Slice key,
					 rocksdb::Slice value) {
 ...
 * Signature: (J[BI)V
 */
void Java_org_rocksdb_WriteBatchWithIndex_remove__J_3BI(JNIEnv *env,
							jobject jobj,
							jlong jwbwi_handle,
							jbyteArray jkey,
							jint jkey_len)
{
	auto *wbwi =
		reinterpret_cast<rocksdb::WriteBatchWithIndex *>(jwbwi_handle);
	assert(wbwi != nullptr);
	auto remove = [&wbwi](rocksdb::Slice key) { wbwi->Delete(key); };
	...
	jint jbegin_key_len, jbyteArray jend_key, jint jend_key_len)
{
	auto *wbwi =
		reinterpret_cast<rocksdb::WriteBatchWithIndex *>(jwbwi_handle);
	assert(wbwi != nullptr);
	auto deleteRange = [&wbwi](rocksdb::Slice beginKey,
 ...
 * Signature: (J)V
 */
void Java_org_rocksdb_WriteBatchWithIndex_clear0(JNIEnv *env, jobject jobj,
						 jlong jwbwi_handle)
{
	auto *wbwi =
		reinterpret_cast<rocksdb::WriteBatchWithIndex *>(jwbwi_handle);
 ...
 * Signature: (J)J
 */
jlong Java_org_rocksdb_WriteBatchWithIndex_iterator0(JNIEnv *env, jobject jobj,
						     jlong jwbwi_handle)
{
	auto *wbwi =
		reinterpret_cast<rocksdb::WriteBatchWithIndex *>(jwbwi_handle);
 ...
 * Signature: (JJ)J
 */
jlong Java_org_rocksdb_WriteBatchWithIndex_iterator1(JNIEnv *env, jobject jobj,
						     jlong jwbwi_handle,
						     jlong jcf_handle)
{
	auto *wbwi =
	...
	jbyteArray jkey, jint jkey_len)
{
	...
	auto getter = [&wbwi, &dbopt](const rocksdb::Slice &key,
				      std::string *value) {
	...
	jlong jreadopt_handle, jbyteArray jkey, jint jkey_len)
{
	...
	auto *readopt =
		reinterpret_cast<rocksdb::ReadOptions *>(jreadopt_handle);
	...
	jlong jreadopt_handle, jbyteArray jkey, jint jkey_len, jlong jcf_handle)
{
	...
	auto getter = [&wbwi, &db, &cf_handle, &readopt](
		...
		return wbwi->GetFromBatchAndDB(db, *readopt, cf_handle, key,
					       value);
	};
						     ...
						     jlong handle)
{
	...
	if (we.type == rocksdb::kDeleteRecord ||
		...
		auto *value_slice =
			new rocksdb::Slice(we.value.data(), we.value.size());
	...
	if (jresults == nullptr) {
		...
		if (results[2] != 0) {
			auto *value_slice =
				reinterpret_cast<rocksdb::Slice *>(results[2]);
			delete value_slice;
		}
		delete key_slice;
		return nullptr;
