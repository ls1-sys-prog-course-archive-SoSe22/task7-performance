...
void WriteBatchHandlerJniCallback::Put(const Slice &key, const Slice &value)
{
	...
	m_env->CallVoidMethod(m_jWriteBatchHandler, m_jPutMethodId, j_key,
			      j_value);
	if (m_env->ExceptionCheck()) {
		// exception thrown
		m_env->ExceptionDescribe();
...
void WriteBatchHandlerJniCallback::Merge(const Slice &key, const Slice &value)
{
	...
	m_env->CallVoidMethod(m_jWriteBatchHandler, m_jMergeMethodId, j_key,
			      j_value);
	if (m_env->ExceptionCheck()) {
		// exception thrown
		m_env->ExceptionDescribe();
...
void WriteBatchHandlerJniCallback::Delete(const Slice &key)
{
	...
	m_env->CallVoidMethod(m_jWriteBatchHandler, m_jDeleteMethodId, j_key);
	if (m_env->ExceptionCheck()) {
		// exception thrown
		m_env->ExceptionDescribe();
		if (j_key != nullptr) {
...
void WriteBatchHandlerJniCallback::LogData(const Slice &blob)
{
	...
	m_env->CallVoidMethod(m_jWriteBatchHandler, m_jLogDataMethodId, j_blob);
	if (m_env->ExceptionCheck()) {
		// exception thrown
		m_env->ExceptionDescribe();
		if (j_blob != nullptr) {
...
{
	jboolean jContinue = m_env->CallBooleanMethod(m_jWriteBatchHandler,
						      m_jContinueMethodId);
	if (m_env->ExceptionCheck()) {
		// exception thrown
		m_env->ExceptionDescribe();
