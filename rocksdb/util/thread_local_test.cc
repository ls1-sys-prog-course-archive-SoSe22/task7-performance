...
struct Params {
	Params(port::Mutex *m, port::CondVar *c, int *u, int n,
		...
		: mu(m), cv(c), unref(u), total(n), started(0), completed(0),
		  doWrite(false), tls1(handler), tls2(nullptr)
	{
	}
	...
	auto func = [](void *ptr) {
		...
		while (env->NowMicros() - start < 1000 * 1000) {
			for (int iter = 0; iter < 100000; ++iter) {
				ASSERT_TRUE(p.tls1.Get() ==
					    reinterpret_cast<size_t *>(own));
				ASSERT_TRUE(
					p.tls2->Get() ==
					reinterpret_cast<size_t *>(own + 1));
				if (p.doWrite) {
					p.tls1.Reset(reinterpret_cast<size_t *>(
						own));
					p.tls2->Reset(
						reinterpret_cast<size_t *>(own +
									   1));
...
TEST_F(ThreadLocalTest, Swap)
{
	...
	ASSERT_EQ(reinterpret_cast<int64_t>(
			  tls.Swap(reinterpret_cast<void *>(3))),
		  2);
}
...
TEST_F(ThreadLocalTest, Fold)
{
	...
	auto func = [](void *ptr) {
		...
		for (int i = 0; i < kItersPerThread; ++i) {
			static_cast<std::atomic<int64_t> *>(p.tls1.Get())
				->fetch_add(1);
		}
	...
	params.tls1.Fold(
		[](void *ptr, void *res) {
			...
			*sum_ptr +=
				static_cast<std::atomic<int64_t> *>(ptr)->load();
		},
		&sum);
