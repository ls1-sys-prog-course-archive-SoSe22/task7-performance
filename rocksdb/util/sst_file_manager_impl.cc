...
SstFileManagerImpl::SstFileManagerImpl(Env *env, std::shared_ptr<Logger> logger,
	...
	: env_(env), logger_(logger), total_files_size_(0),
...
std::unordered_map<std::string, uint64_t> SstFileManagerImpl::GetTrackedFiles()
{
	MutexLock l(&mu_);
...
{
	SstFileManagerImpl *res = new SstFileManagerImpl(
		env, info_log, trash_dir, rate_bytes_per_sec);
	...
	if (trash_dir != "") {
		...
		if (s.ok() && delete_existing_trash) {
			...
			if (s.ok()) {
				for (const std::string &trash_file :
				     files_in_trash) {
					if (trash_file == "." ||
					    trash_file == "..") {
						continue;
					...
					std::string path_in_trash =
						trash_dir + "/" + trash_file;
					res->OnAddFile(path_in_trash);
					Status file_delete =
						res->ScheduleFileDeletion(
							path_in_trash);
