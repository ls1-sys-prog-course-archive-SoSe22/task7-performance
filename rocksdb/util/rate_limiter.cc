...
GenericRateLimiter::GenericRateLimiter(int64_t rate_bytes_per_sec,
	...
	: refill_period_us_(refill_period_us),
	  ...
	  env_(Env::Default()), stop_(false), exit_cv_(&request_mutex_),
	  requests_to_wait_(0), available_bytes_(0),
	  next_refill_us_(NowMicrosMonotonic(env_)),
	  fairness_(fairness > 100 ? 100 : fairness),
	  rnd_((uint32_t)time(nullptr)), leader_(nullptr)
{
	total_requests_[0] = 0;
	total_requests_[1] = 0;
...
void GenericRateLimiter::Refill()
{
	...
	for (int q = 0; q < 2; ++q) {
		auto use_pri =
			(use_low_pri_first == q) ? Env::IO_LOW : Env::IO_HIGH;
...
int64_t
GenericRateLimiter::CalculateRefillBytesPerPeriod(int64_t rate_bytes_per_sec)
{
	if (port::kMaxInt64 / rate_bytes_per_sec < refill_period_us_) {
		...
		return std::max(kMinRefillBytesPerPeriod,
				rate_bytes_per_sec * refill_period_us_ /
					1000000);
	}
}
				   ...
				   int64_t refill_period_us, int32_t fairness)
{
	...
	assert(fairness > 0);
	return new GenericRateLimiter(rate_bytes_per_sec, refill_period_us,
				      fairness);
}
