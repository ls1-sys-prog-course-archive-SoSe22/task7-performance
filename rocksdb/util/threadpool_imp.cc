...
struct ThreadPoolImpl::Impl {
	...
	{
		return static_cast<int>(bgthreads_.size()) >
...
inline ThreadPoolImpl::Impl::Impl()
	: low_io_priority_(false), priority_(Env::LOW), env_(nullptr),
	  total_threads_limit_(1), queue_len_(), exit_all_threads_(false),
	  wait_for_jobs_to_complete_(false), queue_(), mu_(), bgsignal_(),
	  bgthreads_()
{
}
...
void ThreadPoolImpl::Impl::BGThread(size_t thread_id)
{
	...
	while (true) {
		...
		bool decrease_io_priority =
			(low_io_priority != low_io_priority_);
		lock.unlock();
...
void *ThreadPoolImpl::Impl::BGThreadWrapper(void *arg)
{
	...
	// for thread-status
	ThreadStatusUtil::RegisterThread(tp->GetHostEnv(),
					 (tp->GetThreadPriority() ==
							  Env::Priority::HIGH ?
...
// Set the thread name to aid debugging
		...
		snprintf(name_buf, sizeof name_buf,
			 "rocksdb:bg%" ROCKSDB_PRIszt, bgthreads_.size());
...
void ThreadPoolImpl::SetHostEnv(Env *env)
{
	impl_->SetHostEnv(env);
}

Env *ThreadPoolImpl::GetHostEnv() const
{
	return impl_->GetHostEnv();
}
