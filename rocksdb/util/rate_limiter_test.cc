...
TEST_F(RateLimiterTest, StartStop)
{
	std::unique_ptr<RateLimiter> limiter(
		NewGenericRateLimiter(100, 100, 10));
}
...
TEST_F(RateLimiterTest, Rate)
{
	...
	struct Arg {
		Arg(int32_t _target_rate, int _burst)
			: limiter(NewGenericRateLimiter(_target_rate,
							100 * 1000, 10)),
			  request_size(_target_rate / 10), burst(_burst)
		{
		}
		std::unique_ptr<RateLimiter> limiter;
	...
	auto writer = [](void *p) {
		...
		while (thread_env->NowMicros() < until) {
			for (int i = 0;
			     i < static_cast<int>(r.Skewed(arg->burst) + 1);
			     ++i) {
				arg->limiter->Request(
					r.Uniform(arg->request_size - 1) + 1,
					Env::IO_HIGH, nullptr /* stats */);
			}
			arg->limiter->Request(r.Uniform(arg->request_size - 1) +
						      1,
	...
	for (int i = 1; i <= 16; i *= 2) {
		...
		for (int iter = 1; iter <= 2; ++iter) {
			...
			double rate = (arg.limiter->GetTotalBytesThrough() -
				       old_total_bytes_through) *
				      1000000.0 / elapsed;
			old_total_bytes_through =
				arg.limiter->GetTotalBytesThrough();
			fprintf(stderr,
				"request size [1 - %" PRIi32 "], limit %" PRIi32
				" KB/sec, actual rate: %lf KB/sec, elapsed %.2lf seconds\n",
				arg.request_size - 1, target / 1024,
...
TEST_F(RateLimiterTest, LimitChangeTest)
{
	...
	struct Arg {
		Arg(int32_t _request_size, Env::IOPriority _pri,
			...
			: request_size(_request_size), pri(_pri),
			  limiter(_limiter)
		{
		}
		int32_t request_size;
	...
	for (uint32_t i = 1; i <= 16; i <<= 1) {
		...
		for (int iter = 0; iter < 2; iter++) {
			...
			env->WaitForJoin();
			fprintf(stderr,
				"[COMPLETE] request size %" PRIi32
				" KB, new limit %" PRIi32
				"KB/sec, refill period %" PRIi64 " ms\n",
				target / 1024, new_limit / 1024,
				refill_period / 1000);
		}
	}
}
