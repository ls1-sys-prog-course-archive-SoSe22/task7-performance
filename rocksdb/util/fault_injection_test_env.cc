...
TestWritableFile::TestWritableFile(const std::string &fname,
	...
	: state_(fname), target_(std::move(f)), writable_file_opened_(true),
	  env_(env)
{
					      ...
					      const EnvOptions &soptions)
{
	...
	if (s.ok()) {
		result->reset(
			new TestWritableFile(fname, std::move(*result), this));
		...
		auto &list =
			dir_to_new_files_since_last_sync_[dir_and_name.first];
					 ...
					 const std::string &t)
{
	...
	if (ret.ok()) {
		...
		if (dir_to_new_files_since_last_sync_[sdn.first].erase(
			    sdn.second) != 0) {
			auto &tlist =
				dir_to_new_files_since_last_sync_[tdn.first];
