...
class SimulatedBackgroundTask {
	...
	SimulatedBackgroundTask(
		...
		: db_key_(db_key), db_name_(db_name), cf_key_(cf_key),
		  cf_name_(cf_name), operation_type_(operation_type),
		  state_type_(state_type), should_run_(true), running_count_(0)
	{
	...
	~SimulatedBackgroundTask()
	{
		Env::Default()->GetThreadStatusUpdater()->EraseDatabaseInfo(
			db_key_);
	}
	...
	void Run()
	{
		...
		running_count_++;
		Env::Default()->GetThreadStatusUpdater()->SetColumnFamilyInfoKey(
			cf_key_);
		Env::Default()->GetThreadStatusUpdater()->SetThreadOperation(
			operation_type_);
		Env::Default()->GetThreadStatusUpdater()->SetThreadState(
			state_type_);
		while (should_run_) {
			bg_cv_.wait(l);
		}
...
TEST_F(ThreadListTest, SimpleColumnFamilyInfoTest)
{
	...
	SimulatedBackgroundTask running_task(reinterpret_cast<void *>(1234),
					     "running",
	...
	ASSERT_EQ(running_count[ThreadStatus::LOW_PRIORITY],
...
TEST_F(ThreadListTest, SimpleEventTest)
{
	...
	SimulatedBackgroundTask flush_write_task(reinterpret_cast<void *>(1234),
						 "running",
						 reinterpret_cast<void *>(5678),
						 "pikachu",
	...
	correct_operation_counts[ThreadStatus::OP_COMPACTION] =
		kCompactionWriteTasks + kCompactionReadTasks +
