...
inline bool GetDecompressedSizeInfo(const char **input_data,
				    size_t *input_length, uint32_t *output_len)
{
	auto new_input_data = GetVarint32Ptr(
			  ...
			  const Slice &compression_dict = Slice())
{
	...
	int st = deflateInit2(&_stream, opts.level, Z_DEFLATED,
			      opts.window_bits, memLevel, opts.strategy);
	if (st != Z_OK) {
		return false;
	}
	...
	_stream.next_out =
		reinterpret_cast<Bytef *>(&(*output)[output_header_len]);
			     ...
			     int windowBits = -14)
{
	...
	if (compress_format_version == 2) {
		if (!compression::GetDecompressedSizeInfo(
			    &input_data, &input_length, &output_len)) {
			return nullptr;
		}
	} else {
		// Assume the decompressed data size will 5x of compressed size, but round
		// to the page size
		size_t proposed_output_len =
			((input_length * 5) & (~(4096 - 1))) + 4096;
		output_len = static_cast<uint32_t>(std::min(
			proposed_output_len,
			static_cast<size_t>(
	...
	while (!done) {
		...
		case Z_OK: {
			...
			output_len +=
				output_len_delta < 10 ? 10 : output_len_delta;
			...
			_stream.avail_out =
				static_cast<unsigned int>(output_len - old_sz);
			break;
		}
		case Z_BUF_ERROR:
		default:
			   ...
			   size_t length, ::std::string *output)
{
	...
	_stream.avail_out = static_cast<unsigned int>(length);
	_stream.next_out =
		reinterpret_cast<char *>(&(*output)[output_header_len]);
			 ...
			 const Slice compression_dict = Slice())
{
	...
	outlen = LZ4_compress_fast_continue(stream, input,
			    ...
			    const Slice &compression_dict = Slice())
{
	...
	*decompress_size =
		LZ4_decompress_safe_continue(stream, input_data, output,
					     static_cast<int>(input_length),
					     static_cast<int>(output_len));
	LZ4_freeStreamDecode(stream);
#else // up to r123
	*decompress_size = LZ4_decompress_safe(input_data, output,
					       static_cast<int>(input_length),
					       static_cast<int>(output_len));
#endif // LZ4_VERSION_NUMBER >= 10400
			   ...
			   const Slice &compression_dict = Slice())
{
...
#if LZ4_VERSION_NUMBER >= 10700 // r129+
	outlen = LZ4_compress_HC_continue(stream, input,
					  &(*output)[output_header_len],
					  static_cast<int>(length),
					  compress_bound);
#else // r124-r128
	...
	outlen = LZ4_compressHC2_limitedOutput(input,
					       &(*output)[output_header_len],
					       static_cast<int>(length),
					       compress_bound, opts.level);
#else // up to r112
	outlen = LZ4_compressHC_limitedOutput(input,
					      &(*output)[output_header_len],
					      static_cast<int>(length),
					      compress_bound);
#endif // LZ4_VERSION_NUMBER >= 10400
...
{
#ifdef XPRESS
	return port::xpress::Decompress(input_data, input_length,
					decompress_size);
#endif
	return nullptr;
}
			  ...
			  const Slice &compression_dict = Slice())
{
	...
	outlen = ZSTD_compress_usingDict(context, &(*output)[output_header_len],
					 ...
					 compression_dict.data(),
					 compression_dict.size(), opts.level);
