...
{
	if (port::kLittleEndian) {
		dst->append(const_cast<const char *>(
					...
					const SliceParts &slice_parts)
{
	...
	for (int i = 0; i < slice_parts.num_parts; ++i) {
		dst->append(slice_parts.parts[i].data(),
			    slice_parts.parts[i].size());
	}
}
...
{
	uint32_t len = 0;
	for (len = 0; len < slice->size() && slice->data()[len] != delimiter;
	     ++len) {
		// nothing
	}
...
PutUnaligned(T *memory, const T &value)
{
	...
	memcpy(nonAlignedMemory, reinterpret_cast<const char *>(&value),
	       sizeof(T));
#else
	*memory = value;
#endif
...
GetUnaligned(const T *memory, T *value)
{
	...
	memcpy(nonAlignedMemory, reinterpret_cast<const char *>(memory),
	       sizeof(T));
#else
	*value = *memory;
#endif
