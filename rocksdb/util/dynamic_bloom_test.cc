...
TEST_F(DynamicBloomTest, VaryingLengths)
{
	...
	for (uint32_t enable_locality = 0; enable_locality < 2;
	     ++enable_locality) {
		for (uint32_t num = 1; num <= 10000; num = NextNum(num)) {
			...
			if (enable_locality == 0) {
				bloom_bits =
					std::max(num * FLAGS_bits_per_key, 64U);
			} else {
				bloom_bits = std::max(
					num * FLAGS_bits_per_key,
					enable_locality * CACHE_LINE_SIZE * 8);
			}
			DynamicBloom bloom(&arena, bloom_bits, enable_locality,
					   num_probes);
...
TEST_F(DynamicBloomTest, perf)
{
	...
	for (uint32_t m = 1; m <= 8; ++m) {
		...
		timer.Start();
		for (uint64_t i = 1; i <= num_keys; ++i) {
			if (std_bloom.MayContain(Slice(
				    reinterpret_cast<const char *>(&i), 8))) {
				++count;
			}
		}
		...
		assert(count > 0);
		fprintf(stderr,
			"standard bloom, avg query latency %" PRIu64 "\n",
			elapsed / count);

		// Locality enabled version
		DynamicBloom blocked_bloom(&arena, num_keys * 10, 1,
					   num_probes);
		...
		timer.Start();
		for (uint64_t i = 1; i <= num_keys; ++i) {
			if (blocked_bloom.MayContain(Slice(
				    reinterpret_cast<const char *>(&i), 8))) {
				++count;
			}
		}
...
TEST_F(DynamicBloomTest, concurrent_with_perf)
{
	...
	for (uint32_t m = 1; m <= m_limit; ++m) {
		for (uint32_t locality = 0; locality <= locality_limit;
		     ++locality) {
			Arena arena;
			const uint32_t num_keys = m * 8 * 1024 * 1024;
			fprintf(stderr,
				"testing %" PRIu32 "M keys with %" PRIu32
				" locality\n",
				m * 8, locality);

			DynamicBloom std_bloom(&arena, num_keys * 10, locality,
					       num_probes);

			timer.Start();

			std::function<void(size_t)> adder = [&](size_t t) {
				for (uint64_t i = 1 + t; i <= num_keys;
				     i += num_threads) {
					std_bloom.AddConcurrently(Slice(
			...
			std::function<void(size_t)> hitter = [&](size_t t) {
				for (uint64_t i = 1 + t; i <= num_keys;
				     i += num_threads) {
					bool f = std_bloom.MayContain(Slice(
						reinterpret_cast<const char *>(
							&i),
						8));
					ASSERT_TRUE(f);
			...
			std::function<void(size_t)> misser = [&](size_t t) {
				for (uint64_t i = num_keys + 1 + t;
				     i <= 2 * num_keys; i += num_threads) {
					bool f = std_bloom.MayContain(Slice(
						reinterpret_cast<const char *>(
							&i),
						8));
