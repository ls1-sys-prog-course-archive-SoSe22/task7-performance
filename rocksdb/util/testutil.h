...
class VectorIterator : public InternalIterator {
	...
	{
		current_ = std::upper_bound(keys_.begin(), keys_.end(),
					    target.ToString()) -
			   keys_.begin();
		if (!Valid()) {
			SeekToLast();
...
class StringSink : public WritableFile {
	...
	explicit StringSink(Slice *reader_contents = nullptr)
		: WritableFile(), contents_(""),
		  reader_contents_(reader_contents), last_flush_(0)
	{
	...
	virtual Status Flush() override
	{
		if (reader_contents_ != nullptr) {
			...
			*reader_contents_ = Slice(contents_.data() + offset,
		    ...
		    char *scratch) const
	{
		*result = Slice(nullptr, 0);
		if (offset < ss_->contents_.size()) {
			size_t str_res_sz = std::min(
				static_cast<size_t>(ss_->contents_.size() -
						    offset),
				n);
			*result = Slice(ss_->contents_.data() + offset,
					str_res_sz);
		}
		return Status::OK();
	}
...
class StringSource : public RandomAccessFile {
	...
	explicit StringSource(const Slice &contents, uint64_t uniq_id = 0,
		...
		: contents_(contents.data(), contents.size()),
		  uniq_id_(uniq_id), mmap_(mmap), total_reads_(0)
	{
	}
			    ...
			    char *scratch) const override
	{
		...
		if (!mmap_) {
			memcpy(scratch, &contents_[static_cast<size_t>(offset)],
			       n);
			*result = Slice(scratch, n);
		} else {
			*result = Slice(&contents_[static_cast<size_t>(offset)],
					n);
		}
		return Status::OK();
	}
			       ...
			       const EnvOptions &options) override
	{
		...
		if (iter != files_.end()) {
			return Status::IOError(
				"The specified file already exists", f);
		}
