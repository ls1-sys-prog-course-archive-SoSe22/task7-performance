...
DeleteScheduler::DeleteScheduler(Env *env, const std::string &trash_dir,
	...
	: env_(env), trash_dir_(trash_dir),
	  rate_bytes_per_sec_(rate_bytes_per_sec), pending_files_(0),
	  closing_(false), cv_(&mu_), info_log_(info_log),
...
Status DeleteScheduler::DeleteFile(const std::string &file_path)
{
	...
	if (!s.ok()) {
		ROCKS_LOG_ERROR(info_log_,
				"Failed to move %s to trash directory (%s)",
...
void DeleteScheduler::BackgroundEmptyTrash()
{
	...
	while (true) {
		...
		while (!queue_.empty() && !closing_) {
			...
			Status s =
				DeleteTrashFile(path_in_trash, &deleted_bytes);
			...
			if (current_delete_rate > 0) {
				...
				total_penlty = ((total_deleted_bytes *
						 kMicrosInSecond) /
						current_delete_rate);
				while (!closing_ &&
				       !cv_.TimedWait(start_time +
						      total_penlty)) {
				}
			} else {
				// rate limiting is disabled
