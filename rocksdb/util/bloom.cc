...
class FullFilterBitsBuilder : public FilterBitsBuilder {
	...
	virtual Slice Finish(std::unique_ptr<const char[]> *buf) override
	{
		...
		char *data =
			ReserveSpace(static_cast<int>(hash_entries_.size()),
		...
		EncodeFixed32(data + total_bits / 8 + 1,
					  ...
					  uint32_t *num_lines)
{
	...
	char *data = nullptr;
	if (num_entry != 0) {
		uint32_t total_bits_tmp =
			num_entry * static_cast<uint32_t>(bits_per_key_);
...
class FullFilterBitsReader : public FilterBitsReader {
	...
	virtual bool MayMatch(const Slice &entry) override
	{
		...
		if (num_probes_ == 0 || num_lines_ == 0)
			return true;
		uint32_t hash = BloomHash(entry);
		return HashMayMatch(hash, Slice(data_, data_len_), num_probes_,
				    num_lines_);
	}
...
class BloomFilterPolicy : public FilterPolicy {
				  ...
				  std::string *dst) const override
	{
		...
		dst->push_back(
			static_cast<char>(num_probes_)); // Remember # of probes
	...
	void initialize()
	{
		...
		num_probes_ = static_cast<size_t>(bits_per_key_ *
						  0.69); // 0.69 =~ ln(2)
		if (num_probes_ < 1)
			num_probes_ = 1;
		if (num_probes_ > 30)
