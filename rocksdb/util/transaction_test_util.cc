...
RandomTransactionInserter::RandomTransactionInserter(
	...
	: rand_(rand), write_options_(write_options),
	  read_options_(read_options), num_keys_(num_keys), num_sets_(num_sets)
{
}
...
{
	optimistic_txn_ = db->BeginTransaction(write_options_, txn_options,
					       optimistic_txn_);
					 ...
					 bool is_optimistic)
{
	...
	for (uint8_t i = 0; i < num_sets_; i++) {
		...
		std::string full_key =
			std::string(prefix_buf) + base_key.ToString();
		Slice key(full_key);
...
Status RandomTransactionInserter::Verify(DB *db, uint16_t num_sets)
{
	...
	for (uint32_t i = 0; i < num_sets; i++) {
		...
		for (iter->Seek(Slice(prefix_buf, 4)); iter->Valid();
		     iter->Next()) {
			Slice key = iter->key();
