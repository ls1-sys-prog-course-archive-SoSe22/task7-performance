...
class DeleteSchedulerTest : public testing::Test {
	...
	{
		dummy_files_dir_ =
			test::TmpDir(env_) + "/delete_scheduler_dummy_data_dir";
	...
	std::string NewDummyFile(const std::string &file_name,
				 uint64_t size = 1024)
	{
	...
	void NewDeleteScheduler()
	{
		...
		delete_scheduler_.reset(new DeleteScheduler(env_, trash_dir_,
							    rate_bytes_per_sec_,
							    nullptr, nullptr));
	}
	...
	for (size_t t = 0; t < delete_kbs_per_sec.size(); t++) {
		...
		for (int i = 0; i < num_files; i++) {
			...
			generated_files.push_back(
				NewDummyFile(file_name, file_size));
		}

		// Delete dummy files and measure time spent to empty trash
		for (int i = 0; i < num_files; i++) {
			ASSERT_OK(delete_scheduler_->DeleteFile(
				generated_files[i]));
		}
		...
		uint64_t time_spent_deleting =
			env_->NowMicros() - delete_start_time;
		...
		for (int i = 0; i < num_files; i++) {
			...
			expected_penlty = ((total_files_size * 1000000) /
					   rate_bytes_per_sec_);
...
TEST_F(DeleteSchedulerTest, BackgroundError)
{
	...
	for (int i = 0; i < 10; i++) {
		...
		ASSERT_OK(
			delete_scheduler_->DeleteFile(NewDummyFile(file_name)));
	}
	ASSERT_EQ(CountFilesInDir(dummy_files_dir_), 0);
	ASSERT_EQ(CountFilesInDir(trash_dir_), 10);
...
TEST_F(DeleteSchedulerTest, DISABLED_DynamicRateLimiting1)
{
	...
	std::vector<int64_t> delete_kbs_per_sec = {
	...
	for (size_t t = 0; t < delete_kbs_per_sec.size(); t++) {
		...
		if (rate_bytes_per_sec_ > 0) {
			...
			auto bg_errors =
				delete_scheduler_->GetBackgroundErrors();
			ASSERT_EQ(bg_errors.size(), 0);
