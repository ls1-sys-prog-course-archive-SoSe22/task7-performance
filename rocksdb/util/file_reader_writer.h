...
class SequentialFileReader {
	...
	operator=(SequentialFileReader &&o) ROCKSDB_NOEXCEPT
	{
...
class RandomAccessFileReader {
	...
	explicit RandomAccessFileReader(std::unique_ptr<RandomAccessFile> &&raf,
		...
		: file_(std::move(raf)), env_(env), stats_(stats),
		  hist_type_(hist_type), file_read_hist_(file_read_hist)
	{
	}
	...
	operator=(RandomAccessFileReader &&o) ROCKSDB_NOEXCEPT
	{
		file_ = std::move(o.file_);
	...
	RandomAccessFileReader &
	operator=(const RandomAccessFileReader &) = delete;
...
class WritableFileWriter {
	...
	WritableFileWriter(std::unique_ptr<WritableFile> &&file,
			   ...
			   Statistics *stats = nullptr)
		: writable_file_(std::move(file)), buf_(),
		  max_buffer_size_(options.writable_file_max_buffer_size),
		  filesize_(0),
#ifndef ROCKSDB_LITE
		  next_write_offset_(0),
#endif // ROCKSDB_LITE
		  pending_sync_(false), last_sync_size_(0),
		  bytes_per_sync_(options.bytes_per_sync),
		  rate_limiter_(options.rate_limiter), stats_(stats)
	{
		buf_.Alignment(writable_file_->GetRequiredBufferAlignment());
		buf_.AllocateNewBuffer(
			use_direct_io() ?
				      max_buffer_size_ :
