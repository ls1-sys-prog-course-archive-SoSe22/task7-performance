...
template <typename T> CoreLocalArray<T>::CoreLocalArray()
{
...
template <typename T> size_t CoreLocalArray<T>::Size() const
{
	return static_cast<size_t>(1) << size_shift_;
}

template <typename T> T *CoreLocalArray<T>::Access() const
{
...
std::pair<T *, size_t> CoreLocalArray<T>::AccessElementAndIndex() const
{
	...
	if (UNLIKELY(cpuid < 0)) {
		...
		core_idx =
			static_cast<size_t>(cpuid & ((1 << size_shift_) - 1));
	}
	return { AccessAtCore(core_idx), core_idx };
}

template <typename T> T *CoreLocalArray<T>::AccessAtCore(size_t core_idx) const
{
