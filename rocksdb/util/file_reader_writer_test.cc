...
TEST_F(WritableFileWriterTest, AppendStatusReturn)
{
	class FakeWF : public WritableFile {
		...
		void Setuse_direct_io(bool val)
		{
			use_direct_io_ = val;
		}
...
class ReadaheadRandomAccessFileTest
	...
	void ResetSourceStr(const std::string &str = "")
	{
		...
		test_read_holder_ = NewReadaheadRandomAccessFile(
			std::move(read_holder), readahead_size_);
	}
	size_t GetReadaheadSize() const
	{
       ...
       SourceStrLenCanBeGreaterThanReadaheadSizeTest)
{
	...
	for (int k = 0; k < 100; ++k) {
		...
		std::string str = test::RandomHumanReadableString(
			&rng, static_cast<int>(strLen));
		ResetSourceStr(str);
		for (int test = 1; test <= 100; ++test) {
			size_t offset = rng.Uniform(static_cast<int>(strLen));
			size_t n = rng.Uniform(
				static_cast<int>(GetReadaheadSize()));
			ASSERT_EQ(str.substr(offset,
					     std::min(n, str.size() - offset)),
				  Read(offset, n));
		}
	}
}
...
INSTANTIATE_TEST_CASE_P(
	...
	::testing::ValuesIn(
		ReadaheadRandomAccessFileTest::GetReadaheadSizeList()));
INSTANTIATE_TEST_CASE_P(
