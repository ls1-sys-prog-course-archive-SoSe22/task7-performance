...
Status SequentialFileReader::Read(size_t n, Slice *result, char *scratch)
{
	...
	if (use_direct_io()) {
		...
		Slice tmp;
		s = file_->PositionedRead(aligned_offset, size, &tmp,
					  buf.BufferStart());
				    ...
				    char *scratch) const
{
	...
	uint64_t elapsed = 0;
	{
		...
		if (use_direct_io()) {
			...
			size_t offset_advance = offset - aligned_offset;
			size_t size =
				Roundup(offset + n, alignment) - aligned_offset;
			size_t r = 0;
			AlignedBuffer buf;
			buf.Alignment(alignment);
			buf.AllocateNewBuffer(size);
			Slice tmp;
			s = file_->Read(aligned_offset, size, &tmp,
					buf.BufferStart());
			if (s.ok() && offset_advance < tmp.size()) {
				buf.Size(tmp.size());
				r = buf.Read(
					scratch, offset_advance,
					std::min(tmp.size() - offset_advance,
						 n));
			}
			*result = Slice(scratch, r);
#endif // !ROCKSDB_LITE
...
Status WritableFileWriter::Append(const Slice &data)
{
...

	{
		...
		writable_file_->PrepareWrite(static_cast<size_t>(GetFileSize()),
					     left);
	}
	...
	if (!use_direct_io() && (buf_.Capacity() - buf_.CurrentSize()) < left) {
		...
		if (buf_.Capacity() < max_buffer_size_) {
			...
			desiredCapacity =
				std::min(desiredCapacity, max_buffer_size_);
...
Status WritableFileWriter::Flush()
{
	...
	if (buf_.CurrentSize() > 0) {
		if (use_direct_io()) {
			...
			s = WriteBuffered(buf_.BufferStart(),
					  buf_.CurrentSize());
		}
		if (!s.ok()) {
			return s;
	...
	if (!use_direct_io() && bytes_per_sync_) {
		...
		if (filesize_ > kBytesNotSyncRange) {
			uint64_t offset_sync_to =
				filesize_ - kBytesNotSyncRange;
			offset_sync_to -= offset_sync_to % kBytesAlignWhenSync;
			assert(offset_sync_to >= last_sync_size_);
			if (offset_sync_to > 0 &&
			    offset_sync_to - last_sync_size_ >=
				    bytes_per_sync_) {
				s = RangeSync(last_sync_size_,
					      offset_sync_to - last_sync_size_);
...
size_t WritableFileWriter::RequestToken(size_t bytes, bool align)
{
	...
	if (rate_limiter_ &&
		...
		bytes = std::min(bytes,
				 static_cast<size_t>(
					 rate_limiter_->GetSingleBurstBytes()));
		...
		if (align) {
			...
			bytes = std::max(alignment, TruncateToPageBoundary(
							    alignment, bytes));
		}
...
Status WritableFileWriter::WriteDirect()
{
	...
	while (left > 0) {
...

		{
			...
			s = writable_file_->PositionedAppend(Slice(src, size),
							     write_offset);
			if (!s.ok()) {
...
class ReadaheadRandomAccessFile : public RandomAccessFile {
	...
	ReadaheadRandomAccessFile(std::unique_ptr<RandomAccessFile> &&file,
		...
		: file_(std::move(file)),
		  ...
		  buffer_(), buffer_offset_(0), buffer_len_(0)
	{
	...
	operator=(const ReadaheadRandomAccessFile &) = delete;
			    ...
			    char *scratch) const override
	{
		...
		size_t chunk_offset =
			TruncateToPageBoundary(alignment_, advanced_offset);
		...
		if (s.ok()) {
			...
			if (advanced_offset < chunk_offset + buffer_len_) {
				...
				size_t chunk_padding =
					advanced_offset - chunk_offset;
				auto remaining_len =
					std::min(buffer_len_ - chunk_padding,
						 n - cached_len);
				memcpy(scratch + cached_len,
				       buffer_.BufferStart() + chunk_padding,
				       remaining_len);
				*result = Slice(scratch,
						cached_len + remaining_len);
			} else {
				*result = Slice(scratch, cached_len);
			}
		}
	...
	virtual Status Prefetch(uint64_t offset, size_t n) override
	{
		size_t prefetch_offset =
			TruncateToPageBoundary(alignment_, offset);
		if (prefetch_offset == buffer_offset_) {
			return Status::OK();
		}
		return ReadIntoBuffer(prefetch_offset,
				      Roundup(offset + n, alignment_) -
					      prefetch_offset);
	}
			      ...
			      char *scratch) const
	{
		...
		*cached_len = std::min(
			buffer_len_ - static_cast<size_t>(offset_in_buffer), n);
		memcpy(scratch, buffer_.BufferStart() + offset_in_buffer,
		       *cached_len);
		return true;
	}
	...
	Status ReadIntoBuffer(uint64_t offset, size_t n) const
	{
		...
		Slice result;
		Status s =
			file_->Read(offset, n, &result, buffer_.BufferStart());
		if (s.ok()) {
...
NewReadaheadRandomAccessFile(std::unique_ptr<RandomAccessFile> &&file,
			     size_t readahead_size)
{
