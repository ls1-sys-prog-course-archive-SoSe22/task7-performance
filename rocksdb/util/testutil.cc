...
Slice RandomString(Random *rnd, int len, std::string *dst)
{
	...
	for (int i = 0; i < len; i++) {
		(*dst)[i] =
			static_cast<char>(' ' + rnd->Uniform(95)); // ' ' .. '~'
	}
	return Slice(*dst);
}
...
void RandomInitDBOptions(DBOptions *db_opt, Random *rnd)
{
	...
	db_opt->delete_obsolete_files_period_micros =
...
void RandomInitCFOptions(ColumnFamilyOptions *cf_opt, Random *rnd)
{
	...
	// vector int options
	cf_opt->max_bytes_for_level_multiplier_additional.resize(
		cf_opt->num_levels);
	for (int i = 0; i < cf_opt->num_levels; i++) {
		cf_opt->max_bytes_for_level_multiplier_additional[i] =
			rnd->Uniform(100);
	}
	...
	// uint64_t options
	static const uint64_t uint_max = static_cast<uint64_t>(UINT_MAX);
	cf_opt->max_sequential_skip_in_iterations =
	...
	cf_opt->compaction_filter_factory.reset(
		RandomCompactionFilterFactory(rnd));
