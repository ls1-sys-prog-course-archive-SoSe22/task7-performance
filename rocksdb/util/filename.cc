			     ...
			     const std::string &db_absolute_path)
{
	if (!has_log_dir) {
		...
		size_t len =
			GetInfoLogPrefix(db_absolute_path, buf, sizeof(buf));
	...
	if (rest == "IDENTITY") {
		...
		*type = is_temp_file ? kTempFile : kOptionsFile;
		...
		if (rest.starts_with(ARCHIVAL_DIR)) {
			...
			rest.remove_prefix(ARCHIVAL_DIR.size() +
...
Status SetCurrentFile(Env *env, const std::string &dbname,
		      uint64_t descriptor_number, Directory *directory_to_fsync)
{
	...
	if (s.ok()) {
		TEST_KILL_RANDOM("SetCurrentFile:0",
				 rocksdb_kill_odds * REDUCE_ODDS2);
		s = env->RenameFile(tmp, CurrentFileName(dbname));
		TEST_KILL_RANDOM("SetCurrentFile:1",
				 rocksdb_kill_odds * REDUCE_ODDS2);
	}
	if (s.ok()) {
		    ...
		    WritableFileWriter *file)
{
	...
	StopWatch sw(env, db_options->statistics.get(),
