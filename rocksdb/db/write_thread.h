...
class WriteThread {
	...
	struct WriteGroup {
		...
		{
			return Iterator(leader, last_writer);
		}
		Iterator end() const
		{
	...
	struct Writer {
		...
		std::atomic<uint8_t>
			state; // write under StateMutex() or pre-link
		...
		std::aligned_storage<sizeof(std::condition_variable)>::type
			state_cv_bytes;
		Writer *link_older; // read/write only before linking, or as leader
		Writer *link_newer; // lazy, read/write only before linking, or as leader

		Writer()
			: batch(nullptr), sync(false), no_slowdown(false),
			  disable_wal(false), disable_memtable(false),
			  log_used(0), log_ref(0), callback(nullptr),
			  made_waitable(false), state(STATE_INIT),
			  write_group(nullptr), link_older(nullptr),
			  link_newer(nullptr)
		{
		}
		...
		Writer(const WriteOptions &write_options, WriteBatch *_batch,
			...
			: batch(_batch), sync(write_options.sync),
			  no_slowdown(write_options.no_slowdown),
			  disable_wal(write_options.disableWAL),
			  disable_memtable(_disable_memtable), log_used(0),
			  log_ref(_log_ref), callback(_callback),
			  made_waitable(false), state(STATE_INIT),
			  write_group(nullptr), link_older(nullptr),
			  link_newer(nullptr)
		{
		...
		{
			return status.ok() && !CallbackFailed() &&
			       !disable_memtable;
		}
	...
	struct AdaptationContext {
		...
		explicit AdaptationContext(const char *name0)
			: name(name0), value(0)
		{
		}
	};
	...
	uint8_t AwaitState(Writer *w, uint8_t goal_mask,
	...
	bool LinkGroup(WriteGroup &write_group,
