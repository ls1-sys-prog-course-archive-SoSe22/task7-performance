...
class LogFileImpl : public LogFile {
	...
	LogFileImpl(uint64_t logNum, WalFileType logType,
		...
		: logNumber_(logNum), type_(logType), startSequence_(startSeq),
...
class TransactionLogIteratorImpl : public TransactionLogIterator {
	...
	struct LogReporter : public log::Reader::Reporter {
		...
		{
			ROCKS_LOG_INFO(info_log, "%s", s);
		}
	} reporter_;

	SequenceNumber
		currentBatchSeq_; // sequence number at start of current batch
	...
	void SeekToStartSequence(uint64_t startFileIndex = 0,
				 bool strict = false);
