...
class CompactionJobStatsTest : public testing::Test,
				  ...
				  const Options &options)
	{
		...
		for (auto cf : cfs) {
			ASSERT_OK(db_->CreateColumnFamily(cf_opts, cf,
							  &handles_[cfi++]));
		}
	}
					   ...
					   const std::vector<Options> &options)
	{
		...
		for (size_t i = 0; i < cfs.size(); ++i) {
			column_families.push_back(
				ColumnFamilyDescriptor(cfs[i], options[i]));
		}
		DBOptions db_opts = DBOptions(options[0]);
		return DB::Open(db_opts, dbname_, column_families, &handles_,
				&db_);
	}
	...
	int NumTableFilesAtLevel(int level, int cf = 0)
	{
		...
		if (cf == 0) {
			// default cfd
			EXPECT_TRUE(
				db_->GetProperty("rocksdb.num-files-at-level" +
							 NumberToString(level),
						 &property));
		} else {
			EXPECT_TRUE(
				db_->GetProperty(handles_[cf],
	...
	{
		int num_levels = (cf == 0) ? db_->NumberLevels() :
						   db_->NumberLevels(handles_[1]);
		std::string result;
		     ...
		     uint32_t target_path_id)
	{
		...
		ASSERT_OK(db_->CompactRange(compact_options, handles_[cf],
					    &start, &limit));
	}
	...
	{
		ASSERT_OK(db_->CompactRange(CompactRangeOptions(), &start,
					    &limit));
	}

	void TEST_Compact(int level, int cf, const Slice &start,
			  const Slice &limit)
	{
		ASSERT_OK(dbfull()->TEST_CompactRange(
			level, &start, &limit, handles_[cf],
	...
	void MakeTables(int n, const std::string &small,
			const std::string &large, int cf = 0)
	{
		for (int i = 0; i < n; i++) {
	...
	void MakeTableWithKeyValues(Random *rnd, uint64_t smallest,
				    uint64_t largest, int key_size,
				    int value_size, uint64_t interval,
				    double ratio, int cf = 0)
	{
		for (auto key = smallest; key < largest; key += interval) {
			ASSERT_OK(Put(
				cf, Slice(Key(key, key_size)),
				   ...
				   CompactionJobStats *stats, int cf = 0)
	{
		...
		for (auto key = smallest; key <= largest;
		     key += interval, ctr++) {
	...
	bool is_manual = true)
{
	...
	stats.total_input_bytes =
		...
		num_input_files;
	stats.total_output_bytes =
		EstimatedFileSize(num_output_records / num_output_files,
				  key_size, value_size, compression_ratio) *
		num_output_files;
...
TEST_P(CompactionJobStatsTest, CompactionJobStatsTest)
{
	...
	for (int test = 0; test < 2; ++test) {
		...
		for (uint64_t start_key = key_base;
		     ...
		     start_key += key_base) {
			MakeTableWithKeyValues(&rnd, start_key,
					       start_key + key_base - 1,
					       kKeySize, kValueSize,
					       key_interval, compression_ratio,
					       1);
		...
		for (uint64_t start_key = key_base;
			...
			largest_key =
				Key(start_key + key_base - key_interval, 10);
			stats_checker->AddExpectedStats(
				NewManualCompactionJobStats(
					smallest_key, largest_key, 1, 0,
					num_keys_per_L0_file, kKeySize,
					kValueSize, 1, num_keys_per_L0_file,
					compression_ratio, 0));
			ASSERT_EQ(stats_checker->NumberOfUnverifiedStats(), 1U);
			TEST_Compact(0, 1, smallest_key, largest_key);
			snprintf(buf, kBufSize, "%d,%d", num_L0_files - count,
				 count);
			ASSERT_EQ(std::string(buf), FilesPerLevel(1));
		}
		...
		largest_key =
			Key(key_base * (kTestScale + 1) - key_interval, 10);
		stats_checker->AddExpectedStats(NewManualCompactionJobStats(
			smallest_key, largest_key, num_remaining_L0, 0,
			num_keys_per_L0_file * num_remaining_L0, kKeySize,
			kValueSize, 1, num_keys_per_L0_file * num_remaining_L0,
			compression_ratio, 0));
		ASSERT_EQ(stats_checker->NumberOfUnverifiedStats(), 1U);
		TEST_Compact(0, 1, smallest_key, largest_key);
		...
		for (uint64_t start_key = key_base;
			...
			MakeTableWithKeyValues(
				&rnd, start_key,
				start_key + key_base * sparseness - 1, kKeySize,
				kValueSize,
				key_base * sparseness / num_keys_per_L0_file,
				compression_ratio, 1);
			snprintf(buf, kBufSize, "%d,%d", ++num_L0_files,
				 num_L1_files);
			ASSERT_EQ(std::string(buf), FilesPerLevel(1));
		}
		...
		for (uint64_t start_key = key_base; num_L0_files > 1;
		     start_key += key_base * sparseness) {
			smallest_key = Key(start_key, 10);
			largest_key = Key(start_key + key_base * sparseness -
						  key_interval,
					  10);
			stats_checker->AddExpectedStats(NewManualCompactionJobStats(
				smallest_key, largest_key, 3, 2,
				num_keys_per_L0_file * 3, kKeySize, kValueSize,
				num_output_files,
		...
		largest_key =
			Key(key_base * (kTestScale + 1) - key_interval, 10);
		stats_checker->AddExpectedStats(NewManualCompactionJobStats(
			Key(key_base * (kTestScale + 1 - sparseness), 10),
			largest_key, 2, 1, num_keys_per_L0_file * 3, kKeySize,
			kValueSize, 1, num_keys_per_L0_file * 2,
			compression_ratio, num_keys_per_L0_file));
		ASSERT_EQ(stats_checker->NumberOfUnverifiedStats(), 1U);
		Compact(1, smallest_key, largest_key);
		...
		rocksdb::SyncPoint::GetInstance()->SetCallBack(
			"WritableFileWriter::Append:BeforePrepareWrite",
			[&](void *arg) {
		...
		rocksdb::SyncPoint::GetInstance()->SetCallBack(
			"WritableFileWriter::Flush:BeforeAppend",
			[&](void *arg) {
				if (first_flush.load()) {
					options.env->SleepForMicroseconds(3);
...
TEST_P(CompactionJobStatsTest, DeletionStatsTest)
{
	...
	uint64_t cutoff_key_num =
		key_base * (kTestScale / 2 + 1) - key_interval;
	...
	for (uint64_t start_key = key_base;
		...
		MakeTableWithKeyValues(&rnd, start_key,
				       start_key + key_base - 1, kKeySize,
				       kValueSize, key_interval,
				       compression_ratio, 1);
	}
...
int GetUniversalCompactionInputUnits(uint32_t num_flushes)
{
	...
	for (compaction_input_units = 1; num_flushes >= compaction_input_units;
		...
		if ((num_flushes & compaction_input_units) != 0) {
			return compaction_input_units > 1 ?
...
TEST_P(CompactionJobStatsTest, UniversalCompactionTest)
{
	...
	for (uint32_t num_flushes = 2; num_flushes <= kTestScale;
	     num_flushes++) {
		...
		stats_checker->AddExpectedStats(NewManualCompactionJobStats(
			Key(smallest_key, 10),
			Key(smallest_key + key_base * num_input_units -
				    key_interval,
			    10),
			num_input_units,
			num_input_units > 2 ? num_input_units / 2 : 0,
			num_keys_per_table * num_input_units, kKeySize,
			kValueSize, num_input_units,
			num_keys_per_table * num_input_units, 1.0, 0, false));
