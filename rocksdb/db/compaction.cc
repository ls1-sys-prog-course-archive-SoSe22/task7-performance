...
void Compaction::GetBoundaryKeys(VersionStorageInfo *vstorage,
				 ...
				 Slice *largest_user_key)
{
	...
	for (size_t i = 0; i < inputs.size(); ++i) {
		...
		if (inputs[i].level == 0) {
			...
			const Slice &start_user_key =
				inputs[i].files[0]->smallest.user_key();
			if (!initialized ||
			    ucmp->Compare(start_user_key, *smallest_user_key) <
				    0) {
				*smallest_user_key = start_user_key;
			}
			const Slice &end_user_key =
				inputs[i].files.back()->largest.user_key();
			if (!initialized ||
			    ucmp->Compare(end_user_key, *largest_user_key) >
				    0) {
				*largest_user_key = end_user_key;
			}
			initialized = true;
	...
	const std::vector<CompactionInputFiles> &inputs)
{
	...
	for (int i = output_level + 1; i < vstorage->num_levels(); i++) {
		...
		if (vstorage->NumLevelFiles(i) > 0 &&
		    (output_level == 0 ||
		     vstorage->OverlapInLevel(i, &smallest_key,
					      &largest_key))) {
			return false;
		}
	}
...
Compaction::Compaction(VersionStorageInfo *vstorage,
	...
	: input_vstorage_(vstorage), start_level_(_inputs[0].level),
	  output_level_(_output_level),
	  max_output_file_size_(_target_file_size),
	  max_compaction_bytes_(_max_compaction_bytes),
	  immutable_cf_options_(_immutable_cf_options),
	  mutable_cf_options_(_mutable_cf_options), input_version_(nullptr),
	  number_levels_(vstorage->num_levels()), cfd_(nullptr),
	  output_path_id_(_output_path_id), output_compression_(_compression),
	  deletion_compaction_(_deletion_compaction),
	  inputs_(std::move(_inputs)), grandparents_(std::move(_grandparents)),
	  score_(_score),
	  bottommost_level_(
	  ...
	  compaction_reason_(_compaction_reason)
{
	...
	// setup input_levels_
	{
		...
		for (size_t which = 0; which < num_input_levels(); which++) {
			DoGenerateLevelFilesBrief(&input_levels_[which],
						  inputs_[which].files,
						  &arena_);
		}
	}

	GetBoundaryKeys(vstorage, inputs_, &smallest_user_key_,
			&largest_user_key_);
}
...
bool Compaction::InputCompressionMatchesOutput() const
{
	...
	bool matches =
		(GetCompressionType(immutable_cf_options_, input_vstorage_,
...
bool Compaction::IsTrivialMove() const
{
	...
	if ((immutable_cf_options_.compaction_options_universal
		     .allow_trivial_move) &&
	    (output_level_ != 0)) {
	...
	for (const auto &file : inputs_.front().files) {
		...
		input_vstorage_->GetOverlappingInputs(output_level_ + 1,
						      &file->smallest,
						      &file->largest,
						      &file_grand_parents);
		const auto compaction_size = file->fd.GetFileSize() +
					     TotalFileSize(file_grand_parents);
...
const char *
Compaction::InputLevelSummary(InputLevelSummaryBuffer *scratch) const
{
	int len = 0;
	bool is_first = true;
	for (auto &input_level : inputs_) {
		...
		if (!is_first) {
			len += snprintf(scratch->buffer + len,
					sizeof(scratch->buffer) - len, " + ");
		} else {
			is_first = false;
		}
		len += snprintf(scratch->buffer + len,
...
void Compaction::Summary(char *output, int len)
{
	int write =
		snprintf(output, len,
			 "Base version %" PRIu64 " Base level %d, inputs: [",
			 input_version_->GetVersionNumber(), start_level_);
	if (write < 0 || write >= len) {
		return;
	}

	for (size_t level_iter = 0; level_iter < num_input_levels();
	     ++level_iter) {
		if (level_iter > 0) {
		...
		write += InputSummary(inputs_[level_iter].files, output + write,
				      len - write);
		if (write < 0 || write >= len) {
			return;
		}
...
std::unique_ptr<CompactionFilter> Compaction::CreateCompactionFilter() const
{
	...
	return cfd_->ioptions()
		->compaction_filter_factory->CreateCompactionFilter(context);
}
...
bool Compaction::ShouldFormSubcompactions() const
{
	...
	if (cfd_->ioptions()->compaction_style == kCompactionStyleLevel) {
		return start_level_ == 0 && output_level_ > 0 &&
		       !IsOutputLevelEmpty();
