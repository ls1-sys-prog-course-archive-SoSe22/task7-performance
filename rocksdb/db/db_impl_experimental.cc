				   ...
				   const Slice *begin, const Slice *end)
{
	...
	}
	{
		...
		for (int level = 0;
		     level < vstorage->num_non_empty_levels() - 1; ++level) {
...
Status DBImpl::PromoteL0(ColumnFamilyHandle *column_family, int target_level)
{
	...
	{
		InstrumentedMutexLock l(&mutex_);
		auto *cfd = static_cast<ColumnFamilyHandleImpl *>(column_family)
				    ->cfd();
		...
		std::sort(l0_files.begin(), l0_files.end(),
			  [icmp](FileMetaData *f1, FileMetaData *f2) {
				  return icmp->Compare(f1->largest,
						       f2->largest) < 0;
			  });
		...
		for (const auto &f : l0_files) {
			...
			edit.AddFile(target_level, f->fd.GetNumber(),
