...
TEST_F(DBBasicTest, GetSnapshot)
{
	...
	do {
		...
		for (int i = 0; i < 2; i++) {
			std::string key = (i == 0) ? std::string("foo") :
							   std::string(200, 'x');
...
TEST_F(DBBasicTest, FLUSH)
{
	do {
		...
		Get(1, "foo");
		ASSERT_TRUE(
			(int)get_perf_context()->get_from_output_files_time >
			0);
...
TEST_F(DBBasicTest, ManifestRollOver)
{
	do {
		...
		CreateAndReopenWithCF({ "pikachu" }, options);
		{
			ASSERT_OK(Put(1, "manifest_key1",
				      std::string(1000, '1')));
			ASSERT_OK(Put(1, "manifest_key2",
				      std::string(1000, '2')));
			ASSERT_OK(Put(1, "manifest_key3",
				      std::string(1000, '3')));
			...
			ASSERT_EQ(std::string(1000, '1'),
				  Get(1, "manifest_key1"));
			ASSERT_EQ(std::string(1000, '2'),
				  Get(1, "manifest_key2"));
			ASSERT_EQ(std::string(1000, '3'),
				  Get(1, "manifest_key3"));
		}
	} while (ChangeCompactOptions());
}
...
{
	Options options = CurrentOptions();
	CreateAndReopenWithCF({ "pikachu", "ilya", "muromec", "dobrynia",
...
TEST_F(DBBasicTest, MultiGetSimple)
{
	do {
		...
		std::vector<Slice> keys(
			{ "k1", "k2", "k3", "k4", "k5", "no_key" });

		std::vector<std::string> values(
			20, "Temporary data to be overwritten");
