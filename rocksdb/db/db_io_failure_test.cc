...
TEST_F(DBIOFailureTest, DropWrites)
{
	do {
		...
		ASSERT_TRUE(db_->GetProperty("rocksdb.background-errors",
					     &property_value));
...
TEST_F(DBIOFailureTest, NoSpaceCompactRange)
{
	do {
		...
		Status s = dbfull()->TEST_CompactRange(
...
TEST_F(DBIOFailureTest, ManifestWriteError)
{
	...
	for (int iter = 0; iter < 2; iter++) {
		std::atomic<bool> *error_type =
			(iter == 0) ? &env_->manifest_sync_error_ :
		...
		dbfull()->TEST_CompactRange(last, nullptr,
					    nullptr); // Should fail
		ASSERT_EQ("bar", Get("foo"));
