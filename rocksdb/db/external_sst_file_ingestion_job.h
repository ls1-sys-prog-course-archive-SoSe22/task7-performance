...
struct IngestedFileInfo {
	...
	{
		return InternalKey(largest_user_key, assigned_seqno,
				   ValueType::kTypeValue);
	}
};
...
class ExternalSstFileIngestionJob {
	...
	ExternalSstFileIngestionJob(
		...
		: env_(env), versions_(versions), cfd_(cfd),
	...
	// REQUIRES: Mutex held
	Status IngestedFilesOverlapWithMemtables(SuperVersion *sv,
						 bool *overlap);
	...
	// REQUIRES: Mutex held
	Status
	AssignLevelAndSeqnoForIngestedFile(SuperVersion *sv,
	...
	// REQUIRES: Mutex held
	Status
	CheckLevelForIngestedBehindFile(IngestedFileInfo *file_to_ingest);

	// Set the file global sequence number to `seqno`
	Status
	AssignGlobalSeqnoForIngestedFile(IngestedFileInfo *file_to_ingest,
	...
	Status IngestedFileOverlapWithLevel(SuperVersion *sv,
					    IngestedFileInfo *file_to_ingest,
					    int lvl, bool *overlap_with_level);
