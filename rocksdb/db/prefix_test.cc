...
class TestKeyComparator : public Comparator {
	...
	virtual int Compare(const Slice &a, const Slice &b) const override
	{
		...
		if (key_a->prefix != key_b->prefix) {
			...
			if (a.size() != b.size()) {
				...
				EXPECT_TRUE((a.size() == sizeof(uint64_t) &&
					     b.size() == sizeof(TestKey)) ||
					    (b.size() == sizeof(uint64_t) &&
					     a.size() == sizeof(TestKey)));
				if (a.size() < b.size())
					return -1;
				if (a.size() > b.size())
					return 1;
				...
				EXPECT_TRUE(a.size() == sizeof(TestKey) &&
					    b.size() == sizeof(TestKey));
	...
	{
		std::string sa, sb;
		return Compare(TestKeyToSlice(sa, a), TestKeyToSlice(sb, b)) <
		       0;
	}
...
class SamePrefixTransform : public SliceTransform {
	...
	explicit SamePrefixTransform(const Slice &prefix)
		: prefix_(prefix),
		  name_("rocksdb.SamePrefix." + prefix.ToString())
	{
	}
...
TEST(SamePrefixTest, InDomainTest)
{
	...
	ReadOptions read_options;
	{
		...
		ASSERT_OK(db->Put(write_options, "HHKB pro2 Type-S",
				  "June 29, 2011"));
...
TEST_F(PrefixTest, TestResult)
{
	for (int num_buckets = 1; num_buckets <= 2; num_buckets++) {
		...
		while (NextOptions(num_buckets)) {
			...
			ASSERT_EQ(v16.ToString(),
				  Get(db.get(), read_options, 1, 6));
			ASSERT_EQ(kNotFoundResult,
				  Get(db.get(), read_options, 1, 5));
			ASSERT_EQ(kNotFoundResult,
				  Get(db.get(), read_options, 1, 7));
			ASSERT_EQ(kNotFoundResult,
				  Get(db.get(), read_options, 0, 6));
			ASSERT_EQ(kNotFoundResult,
				  Get(db.get(), read_options, 2, 6));
			...
			ASSERT_EQ(v15.ToString(),
				  Get(db.get(), read_options, 1, 5));
			ASSERT_EQ(v16.ToString(),
				  Get(db.get(), read_options, 1, 6));
			ASSERT_EQ(v17.ToString(),
				  Get(db.get(), read_options, 1, 7));
			...
			ASSERT_EQ(v22.ToString(),
				  Get(db.get(), read_options, 2, 2));
			ASSERT_EQ(v02.ToString(),
				  Get(db.get(), read_options, 0, 2));
			ASSERT_EQ(v13.ToString(),
				  Get(db.get(), read_options, 1, 3));
			ASSERT_EQ(v15.ToString(),
				  Get(db.get(), read_options, 1, 5));
			ASSERT_EQ(v16.ToString(),
				  Get(db.get(), read_options, 1, 6));
			ASSERT_EQ(v17.ToString(),
				  Get(db.get(), read_options, 1, 7));
			ASSERT_EQ(v18.ToString(),
				  Get(db.get(), read_options, 1, 8));
		}
	}
}
...
TEST_F(PrefixTest, PrefixValid)
{
	for (int num_buckets = 1; num_buckets <= 2; num_buckets++) {
		...
		while (NextOptions(num_buckets)) {
			...
			ASSERT_EQ(kNotFoundResult,
				  Get(db.get(), read_options, 12346, 8));
...
TEST_F(PrefixTest, DynamicPrefixIterator)
{
	while (NextOptions(FLAGS_bucket_count)) {
		std::cout
			<< "*** Mem table: " << options.memtable_factory->Name()
			<< std::endl;
		DestroyDB(kDbName, Options());
		auto db = OpenDb();
		WriteOptions write_options;
		...
		for (auto prefix : prefixes) {
			for (uint64_t sorted = 0;
			     sorted < FLAGS_items_per_prefix; sorted++) {
		...
		for (auto prefix : prefixes) {
			...
			StopWatchNano timer(Env::Default(), true);
			auto key_prefix =
				options.prefix_extractor->Transform(key);
			uint64_t total_keys = 0;
			for (iter->Seek(key);
			     iter->Valid() &&
			     iter->key().starts_with(key_prefix);
			     iter->Next()) {
...
TEST_F(PrefixTest, PrefixSeekModePrev)
{
	...
	for (size_t m = 1; m < 100; m++) {
		...
		WriteOptions write_options;
		ReadOptions read_options;
		std::map<TestKey, std::string, TestKeyComparator> entry_maps[3],
			whole_map;
		for (uint64_t i = 0; i < 10; i++) {
			int div = i % 3 + 1;
			for (uint64_t j = 0; j < 10; j++) {
				whole_map[TestKey(i, j)] =
					entry_maps[rnd.Uniform(div)]
						  [TestKey(i, j)] =
							  'v' +
							  std::to_string(i) +
							  std::to_string(j);
		...
		for (size_t i = 0; i < 3; i++) {
			for (auto &kv : entry_maps[i]) {
				if (rnd.OneIn(3)) {
					PutKey(db.get(), write_options,
					       kv.first, kv.second);
					type_map[kv.first] = "value";
				} else {
					MergeKey(db.get(), write_options,
						 kv.first, kv.second);
		...
		for (size_t i = 0; i < 2; i++) {
			for (auto &kv : entry_maps[i]) {
				if (rnd.OneIn(10)) {
					...
					DeleteKey(db.get(), write_options,
						  kv.first);
		...
		for (uint64_t prefix = 0; prefix < 10; prefix++) {
			...
			ASSERT_TRUE(iter->Valid());
			if (FLAGS_enable_print) {
				std::cout << "round " << prefix << " iter: "
					  << SliceToTestKey(iter->key()).prefix
...
TEST_F(PrefixTest, PrefixSeekModePrev3)
{
...

	{
		...
		PutKey(db.get(), write_options, TestKey(1, 4), "v14");
		db->Flush(FlushOptions());
		reinterpret_cast<DBImpl *>(db.get())
