...
fromInternalValueType(ValueType vt)
{
	switch (vt) {
	case kTypeDeletion:
...
CompactionIterator::CompactionIterator(
	...
	: CompactionIterator(
		  ...
		  std::unique_ptr<CompactionProxy>(
			  compaction ? new CompactionProxy(compaction) :
					     nullptr),
...
CompactionIterator::CompactionIterator(
	...
	: input_(input), cmp_(cmp), merge_helper_(merge_helper),
	  ...
	  range_del_agg_(range_del_agg), compaction_(std::move(compaction)),
	  compaction_filter_(compaction_filter),
#ifndef ROCKSDB_LITE
	  compaction_listener_(compaction_listener),
#endif // ROCKSDB_LITE
	  shutting_down_(shutting_down), ignore_snapshots_(false),
	  merge_out_iter_(merge_helper_)
{
	...
	if (compaction_ != nullptr) {
		level_ptrs_ =
			std::vector<size_t>(compaction_->number_levels(), 0);
	}
...
void CompactionIterator::Next()
{
	...
	if (merge_out_iter_.Valid()) {
		...
		if (merge_out_iter_.Valid()) {
			...
			current_key_.UpdateInternalKey(ikey_.sequence,
						       ikey_.type);
	...
	while (!valid_ && input_->Valid() && !IsShuttingDown()) {
		...
		if (!ParseInternalKey(key_, &ikey_)) {
			...
			if (expect_valid_internal_key_) {
				...
				status_ = Status::Corruption(
					"Corrupted internal key not expected.");
				break;
			}
		...
		if (!has_current_user_key_ ||
			...
			if (compaction_listener_) {
				compaction_listener_->OnCompaction(
			...
			if (compaction_filter_ != nullptr &&
				...
				if (filter == CompactionFilter::Decision::
						      kRemoveAndSkipUntil &&
				    cmp_->Compare(
				...
				if (filter ==
					...
					current_key_.UpdateInternalKey(
						ikey_.sequence, kTypeDeletion);
		...
		current_user_key_snapshot_ =
			visible_at_tip_ ?
				      earliest_snapshot_ :
				      findEarliestVisibleSnapshot(ikey_.sequence,
							    &prev_snapshot);
		...
		if (need_skip) {
			...
			if (input_->Valid() &&
				...
				// SingleDelete.
				if (prev_snapshot == 0 ||
				    next_ikey.sequence > prev_snapshot) {
					if (next_ikey.type ==
					    kTypeSingleDeletion) {
			...
			bool should_delete = range_del_agg_->ShouldDelete(
				key_, RangeDelAggregator::RangePositioningMode::
					      kForwardTraversal);
			if (should_delete) {
