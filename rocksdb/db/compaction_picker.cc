			   ...
			   CompactionInputFiles *comp_inputs)
{
	...
	for (span_len = 1; span_len < level_files.size(); ++span_len) {
		...
		if (level_files[span_len]->being_compacted ||
		    new_compact_bytes_per_del_file >
				   ...
				   const bool enable_compression)
{
	...
	if (!ioptions.compression_per_level.empty()) {
		...
		const int n = static_cast<int>(
				      ioptions.compression_per_level.size()) -
			      1;
		...
		for (size_t i = 0; i < inputs.size(); i++) {
			...
			if (i == 0) {
				...
				*largest = f->largest;
				...
				if (icmp_->Compare(f->smallest, *smallest) <
				    0) {
					*smallest = f->smallest;
				}
				...
				InternalKey *largest) const
{
	...
	if (inputs1.empty()) {
		...
		*smallest = icmp_->Compare(smallest1, smallest2) < 0 ?
					  smallest1 :
					  smallest2;
		*largest = icmp_->Compare(largest1, largest2) < 0 ? largest2 :
									  largest1;
	}
}
...
bool CompactionPicker::RangeOverlapWithCompaction(const Slice &smallest_user_key,
						  ...
						  int level) const
{
	...
	for (Compaction *c : compactions_in_progress_) {
		if (c->output_level() == level &&
		    ucmp->Compare(smallest_user_key, c->GetLargestUserKey()) <=
			    0 &&
		    ucmp->Compare(largest_user_key, c->GetSmallestUserKey()) >=
			    0) {
			// Overlap
			return true;
		}
	...
	const std::vector<CompactionInputFiles> &inputs, int level) const
{
	...
	InternalKey smallest, largest;
	GetRange(inputs, &smallest, &largest);
	return RangeOverlapWithCompaction(smallest.user_key(),
					  largest.user_key(), level);
}
...
Compaction *CompactionPicker::CompactFiles(
	...
	VersionStorageInfo *vstorage,
	const MutableCFOptions &mutable_cf_options, uint32_t output_path_id)
{
	...
	if ((input_files[0].level == 0) &&
	    !level0_compactions_in_progress_.empty()) {
		return nullptr;
	}
	...
	auto c = new Compaction(vstorage, ioptions_, mutable_cf_options,
	...
	const CompactionOptions &compact_options) const
{
	...
	for (int level = 0; level < vstorage->num_levels(); ++level) {
		for (auto file : vstorage->LevelFiles(level)) {
			...
			if (iter != input_set->end()) {
				matched_input_files[level].files.push_back(
					file);
	...
	for (int level = first_non_empty_level; level <= last_non_empty_level;
		...
		input_files->emplace_back(
			std::move(matched_input_files[level]));
	}
	...
	int base_index)
{
	...
	if (!output_level_inputs->empty()) {
		...
		const uint64_t inputs_size =
			TotalCompensatedFileSize(inputs->files);
		...
		vstorage->GetOverlappingInputs(input_level, &all_start,
					       &all_limit,
					       &expanded_inputs.files,
					       base_index, nullptr);
		...
		if (expand_inputs) {
			ROCKS_LOG_INFO(ioptions_.info_log,
				       "[%s] Expanding@%d %" ROCKSDB_PRIszt
				       "+%" ROCKSDB_PRIszt "(%" PRIu64
				       "+%" PRIu64 " bytes) to %" ROCKSDB_PRIszt
				       "+%" ROCKSDB_PRIszt " (%" PRIu64
				       "+%" PRIu64 "bytes)\n",
	...
	std::vector<FileMetaData *> *grandparents)
{
	...
	if (output_level_inputs.level + 1 < NumberLevels()) {
		vstorage->GetOverlappingInputs(output_level_inputs.level + 1,
					       &start, &limit, grandparents);
	}
}
	...
	bool *manual_conflict)
{
	...
	if (input_level == ColumnFamilyData::kCompactAllLevels) {
		...
		*compaction_end = nullptr;
		...
		std::vector<CompactionInputFiles> inputs(
			vstorage->num_levels() - start_level);
	...
	assert(output_path_id <
	       static_cast<uint32_t>(ioptions_.db_paths.size()));
	...
	TEST_SYNC_POINT_CALLBACK("CompactionPicker::CompactRange:Return",
				 compaction);
	...
	const ColumnFamilyMetaData &cf_meta, const int output_level) const
{
	...
	for (int l = 0; l <= output_level; ++l) {
		...
		for (size_t f = 0; f < current_files.size(); ++f) {
			if (input_files->find(TableFileNameToNumber(
				    current_files[f].name)) !=
			    input_files->end()) {
				first_included = std::min(first_included,
							  static_cast<int>(f));
				last_included = std::max(last_included,
							 static_cast<int>(f));
				if (is_first == false) {
					smallestkey =
						current_files[f].smallestkey;
					largestkey =
						current_files[f].largestkey;
					is_first = true;
				}
			}
		}
		...
		if (l != 0) {
			...
			while (first_included > 0) {
				if (comparator->Compare(
					    current_files[first_included - 1]
						    .largestkey,
					    current_files[first_included]
						    .smallestkey) < 0) {
					break;
				}
				first_included--;
			...
			while (last_included <
			       static_cast<int>(current_files.size()) - 1) {
				if (comparator->Compare(
					    current_files[last_included + 1]
						    .smallestkey,
					    current_files[last_included]
						    .largestkey) > 0) {
					break;
				}
				last_included++;
		...
		for (int f = first_included; f <= last_included; ++f) {
			if (current_files[f].being_compacted) {
				return Status::Aborted(
					"Necessary compaction input file " +
					current_files[f].name +
		...
		if (l == 0) {
			for (int f = first_included; f <= last_included; ++f) {
				if (comparator->Compare(
					    smallestkey,
					    current_files[f].smallestkey) > 0) {
					smallestkey =
						current_files[f].smallestkey;
				}
				if (comparator->Compare(
					    largestkey,
					    current_files[f].largestkey) < 0) {
					largestkey =
						current_files[f].largestkey;
				}
			}
			...
			if (comparator->Compare(
			    ...
			    0) {
				smallestkey = current_files[first_included]
						      .smallestkey;
			}
			if (comparator->Compare(
				    largestkey,
		...
		for (int m = std::max(l, 1); m <= output_level; ++m) {
			for (auto &next_lv_file : levels[m].files) {
				if (HaveOverlappingKeyRanges(
					    comparator, aggregated_file_meta,
					    next_lv_file)) {
	...
	const ColumnFamilyMetaData &cf_meta, const int output_level) const
{
	...
	if (output_level >= static_cast<int>(cf_meta.levels.size())) {
		return Status::InvalidArgument(
			...
			" must between [0, " +
			ToString(cf_meta.levels[cf_meta.levels.size() - 1]
					 .level) +
			"].");
	}
	...
	for (auto file_num : *input_files) {
		...
		for (auto level_meta : cf_meta.levels) {
			for (auto file_meta : level_meta.files) {
				if (file_num ==
				    TableFileNameToNumber(file_meta.name)) {
...
void CompactionPicker::RegisterCompaction(Compaction *c)
{
	...
	assert(ioptions_.compaction_style != kCompactionStyleLevel ||
	       c->output_level() == 0 ||
	       !FilesRangeOverlapWithCompaction(*c->inputs(),
						c->output_level()));
	if (c->start_level() == 0 ||
...
class LevelCompactionBuilder {
	...
	LevelCompactionBuilder(const std::string &cf_name,
		...
		: cf_name_(cf_name), vstorage_(vstorage),
		  compaction_picker_(compaction_picker),
		  log_buffer_(log_buffer),
		  mutable_cf_options_(mutable_cf_options), ioptions_(ioptions)
	{
	}
...
void LevelCompactionBuilder::PickFilesMarkedForCompaction()
{
	...
	auto continuation = [&](std::pair<int, FileMetaData *> level_file) {
		...
		output_level_ = (start_level_ == 0) ? vstorage_->base_level() :
							    start_level_ + 1;
	...
	size_t random_file_index =
		static_cast<size_t>(rnd.Uniform(static_cast<uint64_t>(
			vstorage_->FilesMarkedForCompaction().size())));

	if (continuation(
		    vstorage_->FilesMarkedForCompaction()[random_file_index])) {
...
void LevelCompactionBuilder::SetupInitialFiles()
{
	...
	for (int i = 0; i < compaction_picker_->NumberLevels() - 1; i++) {
		...
		if (start_level_score_ >= 1) {
			if (skipped_l0_to_base &&
			    start_level_ == vstorage_->base_level()) {
			...
			if (PickFileToCompact() &&
				...
				if (start_level_ == 0) {
					...
					compaction_reason_ = CompactionReason::
						kLevelL0FilesNum;
				} else {
...
bool LevelCompactionBuilder::SetupOtherL0FilesIfNeeded()
{
	if (start_level_ == 0 && output_level_ != 0) {
		...
		assert(compaction_picker_->level0_compactions_in_progress()
			       ->empty());
		InternalKey smallest, largest;
		compaction_picker_->GetRange(start_level_inputs_, &smallest,
					     &largest);
		...
		if (compaction_picker_->IsRangeInCompaction(
			    vstorage_, &smallest, &largest, output_level_,
			    &parent_index_)) {
			return false;
		}
	}
...
Compaction *LevelCompactionBuilder::PickCompaction()
{
	...
	TEST_SYNC_POINT_CALLBACK("LevelCompactionPicker::PickCompaction:Return",
				 c);
...
Compaction *LevelCompactionBuilder::GetCompaction()
{
	auto c = new Compaction(
		vstorage_, ioptions_, mutable_cf_options_,
		std::move(compaction_inputs_), output_level_,
		mutable_cf_options_.MaxFileSizeForLevel(output_level_),
 ...
 */
uint32_t
LevelCompactionBuilder::GetPathId(const ImmutableCFOptions &ioptions,
				  const MutableCFOptions &mutable_cf_options,
				  int level)
{
	uint32_t p = 0;
...
bool LevelCompactionBuilder::PickFileToCompact()
{
	...
	for (unsigned int i = vstorage_->NextCompactionIndex(start_level_);
		...
		parent_index_ = -1;
		if (compaction_picker_->IsRangeInCompaction(
			    vstorage_, &f->smallest, &f->largest, output_level_,
			    &parent_index_)) {
			continue;
		}
	...
	VersionStorageInfo *vstorage, LogBuffer *log_buffer)
{
	...
	for (auto ritr = level_files.rbegin(); ritr != level_files.rend();
		...
		char tmp_fsize[16];
		AppendHumanBytes(f->fd.GetFileSize(), tmp_fsize,
				 sizeof(tmp_fsize));
	...
	Compaction *c =
		new Compaction(vstorage, ioptions_, mutable_cf_options,
