...
Writer::Writer(unique_ptr<WritableFileWriter> &&dest, uint64_t log_number,
	...
	: dest_(std::move(dest)), block_offset_(0), log_number_(log_number),
...
Status Writer::AddRecord(const Slice &slice)
{
	...
	do {
		...
		if (begin && end) {
			type = recycle_log_files_ ? kRecyclableFullType :
							  kFullType;
		} else if (begin) {
			type = recycle_log_files_ ? kRecyclableFirstType :
							  kFirstType;
		} else if (end) {
			type = recycle_log_files_ ? kRecyclableLastType :
							  kLastType;
		} else {
			type = recycle_log_files_ ? kRecyclableMiddleType :
							  kMiddleType;
		}
