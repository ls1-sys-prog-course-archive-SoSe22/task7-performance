...
class ExternalSSTFileTest : public DBTestBase {
		...
		ColumnFamilyHandle *cfh = nullptr)
	{
		...
		if (sort_data) {
			std::sort(data.begin(), data.end(),
				  [&](const std::pair<std::string, std::string>
				      ...
				      const std::pair<std::string, std::string>
					      &e2) {
					  return options.comparator->Compare(
							 e1.first, e2.first) <
						 0;
				  });
		...
		if (s.ok()) {
			...
			if (cfh) {
				s = db_->IngestExternalFile(cfh, { file_path },
							    ifo);
			} else {
	...
	Status GenerateAndAddExternalFileIngestBehind(
		...
		std::vector<std::pair<std::string, std::string> > data,
		int file_id = -1, bool sort_data = false,
		std::map<std::string, std::string> *true_data = nullptr,
		ColumnFamilyHandle *cfh = nullptr)
	{
		// Generate a file id if not provided
		...
		ColumnFamilyHandle *cfh = nullptr)
	{
		std::vector<std::pair<std::string, std::string> > file_data;
		for (auto &k : keys) {
			file_data.emplace_back(Key(k),
					       Key(k) + ToString(file_id));
		}
		return GenerateAndAddExternalFile(options, file_data, file_id,
						  allow_global_seqno, sort_data,
						  true_data, cfh);
...
TEST_F(ExternalSSTFileTest, Basic)
{
	do {
		...
		for (int k = 195; k < 300; k++) {
			ASSERT_OK(sst_file_writer.Put(Key(k),
						      Key(k) + "_val_overlap"));
		}
		...
		ASSERT_OK(db_->CompactRange(CompactRangeOptions(), nullptr,
					    nullptr));
...
class SstFileWriterCollector : public TablePropertiesCollector {
    public:
	explicit SstFileWriterCollector(const std::string prefix)
		: prefix_(prefix)
	{
...
class SstFileWriterCollectorFactory : public TablePropertiesCollectorFactory {
	...
	const char *Name() const override
	{
		return "SstFileWriterCollectorFactory";
	}
...
TEST_F(ExternalSSTFileTest, AddList)
{
	do {
		...
		auto abc_collector =
			std::make_shared<SstFileWriterCollectorFactory>("abc");
		auto xyz_collector =
			std::make_shared<SstFileWriterCollectorFactory>("xyz");
		...
		for (auto file_props : props) {
			auto user_props =
				file_props.second->user_collected_properties;
			ASSERT_EQ(user_props["abc_SstFileWriterCollector"],
				  "YES");
			ASSERT_EQ(user_props["xyz_SstFileWriterCollector"],
				  "YES");
...
TEST_F(ExternalSSTFileTest, AddListAtomicity)
{
	do {
		...
		for (int i = 0; i < n; i++) {
			files[i] = sst_files_dir_ + "file" + std::to_string(i) +
				   ".sst";
			...
			ASSERT_EQ(files_info[i].largest_key,
				  Key((i + 1) * 100 - 1));
		}
		files.push_back(sst_files_dir_ + "file" + std::to_string(n) +
				".sst");
...
TEST_F(ExternalSSTFileTest, PurgeObsoleteFilesBug)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"DBImpl::AddFile:FileCopied", [&](void *arg) {
			...
			ASSERT_OK(Flush());
			db_->CompactRange(CompactRangeOptions(), nullptr,
					  nullptr);
		});
...
TEST_F(ExternalSSTFileTest, MultiThreaded)
{
	...
	do {
		...
		std::function<void()> load_file_func = [&]() {
			...
			if (static_cast<size_t>(file_idx + 1) <
			    file_names.size()) {
			...
			if (s.ok()) {
				files_added +=
					static_cast<int>(files_to_add.size());
			}
		};
...
TEST_F(ExternalSSTFileTest, OverlappingRanges)
{
	...
	do {
		...
		for (int i = 0; i < 500; i++) {
			...
			key_ranges.emplace_back(range_start,
						range_start + keys_per_range);
		}
		...
		for (size_t i = 0; i < key_ranges.size(); i++) {
			...
			if (i && i % 75 == 0) {
				db_->CompactRange(CompactRangeOptions(),
						  nullptr, nullptr);
			}
		}

		printf("Total: %" ROCKSDB_PRIszt " ranges\n"
		       "AddFile()|Success: %d ranges\n"
		       "AddFile()|RangeConflict: %d ranges\n"
		       "Put(): %d ranges\n",
		       key_ranges.size(), success_add_file, failed_add_file,
		       memtable_add);
...
TEST_F(ExternalSSTFileTest, PickedLevel)
{
	...
	ASSERT_OK(GenerateAndAddExternalFile(options, { 1, 10 }, -1, false,
					     false, &true_data));
	EXPECT_EQ(FilesPerLevel(), "0,0,0,1");

	// File 1 will go to level L2 (since it overlap with file 0 in L3)
	ASSERT_OK(GenerateAndAddExternalFile(options, { 2, 9 }, -1, false,
					     false, &true_data));
	EXPECT_EQ(FilesPerLevel(), "0,0,1,1");

	rocksdb::SyncPoint::GetInstance()->LoadDependency({
		{ "ExternalSSTFileTest::PickedLevel:0",
	...
	ASSERT_OK(GenerateAndAddExternalFile(options, { 4, 7 }, -1, false,
					     false, &true_data));
	EXPECT_EQ(FilesPerLevel(), "5,0,1,1");

	// This file does not overlap with any file or with the running compaction
	ASSERT_OK(GenerateAndAddExternalFile(options, { 9000, 9001 }, -1, false,
					     false, &true_data));
...
TEST_F(ExternalSSTFileTest, PickedLevelBug)
{
	...
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "DBImpl::AddFile:MutexLock",
		    "ExternalSSTFileTest::PickedLevelBug:0" },
		  { "ExternalSSTFileTest::PickedLevelBug:1",
		    "DBImpl::AddFile:MutexUnlock" },
...
TEST_F(ExternalSSTFileTest, CompactDuringAddFileRandom)
{
	...
	std::function<void()> bg_addfile = [&]() {
		ASSERT_OK(GenerateAndAddExternalFile(options, file_keys,
						     range_id));
	};
...
TEST_F(ExternalSSTFileTest, PickedLevelDynamic)
{
	...
	ASSERT_OK(GenerateAndAddExternalFile(options, { 1, 15, 19 }, -1, false,
					     false, &true_data));
	ASSERT_EQ(FilesPerLevel(), "1,0,0,3");

	ASSERT_OK(GenerateAndAddExternalFile(options, { 1000, 1001, 1002 }, -1,
					     false, false, &true_data));
	ASSERT_EQ(FilesPerLevel(), "1,0,0,4");

	ASSERT_OK(GenerateAndAddExternalFile(options, { 500, 600, 700 }, -1,
					     false, false, &true_data));
	ASSERT_EQ(FilesPerLevel(), "1,0,0,5");

	// File 5 overlaps with file 2 (L3 / base level)
	ASSERT_OK(GenerateAndAddExternalFile(options, { 2, 10 }, -1, false,
					     false, &true_data));
	ASSERT_EQ(FilesPerLevel(), "2,0,0,5");

	// File 6 overlaps with file 2 (L3 / base level) and file 5 (L0)
	ASSERT_OK(GenerateAndAddExternalFile(options, { 3, 9 }, -1, false,
					     false, &true_data));
	...
	ASSERT_OK(GenerateAndAddExternalFile(options, { 650, 651, 652 }, -1,
					     false, false, &true_data));
...
TEST_F(ExternalSSTFileTest, AddExternalSstFileWithCustomCompartor)
{
	...
	for (int i = 0; i < 10; i++) {
		std::string file_name =
			sst_files_dir_ + env_->GenerateUniqueId();
		ASSERT_OK(sst_file_writer.Open(file_name));
	...
	in_files = { generated_files[4], generated_files[6],
		     generated_files[8] };
	ASSERT_OK(DeprecatedAddFile(in_files));
...
TEST_F(ExternalSSTFileTest, AddFileTrivialMoveBug)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"CompactionJob::Run():Start", [&](void *arg) {
			...
			ASSERT_OK(GenerateAndAddExternalFile(options,
							     { 15, 16 }, 7));
		});
	rocksdb::SyncPoint::GetInstance()->EnableProcessing();
		...
		for (int i = 0; i < 2000; i++) {
			...
			if (write_to_memtable && rnd.OneIn(4)) {
				...
				ASSERT_OK(GenerateAndAddExternalFile(
					options, random_data, -1, true, true,
					&true_data));
			}
		}
		size_t kcnt = 0;
...
TEST_F(ExternalSSTFileTest, IngestFileWithGlobalSeqnoMemtableFlush)
{
	...
	// No need for flush
	ASSERT_OK(GenerateAndAddExternalFile(options, { 90, 100, 110 }, -1,
	...
	ASSERT_OK(GenerateAndAddExternalFile(options, { 202, 203, 204 }, -1,
					     true, false, &true_data));
	db_->GetIntProperty(DB::Properties::kNumEntriesActiveMemTable,
			    &entries_in_memtable);
	ASSERT_GE(entries_in_memtable, 1);
...
TEST_F(ExternalSSTFileTest, L0SortingIssue)
{
	...
	ASSERT_OK(
		GenerateAndAddExternalFile(options, { 7, 8 }, -1, true, false));
...
TEST_F(ExternalSSTFileTest, CompactionDeadlock)
{
	...
	rocksdb::SyncPoint::GetInstance()->LoadDependency({
		{ "DBImpl::DelayWrite:Wait",
		  "ExternalSSTFileTest::DeadLock:0" },
		{ "ExternalSSTFileTest::DeadLock:1", "DBImpl::AddFile:Start" },
		{ "DBImpl::AddFile:MutexLock",
		  "ExternalSSTFileTest::DeadLock:2" },
		{ "ExternalSSTFileTest::DeadLock:3",
		  "BackgroundCallCompaction:0" },
	});
	rocksdb::SyncPoint::GetInstance()->EnableProcessing();
...
TEST_F(ExternalSSTFileTest, FileWithCFInfo)
{
	...
	// SST CF dont match
	ASSERT_NOK(
		db_->IngestExternalFile(handles_[1], { cf_default_sst }, ifo));
	// SST CF dont match
	ASSERT_NOK(
		db_->IngestExternalFile(handles_[2], { cf_default_sst }, ifo));
	// SST CF match
	ASSERT_OK(
		db_->IngestExternalFile(handles_[0], { cf_default_sst }, ifo));
...
TEST_F(ExternalSSTFileTest, IngestionListener)
{
	...
	ASSERT_EQ(listener->ingested_files.back()
			  .table_properties.column_family_id,
		  0);
	ASSERT_EQ(listener->ingested_files.back()
			  .table_properties.column_family_name,
		  "default");
...
TEST_F(ExternalSSTFileTest, IngestBehind)
{
	...
	ASSERT_NOK(GenerateAndAddExternalFileIngestBehind(
		options, ifo, file_data, -1, false, &true_data));
	...
	ASSERT_OK(GenerateAndAddExternalFileIngestBehind(
		options, ifo, file_data, -1, false, &true_data));
