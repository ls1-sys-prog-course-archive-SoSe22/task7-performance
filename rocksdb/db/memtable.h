...
class MemTable {
	...
	struct KeyComparator : public MemTableRep::KeyComparator {
		...
		virtual int
		operator()(const char *prefix_len_key1,
	...
	{
		return flush_state_.load(std::memory_order_relaxed) ==
		       FLUSH_REQUESTED;
	}
	...
	bool MarkFlushScheduled()
	{
		...
		return flush_state_.compare_exchange_strong(
			before, FLUSH_SCHEDULED, std::memory_order_relaxed,
	...
	// REQUIRES: external synchronization to prevent simultaneous
	// operations on the same MemTable.
	bool UpdateCallback(SequenceNumber seq, const Slice &key,
			    const Slice &delta);
	...
	void BatchPostProcess(const MemTablePostProcessInfo &update_counters)
	{
		num_entries_.fetch_add(update_counters.num_entries,
				       std::memory_order_relaxed);
		data_size_.fetch_add(update_counters.data_size,
				     std::memory_order_relaxed);
	...
	SequenceNumber GetCreationSeq() const
	{
		return creation_seq_;
	}

	void SetCreationSeq(SequenceNumber sn)
	{
		creation_seq_ = sn;
	}
