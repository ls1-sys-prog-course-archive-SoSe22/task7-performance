...
TEST_F(DBTestDynamicLevel, DynamicLevelMaxBytesBase)
{
	...
	for (int ordered_insert = 0; ordered_insert <= 1; ordered_insert++) {
		...
		for (int max_background_compactions = 1;
			...
			for (int i = 0; i < kNKeys; i++) {
				int key = keys[i];
				ASSERT_OK(Put(Key(kNKeys + key),
					      RandomString(&rnd, 102)));
				ASSERT_OK(
					Put(Key(key), RandomString(&rnd, 102)));
				ASSERT_OK(Put(Key(kNKeys * 2 + key),
					      RandomString(&rnd, 102)));
			...
			ASSERT_TRUE(db_->GetIntProperty(
				"rocksdb.background-errors", &int_prop));
...
TEST_F(DBTestDynamicLevel, DynamicLevelMaxBytesBase2)
{
	...
	rocksdb::port::Thread thread([this] {
		TEST_SYNC_POINT(
			"DynamicLevelMaxBytesBase2:compact_range_start");
