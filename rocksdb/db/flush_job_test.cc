...
class FlushJobTest : public testing::Test {
	...
	FlushJobTest()
		: env_(Env::Default()),
		  dbname_(test::TmpDir() + "/flush_job_test"), options_(),
		  db_options_(options_), table_cache_(NewLRUCache(50000, 16)),
...
TEST_F(FlushJobTest, NonEmpty)
{
	JobContext job_context(0);
	auto cfd = versions_->GetColumnFamilySet()->GetDefault();
	auto new_mem = cfd->ConstructNewMemtable(
	...
	for (int i = 1; i < 10000; ++i) {
		...
		if ((i + 1000) % 10000 < 9995) {
			InternalKey internal_key(key, SequenceNumber(i),
						 kTypeValue);
			inserted_keys.insert(
				{ internal_key.Encode().ToString(), value });
		}
	}
	new_mem->Add(SequenceNumber(10000), kTypeRangeDeletion, "9995",
		     "9999a");
	InternalKey internal_key("9995", SequenceNumber(10000),
				 kTypeRangeDeletion);
	...
	FlushJob flush_job(dbname_,
			   ...
			   &shutting_down_, {}, kMaxSequenceNumber,
			   &job_context, nullptr, nullptr, nullptr,
			   kNoCompression, nullptr, &event_logger, true);
	FileMetaData fd;
	mutex_.Lock();
	flush_job.PickMemTable();
...
TEST_F(FlushJobTest, Snapshots)
{
	...
	for (int i = 0; i < keys / 2; ++i) {
		snapshots.push_back(
			rnd.Uniform(keys * (max_inserts_per_keys / 2)) + 1);
	...
	for (int i = 1; i < keys; ++i) {
		...
		for (int j = 0; j < insertions; ++j) {
			...
			new_mem->Add(SequenceNumber(seqno), kTypeValue, key,
				     value);
			...
			bool visible = (j == insertions - 1) ||
				       (snapshots_set.find(seqno) !=
					snapshots_set.end());
			if (visible) {
				InternalKey internal_key(key, seqno,
							 kTypeValue);
				inserted_keys.insert(
	...
	FlushJob flush_job(dbname_,
			   ...
			   db_options_, *cfd->GetLatestMutableCFOptions(),
			   env_options_, versions_.get(), &mutex_,
			   &shutting_down_, snapshots, kMaxSequenceNumber,
			   &job_context, nullptr, nullptr, nullptr,
			   kNoCompression, nullptr, &event_logger, true);
	mutex_.Lock();
	flush_job.PickMemTable();
