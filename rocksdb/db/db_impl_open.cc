...
DBOptions SanitizeOptions(const std::string &dbname, const DBOptions &src)
{
	...
	if (result.info_log == nullptr) {
		Status s = CreateLoggerFromOptions(dbname, result,
						   &result.info_log);
		if (!s.ok()) {
	...
	auto bg_job_limits =
		DBImpl::GetBGJobLimits(result.max_background_flushes,
	...
	if (result.delayed_write_rate == 0) {
		if (result.rate_limiter.get() != nullptr) {
			result.delayed_write_rate =
				result.rate_limiter->GetBytesPerSecond();
		}
		if (result.delayed_write_rate == 0) {
	...
	if (result.wal_dir.back() == '/') {
		result.wal_dir =
			result.wal_dir.substr(0, result.wal_dir.size() - 1);
	}

	if (result.db_paths.size() == 0) {
		result.db_paths.emplace_back(
			dbname, std::numeric_limits<uint64_t>::max());
	}
	...
	const std::vector<ColumnFamilyDescriptor> &column_families)
{
	...
	for (auto cf : column_families) {
		s = cf.options.table_factory->SanitizeOptions(db_opts,
							      cf.options);
		if (!s.ok()) {
			return s;
		}
		...
		const std::vector<ColumnFamilyDescriptor> &column_families)
{
	...
	if (db_options.keep_log_file_num == 0) {
		return Status::InvalidArgument(
			"keep_log_file_num must be greater than 0");
	}
...
Status DBImpl::NewDB()
{
	...
	const std::string manifest = DescriptorFileName(dbname_, 1);
	{
		...
		if (s.ok()) {
			s = SyncManifest(env_, &immutable_db_options_,
					 log.file());
		}
	}
	if (s.ok()) {
...
Status
DBImpl::Directories::SetDirectories(Env *env, const std::string &dbname,
				    ...
				    const std::vector<DbPath> &data_paths)
{
	...
	for (auto &p : data_paths) {
		...
		if (db_path == dbname) {
			...
			s = CreateAndNewDirectory(env, db_path,
						  &path_directory);
			if (!s.ok()) {
				return s;
			}
...
Status
DBImpl::Recover(const std::vector<ColumnFamilyDescriptor> &column_families,
		...
		bool error_if_data_exists_in_logs)
{
	...
	if (!read_only) {
		Status s = directories_.SetDirectories(
			env_, dbname_, immutable_db_options_.wal_dir,
	...
	if (s.ok()) {
		...
		default_cf_handle_ = new ColumnFamilyHandleImpl(
			versions_->GetColumnFamilySet()->GetDefault(), this,
			&mutex_);
		default_cf_internal_stats_ =
			default_cf_handle_->cfd()->internal_stats();
		single_column_family_mode_ =
			versions_->GetColumnFamilySet()
				->NumberOfColumnFamilies() == 1;
		...
		s = env_->GetChildren(immutable_db_options_.wal_dir,
				      &filenames);
		if (!s.ok()) {
			return s;
		}
			       ...
			       SequenceNumber *next_sequence, bool read_only)
{
	...
	if (immutable_db_options_.wal_filter != nullptr) {
		...
		for (auto cfd : *versions_->GetColumnFamilySet()) {
			...
			cf_lognumber_map.insert(std::make_pair(
				cfd->GetID(), cfd->GetLogNumber()));
		}

		immutable_db_options_.wal_filter->ColumnFamilyLogNumberMap(
			cf_lognumber_map, cf_name_id_map);
	}
#endif
	...
	for (auto log_number : log_numbers) {
		...
		log::Reader reader(immutable_db_options_.info_log,
		...
		while (!stop_replay_by_wal_filter &&
			...
			if (immutable_db_options_.wal_filter != nullptr) {
				...
				case WalFilter::WalProcessingOption::
					kContinueProcessing:
			...
			status = WriteBatchInternal::InsertInto(
				...
				&flush_scheduler_, true, log_number, this,
			...
			if (has_valid_writes && !read_only) {
				...
				while ((cfd = flush_scheduler_
						      .TakeNextColumnFamily()) !=
				       nullptr) {
					cfd->Unref();
					// If this asserts, it means that InsertInto failed in
					// filtering updates to already-flushed column families
					assert(cfd->GetLogNumber() <=
					       log_number);
					auto iter = version_edits.find(
						cfd->GetID());
					assert(iter != version_edits.end());
					VersionEdit *edit = &iter->second;
					status = WriteLevel0TableForRecovery(
						job_id, cfd, cfd->mem(), edit);
					if (!status.ok()) {
	...
	if (!read_only) {
		...
		for (auto cfd : *versions_->GetColumnFamilySet()) {
			...
			if (cfd->GetLogNumber() > max_log_number) {
				...
				assert(cfd->mem()->GetFirstSequenceNumber() ==
				       0);
			...
			// log number
			versions_->MarkFileNumberUsedDuringRecovery(
				max_log_number + 1);
					   ...
					   MemTable *mem, VersionEdit *edit)
{
	...
	TableProperties table_properties;
	{
			...
			cfd->GetLatestMutableCFOptions()->paranoid_file_checks;
		{
			...
			s = BuildTable(
				dbname_, env_, *cfd->ioptions(),
				mutable_cf_options, optimized_env_options,
				cfd->table_cache(), iter.get(),
				std::unique_ptr<InternalIterator>(
					mem->NewRangeTombstoneIterator(ro)),
	...
	if (s.ok() && meta.fd.GetFileSize() > 0) {
		edit->AddFile(level, meta.fd.GetNumber(), meta.fd.GetPathId(),
			      meta.fd.GetFileSize(), meta.smallest,
			      meta.largest, meta.smallest_seqno,
			      meta.largest_seqno, meta.marked_for_compaction);
	}
...
Status DB::Open(const Options &options, const std::string &dbname, DB **dbptr)
{
	...
	Status s =
		DB::Open(db_options, dbname, column_families, &handles, dbptr);
	if (s.ok()) {
		...
		std::vector<ColumnFamilyHandle *> *handles, DB **dbptr)
{
	...
	*dbptr = nullptr;
	...
	for (auto cf : column_families) {
		max_write_buffer_size = std::max(max_write_buffer_size,
						 cf.options.write_buffer_size);
	}
	...
	if (s.ok()) {
		...
		EnvOptions opt_env_options =
			impl->immutable_db_options_.env->OptimizeForLogWrite(
				soptions,
				BuildDBOptions(impl->immutable_db_options_,
					       impl->mutable_db_options_));
		s = NewWritableFile(
			impl->immutable_db_options_.env,
			LogFileName(impl->immutable_db_options_.wal_dir,
				    new_log_number),
		...
		if (s.ok()) {
			...
			for (auto cf : column_families) {
				auto cfd = impl->versions_->GetColumnFamilySet()
						   ->GetColumnFamily(cf.name);
				if (cfd != nullptr) {
					handles->push_back(
		...
		if (s.ok()) {
			for (auto cfd :
			     *impl->versions_->GetColumnFamilySet()) {
				delete impl->InstallSuperVersionAndScheduleWork(
					cfd, nullptr,
					*cfd->GetLatestMutableCFOptions());
			}
			impl->alive_log_files_.push_back(
				DBImpl::LogFileNumberSize(
					impl->logfile_number_));
	...
	if (s.ok()) {
		for (auto cfd : *impl->versions_->GetColumnFamilySet()) {
			if (cfd->ioptions()->compaction_style ==
				...
				for (int i = 1; i < vstorage->num_levels();
				     ++i) {
					int num_files =
						vstorage->NumLevelFiles(i);
					if (num_files > 0) {
						s = Status::InvalidArgument(
	...
	if (s.ok() && sfm) {
		...
		impl->immutable_db_options_.env->GetChildren(db_path.path,
							     &existing_files);
		for (auto &file_name : existing_files) {
			uint64_t file_number;
			FileType file_type;
			std::string file_path = db_path.path + "/" + file_name;
			if (ParseFileName(file_name, &file_number,
					  &file_type) &&
