...
class CompactionPickerTest : public testing::Test {
	...
	CompactionPickerTest()
		: ucmp_(BytewiseComparator()), icmp_(ucmp_),
		  ioptions_(options_), mutable_cf_options_(options_),
		  level_compaction_picker(ioptions_, &icmp_), cf_name_("dummy"),
		  log_buffer_(InfoLogLevel::INFO_LEVEL, &logger_), file_num_(1),
		  vstorage_(nullptr)
	{
		fifo_options_.max_table_files_size = 1;
		mutable_cf_options_.RefreshDerivedOptions(ioptions_);
		ioptions_.db_paths.emplace_back(
			"dummy", std::numeric_limits<uint64_t>::max());
	}
	...
	void NewVersionStorage(int num_levels, CompactionStyle style)
	{
		...
		vstorage_.reset(new VersionStorageInfo(&icmp_, ucmp_,
						       options_.num_levels,
						       style, nullptr, false));
	...
	void UpdateVersionStorageInfo()
	{
		...
		vstorage_->ComputeCompactionScore(ioptions_,
						  mutable_cf_options_);
...
TEST_F(CompactionPickerTest, NeedsCompactionLevel)
{
	...
	for (int level = 0; level < kLevels - 1; ++level) {
		...
		uint64_t file_size =
			vstorage_->MaxBytesForLevel(level) * 2 / kFileCount;
		for (int file_count = 1; file_count <= kFileCount;
		     ++file_count) {
...
TEST_F(CompactionPickerTest, NeedsCompactionUniversal)
{
	...
	UniversalCompactionPicker universal_compaction_picker(ioptions_,
							      &icmp_);
	...
	for (int i = 1;
		...
		Add(0, i, ToString((i + 100) * 1000).c_str(),
		    ToString((i + 100) * 1000 + 999).c_str(), 1000000, 0,
		    i * 100, i * 100 + 99);
		UpdateVersionStorageInfo();
		ASSERT_EQ(level_compaction_picker.NeedsCompaction(
				  vstorage_.get()),
...
TEST_F(CompactionPickerTest, NeedsCompactionFIFO)
{
	...
	for (int i = 1; i <= kFileCount; ++i) {
		NewVersionStorage(1, kCompactionStyleFIFO);
		Add(0, i, ToString((i + 100) * 1000).c_str(),
		    ToString((i + 100) * 1000 + 999).c_str(), kFileSize, 0,
		    i * 100, i * 100 + 99);
...
TEST_F(CompactionPickerTest, CompactionPriMinOverlapping2)
{
	...
	Add(2, 7U, "176", "200",
	    60000000U); // Overlaps with file 27, 28, total size
...
TEST_F(CompactionPickerTest, CompactionPriMinOverlapping3)
{
	...
	Add(2, 8U, "201", "300",
	    61000000U); // Overlaps with file 28, but the file
...
TEST_F(CompactionPickerTest, IsBottommostLevelTest)
{
	...
	bool result = Compaction::TEST_IsBottommostLevel(2, vstorage_.get(),
							 input_files_);
	ASSERT_TRUE(result);
	...
	AddToCompactionFiles(3U);
	AddToCompactionFiles(5U);
	result = Compaction::TEST_IsBottommostLevel(2, vstorage_.get(),
						    input_files_);
	ASSERT_TRUE(result);
