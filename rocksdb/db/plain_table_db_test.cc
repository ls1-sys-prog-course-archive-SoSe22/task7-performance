...
TEST_F(PlainTableKeyDecoderTest, ReadNonMmap)
{
	...
	unique_ptr<PlainTableReaderFileInfo> file_info(
		new PlainTableReaderFileInfo(std::move(file_reader),
					     EnvOptions(), kLength));
	...
	for (size_t i = 0; i < reads.size(); i++) {
		...
		for (auto p : reads[i]) {
			...
			ASSERT_EQ(0,
				  out.compare(tmp.substr(p.first, p.second)));
		}
...
class PlainTableDBTest : public testing::Test,
	...
	Options CurrentOptions()
	{
		...
		options.memtable_factory.reset(
			NewHashLinkListRepFactory(4, 0, 3, true));
	...
	{
		std::string property;
		EXPECT_TRUE(db_->GetProperty("rocksdb.num-files-at-level" +
...
class TestPlainTableReader : public PlainTableReader {
	...
	TestPlainTableReader(const EnvOptions &env_options,
		...
		: PlainTableReader(ioptions, std::move(file), env_options,
...
class TestPlainTableFactory : public PlainTableFactory {
		       ...
		       bool prefetch_index_and_filter_in_cache) const override
	{
		...
		if (store_index_in_file_) {
			...
			s = FindMetaBlock(file.get(), file_size,
					  kPlainTableMagicNumber,
					  table_reader_options.ioptions,
			...
			s = FindMetaBlock(
				file.get(), file_size, kPlainTableMagicNumber,
				table_reader_options.ioptions,
		...
		std::unique_ptr<PlainTableReader> new_reader(
			new TestPlainTableReader(
				table_reader_options.env_options,
				table_reader_options.internal_comparator,
				encoding_type, file_size, bloom_bits_per_key_,
				hash_table_ratio_, index_sparseness_, props,
				std::move(file), table_reader_options.ioptions,
				expect_bloom_not_match_, store_index_in_file_,
...
{
	for (size_t huge_page_tlb_size = 0;
	     huge_page_tlb_size <= 2 * 1024 * 1024;
	     huge_page_tlb_size += 2 * 1024 * 1024) {
		for (EncodingType encoding_type : { kPlain, kPrefix }) {
			for (int bloom_bits = 0; bloom_bits <= 117;
			     bloom_bits += 117) {
				for (int total_order = 0; total_order <= 1;
				     total_order++) {
					for (int store_index_in_file = 0;
					     store_index_in_file <= 1;
					     ++store_index_in_file) {
						Options options =
							CurrentOptions();
						...
						ASSERT_OK(
							Put("1000000000000foo",
							    "v1"));
						ASSERT_OK(
							Put("0000000000000bar",
							    "v2"));
						ASSERT_OK(
							Put("1000000000000foo",
							    "v3"));
			...
			for (int bloom_bits = 0; bloom_bits <= 117;
				...
				for (int total_order = 0; total_order <= 1;
					...
					for (int store_index_in_file = 0;
						...
						ASSERT_OK(
							Put("0000000000000eee",
							    "v3"));
						dbfull()->TEST_FlushMemTable();
						ASSERT_EQ(
							"v3",
							Get("0000000000000eee"));
						...
						if (bloom_bits > 0) {
							...
							ASSERT_EQ(
								"NOT_FOUND",
								Get("5_not00000000bar"));
							// Key doesn't exist any more but prefix exists.
							if (total_order) {
								ASSERT_EQ(
									"NOT_FOUND",
									Get("1000000000000not"));
								ASSERT_EQ(
									"NOT_FOUND",
									Get("0000000000000not"));
							}
							expect_bloom_not_match =
								false;
						}
		...
		for (EncodingType encoding_type : { kPlain, kPrefix }) {
			for (int bloom_bits = 0; bloom_bits <= 117;
				...
				for (int total_order = 0; total_order <= 1;
					...
					ASSERT_OK(
						Put("1000000000foo002", "v_2"));
					ASSERT_OK(Put("0000000000000bar",
						      "random"));
					ASSERT_OK(
						Put("1000000000foo001", "v1"));
					ASSERT_OK(Put("3000000000000bar",
						      "bar_v"));
					ASSERT_OK(Put("1000000000foo003",
						      "v__3"));
					ASSERT_OK(Put("1000000000foo004",
						      "v__4"));
					ASSERT_OK(Put("1000000000foo005",
						      "v__5"));
					ASSERT_OK(Put("1000000000foo007",
						      "v__7"));
					ASSERT_OK(Put("1000000000foo008",
						      "v__8"));
					dbfull()->TEST_FlushMemTable();
					ASSERT_EQ("v1",
						  Get("1000000000foo001"));
					ASSERT_EQ("v__3",
						  Get("1000000000foo003"));
					Iterator *iter = dbfull()->NewIterator(
						ReadOptions());
					iter->Seek("1000000000foo000");
					ASSERT_TRUE(iter->Valid());
					ASSERT_EQ("1000000000foo001",
						  iter->key().ToString());
					ASSERT_EQ("v1",
						  iter->value().ToString());
...
TEST_P(PlainTableDBTest, HashBucketConflict)
{
	for (size_t huge_page_tlb_size = 0;
		...
		for (unsigned char i = 1; i <= 3; i++) {
			...
			plain_table_options.huge_page_tlb_size =
				huge_page_tlb_size;
...
TEST_P(PlainTableDBTest, HashBucketConflictReverseSuffixComparator)
{
	for (size_t huge_page_tlb_size = 0;
		...
		for (unsigned char i = 1; i <= 3; i++) {
			...
			iter->Seek(seek_key);
			ASSERT_TRUE(!iter->Valid() ||
				    options.prefix_extractor->Transform(
					    iter->key()) !=
					    options.prefix_extractor->Transform(
						    seek_key));
