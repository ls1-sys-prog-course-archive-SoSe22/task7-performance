...
struct SmallestKeyHeapComparator {
	explicit SmallestKeyHeapComparator(const Comparator *ucmp)
	{
		ucmp_ = ucmp;
	}
	...
	char *out_buf, size_t out_buf_size, size_t sorted_run_count) const
{
	if (level == 0) {
		...
		snprintf(out_buf, out_buf_size,
			 ...
			 file->fd.GetNumber(), sorted_run_count,
	...
	const VersionStorageInfo &vstorage, const ImmutableCFOptions &ioptions)
{
	...
	for (FileMetaData *f : vstorage.LevelFiles(0)) {
		ret.emplace_back(0, f, f->fd.GetFileSize(),
	...
	for (int level = 1; level < vstorage.num_levels(); level++) {
		...
		for (FileMetaData *f : vstorage.LevelFiles(level)) {
			...
			if (ioptions.compaction_options_universal
				...
				assert(is_first ||
				       f->being_compacted == being_compacted);
			}
			if (is_first) {
				being_compacted = f->being_compacted;
				is_first = false;
		...
		if (total_compensated_size > 0) {
			ret.emplace_back(level, nullptr, total_size,
	...
	VersionStorageInfo *vstorage, LogBuffer *log_buffer)
{
	...
	if ((c = PickCompactionToReduceSizeAmp(cf_name, mutable_cf_options,
		...
		unsigned int ratio =
			ioptions_.compaction_options_universal.size_ratio;

		if ((c = PickCompactionToReduceSortedRuns(
			     cf_name, mutable_cf_options, vstorage, score,
			     ratio, UINT_MAX, sorted_runs, log_buffer)) !=
		    nullptr) {
			ROCKS_LOG_BUFFER(
				log_buffer,
			...
			if (num_sr_not_compacted >
				...
				unsigned int num_files =
					...
					1;
				if ((c = PickCompactionToReduceSortedRuns(
					     cf_name, mutable_cf_options,
					     vstorage, score, UINT_MAX,
					     num_files, sorted_runs,
					     log_buffer)) != nullptr) {
					ROCKS_LOG_BUFFER(
						log_buffer,
...
// validate that all the chosen files of L0 are non overlapping in time
	...
	for (; level_index < c->num_input_levels(); level_index++) {
		if (c->num_input_files(level_index) != 0) {
			...
			GetSmallestLargestSeqno(*(c->inputs(level_index)),
						&smallest_seqno,
						&largest_seqno);
			if (is_first) {
				is_first = false;
	...
	TEST_SYNC_POINT_CALLBACK(
		"UniversalCompactionPicker::PickCompaction:Return", c);
	return c;
}

uint32_t
UniversalCompactionPicker::GetPathId(const ImmutableCFOptions &ioptions,
				     uint64_t file_size)
{
	...
	for (; p < ioptions.db_paths.size() - 1; p++) {
		...
		if (target_size > file_size &&
		    accumulated_size + (target_size - file_size) >
			    future_size) {
			return p;
		}
	...
	const std::vector<SortedRun> &sorted_runs, LogBuffer *log_buffer)
{
	...
	for (size_t loop = 0; loop < sorted_runs.size(); loop++) {
		...
		uint64_t candidate_size =
			sr != nullptr ? sr->compensated_file_size : 0;
		if (sr != nullptr) {
			char file_num_buf[kFormatFileNumberBufSize];
		...
		for (size_t i = loop + 1;
		     candidate_count < max_files_to_compact &&
		     i < sorted_runs.size();
		     i++) {
			...
			if (ioptions_.compaction_options_universal.stop_style ==
				...
				sz = (succeeding_sr->size * (100.0 + ratio)) /
				     100.0;
				...
				candidate_size =
					succeeding_sr->compensated_file_size;
		...
		if (candidate_count >= (unsigned int)min_merge_width) {
			...
			for (size_t i = loop; i < loop + candidate_count &&
					      i < sorted_runs.size();
			     i++) {
				const SortedRun *skipping_sr = &sorted_runs[i];
				char file_num_buf[256];
				skipping_sr->DumpSizeInfo(file_num_buf,
							  sizeof(file_num_buf),
							  loop);
				ROCKS_LOG_BUFFER(log_buffer,
	...
	if (ratio_to_compress >= 0) {
		...
		for (size_t i = sorted_runs.size() - 1; i >= first_index_after;
			...
			if (older_file_size * 100L >=
			    total_size * (long)ratio_to_compress) {
	...
	for (size_t i = start_index; i < first_index_after; i++) {
		...
		ROCKS_LOG_BUFFER(log_buffer, "[%s] Universal: Picking %s",
				 cf_name.c_str(), file_num_buf);
	}
	...
	return new Compaction(
		...
		output_level,
		mutable_cf_options.MaxFileSizeForLevel(output_level), LLONG_MAX,
		path_id,
	...
	const std::vector<SortedRun> &sorted_runs, LogBuffer *log_buffer)
{
	...
	for (size_t loop = 0; loop < sorted_runs.size() - 1; loop++) {
		...
		if (!sr->being_compacted) {
			start_index =
				loop; // Consider this as the first candidate.
			break;
		}
		char file_num_buf[kFormatFileNumberBufSize];
		sr->Dump(file_num_buf, sizeof(file_num_buf), true);
	...
	}
	{
		...
		ROCKS_LOG_BUFFER(
			log_buffer,
			"[%s] Universal: First candidate %s[%" ROCKSDB_PRIszt
			"] %s",
	...
	for (size_t loop = start_index; loop < sorted_runs.size(); loop++) {
		...
		char file_num_buf[256];
		picking_sr.DumpSizeInfo(file_num_buf, sizeof(file_num_buf),
					loop);
		ROCKS_LOG_BUFFER(log_buffer,
