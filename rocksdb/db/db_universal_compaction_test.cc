...
TEST_P(DBTestUniversalCompaction, UniversalCompactionTrigger)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"DBTestWritableFile.GetPreallocationStatus", [&](void *arg) {
			...
			if (num_levels_ > 3) {
				ASSERT_LE(preallocation_size,
					  options.target_file_size_base * 1.1);
			}
		});
...
TEST_P(DBTestUniversalCompaction, CompactFilesOnUniversalCompaction)
{
	...
	ASSERT_FALSE(dbfull()->CompactFiles(CompactionOptions(), handles_[1],
	...
	compaction_input_file_names.push_back(
		cf_meta.levels[0]
			.files[cf_meta.levels[0].files.size() - 1]
			.name);
	ASSERT_OK(dbfull()->CompactFiles(CompactionOptions(), handles_[1],
					 compaction_input_file_names, 0));
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"CompactionJob::Run():Start", [&](void *arg) {
			...
			for (int nwait = 0; nwait < 20000; nwait++) {
				if (has_parallel.load() ||
				    num_compactions_running.load() > 1) {
					has_parallel.store(true);
					break;
				}
...
TEST_P(DBTestUniversalCompaction, UniversalCompactionOptions)
{
	...
	for (int num = 0; num < options.level0_file_num_compaction_trigger;
		...
		for (int i = 0; i < 100; i++) {
			ASSERT_OK(
				Put(1, Key(key_idx), RandomString(&rnd, 990)));
			key_idx++;
		}
		dbfull()->TEST_WaitForFlushMemTable(handles_[1]);
...
TEST_P(DBTestUniversalCompaction, IncreaseUniversalCompactionNumLevels)
{
	std::function<void(int)> verify_func = [&](int num_keys_in_db) {
		...
		Iterator *iter =
			dbfull()->NewIterator(ReadOptions(), handles_[1]);
       ...
       ManualCompactionOutputPathId)
{
	...
	ReopenWithColumnFamilies({ kDefaultColumnFamilyName, "pikachu" },
				 options);
	ASSERT_EQ(1, TotalLiveFiles(1));
	ASSERT_EQ(0, GetSstFileCount(options.db_paths[0].path));
	ASSERT_EQ(1, GetSstFileCount(options.db_paths[1].path));
	...
	compact_options.exclusive_manual_compaction =
		exclusive_manual_compaction_;
	ASSERT_TRUE(db_->CompactRange(compact_options, handles_[1], nullptr,
				      nullptr)
			    .IsInvalidArgument());
}
