	...
	void readOnceFromShard(uint64_t shard)
	{
		...
		if (!state.it) {
			...
			if (FLAGS_iterate_upper_bound) {
				state.upper_bound = Key(
					shard,
					std::numeric_limits<uint64_t>::max());
				state.upper_bound_slice = rocksdb::Slice(
					(const char *)&state.upper_bound,
					sizeof(state.upper_bound));
				options.iterate_upper_bound =
					&state.upper_bound_slice;
			}
		...
		for (rocksdb::Iterator *it :
			...
			bool need_seek = true;
			for (uint64_t seq = state.last_read.load() + 1;
			     seq <= upto; ++seq) {
				if (need_seek) {
					Key from(shard,
						 state.last_read.load() + 1);
...
struct Writer {
	...
	void start()
	{
		thread_ = port::Thread(&Writer::run, this);
	}
	...
	void run()
	{
		...
		std::chrono::steady_clock::time_point deadline(
			...
			std::chrono::nanoseconds(
				(uint64_t)(1000000000 * FLAGS_runtime)));
		...
		while (1) {
			...
			if (workq.empty()) {
				for (int i = 0; i < FLAGS_rate;
				     i += FLAGS_writers) {
					std::chrono::nanoseconds offset(
						1000000000LL * i / FLAGS_rate);
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i <= FLAGS_shards; ++i) {
		std::uniform_int_distribution<int> reader_dist(
			0, FLAGS_readers - 1);
		std::uniform_int_distribution<int> writer_dist(
			0, FLAGS_writers - 1);
