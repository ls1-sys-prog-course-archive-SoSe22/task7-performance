...
class CompactionJobTest : public testing::Test {
	...
	CompactionJobTest()
		: env_(Env::Default()),
		  ...
		  db_options_(), mutable_cf_options_(cf_options_),
		  ...
		  mock_table_factory_(new mock::MockTableFactory())
	{
		...
		db_options_.db_paths.emplace_back(
			dbname_, std::numeric_limits<uint64_t>::max());
	}
	...
	std::string GenerateFileName(uint64_t file_number)
	{
		...
		return TableFileName(db_paths, meta.fd.GetNumber(),
				     meta.fd.GetPathId());
	}

	std::string KeyStr(const std::string &user_key,
	...
	void AddMockFile(const stl_wrappers::KVMap &contents, int level = 0)
	{
		...
		for (auto kv : contents) {
			...
			if (first_key || cfd_->user_comparator()->Compare(
						 key.user_key, smallest) < 0) {
				smallest.assign(key.user_key.data(),
						key.user_key.size());
				smallest_key.DecodeFrom(skey);
			}
			if (first_key || cfd_->user_comparator()->Compare(
						 key.user_key, largest) > 0) {
				largest.assign(key.user_key.data(),
					       key.user_key.size());
		...
		VersionEdit edit;
		edit.AddFile(level, file_number, 0, 10, smallest_key,
	...
	stl_wrappers::KVMap CreateTwoFiles(bool gen_corrupted_keys)
	{
		...
		auto corrupt_id = [&](int id) {
			return gen_corrupted_keys && id > 0 &&
			       id <= kCorruptKeysPerFile;
		};
		...
		for (int i = 0; i < 2; ++i) {
			...
			for (int k = 0; k < kKeysPerFile; ++k) {
				...
				InternalKey internal_key(key, ++sequence_number,
							 kTypeValue);
				...
				contents.insert(
					{ internal_key.Encode().ToString(),
					  value });
				if (i == 1 || k < kMatchingKeys ||
				    corrupt_id(k - kMatchingKeys)) {
			...
			kMaxSequenceNumber)
	{
		...
		for (size_t level = 0; level < input_files.size(); level++) {
			...
			compaction_level.files.insert(
				compaction_level.files.end(),
		...
		CompactionJob compaction_job(
			...
			versions_.get(), &shutting_down_, &log_buffer, nullptr,
			nullptr, nullptr, &mutex_, &bg_error_, snapshots,
		...
		if (expected_results.size() == 0) {
			...
			ASSERT_EQ(compaction_job_stats_.num_input_files,
				  num_input_files);
...
TEST_F(CompactionJobTest, SimpleDeletion)
{
	...
	auto file2 =
		mock::MakeMockFile({ { KeyStr("b", 2U, kTypeValue), "val" },
...
TEST_F(CompactionJobTest, SimpleOverwrite)
{
	...
	auto file2 =
		mock::MakeMockFile({ { KeyStr("a", 1U, kTypeValue), "val" },
...
TEST_F(CompactionJobTest, SimpleNonLastLevel)
{
	...
	auto file2 =
		mock::MakeMockFile({ { KeyStr("a", 3U, kTypeValue), "val" },
...
TEST_F(CompactionJobTest, SimpleMerge)
{
	...
	auto file2 =
		mock::MakeMockFile({ { KeyStr("b", 2U, kTypeMerge), "2" },
...
TEST_F(CompactionJobTest, MergeOperandFilter)
{
	...
	auto file2 = mock::MakeMockFile({
		...
		{ KeyStr("b", 1U, kTypeMerge),
		  test::EncodeInt(10U) } // Filtered
	});
	AddMockFile(file2);

	auto expected_results = mock::MakeMockFile(
		{ { KeyStr("a", 0U, kTypeValue), test::EncodeInt(8U) },
		  { KeyStr("b", 2U, kTypeMerge), test::EncodeInt(2U) } });
...
TEST_F(CompactionJobTest, FilterSomeMergeOperands)
{
	...
	auto file3 = mock::MakeMockFile(
		{ { KeyStr("a", 1U, kTypeMerge), test::EncodeInt(3U) } });
	AddMockFile(file3, 2);
...
TEST_F(CompactionJobTest, FilterAllMergeOperands)
{
	...
	auto file1 = mock::MakeMockFile(
		{ { KeyStr("a", 11U, kTypeMerge), test::EncodeInt(10U) },
		  { KeyStr("a", 10U, kTypeMerge), test::EncodeInt(10U) },
		  { KeyStr("a", 9U, kTypeMerge), test::EncodeInt(10U) } });
	AddMockFile(file1);

	auto file2 = mock::MakeMockFile(
		{ { KeyStr("b", 8U, kTypeMerge), test::EncodeInt(10U) },
	...
	auto file3 = mock::MakeMockFile(
		{ { KeyStr("a", 2U, kTypeMerge), test::EncodeInt(10U) },
		  { KeyStr("b", 1U, kTypeMerge), test::EncodeInt(10U) } });
	AddMockFile(file3, 2);
