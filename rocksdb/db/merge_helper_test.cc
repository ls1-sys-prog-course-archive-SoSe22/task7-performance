...
class MergeHelperTest : public testing::Test {
		   ...
		   SequenceNumber latest_snapshot = 0)
	{
		...
		merge_helper_.reset(new MergeHelper(
			env_, BytewiseComparator(), merge_op_.get(),
			filter_.get(), nullptr, false, latest_snapshot));
...
TEST_F(MergeHelperTest, FilterAllMergeOperands)
{
	...
	ASSERT_EQ(test::KeyStr("a", 24, kTypeDeletion),
		  iter_->key().ToString());
