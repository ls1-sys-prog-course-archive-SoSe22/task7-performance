...
bool VersionEdit::EncodeTo(std::string *dst) const
{
	...
	for (const auto &deleted : deleted_files_) {
		PutVarint32Varint32Varint64(dst, kDeletedFile,
					    deleted.first /* level */,
	...
	for (size_t i = 0; i < new_files_.size(); i++) {
		...
		if (has_customized_fields) {
			...
			TEST_SYNC_POINT_CALLBACK(
				"VersionEdit::EncodeTo:NewFile4:CustomizeFields",
				dst);
...
Status VersionEdit::DecodeFrom(const Slice &src)
{
	...
	while (msg == nullptr && GetVarint32(&input, &tag)) {
		...
		case kNewFile3: {
			...
			if (GetLevel(&input, &level, &msg) &&
			    GetVarint64(&input, &number) &&
			    GetVarint32(&input, &path_id) &&
			    GetVarint64(&input, &file_size) &&
			    GetInternalKey(&input, &f.smallest) &&
			    GetInternalKey(&input, &f.largest) &&
			    GetVarint64(&input, &f.smallest_seqno) &&
			    GetVarint64(&input, &f.largest_seqno)) {
				f.fd = FileDescriptor(number, path_id,
						      file_size);
				new_files_.push_back(std::make_pair(level, f));
			} else {
				if (!msg) {
...
std::string VersionEdit::DebugJSON(int edit_num, bool hex_key) const
{
	...
	if (!deleted_files_.empty()) {
		...
		for (DeletedFileSet::const_iterator iter =
