...
uint64_t DBImpl::MinLogNumberToKeep()
{
	...
	if (allow_2pc()) {
		...
		if (min_log_in_prep_heap != 0 &&
		    min_log_in_prep_heap < log_number) {
			log_number = min_log_in_prep_heap;
		}

		auto min_log_refed_by_mem =
			FindMinPrepLogReferencedByMemTable();

		if (min_log_refed_by_mem != 0 &&
		    min_log_refed_by_mem < log_number) {
			       ...
			       bool no_full_scan)
{
	...
	if (doing_the_full_scan) {
		for (size_t path_id = 0;
		     path_id < immutable_db_options_.db_paths.size();
		     path_id++) {
			...
			for (std::string file : files) {
				...
				job_context->full_scan_candidate_files
					.emplace_back(
						"/" + file,
		...
		if (!immutable_db_options_.db_log_dir.empty() &&
			...
			// Ignore errors
			env_->GetChildren(immutable_db_options_.db_log_dir,
					  &info_log_files);
	...
	if (!alive_log_files_.empty() && !logs_.empty()) {
		...
		while (alive_log_files_.begin()->number < min_log_number) {
			...
			if (immutable_db_options_.recycle_log_file_num >
				...
				job_context->log_delete_files.push_back(
					earliest.number);
			}
			if (job_context->size_log_to_delete == 0) {
				job_context->prev_total_log_size =
					total_log_size_;
				job_context->num_alive_log_files =
					num_alive_log_files;
			}
		...
		while (!logs_.empty() &&
		       logs_.front().number < min_log_number) {
				    ...
				    uint64_t number, uint32_t path_id)
{
	...
	if (file_deletion_status.ok()) {
		...
		ROCKS_LOG_ERROR(immutable_db_options_.info_log,
				"[JOB %d] Failed to delete %s type=%d #%" PRIu64
				" -- %s\n",
				job_id, fname.c_str(), type, number,
				file_deletion_status.ToString().c_str());
	}
	if (type == kTableFile) {
		EventHelpers::LogAndNotifyTableFileDeletion(
			&event_logger_, job_id, number, fname,
...
void DBImpl::PurgeObsoleteFiles(const JobContext &state, bool schedule_only)
{
	...
	for (auto file_num : state.log_delete_files) {
		if (file_num > 0) {
			candidate_files.emplace_back(
				LogFileName(kDumbDbName, file_num), 0);
		}
	}
	...
	candidate_files.erase(std::unique(candidate_files.begin(),
					  candidate_files.end()),
			      candidate_files.end());
	...
	for (const auto &candidate_file : candidate_files) {
		...
		if (!ParseFileName(to_delete, &number, info_log_prefix.prefix,
				   &type)) {
			continue;
		}
		...
		case kTempFile:
			...
			keep = (sst_live_map.find(number) !=
				sst_live_map.end()) ||
			       ...
			       (to_delete.find(kOptionsFileNamePrefix) !=
				std::string::npos);
			break;
		case kInfoLogFile:
			keep = true;
		...
		if (type == kTableFile) {
			...
			fname = TableFileName(immutable_db_options_.db_paths,
					      number, path_id);
		} else {
			fname = ((type == kLogFile) ?
		...
		if (schedule_only) {
			...
			SchedulePendingPurge(fname, type, number, path_id,
					     state.job_id);
		} else {
			DeleteObsoleteFileImpl(file_deletion_status,
					       state.job_id, fname, type,
					       number, path_id);
		}
	}
	...
	if (old_info_log_file_count != 0 &&
	    old_info_log_file_count >=
		    immutable_db_options_.keep_log_file_num) {
		std::sort(old_info_log_files.begin(), old_info_log_files.end());
		size_t end = old_info_log_file_count -
			     immutable_db_options_.keep_log_file_num;
		for (unsigned int i = 0; i <= end; i++) {
			...
			if (!s.ok()) {
				if (env_->FileExists(full_path_to_delete)
					    .IsNotFound()) {
					ROCKS_LOG_INFO(
						immutable_db_options_.info_log,
