...
class DBBlockCacheTest : public DBTestBase {
	...
	void RecordCacheCounters(const Options &options)
	{
		...
		compressed_miss_count_ = TestGetTickerCount(
		...
		compressed_failure_count_ = TestGetTickerCount(
			options, BLOCK_CACHE_COMPRESSED_ADD_FAILURES);
	}
				...
				size_t expected_failures)
	{
		...
		ASSERT_EQ(failure_count_ + expected_failures,
			  new_failure_count);
	...
	void CheckCompressedCacheCounters(const Options &options,
					  ...
					  size_t expected_failures)
	{
		size_t new_miss_count = TestGetTickerCount(
			options, BLOCK_CACHE_COMPRESSED_MISS);
		size_t new_hit_count =
			TestGetTickerCount(options, BLOCK_CACHE_COMPRESSED_HIT);
		size_t new_insert_count =
			TestGetTickerCount(options, BLOCK_CACHE_COMPRESSED_ADD);
		size_t new_failure_count = TestGetTickerCount(
			options, BLOCK_CACHE_COMPRESSED_ADD_FAILURES);
		ASSERT_EQ(compressed_miss_count_ + expected_misses,
			  new_miss_count);
		ASSERT_EQ(compressed_hit_count_ + expected_hits, new_hit_count);
		ASSERT_EQ(compressed_insert_count_ + expected_inserts,
			  new_insert_count);
		ASSERT_EQ(compressed_failure_count_ + expected_failures,
			  new_failure_count);
...
TEST_F(DBBlockCacheTest, TestWithCompressedBlockCache)
{
	...
	std::shared_ptr<Cache> cache = NewLRUCache(0, 0, false);
	std::shared_ptr<Cache> compressed_cache =
...
class MockCache : public LRUCache {
			      ...
			      Handle **handle, Priority priority) override
	{
		...
		return LRUCache::Insert(key, value, charge, deleter, handle,
					priority);
	}
};
...
TEST_F(DBBlockCacheTest, IndexAndFilterBlocksCachePriority)
{
	for (auto priority : { Cache::Priority::LOW, Cache::Priority::HIGH }) {
		...
		ASSERT_EQ(2, /* only index/filter were added */
			  TestGetTickerCount(options, BLOCK_CACHE_ADD));
		ASSERT_EQ(0,
			  TestGetTickerCount(options, BLOCK_CACHE_DATA_MISS));
...
TEST_F(DBBlockCacheTest, ParanoidFileChecks)
{
	...
	ASSERT_OK(dbfull()->SetOptions(
		handles_[1], { { "paranoid_file_checks", "false" } }));
...
TEST_F(DBBlockCacheTest, CompressedCache)
{
	...
	for (int iter = 0; iter < 4; iter++) {
		...
		case 0:
			// only uncompressed block cache
			ASSERT_GT(TestGetTickerCount(options, BLOCK_CACHE_MISS),
				  0);
		...
		case 1:
			// no block cache, only compressed cache
			ASSERT_EQ(TestGetTickerCount(options, BLOCK_CACHE_MISS),
				  0);
			ASSERT_GT(TestGetTickerCount(
					  options, BLOCK_CACHE_COMPRESSED_MISS),
				  0);
			break;
		case 2:
		...
		case 3:
			...
			ASSERT_GT(TestGetTickerCount(options, BLOCK_CACHE_MISS),
				  0);
			ASSERT_GT(TestGetTickerCount(options, BLOCK_CACHE_HIT),
				  0);
			ASSERT_GT(TestGetTickerCount(
					  options, BLOCK_CACHE_COMPRESSED_MISS),
				  0);
