...
TEST_F(DBTest2, IteratorPropertyVersionNumber)
{
	...
	ASSERT_OK(iter1->GetProperty("rocksdb.iterator.super-version-number",
				     &prop_value));
	...
	ASSERT_OK(iter2->GetProperty("rocksdb.iterator.super-version-number",
				     &prop_value));
	...
	ASSERT_OK(iter3->GetProperty("rocksdb.iterator.super-version-number",
				     &prop_value));
...
TEST_P(DBTestSharedWriteBufferAcrossCFs, SharedWriteBufferAcrossCFs)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"Arena::AllocateNewBlock:0", [&](void *arg) {
			std::pair<size_t *, size_t *> *pair =
				static_cast<std::pair<size_t *, size_t *> *>(
					arg);
	...
	if (use_old_interface_) {
		...
		options.write_buffer_manager.reset(
			new WriteBufferManager(114285));
	} else {
		options.write_buffer_manager.reset(
			new WriteBufferManager(114285, cache));
	}
			  ...
			  const std::vector<Slice> &keys_must_not_exist)
{
	...
	if (keys_must_not_exist.size() > 0) {
		std::vector<Status> status_list = db->MultiGet(
			ReadOptions(), keys_must_not_exist, &values);
...
TEST_F(DBTest2, WalFilterTest)
{
	class TestWalFilter : public WalFilter {
		...
		LogRecord(const WriteBatch &batch, WriteBatch *new_batch,
			  bool *batch_changed) const override
		{
			WalFilter::WalProcessingOption option_to_return;

			if (current_record_index_ ==
			    apply_option_at_record_index_) {
	...
	for (int option = 0;
		...
		for (size_t i = 0; i < batch_keys.size(); i++) {
			...
			for (size_t j = 0; j < batch_keys[i].size(); j++) {
				batch.Put(handles_[0], batch_keys[i][j],
					  DummyString(1024));
			}
		...
		Status status = TryReopenWithColumnFamilies(
			{ "default", "pikachu" }, options);
		...
		case WalFilter::WalProcessingOption::kContinueProcessing: {
			...
			for (size_t i = 0; i < batch_keys.size(); i++) {
				for (size_t j = 0; j < batch_keys[i].size();
				     j++) {
					keys_must_exist.push_back(
						Slice(batch_keys[i][j]));
				}
			}
			break;
		}
		case WalFilter::WalProcessingOption::kIgnoreCurrentRecord: {
			fprintf(stderr,
				"Testing with ignoring record %" ROCKSDB_PRIszt
				" only\n",
				apply_option_for_record_index);
			...
			for (size_t i = 0; i < batch_keys.size(); i++) {
				for (size_t j = 0; j < batch_keys[i].size();
					...
					if (i ==
					    apply_option_for_record_index) {
						keys_must_not_exist.push_back(
							Slice(batch_keys[i][j]));
					} else {
						keys_must_exist.push_back(Slice(
							batch_keys[i][j]));
					}
				}
			}
		...
		while (true) {
			...
			ValidateKeyExistence(db_, keys_must_exist,
					     keys_must_not_exist);
...
TEST_F(DBTest2, WalFilterTestWithChangeBatch)
{
	...
	class TestWalFilterWithChangeBatch : public WalFilter {
		...
		{
			if (current_record_index_ >=
			    change_records_from_index_) {
				ChangeBatchHandler handler(
					new_batch,
					num_keys_to_add_in_new_batch_);
	...
	for (size_t i = 0; i < batch_keys.size(); i++) {
		for (size_t j = 0; j < batch_keys[i].size(); j++) {
			if (i >= change_records_from_index &&
			    j >= num_keys_to_add_in_new_batch) {
				keys_must_not_exist.push_back(
					Slice(batch_keys[i][j]));
			} else {
				keys_must_exist.push_back(
...
TEST_F(DBTest2, WalFilterTestWithColumnFamilies)
{
	class TestWalFilterWithColumnFamilies : public WalFilter {
			       ...
			       bool *batch_changed) override
		{
			class LogRecordBatchHandler
				...
				{
					auto it = cf_log_number_map_.find(
						column_family_id);
					assert(it != cf_log_number_map_.end());
					unsigned long long log_number_for_cf =
						it->second;
	...
	for (size_t i = 0; i < batch_keys_pre_flush.size(); i++) {
		...
		for (size_t j = 0; j < batch_keys_pre_flush[i].size(); j++) {
			batch.Put(handles_[0], batch_keys_pre_flush[i][j],
				  DummyString(1024));
			batch.Put(handles_[1], batch_keys_pre_flush[i][j],
				  DummyString(1024));
		}
		dbfull()->Write(WriteOptions(), &batch);
	}
	...
	for (size_t i = 0; i < batch_keys_post_flush.size(); i++) {
		...
		for (size_t j = 0; j < batch_keys_post_flush[i].size(); j++) {
			batch.Put(handles_[0], batch_keys_post_flush[i][j],
				  DummyString(1024));
			batch.Put(handles_[1], batch_keys_post_flush[i][j],
				  DummyString(1024));
		}
		dbfull()->Write(WriteOptions(), &batch);
	}
	...
	auto cf_wal_keys =
		test_wal_filter_column_families.GetColumnFamilyKeys();
	auto name_id_map =
...
TEST_F(DBTest2, PresetCompressionDict)
{
	...
	for (auto compression_type : compression_types) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			CreateAndReopenWithCF({ "pikachu" }, options);
			Random rnd(301);
			std::string seq_data = RandomString(
				&rnd,
				kBlockSizeBytes - kApproxPerBlockOverheadBytes);

			ASSERT_EQ(0, NumTableFilesAtLevel(0, 1));
			for (int j = 0; j < kNumL0Files; ++j) {
				for (size_t k = 0;
				     k < kL0FileBytes / kBlockSizeBytes + 1;
				     ++k) {
					ASSERT_OK(Put(
						1,
						Key(static_cast<int>(
			...
			db_->CompactRange(CompactRangeOptions(), handles_[1],
					  nullptr, nullptr);
			ASSERT_EQ(0, NumTableFilesAtLevel(0, 1));
			...
			for (const auto &file : files) {
				uint64_t curr_bytes;
				env_->GetFileSize(dbname_ + "/" + file,
						  &curr_bytes);
				out_bytes += static_cast<size_t>(curr_bytes);
			}

			for (size_t j = 0;
			     j < kNumL0Files * (kL0FileBytes / kBlockSizeBytes);
			     j++) {
				ASSERT_EQ(seq_data,
					  Get(1, Key(static_cast<int>(j))));
			}
			if (i) {
...
class CompactionCompressionListener : public EventListener {
	...
	void OnCompactionCompleted(DB *db, const CompactionJobInfo &ci) override
	{
		...
		for (int level = 0; level < db->NumberLevels(); level++) {
			...
			ASSERT_TRUE(
				db->GetProperty("rocksdb.num-files-at-level" +
...
TEST_F(DBTest2, CompressionOptions)
{
	...
	for (int iter = 0; iter <= 2; iter++) {
		...
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 5; j++) {
				ASSERT_OK(Put(RandomString(&rnd, kKeySize),
					      RandomString(&rnd, kValSize)));
			}
			ASSERT_OK(Flush());
...
TEST_F(DBTest2, CompactionStall)
{
	...
	CompactionStallTestListener *listener =
		new CompactionStallTestListener();
	options.listeners.emplace_back(listener);
	DestroyAndReopen(options);
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 10; j++) {
			ASSERT_OK(Put(RandomString(&rnd, 10),
				      RandomString(&rnd, 10)));
		}
		ASSERT_OK(Flush());
	}
...
class PinL0IndexAndFilterBlocksTest : public DBTestBase,
	...
	PinL0IndexAndFilterBlocksTest()
		: DBTestBase("/db_pin_l0_index_bloom_test")
	{
	}
	virtual void SetUp() override
	{
		infinite_max_files_ = GetParam();
	}
...
class MockPersistentCache : public PersistentCache {
    public:
	explicit MockPersistentCache(const bool is_compressed,
		      ...
		      const size_t size) override
	{
		...
		data_.insert(std::make_pair(page_key.ToString(),
	...
	for (auto bsize : bsizes) {
		for (auto type : types) {
			...
			table_options.block_cache =
				bsize ? NewLRUCache(bsize) : nullptr;
			...
			auto hit = options.statistics->getTickerCount(
				PERSISTENT_CACHE_HIT);
			auto miss = options.statistics->getTickerCount(
...
TEST_F(DBTest2, SyncPointMarker)
{
	...
	rocksdb::SyncPoint::GetInstance()->LoadDependencyAndMarkers(
		{ { "DBTest2::SyncPointMarker:Thread1First",
		    "DBTest2::MarkedPoint" } },
		{ { "DBTest2::SyncPointMarker:Marker",
		    "DBTest2::MarkedPoint" } });
...
TEST_F(DBTest2, ReadAmpBitmap)
{
	...
	for (size_t k = 0; k < 2; k++) {
		...
		for (size_t i = 0; i < kNumEntries; i++) {
			ASSERT_OK(Put(Key(static_cast<int>(i)),
				      RandomString(&rnd, 100)));
		}
		ASSERT_OK(Flush());
		...
		for (size_t i = 0; i < kNumEntries * 5; i++) {
			...
			if (read_keys.find(key_idx) == read_keys.end()) {
				auto internal_key = InternalKey(
					key, 0, ValueType::kTypeValue);
				total_useful_bytes += GetEncodedEntrySize(
			...
			double error_pct =
				fabs(expected_read_amp - read_amp) * 100;
		...
		if (k == 0) {
			...
			ASSERT_NEAR(options.statistics->getTickerCount(
					    READ_AMP_ESTIMATE_USEFUL_BYTES) *
					    1.0f /
					    options.statistics->getTickerCount(
						    READ_AMP_TOTAL_READ_BYTES),
...
TEST_F(DBTest2, ReadAmpBitmapLiveInCacheAfterDBClose)
{
	...
	uint32_t bytes_per_bit[2] = { 1, 16 };
	for (size_t k = 0; k < 2; k++) {
		std::shared_ptr<Cache> lru_cache =
			NewLRUCache(1024 * 1024 * 1024);
		...
		// Read amp is on average 100% since we read all what we loaded in memory
		if (k == 0) {
			ASSERT_EQ(total_useful_bytes_iter1 +
					  total_useful_bytes_iter2,
				  total_loaded_bytes_iter1 +
					  total_loaded_bytes_iter2);
		} else {
			ASSERT_NEAR((total_useful_bytes_iter1 +
				     total_useful_bytes_iter2) *
					    1.0f /
					    (total_loaded_bytes_iter1 +
					     total_loaded_bytes_iter2),
...
TEST_F(DBTest2, AutomaticCompactionOverlapManualCompaction)
{
	...
	auto get_stat = [](std::string level_str, LevelStatType type,
		...
		auto prop_str = level_str + "." +
				InternalStats::compaction_level_stats.at(type)
					.property_name.c_str();
