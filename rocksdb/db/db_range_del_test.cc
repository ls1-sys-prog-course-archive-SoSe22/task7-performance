...
TEST_F(DBRangeDelTest, CompactionOutputHasOnlyRangeTombstone)
{
	...
	ASSERT_EQ(0,
		  TestGetTickerCount(opts, COMPACTION_RANGE_DEL_DROP_OBSOLETE));
...
TEST_F(DBRangeDelTest, MaxCompactionBytesCutsOutputFiles)
{
	...
	for (int i = 0; i < kNumFiles; ++i) {
		...
		for (int j = 0; j < kNumPerFile; j++) {
			...
			ASSERT_OK(Put(GetNumericStr(kNumPerFile * i + j),
				      values[j]));
		}
...
TEST_F(DBRangeDelTest, SentinelsOmittedFromOutputFile)
{
	...
	for (const auto &range_del : range_dels) {
		ASSERT_OK(db_->DeleteRange(WriteOptions(),
...
TEST_F(DBRangeDelTest, FlushRemovesCoveredKeys)
{
	...
	for (int i = 0; i < kNum; ++i) {
		...
		std::string value;
		if (i < kRangeBegin || i > kRangeEnd || i < kNum / 3 ||
		    i >= 2 * kNum / 3) {
			ASSERT_OK(
				db_->Get(read_opts, GetNumericStr(i), &value));
		} else {
			ASSERT_TRUE(
				db_->Get(read_opts, GetNumericStr(i), &value)
					.IsNotFound());
...
TEST_F(DBRangeDelTest, CompactionRemovesCoveredKeys)
{
	...
	for (int i = 0; i < kNumFiles; ++i) {
		...
		db_->Put(WriteOptions(), GetNumericStr(kNumPerFile * kNumFiles),
			 "val");

		for (int j = 0; j < kNumPerFile; ++j) {
			db_->Put(WriteOptions(),
				 GetNumericStr(i * kNumPerFile + j), "val");
		}
		dbfull()->TEST_WaitForFlushMemTable();
		ASSERT_EQ(i + 1, NumTableFilesAtLevel(0));
	}
	...
	for (int i = 0; i < kNumFiles; ++i) {
		for (int j = 0; j < kNumPerFile; ++j) {
			...
			if (i == kNumFiles - 1 || j >= kNumPerFile / 2) {
				ASSERT_OK(db_->Get(
					read_opts,
					GetNumericStr(i * kNumPerFile + j),
					&value));
			} else {
				ASSERT_TRUE(
					db_->Get(read_opts,
						 GetNumericStr(i * kNumPerFile +
							       j),
						 &value)
						.IsNotFound());
			}
	...
	for (int i = 0; i < 2; ++i) {
		for (int j = 0; j < kNumFiles; ++j) {
			...
			for (int k = 0; k < kNumPerFile; k++) {
				...
				ASSERT_OK(Put(Key(j * kNumPerFile + k),
					      values[k]));
			}
			...
			if (j ==
				...
				// earlier.
				ASSERT_OK(db_->EnableAutoCompaction(
					{ db_->DefaultColumnFamily() }));
...
TEST_F(DBRangeDelTest, ValidUniversalSubcompactionBoundaries)
{
	...
	for (int i = 0; i < kNumLevels - 1; ++i) {
		...
		dbfull()->TEST_WaitForCompact();
		ASSERT_EQ(NumTableFilesAtLevel(0), 0);
		ASSERT_GT(NumTableFilesAtLevel(kNumLevels - 1 - i),
			  kFilesPerLevel - 1);
	}
	...
	ASSERT_OK(dbfull()->RunManualCompaction(
		reinterpret_cast<ColumnFamilyHandleImpl *>(
			db_->DefaultColumnFamily())
			->cfd(),
		1 /* input_level */, 2 /* output_level */,
		0 /* output_path_id */, nullptr /* begin */, nullptr /* end */,
...
TEST_F(DBRangeDelTest, CompactionRemovesCoveredMergeOperands)
{
	...
	opts.memtable_factory.reset(
		new SpecialSkipListFactory(2 * kNumPerFile));
...
{
	const int kNum = 200, kRangeBegin = 50, kRangeEnd = 150,
		  kNumPerFile = 25;
	Options opts = CurrentOptions();
	opts.comparator = test::Uint64Comparator();
	opts.memtable_factory.reset(new SpecialSkipListFactory(kNumPerFile));
