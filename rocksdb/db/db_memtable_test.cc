...
class MockMemTableRep : public MemTableRep {
	...
	void *last_hint_in()
	{
		return last_hint_in_;
	}
	void *last_hint_out()
	{
		return last_hint_out_;
	}
	int num_insert_with_hint()
	{
		return num_insert_with_hint_;
	}
...
class MockMemTableRepFactory : public MemTableRepFactory {
    public:
	virtual MemTableRep *
	CreateMemTableRep(const MemTableRep::KeyComparator &cmp,
	...
	uint32_t GetLastColumnFamilyId()
	{
		return last_column_family_id_;
	}
...
class TestPrefixExtractor : public SliceTransform {
	...
	virtual bool InRange(const Slice &key) const override
	{
		return false;
	}
...
TEST_F(DBMemTableTest, InsertWithHint)
{
	...
	options.env = env_;
	Reopen(options);
	MockMemTableRep *rep = reinterpret_cast<MockMemTableRepFactory *>(
				       options.memtable_factory.get())
				       ->rep();
...
TEST_F(DBMemTableTest, ColumnFamilyId)
{
	...
	for (int cf = 0; cf < 2; ++cf) {
		...
		ASSERT_OK(Flush(cf));
		ASSERT_EQ(cf, static_cast<MockMemTableRepFactory *>(
				      options.memtable_factory.get())
