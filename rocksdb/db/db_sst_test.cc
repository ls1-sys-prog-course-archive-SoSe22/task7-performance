...
TEST_F(DBSSTTest, DeleteObsoleteFilesPendingOutputs)
{
	...
	ASSERT_OK(
		dbfull()->TEST_CompactRange(3, nullptr, nullptr, nullptr,
...
TEST_F(DBSSTTest, DBWithSstFileManager)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"SstFileManagerImpl::OnAddFile",
		[&](void *arg) { files_added++; });
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"SstFileManagerImpl::OnDeleteFile",
		[&](void *arg) { files_deleted++; });
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"SstFileManagerImpl::OnMoveFile",
		[&](void *arg) { files_moved++; });
...
TEST_F(DBSSTTest, RateLimitedDelete)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"DeleteScheduler::BackgroundEmptyTrash:Wait", [&](void *arg) {
			penalties.push_back(*(static_cast<uint64_t *>(arg)));
		});
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"InstrumentedCondVar::TimedWaitInternal", [&](void *arg) {
			...
			env_->addon_time_.fetch_add(static_cast<uint64_t>(
				Random::GetTLSInstance()->Uniform(10)));
...
TEST_F(DBSSTTest, DBWithMaxSpaceAllowedRandomized)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		...
		[&](void *arg) {
			...
			ASSERT_GE(total_sst_files_size,
				  limit_mb_cb * 1024 * 1024);
	...
	for (auto limit_mb : max_space_limits_mbs) {
		...
		while (true) {
			auto s = Put(RandomString(&rnd, 10),
				     RandomString(&rnd, 50));
			if (!s.ok()) {
				break;
			}
			...
			estimated_db_size =
				keys_written * 60; // ~60 bytes per key
			ASSERT_LT(estimated_db_size, estimate_multiplier *
							     limit_mb * 1024 *
							     1024 * 2);
		}
...
TEST_F(DBSSTTest, OpenDBWithInfiniteMaxOpenFiles)
{
	...
	for (int iter = 0; iter < 2; iter++) {
		...
		for (int i = 0; i < 12; i++) {
			ASSERT_EQ(Get("L0_" + Key(i)),
				  "L0_" + Key(i) + std::string(1000, 'a'));
			ASSERT_EQ(Get("L2_" + Key(i)),
				  "L2_" + Key(i) + std::string(1000, 'a'));
		}
	}
}
