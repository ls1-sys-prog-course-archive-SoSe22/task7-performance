...
class TestIterator : public InternalIterator {
	...
	explicit TestIterator(const Comparator *comparator)
		: initialized_(false), valid_(false), sequence_number_(0),
		  iter_(0), cmp(comparator)
	{
	}
		 ...
		 size_t seq_num, bool update_iter = false)
	{
		...
		data_.push_back(std::pair<std::string, std::string>(
			std::string(), argvalue));
		AppendInternalKey(&data_.back().first, internal_key);
		if (update_iter && valid_ &&
		    cmp.Compare(data_.back().first, key()) < 0) {
...
TEST_F(DBIteratorTest, DBIteratorPrevNext)
{
...

	{
		...
		std::unique_ptr<Iterator> db_iter(NewDBIterator(
			env_, ro, cf_options, BytewiseComparator(),
			internal_iter, 10,
...
TEST_F(DBIteratorTest, DBIteratorUseSkipCountSkips)
{
	...
	std::unique_ptr<Iterator> db_iter(
		NewDBIterator(env_, ro, ImmutableCFOptions(options),
...
TEST_F(DBIteratorTest, DBIterator9)
{
		...
		MergeOperators::CreateFromStringId("stringappend");
	{
		...
		std::unique_ptr<Iterator> db_iter(NewDBIterator(
			env_, ro, ImmutableCFOptions(options),
			BytewiseComparator(), internal_iter, 10,
			options.max_sequential_skip_in_iterations, 0));
...
TEST_F(DBIteratorTest, DBIterator12)
{
	...
	std::unique_ptr<Iterator> db_iter(
		NewDBIterator(env_, ro, ImmutableCFOptions(options),
			      BytewiseComparator(), internal_iter, 10, 0, 0));
	db_iter->SeekToLast();
	ASSERT_TRUE(db_iter->Valid());
	ASSERT_EQ(db_iter->key().ToString(), "c");
	ASSERT_EQ(db_iter->value().ToString(), "3");
...
TEST_F(DBIteratorTest, DBIterator13)
{
	...
	std::unique_ptr<Iterator> db_iter(
		NewDBIterator(env_, ro, ImmutableCFOptions(options),
			      BytewiseComparator(), internal_iter, 2, 3, 0));
	db_iter->Seek("b");
	ASSERT_TRUE(db_iter->Valid());
...
TEST_F(DBIteratorTest, DBIterator14)
{
	...
	std::unique_ptr<Iterator> db_iter(
		NewDBIterator(env_, ro, ImmutableCFOptions(options),
			      BytewiseComparator(), internal_iter, 4, 1, 0));
	db_iter->Seek("b");
	ASSERT_TRUE(db_iter->Valid());
	ASSERT_EQ(db_iter->key().ToString(), "b");
	ASSERT_EQ(db_iter->value().ToString(), "3");
...
class DBIterWithMergeIterTest : public testing::Test {
		...
		: env_(Env::Default()), icomp_(BytewiseComparator())
	{
		...
		db_iter_.reset(
			NewDBIterator(env_, ro_, ImmutableCFOptions(options_),
...
TEST_F(DBIterWithMergeIterTest, InnerMergeIteratorDataRace4)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"MergeIterator::Prev:BeforePrev", [&](void *arg) {
			IteratorWrapper *it =
				reinterpret_cast<IteratorWrapper *>(arg);
			if (it->key().starts_with("z")) {
				internal_iter2_->Add("x", kTypeValue, "7", 16u,
						     true);
				internal_iter2_->Add("x", kTypeValue, "7", 15u,
						     true);
				internal_iter2_->Add("x", kTypeValue, "7", 14u,
						     true);
				internal_iter2_->Add("x", kTypeValue, "7", 13u,
						     true);
				internal_iter2_->Add("x", kTypeValue, "7", 12u,
						     true);
				internal_iter2_->Add("x", kTypeValue, "7", 11u,
						     true);
			}
		});
	rocksdb::SyncPoint::GetInstance()->EnableProcessing();
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"MergeIterator::Prev:BeforePrev", [&](void *arg) {
			...
			if (it->key().starts_with("z")) {
				internal_iter2_->Add("x", kTypeValue, "7", 16u,
						     true);
				internal_iter2_->Add("y", kTypeValue, "7", 17u,
						     true);
			}
		});
	rocksdb::SyncPoint::GetInstance()->EnableProcessing();
