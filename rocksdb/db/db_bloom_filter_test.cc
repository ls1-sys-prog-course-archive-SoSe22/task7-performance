...
TEST_F(DBBloomFilterTest, GetFilterByPrefixBloom)
{
	for (bool partition_filters : { true, false }) {
		...
		if (partition_filters) {
			...
			bbto.index_type = BlockBasedTableOptions::IndexType::
...
TEST_P(DBBloomFilterTestWithParam, BloomFilter)
{
	do {
		...
		if (partition_filters_) {
			table_options.index_type = BlockBasedTableOptions::
...
TEST_F(DBBloomFilterTest, BloomFilterRate)
{
	while (ChangeFilterOptions()) {
		...
		ASSERT_GE(TestGetTickerCount(options, BLOOM_FILTER_USEFUL),
			  maxKey * 0.98);
	}
}
...
class WrappedBloom : public FilterPolicy {
	...
	{
		std::unique_ptr<rocksdb::Slice[]> user_keys(
			new rocksdb::Slice[n]);
		for (int i = 0; i < n; ++i) {
...
class SliceTransformLimitedDomain : public SliceTransform {
	const char *Name() const override
	{
		return "SliceTransformLimitedDomain";
	}
...
TEST_F(DBBloomFilterTest, PrefixExtractorFullFilter)
{
	...
	options.prefix_extractor =
		std::make_shared<SliceTransformLimitedDomain>();
	options.table_factory.reset(NewBlockBasedTableFactory(bbto));
...
TEST_F(DBBloomFilterTest, PrefixExtractorBlockFilter)
{
	...
	std::vector<std::string> expected_res = { "val1", "val2", "val3",
						  "val4" };
...
class BloomStatsTestWithParam
	...
	BloomStatsTestWithParam()
	{
		...
		if (use_block_table_) {
			...
			options_.table_factory.reset(
				NewPlainTableFactory(table_options));
		}
		options_.env = env_;
...
TEST_F(DBBloomFilterTest, OptimizeFiltersForHits)
{
	...
	options.max_open_files =
		100; // setting > -1 makes it not preload all files
	options.statistics = CreateDBStatistics();
	options.table_factory.reset(NewBlockBasedTableFactory(bbto));
