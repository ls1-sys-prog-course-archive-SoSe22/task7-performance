...
TEST_F(DBTest, SkipDelay)
{
	...
	for (bool sync : { true, false }) {
		for (bool disableWAL : { true, false }) {
			...
			// the test flaky
			auto token =
				dbfull()->TEST_write_controler().GetDelayToken(
					1);
	...
	do {
		...
		for (int disableWAL = 0; disableWAL <= 1; ++disableWAL) {
			...
			if (wopt.disableWAL) {
				...
				ASSERT_TRUE(db_->Get(ropt, handles_[1], "foo",
						     &value)
						    .IsNotFound());
				ASSERT_TRUE(db_->Get(ropt, handles_[1], "bar",
						     &value)
						    .IsNotFound());
			} else {
				ASSERT_OK(db_->Get(ropt, handles_[1], "foo",
						   &value));
				ASSERT_OK(db_->Get(ropt, handles_[1], "bar",
						   &value));
			}
			...
			auto statuses =
				db_->MultiGet(ropt, multiget_cfs, multiget_keys,
					      &multiget_values);
			if (wopt.disableWAL) {
			...
			ASSERT_OK(Flush(1));
			ASSERT_OK(db_->Put(wopt, handles_[1], "rocksdb",
					   "hello"));

			// once the data has been flushed, we are able to get the
			// data when kPersistedTier is used.
			ASSERT_TRUE(
				db_->Get(ropt, handles_[1], "foo", &value).ok());
			ASSERT_EQ(value, "first");
			ASSERT_TRUE(
				db_->Get(ropt, handles_[1], "bar", &value).ok());
			ASSERT_EQ(value, "one");
			if (wopt.disableWAL) {
				ASSERT_TRUE(db_->Get(ropt, handles_[1],
						     "rocksdb", &value)
						    .IsNotFound());
			} else {
				ASSERT_OK(db_->Get(ropt, handles_[1], "rocksdb",
						   &value));
			...
			ASSERT_TRUE(db_->Get(ropt, handles_[1], "foo", &value)
					    .IsNotFound());
			if (wopt.disableWAL) {
			...
			ASSERT_TRUE(
				db_->Get(ropt, handles_[1], "rocksdb", &value)
					.ok());
			ASSERT_EQ(value, "hello");
...
TEST_F(DBTest, GetFromImmutableLayer)
{
	do {
		...
		// Block sync calls
		env_->delay_sstable_sync_.store(true,
		...
		// Release sync calls
		env_->delay_sstable_sync_.store(false,
						std::memory_order_release);
	} while (ChangeOptions());
}
...
TEST_F(DBTest, FlushSchedule)
{
	...
	std::function<void()> fill_memtable_func = [&]() {
		...
		for (int k = 0; k < 5000; ++k) {
			ASSERT_OK(db_->Put(wo, handles_[a & 1],
					   RandomString(&rnd, 13), ""));
		}
	};
	...
	auto default_tables =
		GetNumberOfSstFilesForColumnFamily(db_, "default");
	auto pikachu_tables =
		GetNumberOfSstFilesForColumnFamily(db_, "pikachu");
...
class KeepFilterFactory : public CompactionFilterFactory {
	...
	{
		if (check_context_) {
			EXPECT_EQ(expect_full_compaction_.load(),
				  context.is_full_compaction);
			EXPECT_EQ(expect_manual_compaction_.load(),
...
TEST_F(DBTest, DISABLED_RepeatedWritesToSameKey)
{
	do {
		...
		std::string value = RandomString(
			&rnd, static_cast<int>(2 * options.write_buffer_size));
...
TEST_F(DBTest, SparseMerge)
{
	do {
		...
		ASSERT_LE(dbfull()->TEST_MaxNextLevelOverlappingBytes(
				  handles_[1]),
			  20 * 1048576);
		...
		for (int run = 0; run < 3; run++) {
			...
			for (int compact_start = 0; compact_start < N;
				...
				for (int i = 0; i < N; i += 10) {
					ASSERT_TRUE(Between(Size("", Key(i), 1),
							    S1 * i, S2 * i));
					ASSERT_TRUE(Between(
						Size("", Key(i) + ".suffix", 1),
						S1 * (i + 1), S2 * (i + 1)));
					ASSERT_TRUE(Between(
						Size(Key(i), Key(i + 10), 1),
						S1 * 10, S2 * 10));
				}
				ASSERT_TRUE(Between(Size("", Key(50), 1),
						    S1 * 50, S2 * 50));
				ASSERT_TRUE(Between(
					Size("", Key(50) + ".suffix", 1),
					S1 * 50, S2 * 50));
...
TEST_F(DBTest, ApproximateSizes_MixOfSmallAndLarge)
{
	do {
		...
		for (int run = 0; run < 3; run++) {
			...
			ASSERT_TRUE(
				Between(Size("", Key(3), 1), 120000, 121000));
			ASSERT_TRUE(
				Between(Size("", Key(4), 1), 130000, 131000));
			ASSERT_TRUE(
				Between(Size("", Key(5), 1), 230000, 231000));
			ASSERT_TRUE(
				Between(Size("", Key(6), 1), 240000, 241000));
			ASSERT_TRUE(
				Between(Size("", Key(7), 1), 540000, 541000));
			ASSERT_TRUE(
				Between(Size("", Key(8), 1), 550000, 560000));

			ASSERT_TRUE(Between(Size(Key(3), Key(5), 1), 110000,
					    111000));
...
TEST_F(DBTest, ComparatorCheck)
{
	class NewComparator : public Comparator {
		...
		virtual int Compare(const Slice &a,
				    const Slice &b) const override
		{
			return BytewiseComparator()->Compare(a, b);
		}
		virtual void
		FindShortestSeparator(std::string *s,
	...
	do {
		...
		ASSERT_TRUE(!s.ok());
		ASSERT_TRUE(s.ToString().find("comparator") !=
			    std::string::npos)
			<< s.ToString();
	} while (ChangeCompactOptions());
}
...
TEST_F(DBTest, CustomComparator)
{
	class NumberComparator : public Comparator {
		...
		static int ToNumber(const Slice &x)
		{
			...
			EXPECT_TRUE(x.size() >= 2 && x[0] == '[' &&
				    x[x.size() - 1] == ']')
				<< EscapeString(x);
			int val;
			char ignored;
			EXPECT_TRUE(sscanf(x.ToString().c_str(), "[%i]%c", &val,
					   &ignored) == 1)
				<< EscapeString(x);
			return val;
		}
...
TEST_F(DBTest, DBOpen_Change_NumLevels)
{
	...
	Status s =
		TryReopenWithColumnFamilies({ "default", "pikachu" }, options);
	ASSERT_TRUE(strstr(s.ToString().c_str(), "Invalid argument") !=
		    nullptr);
	ASSERT_TRUE(db_ == nullptr);
}
...
TEST_F(DBTest, SnapshotFiles)
{
	do {
		...
		Status stat = DB::Open(opts, snapdir, column_families,
				       &cf_handles, &snapdb);
		ASSERT_OK(stat);
		...
		for (unsigned int i = 0; i < 80; i++) {
			stat = snapdb->Get(roptions, cf_handles[i < 40], Key(i),
					   &val);
		...
		for (size_t i = 0; i < newfiles.size(); i++) {
			...
			// latest manifest file
			if (ParseFileName(newfiles[i].substr(1), &number,
					  &type)) {
				if (type == kDescriptorFile) {
...
TEST_F(DBTest, PurgeInfoLogs)
{
	...
	for (int mode = 0; mode <= 1; mode++) {
		...
		std::vector<std::string> files;
		env_->GetChildren(options.db_log_dir.empty() ?
						dbname_ :
						options.db_log_dir,
				  &files);
	...
	while (t->state->stop.load(std::memory_order_acquire) == false) {
		...
		if (rnd.OneIn(2)) {
			...
			if (rnd.OneIn(2)) {
				...
				for (int cf = 0; cf < kColumnFamilies; ++cf) {
					snprintf(valbuf, sizeof(valbuf),
						 "%d.%d.%d.%d.%-1000d", key, id,
						 static_cast<int>(counter), cf,
						 unique_id);
					batch.Put(t->state->test->handles_[cf],
						  Slice(keybuf), Slice(valbuf));
				}
				ASSERT_OK(db->Write(WriteOptions(), &batch));
			} else {
				WriteBatchWithIndex batch(
					db->GetOptions().comparator);
				for (int cf = 0; cf < kColumnFamilies; ++cf) {
					snprintf(valbuf, sizeof(valbuf),
						 "%d.%d.%d.%d.%-1000d", key, id,
						 static_cast<int>(counter), cf,
				...
				ASSERT_OK(db->Write(WriteOptions(),
						    batch.GetWriteBatch()));
			}
		} else {
			...
			std::vector<Status> statuses =
				db->MultiGet(ReadOptions(),
					     t->state->test->handles_, keys,
					     &values);
			Status s = statuses[0];
...
class MultiThreadedDBTest : public DBTest,
	...
	static std::vector<int> GenerateOptionConfigs()
	{
		...
		for (int optionConfig = kDefault; optionConfig < kEnd;
		     ++optionConfig) {
...
class ModelDB : public DB {
	...
	virtual Status SingleDelete(const WriteOptions &o,
	...
	MultiGet(const ReadOptions &options,
	...
	virtual Status
	IngestExternalFile(ColumnFamilyHandle *column_family,
	...
	virtual Status
	GetPropertiesOfAllTables(ColumnFamilyHandle *column_family,
	...
	virtual Status
	GetPropertiesOfTablesInRange(ColumnFamilyHandle *column_family,
	...
	virtual bool
	GetMapProperty(ColumnFamilyHandle *column_family, const Slice &property,
	...
	virtual void
	GetApproximateSizes(ColumnFamilyHandle *column_family,
	...
	Status EnableAutoCompaction(const std::vector<ColumnFamilyHandle *>
	...
	virtual int
	MaxMemCompactionLevel(ColumnFamilyHandle *column_family) override
	{
		return 1;
	}
	...
	virtual int
	Level0StopWriteTrigger(ColumnFamilyHandle *column_family) override
	{
		return -1;
	}
	...
	using DB::GetOptions;
	virtual Options
	GetOptions(ColumnFamilyHandle *column_family) const override
	{
		return options_;
	}
	...
	virtual void
	GetColumnFamilyMetaData(ColumnFamilyHandle *column_family,
...
class DBTestRandomized : public DBTest,
	...
	static std::vector<int> GenerateOptionConfigs()
	{
		...
		for (int option_config = kDefault; option_config < kEnd;
		     ++option_config) {
...
TEST_P(DBTestRandomized, Randomized)
{
	...
	for (int step = 0; step < N; step++) {
		...
		if (p < 45) { // Put
			...
			v = RandomString(&rnd, rnd.OneIn(20) ?
							     100 + rnd.Uniform(100) :
							     rnd.Uniform(8));
		...
		if ((step % 100) == 0) {
			...
			Reopen(options);
			ASSERT_TRUE(CompareIterators(step, &model, db_, nullptr,
						     nullptr));
...
TEST_P(DBTestWithParam, FIFOCompactionTest)
{
	for (int iter = 0; iter < 2; ++iter) {
		...
		for (int i = 0; i < 6; ++i) {
			for (int j = 0; j < 110; ++j) {
				ASSERT_OK(Put(ToString(i * 100 + j),
					      RandomString(&rnd, 980)));
			}
...
TEST_F(DBTest, FIFOCompactionTestWithCompaction)
{
	...
	options.arena_block_size = 4096;
	options.compaction_options_fifo.max_table_files_size = 1500
							       << 10; // 1MB
	...
	for (int i = 0; i < 60; i++) {
		...
		for (int j = 0; j < 20; j++) {
			ASSERT_OK(Put(ToString(i * 20 + j + 2000),
				      RandomString(&rnd, 980)));
		}
		Flush();
		ASSERT_OK(dbfull()->TEST_WaitForCompact());
...
TEST_F(DBTest, DISABLED_RateLimitingTest)
{
	...
	Close();
	ASSERT_EQ(options.rate_limiter->GetTotalBytesThrough(),
		  env_->bytes_written_);
...
TEST_F(DBTest, SanitizeNumThreads)
{
	for (int attempt = 0; attempt < 2; attempt++) {
		...
		for (size_t i = 0; i < kTotalTasks; i++) {
			...
			env_->Schedule(
				&test::SleepingBackgroundTask::DoSleepTask,
				&sleeping_tasks[i],
				(i < 4) ? Env::Priority::LOW :
						Env::Priority::HIGH);
		}
		...
		ASSERT_EQ(1U, options.env->GetThreadPoolQueueLen(
				      Env::Priority::LOW));
		// pool size 2, total task 4. Queue size should be 2.
		ASSERT_EQ(2U, options.env->GetThreadPoolQueueLen(
				      Env::Priority::HIGH));
...
TEST_F(DBTest, DynamicMemtableOptions)
{
	...
	auto gen_l0_kb = [this](int size) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (i % kNumPutsBeforeWaitForFlush ==
			    kNumPutsBeforeWaitForFlush - 1) {
				dbfull()->TEST_WaitForFlushMemTable();
			}
		}
		dbfull()->TEST_WaitForFlushMemTable();
...
TEST_F(DBTest, GetThreadStatus)
{
	...
	for (int i = 0; i < 2; ++i) {
		...
		for (int test = 0; test < kTestCount; ++test) {
			...
			env_->SetBackgroundThreads(kHighPriCounts[test],
						   Env::HIGH);
			env_->SetBackgroundThreads(kLowPriCounts[test],
						   Env::LOW);
			// Wait to ensure the all threads has been registered
			unsigned int thread_type_counts
			...
			for (int num_try = 0; num_try < 60000; num_try++) {
				...
				ASSERT_OK(s);
				memset(thread_type_counts, 0,
				       sizeof(thread_type_counts));
				for (auto thread : thread_list) {
					ASSERT_LT(
						thread.thread_type,
						ThreadStatus::NUM_THREAD_TYPES);
				...
				if (thread_type_counts[ThreadStatus::
							       HIGH_PRIORITY] ==
					    kHighPriCounts[test] &&
				    thread_type_counts[ThreadStatus::
							       LOW_PRIORITY] ==
					    kLowPriCounts[test]) {
					break;
				}
		...
		if (i == 0) {
			...
			CreateAndReopenWithCF({ "pikachu", "about-to-remove" },
					      options);
			env_->GetThreadStatusUpdater()
				->TEST_VerifyColumnFamilyInfoMap(handles_,
								 true);
		}
	}
	...
	for (int tests = 0; tests < 2; ++tests) {
		...
		for (int file = 0; file < kNumL0Files; ++file) {
			for (int key = 0; key < kEntriesPerBuffer; ++key) {
				ASSERT_OK(Put(
					ToString(key + file * kEntriesPerBuffer),
		...
		if (options.enable_thread_tracking) {
			...
			VerifyOperationCount(env_, ThreadStatus::OP_COMPACTION,
					     1);
		} else {
...
TEST_P(DBTestWithParam, PreShutdownManualCompaction)
{
	...
	for (int iter = 0; iter < 2; ++iter) {
		...
		db_->CompactRange(CompactRangeOptions(), handles_[1], nullptr,
				  nullptr);
		ASSERT_EQ("1,0,2", FilesPerLevel(1));
...
TEST_F(DBTest, PreShutdownFlush)
{
	...
	ASSERT_OK(Put(1, "key", "value"));
	CancelAllBackgroundWork(db_);
	Status s = db_->CompactRange(CompactRangeOptions(), handles_[1],
				     nullptr, nullptr);
...
TEST_F(DBTest, DynamicLevelCompressionPerLevel)
{
	...
	ASSERT_GT(SizeAtLevel(0) + SizeAtLevel(3),
		  num_keys * 4000U + num_keys * 10U);
}
...
TEST_F(DBTest, DynamicLevelCompressionPerLevel2)
{
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"LevelCompactionPicker::PickCompaction:Return", [&](void *arg) {
			Compaction *compaction =
				reinterpret_cast<Compaction *>(arg);
			if (compaction->output_level() == 4) {
				ASSERT_TRUE(compaction->output_compression() ==
					    kLZ4Compression);
				num_lz4.fetch_add(1);
			}
		});
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		...
		[&](void *arg) {
			auto *compression =
				reinterpret_cast<CompressionType *>(arg);
...
TEST_F(DBTest, DynamicCompactionOptions)
{
	...
	auto gen_l0_kb = [this](int start, int size, int stride) {
		Random rnd(301);
		for (int i = 0; i < size; i++) {
			ASSERT_OK(Put(Key(start + stride * i),
				      RandomString(&rnd, 1024)));
		}
		dbfull()->TEST_WaitForFlushMemTable();
	};
...
TEST_F(DBTest, DynamicMiscOptions)
{
	...
	auto assert_reseek_count = [this, &options](int key_start,
						    int num_reseek) {
		int key0 = key_start;
	...
	// sanity check
	ASSERT_OK(
		dbfull()->SetOptions({ { "compression", "kNoCompression" } }));
	ASSERT_OK(dbfull()->TEST_GetLatestMutableCFOptions(
		handles_[0], &mutable_cf_options));
	ASSERT_EQ(CompressionType::kNoCompression,
...
TEST_F(DBTest, L0L1L2AndUpHitCounter)
{
	...
	ASSERT_EQ(numkeys,
		  TestGetTickerCount(options, GET_HIT_L0) +
	...
	for (auto comp : compressions) {
		...
		for (int first_table_version = 1; first_table_version <= 2;
			...
			for (int i = 0; i < kNumKeysWritten; ++i) {
				// compressible string
				ASSERT_OK(Put(Key(i),
					      RandomString(&rnd, 128) +
						      std::string(128, 'a')));
			}

			table_options.format_version =
				first_table_version == 1 ? 2 : 1;
			options.table_factory.reset(
				NewBlockBasedTableFactory(table_options));
			Reopen(options);
			for (int i = 0; i < kNumKeysWritten; ++i) {
...
TEST_P(DBTestWithParam, MergeCompactionTimeTest)
{
	...
	options.compaction_filter_factory =
		std::make_shared<KeepFilterFactory>();
	options.statistics = rocksdb::CreateDBStatistics();
	options.merge_operator.reset(new DelayedMergeOperator(this));
...
TEST_F(DBTest, TestLogCleanup)
{
	...
	for (int i = 0; i < 100000; ++i) {
		...
		// 2
		ASSERT_LT(dbfull()->TEST_LogsToFreeSize(),
...
{
	class CompactionFilterFactoryGetContext
		...
		static bool
		IsManual(CompactionFilterFactory *compaction_filter_factory)
		{
			return reinterpret_cast<
				       CompactionFilterFactoryGetContext *>(
	...
	Options options = CurrentOptions();
	options.memtable_factory.reset(new SpecialSkipListFactory(
	...
	for (int i = 0; i < 3; ++i) {
		ASSERT_OK(experimental::SuggestCompactRange(db_, nullptr,
							    nullptr));
		dbfull()->TEST_WaitForCompact();
	}
...
TEST_F(DBTest, LargeBatchWithColumnFamilies)
{
	...
	for (int i = 0; i < 5; i++) {
		for (int pass = 1; pass <= 3; pass++) {
			...
			fprintf(stderr,
				"prepare: %" ROCKSDB_PRIszt " MB, pass:%d\n",
				(write_size / 1024 / 1024), pass);
			for (;;) {
				std::string data(3000, j++ % 127 + 20);
				data += ToString(j);
				batch.Put(handles_[0], Slice(data),
					  Slice(data));
...
TEST_F(DBTest, FailWhenCompressionNotSupportedTest)
{
	...
	for (auto comp : compressions) {
		if (!CompressionTypeSupported(comp)) {
			...
			ColumnFamilyHandle *handle;
			ASSERT_TRUE(!db_->CreateColumnFamily(cf_options, "name",
							     &handle)
					     .ok());
		}
	}
