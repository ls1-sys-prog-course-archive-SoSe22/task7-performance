...
class CorruptionTest : public testing::Test {
	...
	void Check(int min_expected, int max_expected)
	{
		...
		for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
			...
			next_expected = key + 1;
			if (iter->value() !=
			    Value(static_cast<int>(key), &value_space)) {
				bad_values++;
			} else {
				correct++;
			}
		...
		fprintf(stderr,
			...
			min_expected, max_expected, correct, bad_keys,
			bad_values, static_cast<unsigned long long>(missed));
		ASSERT_LE(min_expected, correct);
		ASSERT_GE(max_expected, correct);
	}

	void CorruptFile(const std::string &fname, int offset,
			 int bytes_to_corrupt)
	{
		...
		if (offset < 0) {
			...
			if (-offset > sbuf.st_size) {
				...
				offset =
					static_cast<int>(sbuf.st_size + offset);
			}
		}
		if (offset > sbuf.st_size) {
		...
		for (size_t i = 0; i < filenames.size(); i++) {
			if (ParseFileName(filenames[i], &number, &type) &&
			    ...
			    static_cast<int>(number) >
				    picked_number) { // Pick latest file
	...
	// asserts
	void CorruptTableFileAtLevel(int level, int offset,
				     int bytes_to_corrupt)
	{
		...
		for (const auto &m : metadata) {
			if (m.level == level) {
				CorruptFile(dbname_ + "/" + m.name, offset,
					    bytes_to_corrupt);
				return;
			}
		}
...
TEST_F(CorruptionTest, Recovery)
{
	...
	Corrupt(kLogFile, log::kBlockSize + 1000,
...
TEST_F(CorruptionTest, CompactionInputErrorParanoid)
{
	...
	for (int level = 1; level < dbi->NumberLevels(); level++) {
		...
		dbi->TEST_FlushMemTable();
		for (int comp_level = 0;
		     comp_level < dbi->NumberLevels() - level; ++comp_level) {
