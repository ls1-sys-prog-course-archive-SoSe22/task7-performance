...
{
	std::string fname = TableFileName(ioptions_.db_paths, fd.GetNumber(),
					  fd.GetPathId());
	unique_ptr<RandomAccessFile> file;
	Status s =
		ioptions_.env->NewRandomAccessFile(fname, &file, env_options);
	...
	if (s.ok()) {
		...
		s = ioptions_.table_factory->NewTableReader(
			TableReaderOptions(ioptions_, env_options,
	...
	bool skip_filters, int level)
{
	...
	if (s.ok()) {
		...
		if (for_compaction) {
#ifndef NDEBUG
			bool use_direct_reads_for_compaction =
				env_options.use_direct_reads;
			TEST_SYNC_POINT_CALLBACK(
				"TableCache::NewIterator:for_compaction",
		...
		if (create_new_table_reader) {
			...
			s = GetTableReader(env_options, icomparator, fd,
					   true /* sequential_mode */,
					   readahead,
					   !for_compaction /* record stats */,
					   nullptr, &table_reader_unique_ptr,
					   false /* skip_filters */, level);
			if (s.ok()) {
				table_reader =
					table_reader_unique_ptr.release();
			}
		} else {
	...
	if (s.ok()) {
		result = table_reader->NewIterator(options, arena, &icomparator,
						   skip_filters);
		if (create_new_table_reader) {
			assert(handle == nullptr);
			result->RegisterCleanup(&DeleteTableReader,
	...
	HistogramImpl *file_read_hist, bool skip_filters, int level)
{
	...
	if (table_reader == nullptr) {
		s = FindTable(env_options, icomparator, fd, &handle,
			      ...
			      true /* record read_stats */, file_read_hist,
			      skip_filters, level);
		if (s.ok()) {
		       ...
		       bool skip_filters, int level)
{
	...
	if (ioptions_.row_cache && !get_context->NeedToReadSequence()) {
		...
		uint64_t seq_no = options.snapshot == nullptr ?
						0 :
						1 + GetInternalKeySeqno(k);

		// Compute row cache key.
		row_cache_key.TrimAppend(row_cache_key.Size(),
					 row_cache_id_.data(),
					 row_cache_id_.size());
		...
		if (auto row_handle = ioptions_.row_cache->Lookup(
			    row_cache_key.GetUserKey())) {
			auto found_row_cache_entry =
				static_cast<const std::string *>(
					ioptions_.row_cache->Value(row_handle));
			replayGetContextLog(*found_row_cache_entry, user_key,
					    get_context);
	...
	if (!done && s.ok()) {
		if (t == nullptr) {
			s = FindTable(env_options_, internal_comparator, fd,
				      &handle,
				      options.read_tier ==
					      kBlockCacheTier /* no_io */,
		...
		if (s.ok()) {
			get_context->SetReplayLog(
				row_cache_entry); // nullptr if no cache.
			s = t->Get(options, k, get_context, skip_filters);
			get_context->SetReplayLog(nullptr);
		} else if (options.read_tier == kBlockCacheTier &&
			   s.IsIncomplete()) {
	...
	if (!done && s.ok() && row_cache_entry && !row_cache_entry->empty()) {
		...
		ioptions_.row_cache->Insert(row_cache_key.GetUserKey(), row_ptr,
