...
TransactionLogIteratorImpl::TransactionLogIteratorImpl(
	...
	: dir_(dir), options_(options), read_options_(read_options),
	  soptions_(soptions), startingSequenceNumber_(seq),
	  files_(std::move(files)), started_(false), isValid_(false),
	  currentFileIndex_(0), currentBatchSeq_(0), currentLastSeq_(0),
	  versions_(versions)
{
	...
	const LogFile *logFile, unique_ptr<SequentialFileReader> *file_reader)
{
	...
	if (logFile->Type() == kArchivedLogFile) {
		std::string fname =
			ArchivedLogFileName(dir_, logFile->LogNumber());
						     ...
						     bool strict)
{
	...
	if (strict) {
		currentStatus_ =
			Status::Corruption("Gap in sequence number. Could not "
					   "seek to required sequence number");
		reporter_.Info(currentStatus_.ToString().c_str());
	} else if (files_->size() != 1) {
		currentStatus_ =
			Status::Corruption("Start sequence was not found, "
...
void TransactionLogIteratorImpl::NextImpl(bool internal)
{
	...
	while (true) {
		...
		if (currentFileIndex_ < files_->size() - 1) {
			++currentFileIndex_;
			Status s = OpenLogReader(
				files_->at(currentFileIndex_).get());
			if (!s.ok()) {
				isValid_ = false;
				currentStatus_ = s;
				return;
...
void TransactionLogIteratorImpl::UpdateCurrentWriteBatch(const Slice &record)
{
	...
	if (started_ && !IsBatchExpected(batch.get(), expectedSeq)) {
		...
		if (expectedSeq <
		    files_->at(currentFileIndex_)->StartSequence()) {
