...
class GenerateLevelFilesBriefTest : public testing::Test {
	...
	int Compare()
	{
		int diff = 0;
		for (size_t i = 0; i < files_.size(); i++) {
			if (file_level_.files[i].fd.GetNumber() !=
			    files_[i]->fd.GetNumber()) {
				diff++;
			}
		}
		return diff;
...
class FindLevelFileTest : public testing::Test {
	...
	{
		char *mem =
			arena_.AllocateAligned(num * sizeof(FdWithKeyRange));
		 ...
		 SequenceNumber largest_seq = 100)
	{
		...
		InternalKey largest_key =
			InternalKey(largest, largest_seq, kTypeValue);

		Slice smallest_slice = smallest_key.Encode();
		Slice largest_slice = largest_key.Encode();

		char *mem = arena_.AllocateAligned(smallest_slice.size() +
						   largest_slice.size());
	...
	bool Overlaps(const char *smallest, const char *largest)
	{
		...
		return SomeFileOverlapsRange(
			cmp, disjoint_sorted_files_, file_level_,
