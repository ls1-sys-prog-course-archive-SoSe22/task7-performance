...
ManagedIterator::ManagedIterator(DBImpl *db, const ReadOptions &read_options,
	...
	: db_(db), read_options_(read_options), cfd_(cfd),
	  svnum_(cfd->GetSuperVersionNumber()), mutable_iter_(nullptr),
	  valid_(false), snapshot_created_(false), release_supported_(true)
{
	...
	cfh_.SetCFD(cfd);
	mutable_iter_ =
		unique_ptr<Iterator>(db->NewIterator(read_options_, &cfh_));
}
...
void ManagedIterator::RebuildIterator()
{
	...
	mutable_iter_ =
		unique_ptr<Iterator>(db_->NewIterator(read_options_, &cfh_));
}
