...
class SkipEvenFilter : public CompactionFilter {
				  ...
				  std::string *skip_until) const override
	{
		...
		if (i / 10 % 2 == 0) {
			char key_str[100];
			snprintf(key_str, sizeof(key), "%010d",
				 i / 10 * 10 + 10);
			*skip_until = key_str;
			++cfilter_skips;
...
TEST_F(DBTestCompactionFilter, CompactionFilter)
{
	...
	Arena arena;
	{
		...
		ScopedArenaIterator iter(dbfull()->NewInternalIterator(
			&arena, &range_del_agg, handles_[1]));
		iter->SeekToFirst();
	...
	options.compaction_filter_factory =
		std::make_shared<DeleteFilterFactory>();
...
TEST_F(DBTestCompactionFilter, CompactionFilterWithValueChange)
{
	do {
		...
		if (option_config_ != kUniversalCompactionMultiLevel &&
			...
			dbfull()->CompactRange(CompactRangeOptions(),
...
TEST_F(DBTestCompactionFilter, CompactionFilterIgnoreSnapshot)
{
	...
	Options options = CurrentOptions();
	options.compaction_filter_factory =
		std::make_shared<DeleteISFilterFactory>();
	options.disable_auto_compactions = true;
	options.create_if_missing = true;
	DestroyAndReopen(options);
...
TEST_F(DBTestCompactionFilter, SkipUntil)
{
	Options options = CurrentOptions();
	options.compaction_filter_factory =
		std::make_shared<SkipEvenFilterFactory>();
	options.disable_auto_compactions = true;
	options.create_if_missing = true;
	DestroyAndReopen(options);
