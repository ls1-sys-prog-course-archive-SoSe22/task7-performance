...
TEST_F(WriteBatchTest, Corruption)
{
	...
	WriteBatchInternal::SetContents(&batch, Slice(contents.data(),
						      contents.size() - 1));
...
TEST_F(WriteBatchTest, Continue)
{
	...
	struct Handler : public TestHandler {
		...
		{
			++num_seen;
			return TestHandler::SingleDeleteCF(column_family_id,
							   key);
		}
		virtual Status MergeCF(uint32_t column_family_id,
				       const Slice &key,
		...
		{
			++num_seen;
			return TestHandler::MergeCF(column_family_id, key,
						    value);
		}
		virtual void LogData(const Slice &blob) override
		{
...
TEST_F(WriteBatchTest, PutGatherSlices)
{
...

	{
		...
		Slice key_slices[3] = { Slice("key"), Slice("part2"),
					Slice("part3") };
