...
size_t CompactedDBImpl::FindFile(const Slice &key)
{
	...
	while (left < right) {
		...
		if (user_comparator_->Compare(ExtractUserKey(f.largest_key),
					      key) < 0) {
			    ...
			    const Slice &key, PinnableSlice *value)
{
	GetContext get_context(user_comparator_, nullptr, nullptr, nullptr,
			       GetContext::kNotFound, key, value, nullptr,
	...
	const std::vector<Slice> &keys, std::vector<std::string> *values)
{
	...
	for (const auto &key : keys) {
		...
		if (user_comparator_->Compare(
			    key, ExtractUserKey(f.smallest_key)) < 0) {
	...
	for (auto *r : reader_list) {
		if (r != nullptr) {
			...
			GetContext get_context(user_comparator_, nullptr,
					       nullptr, nullptr,
					       GetContext::kNotFound, keys[idx],
					       &pinnable_val, nullptr, nullptr,
					       nullptr, nullptr);
			     ...
			     DB **dbptr)
{
	*dbptr = nullptr;
	...
	std::unique_ptr<CompactedDBImpl> db(
		new CompactedDBImpl(db_options, dbname));
