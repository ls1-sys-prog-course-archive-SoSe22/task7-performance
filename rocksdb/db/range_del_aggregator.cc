...
void RangeDelAggregator::InitRep(const std::vector<SequenceNumber> &snapshots)
{
	...
	for (auto snapshot : snapshots) {
		rep_->stripe_map_.emplace(
			snapshot, PositionalTombstoneMap(TombstoneMap(
					  stl_wrappers::LessOfComparator(
	...
	RangeDelAggregator::RangePositioningMode mode)
{
	...
	auto &positional_tombstone_map =
		GetPositionalTombstoneMap(parsed.sequence);
	...
	case kFullScan:
		...
		while (tombstone_map_iter != tombstone_map.end()) {
			...
			if (icmp_.user_comparator()->Compare(
				    parsed.user_key, tombstone.start_key_) <
			    0) {
				break;
			}
			if (parsed.sequence < tombstone.seq_ &&
			    icmp_.user_comparator()->Compare(
				    parsed.user_key, tombstone.end_key_) < 0) {
				return true;
			}
			++tombstone_map_iter;
		}
	...
	case kBackwardTraversal:
		...
		while (tombstone_map_iter != tombstone_map.begin() &&
		       icmp_.user_comparator()->Compare(
			       parsed.user_key, tombstone_map_iter->first) <
			       0) {
			--tombstone_map_iter;
		}
		if (tombstone_map_iter == tombstone_map.begin() &&
	...
	assert(std::next(tombstone_map_iter) == tombstone_map.end() ||
	       icmp_.user_comparator()->Compare(
		       parsed.user_key, std::next(tombstone_map_iter)->first) <
		       0);
	return parsed.sequence < tombstone_map_iter->second.seq_;
}

bool RangeDelAggregator::ShouldAddTombstones(bool bottommost_level /* = false */)
{
...
Status
RangeDelAggregator::AddTombstones(std::unique_ptr<InternalIterator> input)
{
	if (input == nullptr) {
		return Status::OK();
	}
	...
	while (input->Valid()) {
		...
		if (!ParseInternalKey(input->key(), &parsed_key)) {
			return Status::Corruption(
				"Unable to parse range tombstone InternalKey");
		}
	...
	if (!first_iter) {
		rep_->pinned_iters_mgr_.PinIterator(input.release(),
						    false /* arena */);
	}
	return Status::OK();
}
...
void RangeDelAggregator::InvalidateTombstoneMapPositions()
{
	...
	for (auto stripe_map_iter = rep_->stripe_map_.begin();
		...
		stripe_map_iter->second.iter =
			stripe_map_iter->second.raw_map.end();
	}
}
...
{
	auto &positional_tombstone_map =
		GetPositionalTombstoneMap(tombstone.seq_);
	...
	if (collapse_deletions_) {
		...
		std::vector<RangeTombstone> new_range_dels{
			tombstone,
			RangeTombstone(tombstone.end_key_, Slice(), 0)
		};
		...
		auto tombstone_map_iter = tombstone_map.upper_bound(
			new_range_dels_iter->start_key_);
		...
		while (tombstone_map_iter != tombstone_map.end() &&
			...
			if (tombstone_map_iter != tombstone_map.end()) {
				auto next_tombstone_map_iter =
					std::next(tombstone_map_iter);
				if (next_tombstone_map_iter !=
				    tombstone_map.end()) {
					tombstone_map_iter_end =
						&next_tombstone_map_iter->first;
				}
			}
			if (new_range_dels_iter != new_range_dels.end()) {
				auto next_new_range_dels_iter =
					std::next(new_range_dels_iter);
				if (next_new_range_dels_iter !=
				    new_range_dels.end()) {
					new_range_dels_iter_end =
						&next_new_range_dels_iter
							 ->start_key_;
				}
			}
			...
			if (new_to_old_start_cmp < 0) {
				...
				// it's covered.
				if (tombstone_map_iter->second.seq_ <
				    new_range_dels_iter->seq_) {
					untermed_seq =
						tombstone_map_iter->second.seq_;
					if (tombstone_map_iter !=
						    tombstone_map.begin() &&
					    std::prev(tombstone_map_iter)
							    ->second.seq_ ==
						    new_range_dels_iter->seq_) {
						tombstone_map_iter =
							tombstone_map.erase(
		...
		tombstone_map.emplace(tombstone.start_key_,
				      std::move(tombstone));
	}
	return Status::OK();
}
...
void RangeDelAggregator::AddToBuilder(
	TableBuilder *builder, const Slice *lower_bound,
	...
	bool bottommost_level /* = false */)
{
	...
	if (bottommost_level) {
		...
		if (!stripe_map_iter->second.raw_map.empty()) {
			range_del_out_stats->num_range_del_drop_obsolete +=
				static_cast<int64_t>(
					stripe_map_iter->second.raw_map.size()) -
	...
	while (stripe_map_iter != rep_->stripe_map_.end()) {
		for (auto tombstone_map_iter =
			     stripe_map_iter->second.raw_map.begin();
		     tombstone_map_iter !=
		     stripe_map_iter->second.raw_map.end();
		     ++tombstone_map_iter) {
			...
			if (collapse_deletions_) {
				auto next_tombstone_map_iter =
					std::next(tombstone_map_iter);
				if (next_tombstone_map_iter ==
					    stripe_map_iter->second.raw_map
						    .end() ||
			...
			if (upper_bound != nullptr &&
			    icmp_.user_comparator()->Compare(
				    *upper_bound, tombstone.start_key_) <= 0) {
			...
			if (lower_bound != nullptr &&
			    icmp_.user_comparator()->Compare(
				    tombstone.end_key_, *lower_bound) <= 0) {
			...
			builder->Add(ikey_and_end_key.first.Encode(),
				     ikey_and_end_key.second);
			if (!first_added) {
				first_added = true;
				InternalKey smallest_candidate =
					std::move(ikey_and_end_key.first);
				;
				if (lower_bound != nullptr &&
				    icmp_.user_comparator()->Compare(
					    smallest_candidate.user_key(),
					    *lower_bound) <= 0) {
					...
					// key.
					smallest_candidate =
						InternalKey(*lower_bound, 0,
							    kTypeRangeDeletion);
				}
				if (meta->smallest.size() == 0 ||
				    icmp_.Compare(smallest_candidate,
						  meta->smallest) < 0) {
					meta->smallest =
						std::move(smallest_candidate);
				}
			}
			...
			if (upper_bound != nullptr &&
				...
				largest_candidate =
					InternalKey(*upper_bound,
						    kMaxSequenceNumber,
						    kTypeRangeDeletion);
			}
			if (meta->largest.size() == 0 ||
			    icmp_.Compare(meta->largest, largest_candidate) <
				    0) {
				meta->largest = std::move(largest_candidate);
			}
			meta->smallest_seqno =
				std::min(meta->smallest_seqno, tombstone.seq_);
			meta->largest_seqno =
				std::max(meta->largest_seqno, tombstone.seq_);
		}
		++stripe_map_iter;
	}
}
