...
TEST_P(DBCompactionTestWithParam, CompactionTrigger)
{
	...
	options.memtable_factory.reset(
		new SpecialSkipListFactory(kNumKeysPerFile));
...
TEST_F(DBCompactionTest, BGCompactionsAllowed)
{
	...
	options.soft_pending_compaction_bytes_limit = 1
						      << 30; // Infinitely large
	...
	for (int cf = 0; cf < 4; cf++) {
		for (int num = 0;
		     num < options.level0_file_num_compaction_trigger; num++) {
			for (int i = 0; i < kNumKeysPerFile; i++) {
	...
	for (int num = 0; num < options.level0_file_num_compaction_trigger;
	     num++) {
		for (int i = 0; i < kNumKeysPerFile; i++) {
...
TEST_F(DBCompactionTest, ManualCompactionUnknownOutputSize)
{
	...
	for (int i = 0; i < 2; ++i) {
		for (int j = 0; j < options.level0_file_num_compaction_trigger;
		     j++) {
	...
	dbfull()->GetColumnFamilyMetaData(dbfull()->DefaultColumnFamily(),
					  &cf_meta);
...
TEST_F(DBCompactionTest, RecoverDuringMemtableCompaction)
{
	do {
		...
		ASSERT_OK(Put(1, "big1",
			      std::string(10000000, 'x'))); // Fills memtable
		ASSERT_OK(Put(1, "big2",
			      std::string(1000, 'y'))); // Triggers compaction
...
TEST_P(DBCompactionTestWithParam, TrivialMoveNonOverlappingFiles)
{
	...
	ranges = {
		{ 100, 199 }, { 300, 399 }, { 0, 99 },	  { 200, 299 },
		{ 600, 699 }, { 400, 499 }, { 500, 560 }, // this range overlap with the next one
		{ 551, 599 },
	};
	for (size_t i = 0; i < ranges.size(); i++) {
		for (int32_t j = ranges[i].first; j <= ranges[i].second; j++) {
...
TEST_P(DBCompactionTestWithParam, ManualCompactionPartial)
{
	...
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "DBCompaction::ManualPartial:4",
		    "DBCompaction::ManualPartial:1" },
		  { "DBCompaction::ManualPartial:5",
		    "DBCompaction::ManualPartial:2" },
		  { "DBCompaction::ManualPartial:5",
		    "DBCompaction::ManualPartial:3" } });
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
...
TEST_F(DBCompactionTest, DISABLED_ManualPartialFill)
{
	...
	bool second = true;
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "DBCompaction::PartialFill:4",
		    "DBCompaction::PartialFill:1" },
		  { "DBCompaction::PartialFill:2",
		    "DBCompaction::PartialFill:3" } });
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"DBImpl::BackgroundCompaction:NonTrivial:AfterRun",
		[&](void *arg) {
			if (first) {
	...
	for (int32_t i = 1; i < options.num_levels; i++) {
		...
		target_size = static_cast<uint64_t>(
...
TEST_F(DBCompactionTest, DeleteFileRange)
{
	...
	Slice begin(begin_string);
	Slice end(end_string);
	ASSERT_OK(DeleteFilesInRange(db_, db_->DefaultColumnFamily(), &begin,
				     &end));
	...
	ASSERT_OK(DeleteFilesInRange(db_, db_->DefaultColumnFamily(), &begin1,
				     &end1));
	...
	ASSERT_OK(DeleteFilesInRange(db_, db_->DefaultColumnFamily(), nullptr,
				     nullptr));
...
TEST_P(DBCompactionTestWithParam, ConvertCompactionStyle)
{
	...
	for (int i = max_key_level_insert / 2; i <= max_key_universal_insert;
	     i++) {
		ASSERT_OK(Put(1, Key(i), RandomString(&rnd, 10000)));
	}
...
TEST_F(DBCompactionTest, L0_CompactionBug_Issue44_a)
{
	do {
		...
		env_->SleepForMicroseconds(
			1000000); // Wait for compaction to finish
		ASSERT_EQ("(a->v)", Contents(1));
	} while (ChangeCompactOptions());
}
...
TEST_P(DBCompactionTestWithParam, ManualLevelCompactionOutputPathId)
{
	...
	for (int iter = 0; iter < 2; ++iter) {
		...
		compact_options.exclusive_manual_compaction =
			exclusive_manual_compaction_;
		db_->CompactRange(compact_options, handles_[1], nullptr,
				  nullptr);
		...
		if (iter == 0) {
			DestroyAndReopen(options);
			options = CurrentOptions();
			options.db_paths.emplace_back(dbname_ + "_2",
						      2 * 10485760);
			options.db_paths.emplace_back(dbname_ + "_3",
						      100 * 10485760);
			options.db_paths.emplace_back(dbname_ + "_4",
						      120 * 10485760);
...
TEST_P(DBCompactionTestWithParam, DISABLED_CompactFilesOnLevelCompaction)
{
	...
	for (int file_picked = 5; file_picked > 0; --file_picked) {
		...
		for (int f = 0; f < file_picked; ++f) {
			int level = 0;
			auto file_meta =
				PickFileRandomly(cf_meta, &rnd, &level);
			compaction_input_file_names.push_back(file_meta->name);
			GetOverlappingFileNumbersForLevelCompaction(
				cf_meta, options.comparator, level,
				output_level, file_meta,
...
TEST_P(DBCompactionTestWithParam, DeleteMovedFileAfterCompaction)
{
	...
	for (int iter = 0; iter < 2; ++iter) {
		...
		for (int i = 0; i < 2; ++i) {
			// Create 1MB sst file
			for (int j = 0; j < 100; ++j) {
				ASSERT_OK(Put(Key(i * 50 + j),
					      RandomString(&rnd, 10 * 1024)));
			}
			ASSERT_OK(Flush());
		}
...
TEST_P(DBCompactionDirectIOTest, DirectIO)
{
	...
	if (options.use_direct_io_for_flush_and_compaction) {
		SyncPoint::GetInstance()->SetCallBack(
			"SanitizeOptions:direct_io",
			[&](void *arg) { readahead = true; });
	}
