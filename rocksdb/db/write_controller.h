...
class WriteController {
    public:
	explicit WriteController(uint64_t _delayed_write_rate = 1024u * 1024u *
								32u,
				 int64_t low_pri_rate_bytes_per_sec = 1024 *
								      1024)
		: total_stopped_(0), total_delayed_(0),
		  total_compaction_pressure_(0), bytes_left_(0),
		  last_refill_time_(0),
	...
	GetDelayToken(uint64_t delayed_write_rate);
	...
	{
		return total_delayed_.load() > 0;
	}
	bool NeedSpeedupCompaction() const
	{
		return IsStopped() || NeedsDelay() ||
	...
	uint64_t max_delayed_write_rate() const
	{
		return max_delayed_write_rate_;
	}

	RateLimiter *low_pri_rate_limiter()
	{
		return low_pri_rate_limiter_.get();
	}
