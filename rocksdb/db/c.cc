...
struct rocksdb_compactionfilter_t : public CompactionFilter {
	...
	unsigned char (*filter_)(void *, int level, const char *key,
...
struct rocksdb_compactionfilterfactory_t : public CompactionFilterFactory {
		...
		const CompactionFilter::Context &context) override
	{
		...
		CompactionFilter *cf =
			(*create_compaction_filter_)(state_, &ccontext);
...
struct rocksdb_comparator_t : public Comparator {
	...
	{
		return (*compare_)(state_, a.data(), a.size(), b.data(),
				   b.size());
	}
...
struct rocksdb_filterpolicy_t : public FilterPolicy {
	...
	void (*delete_filter_)(void *, const char *filter,
			       size_t filter_length);
				  ...
				  std::string *dst) const override
	{
		...
		size_t len;
		char *filter = (*create_)(state_, &key_pointers[0],
					  &key_sizes[0], n, &len);
...
struct rocksdb_mergeoperator_t : public MergeOperator {
				 ...
				 MergeOperationOutput *merge_out) const override
	{
		...
		char *tmp_new_value =
			(*full_merge_)(state_, merge_in.key.data(),
				       merge_in.key.size(), existing_value_data,
				       existing_value_len, &operand_pointers[0],
				       &operand_sizes[0], static_cast<int>(n),
				       ...
				       Logger *logger) const override
	{
		...
		char *tmp_new_value = (*partial_merge_)(
			...
			&operand_sizes[0], static_cast<int>(operand_count),
			&success, &new_value_len);
...
{
	char *result =
		reinterpret_cast<char *>(malloc(sizeof(char) * str.size()));
				      ...
				      char **errptr)
{
	DB *db;
	if (SaveError(errptr,
		      DB::OpenForReadOnly(options->rep, std::string(name), &db,
...
{
	SaveError(errptr, be->rep->RestoreDBFromLatestBackup(
				  std::string(db_dir), std::string(wal_dir),
				  restore_options->rep));
}
...
{
	SaveError(errptr,
		  checkpoint->rep->CreateCheckpoint(std::string(checkpoint_dir),
						    log_size_for_flush));
}
	...
	rocksdb_column_family_handle_t **column_family_handles, char **errptr)
{
	...
	if (SaveError(errptr,
		      DB::Open(DBOptions(db_options->rep), std::string(name),
		       ...
		       size_t *values_list_sizes, char **errs)
{
	...
	std::vector<Status> statuses =
		db->rep->MultiGet(options->rep, keys, &values);
	for (size_t i = 0; i < num_keys; i++) {
		if (statuses[i].ok()) {
			      ...
			      char **errptr)
{
	...
	Status status =
		db->rep->NewIterators(opts->rep, column_families_vec, &res);
	...
	const size_t *range_limit_key_len, uint64_t *sizes)
{
	...
	db->rep->GetApproximateSizes(column_family->rep, ranges, num_ranges,
				     sizes);
	delete[] ranges;
}
			   ...
			   size_t limit_key_len)
{
	...
	db->rep->CompactRange(
		...
		(start_key ? (a = Slice(start_key, start_key_len), &a) :
				   nullptr),
		(limit_key ? (b = Slice(limit_key, limit_key_len), &b) :
				   nullptr));
}
...
void rocksdb_writebatch_deletev_cf(rocksdb_writebatch_t *b,
				   rocksdb_column_family_handle_t *column_family,
				   int num_keys, const char *const *keys_list,
				   const size_t *keys_list_sizes)
{
	...
	b->rep.Delete(column_family->rep,
		      SliceParts(key_slices.data(), num_keys));
}
				      ...
				      const size_t *end_keys_list_sizes)
{
	...
	for (int i = 0; i < num_keys; i++) {
		start_key_slices[i] =
			Slice(start_keys_list[i], start_keys_list_sizes[i]);
		end_key_slices[i] =
			Slice(end_keys_list[i], end_keys_list_sizes[i]);
	}
...
rocksdb_writebatch_wi_create(size_t reserved_bytes, unsigned char overwrite_key)
{
...
void rocksdb_writebatch_wi_iterate(rocksdb_writebatch_wi_t *b, void *state,
				   void (*put)(void *, const char *k,
					       size_t klen, const char *v,
					       size_t vlen),
				   void (*deleted)(void *, const char *k,
						   size_t klen))
{
...
{
	rocksdb_iterator_t *result = new rocksdb_iterator_t;
	result->rep = wbwi->rep->NewIteratorWithBase(column_family->rep,
						     base_iterator->rep);
	delete base_iterator;
	return result;
}
					   ...
					   size_t *vallen, char **errptr)
{
	...
	std::string tmp;
	Status s =
		wbwi->rep->GetFromBatch(options->rep, Slice(key, keylen), &tmp);
	if (s.ok()) {
		*vallen = tmp.size();
		result = CopyString(tmp);
	} else {
	...
	rocksdb_cache_t *block_cache_compressed)
{
	if (block_cache_compressed) {
		options->rep.block_cache_compressed =
			block_cache_compressed->rep;
	}
}
...
void rocksdb_block_based_options_set_index_type(
	rocksdb_block_based_table_options_t *options, int v)
{
	options->rep.index_type =
		static_cast<BlockBasedTableOptions::IndexType>(v);
}
...
void rocksdb_options_compaction_readahead_size(rocksdb_options_t *opt, size_t s)
{
...
void rocksdb_options_set_merge_operator(rocksdb_options_t *opt,
...
void rocksdb_options_set_db_write_buffer_size(rocksdb_options_t *opt, size_t s)
{
	...
	rocksdb_options_t *opt, int *level_values, size_t num_levels)
{
	...
	for (size_t i = 0; i < num_levels; ++i) {
		opt->rep.max_bytes_for_level_multiplier_additional[i] =
			level_values[i];
	}
}
...
void rocksdb_options_set_max_mem_compaction_level(rocksdb_options_t *opt, int n)
{
}
...
void rocksdb_options_set_recycle_log_file_num(rocksdb_options_t *opt, size_t v)
{
...
void rocksdb_options_set_table_cache_numshardbits(rocksdb_options_t *opt, int v)
{
...
void rocksdb_options_set_max_successive_merges(rocksdb_options_t *opt, size_t v)
{
...
void rocksdb_options_set_compaction_style(rocksdb_options_t *opt, int style)
{
	opt->rep.compaction_style =
		static_cast<rocksdb::CompactionStyle>(style);
}
...
rocksdb_filterpolicy_create_bloom_format(int bits_per_key, bool original_format)
{
...
void rocksdb_flushoptions_set_wait(rocksdb_flushoptions_t *opt, unsigned char v)
{
	opt->rep.wait = v;
}
...
{
	SaveError(errptr,
		  writer->rep->Merge(Slice(key, keylen), Slice(val, vallen)));
}
	...
	const rocksdb_ingestexternalfileoptions_t *opt, char **errptr)
{
	...
	SaveError(errptr,
		  db->rep->IngestExternalFile(handle->rep, files, opt->rep));
}
...
rocksdb_slicetransform_create_fixed_prefix(size_t prefixLen)
{
			  ...
			  rocksdb_transaction_t *old_txn)
{
	if (old_txn == nullptr) {
		...
		result->rep = txn_db->rep->BeginTransaction(
			write_options->rep, txn_options->rep, nullptr);
		return result;
	}
	old_txn->rep = txn_db->rep->BeginTransaction(
		write_options->rep, txn_options->rep, old_txn->rep);
	return old_txn;
}
