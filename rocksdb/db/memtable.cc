...
MemTableOptions::MemTableOptions(const ImmutableCFOptions &ioptions,
	...
	: write_buffer_size(mutable_cf_options.write_buffer_size),
	  ...
	  merge_operator(ioptions.merge_operator), info_log(ioptions.info_log)
{
}
...
MemTable::MemTable(const InternalKeyComparator &cmp,
	...
	: comparator_(cmp), moptions_(ioptions, mutable_cf_options), refs_(0),
	  ...
	  range_del_table_(SkipListFactory().CreateMemTableRep(
		  ...
		  ioptions.info_log, column_family_id)),
	  is_range_del_table_empty_(true), data_size_(0), num_entries_(0),
	  num_deletes_(0), flush_in_progress_(false), flush_completed_(false),
	  file_number_(0), first_seqno_(0), earliest_seqno_(latest_seq),
	  creation_seq_(latest_seq), mem_next_logfile_number_(0),
	  min_prep_log_referenced_(0),
	  locks_(moptions_.inplace_update_support ?
			       moptions_.inplace_update_num_locks :
			       0),
	  prefix_extractor_(ioptions.prefix_extractor),
	  flush_state_(FLUSH_NOT_REQUESTED), env_(ioptions.env),
		  ...
		  ioptions.memtable_insert_with_hint_prefix_extractor)
{
	...
	if (prefix_extractor_ && moptions_.memtable_prefix_bloom_bits > 0) {
		prefix_bloom_.reset(new DynamicBloom(
			...
			nullptr, moptions_.memtable_huge_page_size,
			ioptions.info_log));
	}
}
...
class MemTableIterator : public InternalIterator {
	...
	MemTableIterator(const MemTable &mem, const ReadOptions &read_options,
		...
		: bloom_(nullptr), prefix_extractor_(mem.prefix_extractor_),
		  comparator_(mem.comparator_), valid_(false),
...
#ifndef NDEBUG
	virtual void
	SetPinnedItersMgr(PinnedIteratorsManager *pinned_iters_mgr) override
	{
	...
	virtual void Seek(const Slice &k) override
	{
		...
		if (bloom_ != nullptr) {
			if (!bloom_->MayContain(prefix_extractor_->Transform(
				    ExtractUserKey(k)))) {
	...
	virtual void SeekForPrev(const Slice &k) override
	{
		...
		while (Valid() &&
		       comparator_.comparator.Compare(k, key()) < 0) {
			Prev();
		}
	}
	...
	virtual Slice value() const override
	{
		...
		return GetLengthPrefixedSlice(key_slice.data() +
					      key_slice.size());
	}
...
InternalIterator *
MemTable::NewRangeTombstoneIterator(const ReadOptions &read_options)
{
						   ...
						   const Slice &end_ikey)
{
	uint64_t entry_count =
		table_->ApproximateNumEntries(start_ikey, end_ikey);
	entry_count +=
		range_del_table_->ApproximateNumEntries(start_ikey, end_ikey);
	if (entry_count == 0) {
		return { 0, 0 };
	}
		   ...
		   MemTablePostProcessInfo *post_process_info)
{
	...
	if (!allow_concurrent) {
		...
		if (insert_with_hint_prefix_extractor_ != nullptr &&
			...
			Slice prefix =
				insert_with_hint_prefix_extractor_->Transform(
					key_slice);
		...
		num_entries_.store(
			num_entries_.load(std::memory_order_relaxed) + 1,
			std::memory_order_relaxed);
		...
		if (prefix_bloom_) {
			assert(prefix_extractor_);
			prefix_bloom_->AddConcurrently(
				prefix_extractor_->Transform(key));
		}

		// atomically update first_seqno_ and earliest_seqno_.
		uint64_t cur_seq_num =
		...
		while ((cur_earliest_seqno == kMaxSequenceNumber ||
			s < cur_earliest_seqno) &&
		       !first_seqno_.compare_exchange_weak(cur_earliest_seqno,
							   s)) {
		}
	}
		...
		case kTypeRangeDeletion: {
			if (*(s->merge_in_progress)) {
				*(s->status) = MergeHelper::TimedFullMerge(
					...
					nullptr, merge_context->GetOperands(),
					s->value, s->logger, s->statistics,
					s->env_, nullptr /* result_operand */,
					true);
			} else {
...
void MemTable::Update(SequenceNumber seq, const Slice &key, const Slice &value)
{
	...
	if (iter->Valid()) {
		...
		if (comparator_.comparator.user_comparator()->Equal(
			...
			if (type == kTypeValue) {
				Slice prev_value = GetLengthPrefixedSlice(
					key_ptr + key_length);
				...
				if (new_size <= prev_size) {
					char *p = EncodeVarint32(
						const_cast<char *>(key_ptr) +
							key_length,
						new_size);
					WriteLock wl(GetLock(lkey.user_key()));
					memcpy(p, value.data(), value.size());
					assert((unsigned)((p + value.size()) -
							  entry) ==
					       (unsigned)(VarintLength(
								  key_length) +
							  key_length +
							  VarintLength(
								  value.size()) +
	...
	if (iter->Valid()) {
		...
		if (comparator_.comparator.user_comparator()->Equal(
			...
			case kTypeValue: {
				...
				char *prev_buffer =
					const_cast<char *>(prev_value.data());
				uint32_t new_prev_size = prev_size;

				std::string str_value;
				WriteLock wl(GetLock(lkey.user_key()));
				auto status = moptions_.inplace_callback(
					prev_buffer, &new_prev_size, delta,
					&str_value);
				if (status == UpdateStatus::UPDATED_INPLACE) {
					...
					if (new_prev_size < prev_size) {
						...
						if (VarintLength(
							...
							memcpy(p, prev_buffer,
							       new_prev_size);
						}
					}
					RecordTick(moptions_.statistics,
						   NUMBER_KEYS_UPDATED);
					UpdateFlushState();
					return true;
...
size_t MemTable::CountSuccessiveMergeEntries(const LookupKey &key)
{
	...
	for (; iter->Valid(); iter->Next()) {
		...
		const char *iter_key_ptr =
			GetVarint32Ptr(entry, entry + 5, &key_length);
		if (!comparator_.comparator.user_comparator()->Equal(
			    Slice(iter_key_ptr, key_length - 8),
			    key.user_key())) {
			break;
		}

		const uint64_t tag =
			DecodeFixed64(iter_key_ptr + key_length - 8);
		ValueType type;
		uint64_t unused;
		UnPackSequenceAndType(tag, &unused, &type);
		if (type != kTypeMerge) {
