...
class DBImpl : public DB {
	...
	virtual Status CreateColumnFamilies(
		...
		std::vector<ColumnFamilyHandle *> *handles) override;
	virtual Status
	DropColumnFamily(ColumnFamilyHandle *column_family) override;
	...
	virtual bool
	GetAggregatedIntProperty(const Slice &property,
	...
	virtual void
	GetLiveFilesMetaData(std::vector<LiveFileMetaData> *metadata) override;
	...
	InternalIterator *
	NewInternalIterator(Arena *arena, RangeDelAggregator *range_del_agg,
	...
	Status TEST_CompactRange(int level, const Slice *begin,
				 const Slice *end,
				 ColumnFamilyHandle *column_family = nullptr,
				 bool disallow_trivial_move = false);
	...
	Status
	TEST_WaitForFlushMemTable(ColumnFamilyHandle *column_family = nullptr);
	...
	{
		return total_log_size_;
	}

	// Returns column family name to ImmutableCFOptions map.
	Status TEST_GetAllImmutableCFOptions(
		std::unordered_map<std::string, const ImmutableCFOptions *>
			*iopts_map);

	// Return the lastest MutableCFOptions of a column family
	Status
	TEST_GetLatestMutableCFOptions(ColumnFamilyHandle *column_family,
	...
	void ReturnAndCleanupSuperVersion(ColumnFamilyData *cfd,
					  SuperVersion *sv);

	// Similar to the previous function but looks up based on a column family id.
	// nullptr will be returned if this column family no longer exists.
	// REQUIRED: this function should only be called on the write thread.
	void ReturnAndCleanupSuperVersion(uint32_t colun_family_id,
					  SuperVersion *sv);
	...
	ColumnFamilyHandle *
	GetColumnFamilyHandleUnlocked(uint32_t column_family_id);
	...
	struct RecoveredTransaction {
		...
		std::string name_;
		WriteBatch *batch_;
		explicit RecoveredTransaction(const uint64_t log,
	...
	{
		return immutable_db_options_.allow_2pc;
	}
	...
	void InsertRecoveredTransaction(const uint64_t log,
					const std::string &name,
					WriteBatch *batch)
	{
		recovered_transactions_[name] =
			new RecoveredTransaction(log, name, batch);
	...
	InternalIterator *
	NewInternalIterator(const ReadOptions &, ColumnFamilyData *cfd,
	...
	Status PipelinedWriteImpl(const WriteOptions &options,
				  WriteBatch *updates,
				  WriteCallback *callback = nullptr,
	...
	Status
	Recover(const std::vector<ColumnFamilyDescriptor> &column_families,
	...
	void
	ReleaseFileNumberFromPendingOutputs(std::list<uint64_t>::iterator v);
	...
	Status
	FlushMemTableToOutputFile(ColumnFamilyData *cfd,
				  const MutableCFOptions &mutable_cf_options,
	...
	Status
	CompactFilesImpl(const CompactionOptions &compact_options,
	...
	void SchedulePendingPurge(std::string fname, FileType type,
				  uint64_t number, uint32_t path_id,
				  int job_id);
	...
	int
	FindMinimumEmptyLevelFitting(ColumnFamilyData *cfd,
				     const MutableCFOptions &mutable_cf_options,
				     int level);
	...
	// hold the data set.
	Status ReFitLevel(ColumnFamilyData *cfd, int level,
			  int target_level = -1);
	...
	struct PurgeFileInfo {
		...
		uint32_t path_id;
		int job_id;
		PurgeFileInfo(std::string fn, FileType t, uint64_t num,
			      uint32_t pid, int jid)
			: fname(fn), type(t), number(num), path_id(pid),
			  job_id(jid)
		{
		}
	};
	...
	struct ManualCompaction {
		...
		InternalKey
			tmp_storage1; // Used to keep track of compaction progress
	...
	Status GetImpl(const ReadOptions &options,
		       ColumnFamilyHandle *column_family, const Slice &key,
		       PinnableSlice *value, bool *value_found = nullptr);
...
GetCompressionFlush(const ImmutableCFOptions &ioptions,
		    const MutableCFOptions &mutable_cf_options);
