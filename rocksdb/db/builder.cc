...
TableBuilder *
NewTableBuilder(const ImmutableCFOptions &ioptions,
...
BuildTable(const std::string &dbname, Env *env,
	   ...
	   TableFileCreationReason reason, EventLogger *event_logger,
	   ...
	   TableProperties *table_properties, int level)
{
	...
	std::string fname = TableFileName(
		ioptions.db_paths, meta->fd.GetNumber(), meta->fd.GetPathId());
#ifndef ROCKSDB_LITE
	EventHelpers::NotifyTableFileCreationStarted(ioptions.listeners, dbname,
						     column_family_name, fname,
						     job_id, reason);
#endif // !ROCKSDB_LITE
	TableProperties tp;
	...
	if (iter->Valid() || range_del_agg->ShouldAddTombstones()) {
		...
		unique_ptr<WritableFileWriter> file_writer;
		{
			...
			TEST_SYNC_POINT_CALLBACK("BuildTable:create_file",
						 &use_direct_writes);
#endif // !NDEBUG
			...
			if (!s.ok()) {
				EventHelpers::
					LogAndNotifyTableFileCreationFinished(
						event_logger,
						ioptions.listeners, dbname,
						column_family_name, fname,
						job_id, meta->fd, tp, reason,
						s);
				return s;
			}
			...
			builder = NewTableBuilder(
				...
				file_writer.get(), compression,
				compression_opts, level);
		}

		MergeHelper merge(env, internal_comparator.user_comparator(),
				  ioptions.merge_operator, nullptr,
				  ioptions.info_log,
				  true /* internal key corruption is not ok */,
				  snapshots.empty() ? 0 : snapshots.back());

		CompactionIterator c_iter(
			iter, internal_comparator.user_comparator(), &merge,
		...
		if (s.ok() && !empty) {
			StopWatch sw(env, ioptions.statistics,
				     TABLE_SYNC_MICROS);
	...
	EventHelpers::LogAndNotifyTableFileCreationFinished(
		...
		fname, job_id, meta->fd, tp, reason, s);
