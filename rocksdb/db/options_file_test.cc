	...
	DB *db, const std::unordered_set<std::string> &past_filenames)
{
	...
	for (auto past_filename : past_filenames) {
		if (current_filenames.find(past_filename) !=
		    current_filenames.end()) {
			continue;
		}
...
TEST_F(OptionsFileTest, OptionsFileName)
{
	...
	auto temp_options_file_name =
		TempOptionsFileName("", kTempOptionsFileNum);
	ASSERT_TRUE(
		ParseFileName(temp_options_file_name, &number, &type, nullptr));
