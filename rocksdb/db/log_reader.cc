...
Reader::Reader(std::shared_ptr<Logger> info_log,
	...
	: info_log_(info_log), file_(std::move(_file)), reporter_(reporter),
	  checksum_(checksum), backing_store_(new char[kBlockSize]), buffer_(),
	  eof_(false), read_error_(false), eof_offset_(0),
	  last_record_offset_(0), end_of_buffer_offset_(0),
	  initial_offset_(initial_offset), log_number_(log_num),
	  recycled_(false)
{
}
...
bool Reader::SkipToInitialBlock()
{
	...
	uint64_t block_start_location =
		initial_offset_ - initial_offset_in_block;
	...
	if (block_start_location > 0) {
		...
		if (!skip_status.ok()) {
			ReportDrop(static_cast<size_t>(block_start_location),
				   skip_status);
			return false;
		}
	}
			...
			WALRecoveryMode wal_recovery_mode)
{
	...
	while (true) {
		...
		const unsigned int record_type =
			ReadPhysicalRecord(&fragment, &drop_size);
		switch (record_type) {
		case kFullType:
...
void Reader::UnmarkEOF()
{
	...
	if (buffer_.data() != backing_store_ + consumed_bytes) {
		...
		memmove(backing_store_ + consumed_bytes, buffer_.data(),
			buffer_.size());
	}
...
unsigned int Reader::ReadPhysicalRecord(Slice *result, size_t *drop_size)
{
	while (true) {
		...
		if (checksum_) {
			...
			uint32_t actual_crc = crc32c::Value(
				header + 6, length + header_size - 6);
		...
		if (end_of_buffer_offset_ - buffer_.size() - header_size -
			    length <
		    initial_offset_) {
			result->clear();
			return kBadRecord;
