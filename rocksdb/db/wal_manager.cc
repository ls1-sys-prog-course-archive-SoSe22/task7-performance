	...
	VersionSet *version_set)
{
	...
	iter->reset(new TransactionLogIteratorImpl(
		db_options_.wal_dir, &db_options_, read_options, env_options_,
		seq, std::move(wal_files), version_set));
	return (*iter)->status();
}
	...
	for (auto &f : files) {
		...
		if (ParseFileName(f, &number, &type) && type == kLogFile) {
			...
			if (ttl_enabled) {
				...
				if (now_seconds - file_m_time >
				    db_options_.wal_ttl_seconds) {
					...
					if (!s.ok()) {
						...
						MutexLock l(
							&read_first_record_cache_mutex_);
						read_first_record_cache_.erase(
							number);
					}
					continue;
				}
				       ...
				       WalFileType log_type)
{
	...
	for (const auto &f : all_files) {
		...
		if (ParseFileName(f, &number, &type) && type == kLogFile) {
			...
			uint64_t size_bytes;
			s = env_->GetFileSize(LogFileName(path, number),
					      &size_bytes);
			// re-try in case the alive log file has been moved to archive.
			std::string archived_file =
				ArchivedLogFileName(path, number);
			if (!s.ok() && log_type == kAliveLogFile &&
			    env_->FileExists(archived_file).ok()) {
				s = env_->GetFileSize(archived_file,
						      &size_bytes);
...
{
	int64_t start =
		0; // signed to avoid overflow when target is < first file.
				 ...
				 SequenceNumber *sequence)
{
	...
	log::Reader reader(db_options_.info_log, std::move(file_reader),
