...
TEST_F(FormatTest, InternalKey_EncodeDecode)
{
	...
	for (unsigned int k = 0; k < sizeof(keys) / sizeof(keys[0]); k++) {
		for (unsigned int s = 0; s < sizeof(seq) / sizeof(seq[0]);
		     s++) {
...
TEST_F(FormatTest, InternalKeyShortSeparator)
{
	...
	ASSERT_EQ(IKey("AAA1B", kMaxSequenceNumber, kValueTypeForSeek),
		  Shorten(IKey("AAA1AAA", 100, kTypeValue),
	...
	ASSERT_EQ(IKey("AAA1", 100, kTypeValue),
		  Shorten(IKey("AAA1", 100, kTypeValue),
			  IKey("AAA2", 200, kTypeValue)));
