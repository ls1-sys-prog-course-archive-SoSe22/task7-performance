			      ...
			      std::vector<FileMetaData *> *const files)
{
	...
	for (size_t level = 1; level < num_levels_ - 1; ++level) {
		...
		const int32_t upper_size =
			static_cast<int32_t>(upper_files.size());
		...
		CalculateLB(
			...
			[this](const FileMetaData *a,
			       const FileMetaData *b) -> int {
				return ucmp_->Compare(a->smallest.user_key(),
						      b->largest.user_key());
			},
		...
		CalculateLB(
			...
			[this](const FileMetaData *a,
			       const FileMetaData *b) -> int {
				return ucmp_->Compare(a->largest.user_key(),
						      b->largest.user_key());
			},
			[](IndexUnit *index, int32_t f_idx) {
	...
	std::function<void(IndexUnit *, int32_t)> set_index)
{
	...
	while (upper_idx < upper_size && lower_idx < lower_size) {
		int cmp =
			cmp_op(upper_files[upper_idx], lower_files[lower_idx]);
