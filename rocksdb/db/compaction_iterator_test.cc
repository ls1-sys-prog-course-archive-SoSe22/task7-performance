...
class LoggingForwardVectorIterator : public InternalIterator {
	...
	struct Action {
		...
		{
			return std::tie(type, arg) ==
			       std::tie(rhs.type, rhs.arg);
		}
	};
	...
	virtual void Seek(const Slice &target) override
	{
		...
		current_ = std::lower_bound(keys_.begin(), keys_.end(),
					    target.ToString()) -
			   keys_.begin();
	}
...
class FakeCompaction : public CompactionIterator::CompactionProxy {
	...
	virtual int level(size_t compaction_input_level) const
	{
		return 0;
	}
	virtual bool
	KeyNotExistsBeyondOutputLevel(const Slice &user_key,
	...
	virtual int number_levels() const
	{
		return 1;
	}
...
class CompactionIteratorTest : public testing::Test {
			   ...
			   CompactionFilter *filter = nullptr)
	{
		...
		ASSERT_OK(range_del_agg_->AddTombstones(
		...
		merge_helper_.reset(new MergeHelper(
			Env::Default(), cmp_, merge_op, filter, nullptr, false,
			0, 0, nullptr, &shutting_down_));
		iter_.reset(new LoggingForwardVectorIterator(ks, vs));
		iter_->SeekToFirst();
		c_iter_.reset(new CompactionIterator(
			iter_.get(), cmp_, merge_helper_.get(), last_sequence,
			&snapshots_, kMaxSequenceNumber, Env::Default(), false,
			range_del_agg_.get(), std::move(compaction), filter,
...
TEST_F(CompactionIteratorTest, CorruptionAfterSingleDeletion)
{
	...
	c_iter_->Next();
	ASSERT_TRUE(c_iter_->Valid());
	ASSERT_EQ(test::KeyStr("a", 3, kTypeValue, true),
		  c_iter_->key().ToString());
	c_iter_->Next();
	ASSERT_TRUE(c_iter_->Valid());
...
TEST_F(CompactionIteratorTest, SimpleRangeDeletion)
{
	InitIterators({ test::KeyStr("morning", 5, kTypeValue),
			...
			test::KeyStr("night", 3, kTypeValue) },
		      { "zao", "zao", "wan" },
		      { test::KeyStr("ma", 4, kTypeRangeDeletion) }, { "mz" },
		      5);
	c_iter_->SeekToFirst();
	ASSERT_TRUE(c_iter_->Valid());
	ASSERT_EQ(test::KeyStr("morning", 5, kTypeValue),
		  c_iter_->key().ToString());
	c_iter_->Next();
	ASSERT_TRUE(c_iter_->Valid());
	ASSERT_EQ(test::KeyStr("night", 3, kTypeValue),
		  c_iter_->key().ToString());
	c_iter_->Next();
	ASSERT_FALSE(c_iter_->Valid());
}
...
TEST_F(CompactionIteratorTest, RangeDeletionWithSnapshots)
{
	...
	c_iter_->Next();
	ASSERT_TRUE(c_iter_->Valid());
	ASSERT_EQ(test::KeyStr("night", 40, kTypeValue),
		  c_iter_->key().ToString());
	c_iter_->Next();
	ASSERT_FALSE(c_iter_->Valid());
}
...
TEST_F(CompactionIteratorTest, CompactionFilterSkipUntil)
{
	class Filter : public CompactionFilter {
		virtual Decision
		FilterV2(int level, const Slice &key, ValueType t,
	...
	InitIterators(
		...
		{ "av50", "am45", "bv60", "bv40", "cv35", "dm70", "em71",
		  "fm65", "fm30", "fv25", "gv90", "hv91", "im95", "jv99" },
	...
	std::vector<A> expected_actions = {
		...
		A(T::NEXT),
		A(T::SEEK,
		  test::KeyStr("d+", kMaxSequenceNumber, kValueTypeForSeek)),
		A(T::NEXT),
		A(T::NEXT),
		A(T::SEEK,
		  test::KeyStr("g+", kMaxSequenceNumber, kValueTypeForSeek)),
		A(T::NEXT),
		A(T::SEEK,
		  test::KeyStr("z", kMaxSequenceNumber, kValueTypeForSeek))
	};
...
TEST_F(CompactionIteratorTest, ShuttingDownInFilter)
{
	...
	InitIterators({ test::KeyStr("1", 1, kTypeValue),
			test::KeyStr("2", 2, kTypeValue),
			test::KeyStr("3", 3, kTypeValue),
			test::KeyStr("4", 4, kTypeValue) },
		      { "v1", "v2", "v3", "v4" }, {}, {}, kMaxSequenceNumber,
		      &merge_op, &filter);
