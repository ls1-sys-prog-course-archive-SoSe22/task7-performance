...
class VersionBuilder::Rep {
    ...
    public:
	Rep(const EnvOptions &env_options, Logger *info_log,
	    TableCache *table_cache, VersionStorageInfo *base_vstorage)
		: env_options_(env_options), info_log_(info_log),
		  table_cache_(table_cache), base_vstorage_(base_vstorage)
	{
		...
		for (int level = 0; level < vstorage->num_levels(); level++) {
			...
			for (size_t i = 1; i < level_files.size(); i++) {
				...
				if (level == 0) {
					...
					if (f2->smallest_seqno ==
					    f2->largest_seqno) {
						// This is an external file that we ingested
						SequenceNumber external_file_seqno =
							f2->smallest_seqno;
						if (!(external_file_seqno <
							      f1->largest_seqno ||
						      external_file_seqno ==
							      0)) {
							fprintf(stderr,
					...
					if (!level_nonzero_cmp_(f1, f2)) {
						fprintf(stderr,
							"L%d files are not sorted properly",
							level);
						abort();
					}

					// Make sure there is no overlap in levels > 0
					if (vstorage->InternalComparator()
						    ->Compare(f1->largest,
							      f2->smallest) >=
					    0) {
						fprintf(stderr,
					...
					int level)
	{
		...
		bool found = false;
		for (int l = 0; !found && l < base_vstorage_->num_levels();
		     l++) {
		...
		// version
		for (int l = level + 1;
		     !found && l < base_vstorage_->num_levels(); l++) {
	...
	void Apply(VersionEdit *edit)
	{
		...
		// Delete files
		const VersionEdit::DeletedFileSet &del =
			edit->GetDeletedFiles();
		...
		for (const auto &new_file : edit->GetNewFiles()) {
			...
			assert(levels_[level].added_files.find(
				       f->fd.GetNumber()) ==
	...
	void SaveTo(VersionStorageInfo *vstorage)
	{
		...
		for (int level = 0; level < base_vstorage_->num_levels();
		     level++) {
			const auto &cmp = (level == 0) ? level_zero_cmp_ :
							       level_nonzero_cmp_;
			// Merge the set of added files with the set of pre-existing files.
			// Drop any deleted files.  Store the result in *v.
			const auto &base_files =
				base_vstorage_->LevelFiles(level);
			auto base_iter = base_files.begin();
			auto base_end = base_files.end();
			const auto &unordered_added_files =
			...
			for (const auto &added : added_files) {
				...
				for (auto bpos = std::upper_bound(
					     base_iter, base_end, added, cmp);
				     base_iter != bpos; ++base_iter) {
					MaybeAddFile(vstorage, level,
						     *base_iter);
				}
			       ...
			       bool prefetch_index_and_filter_in_cache)
	{
		...
		std::function<void()> load_handlers_func = [&]() {
			while (true) {
				size_t file_idx =
					next_file_meta_idx.fetch_add(1);
				...
				if (file_meta->table_reader_handle != nullptr) {
					...
					file_meta->fd.table_reader =
						table_cache_->GetTableReaderFromHandle(
							file_meta
								->table_reader_handle);
				}
			}
