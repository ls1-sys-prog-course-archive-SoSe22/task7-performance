...
class FaultInjectionTest : public testing::Test,
	...
	FaultInjectionTest()
		: option_config_(kDefault), sync_use_wal_(false),
		  sync_use_compact_(true), base_env_(nullptr), env_(NULL),
		  db_(NULL)
	{
	}
	...
	Options CurrentOptions()
	{
		...
		case kWalDir:
			options.wal_dir =
				test::TmpDir(env_) + "/fault_test_wal";
			break;
		case kDifferentDataDir:
			options.db_paths.emplace_back(
				test::TmpDir(env_) + "/fault_test_data",
				1000000U);
			break;
		case kSyncWal:
			sync_use_wal_ = true;
	...
	Status NewDB()
	{
		...
		env_ = new FaultInjectionTestEnv(base_env_ ? base_env_.get() :
								   Env::Default());
	...
	void Build(const WriteOptions &write_options, int start_idx,
		   int num_vals)
	{
		      ...
		      ExpectedVerifResult expected) const
	{
		...
		Status s;
		for (int i = start_idx; i < start_idx + num_vals && s.ok();
		     i++) {
			Value(i, &value_space);
			s = ReadValue(i, &val);
			if (s.ok()) {
	...
	// Return the ith key
	Slice
	Key(int i, std::string *storage) const
	{
		int num = i;
	...
	do {
		...
		for (size_t idx = 0; idx < kNumIterations; idx++) {
			...
			PartialCompactTestReopenWithFault(
				kResetDropUnsyncedData, num_pre_sync,
				num_post_sync);
			NoWriteTestPreFault();
			...
			PartialCompactTestPreFault(num_pre_sync, num_post_sync);
			PartialCompactTestReopenWithFault(
				kResetDropRandomUnsyncedData, num_pre_sync,
				num_post_sync, &rnd);
			NoWriteTestPreFault();
			NoWriteTestReopenWithFault(kResetDropUnsyncedData);
			...
			PartialCompactTestPreFault(num_pre_sync, num_post_sync);
			PartialCompactTestReopenWithFault(
				kResetDropAndDeleteUnsynced, num_pre_sync,
				num_post_sync);
			NoWriteTestPreFault();
			...
			// dropped.
			PartialCompactTestReopenWithFault(
				kResetDeleteUnsyncedFiles, num_pre_sync,
				num_post_sync);
			NoWriteTestPreFault();
...
TEST_P(FaultInjectionTest, WriteOptionSyncTest)
{
	...
	env_->Schedule(&test::SleepingBackgroundTask::DoSleepTask,
		       &sleeping_task_low, Env::Priority::HIGH);
	...
	std::string key_space, value_space;
	ASSERT_OK(db_->Put(write_options, Key(1, &key_space),
			   Value(1, &value_space)));
	FlushOptions flush_options;
	flush_options.wait = false;
	ASSERT_OK(db_->Flush(flush_options));
	write_options.sync = true;
	ASSERT_OK(db_->Put(write_options, Key(2, &key_space),
			   Value(2, &value_space)));
...
TEST_P(FaultInjectionTest, UninstalledCompaction)
{
	...
	if (!sequential_order_) {
		rocksdb::SyncPoint::GetInstance()->LoadDependency({
			{ "FaultInjectionTest::FaultTest:0",
			  "DBImpl::BGWorkCompaction" },
			{ "CompactionJob::Run():End",
			  "FaultInjectionTest::FaultTest:1" },
