...
ColumnFamilyHandleImpl::ColumnFamilyHandleImpl(
	ColumnFamilyData *column_family_data, DBImpl *db,
...
{
	auto &collector_factories =
		ioptions.table_properties_collector_factories;
	for (size_t i = 0;
	     i < ioptions.table_properties_collector_factories.size(); ++i) {
...
Status CheckCompressionSupported(const ColumnFamilyOptions &cf_options)
{
	if (!cf_options.compression_per_level.empty()) {
		for (size_t level = 0;
		     level < cf_options.compression_per_level.size(); ++level) {
			if (!CompressionTypeSupported(
				    ...
				    const ColumnFamilyOptions &src)
{
	...
	if (!result.prefix_extractor) {
		...
		if (name.compare("HashSkipListRepFactory") == 0 ||
			...
			result.memtable_factory =
				std::make_shared<SkipListFactory>();
		}
	}
...
void SuperVersion::Cleanup()
{
	...
	if (m != nullptr) {
		auto *memory_usage =
			current->cfd()->imm()->current_memory_usage();
...
ColumnFamilyData::ColumnFamilyData(uint32_t id, const std::string &name,
	...
	: id_(id), name_(name), dummy_versions_(_dummy_versions),
	  current_(nullptr), refs_(0), initialized_(false), dropped_(false),
	  ...
	  is_delete_range_supported_(strcmp(cf_options.table_factory->Name(),
				     ...
				     0),
	  write_buffer_manager_(write_buffer_manager), mem_(nullptr),
	  imm_(ioptions_.min_write_buffer_number_to_merge,
	       ioptions_.max_write_buffer_number_to_maintain),
	  super_version_(nullptr), super_version_number_(0),
	  local_sv_(new ThreadLocalPtr(&SuperVersionUnrefHandle)),
	  next_(nullptr), prev_(nullptr), log_number_(0),
	  column_family_set_(column_family_set), pending_flush_(false),
	  pending_compaction_(false), prev_compaction_needed_bytes_(0),
	  allow_2pc_(db_options.allow_2pc)
{
	...
	if (_dummy_versions != nullptr) {
		internal_stats_.reset(new InternalStats(ioptions_.num_levels,
							db_options.env, this));
		table_cache_.reset(
			new TableCache(ioptions_, env_options, _table_cache));
		if (ioptions_.compaction_style == kCompactionStyleLevel) {
			compaction_picker_.reset(new LevelCompactionPicker(
				ioptions_, &internal_comparator_));
#ifndef ROCKSDB_LITE
		} else if (ioptions_.compaction_style ==
...
{
	return BuildColumnFamilyOptions(initial_cf_options_,
					mutable_cf_options_);
}
...
SetupDelay(WriteController *write_controller, uint64_t compaction_needed_bytes,
	   ...
	   bool auto_comapctions_disabled)
{
	...
	if (auto_comapctions_disabled) {
		...
		} else if (prev_compaction_need_bytes >
			   compaction_needed_bytes) {
				    ...
				    int level0_slowdown_writes_trigger)
{
	...
	assert(level0_file_num_compaction_trigger <=
	...
	int64_t res =
		std::min(twice_level0_trigger, one_fourth_trigger_slowdown);
	...
	const MutableCFOptions &mutable_cf_options)
{
	if (current_ != nullptr) {
		...
		if (imm()->NumNotFlushed() >=
		    mutable_cf_options.max_write_buffer_number) {
			write_controller_token_ =
				write_controller->GetStopToken();
			internal_stats_->AddCFStats(
				InternalStats::MEMTABLE_COMPACTION, 1);
			ROCKS_LOG_WARN(
				ioptions_.info_log,
			...
			write_controller_token_ =
				write_controller->GetStopToken();
			internal_stats_->AddCFStats(
				InternalStats::LEVEL0_NUM_FILES_TOTAL, 1);
			...
			write_controller_token_ =
				write_controller->GetStopToken();
			internal_stats_->AddCFStats(
				InternalStats::HARD_PENDING_COMPACTION_BYTES_LIMIT,
				1);
			ROCKS_LOG_WARN(
				ioptions_.info_log,
			   ...
			   imm()->NumNotFlushed() >=
				   mutable_cf_options.max_write_buffer_number -
					   1) {
			write_controller_token_ = SetupDelay(
				write_controller, compaction_needed_bytes,
				prev_compaction_needed_bytes_, was_stopped,
				mutable_cf_options.disable_auto_compactions);
			internal_stats_->AddCFStats(
				InternalStats::MEMTABLE_SLOWDOWN, 1);
			ROCKS_LOG_WARN(
				ioptions_.info_log,
			...
			write_controller_token_ = SetupDelay(
				...
				was_stopped || near_stop,
				mutable_cf_options.disable_auto_compactions);
			internal_stats_->AddCFStats(
				InternalStats::LEVEL0_SLOWDOWN_TOTAL, 1);
			if (compaction_picker_->IsLevel0CompactionInProgress()) {
				internal_stats_->AddCFStats(
					InternalStats::
			...
			write_controller_token_ = SetupDelay(
				...
				mutable_cf_options.disable_auto_compactions);
			internal_stats_->AddCFStats(
				InternalStats::SOFT_PENDING_COMPACTION_BYTES_LIMIT,
				1);
			ROCKS_LOG_WARN(
				ioptions_.info_log,
			...
			if (needed_delay) {
				...
				// pressure.
				write_controller->low_pri_rate_limiter()
					->SetBytesPerSecond(write_rate / 4);
			}
		}
...
Compaction *
ColumnFamilyData::PickCompaction(const MutableCFOptions &mutable_options,
				 LogBuffer *log_buffer)
{
	auto *result =
		compaction_picker_->PickCompaction(GetName(), mutable_options,
...
bool ColumnFamilyData::RangeOverlapWithCompaction(const Slice &smallest_user_key,
	...
	const InternalKey *end, InternalKey **compaction_end, bool *conflict)
{
	auto *result = compaction_picker_->CompactRange(
		GetName(), mutable_cf_options, current_->storage_info(),
		input_level, output_level, output_path_id, begin, end,
...
SuperVersion *
ColumnFamilyData::GetReferencedSuperVersion(InstrumentedMutex *db_mutex)
{
...
SuperVersion *
ColumnFamilyData::GetThreadLocalSuperVersion(InstrumentedMutex *db_mutex)
{
	SuperVersion *sv = nullptr;
...
SuperVersion *
ColumnFamilyData::InstallSuperVersion(SuperVersion *new_superversion,
				      InstrumentedMutex *db_mutex)
{
	db_mutex->AssertHeld();
	return InstallSuperVersion(new_superversion, db_mutex,
				   mutable_cf_options_);
}
	...
	const std::unordered_map<std::string, std::string> &options_map)
{
	...
	Status s = GetMutableOptionsFromStrings(
		mutable_cf_options_, options_map, &new_mutable_cf_options);
	if (s.ok()) {
...
ColumnFamilySet::ColumnFamilySet(const std::string &dbname,
	...
	: max_column_family_(0),
	  dummy_cfd_(new ColumnFamilyData(0, "", nullptr, nullptr, nullptr,
					  ...
					  env_options, nullptr)),
	  default_cfd_cache_(nullptr), db_name_(dbname),
...
ColumnFamilyData *
ColumnFamilySet::GetColumnFamily(const std::string &name) const
{
...
{
	max_column_family_ =
		std::max(new_max_column_family, max_column_family_);
}
...
ColumnFamilyData *
ColumnFamilySet::CreateColumnFamily(const std::string &name, uint32_t id,
...
bool ColumnFamilyMemTablesImpl::Seek(uint32_t column_family_id)
{
	if (column_family_id == 0) {
		...
		current_ =
			column_family_set_->GetColumnFamily(column_family_id);
	}
...
const Comparator *
GetColumnFamilyUserComparator(ColumnFamilyHandle *column_family)
{
	if (column_family != nullptr) {
