...
class CountMergeOperator : public AssociativeMergeOperator {
			   ...
			   Logger *logger) const override
	{
		...
		return mergeOperator_->PartialMerge(key, *existing_value, value,
						    new_value, logger);
	}
				       ...
				       Logger *logger) const override
	{
		...
		return mergeOperator_->PartialMergeMulti(key, operand_list,
							 new_value, logger);
	}
...
class Counters {
	...
	explicit Counters(std::shared_ptr<DB> db, uint64_t defaultCount = 0)
		: db_(db), put_option_(), get_option_(), delete_option_(),
	...
	void assert_remove(const std::string &key)
	{
		assert(remove(key));
	}
...
class MergeBasedCounters : public Counters {
    ...
    public:
	explicit MergeBasedCounters(std::shared_ptr<DB> db,
				    ...
				    size_t num_merges)
{
	...
	ASSERT_EQ(num_merge_operator_calls,
		  static_cast<size_t>(num_merges -
				      (num_merges % (max_num_merges + 1))));
...
void runTest(int argc, const std::string &dbname, const bool use_ttl = false)
{
...

	{
		...
		for (uint32_t count = min_merge - 1; count <= min_merge + 1;
		     count++) {
			auto db = OpenDb(dbname, use_ttl, max_merge);
			MergeBasedCounters counters(db, 0);
			testPartialMerge(&counters, db.get(), max_merge,
					 min_merge, count);
			DestroyDB(dbname, Options());
		}
		{
...

	{
		std::cout << "Test merge-operator not set after reopen\n";
		{
			...
			counters.add("test-key", 1);
			counters.add("test-key", 1);
			db->CompactRange(CompactRangeOptions(), nullptr,
					 nullptr);
		}
		...
		std::string value;
		ASSERT_TRUE(!(
			reopen_db->Get(ReadOptions(), "test-key", &value).ok()));
		delete reopen_db;
		DestroyDB(dbname, Options());
	}
...
// DBWithTTL is not supported in ROCKSDB_LITE
#ifndef ROCKSDB_LITE
	runTest(argc, test::TmpDir() + "/merge_testdbttl",
