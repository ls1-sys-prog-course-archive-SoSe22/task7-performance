...
{
	int written_size = snprintf(buf, len, "\n** Compaction Stats [%s] **\n",
				    cf_name.c_str());
	auto hdr = [](LevelStatType t) {
		return InternalStats::compaction_level_stats.at(t)
			.header_name.c_str();
	};
...
{
	uint64_t bytes_read = stats.bytes_read_non_output_levels +
	...
	(*level_stats)[LevelStatType::RNP1_GB] =
		stats.bytes_read_output_level / kGB;
		     ...
		     const std::map<LevelStatType, double> &stat_value)
{
	snprintf(
		...
		NumberToHumanString(static_cast<std::int64_t>(stat_value.at(
		     ...
		     const InternalStats::CompactionStats &stats)
{
	...
	PrepareLevelStats(&level_stats, num_files, being_compacted,
			  total_file_size, score, w_amp, stats);
...
const std::unordered_map<std::string, DBPropertyInfo>
	InternalStats::ppt_name_to_info = {
		...
		{ DB::Properties::kCompressionRatioAtLevelPrefix,
		  { false, &InternalStats::HandleCompressionRatioAtLevelPrefix,
		    nullptr, nullptr } },
		...
		{ DB::Properties::kCFStatsNoFileHistogram,
		  { false, &InternalStats::HandleCFStatsNoFileHistogram,
		    nullptr, nullptr } },
		...
		{ DB::Properties::kAggregatedTableProperties,
		  { false, &InternalStats::HandleAggregatedTableProperties,
		    nullptr, nullptr } },
		...
		{ DB::Properties::kNumImmutableMemTableFlushed,
		  { false, nullptr,
		    &InternalStats::HandleNumImmutableMemTableFlushed,
		    nullptr } },
		...
		{ DB::Properties::kNumEntriesActiveMemTable,
		  { false, nullptr,
		    &InternalStats::HandleNumEntriesActiveMemTable, nullptr } },
		{ DB::Properties::kNumEntriesImmMemTables,
		  { false, nullptr,
		    &InternalStats::HandleNumEntriesImmMemTables, nullptr } },
		{ DB::Properties::kNumDeletesActiveMemTable,
		  { false, nullptr,
		    &InternalStats::HandleNumDeletesActiveMemTable, nullptr } },
		{ DB::Properties::kNumDeletesImmMemTables,
		  { false, nullptr,
		    &InternalStats::HandleNumDeletesImmMemTables, nullptr } },
		{ DB::Properties::kEstimateNumKeys,
		  { false, nullptr, &InternalStats::HandleEstimateNumKeys,
		    nullptr } },
		{ DB::Properties::kEstimateTableReadersMem,
		  { true, nullptr,
		    &InternalStats::HandleEstimateTableReadersMem, nullptr } },
		{ DB::Properties::kIsFileDeletionsEnabled,
		  { false, nullptr,
		    &InternalStats::HandleIsFileDeletionsEnabled, nullptr } },
		...
		{ DB::Properties::kCurrentSuperVersionNumber,
		  { false, nullptr,
		    &InternalStats::HandleCurrentSuperVersionNumber,
		    nullptr } },
		...
		{ DB::Properties::kActualDelayedWriteRate,
		  { false, nullptr,
		    &InternalStats::HandleActualDelayedWriteRate, nullptr } },
		{ DB::Properties::kIsWriteStopped,
		  { false, nullptr, &InternalStats::HandleIsWriteStopped,
		    nullptr } },
	};
							...
							Slice suffix)
{
	...
	*value = ToString(vstorage->GetEstimatedCompressionRatioAtLevel(
...
void InternalStats::DumpDBStats(std::string *value)
{
	...
	double seconds_up =
		(env_->NowMicros() - started_at_ + 1) / kMicrosInSec;
	...
	snprintf(buf, sizeof(buf),
		 ...
		 (write_other + write_self) /
			 static_cast<double>(write_self + 1),
	...
	AppendHumanMicros(write_stall_micros, human_micros, kHumanMicrosLen,
			  true);
	snprintf(
		buf, sizeof(buf), "Cumulative stall: %s, %.1f percent\n",
		human_micros,
	...
	// Interval
	uint64_t interval_write_other =
		write_other - db_stats_snapshot_.write_other;
	uint64_t interval_write_self =
		write_self - db_stats_snapshot_.write_self;
	...
	snprintf(buf, sizeof(buf),
		 ...
		 static_cast<double>(interval_write_other +
				     interval_write_self) /
	...
	uint64_t interval_wal_synced =
		wal_synced - db_stats_snapshot_.wal_synced;
	...
	// Stall
	AppendHumanMicros(write_stall_micros -
				  db_stats_snapshot_.write_stall_micros,
			  human_micros, kHumanMicrosLen, true);
	snprintf(
		buf, sizeof(buf), "Interval stall: %s, %.1f percent\n",
		human_micros,
		// 10000 = divide by 1M to get secs, then multiply by 100 for pct
...
void InternalStats::DumpCFMapStats(std::map<std::string, double> *cf_stats)
{
	...
	for (auto const &level_ent : levels_stats) {
		auto level_str = level_ent.first == -1 ?
					       "Sum" :
					       "L" + ToString(level_ent.first);
		for (auto const &stat_ent : level_ent.second) {
			auto stat_type = stat_ent.first;
			auto key_str = level_str + "." +
...
void InternalStats::DumpCFStatsNoFileHistogram(std::string *value)
{
	...
	for (int l = 0; l < number_levels_; ++l) {
		if (levels_stats.find(l) != levels_stats.end()) {
			PrintLevelStats(buf, sizeof(buf), "L" + ToString(l),
					levels_stats[l]);
			value->append(buf);
		}
	}
	...
	uint64_t flush_ingest = cf_stats_value_[BYTES_FLUSHED];
	uint64_t add_file_ingest = cf_stats_value_[BYTES_INGESTED_ADD_FILE];
	uint64_t ingest_files_addfile =
		cf_stats_value_[INGESTED_NUM_FILES_TOTAL];
	...
	double w_amp = interval_stats.bytes_written /
		       static_cast<double>(interval_ingest);
	PrintLevelStats(buf, sizeof(buf), "Int", 0, 0, 0, 0, w_amp,
			interval_stats);
	value->append(buf);
	...
	snprintf(buf, sizeof(buf),
		 "AddFile(Total Files): cumulative %" PRIu64
		 ", interval %" PRIu64 "\n",
	...
	uint64_t interval_ingest_l0_files_addfile =
		...
		cf_stats_snapshot_.ingest_l0_files_addfile;
	snprintf(buf, sizeof(buf),
