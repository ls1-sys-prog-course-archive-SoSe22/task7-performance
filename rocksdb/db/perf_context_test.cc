...
TEST_F(PerfContextTest, SeekIntoDeletion)
{
...

	{
		...
		iter->SeekToFirst();
		hist_seek_to_first.Add(
			get_perf_context()->user_key_comparison_count);
		auto elapsed_nanos = timer.ElapsedNanos();

		if (FLAGS_verbose) {
			std::cout
				<< "SeekToFirst uesr key comparison: \n"
...
void ProfileQueries(bool enabled_time = false)
{
	...
	if (FLAGS_verbose) {
		std::cout << "Inserting " << FLAGS_total_keys
			  << " key/value pairs\n...\n";
	}
	...
	for (const int i : keys) {
		...
		if (++num_mutex_waited > 3) {
			...
			ThreadStatusUtil::TEST_SetStateDelay(
				ThreadStatus::STATE_MUTEX_WAIT, 0U);
#endif
		}
		...
		hist_write_memtable_time.Add(
			get_perf_context()->write_memtable_time);
	...
	for (const int i : keys) {
		...
		hist_get_memtable.Add(
			get_perf_context()->get_from_memtable_time);
		hist_get_files.Add(
			get_perf_context()->get_from_output_files_time);
		hist_num_memtable_checked.Add(
			get_perf_context()->get_from_memtable_count);
		hist_get_post_process.Add(
		...
		hist_mget_memtable.Add(
			get_perf_context()->get_from_memtable_time);
		hist_mget_files.Add(
			get_perf_context()->get_from_output_files_time);
		hist_mget_num_memtable_checked.Add(
			get_perf_context()->get_from_memtable_count);
		hist_mget_post_process.Add(
			get_perf_context()->get_post_process_time);
...
TEST_F(PerfContextTest, SeekKeyComparison)
{
	...
	for (const int i : keys) {
		...
		hist_time_diff.Add(put_time -
				   get_perf_context()->write_wal_time);
	}
...
TEST_F(PerfContextTest, DBMutexLockCounter)
{
	...
	for (PerfLevel perf_level :
		...
		for (int c = 0; c < 2; ++c) {
			InstrumentedMutex mutex(nullptr, Env::Default(),
						stats_code[c]);
			mutex.Lock();
...
TEST_F(PerfContextTest, FalseDBMutexWait)
{
	...
	for (int c = 0; c < 2; ++c) {
		...
		if (stats_code[c] == static_cast<int>(DB_MUTEX_WAIT_MICROS)) {
			// increment the counter only when it's a DB Mutex
			ASSERT_GT(get_perf_context()->db_condition_wait_nanos,
				  0);
		} else {
			ASSERT_EQ(get_perf_context()->db_condition_wait_nanos,
				  0);
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (sscanf(argv[i], "--write_buffer_size=%d%c", &n, &junk) ==
		    1) {
		...
		if (sscanf(argv[i], "--use_set_based_memetable=%d%c", &n,
			   &junk) == 1 &&
		    (n == 0 || n == 1)) {
