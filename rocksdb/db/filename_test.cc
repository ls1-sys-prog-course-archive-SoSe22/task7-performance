...
TEST_F(FileNameTest, Parse)
{
	...
	char kAllMode =
		kDefautInfoLogDir | kDifferentInfoLogDir | kNoCheckLogDir;
	...
	static struct {
		...
		{ "rocksdb_dir_LOG.old", 0, kInfoLogFile,
		  kDifferentInfoLogDir },
		{ "rocksdb_dir_LOG.old.6688", 6688, kInfoLogFile,
		  kDifferentInfoLogDir },
	...
	for (char mode :
		...
		for (unsigned int i = 0; i < sizeof(cases) / sizeof(cases[0]);
		     i++) {
			InfoLogPrefix info_log_prefix(mode != kDefautInfoLogDir,
						      "/rocksdb/dir");
			if (cases[i].mode & mode) {
				std::string f = cases[i].fname;
				if (mode == kNoCheckLogDir) {
					ASSERT_TRUE(ParseFileName(f, &number,
								  &type))
						<< f;
				} else {
					ASSERT_TRUE(ParseFileName(
						f, &number,
...
TEST_F(FileNameTest, InfoLogFileName)
{
	...
	ASSERT_EQ("/data/rocksdb/LOG",
		  InfoLogFileName(dbname, db_absolute_path, ""));
	...
	ASSERT_EQ("/data/rocksdb_log/data_rocksdb_LOG",
		  InfoLogFileName(dbname, db_absolute_path,
				  "/data/rocksdb_log"));
	ASSERT_EQ("/data/rocksdb_log/data_rocksdb_LOG.old.666",
		  OldInfoLogFileName(dbname, 666u, db_absolute_path,
				     "/data/rocksdb_log"));
}
