...
class MemTableListTest : public testing::Test {
		...
		autovector<MemTable *> *to_delete)
	{
		...
		return list->InstallMemtableFlushResults(
			cfd, mutable_cf_options, m, &versions, &mutex, 1,
...
TEST_F(MemTableListTest, GetTest)
{
	...
	MemTable *mem2 =
		new MemTable(cmp, ioptions, MutableCFOptions(options), &wb2,
			     kMaxSequenceNumber, 0 /* column_family_id */);
	mem2->Ref();
...
TEST_F(MemTableListTest, GetFromHistoryTest)
{
	...
	merge_context.Clear();
	found = list.current()->GetFromHistory(LookupKey("key1", seq), &value,
					       &s, &merge_context,
					       &range_del_agg, ReadOptions());
	ASSERT_TRUE(found && s.IsNotFound());

	merge_context.Clear();
	found = list.current()->GetFromHistory(LookupKey("key2", seq), &value,
					       &s, &merge_context,
					       &range_del_agg, ReadOptions());
	ASSERT_TRUE(found);
	ASSERT_EQ("value2.2", value);
	...
	MemTable *mem3 =
		new MemTable(cmp, ioptions, MutableCFOptions(options), &wb3,
			     kMaxSequenceNumber, 0 /* column_family_id */);
	mem3->Ref();
	...
	merge_context.Clear();
	found = list.current()->GetFromHistory(LookupKey("key3", seq), &value,
					       &s, &merge_context,
					       &range_del_agg, ReadOptions());
	ASSERT_TRUE(found);
...
TEST_F(MemTableListTest, FlushPendingTest)
{
	...
	ASSERT_EQ(0,
		  to_flush3.size()); // nothing not in progress of being flushed
	ASSERT_EQ(5, list.NumNotFlushed());
	ASSERT_FALSE(list.IsFlushPending());
	ASSERT_FALSE(list.imm_flush_needed.load(std::memory_order_acquire));
