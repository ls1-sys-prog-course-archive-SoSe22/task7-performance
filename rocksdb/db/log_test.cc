...
class LogTest : public ::testing::TestWithParam<int> {
	...
	class StringSource : public SequentialFile {
		...
		explicit StringSource(Slice &contents)
			: contents_(contents), force_error_(false),
			  force_error_position_(0), force_eof_(false),
			  force_eof_position_(0), returned_partial_(false)
		{
		}
	...
	{
		auto dest = dynamic_cast<test::StringSink *>(
			writer_.file()->writable_file());
		assert(dest);
	...
	{
		auto dest = dynamic_cast<const test::StringSink *>(
			writer_.file()->writable_file());
		assert(dest);
		return dest->contents_;
	}
	...
	void FixChecksum(int header_offset, int len, bool recyclable)
	{
		...
		int header_size =
			recyclable ? kRecyclableHeaderSize : kHeaderSize;
		uint32_t crc =
			crc32c::Value(&dest_contents()[header_offset + 6],
				      header_size - 6 + len);
		crc = crc32c::Mask(crc);
				      ...
				      int expected_record_offset)
	{
		...
		ASSERT_EQ(initial_offset_record_sizes_[expected_record_offset],
			  record.size());
		ASSERT_EQ(initial_offset_last_record_offsets_
				  [expected_record_offset],
			  offset_reader->LastRecordOffset());
		ASSERT_EQ((char)('a' + expected_record_offset),
			  record.data()[0]);
	}
};
...
TEST_P(LogTest, ErrorJoinsRecords)
{
	...
	for (unsigned int offset = kBlockSize; offset < 2 * kBlockSize;
	     offset++) {
		SetByte(offset, 'x');
	}
...
{
	int header_size = GetParam() ? kRecyclableHeaderSize : kHeaderSize;
	CheckInitialOffsetRecord(2 * (header_size + 1000) +
