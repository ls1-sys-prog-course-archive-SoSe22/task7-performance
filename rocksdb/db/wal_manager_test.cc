...
class WalManagerTest : public testing::Test {
	...
	WalManagerTest()
		: env_(new MockEnv(Env::Default())),
		  dbname_(test::TmpDir() + "/wal_manager_test"), db_options_(),
	...
	std::unique_ptr<TransactionLogIterator>
	OpenTransactionLogIter(const SequenceNumber seq)
	{
...
TEST_F(WalManagerTest, ReadFirstRecordCache)
{
	...
	ASSERT_OK(wal_manager_->TEST_ReadFirstRecord(kAliveLogFile,
						     1 /* number */, &s));
	ASSERT_EQ(s, 0U);
...
std::vector<std::uint64_t> ListSpecificFiles(Env *env, const std::string &path,
...
TEST_F(WalManagerTest, WALArchivalSizeLimit)
{
	...
	ASSERT_TRUE(archive_size <=
		    db_options_.wal_size_limit_mb * 1024 * 1024);
