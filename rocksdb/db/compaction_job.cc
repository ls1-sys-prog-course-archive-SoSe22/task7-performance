...
struct CompactionJob::SubcompactionState {
	...
	SubcompactionState(Compaction *c, Slice *_start, Slice *_end,
			   uint64_t size = 0)
		: compaction(c), start(_start), end(_end), outfile(nullptr),
		  builder(nullptr), current_output_file_size(0), total_bytes(0),
		  num_input_records(0), num_output_records(0),
		  approx_size(size), grandparent_index(0), overlapped_bytes(0),
		  seen_key(false), compression_dict()
	{
	...
	SubcompactionState &operator=(SubcompactionState &&o)
	{
		...
		current_output_file_size =
			std::move(o.current_output_file_size);
			      ...
			      uint64_t curr_file_size)
	{
		...
		while (grandparent_index < grandparents.size() &&
		       icmp->Compare(internal_key,
				     grandparents[grandparent_index]
					     ->largest.Encode()) > 0) {
			if (seen_key) {
				overlapped_bytes +=
					grandparents[grandparent_index]
			...
			assert(grandparent_index + 1 >= grandparents.size() ||
			       icmp->Compare(grandparents[grandparent_index]
						     ->largest.Encode(),
					     grandparents[grandparent_index + 1]
						     ->smallest.Encode()) <= 0);
			grandparent_index++;
		}
		seen_key = true;
...
struct CompactionJob::CompactionState {
	...
	explicit CompactionState(Compaction *c)
		: compaction(c), total_bytes(0), num_input_records(0),
	...
	Slice SmallestUserKey()
	{
		for (const auto &sub_compact_state : sub_compact_states) {
			if (!sub_compact_state.outputs.empty() &&
				...
				return sub_compact_state.outputs[0]
	...
	{
		for (auto it = sub_compact_states.rbegin();
		     it < sub_compact_states.rend(); ++it) {
			if (!it->outputs.empty() &&
			    it->current_output()->finished) {
				assert(it->current_output() != nullptr);
				return it->current_output()
...
CompactionJob::CompactionJob(
	...
	: job_id_(job_id), compact_(new CompactionState(compaction)),
	  compaction_job_stats_(compaction_job_stats), compaction_stats_(1),
	  dbname_(dbname), db_options_(db_options), env_options_(env_options),
	  env_(db_options.env), versions_(versions),
	  shutting_down_(shutting_down), log_buffer_(log_buffer),
	  db_directory_(db_directory), output_directory_(output_directory),
	  stats_(stats), db_mutex_(db_mutex), db_bg_error_(db_bg_error),
	  existing_snapshots_(std::move(existing_snapshots)),
	  earliest_write_conflict_snapshot_(earliest_write_conflict_snapshot),
	  table_cache_(std::move(table_cache)), event_logger_(event_logger),
...
void CompactionJob::ReportStartedCompaction(Compaction *compaction)
{
	...
	ThreadStatusUtil::SetThreadOperationProperty(
		...
		(static_cast<uint64_t>(compact_->compaction->start_level())
		 << 32) +
...
void CompactionJob::Prepare()
{
	...
	if (c->ShouldFormSubcompactions()) {
		...
		for (size_t i = 0; i <= boundaries_.size(); i++) {
			...
			Slice *end = i == boundaries_.size() ? nullptr :
								     &boundaries_[i];
...
void CompactionJob::GenSubcompactionBoundaries()
{
		...
		if (lvl >= start_lvl && lvl <= out_lvl) {
			...
			if (lvl == 0) {
				...
				for (size_t i = 0; i < num_files; i++) {
					bounds.emplace_back(
						flevel->files[i].smallest_key);
					bounds.emplace_back(
						flevel->files[i].largest_key);
				}
			} else {
				...
				bounds.emplace_back(flevel->files[num_files - 1]
							    .largest_key);
				if (lvl == out_lvl) {
	...
	bounds.erase(std::unique(bounds.begin(), bounds.end(),
				 [cfd_comparator](const Slice &a,
						  const Slice &b) -> bool {
					 return cfd_comparator->Compare(
							ExtractUserKey(a),
							ExtractUserKey(b)) == 0;
				 }),
		     bounds.end());
	...
	for (auto it = bounds.begin();;) {
		...
		const Slice b = *it;
		uint64_t size = versions_->ApproximateSize(v, a, b, start_lvl,
							   out_lvl + 1);
		...
		for (size_t i = 0; i < ranges.size() - 1; i++) {
			...
			if (sum >= mean) {
				boundaries_.emplace_back(
					ExtractUserKey(ranges[i].range.limit));
...
Status CompactionJob::Run()
{
	...
	for (size_t i = 1; i < compact_->sub_compact_states.size(); i++) {
		thread_pool.emplace_back(
			&CompactionJob::ProcessKeyValueCompaction, this,
	...
	for (const auto &state : compact_->sub_compact_states) {
		for (const auto &output : state.outputs) {
			auto fn = TableFileName(db_options_.db_paths,
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState *sub_compact)
{
	...
	std::unique_ptr<RangeDelAggregator> range_del_agg(
		new RangeDelAggregator(cfd->internal_comparator(),
				       existing_snapshots_));
	...
	if (bottommost_level_ &&
		...
		if (kOutFileLen != port::kMaxSizet) {
			const size_t kOutFileNumSamples =
				kOutFileLen >> kSampleLenShift;
			Random64 generator{ versions_->NewFileNumber() };
			for (size_t i = 0; i < kMaxSamples; ++i) {
				sample_begin_offsets.insert(
					generator.Uniform(kOutFileNumSamples)
					<< kSampleLenShift);
			}
		}
	}
	...
	if (start != nullptr) {
		IterKey start_iter;
		start_iter.SetInternalKey(*start, kMaxSequenceNumber,
					  kValueTypeForSeek);
	...
	sub_compact->c_iter.reset(new CompactionIterator(
		...
		earliest_write_conflict_snapshot_, env_, false,
	...
	while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
		...
		if (end != nullptr && cfd->user_comparator()->Compare(
					      c_iter->user_key(), *end) >= 0) {
			break;
		}
		if (c_iter_stats.num_input_records % kRecordStatsEvery ==
		    kRecordStatsEvery - 1) {
			RecordDroppedKeys(c_iter_stats,
		...
		if (sub_compact->outputs.size() == 1) { // first output file
			...
			for (const auto &data_elmt : { key, value }) {
				size_t data_end_offset =
					data_begin_offset + data_elmt.size();
				while (sample_begin_offset_iter !=
					       sample_begin_offsets.cend() &&
					...
					assert(data_begin_offset <
					       sample_end_offset);

					size_t data_elmt_copy_offset,
						data_elmt_copy_len;
					if (*sample_begin_offset_iter <=
					    data_begin_offset) {
		...
		if (output_file_ended) {
			...
			status = FinishCompactionOutputFile(
				input_status, sub_compact, range_del_agg.get(),
	...
	const Slice *next_table_min_key /* = nullptr */)
{
	...
	if (s.ok()) {
		...
		range_del_agg->AddToBuilder(sub_compact->builder.get(),
					    lower_bound, upper_bound, meta,
					    range_del_out_stats,
					    bottommost_level_);
	}
	...
	if (s.ok() && current_entries > 0) {
		...
		if (s.ok()) {
			...
			ROCKS_LOG_INFO(db_options_.info_log,
				       "[%s] [JOB %d] Generated table #%" PRIu64
				       ": %" PRIu64 " keys, %" PRIu64
				       " bytes%s",
	...
	std::string fname =
		TableFileName(db_options_.db_paths, meta->fd.GetNumber(),
			      meta->fd.GetPathId());
	EventHelpers::LogAndNotifyTableFileCreationFinished(
		event_logger_, cfd->ioptions()->listeners, dbname_,
		cfd->GetName(), fname, job_id_, meta->fd, tp,
	...
	if (sfm && meta->fd.GetPathId() == 0) {
		auto fn = TableFileName(cfd->ioptions()->db_paths,
					meta->fd.GetNumber(),
					meta->fd.GetPathId());
		sfm->OnAddFile(fn);
	...
	const MutableCFOptions &mutable_cf_options)
{
	...
	for (const auto &sub_compact : compact_->sub_compact_states) {
		for (const auto &out : sub_compact.outputs) {
			compaction->edit()->AddFile(compaction->output_level(),
						    out.meta);
		}
	}
...
Status CompactionJob::OpenCompactionOutputFile(SubcompactionState *sub_compact)
{
	...
	std::string fname =
		TableFileName(db_options_.db_paths, file_number,
			      sub_compact->compaction->output_path_id());
	// Fire events.
	ColumnFamilyData *cfd = sub_compact->compaction->column_family_data();
#ifndef ROCKSDB_LITE
	EventHelpers::NotifyTableFileCreationStarted(
		cfd->ioptions()->listeners, dbname_, cfd->GetName(), fname,
		job_id_, TableFileCreationReason::kCompaction);
#endif // !ROCKSDB_LITE
	// Make the output file
	unique_ptr<WritableFile> writable_file;
	EnvOptions opt_env_opts = env_->OptimizeForCompactionTableWrite(
	...
	if (!s.ok()) {
		...
		EventHelpers::LogAndNotifyTableFileCreationFinished(
			...
			TableProperties(), TableFileCreationReason::kCompaction,
			s);
		return s;
	}

	SubcompactionState::Output out;
	out.meta.fd = FileDescriptor(
		file_number, sub_compact->compaction->output_path_id(), 0);
	out.finished = false;
	...
	for (SubcompactionState &sub_compact : compact_->sub_compact_states) {
		...
		if (sub_compact.builder != nullptr) {
			...
			assert(!sub_status.ok() ||
			       sub_compact.outfile == nullptr);
		}
		for (const auto &out : sub_compact.outputs) {
			// If this file was inserted into the table cache then remove
			// them here because this compaction was not committed.
			if (!sub_status.ok()) {
				TableCache::Evict(table_cache_.get(),
...
void CompactionJob::UpdateCompactionStats()
{
	...
	for (int input_level = 0;
	     ...
	     ++input_level) {
		if (compaction->level(input_level) !=
	...
	for (const auto &sub_compact : compact_->sub_compact_states) {
		...
		for (const auto &out : sub_compact.outputs) {
			compaction_stats_.bytes_written +=
				out.meta.fd.file_size;
		}
		if (sub_compact.num_input_records >
		    sub_compact.num_output_records) {
