...
class VersionStorageInfo {
	...
	{
		file_indexer_.UpdateIndex(&arena_, num_non_empty_levels_,
					  files_);
	}
	...
	void
	ComputeCompactionScore(const ImmutableCFOptions &immutable_cf_options,
	...
	FilesMarkedForCompaction() const
	{
		assert(finalized_);
	...
	const char *LevelFileSummary(FileSummaryStorage *scratch,
				     int level) const;
	...
	uint64_t GetAverageValueSize() const
	{
		...
		assert(accumulated_raw_key_size_ + accumulated_raw_value_size_ >
		       0);
		assert(accumulated_file_size_ > 0);
		return accumulated_raw_value_size_ /
		       accumulated_num_non_deletions_ * accumulated_file_size_ /
	...
	bool force_consistency_checks() const
	{
		return force_consistency_checks_;
	}
...
class Version {
	...
	void AddIteratorsForLevel(const ReadOptions &,
	...
	void AddRangeDelIteratorsForLevel(
		...
		int level, std::vector<InternalIterator *> *range_del_iters);
	...
	Status GetPropertiesOfAllTables(TablePropertiesCollection *props,
					int level);
	Status
	GetPropertiesOfTablesInRange(const Range *range, std::size_t n,
	...
	Status
	GetAggregatedTableProperties(std::shared_ptr<const TableProperties> *tp,
				     int level = -1);
...
class VersionSet {
	...
	Status
	LogAndApply(ColumnFamilyData *column_family_data,
		    const MutableCFOptions &mutable_cf_options,
		    ...
		    const ColumnFamilyOptions *column_family_options = nullptr)
	{
		...
		return LogAndApply(column_family_data, mutable_cf_options,
				   edit_list, mu, db_directory,
	...
	// column_families.
	static Status
	ListColumnFamilies(std::vector<std::string> *column_families,
	...
	uint64_t options_file_number() const
	{
		return options_file_number_;
	}
	...
	uint64_t ApproximateSize(Version *v, const Slice &start,
				 const Slice &end, int start_level = 0,
				 int end_level = -1);
	...
	uint64_t ApproximateSizeLevel0(Version *v,
	...
	ColumnFamilyData *
	CreateColumnFamily(const ColumnFamilyOptions &cf_options,
			   VersionEdit *edit);
