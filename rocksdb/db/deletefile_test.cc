...
class DeleteFileTest : public testing::Test {
			    ...
			    std::vector<int> *keysperlevel = nullptr)
	{
		...
		for (size_t i = 0; i < metadata.size(); i++) {
			...
			if (keysperlevel != nullptr) {
				(*keysperlevel)[(int)metadata[i].level] +=
					numkeysinfile;
			}
			fprintf(stderr,
	...
	void CheckFileTypeCounts(std::string &dir, int required_log,
...
TEST_F(DeleteFileTest, BackgroundPurgeTestMultipleJobs)
{
	...
	delete itr2;
	env_->Schedule(&DeleteFileTest::GuardFinish, nullptr,
		       Env::Priority::HIGH);
	CloseDB();
...
TEST_F(DeleteFileTest, DeleteLogFiles)
{
	...
	ASSERT_OK(env_->FileExists(options_.wal_dir + "/" +
				   alive_log->PathName()));
	...
	ASSERT_EQ(Status::NotFound(),
		  env_->FileExists(options_.wal_dir + "/" +
				   archived_log->PathName()));
	CloseDB();
}
...
TEST_F(DeleteFileTest, DeleteNonDefaultColumnFamily)
{
	...
	rocksdb::DB *db;
	ASSERT_OK(
		DB::Open(db_options, dbname_, column_families, &handles, &db));

	Random rnd(5);
	for (int i = 0; i < 1000; ++i) {
		ASSERT_OK(db->Put(WriteOptions(), handles[1],
	...
	auto old_file =
		metadata[0].smallest_seqno < metadata[1].smallest_seqno ?
			      metadata[0].name :
			      metadata[1].name;
	auto new_file =
		metadata[0].smallest_seqno > metadata[1].smallest_seqno ?
	...
	{
		std::unique_ptr<Iterator> itr(
			db->NewIterator(ReadOptions(), handles[1]));
		int count = 0;
