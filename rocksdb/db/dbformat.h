...
class InternalKeyComparator : public Comparator {
	...
	int Compare(const ParsedInternalKey &a,
...
class InternalKey {
	...
	{
		AppendInternalKey(&rep_, ParsedInternalKey(_user_key,
							   kMaxSequenceNumber,
							   kValueTypeForSeek));
	}
...
class IterKey {
    public:
	IterKey()
		: buf_(space_), buf_size_(sizeof(space_)), key_(buf_),
		  key_size_(0), is_user_key_(true)
	{
	}
			    ...
			    ValueType value_type = kValueTypeForSeek)
	{
		...
		EncodeFixed64(buf_ + usize + psize,
	...
	void EncodeLengthPrefixedKey(const Slice &key)
	{
		auto size = key.size();
		EnlargeBufferIfNeeded(size +
				      static_cast<size_t>(VarintLength(size)));
