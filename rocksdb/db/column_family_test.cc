...
class ColumnFamilyTest : public testing::Test {
	...
	void CheckMissed()
	{
		...
		for (int cf = 0; cf < 3; cf++) {
			...
			Iterator *iter = db_->NewIterator(
				ReadOptions(false, true), handles_[cf]);
			for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
				uint64_t key;
				Slice in(iter->key());
				in.remove_prefix(3);
				if (!ConsumeDecimalNumber(&in, &key) ||
				    !in.empty() || key < next_expected) {
					bad_keys++;
					continue;
				}
		       ...
		       std::vector<ColumnFamilyOptions> options = {})
	{
		...
		return DB::Open(db_options_, dbname_, column_families,
				&handles_, &db_);
	}
			    ...
			    std::vector<ColumnFamilyOptions> options = {})
	{
		...
		return DB::OpenForReadOnly(db_options_, dbname_,
					   column_families, &handles_, &db_);
	}
	...
	int GetProperty(int cf, std::string property)
	{
		std::string value;
		EXPECT_TRUE(
			dbfull()->GetProperty(handles_[cf], property, &value));
#ifndef CYGWIN
...
#ifndef ROCKSDB_LITE
		uint64_t v;
		EXPECT_TRUE(dbfull()->GetIntProperty("rocksdb.is-write-stopped",
						     &v));
		return (v == 1);
#else
...
#ifndef ROCKSDB_LITE
		uint64_t v;
		EXPECT_TRUE(dbfull()->GetIntProperty(
	...
	{
		Close();
		ASSERT_OK(DestroyDB(dbname_, Options(db_options_,
						     column_family_options_)));
	}
		...
		const std::vector<ColumnFamilyOptions> options = {})
	{
		...
		for (size_t i = 0; i < cfs.size(); ++i) {
			const auto &current_cf_opt =
				options.size() == 0 ? column_family_options_ :
							    options[i];
			ASSERT_OK(db_->CreateColumnFamily(
				current_cf_opt, cfs[i], &handles_[cfi]));
			names_[cfi] = cfs[i];
			...
			RocksDBOptionsParser::VerifyCFOptions(desc.options,
							      current_cf_opt);
#endif // !ROCKSDB_LITE
			cfi++;
		}
	...
	{
		return db_->Put(WriteOptions(), handles_[cf], Slice(key),
				Slice(value));
	}
	Status Merge(int cf, const std::string &key, const std::string &value)
	{
		return db_->Merge(WriteOptions(), handles_[cf], Slice(key),
				  Slice(value));
	}
	Status Flush(int cf)
	{
	...
	{
		return GetProperty(cf, "rocksdb.num-files-at-level" +
					       ToString(level));
	}
	...
	std::string FilesPerLevel(int cf)
	{
		...
		for (int level = 0;
		     level < dbfull()->NumberLevels(handles_[cf]); level++) {
		      ...
		      uint64_t size = 0)
	{
		...
		ASSERT_OK(env_->NewWritableFile(destination, &destfile,
						soptions));
...
TEST_F(ColumnFamilyTest, DontReuseColumnFamilyID)
{
	for (int iter = 0; iter < 3; ++iter) {
		...
		auto cfh3 =
			reinterpret_cast<ColumnFamilyHandleImpl *>(handles_[3]);
...
TEST_F(ColumnFamilyTest, CreateCFRaceWithGetAggProperty)
{
	...
	uint64_t pv;
	db_->GetAggregatedIntProperty(DB::Properties::kEstimateTableReadersMem,
				      &pv);
...
TEST_F(ColumnFamilyTest, BulkAddDrop)
{
	...
	for (int i = 1; i <= kNumCF; i++) {
		ASSERT_OK(db_->Put(write_options, cf_handles[i - 1], "foo",
				   "bar"));
	}
	...
	cf_handles.clear();
	for (int i = 1; i <= kNumCF; i++) {
		cf_descriptors.emplace_back("cf2-" + ToString(i),
					    ColumnFamilyOptions());
	}
...
TEST_F(ColumnFamilyTest, IgnoreRecoveredLog)
{
	...
	for (auto &log : logs) {
		if (log != ".." && log != ".") {
			CopyFile(db_options_.wal_dir + "/" + log,
				 backup_logs + "/" + log);
		}
	}
...
TEST_F(ColumnFamilyTest, GetComparator)
{
	...
	CreateColumnFamilies({ "second", "third", "fourth" },
			     { second, third, fourth });
...
TEST_F(ColumnFamilyTest, DifferentCompactionStyles)
{
	...
	default_cf.table_factory.reset(
...
TEST_F(ColumnFamilyTest, MultipleManualCompactions)
{
	...
	default_cf.write_buffer_size = 64 << 10; // 64KB
	default_cf.target_file_size_base = 30 << 10;
	default_cf.max_compaction_bytes =
		default_cf.target_file_size_base * 1100;
	BlockBasedTableOptions table_options;
	table_options.no_block_cache = true;
	default_cf.table_factory.reset(
		NewBlockBasedTableFactory(table_options));
	...
	bool cf_1_1 = true;
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "ColumnFamilyTest::MultiManual:4",
		    "ColumnFamilyTest::MultiManual:1" },
		  { "ColumnFamilyTest::MultiManual:2",
		    "ColumnFamilyTest::MultiManual:5" },
		  { "ColumnFamilyTest::MultiManual:2",
		    "ColumnFamilyTest::MultiManual:3" } });
	...
	threads.emplace_back([&] {
		...
		ASSERT_OK(db_->CompactRange(compact_options, handles_[1],
					    nullptr, nullptr));
	});
	...
	threads.emplace_back([&] {
		...
		CompactRangeOptions compact_options;
		compact_options.exclusive_manual_compaction = false;
		ASSERT_OK(db_->CompactRange(compact_options, handles_[2],
					    nullptr, nullptr));
...
TEST_F(ColumnFamilyTest, AutomaticAndManualCompactions)
{
	...
	bool cf_1_1 = true;
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "ColumnFamilyTest::AutoManual:4",
		    "ColumnFamilyTest::AutoManual:1" },
		  { "ColumnFamilyTest::AutoManual:2",
		    "ColumnFamilyTest::AutoManual:5" },
		  { "ColumnFamilyTest::AutoManual:2",
...
TEST_F(ColumnFamilyTest, ManualAndAutomaticCompactions)
{
	...
	bool cf_1_2 = true;
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "ColumnFamilyTest::ManualAuto:4",
		    "ColumnFamilyTest::ManualAuto:1" },
		  { "ColumnFamilyTest::ManualAuto:5",
		    "ColumnFamilyTest::ManualAuto:2" },
		  { "ColumnFamilyTest::ManualAuto:2",
...
TEST_F(ColumnFamilyTest, SameCFManualManualCompactions)
{
	...
	bool cf_1_2 = true;
	rocksdb::SyncPoint::GetInstance()->LoadDependency(
		{ { "ColumnFamilyTest::ManualManual:4",
		    "ColumnFamilyTest::ManualManual:2" },
		  { "ColumnFamilyTest::ManualManual:4",
		    "ColumnFamilyTest::ManualManual:5" },
	...
	for (int i = 0; i < one.level0_file_num_compaction_trigger - 2; ++i) {
		...
		WaitForFlush(1);
		AssertFilesPerLevel(
			ToString(one.level0_file_num_compaction_trigger + i),
			1);
	}
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"DBImpl::RunManualCompaction:NotScheduled", [&](void *arg) {
			InstrumentedMutex *mutex =
				static_cast<InstrumentedMutex *>(arg);
			mutex->Unlock();
...
TEST_F(ColumnFamilyTest, SameCFAutomaticManualCompactions)
{
	...
	CompactRangeOptions compact_options;
	compact_options.exclusive_manual_compaction = false;
	ASSERT_OK(db_->CompactRange(compact_options, handles_[1], nullptr,
				    nullptr));
...
TEST_F(ColumnFamilyTest, ReadOnlyDBTest)
{
	...
	// test newiterators
	{
		std::vector<Iterator *> iterators;
		ASSERT_OK(
			db_->NewIterators(ReadOptions(), handles_, &iterators));
		for (auto it : iterators) {
			it->SeekToFirst();
		}
		ASSERT_EQ(IterStatus(iterators[0]), "a->b");
...
TEST_F(ColumnFamilyTest, DontRollEmptyLogs)
{
	...
	int total_new_writable_files = env_->GetNumberOfNewWritableFileCalls() -
				       num_writable_file_start;
	ASSERT_EQ(static_cast<size_t>(total_new_writable_files),
		  handles_.size() + 1);
	Close();
}
#endif // !ROCKSDB_LITE
			...
			for (int i = 1; i <= 3; i++) {
				for (int j = 1; j <= 3; j++) {
					for (int k = 1; k <= 3; k++) {
						...
						original.compaction_style =
							static_cast<
								CompactionStyle>(
								s);
						original.num_levels = l;
						original.level0_stop_writes_trigger =
							i;
						original.level0_slowdown_writes_trigger =
							j;
...
TEST_F(ColumnFamilyTest, CreateAndDropRace)
{
	...
	for (int i = 0; i < kCfCount; ++i) {
		...
		comparators.push_back(
			new test::SimpleSuffixReverseComparator());
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"PersistRocksDBOptions:start", [&](void *arg) {
			...
			if (main_thread_id == current_thread_id) {
				test_stage =
					kMainThreadStartPersistingOptionsFile;
				while (test_stage <
				       kChildThreadFinishDroppingColumnFamily) {
	...
	rocksdb::SyncPoint::GetInstance()->SetCallBack(
		"WriteThread::EnterUnbatched:Wait", [&](void *arg) {
			...
			test_stage =
				kChildThreadWaitingMainThreadPersistOptions;
...
TEST_F(ColumnFamilyTest, WriteStallSingleColumnFamily)
{
	...
	Open({ "default" });
	ColumnFamilyData *cfd = static_cast<ColumnFamilyHandleImpl *>(
