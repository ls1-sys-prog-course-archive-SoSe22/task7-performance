				      ...
				      const ReadOptions &read_opts)
{
	*seq = kMaxSequenceNumber;
	...
	for (auto &memtable : *list) {
		...
		bool done =
			memtable->Get(key, value, s, merge_context,
...
{
	for (auto &m : memlist_) {
		merge_iter_builder->AddIterator(m->NewIterator(
			options, merge_iter_builder->GetArena()));
	}
}
...
MemTableListVersion::ApproximateStats(const Slice &start_ikey,
...
SequenceNumber
MemTableListVersion::GetEarliestSequenceNumber(bool include_history) const
{
...
void MemTableListVersion::Remove(MemTable *m, autovector<MemTable *> *to_delete)
{
	assert(refs_ ==
	       1); // only when refs_ == 1 is MemTableListVersion mutable
	memlist_.remove(m);
	...
	for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
		...
		if (!m->flush_in_progress_) {
			...
			if (num_flush_not_started_ == 0) {
				imm_flush_needed.store(
					false, std::memory_order_release);
			}
			m->flush_in_progress_ =
	...
	while (s.ok()) {
		...
		if (batch_count > 0) {
			...
			if (s.ok()) { // commit new state
				...
				for (auto it = current_->memlist_.rbegin();
				     batch_count-- > 0; it++) {
					MemTable *m = *it;
...
{
	assert(static_cast<int>(current_->memlist_.size()) >=
	       num_flush_not_started_);
	InstallNewVersion();
