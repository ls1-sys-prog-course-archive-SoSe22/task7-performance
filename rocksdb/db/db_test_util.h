...
class SpecialMemTableRep : public MemTableRep {
	...
	explicit SpecialMemTableRep(Allocator *allocator, MemTableRep *memtable,
		...
		: MemTableRep(allocator), memtable_(memtable),
		  num_entries_flush_(num_entries_flush), num_entries_(0)
	{
	}
	...
	virtual size_t ApproximateMemoryUsage() override
	{
		...
		// to trigger a flush.
		return (num_entries_ < num_entries_flush_) ? 0 :
								   1024 * 1024 * 1024;
	}
...
class SpecialSkipListFactory : public MemTableRepFactory {
			  ...
			  Logger *logger) override
	{
		return new SpecialMemTableRep(
			allocator,
			factory_.CreateMemTableRep(compare, allocator,
						   transform, 0),
			num_entries_flush_);
	}
...
class SpecialEnv : public EnvWrapper {
			       ...
			       const EnvOptions &soptions) override
	{
		class SSTableFile : public WritableFile {
		    ...
		    public:
			SSTableFile(SpecialEnv *env,
				    unique_ptr<WritableFile> &&base)
			...
			Status Append(const Slice &data) override
			{
				...
				if (env_->drop_writes_.load(
					    std::memory_order_acquire)) {
					// Drop writes on the floor
					return Status::OK();
				} else if (env_->no_space_.load(
						   std::memory_order_acquire)) {
						...
						uint64_t offset) override
			{
				...
				if (env_->drop_writes_.load(
					...
					env_->bytes_written_ += data.size();
					return base_->PositionedAppend(data,
								       offset);
				}
			}
...
// SyncPoint is not supported in Released Windows Mode.
				...
				TEST_SYNC_POINT_CALLBACK(
					"DBTestWritableFile.GetPreallocationStatus",
					&preallocation_size);
			...
			{
				++env_->sync_counter_;
				while (env_->delay_sstable_sync_.load(
					std::memory_order_acquire)) {
		...
		class ManifestFile : public WritableFile {
		    public:
			ManifestFile(SpecialEnv *env,
				     unique_ptr<WritableFile> &&b)
...
#endif
				Status s;
				if (env_->log_write_error_.load(
					    std::memory_order_acquire)) {
					s = Status::IOError(
						"simulated writer error");
				} else {
					int slowdown =
						env_->log_write_slowdown_.load(
							std::memory_order_acquire);
					if (slowdown > 0) {
...
// SyncPoint is not supported in Released Windows Mode.
				...
				size_t preallocation_size =
					preallocation_block_size();
				TEST_SYNC_POINT_CALLBACK(
					"DBTestWalFile.GetPreallocationStatus",
					&preallocation_size);
#endif // !(defined NDEBUG) || !defined(OS_WIN)
				   ...
				   const EnvOptions &soptions) override
	{
		class CountingFile : public RandomAccessFile {
			...
			{
				counter_->Increment();
				Status s = target_->Read(offset, n, result,
							 scratch);
		...
		if (s.ok() && count_random_reads_) {
			r->reset(new CountingFile(std::move(*r),
...
class DBTestBase : public testing::Test {
	...
	static bool ShouldSkipOptions(int option_config,
				      int skip_mask = kNoSkip);
	...
	void GenerateNewFile(int fd, Random *rnd, int *key_idx,
			     bool nowait = false);
	...
	static UpdateStatus updateInPlaceNoAction(char *prevValue,
						  uint32_t *prevSize,
						  Slice delta,
						  std::string *newValue);
	...
	GetAllSSTFiles(uint64_t *total_size = nullptr);
...
#ifndef ROCKSDB_LITE
	uint64_t
	GetNumberOfSstFilesForColumnFamily(DB *db,
