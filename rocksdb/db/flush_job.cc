...
FlushJob::FlushJob(const std::string &dbname, ColumnFamilyData *cfd,
	...
	: dbname_(dbname), cfd_(cfd), db_options_(db_options),
	  mutable_cf_options_(mutable_cf_options), env_options_(env_options),
	  versions_(versions), db_mutex_(db_mutex),
	  shutting_down_(shutting_down),
	  existing_snapshots_(std::move(existing_snapshots)),
	  earliest_write_conflict_snapshot_(earliest_write_conflict_snapshot),
	  job_context_(job_context), log_buffer_(log_buffer),
	  db_directory_(db_directory),
	  output_file_directory_(output_file_directory),
	  output_compression_(output_compression), stats_(stats),
	  event_logger_(event_logger), measure_io_stats_(measure_io_stats),
...
Status FlushJob::WriteLevel0Table()
{
	...
	Status s;
	{
...

		{
			...
			EnvOptions optimized_env_options =
				db_options_.env->OptimizeForCompactionTableWrite(
					env_options_, db_options_);
			s = BuildTable(dbname_, db_options_.env,
				       *cfd_->ioptions(), mutable_cf_options_,
				       optimized_env_options,
				       cfd_->table_cache(), iter.get(),
				       std::move(range_del_iter), &meta_,
				       cfd_->internal_comparator(),
	...
	if (s.ok() && meta_.fd.GetFileSize() > 0) {
		...
		// that key range.
		// Add file to L0
		edit_->AddFile(0 /* level */, meta_.fd.GetNumber(),
