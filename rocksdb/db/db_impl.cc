				    ...
				    const MutableCFOptions &mutable_cf_options)
{
	...
	if (ioptions.compaction_style == kCompactionStyleUniversal) {
		if (ioptions.compaction_options_universal
...
DBImpl::DBImpl(const DBOptions &options, const std::string &dbname)
	: env_(options.env), dbname_(dbname),
	  ...
	  stats_(immutable_db_options_.statistics.get()), db_lock_(nullptr),
	  mutex_(stats_, env_, DB_MUTEX_WAIT_MICROS,
		 immutable_db_options_.use_adaptive_mutex),
	  shutting_down_(false), bg_cv_(&mutex_), logfile_number_(0),
	  log_dir_synced_(false), log_empty_(true), default_cf_handle_(nullptr),
	  log_sync_cv_(&mutex_), total_log_size_(0),
	  max_total_in_memory_state_(0), is_snapshot_supported_(true),
	  write_buffer_manager_(
		  immutable_db_options_.write_buffer_manager.get()),
	  ...
	  low_pri_write_rate_limiter_(NewGenericRateLimiter(
		  std::min(static_cast<int64_t>(
				   mutable_db_options_.delayed_write_rate / 8),
			   kDefaultLowPriThrottledRate))),
	  last_batch_group_size_(0), unscheduled_flushes_(0),
	  unscheduled_compactions_(0), bg_compaction_scheduled_(0),
	  num_running_compactions_(0), bg_flush_scheduled_(0),
	  num_running_flushes_(0), bg_purge_scheduled_(0),
	  disable_delete_obsolete_files_(0),
	  delete_obsolete_files_last_run_(env_->NowMicros()),
	  last_stats_dump_time_microsec_(0), next_job_id_(1),
	  has_unpersisted_data_(false), unable_to_flush_oldest_log_(false),
	  env_options_(
	  ...
	  bg_work_paused_(0), bg_compaction_paused_(0), refitting_level_(false),
	  opened_successfully_(false)
{
	...
	table_cache_ =
		NewLRUCache(table_cache_size,
			    immutable_db_options_.table_cache_numshardbits);

	versions_.reset(new VersionSet(
		dbname_, &immutable_db_options_, env_options_,
	...
	if (last_stats_dump_time_microsec_ + stats_dump_period_sec * 1000000 <=
		...
		std::string stats;
		{
			...
			default_cf_internal_stats_->GetStringProperty(
				*db_property_info, DB::Properties::kDBStats,
				&stats);
			for (auto cfd : *versions_->GetColumnFamilySet()) {
				if (cfd->initialized()) {
		...
		ROCKS_LOG_WARN(immutable_db_options_.info_log,
			       "------- DUMPING STATS -------");
		ROCKS_LOG_WARN(immutable_db_options_.info_log, "%s",
			       stats.c_str());
	...
	const std::unordered_map<std::string, std::string> &options_map)
{
	...
	WriteThread::Writer w;
	{
		...
		if (s.ok()) {
			...
			versions_->LogAndApply(cfd, new_options, &dummy_edit,
					       &mutex_,
					       directories_.GetDbDir());
			// Trigger possible flush/compactions. This has to be before we persist
			// options to file, otherwise there will be a deadlock with writer
			// thread.
			auto *old_sv = InstallSuperVersionAndScheduleWork(
				cfd, nullptr, new_options);
			delete old_sv;

			persist_options_status = WriteOptionsFile(
				false /*need_mutex_lock*/,
	...
	const std::unordered_map<std::string, std::string> &options_map)
{
	...
	{
		InstrumentedMutexLock l(&mutex_);
		s = GetMutableDBOptionsFromStrings(mutable_db_options_,
						   options_map, &new_options);
		if (s.ok()) {
	...
	int level)
{
	...
	for (int i = level - 1; i > 0; --i) {
		...
		if (vstorage->MaxBytesForLevel(i) <
...
Status DBImpl::SyncWAL()
{
...

	{
		...
		for (auto it = logs_.begin();
		     it != logs_.end() && it->number <= current_log_number;
		     ++it) {
			if (!it->writer->file()
				     ->writable_file()
				     ->IsSyncThreadSafe()) {
	...
	for (log::Writer *log : logs_to_sync) {
		status = log->file()->SyncWithoutFlush(
...
void DBImpl::SchedulePurge()
{
	...
	bg_purge_scheduled_++;
	env_->Schedule(&DBImpl::BGWorkPurge, this, Env::Priority::HIGH,
		       nullptr);
}
...
void DBImpl::BackgroundCallPurge()
{
	...
	while (!purge_queue_.empty() || !logs_to_free_queue_.empty()) {
		if (!purge_queue_.empty()) {
			...
			DeleteObsoleteFileImpl(file_deletion_status, job_id,
					       fname, type, number, path_id);
			mutex_.Lock();
		} else {
			assert(!logs_to_free_queue_.empty());
			log::Writer *log_writer =
				*(logs_to_free_queue_.begin());
...
{
struct IterState {
	IterState(DBImpl *_db, InstrumentedMutex *_mu,
	...
	if (state->super_version->Unref()) {
		...
		if (job_context.HaveSomethingToDelete()) {
			if (state->background_purge) {
				...
				// background thread.
				state->db->PurgeObsoleteFiles(
					job_context, true /* schedule only */);
				state->mu->Lock();
					      ...
					      RangeDelAggregator *range_del_agg)
{
	...
	if (s.ok()) {
		super_version->imm->AddIterators(read_options,
						 &merge_iter_builder);
		if (!read_options.ignore_range_deletions) {
			s = super_version->imm->AddRangeTombstoneIterators(
				read_options, arena, range_del_agg);
		}
	}
	if (s.ok()) {
		...
		if (read_options.read_tier != kMemtableTier) {
			super_version->current->AddIterators(
				read_options, env_options_, &merge_iter_builder,
				range_del_agg);
		}
		       ...
		       PinnableSlice *pinnable_val, bool *value_found)
{
	...
	bool done = false;
	if (!skip_memtable) {
		if (sv->mem->Get(lkey, pinnable_val->GetSelf(), &s,
			...
			RecordTick(stats_, MEMTABLE_HIT);
			   ...
			   sv->imm->Get(lkey, pinnable_val->GetSelf(), &s,
					&merge_context, &range_del_agg,
					read_options)) {
			done = true;
			pinnable_val->PinSelf();
	...
	if (!done) {
		...
		sv->current->Get(read_options, lkey, pinnable_val, &s,
				 &merge_context, &range_del_agg, value_found);
...
std::vector<Status>
DBImpl::MultiGet(const ReadOptions &read_options,
		 ...
		 std::vector<std::string> *values)
{
	...
	std::unordered_map<uint32_t, MultiGetColumnFamilyData *>
		multiget_cf_data;
	// fill up and allocate outside of mutex
	for (auto cf : column_family) {
		auto cfh = reinterpret_cast<ColumnFamilyHandleImpl *>(cf);
		auto cfd = cfh->cfd();
		if (multiget_cf_data.find(cfd->GetID()) ==
		    multiget_cf_data.end()) {
	...
	for (size_t i = 0; i < num_keys; ++i) {
		...
		auto cfh = reinterpret_cast<ColumnFamilyHandleImpl *>(
			column_family[i]);
		RangeDelAggregator range_del_agg(
			cfh->cfd()->internal_comparator(), snapshot);
		...
		if (!done) {
			...
			PERF_TIMER_GUARD(get_from_output_files_time);
			super_version->current->Get(read_options, lkey,
						    &pinnable_val, &s,
						    &merge_context,
						    &range_del_agg);
	...
	std::vector<ColumnFamilyHandle *> *handles)
{
	...
	for (size_t i = 0; i < num_cf; i++) {
		ColumnFamilyHandle *handle;
		s = CreateColumnFamilyImpl(cf_options, column_family_names[i],
					   &handle);
		if (!s.ok()) {
			break;
		}
	...
	if (success_once) {
		Status persist_options_status =
			WriteOptionsFile(true /*need_mutex_lock*/,
					 true /*need_enter_write_thread*/);
	...
	Status s;
	{
		...
		if (s.ok()) {
			...
			s = versions_->LogAndApply(
				cfd, *cfd->GetLatestMutableCFOptions(), &edit,
				&mutex_);
			write_thread_.ExitUnbatched(&w);
		}
			 ...
			 std::string *value, bool *value_found)
{
	...
	PinnableSlice pinnable_val;
	auto s = GetImpl(roptions, column_family, key, &pinnable_val,
			 value_found);
	value->assign(pinnable_val.data(), pinnable_val.size());
			      ...
			      ColumnFamilyHandle *column_family)
{
	...
	if (read_options.managed) {
		...
		ArenaWrappedDBIter *db_iter = NewArenaWrappedDbIterator(
			env_, read_options, *cfd->ioptions(),
			cfd->user_comparator(), snapshot,
			sv->mutable_cf_options.max_sequential_skip_in_iterations,
			sv->version_number);

		InternalIterator *internal_iter =
			NewInternalIterator(read_options, cfd, sv,
					    db_iter->GetArena(),
...
Status
DBImpl::NewIterators(const ReadOptions &read_options,
		     const std::vector<ColumnFamilyHandle *> &column_families,
		     std::vector<Iterator *> *iterators)
{
	...
	if (read_options.managed) {
		...
		for (auto cfh : column_families) {
			auto cfd =
				reinterpret_cast<ColumnFamilyHandleImpl *>(cfh)
					->cfd();
			auto iter =
				new ManagedIterator(this, read_options, cfd);
		...
		for (auto cfh : column_families) {
			auto cfd =
				reinterpret_cast<ColumnFamilyHandleImpl *>(cfh)
					->cfd();
			SuperVersion *sv =
				cfd->GetReferencedSuperVersion(&mutex_);
			auto iter = new ForwardIterator(this, read_options, cfd,
							sv);
			iterators->push_back(NewDBIterator(
				env_, read_options, *cfd->ioptions(),
				cfd->user_comparator(), iter,
				kMaxSequenceNumber,
...
Options DBImpl::GetOptions(ColumnFamilyHandle *column_family) const
{
	InstrumentedMutexLock l(&mutex_);
	auto cfh = reinterpret_cast<ColumnFamilyHandleImpl *>(column_family);
	return Options(BuildDBOptions(immutable_db_options_,
				      mutable_db_options_),
			 ...
			 const Slice &property, std::string *value)
{
	...
	if (property_info == nullptr) {
		...
		uint64_t int_value;
		bool ret_value = GetIntPropertyInternal(cfd, *property_info,
							false, &int_value);
		if (ret_value) {
		...
		InstrumentedMutexLock l(&mutex_);
		return cfd->internal_stats()->GetStringProperty(
			    ...
			    std::map<std::string, double> *value)
{
	...
	if (property_info == nullptr) {
		...
		InstrumentedMutexLock l(&mutex_);
		return cfd->internal_stats()->GetMapProperty(*property_info,
							     property, value);
	}
				    ...
				    bool is_locked, uint64_t *value)
{
	...
	if (!property_info.need_out_of_mutex) {
		if (is_locked) {
			mutex_.AssertHeld();
			return cfd->internal_stats()->GetIntProperty(
				property_info, value, this);
		} else {
			InstrumentedMutexLock l(&mutex_);
			return cfd->internal_stats()->GetIntProperty(
				property_info, value, this);
				      ...
				      uint64_t *aggregated_value)
{
	...
	uint64_t sum = 0;
	{
		...
		for (auto *cfd : *versions_->GetColumnFamilySet()) {
			...
			if (GetIntPropertyInternal(cfd, *property_info, true,
						   &value)) {
				sum += value;
			} else {
				return false;
...
ColumnFamilyHandle *
DBImpl::GetColumnFamilyHandleUnlocked(uint32_t column_family_id)
{
	ColumnFamilyMemTables *cf_memtables = column_family_memtables_.get();
				 ...
				 uint8_t include_flags)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if (include_flags & DB::SizeApproximationFlags::INCLUDE_FILES) {
			sizes[i] += versions_->ApproximateSize(v, k1.Encode(),
							       k2.Encode());
		}
		if (include_flags &
		    DB::SizeApproximationFlags::INCLUDE_MEMTABLES) {
			sizes[i] += sv->mem->ApproximateStats(k1.Encode(),
							      k2.Encode())
					    .size;
			sizes[i] += sv->imm->ApproximateStats(k1.Encode(),
							      k2.Encode())
					    .size;
		}
	}
...
std::list<uint64_t>::iterator DBImpl::CaptureCurrentFileNumberInPendingOutputs()
{
...
void DBImpl::ReleaseFileNumberFromPendingOutputs(std::list<uint64_t>::iterator v)
{
...
#ifndef ROCKSDB_LITE
Status
DBImpl::GetUpdatesSince(SequenceNumber seq,
...
Status DBImpl::DeleteFile(std::string name)
{
	...
	if (type == kLogFile) {
		...
		status = env_->DeleteFile(immutable_db_options_.wal_dir + "/" +
					  name.c_str());
		if (!status.ok()) {
			ROCKS_LOG_ERROR(immutable_db_options_.info_log,
	...
	{
		InstrumentedMutexLock l(&mutex_);
		status = versions_->GetMetadataForFile(number, &level,
						       &metadata, &cfd);
		if (!status.ok()) {
			ROCKS_LOG_WARN(immutable_db_options_.info_log,
		...
		if (level == 0 &&
			...
			job_context.Clean();
			return Status::InvalidArgument(
				"File in level 0, but not oldest");
		}
		edit.SetColumnFamily(cfd->GetID());
				  ...
				  const Slice *begin, const Slice *end)
{
	...
	JobContext job_context(next_job_id_.fetch_add(1), true);
	{
		...
		for (int i = 1; i < cfd->NumberLevels(); i++) {
			...
			InternalKey begin_storage, end_storage, *begin_key,
				*end_key;
			if (begin == nullptr) {
				begin_key = nullptr;
			} else {
			...
			for (uint32_t j = 0; j < level_files.size(); j++) {
				...
				if (((begin == nullptr) ||
				     (cfd->internal_comparator()
					      .user_comparator()
					      ->Compare(level_file->smallest
								.user_key(),
							*begin) >= 0)) &&
				    ((end == nullptr) ||
				     (cfd->internal_comparator()
					      .user_comparator()
					      ->Compare(level_file->largest
								.user_key(),
							*end) <= 0))) {
...
Status DBImpl::CheckConsistency()
{
	...
	for (const auto &md : metadata) {
		...
		if (!s.ok() &&
		    env_->GetFileSize(Rocks2LevelTableFileName(file_path),
				      &fsize)
			    .ok()) {
			s = Status::OK();
		}
		if (!s.ok()) {
			corruption_messages += "Can't access " + md.name +
					       ": " + s.ToString() + "\n";
		} else if (fsize != md.size) {
			corruption_messages +=
				"Sst file size mismatch: " + file_path +
...
Status DBImpl::GetDbIdentity(std::string &identity) const
{
	...
	Status s;
	{
		...
		id_file_reader.reset(
			new SequentialFileReader(std::move(idfile)));
	}
...
Status
DB::CreateColumnFamilies(const ColumnFamilyOptions &cf_options,
			 const std::vector<std::string> &column_family_names,
			 std::vector<ColumnFamilyHandle *> *handles)
{
	return Status::NotSupported("");
...
Status
DB::DropColumnFamilies(const std::vector<ColumnFamilyHandle *> &column_families)
{
	return Status::NotSupported("");
}
...
{
	return VersionSet::ListColumnFamilies(column_families, name,
					      db_options.env);
}
...
Status DestroyDB(const std::string &dbname, const Options &options)
{
	...
	if (result.ok()) {
		uint64_t number;
		FileType type;
		InfoLogPrefix info_log_prefix(!soptions.db_log_dir.empty(),
					      dbname);
		for (size_t i = 0; i < filenames.size(); i++) {
			if (ParseFileName(filenames[i], &number,
					  info_log_prefix.prefix, &type) &&
			    type != kDBLockFile) { // Lock file will be deleted at end
				Status del;
				std::string path_to_delete =
					dbname + "/" + filenames[i];
				if (type == kMetaDatabase) {
					del = DestroyDB(path_to_delete,
							options);
				} else if (type == kTableFile) {
					del = DeleteSSTFile(&soptions,
							    path_to_delete, 0);
				} else {
		...
		for (size_t path_id = 0; path_id < options.db_paths.size();
		     path_id++) {
			...
			for (size_t i = 0; i < filenames.size(); i++) {
				if (ParseFileName(filenames[i], &number,
					...
					std::string table_path = db_path.path +
								 "/" +
								 filenames[i];
					Status del = DeleteSSTFile(
						&soptions, table_path,
		...
		for (const auto &file : walDirFiles) {
			if (ParseFileName(file, &number, &type) &&
			    type == kLogFile) {
				Status del = env->DeleteFile(soptions.wal_dir +
							     "/" + file);
				if (result.ok() && !del.ok()) {
					result = del;
				}
				...
				bool need_enter_write_thread)
{
	...
	Status s = PersistRocksDBOptions(db_options, cf_names, cf_opts,
					 file_name, GetEnv());
...
void DeleteOptionsFilesHelper(const std::map<uint64_t, std::string> &filenames,
			      ...
			      const std::shared_ptr<Logger> &info_log, Env *env)
{
	...
	for (auto iter = std::next(filenames.begin(), num_files_to_keep);
		...
		if (!env->DeleteFile(iter->second).ok()) {
			ROCKS_LOG_WARN(info_log,
				       "Unable to delete options file %s",
				       iter->second.c_str());
		}
	}
}
...
Status DBImpl::DeleteObsoleteOptionsFiles()
{
	...
	for (auto &filename : filenames) {
		...
		FileType type;
		if (ParseFileName(filename, &file_number, &type) &&
		    type == kOptionsFile) {
				       ...
				       bool *found_record_for_key)
{
	...
	*found_record_for_key = false;
	...
	sv->imm->GetFromHistory(lkey, nullptr, &s, &merge_context,
				&range_del_agg, seq, read_options);
	...
	if (!cache_only) {
		// Check tables
		sv->current->Get(read_options, lkey, nullptr, &s,
				 &merge_context, &range_del_agg,
				 nullptr /* value_found */,
...
Status
DBImpl::IngestExternalFile(ColumnFamilyHandle *column_family,
			   ...
			   const IngestExternalFileOptions &ingestion_options)
{
	...
	TEST_SYNC_POINT("DBImpl::AddFile:Start");
	{
		...
		if (status.ok()) {
			status = versions_->LogAndApply(
				cfd, *mutable_cf_options, ingestion_job.edit(),
				&mutex_, directories_.GetDbDir());
		}
		if (status.ok()) {
			delete InstallSuperVersionAndScheduleWork(
				cfd, nullptr, *mutable_cf_options);
		}
