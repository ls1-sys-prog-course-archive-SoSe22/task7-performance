...
extern Status
CheckConcurrentWritesSupported(const ColumnFamilyOptions &cf_options);
...
class ColumnFamilyData {
	...
	bool is_delete_range_supported()
	{
		return is_delete_range_supported_;
	}
	...
	MemTable *
	ConstructNewMemtable(const MutableCFOptions &mutable_cf_options,
	...
	// REQUIRES: DB mutex held
	Compaction *
	CompactRange(const MutableCFOptions &mutable_cf_options,
...
class ColumnFamilySet {
	...
	ColumnFamilyData *
	CreateColumnFamily(const std::string &name, uint32_t id,
