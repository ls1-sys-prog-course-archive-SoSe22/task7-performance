	...
	void OnCompactionCompleted(DB *db, const CompactionJobInfo &ci) override
	{
		...
		for (auto fl : { ci.input_files, ci.output_files }) {
			for (auto fn : fl) {
				...
				ASSERT_EQ(tp->user_collected_properties
						  .find("0")
						  ->second,
					  "1");
			}
...
TEST_F(EventListenerTest, OnSingleDBCompactionTest)
{
	...
	for (int i = 1; i < 8; ++i) {
		...
		ASSERT_OK(dbfull()->CompactRange(CompactRangeOptions(),
						 handles_[i], &kRangeStart,
						 &kRangeEnd));
...
class TestFlushListener : public EventListener {
	...
	void OnTableFileCreated(const TableFileCreationInfo &info) override
	{
		...
		for (auto thread_status : thread_list) {
			if (thread_status.operation_type ==
				    ThreadStatus::OP_FLUSH ||
			    thread_status.operation_type ==
	...
	void OnFlushCompleted(DB *db, const FlushJobInfo &info) override
	{
		...
		ASSERT_EQ(info.table_properties.user_collected_properties
				  .find("0")
				  ->second,
			  "1");
	}
...
TEST_F(EventListenerTest, OnSingleDBFlushTest)
{
	...
	for (size_t i = 0; i < cf_names.size(); ++i) {
		...
		ASSERT_EQ(listener->flushed_column_family_names_[i],
			  cf_names[i]);
	}
}
...
TEST_F(EventListenerTest, MultiDBMultiListeners)
{
	...
	for (size_t c = 0; c < cf_names.size(); ++c) {
		for (int d = 0; d < kNumDBs; ++d) {
			ASSERT_OK(dbs[d]->Flush(FlushOptions(),
						vec_handles[d][c]));
			reinterpret_cast<DBImpl *>(dbs[d])
	...
	for (auto *listener : listeners) {
		...
		for (size_t c = 0; c < cf_names.size(); ++c) {
			for (int d = 0; d < kNumDBs; ++d) {
				...
				ASSERT_EQ(listener->flushed_column_family_names_
						  [pos],
					  cf_names[c]);
				pos++;
			}
...
TEST_F(EventListenerTest, DisableBGCompaction)
{
	...
	for (int i = 0;
	     static_cast<int>(cf_meta.file_count) < kSlowdownTrigger * 10;
	     ++i) {
...
TEST_F(EventListenerTest, CompactionReasonLevel)
{
	...
	TestCompactionReasonListener *listener =
...
class TableFileCreationListener : public EventListener {
	...
	class TestEnv : public EnvWrapper {
		...
		{
			if (fname.size() > 4 &&
			    fname.substr(fname.size() - 4) == ".sst") {
				if (!status_.ok()) {
					return status_;
				}
			}
			return Env::Default()->NewWritableFile(fname, result,
							       options);
		}
...
class ColumnFamilyHandleDeletionStartedListener : public EventListener {
	...
	void
	OnColumnFamilyHandleDeletionStarted(ColumnFamilyHandle *handle) override
	{
...
TEST_F(EventListenerTest, ColumnFamilyHandleDeletionStartedListenerTest)
{
	...
	auto listener =
		std::make_shared<ColumnFamilyHandleDeletionStartedListener>(
			cfs);
	Options options;
	options.create_if_missing = true;
	options.listeners.push_back(listener);
