...
class LevelIterator : public InternalIterator {
	...
	LevelIterator(const ColumnFamilyData *const cfd,
		...
		: cfd_(cfd), read_options_(read_options), files_(files),
		  ...
		  file_iter_(nullptr), pinned_iters_mgr_(nullptr)
	{
	}
	...
	void Reset()
	{
		...
		file_iter_ = cfd_->table_cache()->NewIterator(
			...
			cfd_->internal_comparator(), files_[file_index_]->fd,
...
ForwardIterator::ForwardIterator(DBImpl *db, const ReadOptions &read_options,
	...
	: db_(db), read_options_(read_options), cfd_(cfd),
	  prefix_extractor_(cfd->ioptions()->prefix_extractor),
	  user_comparator_(cfd->user_comparator()),
	  immutable_min_heap_(MinIterComparator(&cfd_->internal_comparator())),
	  sv_(current_sv), mutable_iter_(nullptr), current_(nullptr),
	  valid_(false), status_(Status::OK()), immutable_status_(Status::OK()),
	  has_iter_trimmed_for_upper_bound_(false),
	  current_over_upper_bound_(false), is_prev_set_(false),
	  is_prev_inclusive_(false), pinned_iters_mgr_(nullptr)
{
	if (sv_) {
...
void ForwardIterator::SVCleanup()
{
	if (sv_ != nullptr && sv_->Unref()) {
		...
		db_->mutex_.Unlock();
		if (pinned_iters_mgr_ && pinned_iters_mgr_->PinningEnabled()) {
			pinned_iters_mgr_->PinPtr(sv_,
						  &ReleaseSuperVersionFunc);
		} else {
			delete sv_;
		}
				   ...
				   bool seek_to_first)
{
	...
	if (seek_to_first || NeedToSeekImmutable(internal_key)) {
		...
		{
			auto tmp = MinIterHeap(MinIterComparator(
				&cfd_->internal_comparator()));
			immutable_min_heap_.swap(tmp);
		}
		for (size_t i = 0; i < imm_iters_.size(); i++) {
			auto *m = imm_iters_[i];
			seek_to_first ? m->SeekToFirst() :
					      m->Seek(internal_key);
			if (!m->status().ok()) {
		...
		for (size_t i = 0; i < l0.size(); ++i) {
			...
			if (seek_to_first) {
				...
				if (user_comparator_->Compare(
					...
					if (read_options_.iterate_upper_bound !=
					    nullptr) {
		...
		for (int32_t level = 1; level < vstorage->num_levels();
		     ++level) {
			...
			if (f_idx < level_files.size()) {
				...
				seek_to_first ?
					      level_iters_[level - 1]->SeekToFirst() :
					      level_iters_[level - 1]->Seek(
						internal_key);
		...
		TEST_SYNC_POINT_CALLBACK(
			"ForwardIterator::SeekInternal:Immutable", this);
...
void ForwardIterator::Next()
{
	...
	if (sv_ == nullptr ||
		...
		if (is_prev_set_ && prefix_extractor_) {
			...
			update_prev_key =
				prefix_extractor_
					->Transform(prev_key_.GetUserKey())
...
void ForwardIterator::SetPinnedItersMgr(PinnedIteratorsManager *pinned_iters_mgr)
{
	pinned_iters_mgr_ = pinned_iters_mgr;
...
void ForwardIterator::RebuildIterators(bool refresh_sv)
{
	...
	RangeDelAggregator range_del_agg(
		InternalKeyComparator(cfd_->internal_comparator()),
		{} /* snapshots */);
	...
	for (const auto *l0 : l0_files) {
		...
		l0_iters_.push_back(cfd_->table_cache()->NewIterator(
			read_options_, *cfd_->soptions(),
			cfd_->internal_comparator(), l0->fd,
			read_options_.ignore_range_deletions ? nullptr :
								     &range_del_agg));
	}
...
void ForwardIterator::RenewIterators()
{
	...
	for (inew = 0; inew < l0_files_new.size(); inew++) {
		...
		l0_iters_new.push_back(cfd_->table_cache()->NewIterator(
			read_options_, *cfd_->soptions(),
			cfd_->internal_comparator(), l0_files_new[inew]->fd,
			read_options_.ignore_range_deletions ? nullptr :
								     &range_del_agg));
	}
		    ...
		    ((read_options_.iterate_upper_bound != nullptr) &&
		     (user_comparator_->Compare(
			      *read_options_.iterate_upper_bound,
			      level_files[0]->smallest.user_key()) < 0))) {
			level_iters_.push_back(nullptr);
			if (!level_files.empty()) {
				has_iter_trimmed_for_upper_bound_ = true;
			}
		} else {
			level_iters_.push_back(new LevelIterator(
...
void ForwardIterator::ResetIncompleteIterators()
{
	...
	for (size_t i = 0; i < l0_iters_.size(); ++i) {
		...
		l0_iters_[i] = cfd_->table_cache()->NewIterator(
			read_options_, *cfd_->soptions(),
			cfd_->internal_comparator(), l0_files[i]->fd,
...
bool ForwardIterator::NeedToSeekImmutable(const Slice &target)
{
	...
	if (cfd_->internal_comparator().InternalKeyComparator::Compare(
		    target, current_ == mutable_iter_ ?
					  immutable_min_heap_.top()->key() :
					  current_->key()) > 0) {
		return true;
	}
	return false;
...
uint32_t
ForwardIterator::FindFileInRange(const std::vector<FileMetaData *> &files,
