	...
	bool *made_progress, JobContext *job_context, LogBuffer *log_buffer)
{
	...
	FlushJob flush_job(
		...
		snapshot_seqs, earliest_write_conflict_snapshot, job_context,
	...
	if (s.ok()) {
		...
		if (sfm) {
			...
			std::string file_path = MakeTableFileName(
				immutable_db_options_.db_paths[0].path,
				...
				int job_id, TableProperties prop)
{
	...
	mutex_.Unlock();
	{
		...
		info.file_path = MakeTableFileName(
			immutable_db_options_.db_paths[0].path,
			file_meta->fd.GetNumber());
			    ...
			    const Slice *begin, const Slice *end)
{
	...
	int max_level_with_files = 0;
	{
		...
		for (int level = 1;
		     level < base->storage_info()->num_non_empty_levels();
		     level++) {
			if (base->storage_info()->OverlapInLevel(level, begin,
								 end)) {
	...
	if (cfd->ioptions()->compaction_style == kCompactionStyleUniversal &&
		...
		s = RunManualCompaction(cfd,
					ColumnFamilyData::kCompactAllLevels,
					final_output_level,
					options.target_path_id, begin, end,
					exclusive);
	} else {
		for (int level = 0; level <= max_level_with_files; level++) {
			...
			if (cfd->ioptions()->compaction_style ==
				...
				if (cfd->ioptions()->compaction_style ==
				    ...
				    level == 0) {
					output_level = ColumnFamilyData::
						kCompactToBaseLevel;
				}
			}
			s = RunManualCompaction(cfd, level, output_level,
						options.target_path_id, begin,
						end, exclusive);
			if (!s.ok()) {
				break;
			}
	...
	if (options.change_level) {
		...
		if (s.ok()) {
			s = ReFitLevel(cfd, final_output_level,
...
#ifndef ROCKSDB_LITE
Status
DBImpl::CompactFilesImpl(const CompactionOptions &compact_options,
			 ...
			 JobContext *job_context, LogBuffer *log_buffer)
{
	...
	s = cfd->compaction_picker()->GetCompactionInputsFromFileNumbers(
		&input_files, &input_set, version->storage_info(),
		compact_options);
	if (!s.ok()) {
		return s;
	}
	...
	for (auto inputs : input_files) {
		if (cfd->compaction_picker()->AreFilesInCompaction(
			    inputs.files)) {
	...
	version->storage_info()->ComputeCompactionScore(
		*cfd->ioptions(), *c->mutable_cf_options());
	...
	if (status.ok()) {
		InstallSuperVersionAndScheduleWorkWrapper(
			c->column_family_data(), job_context,
...
Status DBImpl::ReFitLevel(ColumnFamilyData *cfd, int level, int target_level)
{
	...
	if (target_level < 0) {
		to_level = FindMinimumEmptyLevelFitting(cfd, mutable_cf_options,
							level);
	}
	...
	if (to_level != level) {
		...
		for (const auto &f : vstorage->LevelFiles(level)) {
			...
			edit.AddFile(to_level, f->fd.GetNumber(),
		...
		status = versions_->LogAndApply(cfd, mutable_cf_options, &edit,
						&mutex_,
						directories_.GetDbDir());
				   ...
				   bool exclusive, bool disallow_trivial_move)
{
	...
	TEST_SYNC_POINT_CALLBACK("DBImpl::RunManualCompaction:NotScheduled",
				 &mutex_);
	if (exclusive) {
	...
	while (!manual.done) {
		...
		if (ShouldntRunManualCompaction(&manual) ||
		    (manual.in_progress == true) || scheduled ||
		    ((manual.manual_end = &manual.tmp_storage1) &&
		     ((manual.compaction = manual.cfd->CompactRange(
			       *manual.cfd->GetLatestMutableCFOptions(),
			       manual.input_level, manual.output_level,
			       manual.output_path_id, manual.begin, manual.end,
			       &manual.manual_end, &manual_conflict)) ==
		      nullptr) &&
		     manual_conflict)) {
			...
			manual.incomplete = false;
			bg_compaction_scheduled_++;
			env_->Schedule(&DBImpl::BGWorkCompaction, ca,
				       Env::Priority::LOW, this,
	...
	const std::vector<ColumnFamilyHandle *> &column_family_handles)
{
	...
	for (auto cf_ptr : column_family_handles) {
		Status status = this->SetOptions(
			cf_ptr, { { "disable_auto_compactions", "false" } });
		if (!status.ok()) {
			s = status;
		}
	}
...
void DBImpl::MaybeScheduleFlushOrCompaction()
{
	...
	if (is_flush_pool_empty) {
		while (unscheduled_flushes_ > 0 &&
			...
			unscheduled_flushes_--;
			bg_flush_scheduled_++;
			env_->Schedule(&DBImpl::BGWorkFlush, this,
				       Env::Priority::LOW, this);
		}
	}
					   ...
					   bool parallelize_compactions)
{
	...
	if (max_background_flushes == -1 && max_background_compactions == -1) {
		...
		res.max_compactions =
			std::max(1, max_background_jobs - res.max_flushes);
	} else {
			       ...
			       LogBuffer *log_buffer)
{
	...
	if (cfd != nullptr) {
		...
		ROCKS_LOG_BUFFER(
			...
			bg_compaction_scheduled_);
		status = FlushMemTableToOutputFile(cfd, mutable_cf_options,
						   made_progress, job_context,
						   log_buffer);
		if (cfd->Unref()) {
			delete cfd;
		}
...
void DBImpl::BackgroundCallCompaction(void *arg)
{
			     ...
			     immutable_db_options_.info_log.get());
	{
		...
		Status s = BackgroundCompaction(&made_progress, &job_context,
						&log_buffer, m);
	...
	*made_progress = false;
	...
	if (is_manual) {
		...
		if (!mutable_cf_options->disable_auto_compactions &&
			...
			c.reset(cfd->PickCompaction(*mutable_cf_options,
						    log_buffer));
			TEST_SYNC_POINT(
	...
	if (!c) {
		...
		*made_progress = true;
		...
		for (unsigned int l = 0; l < c->num_input_levels(); l++) {
			...
			for (size_t i = 0; i < c->num_input_files(l); i++) {
				...
				c->edit()->DeleteFile(c->level(l),
						      f->fd.GetNumber());
				c->edit()->AddFile(
					c->output_level(), f->fd.GetNumber(),
					f->fd.GetPathId(), f->fd.GetFileSize(),
			...
			c->output_level(), moved_bytes);
		{
			event_logger_.LogToBuffer(log_buffer)
				...
				<< "trivial_move"
				<< "destination_level" << c->output_level()
				<< "files" << moved_files << "total_files_size"
				<< moved_bytes;
		}
		ROCKS_LOG_BUFFER(log_buffer,
				 "[%s] Moved #%d files to level-%d %" PRIu64
				 " bytes %s: %s\n",
				 c->column_family_data()->GetName().c_str(),
				 moved_files, c->output_level(), moved_bytes,
		...
		*made_progress = true;
		...
		CompactionJob compaction_job(
			...
			&mutex_, &bg_error_, snapshot_seqs,
			earliest_write_conflict_snapshot, table_cache_,
			&event_logger_,
			c->mutable_cf_options()->paranoid_file_checks,
			c->mutable_cf_options()->report_bg_io_stats, dbname_,
	...
	if (c != nullptr) {
		...
		*made_progress = true;
		NotifyOnCompactionCompleted(c->column_family_data(), c.get(),
					    status, compaction_job_stats,
					    job_context->job_id);
	}
	...
	const MutableCFOptions &mutable_cf_options)
{
	...
	if (old_sv) {
		old_memtable_size =
			old_sv->mutable_cf_options.write_buffer_size *
			old_sv->mutable_cf_options.max_write_buffer_number;
	}

	auto *old =
		cfd->InstallSuperVersion(new_sv ? new_sv : new SuperVersion(),
