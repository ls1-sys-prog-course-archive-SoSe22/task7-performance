				   ...
				   bool update_num_ops_stats)
{
	...
	const MergeOperator::MergeOperationInput merge_in(key, value, operands,
							  logger);
	MergeOperator::MergeOperationOutput merge_out(*result,
						      tmp_result_operand);
	{
		...
		if (tmp_result_operand.data()) {
			...
			if (result_operand != nullptr) {
				*result_operand = tmp_result_operand;
				...
				result->assign(tmp_result_operand.data(),
					       tmp_result_operand.size());
			}
			       ...
			       const bool at_bottom)
{
	...
	for (; iter->Valid(); iter->Next(), original_key_is_iter = false) {
		...
		if (!ParseInternalKey(iter->key(), &ikey)) {
			...
			assert(user_comparator_->Equal(ikey.user_key,
						       orig_ikey.user_key));
			first_key = false;
		} else if (!user_comparator_->Equal(ikey.user_key,
						    orig_ikey.user_key)) {
		...
		if (ikey.type != kTypeMerge) {
			...
			if (s.ok()) {
				...
				UpdateInternalKey(&original_key,
						  orig_ikey.sequence,
						  orig_ikey.type);
				keys_.clear();
				merge_context_.Clear();
			...
			if (range_del_agg != nullptr &&
			    ...
			    range_del_agg->ShouldDelete(
				    ...
				    RangeDelAggregator::RangePositioningMode::
					    kForwardTraversal) &&
			    filter != CompactionFilter::Decision::
					      kRemoveAndSkipUntil) {
			...
			} else if (filter == CompactionFilter::Decision::
						     kRemoveAndSkipUntil) {
	...
	if (surely_seen_the_beginning) {
		...
		std::string merge_result;
		s = TimedFullMerge(user_merge_operator_, orig_ikey.user_key,
