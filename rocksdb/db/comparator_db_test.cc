			    ...
			    int num_trigger_flush)
{
	...
	for (int i = 0; i < num_writes; i++) {
		if (num_trigger_flush > 0 && i != 0 &&
		    i % num_trigger_flush == 0) {
			db->Flush(FlushOptions());
		}

		int type = rnd->Uniform(2);
		int index =
			rnd->Uniform(static_cast<int>(source_strings.size()));
	...
	for (int i = 0; i < num_iter_ops; i++) {
		...
		case 2: {
			// Seek to random key
			auto key_idx = rnd->Uniform(
				static_cast<int>(source_strings.size()));
...
TEST_F(ComparatorDBTest, SimpleSuffixReverseComparator)
{
	...
	for (int rnd_seed = 301; rnd_seed < 316; rnd_seed++) {
		...
		for (int i = 0; i < 5; i++) {
			source_prefixes.push_back(
				test::RandomHumanReadableString(&rnd, 8));
		}
		for (int j = 0; j < 20; j++) {
			int prefix_index = rnd.Uniform(
				static_cast<int>(source_prefixes.size()));
			std::string key = source_prefixes[prefix_index] +
					  test::RandomHumanReadableString(
						  &rnd, rnd.Uniform(8));
			source_strings.push_back(key);
		}

		DoRandomIteraratorTest(GetDB(), source_strings, &rnd, 30, 600,
				       66);
	}
}
...
TEST_F(ComparatorDBTest, Uint64Comparator)
{
	...
	for (int rnd_seed = 301; rnd_seed < 316; rnd_seed++) {
		...
		DoRandomIteraratorTest(GetDB(), source_strings, &rnd, 200, 1000,
				       66);
	}
}
