...
class DBIter : public Iterator {
	...
	struct LocalStatistics {
		...
		{
			RecordTick(global_statistics, NUMBER_DB_NEXT,
				   next_count_);
			RecordTick(global_statistics, NUMBER_DB_NEXT_FOUND,
				   next_found_count_);
			RecordTick(global_statistics, NUMBER_DB_PREV,
				   prev_count_);
			RecordTick(global_statistics, NUMBER_DB_PREV_FOUND,
				   prev_found_count_);
			RecordTick(global_statistics, ITER_BYTES_READ,
				   bytes_read_);
			ResetCounters();
		}
	...
	DBIter(Env *env, const ReadOptions &read_options,
		...
		: arena_mode_(arena_mode), env_(env),
		  logger_(cf_options.info_log), user_comparator_(cmp),
		  merge_operator_(cf_options.merge_operator), iter_(iter),
	...
	virtual Slice value() const override
	{
		...
		if (current_entry_is_merged_) {
			...
			return pinned_value_.data() ? pinned_value_ :
							    saved_value_;
...
inline bool DBIter::ParseKey(ParsedInternalKey *ikey)
{
	if (!ParseInternalKey(iter_->key(), ikey)) {
		status_ =
			Status::Corruption("corrupted internal key in DBIter");
			...
			if (skipping && user_comparator_->Compare(
				...
				case kTypeValue:
					...
					if (range_del_agg_.ShouldDelete(
						    ikey,
						    RangeDelAggregator::RangePositioningMode::
							    kForwardTraversal)) {
						// Arrange to skip all upcoming entries for this key since
						// they are hidden by this deletion.
						skipping = true;
...
void DBIter::MergeValuesNewToOld()
{
	if (!merge_operator_) {
		...
		status_ =
			Status::InvalidArgument("merge_operator_ must be set.");
		valid_ = false;
		return;
	}
	...
	for (iter_->Next(); iter_->Valid(); iter_->Next()) {
		...
		if (!user_comparator_->Equal(ikey.user_key,
					     saved_key_.GetUserKey())) {
			// hit the next user key, stop right here
			break;
		} else if (kTypeDeletion == ikey.type ||
			   kTypeSingleDeletion == ikey.type ||
			...
			s = MergeHelper::TimedFullMerge(
				merge_operator_, ikey.user_key, &val,
...
void DBIter::ReverseToForward()
{
	if (prefix_extractor_ != nullptr && !total_order_seek_) {
		...
		last_key.SetInternalKey(ParsedInternalKey(
			saved_key_.GetUserKey(), kMaxSequenceNumber,
			kValueTypeForSeek));
...
void DBIter::ReverseToBackward()
{
	if (prefix_extractor_ != nullptr && !total_order_seek_) {
		IterKey last_key;
		last_key.SetInternalKey(ParsedInternalKey(
			saved_key_.GetUserKey(), 0, kValueTypeForSeekForPrev));
	...
	if (current_entry_is_merged_) {
		...
		while (iter_->Valid() &&
		       user_comparator_->Compare(ikey.user_key,
						 saved_key_.GetUserKey()) > 0) {
			if (ikey.sequence > sequence_) {
				PERF_COUNTER_ADD(internal_recent_skipped_count,
						 1);
			} else {
				PERF_COUNTER_ADD(internal_key_skipped_count, 1);
			}
	...
	if (iter_->Valid()) {
		...
		assert(user_comparator_->Compare(ikey.user_key,
						 saved_key_.GetUserKey()) <= 0);
	}
#endif
...
void DBIter::PrevInternal()
{
	...
	while (iter_->Valid()) {
		...
		if (FindValueForCurrentKey()) {
			...
			if (valid_ && prefix_extractor_ &&
			    prefix_same_as_start_ &&
			    prefix_extractor_
					    ->Transform(saved_key_.GetUserKey())
					    .compare(prefix_start_key_) != 0) {
				valid_ = false;
			}
			return;
...
bool DBIter::FindValueForCurrentKey()
{
	...
	while (iter_->Valid() && ikey.sequence <= sequence_ &&
	       user_comparator_->Equal(ikey.user_key,
				       saved_key_.GetUserKey())) {
		if (TooManyInternalKeysSkipped()) {
			return false;
		}
		...
		PERF_COUNTER_ADD(internal_key_skipped_count, 1);
		assert(user_comparator_->Equal(ikey.user_key,
					       saved_key_.GetUserKey()));
		iter_->Prev();
		++num_skipped;
		FindParseableKey(&ikey, kReverse);
	...
	case kTypeMerge:
		...
		if (last_not_merge_type == kTypeDeletion ||
			...
			s = MergeHelper::TimedFullMerge(
				merge_operator_, saved_key_.GetUserKey(),
				&pinned_value_, merge_context_.GetOperands(),
				&saved_value_, logger_, statistics_, env_,
				&pinned_value_, true);
		}
		break;
...
bool DBIter::FindValueForCurrentKeyUsingSeek()
{
	...
	if (!iter_->Valid() ||
		...
		s = MergeHelper::TimedFullMerge(
			...
			merge_context_.GetOperands(), &saved_value_, logger_,
	...
	if (iter_->Valid()) {
		...
		if (statistics_ != nullptr) {
			if (valid_) {
				...
				RecordTick(statistics_, ITER_BYTES_READ,
					   key().size() + value().size());
			}
		}
	} else {
...
void DBIter::SeekToLast()
{
	...
	if (iter_->Valid() && iterate_upper_bound_ != nullptr) {
		...
		if (!Valid()) {
			return;
		} else if (user_comparator_->Equal(*iterate_upper_bound_,
						   key())) {
			Prev();
		}
	} else {
	...
	uint64_t max_sequential_skip_in_iterations, uint64_t version_number)
{
	...
	DBIter *db_iter = new (mem)
		DBIter(env, read_options, cf_options, user_key_comparator,
		       nullptr, sequence, true,
		       max_sequential_skip_in_iterations, version_number);
