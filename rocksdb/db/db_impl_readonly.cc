				      ...
				      ColumnFamilyHandle *column_family)
{
	...
	auto internal_iter =
		NewInternalIterator(read_options, cfd, super_version,
				    db_iter->GetArena(),
	...
	std::vector<Iterator *> *iterators)
{
	...
	for (auto cfh : column_families) {
		auto *cfd =
			reinterpret_cast<ColumnFamilyHandleImpl *>(cfh)->cfd();
...
Status
DB::OpenForReadOnly(const DBOptions &db_options, const std::string &dbname,
	...
	*dbptr = nullptr;
	...
	if (s.ok()) {
		...
		for (auto cf : column_families) {
			...
			handles->push_back(new ColumnFamilyHandleImpl(
				cfd, impl, &impl->mutex_));
		}
	}
	if (s.ok()) {
		...
		*dbptr = impl;
		for (auto *h : *handles) {
			impl->NewThreadStatusCfInfo(
				reinterpret_cast<ColumnFamilyHandleImpl *>(h)
					->cfd());
		}
	} else {
		for (auto h : *handles) {
