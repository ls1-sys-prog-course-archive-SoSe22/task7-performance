	...
	const std::vector<std::string> &external_files_paths)
{
	...
	for (const IngestedFileInfo &f : files_to_ingest_) {
		if (f.cf_id != TablePropertiesCollectorFactory::Context::
	...
	if (num_files == 0) {
		...
		std::sort(sorted_files.begin(), sorted_files.end(),
			  [&ucmp](const IngestedFileInfo *info1,
				  ...
				  return ucmp->Compare(
						 info1->smallest_user_key,
						 info2->smallest_user_key) < 0;
			  });

		for (size_t i = 0; i < num_files - 1; i++) {
			if (ucmp->Compare(
				    sorted_files[i]->largest_user_key,
				    sorted_files[i + 1]->smallest_user_key) >=
			    0) {
	...
	for (IngestedFileInfo &f : files_to_ingest_) {
		...
		if (!f.smallest_internal_key().Valid() ||
			...
			return Status::Corruption(
				"Generated table have corrupted keys");
		}
	}

	// Copy/Move external files into DB
	for (IngestedFileInfo &f : files_to_ingest_) {
		f.fd = FileDescriptor(versions_->NewFileNumber(), 0,
				      f.file_size);

		const std::string path_outside_db = f.external_file_path;
		const std::string path_inside_db =
			TableFileName(db_options_.db_paths, f.fd.GetNumber(),
				      f.fd.GetPathId());

		if (ingestion_options_.move_files) {
			status =
				env_->LinkFile(path_outside_db, path_inside_db);
			if (status.IsNotSupported()) {
				// Original file is on a different FS, use copy instead of hard linking
				status = CopyFile(env_, path_outside_db,
						  path_inside_db, 0,
						  db_options_.use_fsync);
			}
		} else {
			status = CopyFile(env_, path_outside_db, path_inside_db,
...
Status ExternalSstFileIngestionJob::Run()
{
	...
	if (ingestion_options_.snapshot_consistency &&
	    !db_snapshots_->empty()) {
	...
	for (IngestedFileInfo &f : files_to_ingest_) {
		...
		if (ingestion_options_.ingest_behind) {
			...
			status = AssignLevelAndSeqnoForIngestedFile(
				...
				cfd_->ioptions()->compaction_style, &f,
				&assigned_seqno);
		}
		if (!status.ok()) {
			return status;
		...
		edit_.AddFile(f.picked_level, f.fd.GetNumber(),
			      f.fd.GetPathId(), f.fd.GetFileSize(),
			      f.smallest_internal_key(),
...
void ExternalSstFileIngestionJob::UpdateStats()
{
	...
	cfd_->internal_stats()->AddCFStats(
		InternalStats::INGESTED_NUM_KEYS_TOTAL, total_keys);
	cfd_->internal_stats()->AddCFStats(
		InternalStats::INGESTED_NUM_FILES_TOTAL,
	...
	const std::string &external_file, IngestedFileInfo *file_to_ingest)
{
	...
	Status status =
		env_->GetFileSize(external_file, &file_to_ingest->file_size);
	if (!status.ok()) {
		return status;
	}
	...
	status = env_->NewRandomAccessFile(external_file, &sst_file,
					   env_options_);
	if (!status.ok()) {
		return status;
	}
	...
	status = cfd_->ioptions()->table_factory->NewTableReader(
		...
		std::move(sst_file_reader), file_to_ingest->file_size,
		&table_reader);
	if (!status.ok()) {
		return status;
	}
	...
	auto seqno_iter =
		uprops.find(ExternalSstFilePropertyNames::kGlobalSeqno);
...
Status
ExternalSstFileIngestionJob::IngestedFilesOverlapWithMemtables(SuperVersion *sv,
							       bool *overlap)
{
...
Status ExternalSstFileIngestionJob::AssignLevelAndSeqnoForIngestedFile(
	...
	CompactionStyle compaction_style, IngestedFileInfo *file_to_ingest,
	...
	*assigned_seqno = 0;
	...
	for (int lvl = 0; lvl < cfd_->NumberLevels(); lvl++) {
		...
		if (vstorage->NumLevelFiles(lvl) > 0) {
			...
			status = IngestedFileOverlapWithLevel(
				sv, file_to_ingest, lvl, &overlap_with_level);
			if (!status.ok()) {
				return status;
			}
	...
	IngestedFileInfo *file_to_ingest, SequenceNumber seqno)
{
	...
	Status status = env_->NewRandomRWFile(
		file_to_ingest->internal_file_path, &rwfile, env_options_);
	if (!status.ok()) {
		return status;
	}
	...
	bool *overlap)
{
	...
	InternalKey range_start(file_to_ingest->smallest_user_key,
	...
	*overlap = false;
	if (iter->Valid()) {
		...
		if (ucmp->Compare(seek_result.user_key,
...
Status ExternalSstFileIngestionJob::IngestedFileOverlapWithRangeDeletions(
	const IngestedFileInfo *file_to_ingest,
	InternalIterator *range_del_iter, bool *overlap)
{
	auto *vstorage = cfd_->current()->storage_info();
	auto *ucmp = vstorage->InternalComparator()->user_comparator();
	...
	*overlap = false;
	if (range_del_iter != nullptr) {
		for (range_del_iter->SeekToFirst(); range_del_iter->Valid();
			...
			if (!ParseInternalKey(range_del_iter->key(),
					      &parsed_key)) {
				return Status::Corruption(
					"corrupted range deletion key: " +
					range_del_iter->key().ToString());
			}
			RangeTombstone range_del(parsed_key,
						 range_del_iter->value());
	...
	bool *overlap_with_level)
{
	...
	std::unique_ptr<InternalIterator> level_range_del_iter(
		NewMergingIterator(
			&cfd_->internal_comparator(),
			level_range_del_iters.empty() ?
				      nullptr :
				      &level_range_del_iters[0],
	...
	if (status.ok() && *overlap_with_level == false) {
		status = IngestedFileOverlapWithRangeDeletions(
			file_to_ingest, level_range_del_iter.get(),
			overlap_with_level);
	}
	return status;
}
