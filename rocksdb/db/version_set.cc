		    ...
		    uint32_t left, uint32_t right)
{
	while (left < right) {
		...
		if (icmp.InternalKeyComparator::Compare(f.largest_key, key) <
		    0) {
...
class FilePicker {
	...
	FilePicker(std::vector<FileMetaData *> *files, const Slice &user_key,
		...
		: num_levels_(num_levels),
		  ...
		  is_hit_file_last_in_level_(false), user_key_(user_key),
		  ikey_(ikey), file_indexer_(file_indexer),
		  ...
		  internal_comparator_(internal_comparator)
	{
		...
		if (!search_ended_) {
			...
			for (unsigned int i = 0;
			     i < (*level_files_brief_)[0].num_files; ++i) {
				auto *r = (*level_files_brief_)[0]
						  .files[i]
						  .fd.table_reader;
				if (r) {
					r->Prepare(ikey);
	...
	FdWithKeyRange *GetNextFile()
	{
		while (!search_ended_) { // Loops over different levels.
			while (curr_index_in_curr_level_ <
			       curr_file_level_->num_files) {
				// Loops over all files in current level.
				FdWithKeyRange *f =
					&curr_file_level_->files
				...
				if (prev_file_) {
					if (curr_level_ != 0) {
						...
						assert(curr_index_in_curr_level_ >
						       0);
	...
	bool PrepareNextLevel()
	{
		...
		while (curr_level_ < num_levels_) {
			...
			if (curr_level_ == 0) {
				...
				} else if (search_left_bound_ <
					   search_right_bound_) {
					if (search_right_bound_ ==
					    FileIndexer::kLevelMaxIndex) {
						search_right_bound_ =
							static_cast<int32_t>(
								curr_file_level_
		...
		for (size_t i = 0; i < storage_info_.files_[level].size();
			...
			if (f->refs <= 0) {
				if (f->table_reader_handle) {
					cfd_->table_cache()->EraseHandle(
						f->fd, f->table_reader_handle);
	...
	virtual void Prev() override
	{
		...
		if (index_ == 0) {
			index_ = static_cast<uint32_t>(
				flevel_->num_files); // Marks as invalid
		} else {
			index_--;
		}
	}
...
class LevelFileIteratorState : public TwoLevelIteratorState {
	...
	LevelFileIteratorState(TableCache *table_cache,
		...
		: TwoLevelIteratorState(prefix_enabled),
		  ...
		  for_compaction_(for_compaction), skip_filters_(skip_filters),
		  level_(level), range_del_agg_(range_del_agg)
	{
	}

	InternalIterator *
	NewSecondaryIterator(const Slice &meta_handle) override
	{
		...
		const FileDescriptor *fd =
			reinterpret_cast<const FileDescriptor *>(
				meta_handle.data());
		return table_cache_->NewIterator(
			read_options_, env_options_, icomparator_, *fd,
			range_del_agg_,
			nullptr /* don't need reference to table */,
			file_read_hist_, for_compaction_, nullptr /* arena */,
...
class BaseReferencedVersionBuilder {
	...
	explicit BaseReferencedVersionBuilder(ColumnFamilyData *cfd)
		: version_builder_(new VersionBuilder(
			  cfd->current()->version_set()->env_options(),
				   ...
				   const std::string *fname) const
{
	...
	Status s = table_cache->GetTableProperties(vset_->env_options_,
	...
	if (fname != nullptr) {
		s = ioptions->env->NewRandomAccessFile(*fname, &file,
						       vset_->env_options_);
	} else {
					 ...
					 int level)
{
	for (const auto &file_meta : storage_info_.files_[level]) {
		auto fname = TableFileName(vset_->db_options_->db_paths,
					   file_meta->fd.GetNumber(),
					   file_meta->fd.GetPathId());
		// 1. If the table is already present in table cache, load table
		// properties from there.
		std::shared_ptr<const TableProperties> table_properties;
		Status s = GetTableProperties(&table_properties, file_meta,
					      &fname);
		if (s.ok()) {
...
Status
Version::GetPropertiesOfTablesInRange(const Range *range, std::size_t n,
				      TablePropertiesCollection *props) const
{
	for (int level = 0; level < storage_info_.num_non_empty_levels();
	     level++) {
		for (decltype(n) i = 0; i < n; i++) {
			...
			storage_info_.GetOverlappingInputs(
				level, &k1, &k2, &files, -1, nullptr, false);
			for (const auto &file_meta : files) {
				auto fname = TableFileName(
					vset_->db_options_->db_paths,
					file_meta->fd.GetNumber(),
					file_meta->fd.GetPathId());
				if (props->count(fname) == 0) {
					...
					std::shared_ptr<const TableProperties>
						table_properties;
					Status s = GetTableProperties(
						&table_properties, file_meta,
						&fname);
					if (s.ok()) {
						props->insert(
							{ fname,
...
size_t Version::GetMemoryUsageByTableReaders()
{
	...
	for (auto &file_level : storage_info_.level_files_brief_) {
		for (size_t i = 0; i < file_level.num_files; i++) {
			total_usage +=
				cfd_->table_cache()->GetMemoryUsageByTableReader(
					vset_->env_options_,
					cfd_->internal_comparator(),
...
void Version::GetColumnFamilyMetaData(ColumnFamilyMetaData *cf_meta)
{
	...
	for (int level = 0; level < cfd_->NumberLevels(); level++) {
		...
		cf_meta->levels.emplace_back(level, level_size,
					     std::move(files));
...
uint64_t VersionStorageInfo::GetEstimatedActiveKeys() const
{
	...
	if (current_num_samples_ < file_count) {
		...
		return static_cast<uint64_t>(
			(est * static_cast<double>(file_count) /
...
double VersionStorageInfo::GetEstimatedCompressionRatioAtLevel(int level) const
{
	...
	for (auto *file_meta : files_[level]) {
		...
		sum_data_size_bytes +=
			file_meta->raw_key_size + file_meta->raw_value_size;
	}
			   ...
			   RangeDelAggregator *range_del_agg)
{
	...
	for (int level = 0; level < storage_info_.num_non_empty_levels();
	     level++) {
		AddIteratorsForLevel(read_options, soptions, merge_iter_builder,
				     level, range_del_agg);
	}
}
				   ...
				   int level, RangeDelAggregator *range_del_agg)
{
	...
	if (level == 0) {
		...
		for (size_t i = 0;
		     i < storage_info_.LevelFilesBrief(0).num_files; i++) {
			const auto &file =
				storage_info_.LevelFilesBrief(0).files[i];
			merge_iter_builder->AddIterator(
				cfd_->table_cache()->NewIterator(
					read_options, soptions,
					cfd_->internal_comparator(), file.fd,
					range_del_agg, nullptr,
					cfd_->internal_stats()->GetFileReadHist(
						0),
					false, arena, false /* skip_filters */,
					0 /* level */));
		}
	} else {
		...
		// lazily.
		auto *mem =
			arena->AllocateAligned(sizeof(LevelFileIteratorState));
		...
		merge_iter_builder->AddIterator(NewTwoLevelIterator(
			state, first_level_iter, arena, false));
	}
}
	...
	std::vector<InternalIterator *> *range_del_iters)
{
	...
	for (size_t i = 0; i < storage_info_.LevelFilesBrief(level).num_files;
	     i++) {
		const auto &file =
			storage_info_.LevelFilesBrief(level).files[i];
		auto *range_del_iter =
...
VersionStorageInfo::VersionStorageInfo(
	...
	: internal_comparator_(internal_comparator),
	  ...
	  num_levels_(levels), num_non_empty_levels_(0),
	  file_indexer_(user_comparator), compaction_style_(compaction_style),
	  files_(new std::vector<FileMetaData *>[num_levels_]),
	  base_level_(num_levels_ == 1 ? -1 : 1),
	  files_by_compaction_pri_(num_levels_), level0_non_overlapping_(false),
	  next_file_to_compact_by_size_(num_levels_),
	  compaction_score_(num_levels_), compaction_level_(num_levels_),
	  l0_delay_trigger_count_(0), accumulated_file_size_(0),
	  accumulated_raw_key_size_(0), accumulated_raw_value_size_(0),
	  accumulated_num_non_deletions_(0), accumulated_num_deletions_(0),
	  current_num_non_deletions_(0), current_num_deletions_(0),
	  current_num_samples_(0), estimated_compaction_needed_bytes_(0),
	  finalized_(false),
	  force_consistency_checks_(_force_consistency_checks)
{
	if (ref_vstorage != nullptr) {
		...
		accumulated_raw_key_size_ =
			ref_vstorage->accumulated_raw_key_size_;
		accumulated_raw_value_size_ =
			ref_vstorage->accumulated_raw_value_size_;
		accumulated_num_non_deletions_ =
			ref_vstorage->accumulated_num_non_deletions_;
		accumulated_num_deletions_ =
			ref_vstorage->accumulated_num_deletions_;
		current_num_non_deletions_ =
			ref_vstorage->current_num_non_deletions_;
...
Version::Version(ColumnFamilyData *column_family_data, VersionSet *vset,
	...
	: env_(vset->env_), cfd_(column_family_data),
		  ...
		  bool *key_exists, SequenceNumber *seq)
{
	...
	GetContext get_context(
		...
		status->ok() ? GetContext::kNotFound : GetContext::kMerge,
		user_key, value, value_found, merge_context, range_del_agg,
		this->env_, seq, merge_operator_ ? &pinned_iters_mgr : nullptr);
	...
	FilePicker fp(storage_info_.files_, user_key, ikey,
	...
	while (f != nullptr) {
		*status = table_cache_->Get(
			read_options, *internal_comparator(), f->fd, ikey,
			&get_context,
			cfd_->internal_stats()->GetFileReadHist(
				fp.GetHitFileLevel()),
	...
	if (GetContext::kMerge == get_context.State()) {
		...
		std::string *str_value =
			value != nullptr ? value->GetSelf() : nullptr;
		*status = MergeHelper::TimedFullMerge(
			merge_operator_, user_key, nullptr,
			merge_context->GetOperands(), str_value, info_log_,
			db_statistics_, env_, nullptr /* result_operand */,
			true);
...
void VersionStorageInfo::GenerateLevelFilesBrief()
{
	...
	for (int level = 0; level < num_non_empty_levels_; level++) {
		DoGenerateLevelFilesBrief(&level_files_brief_[level],
					  files_[level], &arena_);
	}
}
			   ...
			   bool update_stats)
{
	...
	storage_info_.UpdateFilesByCompactionPri(
		cfd_->ioptions()->compaction_pri);
...
bool Version::MaybeInitializeFileMetaData(FileMetaData *file_meta)
{
	...
	file_meta->num_deletions =
		GetDeletedKeys(tp->user_collected_properties);
	...
	if (update_stats) {
		...
		for (int level = storage_info_.num_levels_ - 1;
			...
			for (int i = static_cast<int>(
					     storage_info_.files_[level].size()) -
				     1;
			     storage_info_.accumulated_raw_value_size_ == 0 &&
			     i >= 0;
			     --i) {
	...
	for (int level = 0; level < num_levels_; level++) {
		for (auto *file_meta : files_[level]) {
			...
			if (file_meta->compensated_file_size == 0) {
				...
				// shape of LSM tree.
				if (file_meta->num_deletions * 2 >=
				    file_meta->num_entries) {
					file_meta->compensated_file_size +=
						(file_meta->num_deletions * 2 -
						 file_meta->num_entries) *
						average_value_size *
	...
	const MutableCFOptions &mutable_cf_options)
{
	...
	for (int level = base_level(); level <= MaxInputLevel(); level++) {
		...
		if (level_size > level_target) {
			...
			if (bytes_next_level > 0) {
				assert(level_size > 0);
				estimated_compaction_needed_bytes_ += static_cast<
					uint64_t>(
					static_cast<double>(
	...
	for (int level = 0; level <= MaxInputLevel(); level++) {
		...
		if (level == 0) {
			...
			if (compaction_style_ == kCompactionStyleFIFO) {
				...
				if (immutable_cf_options.compaction_options_fifo
					    .allow_compaction) {
					score = std::max(
						static_cast<double>(
							num_sorted_runs) /
	...
	for (int level = 0; level <= last_qualify_level; level++) {
		for (auto *f : files_[level]) {
			if (!f->being_compacted && f->marked_for_compaction) {
				files_marked_for_compaction_.emplace_back(level,
									  f);
			}
		}
	}
	...
	if (level > 0 && !level_files->empty() &&
		...
		if (info_log != nullptr) {
			Error(info_log,
			      ...
			      f->largest.DebugString(true).c_str(), level,
			      f2->fd.GetNumber(),
	...
	std::vector<Fsize> *temp)
{
	...
	for (auto &file : files) {
		...
		while (next_level_it != next_level_files.end() &&
		       icmp.Compare((*next_level_it)->largest, file->smallest) <
			       0) {
			next_level_it++;
		}

		while (next_level_it != next_level_files.end() &&
		       icmp.Compare((*next_level_it)->smallest, file->largest) <
			       0) {
...
void VersionStorageInfo::UpdateFilesByCompactionPri(CompactionPri compaction_pri)
{
	if (compaction_style_ == kCompactionStyleFIFO ||
	...
	for (int level = 0; level < num_levels() - 1; level++) {
		...
		case kOldestLargestSeqFirst:
			std::sort(temp.begin(), temp.end(),
				  [](const Fsize &f1, const Fsize &f2) -> bool {
					  return f1.file->largest_seqno <
						 f2.file->largest_seqno;
				  });
			break;
		case kOldestSmallestSeqFirst:
			std::sort(temp.begin(), temp.end(),
				  [](const Fsize &f1, const Fsize &f2) -> bool {
					  return f1.file->smallest_seqno <
						 f2.file->smallest_seqno;
				  });
			break;
		case kMinOverlappingRatio:
			SortFileByOverlappingRatio(*internal_comparator_,
						   files_[level],
						   files_[level + 1], &temp);
			break;
		default:
...
void VersionStorageInfo::GenerateLevel0NonOverlapping()
{
	...
	std::sort(level0_sorted_file.begin(), level0_sorted_file.end(),
		  [this](const FdWithKeyRange &f1,
			 const FdWithKeyRange &f2) -> bool {
			  return (internal_comparator_->Compare(
					  f1.smallest_key, f2.smallest_key) <
				  0);
		  });
	...
	for (size_t i = 1; i < level0_sorted_file.size(); ++i) {
		...
		if (internal_comparator_->Compare(prev.largest_key,
						  f.smallest_key) >= 0) {
	...
	bool expand_range) const
{
	...
	if (begin != nullptr && end != nullptr && level > 0) {
		GetOverlappingInputsRangeBinarySearch(level, user_begin,
						      user_end, inputs,
	...
	for (size_t i = 0; i < level_files_brief_[level].num_files;) {
		...
		if (begin != nullptr &&
			...
			if (level == 0 && expand_range) {
				...
				if (begin != nullptr &&
				    user_cmp->Compare(file_start, user_begin) <
					    0) {
					user_begin = file_start;
					inputs->clear();
					i = 0;
				} else if (end != nullptr &&
					   user_cmp->Compare(file_limit,
							     user_end) > 0) {
					user_end = file_limit;
					inputs->clear();
					i = 0;
	...
	bool within_interval) const
{
	...
	if (within_interval) {
		...
		ExtendFileRangeOverlappingInterval(level, user_begin, user_end,
						   mid, &start_index,
						   &end_index);
	}
	...
	unsigned int mid_index, int *start_index, int *end_index) const
{
	...
	while (left <= right) {
		const Slice &first_key_in_range =
			ExtractUserKey(files[left].smallest_key);
		...
		if (left > 0) { // If not first file
			...
			if (user_cmp->Equal(first_key_in_range,
					    last_key_before)) {
	...
	while (left <= right) {
		const Slice last_key_in_range =
			ExtractUserKey(files[right].largest_key);
		if (user_cmp->Compare(last_key_in_range, user_end) > 0) {
			right--;
			continue;
		}
		if (right <
		    static_cast<int>(level_files_brief_[level].num_files) -
			    1) { // If not the last file
			const Slice first_key_after =
				ExtractUserKey(files[right + 1].smallest_key);
			if (user_cmp->Equal(last_key_in_range,
					    first_key_after)) {
...
const char *VersionStorageInfo::LevelSummary(LevelSummaryStorage *scratch) const
{
	int len = 0;
	if (compaction_style_ == kCompactionStyleLevel && num_levels() > 1) {
		assert(base_level_ < static_cast<int>(level_max_bytes_.size()));
		len = snprintf(scratch->buffer, sizeof(scratch->buffer),
			       "base level %d max bytes base %" PRIu64 " ",
			       base_level_, level_max_bytes_[base_level_]);
	}
...
const char *VersionStorageInfo::LevelFileSummary(FileSummaryStorage *scratch,
						 int level) const
{
	int len = snprintf(scratch->buffer, sizeof(scratch->buffer),
			   "files_size[");
...
int64_t VersionStorageInfo::MaxNextLevelOverlappingBytes()
{
	...
	for (int level = 1; level < num_levels() - 1; level++) {
		for (const auto &f : files_[level]) {
			GetOverlappingInputs(level + 1, &f->smallest,
					     &f->largest, &overlaps);
					    ...
					    const MutableCFOptions &options)
{
	...
	if (!ioptions.level_compaction_dynamic_level_bytes) {
		base_level_ =
			(ioptions.compaction_style == kCompactionStyleLevel) ?
				      1 :
				      -1;

		// Calculate for static bytes base case
		for (int i = 0; i < ioptions.num_levels; ++i) {
			if (i == 0 && ioptions.compaction_style ==
				...
				level_max_bytes_[i] = MultiplyCheckOverflow(
					...
					options.MaxBytesMultiplerAdditional(i -
									    1));
			} else {
				level_max_bytes_[i] =
					options.max_bytes_for_level_base;
		...
		if (max_level_size == 0) {
			...
			for (int i = num_levels_ - 2;
			     i >= first_non_empty_level; i--) {
			...
			for (int i = base_level_; i < num_levels_; i++) {
				...
				level_max_bytes_[i] =
					std::max(level_size, base_bytes_max);
			}
		}
	}
}
...
uint64_t VersionStorageInfo::EstimateLiveDataSize() const
{
	...
	std::map<InternalKey *, FileMetaData *, decltype(ikey_lt)> ranges(
		ikey_lt);
...
VersionSet::VersionSet(const std::string &dbname,
	...
	: column_family_set_(new ColumnFamilySet(
	  ...
	  env_(db_options->env), dbname_(dbname), db_options_(db_options),
	  next_file_number_(2), manifest_file_number_(0), // Filled by Recover()
	  pending_manifest_file_number_(0), last_sequence_(0),
	  prev_log_number_(0), current_version_number_(0),
	  manifest_file_size_(0), env_options_(storage_options),
	  env_options_compactions_(env_->OptimizeForCompactionTableRead(
...
VersionSet::~VersionSet()
{
	...
	// VersionSet
	column_family_set_->get_table_cache()->ApplyToAllCacheEntries(
		&CloseTables, false);
...
void VersionSet::AppendVersion(ColumnFamilyData *column_family_data, Version *v)
{
			       ...
			       const ColumnFamilyOptions *new_cf_options)
{
	...
	if (w.edit_list.front()->IsColumnFamilyManipulation()) {
		...
		v = new Version(column_family_data, this,
				current_version_number_++);
		builder_guard.reset(
			new BaseReferencedVersionBuilder(column_family_data));
		auto *builder = builder_guard->version_builder();
		for (const auto &writer : manifest_writers_) {
			if (writer->edit_list.front()
			...
			for (const auto &edit : writer->edit_list) {
				LogAndApplyHelper(column_family_data, builder,
						  v, edit, mu);
	...
	// because &w is ensuring that all new writes get queued.
	{
			...
			if (s.ok()) {
				...
				unique_ptr<WritableFileWriter> file_writer(
					new WritableFileWriter(
						std::move(descriptor_file),
						opt_env_opts));
				descriptor_log_.reset(new log::Writer(
					std::move(file_writer), 0, false));
		...
		if (s.ok()) {
			for (auto &e : batch_edits) {
				...
				TEST_KILL_RANDOM(
					"VersionSet::LogAndApply:BeforeAddRecord",
			...
			if (s.ok()) {
				s = SyncManifest(env_, db_options_,
						 descriptor_log_->file());
			}
			if (!s.ok()) {
	...
	if (s.ok()) {
		if (w.edit_list.front()->is_column_family_add_) {
			...
			for (auto &e : batch_edits) {
				if (e->has_log_number_) {
					max_log_number_in_batch = std::max(
		...
		if (new_descriptor_log) {
			ROCKS_LOG_INFO(db_options_->info_log,
				       "Deleting manifest %" PRIu64
				       " current manifest %" PRIu64 "\n",
				       manifest_file_number_,
				       pending_manifest_file_number_);
			descriptor_log_.reset();
			env_->DeleteFile(DescriptorFileName(
				dbname_, pending_manifest_file_number_));
		}
	}
...
Status
VersionSet::Recover(const std::vector<ColumnFamilyDescriptor> &column_families,
		    bool read_only)
{
	...
	Status s = ReadFileToString(env_, CurrentFileName(dbname_),
				    &manifest_filename);
	if (!s.ok()) {
		return s;
	}
...

	{
		...
		while (reader.ReadRecord(&record, &scratch) && s.ok()) {
			...
			bool cf_in_not_found = column_families_not_found.find(
						       edit.column_family_) !=
					       column_families_not_found.end();
			// in builders means that user supplied that column family
			// option AND that we encountered column family add record
			bool cf_in_builders =
				builders.find(edit.column_family_) !=
				builders.end();
			...
			if (edit.is_column_family_add_) {
				...
				auto cf_options = cf_name_to_options.find(
				...
				if (cf_options == cf_name_to_options.end()) {
					...
					cfd = CreateColumnFamily(
						cf_options->second, &edit);
					cfd->set_initialized();
					builders.insert(
						{ edit.column_family_,
				...
				if (cf_in_builders) {
					auto builder = builders.find(
						edit.column_family_);
					assert(builder != builders.end());
					delete builder->second;
					builders.erase(builder);
					cfd = column_family_set_->GetColumnFamily(
						edit.column_family_);
					if (cfd->Unref()) {
						delete cfd;
						cfd = nullptr;
					...
					column_families_not_found.erase(
						edit.column_family_);
				} else {
					s = Status::Corruption(
				...
				assert(cfd != nullptr);
				if (edit.max_level_ >= cfd->current()
							       ->storage_info()
							       ->num_levels()) {
				...
				auto builder =
					builders.find(edit.column_family_);
				assert(builder != builders.end());
				builder->second->version_builder()->Apply(
					&edit);
			}
	...
	if (s.ok()) {
		for (auto cfd : *column_family_set_) {
			...
			auto *builder =
				builders_iter->second->version_builder();

			if (GetColumnFamilySet()
				    ->get_table_cache()
				    ->GetCapacity() ==
			    TableCache::kInfiniteCapacity) {
				// unlimited table cache. Pre-load table handle now.
			...
			Version *v = new Version(cfd, this,
						 current_version_number_++);
			builder->SaveTo(v->storage_info());
					...
					int new_levels)
{
	...
	std::shared_ptr<Cache> tc(
		NewLRUCache(options->max_open_files - 10,
			    options->table_cache_numshardbits));
	WriteController wc(options->delayed_write_rate);
	WriteBufferManager wb(options->db_write_buffer_size);
	VersionSet versions(dbname, &db_options, env_options, tc.get(), &wb,
			    &wc);
	Status status;
		...
		if (file_num != 0) {
			if (first_nonempty_level < 0) {
				...
				snprintf(
					...
					first_nonempty_level_filenum, i,
					file_num);
	...
	if (first_nonempty_level > 0) {
		new_files_list[new_levels - 1] =
			vstorage->LevelFiles(first_nonempty_level);
	}
				...
				bool verbose, bool hex, bool json)
{
	...
	default_cf_edit.AddColumnFamily(kDefaultColumnFamilyName);
	default_cf_edit.SetColumnFamily(0);
	ColumnFamilyData *default_cfd = CreateColumnFamily(
		ColumnFamilyOptions(options), &default_cf_edit);
	builders.insert({ 0, new BaseReferencedVersionBuilder(default_cfd) });

	{
		...
		while (reader.ReadRecord(&record, &scratch) && s.ok()) {
			...
			if (edit.has_comparator_) {
				comparators.insert({ edit.column_family_,
						     edit.comparator_ });
			}
			...
			if (edit.is_column_family_add_) {
				...
				cfd = CreateColumnFamily(
					ColumnFamilyOptions(options), &edit);
				cfd->set_initialized();
				builders.insert(
					{ edit.column_family_,
					  new BaseReferencedVersionBuilder(
	...
	if (s.ok()) {
		...
		printf("next_file_number %lu last_sequence "
...
Status VersionSet::WriteSnapshot(log::Writer *log)
{
	...
	for (auto cfd : *column_family_set_) {
		...
		assert(cfd->initialized());
		{
			...
			edit.SetComparatorName(cfd->internal_comparator()
						       .user_comparator()
						       ->Name());
			std::string record;
...

		{
			...
			for (int level = 0; level < cfd->NumberLevels();
			     level++) {
				for (const auto &f :
				     cfd->current()->storage_info()->LevelFiles(
					     level)) {
				     ...
				     int end_level)
{
	...
	for (int level = start_level; level < end_level; level++) {
		const LevelFilesBrief &files_brief =
			vstorage->LevelFilesBrief(level);
		...
		if (!level) {
			...
			size += ApproximateSizeLevel0(v, files_brief, start,
						      end);
			continue;
		}
		...
		for (uint64_t i = idx_start; i < files_brief.num_files; i++) {
			...
			if (i == idx_start) {
				...
				// key
				val = ApproximateSize(v, files_brief.files[i],
						      start);
				assert(size >= val);
				size -= val;
			}
					   ...
					   const Slice &key_end)
{
	...
	for (size_t i = 0; i < files_brief.num_files; i++) {
		const uint64_t start =
			ApproximateSize(v, files_brief.files[i], key_start);
		const uint64_t end =
			ApproximateSize(v, files_brief.files[i], key_end);
		assert(end >= start);
		size += end - start;
	}
	return size;
				     ...
				     const Slice &key)
{
	...
	if (v->cfd_->internal_comparator().Compare(f.largest_key, key) <= 0) {
	...
	} else if (v->cfd_->internal_comparator().Compare(f.smallest_key, key) >
		   0) {
		...
		InternalIterator *iter = v->cfd_->table_cache()->NewIterator(
			ReadOptions(), env_options_,
			v->cfd_->internal_comparator(), f.fd,
...
void VersionSet::AddLiveFiles(std::vector<FileDescriptor> *live_list)
{
	...
	live_list->reserve(live_list->size() +
			   static_cast<size_t>(total_files));
...
InternalIterator *
VersionSet::MakeInputIterator(const Compaction *c,
			      RangeDelAggregator *range_del_agg)
{
	...
	const size_t space =
		(c->level() == 0 ? c->input_levels(0)->num_files +
	...
	assert(num <= space);
	InternalIterator *result = NewMergingIterator(
		...
		for (int level = 0; level < cfd->NumberLevels(); level++) {
			for (const auto &file :
				...
				if (path_id < db_options_->db_paths.size()) {
					filemetadata.db_path =
						db_options_->db_paths[path_id]
							.path;
				} else {
					assert(!db_options_->db_paths.empty());
					filemetadata.db_path =
						db_options_->db_paths.back()
							.path;
				}
				...
				filemetadata.largestkey =
					file->largest.user_key().ToString();
				filemetadata.smallest_seqno =
					file->smallest_seqno;
				filemetadata.largest_seqno =
					file->largest_seqno;
...
ColumnFamilyData *
VersionSet::CreateColumnFamily(const ColumnFamilyOptions &cf_options,
			       VersionEdit *edit)
{
	...
	v->storage_info()->CalculateBaseBytes(
	...
	for (Version *v = dummy_versions->next_; v != dummy_versions;
		...
		for (int level = 0; level < storage_info->num_levels_;
			...
			for (const auto &file_meta :
			     storage_info->LevelFiles(level)) {
				if (unique_files.find(
					    file_meta->fd
