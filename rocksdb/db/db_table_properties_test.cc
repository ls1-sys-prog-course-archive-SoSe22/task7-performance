...
class DBTablePropertiesTest : public DBTestBase {
	...
	TestGetPropertiesOfTablesInRange(std::vector<Range> ranges,
					 std::size_t *num_properties = nullptr,
...
TEST_F(DBTablePropertiesTest, GetPropertiesOfTablesInRange)
{
	...
	for (int i = 0; i < 10000; i++) {
		ASSERT_OK(
			Put(test::RandomKey(&rnd, 5), RandomString(&rnd, 102)));
	}
	Flush();
	...
	TestGetPropertiesOfTablesInRange(
		{ Range(test::RandomKey(&rnd, 5, test::RandomKeyType::SMALLEST),
			test::RandomKey(&rnd, 5,
					test::RandomKeyType::LARGEST)) },
...
TEST_F(DBTablePropertiesTest, GetColumnFamilyNameProperty)
{
	...
	for (int cf = 0; cf < 2; ++cf) {
		...
		ASSERT_OK(db_->GetPropertiesOfAllTables(handles_[cf],
							&fname_to_props));
