...
TEST_F(DBIteratorTest, IteratorProperty)
{
	...
	ropt.pin_data = false;
	{
		...
		ASSERT_NOK(
			iter->GetProperty("non_existing.value", &prop_value));
		ASSERT_OK(iter->GetProperty("rocksdb.iterator.is-key-pinned",
					    &prop_value));
...
TEST_F(DBIteratorTest, PersistedTierOnIterator)
{
	...
	ASSERT_TRUE(db_->NewIterators(ropt, { handles_[1] }, &iters)
			    .IsNotSupported());
	Close();
}
...
TEST_F(DBIteratorTest, NonBlockingIteration)
{
	do {
		...
		// it is in memtable.
		Iterator *iter =
			db_->NewIterator(non_blocking_opts, handles_[1]);
		int count = 0;
...
TEST_F(DBIteratorTest, IterNextWithNewerSeq)
{
	...
	// Create a key that needs to be skipped for Seq too new
	for (uint64_t i = 0;
	     i < last_options_.max_sequential_skip_in_iterations + 1; i++) {
		ASSERT_OK(Put("b", "f"));
	}
	...
	do {
		...
		if (!CurrentOptions().merge_operator) {
			...
			if (kPlainTableAllBytesPrefix != option_config_ &&
			    ...
			    kHashSkipList !=
				    option_config_) { // doesn't support SeekToLast
				iter->Prev();
...
TEST_F(DBIteratorTest, PinnedDataIteratorRandomized)
{
	...
	for (int run_config = 0; run_config < TestConfig::MAX; run_config++) {
		...
		for (int i = 0; i < puts; i++) {
			...
			true_data[k] = v;
			if (rnd.OneIn(static_cast<int>(100.0 /
						       merge_percentage))) {
...

		{
			...
			for (auto &k : random_keys) {
				iter->Seek(k);
				if (!iter->Valid()) {
					ASSERT_EQ(true_data.lower_bound(k),
						  true_data.end());
					continue;
				}
				std::string prop_value;
				ASSERT_OK(iter->GetProperty(
					"rocksdb.iterator.is-key-pinned",
					&prop_value));
				ASSERT_EQ("1", prop_value);
				keys_slices.push_back(iter->key());
				true_keys.push_back(
					true_data.lower_bound(k)->first);
			}

			for (size_t i = 0; i < keys_slices.size(); i++) {
				ASSERT_EQ(keys_slices[i].ToString(),
					  true_keys[i]);
			}
		}

		{
			...
			for (auto &k : random_keys) {
				iter->SeekForPrev(k);
				if (!iter->Valid()) {
					ASSERT_EQ(true_data.upper_bound(k),
						  true_data.begin());
					continue;
				}
				std::string prop_value;
...
TEST_F(DBIteratorTest, IterPrevKeyCrossingBlocks)
{
...

	{
		...
		ASSERT_EQ(iter->value().ToString(),
			  "val3.0,val3.1,val3.2,val3.3,val3.4");
...
TEST_F(DBIteratorTest, IterPrevKeyCrossingBlocksRandomized)
{
...

	{
		...
		for (iter->SeekToLast(); iter->Valid(); iter->Prev()) {
			...
			int next_count = entries_right > 0 ?
						       rnd.Uniform(std::min(
							 entries_right, 10)) :
						       0;
...
TEST_F(DBIteratorTest, IteratorWithLocalStatistics)
{
	...
	ASSERT_EQ(TestGetTickerCount(options, NUMBER_DB_NEXT),
		  (uint64_t)total_next);
	ASSERT_EQ(TestGetTickerCount(options, NUMBER_DB_NEXT_FOUND),
		  (uint64_t)total_next_found);
	ASSERT_EQ(TestGetTickerCount(options, NUMBER_DB_PREV),
		  (uint64_t)total_prev);
