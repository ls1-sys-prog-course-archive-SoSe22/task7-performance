...
class Repairer {
	...
	Repairer(const std::string &dbname, const DBOptions &db_options,
		...
		: dbname_(dbname), env_(db_options.env), env_options_(),
		  ...
		  default_cf_iopts_(ImmutableCFOptions(immutable_db_options_,
						       default_cf_opts)),
		  ...
		  raw_table_cache_(
			  ...
			  // once.
			  NewLRUCache(10,
				      db_options_.table_cache_numshardbits)),
	...
	GetColumnFamilyOptions(const std::string &cf_name)
	{
	...
	Status AddColumnFamily(const std::string &cf_name, uint32_t cf_id)
	{
		...
		if (cf_opts == nullptr) {
			return Status::Corruption(
				"Encountered unknown column family with name=" +
		...
		mutex_.Lock();
		Status status =
			vset_.LogAndApply(cfd, mut_cf_opts, &edit, &mutex_,
	...
	Status Run()
	{
		...
		if (status.ok()) {
			...
			status = vset_.Recover({ { kDefaultColumnFamilyName,
						   default_cf_opts_ } },
					       false);
		}
		if (status.ok()) {
	...
	const ImmutableCFOptions
		default_cf_iopts_; // table_cache_ holds reference
	...
	Status FindFiles()
	{
		...
		for (size_t path_id = 0; path_id < db_options_.db_paths.size();
		     path_id++) {
			Status status = env_->GetChildren(
				db_options_.db_paths[path_id].path, &filenames);
			if (!status.ok()) {
				return status;
			}
			if (!filenames.empty()) {
		...
		if (!found_file) {
			return Status::Corruption(dbname_,
	...
	Status ConvertLogToTable(uint64_t log)
	{
		...
		// numbers).
		log::Reader reader(db_options_.info_log,
				   std::move(lfile_reader), &reporter,
		...
		auto cf_mems = new ColumnFamilyMemTablesImpl(
			vset_.GetColumnFamilySet());
		...
		while (reader.ReadRecord(&record, &scratch)) {
			...
			status = WriteBatchInternal::InsertInto(&batch, cf_mems,
								nullptr);
			if (status.ok()) {
				counter += WriteBatchInternal::Count(&batch);
			} else {
				ROCKS_LOG_WARN(db_options_.info_log,
					       "Log #%" PRIu64 ": ignoring %s",
					       log, status.ToString().c_str());
				status = Status::
					OK(); // Keep going with rest of file
			}
		}
		...
		for (auto *cfd : *vset_.GetColumnFamilySet()) {
			...
			status = BuildTable(
				...
				*cfd->GetLatestMutableCFOptions(),
				...
				kMaxSequenceNumber, kNoCompression,
	...
	{
		std::string fname = TableFileName(db_options_.db_paths,
						  t->meta.fd.GetNumber(),
						  t->meta.fd.GetPathId());
		int counter = 0;
		uint64_t file_size;
		...
		if (status.ok()) {
			status = table_cache_->GetTableProperties(
				env_options_, icmp_, t->meta.fd, &props);
		}
		if (status.ok()) {
			t->column_family_id =
	...
	Status AddTables()
	{
		...
		for (size_t i = 0; i < tables_.size(); i++) {
			cf_id_to_tables[tables_[i].column_family_id].push_back(
				&tables_[i]);
		...
		for (const auto &cf_id_and_tables : cf_id_to_tables) {
			...
			for (const auto *table : cf_id_and_tables.second) {
				edit.AddFile(0, table->meta.fd.GetNumber(),
					     table->meta.fd.GetPathId(),
					     table->meta.fd.GetFileSize(),
					     table->meta.smallest,
					     table->meta.largest,
					     table->min_sequence,
					     table->max_sequence,
...
Status
GetDefaultCFOptions(const std::vector<ColumnFamilyDescriptor> &column_families,
		    ColumnFamilyOptions *res)
{
	...
	if (iter == column_families.end()) {
		return Status::InvalidArgument(
			"column_families",
			"Must contain entry for default column family");
	}
	*res = iter->options;
	return Status::OK();
}
		...
		const ColumnFamilyOptions &unknown_cf_opts)
{
	...
	if (status.ok()) {
		Repairer repairer(dbname, db_options, column_families,
				  default_cf_opts, unknown_cf_opts,
