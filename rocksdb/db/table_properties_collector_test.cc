...
class RegularKeysStartWithABackwardCompatible
	...
	Status Finish(UserCollectedProperties *properties) override
	{
		std::string encoded;
		PutVarint32(&encoded, count_);
		*properties = UserCollectedProperties{ { "TablePropertiesTest",
							 "Rocksdb" },
						       { "Count", encoded } };
		return Status::OK();
	}
...
class RegularKeysStartWithAFactory : public IntTblPropCollectorFactory,
	...
	CreateIntTblPropCollector(uint32_t column_family_id) override
	{
...
void TestCustomizedTablePropertiesCollector(
	bool backward_mode, uint64_t magic_number,
	bool test_int_tbl_prop_collector, const Options &options,
	const InternalKeyComparator &internal_comparator)
{
	...
	if (test_int_tbl_prop_collector) {
		...
		GetIntTblPropCollectorFactory(
			ioptions, &int_tbl_prop_collector_factories);
	}
	...
	Status s = ReadTableProperties(fake_file_reader.get(),
				       fwf->contents().size(), magic_number,
				       ioptions, &props);
	...
	ASSERT_NE(user_collected.find("TablePropertiesTest"),
		  user_collected.end());
	...
	if (!backward_mode && !test_int_tbl_prop_collector) {
		...
		ASSERT_NE(user_collected.find("NumDeletes"),
			  user_collected.end());
		...
		ASSERT_TRUE(
			GetVarint32(&key_single_deletes, &num_single_deletes));
		ASSERT_EQ(1u, num_single_deletes);

		uint32_t num_size_changes;
		ASSERT_NE(user_collected.find("NumSizeChanges"),
			  user_collected.end());
...
TEST_P(TablePropertiesTest, CustomizedTablePropertiesCollector)
{
	...
	for (bool encode_as_internal : { true, false }) {
		...
		std::shared_ptr<TablePropertiesCollectorFactory>
			collector_factory(new RegularKeysStartWithAFactory(
				backward_mode_));
		...
		TestCustomizedTablePropertiesCollector(
			backward_mode_, kBlockBasedTableMagicNumber,
		...
		options.table_factory = std::make_shared<PlainTableFactory>(
			plain_table_options);
	...
	std::shared_ptr<TableFactory> table_factory)
{
	...
	for (int iter = 0; iter < 2; ++iter) {
		MakeBuilder(options, ioptions, pikc,
			    &int_tbl_prop_collector_factories, &writable,
			    &builder);
		...
		test::StringSink *fwf = static_cast<test::StringSink *>(
			writable->writable_file());
		unique_ptr<RandomAccessFileReader> reader(
			test::GetRandomAccessFileReader(
				new test::StringSource(fwf->contents())));
		TableProperties *props;
		Status s = ReadTableProperties(reader.get(),
					       fwf->contents().size(),
		...
		uint64_t merges =
			GetMergeOperands(user_collected, &property_present);
		ASSERT_TRUE(property_present);
		ASSERT_EQ(2u, merges);

		if (sanitized) {
			uint32_t starts_with_A = 0;
			ASSERT_NE(user_collected.find("Count"),
				  user_collected.end());
			Slice key(user_collected.at("Count"));
			ASSERT_TRUE(GetVarint32(&key, &starts_with_A));
			...
			if (!backward_mode) {
				uint32_t num_puts;
				ASSERT_NE(user_collected.find("NumPuts"),
					  user_collected.end());
				Slice key_puts(user_collected.at("NumPuts"));
				ASSERT_TRUE(GetVarint32(&key_puts, &num_puts));
				...
				ASSERT_NE(user_collected.find("NumDeletes"),
					  user_collected.end());
				Slice key_deletes(
					user_collected.at("NumDeletes"));
				ASSERT_TRUE(GetVarint32(&key_deletes,
							&num_deletes));
				ASSERT_EQ(4u, num_deletes);

				uint32_t num_single_deletes;
				ASSERT_NE(
					user_collected.find("NumSingleDeletes"),
					user_collected.end());
				Slice key_single_deletes(
					user_collected.at("NumSingleDeletes"));
				ASSERT_TRUE(GetVarint32(&key_single_deletes,
							&num_single_deletes));
				ASSERT_EQ(1u, num_single_deletes);
			}
		}
...
TEST_P(TablePropertiesTest, InternalKeyPropertiesCollector)
{
	...
	TestInternalKeyPropertiesCollector(
		backward_mode_, kPlainTableMagicNumber,
		false /* not sanitize */,
