...
static bool ValidateCompressionLevel(const char *flagname, int32_t value)
{
	if (value < -1 || value > 9) {
		fprintf(stderr,
			"Invalid value for --%s: %d, must be between -1 and 9\n",
			flagname, value);
		return false;
	}
	return true;
...
static const bool FLAGS_compression_level_dummy __attribute__((unused)) =
	RegisterFlagValidator(&FLAGS_compression_level,
...
static bool ValidateTableCacheNumshardbits(const char *flagname, int32_t value)
{
	if (0 >= value || value > 20) {
		fprintf(stderr,
...
DEFINE_uint64(time_range, 100000,
...
class ReportFileOpEnv : public EnvWrapper {
				 ...
				 const EnvOptions &soptions) override
	{
		class CountingFile : public SequentialFile {
			...
			{
				counters_->read_counter_.fetch_add(
					1, std::memory_order_relaxed);
				Status rv = target_->Read(n, result, scratch);
				counters_->bytes_read_.fetch_add(
					result->size(),
		...
		if (s.ok()) {
			counters()->open_counter_.fetch_add(
				1, std::memory_order_relaxed);
				   ...
				   const EnvOptions &soptions) override
	{
		class CountingFile : public RandomAccessFile {
					    ...
					    char *scratch) const override
			{
				counters_->read_counter_.fetch_add(
					1, std::memory_order_relaxed);
				Status rv = target_->Read(offset, n, result,
							  scratch);
				counters_->bytes_read_.fetch_add(
					result->size(),
					std::memory_order_relaxed);
			       ...
			       const EnvOptions &soptions) override
	{
		class CountingFile : public WritableFile {
			...
			{
				counters_->append_counter_.fetch_add(
					1, std::memory_order_relaxed);
				Status rv = target_->Append(data);
				counters_->bytes_written_.fetch_add(
...
class RandomGenerator {
	...
	RandomGenerator()
	{
		...
		std::string piece;
		while (data_.size() <
		       (unsigned)std::max(1048576, FLAGS_value_size)) {
			// Add a short fragment that is as compressible as specified
			// by FLAGS_compression_ratio.
			test::CompressibleString(&rnd, FLAGS_compression_ratio,
						 100, &piece);
			data_.append(piece);
		}
		pos_ = 0;
...
struct DBWithColumnFamilies {
	...
	{
		assert(num_hot > 0);
		return cfh[num_created.load(std::memory_order_acquire) -
			   num_hot + rand_num % num_hot];
	}
	...
	void CreateNewCf(ColumnFamilyOptions options, int64_t stage)
	{
		...
		for (size_t i = num_created; i < new_num_created; i++) {
			Status s = db->CreateColumnFamily(
				options, ColumnFamilyName(i), &(cfh[i]));
			if (!s.ok()) {
				fprintf(stderr,
...
class ReporterAgent {
	...
	void SleepAndReport()
	{
		...
		while (true) {
			...
			auto secs_elapsed = (env_->NowMicros() - time_started +
					     kMicrosInSecond / 2) /
					    kMicrosInSecond;
			std::string report = ToString(secs_elapsed) + "," +
					     ToString(total_ops_done_snapshot -
						      last_report_) +
					     "\n";
...
class Stats {
	...
	void Start(int id)
	{
		id_ = id;
		next_report_ =
			FLAGS_stats_interval ? FLAGS_stats_interval : 100;
	...
	void Merge(const Stats &other)
	{
		...
		for (auto it = other.hist_.begin(); it != other.hist_.end();
		     ++it) {
			auto this_it = hist_.find(it->first);
			if (this_it != hist_.end()) {
				this_it->second->Merge(
					*(other.hist_.at(it->first)));
			} else {
	...
	void PrintThreadStatus()
	{
		...
		for (auto ts : thread_list) {
			fprintf(stderr,
				...
				ts.thread_id,
				ThreadStatus::GetThreadTypeName(ts.thread_type)
					.c_str(),
				ts.cf_name.c_str(),
				ThreadStatus::GetOperationName(
					ts.operation_type)
					.c_str(),
				ThreadStatus::MicrosToString(
					ts.op_elapsed_micros)
					.c_str(),
				ThreadStatus::GetOperationStageName(
					ts.operation_stage)
					.c_str(),
				ThreadStatus::GetStateName(ts.state_type)
					.c_str());
	...
	void FinishedOps(DBWithColumnFamilies *db_with_cfh, DB *db,
			 int64_t num_ops, enum OperationType op_type = kOthers)
	{
		if (reporter_agent_) {
		...
		if (done_ >= next_report_) {
			if (!FLAGS_stats_interval) {
				if (next_report_ < 1000)
					...
					next_report_ += 100000;
				fprintf(stderr,
					"... finished %" PRIu64 " ops%30s\r",
					done_, "");
			} else {
				uint64_t now = FLAGS_env->NowMicros();
				int64_t usecs_since_last =
					now - last_report_finish_;
				...
				if (FLAGS_stats_interval_seconds &&
					...
					if (id_ == 0 &&
					    FLAGS_stats_per_interval) {
						std::string stats;

						if (db_with_cfh &&
						    db_with_cfh->num_created
							    .load()) {
							for (size_t i = 0;
							     i <
							     db_with_cfh
...
class Benchmark {
	...
	inline bool CompressSlice(const Slice &input, std::string *compressed)
	{
		...
		case rocksdb::kSnappyCompression:
			ok = Snappy_Compress(Options().compression_opts,
					     input.data(), input.size(),
					     compressed);
			break;
		case rocksdb::kZlibCompression:
			ok = Zlib_Compress(Options().compression_opts, 2,
					   input.data(), input.size(),
					   compressed);
			break;
		case rocksdb::kBZip2Compression:
			ok = BZip2_Compress(Options().compression_opts, 2,
					    input.data(), input.size(),
					    compressed);
			break;
		case rocksdb::kLZ4Compression:
			ok = LZ4_Compress(Options().compression_opts, 2,
					  input.data(), input.size(),
					  compressed);
			break;
		case rocksdb::kLZ4HCCompression:
			ok = LZ4HC_Compress(Options().compression_opts, 2,
					    input.data(), input.size(),
					    compressed);
			break;
		case rocksdb::kXpressCompression:
			ok = XPRESS_Compress(input.data(), input.size(),
					     compressed);
			break;
		case rocksdb::kZSTD:
			ok = ZSTD_Compress(Options().compression_opts,
					   input.data(), input.size(),
					   compressed);
			break;
		default:
	...
	void PrintHeader()
	{
		...
		fprintf(stdout, "RawSize:    %.1f MB (estimated)\n",
			((static_cast<int64_t>(FLAGS_key_size +
					       FLAGS_value_size) *
			  num_) /
			 1048576.0));
		fprintf(stdout, "FileSize:   %.1f MB (estimated)\n",
			(((FLAGS_key_size +
			   FLAGS_value_size * FLAGS_compression_ratio) *
			  num_) /
			 1048576.0));
	...
	void PrintWarnings(const char *compression)
	{
		...
		if (FLAGS_compression_type_e != rocksdb::kNoCompression) {
			...
			bool result =
				CompressSlice(Slice(input_str), &compressed);
...
// Current the following isn't equivalent to OS_LINUX.
	...
	void PrintEnvironment()
	{
		...
		if (cpuinfo != nullptr) {
			...
			while (fgets(line, sizeof(line), cpuinfo) != nullptr) {
				...
				Slice key =
					TrimSpace(Slice(line, sep - 1 - line));
	...
	class ExpiredTimeFilter : public CompactionFilter {
		...
		explicit ExpiredTimeFilter(
			const std::shared_ptr<TimestampEmulator>
				&timestamp_emulator)
	...
	Benchmark()
		: cache_(NewCache(FLAGS_cache_size)),
		  ...
		  readwrites_((FLAGS_writes < 0 && FLAGS_reads < 0) ?
					    FLAGS_num :
					    ((FLAGS_writes > FLAGS_reads) ?
						     FLAGS_writes :
						     FLAGS_reads)),
		  merge_keys_(FLAGS_merge_keys < 0 ? FLAGS_num :
							   FLAGS_merge_keys),
		  ...
		  use_blob_db_(false)
	{
		...
		if (FLAGS_simcache_size >= 0) {
			if (FLAGS_cache_numshardbits >= 1) {
				cache_ =
					NewSimCache(cache_, FLAGS_simcache_size,
						    FLAGS_cache_numshardbits);
			} else {
				cache_ = NewSimCache(cache_,
						     FLAGS_simcache_size, 0);
			}
		}
	...
	void GenerateKeyFromInt(uint64_t v, int64_t num_keys, Slice *key)
	{
		...
		if (keys_per_prefix_ > 0) {
			...
			if (port::kLittleEndian) {
				...
				memcpy(pos, static_cast<void *>(&prefix),
				       bytes_to_fill);
			}
			if (prefix_size_ > 8) {
	...
	void VerifyDBFromDB(std::string &truth_db_name)
	{
		...
		auto s = DB::OpenForReadOnly(open_options_, truth_db_name,
					     &truth_db.db);
		if (!s.ok()) {
		...
		std::unique_ptr<Iterator> truth_iter(
			truth_db.db->NewIterator(ro));
		...
		for (truth_iter->SeekToFirst(); truth_iter->Valid();
		     truth_iter->Next()) {
			std::string value;
		...
		for (db_iter->SeekToFirst(), truth_iter->SeekToFirst();
		     db_iter->Valid(); db_iter->Next(), truth_iter->Next()) {
	...
	void Run()
	{
		...
		while (std::getline(benchmark_stream, name, ',')) {
			...
			writes_ = (FLAGS_writes < 0 ? FLAGS_num : FLAGS_writes);
			deletes_ =
				(FLAGS_deletes < 0 ? FLAGS_num : FLAGS_deletes);
			...
			if (name == "fillseqdeterministic" ||
				...
				if (name == "fillseqdeterministic") {
					...
					method =
						&Benchmark::
							WriteUniqueRandomDeterministic;
				}
				...
				timestamp_emulator_.reset(
					new TimestampEmulator());
				if (FLAGS_expire_style == "compaction_filter") {
					filter.reset(new ExpiredTimeFilter(
						timestamp_emulator_));
					fprintf(stdout,
						"Compaction filter is used to remove expired data");
					open_options_.compaction_filter =
						filter.get();
				}
				fresh_db = true;
			...
			if (fresh_db) {
				if (FLAGS_use_existing_db) {
					...
					for (size_t i = 0;
						...
						if (!open_options_.wal_dir
							     .empty()) {
							options.wal_dir = GetPathForMultiple(
								open_options_
									.wal_dir,
								i);
						}
						DestroyDB(GetPathForMultiple(
								  FLAGS_db, i),
							  options);
					}
					multi_dbs_.clear();
		...
		if (FLAGS_simcache_size >= 0) {
			fprintf(stdout, "SIMULATOR CACHE STATISTICS:\n%s\n",
				std::dynamic_pointer_cast<SimCache>(cache_)
					->ToString()
					.c_str());
		}
	}
			   ...
			   void (Benchmark::*method)(ThreadState *))
	{
		...
		if (FLAGS_benchmark_write_rate_limit > 0) {
			shared.write_rate_limiter.reset(NewGenericRateLimiter(
				FLAGS_benchmark_write_rate_limit));
		}
		...
		for (int i = 0; i < n; i++) {
			...
			arg[i].thread->stats.SetReporterAgent(
				reporter_agent.get());
	...
	void xxHash(ThreadState *thread)
	{
		...
		// Print so result is not dead
		fprintf(stderr, "... xxh32=0x%x\r",
			static_cast<unsigned int>(xxh32));
	...
	void Uncompress(ThreadState *thread)
	{
		...
		while (ok && bytes < 1024 * 1048576) {
			...
			case rocksdb::kSnappyCompression: {
				...
				size_t ulength = 0;
				if (!Snappy_GetUncompressedLength(
					    compressed.data(),
				...
				ok = Snappy_Uncompress(compressed.data(),
						       compressed.size(),
						       uncompressed);
				break;
			}
			case rocksdb::kZlibCompression:
				uncompressed =
					Zlib_Uncompress(compressed.data(),
							compressed.size(),
							&decompress_size, 2);
			...
			case rocksdb::kBZip2Compression:
				uncompressed =
					BZip2_Uncompress(compressed.data(),
							 compressed.size(),
							 &decompress_size, 2);
				ok = uncompressed != nullptr;
				break;
			case rocksdb::kLZ4Compression:
				uncompressed =
					LZ4_Uncompress(compressed.data(),
						       compressed.size(),
						       &decompress_size, 2);
				ok = uncompressed != nullptr;
				break;
			...
			case rocksdb::kXpressCompression:
				uncompressed =
					XPRESS_Uncompress(compressed.data(),
							  compressed.size(),
							  &decompress_size);
				ok = uncompressed != nullptr;
				break;
			case rocksdb::kZSTD:
				uncompressed =
					ZSTD_Uncompress(compressed.data(),
							compressed.size(),
							&decompress_size);
				ok = uncompressed != nullptr;
				break;
	...
	bool InitializeOptionsFromFile(Options *opts)
	{
		...
		if (FLAGS_options_file != "") {
			auto s = LoadOptionsFromFile(FLAGS_options_file,
						     Env::Default(), &db_opts,
						     &cf_descs);
			if (s.ok()) {
	...
	void InitializeOptionsFromFlags(Options *opts)
	{
		...
		options.compaction_options_fifo = CompactionOptionsFIFO(
			FLAGS_fifo_compaction_max_table_files_size_mb * 1024 *
				1024,
		...
		options.memtable_huge_page_size =
			FLAGS_memtable_use_huge_page ? 2048 : 0;
		...
		if (FLAGS_use_plain_table) {
			...
			plain_table_options.bloom_bits_per_key =
				bloom_bits_per_key;
...
#ifndef ROCKSDB_LITE
			if (FLAGS_cuckoo_hash_ratio > 1 ||
			    FLAGS_cuckoo_hash_ratio < 0) {
			...
			table_options.identity_as_first_hash =
			...
			if (FLAGS_cache_high_pri_pool_ratio >
			    1e-6) { // > 0.0 + eps
				block_based_options
					.cache_index_and_filter_blocks_with_high_priority =
					true;
			...
			block_based_options.block_cache_compressed =
				compressed_cache_;
			...
			if (FLAGS_read_cache_path != "") {
				...
				rc_status = FLAGS_env->CreateDirIfMissing(
					FLAGS_read_cache_path);
				std::shared_ptr<Logger> read_cache_logger;
				if (rc_status.ok()) {
					rc_status = FLAGS_env->NewLogger(
						FLAGS_read_cache_path +
							"/rc_LOG",
						&read_cache_logger);
				}
				...
				if (rc_status.ok()) {
					PersistentCacheConfig rc_cfg(
						FLAGS_env,
						FLAGS_read_cache_path,
						FLAGS_read_cache_size,
						read_cache_logger);

					rc_cfg.enable_direct_reads =
						FLAGS_read_cache_direct_read;
					rc_cfg.enable_direct_writes =
						FLAGS_read_cache_direct_write;
					rc_cfg.writer_qdepth = 4;
					rc_cfg.writer_dispatch_size = 4 * 1024;

					auto pcache =
						std::make_shared<BlockCacheTier>(
							rc_cfg);
					block_based_options.persistent_cache =
						pcache;
		...
		if (FLAGS_max_bytes_for_level_multiplier_additional_v.size() >
		    0) {
			if (FLAGS_max_bytes_for_level_multiplier_additional_v
				    .size() != (unsigned int)FLAGS_num_levels) {
				fprintf(stderr,
	...
	void InitializeOptionsGeneral(Options *opts)
	{
		...
		if (FLAGS_num_multi_db <= 1) {
			...
			for (int i = 0; i < FLAGS_num_multi_db; i++) {
				if (!wal_dir.empty()) {
					options.wal_dir =
						GetPathForMultiple(wal_dir, i);
				}
				OpenDb(options, GetPathForMultiple(FLAGS_db, i),
				       &multi_dbs_[i]);
			}
		    ...
		    DBWithColumnFamilies *db)
	{
		...
		if (FLAGS_num_column_families > 1) {
...
#ifndef ROCKSDB_LITE
			if (FLAGS_readonly) {
				s = DB::OpenForReadOnly(options, db_name,
							column_families,
							&db->cfh, &db->db);
			} else if (FLAGS_optimistic_transaction_db) {
				s = OptimisticTransactionDB::Open(
					options, db_name, column_families,
				...
				s = TransactionDB::Open(options, txn_db_options,
							db_name,
							column_families,
							&db->cfh, &ptr);
				if (s.ok()) {
				...
				s = DB::Open(options, db_name, column_families,
					     &db->cfh, &db->db);
			}
#else
			s = DB::Open(options, db_name, column_families,
			...
			s = OptimisticTransactionDB::Open(options, db_name,
							  &db->opt_txn_db);
			if (s.ok()) {
				db->db = db->opt_txn_db->GetBaseDB();
			}
			...
			blob_db::BlobDB *ptr;
			s = blob_db::BlobDB::Open(options, blob_db_options,
						  db_name, &ptr);
			if (s.ok()) {
				db->db = ptr;
			}
	...
	void DoWrite(ThreadState *thread, WriteMode write_mode)
	{
		const int test_duration =
			write_mode == RANDOM ? FLAGS_duration : 0;
		...
		std::vector<std::unique_ptr<KeyGenerator> > key_gens(
			num_key_gens);
		int64_t max_ops = num_ops * num_key_gens;
		int64_t ops_per_stage = max_ops;
		if (FLAGS_num_column_families > 1 &&
		    FLAGS_num_hot_column_families > 0) {
			ops_per_stage = (max_ops -
		...
		for (size_t i = 0; i < num_key_gens; i++) {
			key_gens[i].reset(new KeyGenerator(&(thread->rand),
							   write_mode, num_,
							   ops_per_stage));
		}
		...
		if (FLAGS_expand_range_tombstones) {
			...
			for (auto &expanded_key_guard : expanded_key_guards) {
				expanded_keys.emplace_back(
					AllocateKey(&expanded_key_guard));
			}
		}
		...
		while (!duration.Done(entries_per_batch_)) {
			...
			if (thread->shared->write_rate_limiter.get() !=
			    nullptr) {
				thread->shared->write_rate_limiter->Request(
					entries_per_batch_ *
						(value_size_ + key_size_),
			...
			for (int64_t j = 0; j < entries_per_batch_; j++) {
				...
				if (use_blob_db_) {
					...
					blob_db::BlobDB *blobdb =
						static_cast<blob_db::BlobDB *>(
							db_with_cfh->db);
					s = blobdb->PutWithTTL(write_options_,
							       key, val, ttl);
#endif //  ROCKSDB_LITE
					...
					batch.Put(db_with_cfh->GetCfh(rand_num),
						  key,
						  gen.Generate(value_size_));
				}
								...
								s = db_with_cfh
									    ...
									    ->Delete(
										    ...
										    expanded_keys
											    [offset]);
#endif //  ROCKSDB_LITE
							} else if (FLAGS_num_column_families <=
								   1) {
								batch.Delete(
									expanded_keys
										[offset]);
			...
			if (!use_blob_db_) {
#ifndef ROCKSDB_LITE
				s = db_with_cfh->db->Write(write_options_,
							   &batch);
#endif //  ROCKSDB_LITE
			}
				      ...
				      WriteMode write_mode)
	{
		...
		for (auto db : db_list) {
			...
			if (compaction_style != kCompactionStyleFIFO) {
				...
				db->SetOptions({ { "disable_auto_compactions",
						   "1" } });
			}
		}
		...
		size_t num_levels =
			static_cast<size_t>(open_options_.num_levels);
		size_t output_level = open_options_.num_levels - 1;
		std::vector<std::vector<std::vector<SstFileMetaData> > >
			sorted_runs(num_db);
		std::vector<size_t> num_files_at_level0(num_db, 0);
		if (compaction_style == kCompactionStyleLevel) {
			if (num_levels == 0) {
				return Status::InvalidArgument(
					"num_levels should be larger than 1");
			}
			...
			while (!should_stop) {
				...
				for (size_t i = 0; i < num_db; i++) {
					...
					if (num_files_at_level0[i] ==
						    meta.levels[0].files.size() ||
					    writes_ == 0) {
						should_stop = true;
						continue;
					}
			...
			for (size_t i = 0; i < num_db; i++) {
				...
				for (size_t j = 0; j < sorted_runs[i].size();
				     j++) {
					compactionOptions
						...
						mutable_cf_options.MaxFileSizeForLevel(
							static_cast<int>(
								output_level));
					std::cout << sorted_runs[i][j].size()
						  << std::endl;
					db->CompactFiles(
						compactionOptions,
						{ sorted_runs[i][j].back().name,
						  sorted_runs[i][j]
							  .front()
							  .name },
			...
			auto ratio = open_options_.compaction_options_universal
					     .size_ratio;
			bool should_stop = false;
			while (!should_stop) {
				if (sorted_runs[0].empty()) {
				...
				writes_ = static_cast<int64_t>(
					writes_ * static_cast<double>(100) /
					(ratio + 200));
			}
			for (size_t i = 0; i < num_db; i++) {
			...
			fprintf(stdout,
				...
				"filldeterministic");
			return Status::InvalidArgument(
				"None compaction is not supported");
		}
...
// optimization, so skip the check of the max level.
		...
		for (size_t k = 0; k < num_db; k++) {
			...
			} else if (compaction_style ==
				   kCompactionStyleUniversal) {
				assert(meta.levels[0].files.size() +
					       num_levels - 1 ==
				       sorted_runs[k].size());
			} else if (compaction_style == kCompactionStyleFIFO) {
			...
			for (size_t i = 0; i < sorted_runs[k].size(); i++) {
				...
				SequenceNumber sorted_run_smallest_seqno =
					kMaxSequenceNumber;
				SequenceNumber sorted_run_largest_seqno = 0;
				std::string sorted_run_smallest_key,
					sorted_run_largest_key;
				bool first_key = true;
				for (auto fileMeta : sorted_runs[k][i]) {
					sorted_run_smallest_seqno = std::min(
						sorted_run_smallest_seqno,
						fileMeta.smallest_seqno);
					sorted_run_largest_seqno = std::max(
						sorted_run_largest_seqno,
						fileMeta.largest_seqno);
					if (first_key ||
					    db->DefaultColumnFamily()
							    ->GetComparator()
							    ->Compare(
								    fileMeta.smallestkey,
				...
				if (compaction_style == kCompactionStyleLevel ||
				    (compaction_style ==
					     kCompactionStyleUniversal &&
				     level > 0)) {
					SequenceNumber level_smallest_seqno =
						kMaxSequenceNumber;
					SequenceNumber level_largest_seqno = 0;
					for (auto fileMeta :
					     meta.levels[level].files) {
						level_smallest_seqno = std::min(
							level_smallest_seqno,
							fileMeta.smallest_seqno);
						level_largest_seqno = std::max(
							level_largest_seqno,
							fileMeta.largest_seqno);
					}
					...
					if (level !=
						...
						assert(sorted_run_smallest_seqno ==
						       level_smallest_seqno);
						assert(sorted_run_largest_seqno ==
						       level_largest_seqno);
					}
				} else if (compaction_style ==
					   kCompactionStyleUniversal) {
					...
					auto level0_file =
						meta.levels[0].files
							[sorted_runs[k].size() -
							 1 - i];
					assert(sorted_run_smallest_key ==
					       level0_file.smallestkey);
					assert(sorted_run_largest_key ==
					       level0_file.largestkey);
					if (level !=
					    static_cast<int>(max_level)) {
						assert(sorted_run_smallest_seqno ==
						       level0_file
			...
			for (auto &levelMeta : meta.levels) {
				...
				if (levelMeta.level == 0) {
					...
					fprintf(stdout,
						...
						" bytes)\n",
						levelMeta.level,
						levelMeta.files.front()
							.name.c_str(),
						levelMeta.files.back()
							.name.c_str(),
						levelMeta.size);
				}
			}
	...
	void ReadSequential(ThreadState *thread, DB *db)
	{
		...
		int64_t i = 0;
		int64_t bytes = 0;
		for (iter->SeekToFirst(); i < reads_ && iter->Valid();
		     iter->Next()) {
			bytes += iter->key().size() + iter->value().size();
			thread->stats.FinishedOps(nullptr, db, 1, kRead);
			++i;

			if (thread->shared->read_rate_limiter.get() !=
				    nullptr &&
			    i % 1024 == 1023) {
	...
	void ReadReverse(ThreadState *thread, DB *db)
	{
		Iterator *iter = db->NewIterator(
			ReadOptions(FLAGS_verify_checksum, true));
		int64_t i = 0;
		int64_t bytes = 0;
		for (iter->SeekToLast(); i < reads_ && iter->Valid();
		     iter->Prev()) {
			bytes += iter->key().size() + iter->value().size();
			thread->stats.FinishedOps(nullptr, db, 1, kRead);
			++i;
	...
	int64_t GetRandomKey(Random64 *rand)
	{
		...
		if (read_random_exp_range_ == 0) {
			...
			const uint64_t kBigInt = static_cast<uint64_t>(1U)
						 << 62;
			long double order =
				-static_cast<long double>(rand_int % kBigInt) /
				static_cast<long double>(kBigInt) *
				read_random_exp_range_;
			long double exp_ran = std::exp(order);
			uint64_t rand_num = static_cast<int64_t>(
				exp_ran * static_cast<long double>(FLAGS_num));
			// Map to a different number to avoid locality.
			const uint64_t kBigPrime = 0x5bd1e995;
			// Overflow is like %(2^64). Will have little impact of results.
			key_rand = static_cast<int64_t>((rand_num * kBigPrime) %
							FLAGS_num);
		}
		return key_rand;
	}
	...
	void MultiReadRandom(ThreadState *thread)
	{
		...
		while (!duration.Done(1)) {
			...
			for (int64_t i = 0; i < entries_per_batch_; ++i) {
				GenerateKeyFromInt(GetRandomKey(&thread->rand),
						   FLAGS_num, &keys[i]);
			}
			std::vector<Status> statuses =
				db->MultiGet(options, keys, &values);
			assert(static_cast<int64_t>(statuses.size()) ==
			       entries_per_batch_);
			...
			if (thread->shared->read_rate_limiter.get() !=
				...
				thread->shared->read_rate_limiter->Request(
					256 * entries_per_batch_, Env::IO_HIGH,
					nullptr /* stats */);
			}
			thread->stats.FinishedOps(nullptr, db,
						  entries_per_batch_, kRead);
		}
	...
	void SeekRandom(ThreadState *thread)
	{
		...
		if (db_.db != nullptr) {
			...
			for (const auto &db_with_cfh : multi_dbs_) {
				multi_iters.push_back(
					db_with_cfh.db->NewIterator(options));
			}
		}
		...
		while (!duration.Done(1)) {
			if (!FLAGS_use_tailing_iterator) {
				if (db_.db != nullptr) {
					delete single_iter;
					single_iter =
						db_.db->NewIterator(options);
				} else {
					for (auto iter : multi_iters) {
						delete iter;
					}
					multi_iters.clear();
					for (const auto &db_with_cfh :
					     multi_dbs_) {
						multi_iters.push_back(
							db_with_cfh.db
								->NewIterator(
			...
			if (single_iter == nullptr) {
				iter_to_use = multi_iters[thread->rand.Next() %
							  multi_iters.size()];
			}
			...
			if (iter_to_use->Valid() &&
			    iter_to_use->key().compare(key) == 0) {
				found++;
			}

			for (int j = 0;
			     j < FLAGS_seek_nexts && iter_to_use->Valid();
			     ++j) {
				...
				memcpy(value_buffer, value.data(),
				       std::min(value.size(),
						sizeof(value_buffer)));
				bytes += iter_to_use->key().size() +
					 iter_to_use->value().size();
	...
	void DoDelete(ThreadState *thread, bool seq)
	{
		...
		while (!duration.Done(entries_per_batch_)) {
			...
			for (int64_t j = 0; j < entries_per_batch_; ++j) {
				const int64_t k =
					seq ? i + j :
						    (thread->rand.Next() % FLAGS_num);
				GenerateKeyFromInt(k, FLAGS_num, &key);
				batch.Delete(key);
			}
			auto s = db->Write(write_options_, &batch);
			thread->stats.FinishedOps(nullptr, db,
						  entries_per_batch_, kDelete);
			if (!s.ok()) {
	...
	void BGWriter(ThreadState *thread, enum OperationType write_merge)
	{
		...
		if (FLAGS_benchmark_write_rate_limit > 0) {
			write_rate_limiter.reset(NewGenericRateLimiter(
				FLAGS_benchmark_write_rate_limit));
		}
		...
		while (true) {
			...
			if (write_merge == kWrite) {
				s = db->Put(write_options_, key,
					    gen.Generate(value_size_));
			} else {
				s = db->Merge(write_options_, key,
					      gen.Generate(value_size_));
			}
			written++;
	...
	Status PutMany(DB *db, const WriteOptions &writeoptions,
	...
	void RandomWithVerify(ThreadState *thread)
	{
		...
		for (int64_t i = 0; i < readwrites_; i++) {
			...
			GenerateKeyFromInt(thread->rand.Next() %
						   FLAGS_numdistinct,
			...
			if (get_weight > 0) {
				...
				Status s = PutMany(db, write_options_, key,
						   gen.Generate(value_size_));
				if (!s.ok()) {
		...
		snprintf(msg, sizeof(msg),
			 ...
			 gets_done, puts_done, deletes_done, readwrites_,
			 found);
		thread->stats.AddMessage(msg);
	}
	...
	void ReadRandomWriteRandom(ThreadState *thread)
	{
		...
		while (!duration.Done(1)) {
			...
			if (get_weight > 0) {
				...
				// then do all the corresponding number of puts
				// for all the gets we have done earlier
				Status s = db->Put(write_options_, key,
						   gen.Generate(value_size_));
				if (!s.ok()) {
					fprintf(stderr, "put error: %s\n",
						s.ToString().c_str());
	...
	void MergeRandom(ThreadState *thread)
	{
		...
		while (!duration.Done(1)) {
			DB *db = SelectDB(thread);
			GenerateKeyFromInt(thread->rand.Next() % merge_keys_,
					   merge_keys_, &key);

			Status s = db->Merge(write_options_, key,
					     gen.Generate(value_size_));
		...
		char msg[100];
		snprintf(msg, sizeof(msg), "( updates:%" PRIu64 ")",
			 readwrites_);
		thread->stats.AddBytes(bytes);
		thread->stats.AddMessage(msg);
	}
	...
	void ReadRandomMergeRandom(ThreadState *thread)
	{
		...
		while (!duration.Done(1)) {
			...
			bool do_merge = int(thread->rand.Next() % 100) <
					FLAGS_mergereadpercent;
		...
		snprintf(msg, sizeof(msg),
			 ...
			 num_gets, num_merges, readwrites_, num_hits,
			 max_length);
		thread->stats.AddMessage(msg);
	}
	...
	void WriteSeqSeekSeq(ThreadState *thread)
	{
		...
		for (int64_t i = 0; i < FLAGS_num; ++i) {
			...
			for (int j = 0;
			     j < FLAGS_seek_nexts && i + 1 < FLAGS_num; ++j) {
				if (!FLAGS_reverse_iterator) {
					iter->Next();
				} else {
					iter->Prev();
	...
	void RandomTransaction(ThreadState *thread)
	{
		...
		RandomTransactionInserter inserter(&thread->rand,
						   write_options_, read_options,
						   FLAGS_num,
						   num_prefix_ranges);
		...
		while (!duration.Done(1)) {
			...
			if (FLAGS_optimistic_transaction_db) {
				success =
					inserter.OptimisticTransactionDBInsert(
						db_.opt_txn_db);
			} else if (FLAGS_transaction_db) {
				TransactionDB *txn_db =
					reinterpret_cast<TransactionDB *>(
						db_.db);
				success = inserter.TransactionDBInsert(
					txn_db, txn_options);
			} else {
	...
	void RandomReplaceKeys(ThreadState *thread)
	{
		...
		std::normal_distribution<double> distribution(
			FLAGS_numdistinct / 2.0, FLAGS_stddev);
		Duration duration(FLAGS_duration, FLAGS_num);
		while (!duration.Done(1)) {
			int64_t rnd_id =
				static_cast<int64_t>(distribution(generator));
			int64_t key_id = std::max(
				std::min(FLAGS_numdistinct - 1, rnd_id),
				static_cast<int64_t>(0));
			GenerateKeyFromInt(key_id * max_counter +
						   counters[key_id],
					   FLAGS_num, &key);
			s = FLAGS_use_single_deletes ?
					  db->SingleDelete(write_options_, key) :
					  db->Delete(write_options_, key);
			if (s.ok()) {
				counters[key_id] =
					(counters[key_id] + 1) % max_counter;
				GenerateKeyFromInt(key_id * max_counter +
							   counters[key_id],
						   FLAGS_num, &key);
	...
	void TimeSeriesReadOrDelete(ThreadState *thread, bool do_deletion)
	{
		...
		while (true) {
			...
			int64_t key_id =
				thread->rand.Next() % FLAGS_key_id_range;
			...
			for (iter->Seek(key);
				...
				if (do_deletion) {
					bytes += iter->key().size();
					if (KeyExpired(timestamp_emulator_.get(),
						       iter->key())) {
						thread->stats.FinishedOps(
							&db_, db_.db, 1,
							kDelete);
						db_.db->Delete(write_options_,
							       iter->key());
					} else {
						break;
					}
					...
					bytes += iter->key().size() +
						 iter->value().size();
					thread->stats.FinishedOps(&db_, db_.db,
								  1, kRead);
	...
	void TimeSeriesWrite(ThreadState *thread)
	{
		...
		while (!duration.Done(1)) {
			...
			char *start = const_cast<char *>(key.data());
			char *pos = start + 8;
			int bytes_to_fill = std::min(
				key_size_ - static_cast<int>(pos - start), 8);
			uint64_t timestamp_value = timestamp_emulator_->Get();
			if (port::kLittleEndian) {
				for (int i = 0; i < bytes_to_fill; ++i) {
					pos[i] = (timestamp_value >>
						  ((bytes_to_fill - i - 1)
						   << 3)) &
						 0xFF;
				}
				...
				memcpy(pos,
				       static_cast<void *>(&timestamp_value),
				       bytes_to_fill);
			}
	...
	{
		if (db_.db != nullptr) {
			db_.db->CompactRange(CompactRangeOptions(), nullptr,
					     nullptr);
		}
		for (const auto &db_with_cfh : multi_dbs_) {
			db_with_cfh.db->CompactRange(CompactRangeOptions(),
						     nullptr, nullptr);
		}
	}
...
int db_bench_tool(int argc, char **argv)
{
	...
	FLAGS_compaction_style_e =
		(rocksdb::CompactionStyle)FLAGS_compaction_style;
#ifndef ROCKSDB_LITE
	...
	if (!FLAGS_statistics_string.empty()) {
		...
		dbstats.reset(NewCustomObject<Statistics>(
			FLAGS_statistics_string, &custom_stats_guard));
	...
	if (!FLAGS_hdfs.empty() && !FLAGS_env_uri.empty()) {
		...
		FLAGS_env =
			NewCustomObject<Env>(FLAGS_env_uri, &custom_env_guard);
