...
class DBBenchTest : public testing::Test {
	...
	void AppendArgs(const std::vector<std::string> &args)
	{
		for (const auto &arg : args) {
			...
			snprintf(arg_buffer_ + cursor_, arg.size() + 1, "%s",
				 arg.c_str());
	...
	void RunDbBench(const std::string &options_file_name)
	{
		AppendArgs(
			{ "./db_bench", "--benchmarks=fillseq",
			  "--use_existing_db=0", "--num=1000",
			  std::string(std::string("--db=") + db_path_).c_str(),
			  std::string(std::string("--wal_dir=") + wal_path_)
				  .c_str(),
			  std::string(std::string("--options_file=") +
				      options_file_name)
				  .c_str() });
