...
class Stats {
		...
		if (FLAGS_progress_reports) {
			if (done_ >= next_report_) {
				if (next_report_ < 1000)
					...
					next_report_ += 100000;
				fprintf(stdout, "... finished %ld ops%30s\r",
					done_, "");
			}
		}
	}
	...
	void Report(const char *name)
	{
		...
		fprintf(stdout,
			"%-12s: Single deleted %" ROCKSDB_PRIszt " times\n", "",
			single_deletes_);
...
class SharedState {
	...
	explicit SharedState(StressTest *stress_test)
		: cv_(&mu_), seed_(static_cast<uint32_t>(FLAGS_seed)),
		  max_key_(FLAGS_max_key),
		  log2_keys_per_lock_(
			  static_cast<uint32_t>(FLAGS_log2_keys_per_lock)),
		  num_threads_(FLAGS_threads), num_initialized_(0),
		  num_populated_(0), vote_reopen_(0), num_done_(0),
		  start_(false), start_verify_(false),
		  should_stop_bg_thread_(false), bg_thread_finished_(false),
		  stress_test_(stress_test), verification_failure_(false),
		  no_overwrite_ids_(FLAGS_column_families)
	{
		...
		long num_locks =
			static_cast<long>(max_key_ >> log2_keys_per_lock_);
	...
	{
		return no_overwrite_ids_[cf].find(key) ==
		       no_overwrite_ids_[cf].end();
	}
...
struct ThreadState {
	...
	ThreadState(uint32_t index, SharedState *_shared)
		: tid(index), rand(1000 + index + _shared->GetSeed()),
		  shared(_shared)
	{
	}
};
...
class DbStressListener : public EventListener {
	...
	virtual void OnFlushCompleted(DB *db, const FlushJobInfo &info) override
	{
		...
		std::this_thread::sleep_for(std::chrono::microseconds(
			Random::GetTLSInstance()->Uniform(5000)));
	}
	...
	virtual void
	OnTableFileCreated(const TableFileCreationInfo &info) override
	{
	...
	void VerifyFileName(const std::string &file_name)
	{
		...
		FileType file_type;
		bool result =
			ParseFileName(file_name, &file_number, &file_type);
		assert(result);
...
class StressTest {
		  ...
		  filter_policy_(
			  FLAGS_bloom_bits >= 0 ?
					...
					NewBloomFilterPolicy(FLAGS_bloom_bits,
						       false) :
					nullptr),
		  db_(nullptr), new_column_family_name_(1),
	...
	bool Run()
	{
		...
		if (FLAGS_compaction_thread_pool_adjust_interval > 0) {
			FLAGS_env->StartThread(PoolSizeChangeThread,
					       &bg_thread);
		}
	...
	static void PoolSizeChangeThread(void *v)
	{
		...
		while (true) {
			...
			FLAGS_env->SleepForMicroseconds(
				thread->rand.Next() %
					FLAGS_compaction_thread_pool_adjust_interval *
					1000 +
				1);
		}
	}
			...
			const Slice &value, size_t sz)
	{
		...
		for (int i = 0; i < 10; i++) {
			...
			if (FLAGS_use_merge) {
				batch.Merge(column_family, keys[i],
					    value_slices[i]);
			} else {
				batch.Put(column_family, keys[i],
					  value_slices[i]);
			}
		}
			...
			std::string *value)
	{
		...
		for (int i = 0; i < 10; i++) {
			keys[i] += key.ToString();
			key_slices[i] = keys[i];
			s = db_->Get(readoptionscopy, column_family,
				     key_slices[i], value);
			if (!s.ok() && !s.IsNotFound()) {
				...
				(values[i])[0] =
					' '; // blank out the differing character
			    ...
			    ColumnFamilyHandle *column_family, const Slice &key)
	{
		...
		unique_ptr<Iterator> iter(
			db_->NewIterator(readoptionscopy, column_family));

		iter->Seek(key);
		for (uint64_t i = 0; i < FLAGS_num_iterations && iter->Valid();
		     i++) {
	...
	Status SetOptions(ThreadState *thread)
	{
		...
		std::string name = options_index_[thread->rand.Next() %
						  options_index_.size()];
		int value_idx =
			thread->rand.Next() % options_table_[name].size();
		if (name == "soft_rate_limit" || name == "hard_rate_limit") {
			opts["soft_rate_limit"] =
				options_table_["soft_rate_limit"][value_idx];
			opts["hard_rate_limit"] =
				options_table_["hard_rate_limit"][value_idx];
			...
			opts["level0_slowdown_writes_trigger"] =
				options_table_["level0_slowdown_writes_trigger"]
					      [value_idx];
			opts["level0_stop_writes_trigger"] =
				options_table_["level0_stop_writes_trigger"]
					      [value_idx];
		} else {
	...
	void OperateDb(ThreadState *thread)
	{
		...
		const int prefixBound =
			(int)FLAGS_readpercent + (int)FLAGS_prefixpercent;
		...
		for (uint64_t i = 0; i < FLAGS_ops_per_thread; i++) {
			...
			if (i != 0 && (i % (FLAGS_ops_per_thread /
					    (FLAGS_reopen + 1))) == 0) {
				{
					...
					if (thread->shared->AllVotedReopen()) {
						thread->shared->GetStressTest()
							->Reopen();
						thread->shared->GetCondVar()
							->SignalAll();
					} else {
						thread->shared->GetCondVar()
							->Wait();
			...
			if (FLAGS_set_in_place_one_in > 0 &&
				...
				options_.inplace_update_support ^=
					options_.inplace_update_support;
			}

			if (!FLAGS_test_batches_snapshots &&
			    FLAGS_clear_column_family_one_in != 0 &&
			    FLAGS_column_families > 1) {
				if (thread->rand.OneIn(
			...
			if (FLAGS_compact_files_one_in > 0 &&
			    thread->rand.Uniform(FLAGS_compact_files_one_in) ==
				    0) {
				auto *random_cf =
					column_families_[thread->rand.Next() %
							 FLAGS_column_families];
				rocksdb::ColumnFamilyMetaData cf_meta_data;
				db_->GetColumnFamilyMetaData(random_cf,
							     &cf_meta_data);

				// Randomly compact up to three consecutive files from a level
				const int kMaxRetry = 3;
				for (int attempt = 0; attempt < kMaxRetry;
				     ++attempt) {
					size_t random_level =
						thread->rand.Uniform(static_cast<
								     int>(
							cf_meta_data.levels
								.size()));
					...
					if (files.size() > 0) {
						...
						input_files.push_back(
							files[random_file_index]
								.name);
						if (random_file_index > 0 &&
						    !files[random_file_index - 1]
							     .being_compacted) {
							input_files.push_back(
								files[random_file_index -
								      1]
						...
						if (!s.ok()) {
							printf("Unable to perform CompactFiles(): %s\n",
							       s.ToString()
								       .c_str());
							thread->stats
								.AddNumCompactFilesFailed(
									1);
						} else {
							thread->stats
								.AddNumCompactFilesSucceed(
									1);
						}
						break;
					}
			...
			if (!FLAGS_test_batches_snapshots) {
				l.reset(new MutexLock(shared->GetMutexForKey(
					rand_column_family, rand_key)));
			}
			auto column_family =
				column_families_[rand_column_family];

			int prob_op = thread->rand.Uniform(100);
			if (prob_op >= 0 && prob_op < (int)FLAGS_readpercent) {
				// OPERATION read
				if (!FLAGS_test_batches_snapshots) {
					Status s = db_->Get(read_opts,
							    column_family, key,
							    &from_db);
					if (s.ok()) {
						// found case
					...
					MultiGet(thread, read_opts,
						 column_family, key, &from_db);
				}
			} else if ((int)FLAGS_readpercent <= prob_op &&
				   prob_op < prefixBound) {
				...
				if (!FLAGS_test_batches_snapshots) {
					...
					MultiPrefixScan(thread, read_opts,
							column_family, key);
				}
			} else if (prefixBound <= prob_op &&
				   prob_op < writeBound) {
				// OPERATION write
				uint32_t value_base = thread->rand.Next();
				size_t sz = GenerateValue(value_base, value,
							  sizeof(value));
				Slice v(value, sz);
				if (!FLAGS_test_batches_snapshots) {
					// If the chosen key does not allow overwrite and it already
					// exists, choose another key.
					while (!shared->AllowsOverwrite(
						       rand_column_family,
						       rand_key) &&
					       shared->Exists(rand_column_family,
							      rand_key)) {
						l.reset();
					...
					keystr = Key(rand_key);
					key = keystr;
					column_family = column_families_
						[rand_column_family];
					...
					Status s;
					if (FLAGS_use_merge) {
						s = db_->Merge(write_opts,
							       column_family,
							       key, v);
					} else {
						s = db_->Put(write_opts,
							     column_family, key,
							     v);
					}
					if (!s.ok()) {
					...
					MultiPut(thread, write_opts,
						 column_family, key, v, sz);
				}
				PrintKeyValue(rand_column_family,
					      static_cast<uint32_t>(rand_key),
					      value, sz);
			} else if (writeBound <= prob_op &&
				   prob_op < delBound) {
				// OPERATION delete
				if (!FLAGS_test_batches_snapshots) {
					...
					if (shared->AllowsOverwrite(
						...
						shared->SingleDelete(
							rand_column_family,
							rand_key);
						Status s = db_->SingleDelete(
							write_opts,
							column_family, key);
						thread->stats.AddSingleDeletes(
							1);
						if (!s.ok()) {
							fprintf(stderr,
					...
					MultiDelete(thread, write_opts,
						    column_family, key);
				}
			} else if (delBound <= prob_op &&
				   prob_op < delRangeBound) {
				// OPERATION delete range
				if (!FLAGS_test_batches_snapshots) {
					std::vector<std::unique_ptr<MutexLock> >
						range_locks;
					...
					for (int j = 1;
					     j < FLAGS_range_deletion_width;
					     ++j) {
						if (((rand_key + j) &
						     ((1
						       << FLAGS_log2_keys_per_lock) -
						      1)) == 0) {
							range_locks.emplace_back(new MutexLock(
								shared->GetMutexForKey(
									rand_column_family,
					...
					thread->stats.AddCoveredByRangeDeletions(
						covered);
				}
			} else {
				// OPERATION iterate
				MultiIterate(thread, read_opts, column_family,
					     key);
			}
			thread->stats.FinishedSingleOp();
		}
	...
	void VerifyDb(ThreadState *thread) const
	{
		...
		const int64_t keys_per_thread =
			max_key / shared->GetNumThreads();
		...
		for (size_t cf = 0; cf < column_families_.size(); ++cf) {
			...
			if (!thread->rand.OneIn(2)) {
				...
				for (auto i = start; i < end; i++) {
					...
					VerifyValue(static_cast<int>(cf), i,
						    options, shared, from_db, s,
						    true);
					if (from_db.length()) {
						PrintKeyValue(
							static_cast<int>(cf),
	...
	void VerificationAbort(SharedState *shared, std::string msg, int cf,
			       int64_t key) const
	{
		printf("Verification failed for column family %d key %" PRIi64
		       ": %s\n",
		       cf, key, msg.c_str());
	...
	void PrintEnv() const
	{
		...
		fprintf(stdout, "Ratio #ops/#keys          : %f\n",
			(1.0 * FLAGS_ops_per_thread * FLAGS_threads) /
				FLAGS_max_key);
	...
	void Open()
	{
		...
		options_.max_background_compactions =
			FLAGS_max_background_compactions;
		options_.max_background_flushes = FLAGS_max_background_flushes;
		options_.compaction_style =
			static_cast<rocksdb::CompactionStyle>(
		...
		options_.compaction_readahead_size =
		...
		options_.target_file_size_multiplier =
		...
		if (FLAGS_prefix_size == 0 &&
		    FLAGS_rep_factory == kHashSkipList) {
			fprintf(stderr,
		...
		if (FLAGS_prefix_size != 0 &&
		    FLAGS_rep_factory != kHashSkipList) {
			fprintf(stderr, "WARNING: prefix_size is non-zero but "
					"memtablerep != prefix_hash\n");
		}
		switch (FLAGS_rep_factory) {
		case kSkipList:
		...
		case kHashSkipList:
			options_.memtable_factory.reset(
				NewHashSkipListRepFactory(10000));
			break;
		case kVectorRep:
		...
		if (FLAGS_ttl == -1) {
			...
			s = DB::ListColumnFamilies(
				DBOptions(options_), FLAGS_db,
				&existing_column_families); // ignore errors
			if (!s.ok()) {
				// DB doesn't exist
				assert(existing_column_families.empty());
				assert(column_family_names_.empty());
				column_family_names_.push_back(
			...
			for (auto name : column_family_names_) {
				if (name != kDefaultColumnFamilyName) {
					new_column_family_name_ = std::max(
						new_column_family_name_.load(),
						std::stoi(name) + 1);
				}
				cf_descriptors.emplace_back(
					name, ColumnFamilyOptions(options_));
			}
			while (cf_descriptors.size() <
			       (size_t)FLAGS_column_families) {
			...
			options_.listeners.emplace_back(new DbStressListener(
				FLAGS_db, options_.db_paths));
			options_.create_missing_column_families = true;
			s = DB::Open(DBOptions(options_), FLAGS_db,
			...
			s = DBWithTTL::Open(options_, FLAGS_db, &db_with_ttl,
					    FLAGS_ttl);
			db_ = db_with_ttl;
#else
			fprintf(stderr,
...
int main(int argc, char **argv)
{
	...
	if (FLAGS_test_batches_snapshots && FLAGS_prefix_size <= 0) {
		fprintf(stderr, "Error: please specify prefix_size for "
