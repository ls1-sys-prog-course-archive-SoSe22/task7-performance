...
Handle<Value> DBWrapper::Open(const Arguments &args)
{
	...
	if (!args[1]->IsUndefined()) {
		...
		for (uint i = 0; i < array->Length(); i++) {
			...
			cfs.push_back(*v8::String::Utf8Value(
	...
	db_wrapper->status_ =
		rocksdb::DB::Open(db_wrapper->options_, db_file, families,
				  &handles, &db_wrapper->db_);
...
Handle<Value> DBWrapper::Get(const Arguments &args)
{
	...
	if (args[1]->IsUndefined()) {
		...
		db_wrapper->status_ =
			db_wrapper->db_->Get(rocksdb::ReadOptions(),
...
Handle<Value> DBWrapper::Put(const Arguments &args)
{
	...
	if (args[2]->IsUndefined()) {
		...
		db_wrapper->status_ =
			db_wrapper->db_->Put(rocksdb::WriteOptions(),
					     db_wrapper->columnFamilies_[cf],
					     key, value);
	} else {
		return scope.Close(Boolean::New(false));
...
Handle<Value> DBWrapper::Delete(const Arguments &args)
{
	...
	if (args[1]->IsUndefined()) {
		db_wrapper->status_ =
			db_wrapper->db_->Delete(rocksdb::WriteOptions(), arg0);
	} else {
...
Handle<Value> DBWrapper::Dump(const Arguments &args)
{
	...
	if (args[0]->IsUndefined()) {
		iterator.reset(
			db_wrapper->db_->NewIterator(rocksdb::ReadOptions()));
	} else {
		...
		iterator.reset(db_wrapper->db_->NewIterator(
			rocksdb::ReadOptions(),
			db_wrapper->columnFamilies_[arg0]));
	}
			   ...
			   Handle<Array> array)
{
	...
	for (uint i = 0; i < array->Length(); i++) {
		if (del) {
			...
			batch.Delete(*v8::String::Utf8Value(
				array->Get(i)->ToString()));
			continue;
		}
		...
		if (!put_pair->Get(0)->IsString() ||
		    !put_pair->Get(1)->IsString()) {
			return false;
		}

		batch.Put(*v8::String::Utf8Value(put_pair->Get(0)->ToString()),
			   ...
			   std::string cf)
{
	...
	for (uint i = 0; i < array->Length(); i++) {
		if (del) {
			...
			batch.Delete(db_wrapper->columnFamilies_[cf],
				     *v8::String::Utf8Value(
					     array->Get(i)->ToString()));
			continue;
		}
		...
		batch.Put(db_wrapper->columnFamilies_[cf],
...
Handle<Value> DBWrapper::WriteBatch(const Arguments &args)
{
	...
	for (uint i = 0; i < sub_batches->Length(); i++) {
		...
		if (sub_batch->Has(String::NewSymbol("column_family"))) {
			...
			well_formed = db_wrapper->AddToBatch(
				batch, false,
				Handle<Array>::Cast(sub_batch->Get(
					String::NewSymbol("put"))),
				db_wrapper,
	...
	db_wrapper->status_ =
		db_wrapper->db_->Write(rocksdb::WriteOptions(), &batch);
...
Handle<Value> DBWrapper::CompactOptions(const Arguments &args)
{
	...
	if (options->Has(String::NewSymbol("target_level")) &&
		...
		target_level =
			(int)(options->Get(String::NewSymbol("target_level"))
				      ->ToInt32()
				      ->Value());

		if (options->Has(String::NewSymbol("target_path_id")) ||
		    options->Get(String::NewSymbol("target_path_id"))
			    ->IsInt32()) {
			target_path_id =
				(int)(options->Get(String::NewSymbol(
							   "target_path_id"))
					      ->ToInt32()
					      ->Value());
		}
...
Handle<Value> DBWrapper::CompactAll(const Arguments &args)
{
	...
	db_wrapper->status_ =
		db_wrapper->db_->CompactRange(db_wrapper->columnFamilies_[cf],
					      &begin, &end, true, target_level,
					      target_path_id);
