		...
		if (arg[0] == '-' && arg[1] == '-') {
			...
			if (splits.size() == 2) {
				std::string optionKey =
					splits[0].substr(OPTION_PREFIX.size());
...
LDBCommand *LDBCommand::SelectCommand(const ParsedParams &parsed_params)
{
	if (parsed_params.cmd == GetCommand::Name()) {
		return new GetCommand(parsed_params.cmd_params,
				      parsed_params.option_map,
				      parsed_params.flags);
	} else if (parsed_params.cmd == PutCommand::Name()) {
		return new PutCommand(parsed_params.cmd_params,
				      parsed_params.option_map,
				      parsed_params.flags);
		...
		return new ScanCommand(parsed_params.cmd_params,
				       parsed_params.option_map,
				       parsed_params.flags);
	} else if (parsed_params.cmd == DeleteCommand::Name()) {
		return new DeleteCommand(parsed_params.cmd_params,
					 parsed_params.option_map,
					 parsed_params.flags);
		...
		return new ChangeCompactionStyleCommand(
			parsed_params.cmd_params, parsed_params.option_map,
			parsed_params.flags);
		...
		return new RepairCommand(parsed_params.cmd_params,
					 parsed_params.option_map,
					 parsed_params.flags);
	} else if (parsed_params.cmd == BackupCommand::Name()) {
		return new BackupCommand(parsed_params.cmd_params,
					 parsed_params.option_map,
					 parsed_params.flags);
...
LDBCommand::LDBCommand(const std::map<std::string, std::string> &options,
	...
	: db_(nullptr), is_read_only_(is_read_only), is_key_hex_(false),
	  is_value_hex_(false), is_db_ttl_(false), timestamp_(false),
	  try_load_options_(false), ignore_unknown_options_(false),
	  create_if_missing_(false), option_map_(options), flags_(flags),
...
void LDBCommand::OpenDB()
{
	...
	if (is_db_ttl_) {
		...
		if (!opt_set && column_families_.empty()) {
			...
			st = DB::ListColumnFamilies(DBOptions(), db_path_,
						    &cf_list);
			...
			if (st.ok() && cf_list.size() > 1) {
				...
				for (auto cf_name : cf_list) {
					column_families_.emplace_back(cf_name,
								      opt);
				}
			}
		}
		if (is_read_only_) {
			if (column_families_.empty()) {
				...
				st = DB::OpenForReadOnly(opt, db_path_,
							 column_families_,
							 &handles_opened, &db_);
			}
		} else {
			if (column_families_.empty()) {
				...
				st = DB::Open(opt, db_path_, column_families_,
					      &handles_opened, &db_);
			}
		}
	}
	if (!st.ok()) {
		...
		for (size_t i = 0; i < handles_opened.size(); i++) {
			cf_handles_[column_families_[i].name] =
				handles_opened[i];
...
LDBCommand::BuildCmdLineOptions(std::vector<std::string> options)
{
...
Options LDBCommand::PrepareOptionsForOpenDB()
{
	...
	if (ParseIntOption(option_map_, ARG_BLOOM_BITS, bits, exec_state_)) {
		if (bits > 0) {
			...
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_BLOOM_BITS + " must be > 0.");
		}
	}

	int block_size;
	if (ParseIntOption(option_map_, ARG_BLOCK_SIZE, block_size,
			   exec_state_)) {
		if (block_size > 0) {
			use_table_options = true;
	...
	if (ParseIntOption(option_map_, ARG_COMPRESSION_MAX_DICT_BYTES,
		...
		if (compression_max_dict_bytes >= 0) {
			opt.compression_opts.max_dict_bytes =
				compression_max_dict_bytes;
		} else {
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_COMPRESSION_MAX_DICT_BYTES +
	...
	if (ParseIntOption(option_map_, ARG_DB_WRITE_BUFFER_SIZE,
		...
		if (db_write_buffer_size >= 0) {
			...
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_DB_WRITE_BUFFER_SIZE + " must be >= 0.");
		}
	}
	...
	if (ParseIntOption(option_map_, ARG_WRITE_BUFFER_SIZE,
		...
		if (write_buffer_size > 0) {
			...
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_WRITE_BUFFER_SIZE + " must be > 0.");
		}
	}

	int file_size;
	if (ParseIntOption(option_map_, ARG_FILE_SIZE, file_size,
			   exec_state_)) {
		if (file_size > 0) {
			opt.target_file_size_base = file_size;
		} else {
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_FILE_SIZE + " must be > 0.");
		}
	}

	if (opt.db_paths.size() == 0) {
		opt.db_paths.emplace_back(db_path_,
	...
	if (ParseIntOption(option_map_, ARG_FIX_PREFIX_LEN, fix_prefix_len,
			   exec_state_)) {
		if (fix_prefix_len > 0) {
			opt.prefix_extractor.reset(NewFixedPrefixTransform(
				static_cast<size_t>(fix_prefix_len)));
		} else {
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_FIX_PREFIX_LEN + " must be > 0.");
		}
	}
...
CompactorCommand::CompactorCommand(
	...
	: LDBCommand(options, flags, false,
		     BuildCmdLineOptions({ ARG_FROM, ARG_TO, ARG_HEX,
...
DBLoaderCommand::DBLoaderCommand(
	...
	: LDBCommand(options, flags, false,
	  ...
	  disable_wal_(false), bulk_load_(false), compact_(false)
{
...
void DBLoaderCommand::DoCommand()
{
	...
	while (getline(*istream_p, line, '\n')) {
		std::string key;
		std::string value;
		if (ParseKeyValue(line, &key, &value, is_key_hex_,
				  is_value_hex_)) {
			db_->Put(write_options, GetCfHandle(), Slice(key),
				 Slice(value));
	...
	if (bad_lines > 0) {
		std::cout << "Warning: " << bad_lines << " bad lines ignored."
			  << std::endl;
	}
	if (compact_) {
		db_->CompactRange(CompactRangeOptions(), GetCfHandle(), nullptr,
				  nullptr);
	}
}
...
ManifestDumpCommand::ManifestDumpCommand(
	...
	: LDBCommand(options, flags, false,
	  ...
	  verbose_(false), json_(false), path_("")
{
...
void ManifestDumpCommand::DoCommand()
{
	...
	if (!path_.empty()) {
		...
		std::unique_ptr<DIR, decltype(CloseDir)> d(
			opendir(db_path_.c_str()), CloseDir);
		...
		while ((entry = readdir(d.get())) != nullptr) {
			unsigned int match;
			uint64_t num;
			if (sscanf(entry->d_name, "MANIFEST-%" PRIu64 "%n",
				   &num, &match) &&
...
void ListColumnFamiliesCommand::DoCommand()
{
	...
	Status s =
		DB::ListColumnFamilies(DBOptions(), dbname_, &column_families);
	if (!s.ok()) {
...
InternalDumpCommand::InternalDumpCommand(
	...
	: LDBCommand(options, flags, true,
		     BuildCmdLineOptions({ ARG_HEX, ARG_KEY_HEX, ARG_VALUE_HEX,
					   ...
					   ARG_COUNT_ONLY, ARG_COUNT_DELIM,
					   ARG_STATS, ARG_INPUT_KEY_HEX })),
	  has_from_(false), has_to_(false), max_keys_(-1), delim_("."),
	  count_only_(false), count_delim_(false), print_stats_(false),
	...
	for (auto &key_version : key_versions) {
		...
		if (count_delim_) {
			...
			for (k = 0; val[k] != '\x01' && val[k] != '\0'; k++)
				s1++;
			for (int j = 0; row[j] != delim_[0] && row[j] != '\0' &&
					row[j] != '\x01';
			     j++)
				rtype1 += row[j];
			if (rtype2.compare("") && rtype2.compare(rtype1) != 0) {
				fprintf(stdout, "%s => count:%lld\tsize:%lld\n",
					rtype2.c_str(), (long long)c,
					(long long)s2);
				c = 1;
				s2 = s1;
				rtype2 = rtype1;
		...
		if (!count_only_ && !count_delim_) {
			...
			std::string value =
				Slice(key_version.value).ToString(is_value_hex_);
			std::cout << key << " => " << value << "\n";
		}
	...
	: LDBCommand(options, flags, true,
		     BuildCmdLineOptions(
			     { ARG_TTL, ARG_HEX, ARG_KEY_HEX, ARG_VALUE_HEX,
			       ARG_FROM, ARG_TO, ARG_MAX_KEYS, ARG_COUNT_ONLY,
			       ARG_COUNT_DELIM, ARG_STATS, ARG_TTL_START,
			       ARG_TTL_END, ARG_TTL_BUCKET, ARG_TIMESTAMP,
			       ARG_PATH })),
	  null_from_(true), null_to_(true), max_keys_(-1), count_only_(false),
	  count_delim_(false), print_stats_(false)
{
	std::map<std::string, std::string>::const_iterator itr =
		options.find(ARG_FROM);
	if (itr != options.end()) {
	...
	if (itr != options.end()) {
		try {
			...
			exec_state_ = LDBCommandExecuteResult::Failed(
				ARG_MAX_KEYS + " has an invalid value");
		} catch (const std::out_of_range &) {
			exec_state_ = LDBCommandExecuteResult::Failed(
...
void DBDumperCommand::DoCommand()
{
	if (!db_) {
		...
		if (!ParseFileName(fileName, &number, &type)) {
			exec_state_ = LDBCommandExecuteResult::Failed(
				"Can't parse file type: " + path_);
			return;
		}

		switch (type) {
		case kLogFile:
			DumpWalFile(path_, /* print_header_ */ true,
				    /* print_values_ */ true, &exec_state_);
			break;
		case kTableFile:
		...
		case kDescriptorFile:
			DumpManifestFile(path_, /* verbose_ */ false,
					 is_key_hex_,
					 /*  json_ */ false);
			break;
		default:
...
void DBDumperCommand::DoDumpCommand()
{
	...
	int ttl_start;
	if (!ParseIntOption(option_map_, ARG_TTL_START, ttl_start,
			    exec_state_)) {
		ttl_start =
			DBWithTTLImpl::kMinTimestamp; // TTL introduction time
	}
	int ttl_end;
	if (!ParseIntOption(option_map_, ARG_TTL_END, ttl_end, exec_state_)) {
		ttl_end = DBWithTTLImpl::
			kMaxTimestamp; // Max time allowed by TTL feature
	}
	...
	int bucket_size;
	if (!ParseIntOption(option_map_, ARG_TTL_BUCKET, bucket_size,
			    exec_state_) ||
	    bucket_size <= 0) {
	...
	for (; iter->Valid(); iter->Next()) {
		...
		if (is_db_ttl_ && num_buckets > 1) {
			IncBucketCounts(bucket_counts, ttl_start, time_range,
					bucket_size, rawtime, num_buckets);
		}
		++count;
		if (count_delim_) {
			rtype1 = "";
	...
	if (num_buckets > 1 && is_db_ttl_) {
		PrintBucketCounts(bucket_counts, ttl_start, ttl_end,
...
ReduceDBLevelsCommand::ReduceDBLevelsCommand(
	...
	: LDBCommand(options, flags, false,
	  ...
	  old_levels_(1 << 7), new_levels_(-1), print_old_levels_(false)
{
...
std::vector<std::string>
ReduceDBLevelsCommand::PrepareArgs(const std::string &db_path, int new_levels,
...
Status ReduceDBLevelsCommand::GetOldNumOfLevels(Options &opt, int *levels)
{
	...
	EnvOptions soptions;
	std::shared_ptr<Cache> tc(NewLRUCache(opt.max_open_files - 10,
					      opt.table_cache_numshardbits));
	const InternalKeyComparator cmp(opt.comparator);
	WriteController wc(opt.delayed_write_rate);
	WriteBufferManager wb(opt.db_write_buffer_size);
	VersionSet versions(db_path_, &db_options, soptions, tc.get(), &wb,
			    &wc);
...
void ReduceDBLevelsCommand::DoCommand()
{
	...
	EnvOptions soptions;
	st = VersionSet::ReduceNumberOfLevels(db_path_, &opt, soptions,
					      new_levels_);
	if (!st.ok()) {
		exec_state_ = LDBCommandExecuteResult::Failed(st.ToString());
		return;
...
ChangeCompactionStyleCommand::ChangeCompactionStyleCommand(
	...
	: LDBCommand(options, flags, false,
	  ...
	  old_compaction_style_(-1), new_compaction_style_(-1)
{
...
void ChangeCompactionStyleCommand::Help(std::string &ret)
{
	...
	ret.append(" --" + ARG_OLD_COMPACTION_STYLE +
		   "=<Old compaction style: 0 " +
		   "for level compaction, 1 for universal compaction>");
	ret.append(" --" + ARG_NEW_COMPACTION_STYLE +
	...
	for (int i = 0; i < db_->NumberLevels(GetCfHandle()); i++) {
		...
		if (i > 0 && num_files != 0) {
			exec_state_ = LDBCommandExecuteResult::Failed(
				"Number of db files at "
				"level " +
				ToString(i) + " after compaction is " +
...
class InMemoryHandler : public WriteBatch::Handler {
	...
	void commonPutMerge(const Slice &key, const Slice &value)
	{
		...
		if (print_values_) {
			std::string v =
				LDBCommand::StringToHex(value.ToString());
			row_ << k << " : ";
			row_ << v << " ";
		} else {
			row_ << k << " ";
	...
	if (!status.ok()) {
		if (exec_state) {
			*exec_state = LDBCommandExecuteResult::Failed(
			...
			std::cerr << "Error: Failed to open WAL file "
		...
		log::Reader reader(db_options.info_log,
				   std::move(wal_file_reader), &reporter, true,
				   0, log_number);
		std::string scratch;
		WriteBatch batch;
		Slice record;
		...
		for (size_t i = 0; i < params.size(); i += 2) {
			...
			key_values_.push_back(
				std::pair<std::string, std::string>(
					...
					is_value_hex_ ? HexToString(value) :
							      value));
		}
	}
...
ScanCommand::ScanCommand(const std::vector<std::string> &params,
	...
	: LDBCommand(options, flags, true,
	  ...
	  start_key_specified_(false), end_key_specified_(false),
	  max_keys_scanned_(-1), no_value_(false)
{
	std::map<std::string, std::string>::const_iterator itr =
		options.find(ARG_FROM);
	if (itr != options.end()) {
...
void ScanCommand::DoCommand()
{
	...
	for (; it->Valid() &&
	       (!end_key_specified_ || it->key().ToString() < end_key_);
	     it->Next()) {
		if (is_db_ttl_) {
		...
		if (is_key_hex_) {
			formatted_key =
				"0x" + key_slice.ToString(true /* hex */);
			key_slice = formatted_key;
		} else if (ldb_options_.key_formatter) {
			formatted_key =
				ldb_options_.key_formatter->Format(key_slice);
			key_slice = formatted_key;
		}
		...
		if (max_keys_scanned_ >= 0 &&
		    num_keys_scanned >= max_keys_scanned_) {
			break;
		}
	}
...
void DeleteRangeCommand::DoCommand()
{
	...
	Status st = db_->DeleteRange(WriteOptions(), GetCfHandle(), begin_key_,
				     end_key_);
	if (st.ok()) {
		fprintf(stdout, "OK\n");
	} else {
...
void DBQuerierCommand::DoCommand()
{
	...
	while (getline(std::cin, line, '\n')) {
		...
		if (cmd == HELP_CMD) {
			...
			key = (is_key_hex_ ? HexToString(tokens[1]) :
						   tokens[1]);
			...
			key = (is_key_hex_ ? HexToString(tokens[1]) :
						   tokens[1]);
			value = (is_value_hex_ ? HexToString(tokens[2]) :
						       tokens[2]);
			db_->Put(write_options, GetCfHandle(), Slice(key),
				 Slice(value));
			...
			key = (is_key_hex_ ? HexToString(tokens[1]) :
						   tokens[1]);
			if (db_->Get(read_options, GetCfHandle(), Slice(key),
				     &value)
				    .ok()) {
				fprintf(stdout, "%s\n",
					PrintKeyValue(key, value, is_key_hex_,
						      is_value_hex_)
						.c_str());
			} else {
...
void CheckPointCommand::DoCommand()
{
	...
	if (status.ok()) {
		...
		exec_state_ =
			LDBCommandExecuteResult::Failed(status.ToString());
	}
}
	  ...
	  num_threads_(1)
{
	...
	if (itr == options.end()) {
		exec_state_ = LDBCommandExecuteResult::Failed(
			"--" + ARG_BACKUP_DIR + ": missing backup directory");
	} else {
	...
	if (itr != options.end()) {
		...
		if (stderr_log_level < 0 ||
			...
			logger_.reset(new StderrLogger(
				static_cast<InfoLogLevel>(stderr_log_level)));
		}
	}
}
...
void BackupCommand::DoCommand()
{
	...
	Env *custom_env =
		NewCustomObject<Env>(backup_env_uri_, &custom_env_guard);
	BackupableDBOptions backup_options =
		BackupableDBOptions(backup_dir_, custom_env);
	backup_options.info_log = logger_.get();
	backup_options.max_background_operations = num_threads_;
	status = BackupEngine::Open(Env::Default(), backup_options,
				    &backup_engine);
	if (status.ok()) {
...
RestoreCommand::RestoreCommand(const std::vector<std::string> &params,
			       const std::map<std::string, std::string> &options,
			       const std::vector<std::string> &flags)
	: BackupableCommand(params, options, flags)
{
...
void RestoreCommand::DoCommand()
{
	...
	if (status.ok()) {
		...
		status = restore_engine->RestoreDBFromLatestBackup(db_path_,
								   db_path_);
	}
	if (status.ok()) {
...
void DumpSstFile(std::string filename, bool output_hex, bool show_properties)
{
	...
	Status st = reader.ReadSequential(true, -1, false, // has_from
					  ...
					  to_key);
	if (!st.ok()) {
		std::cerr << "Error in reading SST file " << filename
...
void DBFileDumperCommand::DoCommand()
{
	...
	for (auto &fileMetadata : metadata) {
		...
		std::cout << filename << " level:" << fileMetadata.level
			  << std::endl;
		std::cout << "------------------------------" << std::endl;
	...
	if (!s.ok()) {
		...
		for (auto &wal : wal_files) {
			...
			std::string filename =
				db_->GetOptions().wal_dir + wal->PathName();
