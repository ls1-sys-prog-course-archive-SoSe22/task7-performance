...
class ReduceLevelTest : public testing::Test {
	...
	void MoveL0FileToLevel(int level)
	{
		...
		for (int i = 0; i < level; ++i) {
			ASSERT_OK(db_impl->TEST_CompactRange(i, nullptr,
							     nullptr));
		}
	}
...
bool ReduceLevelTest::ReduceLevels(int target_level)
{
	...
	LDBCommand *level_reducer =
		LDBCommand::InitFromCmdLineArgs(args, Options(), LDBOptions(),
						nullptr,
...
{
	fprintf(stderr,
		"SKIPPED as LDBCommand is not supported in ROCKSDB_LITE\n");
	return 0;
}
