...
class WriteStress {
	...
	void WriteThread()
	{
		...
		while (!stop_.load(std::memory_order_relaxed)) {
			...
			for (int i = 0; i < kPrefixSize; ++i) {
				prefix[i] = key_prefix_[i].load(
					std::memory_order_relaxed);
			}
			auto key = prefix +
				   random_string(rng,
						 FLAGS_key_size - kPrefixSize);
	...
	void IteratorHoldThread()
	{
		while (!stop_.load(std::memory_order_relaxed)) {
			...
			Env::Default()->SleepForMicroseconds(
				FLAGS_iterator_hold_sec * 1000 * 1000LL);
	...
	int Run()
	{
		...
		Env::Default()->SleepForMicroseconds(FLAGS_runtime_sec * 1000 *
						     1000);
...
// function GetLiveFilesMetaData
		...
		for (const auto &child : children) {
			...
			if (type == kTableFile) {
				if (sst_file_numbers.find(number) ==
				    sst_file_numbers.end()) {
					fprintf(stderr,
