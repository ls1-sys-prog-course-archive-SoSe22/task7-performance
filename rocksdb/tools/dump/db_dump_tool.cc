...
namespace rocksdb
{
bool DbDumpTool::Run(const DumpOptions &dump_options, rocksdb::Options options)
{
	rocksdb::DB *dbptr;
	...
	status = env->NewWritableFile(dump_options.dump_location, &dumpfile,
				      rocksdb::EnvOptions());
	if (!status.ok()) {
		std::cerr << "Unable to open dump file '"
			  << dump_options.dump_location
			  << "' for writing: " << status.ToString()
			  << std::endl;
		return false;
	}
	...
	if (!status.ok()) {
		std::cerr << "Append failed: " << status.ToString()
			  << std::endl;
		return false;
	}
		       ...
		       rocksdb::Options options)
{
	...
	if (!status.ok()) {
		std::cerr
			<< "Unable to open database '" << undump_options.db_path
	...
	while (1) {
		...
		if (keysize > last_keysize) {
			while (keysize > last_keysize)
				last_keysize *= 2;
			keyscratch =
				std::unique_ptr<char[]>(new char[last_keysize]);
		}
		...
		if (valsize > last_valsize) {
			while (valsize > last_valsize)
				last_valsize *= 2;
			valscratch =
				std::unique_ptr<char[]>(new char[last_valsize]);
		}
	...
	if (undump_options.compact_db) {
		status = db->CompactRange(rocksdb::CompactRangeOptions(),
					  nullptr, nullptr);
		if (!status.ok()) {
			fprintf(stderr,
