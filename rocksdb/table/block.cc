...
void BlockIter::Prev()
{
	...
	if (prev_entries_idx_ > 0 &&
		...
		if (current_prev_entry.key_ptr != nullptr) {
			...
			key_ptr = prev_entries_keys_buff_.data() +
				  current_prev_entry.key_offset;
			key_pinned_ = false;
		}
	...
	do {
		...
		if (key_.IsKeyPinned()) {
			...
			prev_entries_.emplace_back(current_, current_key.data(),
						   0, current_key.size(),
						   value());
		} else {
			// The key is delta encoded, cache decoded key in buffer
			size_t new_key_offset = prev_entries_keys_buff_.size();
			prev_entries_keys_buff_.append(current_key.data(),
						       current_key.size());

			prev_entries_.emplace_back(current_, nullptr,
						   new_key_offset,
...
void BlockIter::Seek(const Slice &target)
{
	...
	while (true) {
		if (!ParseNextKey() ||
		    Compare(key_.GetInternalKey(), target) >= 0) {
			return;
		}
	}
}
...
bool BlockIter::ParseNextKey()
{
	...
	if (p == nullptr || key_.Size() < shared) {
		...
		if (shared == 0) {
			...
			key_.SetInternalKey(Slice(p, non_shared),
					    false /* copy */);
			key_pinned_ = true;
		} else {
		...
		if (global_seqno_ != kDisableGlobalSequenceNumber) {
			...
			ValueType value_type =
				ExtractValueType(key_.GetInternalKey());
				     ...
				     uint32_t *index)
{
	...
	if (left == right) {
		...
		if (block_ids[left] > 0 &&
		    (left == left_bound ||
		     block_ids[left - 1] != block_ids[left] - 1) &&
...
bool BlockIter::PrefixSeek(const Slice &target, uint32_t *index)
{
	...
	if (num_blocks == 0) {
		...
		return BinaryBlockIndexSeek(target, block_ids, 0,
					    num_blocks - 1, index);
	}
}
...
Block::Block(BlockContents &&contents, SequenceNumber _global_seqno,
	...
	: contents_(std::move(contents)), data_(contents_.data.data()),
	  size_(contents_.data.size()), global_seqno_(_global_seqno)
{
				     ...
				     bool total_order_seek, Statistics *stats)
{
	...
	if (num_restarts == 0) {
		...
		if (iter != nullptr) {
			iter->Initialize(cmp, data_, restart_offset_,
					 num_restarts, prefix_index_ptr,
					 global_seqno_, read_amp_bitmap_.get());
		} else {
			iter = new BlockIter(cmp, data_, restart_offset_,
					     num_restarts, prefix_index_ptr,
					     global_seqno_,
