...
FilterBlockBuilder *
CreateFilterBlockBuilder(const ImmutableCFOptions &opt,
			 const BlockBasedTableOptions &table_opt,
			 PartitionedIndexBuilder *const p_index_builder)
{
	if (table_opt.filter_policy == nullptr)
		return nullptr;

	FilterBitsBuilder *filter_bits_builder =
		table_opt.filter_policy->GetFilterBitsBuilder();
	if (filter_bits_builder == nullptr) {
		return new BlockBasedFilterBlockBuilder(opt.prefix_extractor,
							table_opt);
	} else {
		if (table_opt.partition_filters) {
			assert(p_index_builder != nullptr);
			return new PartitionedFilterBlockBuilder(
				opt.prefix_extractor,
		    ...
		    std::string *compressed_output)
{
	...
	case kSnappyCompression:
		if (Snappy_Compress(compression_options, raw.data(), raw.size(),
				    compressed_output) &&
		    GoodCompressionRatio(compressed_output->size(),
					 raw.size())) {
	...
	case kBZip2Compression:
		if (BZip2_Compress(compression_options,
				   GetCompressFormatForVersion(
					   kBZip2Compression, format_version),
				   raw.data(), raw.size(), compressed_output) &&
		    GoodCompressionRatio(compressed_output->size(),
					 raw.size())) {
			return *compressed_output;
	...
	case kLZ4HCCompression:
		if (LZ4HC_Compress(compression_options,
				   GetCompressFormatForVersion(
					   kLZ4HCCompression, format_version),
				   raw.data(), raw.size(), compressed_output,
				   compression_dict) &&
		    GoodCompressionRatio(compressed_output->size(),
					 raw.size())) {
...
struct BlockBasedTableBuilder::Rep {
	...
	Rep(const ImmutableCFOptions &_ioptions,
		...
		: ioptions(_ioptions), table_options(table_opt),
		  internal_comparator(icomparator), file(f),
		  data_block(table_options.block_restart_interval,
			     table_options.use_delta_encoding),
		  range_del_block(
			  1), // TODO(andrewkr): restart_interval unnecessary
		  ...
		  flush_block_policy(
			  table_options.flush_block_policy_factory
				  ->NewFlushBlockPolicy(table_options,
							data_block)),
		  ...
		  column_family_name(_column_family_name)
	{
		...
		if (skip_filters) {
			...
			filter_builder.reset(CreateFilterBlockBuilder(
				_ioptions, table_options, p_index_builder));
		}
		...
		if (should_flush) {
			...
			if (ok()) {
				r->index_builder->AddIndexEntry(
					&r->last_key, &key, r->pending_handle);
			}
		}
					...
					BlockHandle *handle, bool is_data_block)
{
	...
	if (raw_block_contents.size() < kCompressionSizeLimit) {
		...
		if (is_data_block && r->compression_dict &&
		    r->compression_dict->size()) {
			compression_dict = *r->compression_dict;
		}

		block_contents =
			CompressBlock(raw_block_contents, r->compression_opts,
		...
		if (type != kNoCompression &&
			...
			Status stat = UncompressBlockContentsForCompressionType(
				...
				&contents, r->table_options.format_version,
				compression_dict, type, r->ioptions);

			if (stat.ok()) {
				bool compressed_ok =
					contents.data.compare(
...
{
	Rep *r = rep_;
	StopWatch sw(r->ioptions.env, r->ioptions.statistics,
	...
	if (r->status.ok()) {
		...
		case kCRC32c: {
			auto crc = crc32c::Value(block_contents.data(),
						 block_contents.size());
		...
		case kxxHash: {
			...
			XXH32_update(
				xxh, block_contents.data(),
				static_cast<uint32_t>(block_contents.size()));
			XXH32_update(xxh, trailer,
				     1); // Extend  to cover block type
		...
		if (r->status.ok()) {
			r->status = InsertBlockInCache(block_contents, type,
						       handle);
		}
		if (r->status.ok()) {
...
Status BlockBasedTableBuilder::Finish()
{
	...
	if (ok() && !empty_data_block) {
		r->index_builder->AddIndexEntry(
			&r->last_key, nullptr /* no next data block */,
			r->pending_handle);
	}

	BlockHandle filter_block_handle, metaindex_block_handle,
		index_block_handle, compression_dict_block_handle,
	...
	if (ok()) {
		...
		// Write properties and compression dictionary blocks.
		{
			...
			r->props.filter_policy_name =
				r->table_options.filter_policy != nullptr ?
					      r->table_options.filter_policy->Name() :
					      "";
			r->props.index_size =
				r->index_builder->EstimatedSize() +
				kBlockTrailerSize;
			r->props.comparator_name =
				r->ioptions.user_comparator != nullptr ?
					      r->ioptions.user_comparator->Name() :
					      "nullptr";
			r->props.merge_operator_name =
				r->ioptions.merge_operator != nullptr ?
					      r->ioptions.merge_operator->Name() :
					      "nullptr";
			r->props.compression_name =
				CompressionTypeToString(r->compression_type);
			...
			property_collectors_names += "]";
			r->props.property_collectors_names =
				property_collectors_names;
			...
			NotifyCollectTableCollectorsOnFinish(
				...
				r->ioptions.info_log, &property_block_builder);

			BlockHandle properties_block_handle;
			WriteRawBlock(property_block_builder.Finish(),
				      kNoCompression, &properties_block_handle);
			meta_index_builder.Add(kPropertiesBlock,
					       properties_block_handle);

			// Write compression dictionary block
			if (r->compression_dict &&
			    r->compression_dict->size()) {
				WriteRawBlock(*r->compression_dict,
					      kNoCompression,
		...
		if (ok() && !r->range_del_block.empty()) {
			WriteRawBlock(r->range_del_block.Finish(),
				      kNoCompression, &range_del_block_handle);
			meta_index_builder.Add(kRangeDelBlock,
					       range_del_block_handle);
	...
	if (ok()) {
		...
		WriteBlock(index_blocks.index_block_contents,
