...
class PlainTableKeyEncoder {
	...
	explicit PlainTableKeyEncoder(EncodingType encoding_type,
		...
		: encoding_type_((prefix_extractor != nullptr) ? encoding_type :
								       kPlain),
		  fixed_user_key_len_(user_key_len),
		  prefix_extractor_(prefix_extractor),
		  index_sparseness_((index_sparseness > 1) ? index_sparseness :
								   1),
	...
	Status AppendKey(const Slice &key, WritableFileWriter *file,
...
class PlainTableFileReader {
	...
	bool Read(uint32_t file_offset, uint32_t len, Slice *out)
	{
		if (file_info_->is_mmap_mode) {
			assert(file_offset + len <=
			       file_info_->data_end_offset);
			*out = Slice(file_info_->file_data.data() + file_offset,
				     len);
			return true;
		} else {
	...
	const PlainTableReaderFileInfo *file_info()
	{
		return file_info_;
	}
...
class PlainTableKeyDecoder {
	...
	explicit PlainTableKeyDecoder(const PlainTableReaderFileInfo *file_info,
		...
		: file_reader_(file_info), encoding_type_(encoding_type),
		  prefix_len_(0), fixed_user_key_len_(user_key_len),
		  prefix_extractor_(prefix_extractor), in_prefix_(false)
	{
	}
	// Find the next key.
	...
	Status NextKeyNoValue(uint32_t start_offset,
