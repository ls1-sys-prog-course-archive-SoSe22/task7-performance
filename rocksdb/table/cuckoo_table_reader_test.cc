...
class CuckooReaderTest : public testing::Test {
		...
		const Comparator *ucomp = BytewiseComparator())
	{
		...
		ASSERT_OK(env->NewWritableFile(fname, &writable_file,
					       env_options));
		unique_ptr<WritableFileWriter> file_writer(
			new WritableFileWriter(std::move(writable_file),
					       env_options));

		CuckooTableBuilder builder(file_writer.get(), 0.9, kNumHashFunc,
					   100, ucomp, 2, false, false,
					   GetSliceHash,
					   0 /* column_family_id */,
					   kDefaultColumnFamilyName);
		ASSERT_OK(builder.status());
		for (uint32_t key_idx = 0; key_idx < num_items; ++key_idx) {
			builder.Add(Slice(keys[key_idx]),
				    Slice(values[key_idx]));
			ASSERT_OK(builder.status());
		...
		ASSERT_OK(env->NewRandomAccessFile(fname, &read_file,
						   env_options));
		unique_ptr<RandomAccessFileReader> file_reader(
			new RandomAccessFileReader(std::move(read_file)));
		const ImmutableCFOptions ioptions(options);
		CuckooTableReader reader(ioptions, std::move(file_reader),
		...
		for (uint32_t i = 0; i < num_items; ++i) {
			...
			GetContext get_context(ucomp, nullptr, nullptr, nullptr,
					       ...
					       Slice(user_keys[i]), &value,
					       nullptr, nullptr, nullptr,
					       nullptr);
			ASSERT_OK(reader.Get(ReadOptions(), Slice(keys[i]),
					     &get_context));
...
TEST_F(CuckooReaderTest, WhenKeyNotFound)
{
	...
	GetContext get_context(ucmp, nullptr, nullptr, nullptr,
			       GetContext::kNotFound, Slice(not_found_key),
			       &value, nullptr, nullptr, nullptr, nullptr);
	ASSERT_OK(
		reader.Get(ReadOptions(), Slice(not_found_key), &get_context));
	...
	GetContext get_context2(ucmp, nullptr, nullptr, nullptr,
				...
				&value, nullptr, nullptr, nullptr, nullptr);
	ASSERT_OK(reader.Get(ReadOptions(), Slice(not_found_key2),
			     &get_context2));
	ASSERT_TRUE(value.empty());
	ASSERT_OK(reader.status());
	...
	GetContext get_context3(ucmp, nullptr, nullptr, nullptr,
				GetContext::kNotFound, Slice(unused_key),
				&value, nullptr, nullptr, nullptr, nullptr);
...
void GetKeys(uint64_t num, std::vector<std::string> *keys)
{
	...
	for (uint64_t key_idx = 0; key_idx < num; ++key_idx) {
		...
		std::string new_key(reinterpret_cast<char *>(&value),
				    sizeof(value));
...
void ReadKeys(uint64_t num, uint32_t batch_size)
{
	...
	fprintf(stderr,
		"With %" PRIu64 " items, utilization is %.2f%%, number of"
		" hash functions: %u.\n",
	...
	if (batch_size > 0) {
		for (uint64_t i = 0; i < num; i += batch_size) {
			for (uint64_t j = i; j < i + batch_size && j < num;
			     ++j) {
				reader.Prepare(Slice(
					reinterpret_cast<char *>(&keys[j]),
					16));
			}
			for (uint64_t j = i; j < i + batch_size && j < num;
			     ++j) {
				reader.Get(r_options,
					   Slice(reinterpret_cast<char *>(
							 &keys[j]),
						 16),
					   &get_context);
			}
		...
		for (uint64_t i = 0; i < num; i++) {
			reader.Get(r_options,
				   Slice(reinterpret_cast<char *>(&keys[i]),
					 16),
				   &get_context);
		}
	}
