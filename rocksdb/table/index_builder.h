...
class IndexBuilder {
	...
	virtual Status
	Finish(IndexBlocks *index_blocks,
...
class ShortenedIndexBuilder : public IndexBuilder {
				   ...
				   const BlockHandle &block_handle) override
	{
		if (first_key_in_next_block != nullptr) {
			comparator_->FindShortestSeparator(
				last_key_in_current_block,
				*first_key_in_next_block);
		} else {
			comparator_->FindShortSuccessor(
				last_key_in_current_block);
		}

		std::string handle_encoding;
		block_handle.EncodeTo(&handle_encoding);
		index_block_builder_.Add(*last_key_in_current_block,
					 handle_encoding);
	}
...
class HashIndexBuilder : public IndexBuilder {
	...
	virtual void OnKeyAdded(const Slice &key) override
	{
		...
		if (is_first_entry || pending_entry_prefix_ != key_prefix) {
			...
			auto last_restart_index =
				pending_entry_index_ + pending_block_num_ - 1;
	       ...
	       const BlockHandle &last_partition_block_handle) override
	{
		...
		index_blocks->meta_blocks.insert(
			{ kHashIndexPrefixesMetadataBlock.c_str(),
			  prefix_meta_block_ });
		return Status::OK();
	}
	...
	virtual size_t EstimatedSize() const override
	{
		return primary_index_builder_.EstimatedSize() +
		       prefix_block_.size() + prefix_meta_block_.size();
	}
...
class PartitionedIndexBuilder : public IndexBuilder {
    public:
	static PartitionedIndexBuilder *
	CreateIndexBuilder(const rocksdb::InternalKeyComparator *comparator,
			   const BlockBasedTableOptions &table_opt);

	explicit PartitionedIndexBuilder(
