...
class Constructor {
	...
	virtual Status
	FinishImpl(const Options &options, const ImmutableCFOptions &ioptions,
		   const BlockBasedTableOptions &table_options,
		   const InternalKeyComparator &internal_comparator,
...
class TableConstructor : public Constructor {
		   ...
		   const stl_wrappers::KVMap &kv_map) override
	{
		...
		for (const auto kv : kv_map) {
			if (convert_to_internal_key_) {
				ParsedInternalKey ikey(kv.first,
						       kMaxSequenceNumber,
						       kTypeValue);
				std::string encoded;
				AppendInternalKey(&encoded, ikey);
		...
		file_reader_.reset(test::GetRandomAccessFileReader(
			new test::StringSource(GetSink()->contents(), uniq_id_,
					       ioptions.allow_mmap_reads)));
		return ioptions.table_factory->NewTableReader(
			TableReaderOptions(ioptions, soptions,
					   internal_comparator),
			std::move(file_reader_), GetSink()->contents().size(),
			&table_reader_);
	}
	...
	{
		return static_cast<test::StringSink *>(
			file_writer_->writable_file());
	}
...
class MemTableConstructor : public Constructor {
	...
	explicit MemTableConstructor(const Comparator *cmp,
		...
		: Constructor(cmp), internal_comparator_(cmp),
		  write_buffer_manager_(wb), table_factory_(new SkipListFactory)
	{
		options_.memtable_factory = table_factory_;
		ImmutableCFOptions ioptions(options_);
		memtable_ = new MemTable(internal_comparator_, ioptions,
					 MutableCFOptions(options_), wb,
					 kMaxSequenceNumber,
					 0 /* column_family_id */);
		memtable_->Ref();
	}
	...
	virtual Status
	FinishImpl(const Options &, const ImmutableCFOptions &ioptions,
		   const BlockBasedTableOptions &table_options,
		   const InternalKeyComparator &internal_comparator,
		   const stl_wrappers::KVMap &kv_map) override
	{
		...
		memtable_ =
			new MemTable(internal_comparator_, mem_ioptions,
...
class DBConstructor : public Constructor {
	...
	virtual InternalIterator *NewIterator() const override
	{
		return new InternalIteratorFromIterator(
		...
		for (auto reverse_compare : reverse_compare_types) {
			...
			for (auto restart_interval : restart_intervals) {
				for (auto compression_type :
					...
					TestArgs one_arg;
					one_arg.type = test_type;
					one_arg.reverse_compare =
						reverse_compare;
					one_arg.restart_interval =
						restart_interval;
					one_arg.compression =
						compression_type.first;
					one_arg.format_version =
						compression_type.second ? 2 : 1;
					one_arg.use_mmap = false;
					test_args.push_back(one_arg);
				}
			}
...
class HarnessTest : public testing::Test {
	...
	void Init(const TestArgs &args)
	{
		...
		internal_comparator_.reset(new test::PlainInternalKeyComparator(
			options_.comparator));
...
// Plain table is not supported in ROCKSDB_LITE
		...
		case PLAIN_TABLE_TOTAL_ORDER:
...

			{
				...
				plain_table_options.user_key_len =
		...
		case BLOCK_TEST:
			...
			options_.table_factory.reset(
				new BlockBasedTableFactory(table_options_));
			constructor_ =
				new BlockConstructor(options_.comparator);
			break;
		case MEMTABLE_TEST:
			table_options_.block_size = 256;
			options_.table_factory.reset(
			...
			constructor_ = new MemTableConstructor(
				options_.comparator, &write_buffer_);
			break;
		case DB_TEST:
			table_options_.block_size = 256;
			options_.table_factory.reset(
			     ...
			     const stl_wrappers::KVMap &data)
	{
		...
		iter->SeekToFirst();
		for (stl_wrappers::KVMap::const_iterator model_iter =
			     data.begin();
		...
		ASSERT_TRUE(!iter->Valid());
		if (constructor_->IsArenaMode() &&
		    !constructor_->AnywayDeleteIterator()) {
			      ...
			      const stl_wrappers::KVMap &data)
	{
		...
		ASSERT_TRUE(!iter->Valid());
		iter->SeekToLast();
		for (stl_wrappers::KVMap::const_reverse_iterator model_iter =
			     data.rbegin();
		...
		for (int i = 0; i < 200; i++) {
			...
			case 0: {
				if (iter->Valid()) {
					...
					iter->Next();
					++model_iter;
					ASSERT_EQ(ToString(data, model_iter),
						  ToString(iter));
				}
				break;
			}
	...
	std::string
	ToString(const stl_wrappers::KVMap &data,
				  ...
				  const std::vector<std::string> &keys)
	{
		if (keys.empty()) {
			...
			const int index =
				rnd->Uniform(static_cast<int>(keys.size()));
...
class TableTest : public testing::Test {
	...
	GetPlainInternalComparator(const Comparator *comp)
	{
...
TEST_F(TablePropertyTest, PrefixScanTest)
{
	UserCollectedProperties props{
		{ "num.111.1", "1" }, { "num.111.2", "2" },
		{ "num.111.3", "3" }, { "num.333.1", "1" },
		{ "num.333.2", "2" }, { "num.333.3", "3" },
		{ "num.555.1", "1" }, { "num.555.2", "2" },
		{ "num.555.3", "3" },
	};
...
TEST_F(BlockBasedTableTest, BlockBasedTableProperties2)
{
...

	{
		...
		const ImmutableCFOptions ioptions(options);
		c.Finish(options, ioptions, table_options,
			 GetPlainInternalComparator(options.comparator), &keys,
			 &kvmap);
...

	{
		...
		ASSERT_EQ("rocksdb.ReverseBytewiseComparator",
		...
		ASSERT_EQ(
			"[DummyPropertiesCollector1,DummyPropertiesCollector2]",
			props.property_collectors_names);
		ASSERT_EQ("",
			  props.filter_policy_name); // no filter policy is used
		c.ResetTableReader();
	}
}
...
TEST_F(BlockBasedTableTest, RangeDelBlock)
{
	...
	for (int j = 0; j < 2; ++j) {
		std::unique_ptr<InternalIterator> iter(
			c.GetTableReader()->NewRangeTombstoneIterator(
				ReadOptions()));
		if (j > 0) {
		       ...
		       bool convert = false)
{
	if (convert) {
		for (auto key : keys_in_cache) {
			InternalKey ikey(key, kMaxSequenceNumber, kTypeValue);
			ASSERT_TRUE(table_reader->TEST_KeyInCache(
				ReadOptions(), ikey.Encode()));
		}
		...
		for (auto key : keys_in_cache) {
			ASSERT_TRUE(table_reader->TEST_KeyInCache(ReadOptions(),
								  key));
		}
		for (auto key : keys_not_in_cache) {
			ASSERT_TRUE(!table_reader->TEST_KeyInCache(
		   ...
		   const Status expected_status = Status::OK())
{
	...
	if (key_begin != nullptr) {
		if (c->ConvertToInternalKey()) {
			i_begin.reset(new InternalKey(
				key_begin, kMaxSequenceNumber, kTypeValue));
	...
	if (key_end != nullptr) {
		if (c->ConvertToInternalKey()) {
			i_end.reset(new InternalKey(key_end, kMaxSequenceNumber,
						    kTypeValue));
...
TEST_F(BlockBasedTableTest, PrefetchTest)
{
	...
	PrefetchRange(&c, &opt, &table_options, "k01", "k01",
		      { "k01", "k02", "k03" }, { "k04", "k05", "k06", "k07" });
	// odd
	PrefetchRange(&c, &opt, &table_options, "a", "z",
		      { "k01", "k02", "k03", "k04", "k05", "k06", "k07" }, {});
	PrefetchRange(&c, &opt, &table_options, "k00", "k00",
		      { "k01", "k02", "k03" }, { "k04", "k05", "k06", "k07" });
	// Edge cases
...
void TableTest::IndexTest(BlockBasedTableOptions table_options)
{
	...
	std::vector<std::string> prefixes = { "001", "003", "005", "007",
					      "009" };
	...
	for (size_t i = 0; i < prefixes.size(); ++i) {
		index_iter->Seek(
			InternalKey(prefixes[i], 0, kTypeValue).Encode());
	...
	std::vector<std::string> non_exist_prefixes = { "002", "004", "006",
							"008" };
	for (const auto &prefix : non_exist_prefixes) {
		...
		if (index_iter->Valid()) {
			...
			Slice ukey_prefix =
				options.prefix_extractor->Transform(ukey);
			ASSERT_TRUE(BytewiseComparator()->Compare(
					    prefix, ukey_prefix) < 0);
		}
	}
	c.ResetTableReader();
}
...
TEST_F(TableTest, PartitionIndexTest)
{
	...
	const int est_max_index_size =
		max_index_keys * est_max_index_key_value_size;
...
TEST_F(BlockBasedTableTest, IndexSizeStat)
{
	...
	for (size_t i = 1; i < keys.size(); ++i) {
		...
		const ImmutableCFOptions ioptions(options);
		c.Finish(options, ioptions, table_options,
			 GetPlainInternalComparator(options.comparator), &ks,
			 &kvmap);
		auto index_size =
			c.GetTableReader()->GetTableProperties()->index_size;
...
class BlockCachePropertiesSnapshot {
	...
	explicit BlockCachePropertiesSnapshot(Statistics *statistics)
	{
		...
		filter_block_cache_hit =
			statistics->getTickerCount(BLOCK_CACHE_FILTER_HIT);
		block_cache_bytes_read =
			statistics->getTickerCount(BLOCK_CACHE_BYTES_READ);
	...
	{
		ASSERT_EQ(expected_filter_block_cache_miss,
			  filter_block_cache_miss);
		ASSERT_EQ(expected_filter_block_cache_hit,
			  filter_block_cache_hit);
	}
			 ...
			 int64_t expected_data_block_cache_hit) const
	{
		...
		ASSERT_EQ(expected_index_block_cache_hit,
			  index_block_cache_hit);
		ASSERT_EQ(expected_data_block_cache_miss,
			  data_block_cache_miss);
...
TEST_F(BlockBasedTableTest, BlockCacheDisabledTest)
{
...

	{
		GetContext get_context(options.comparator, nullptr, nullptr,
				       nullptr, GetContext::kNotFound, Slice(),
...
void ValidateBlockSizeDeviation(int value, int expected)
{
	...
	BlockBasedTableFactory *factory =
		new BlockBasedTableFactory(table_options);
...
TEST_F(BlockBasedTableTest, BlockReadCountTest)
{
	...
	for (int bloom_filter_type = 0; bloom_filter_type < 2;
	     ++bloom_filter_type) {
		for (int index_and_filter_in_cache = 0;
		     index_and_filter_in_cache < 2;
			...
			auto reader = c.GetTableReader();
			PinnableSlice value;
			GetContext get_context(options.comparator, nullptr,
					       nullptr, nullptr,
					       GetContext::kNotFound, user_key,
					       &value, nullptr, nullptr,
					       nullptr, nullptr);
			get_perf_context()->Reset();
			ASSERT_OK(reader->Get(ReadOptions(), encoded_key,
					      &get_context));
			if (index_and_filter_in_cache) {
				// data, index and filter block
				ASSERT_EQ(get_perf_context()->block_read_count,
					  3);
			} else {
				// just the data block
				ASSERT_EQ(get_perf_context()->block_read_count,
...
class MockCache : public LRUCache {
    public:
	MockCache(size_t capacity, int num_shard_bits,
			      ...
			      Priority priority = Priority::LOW) override
	{
		...
		return ShardedCache::Insert(key, value, charge, &MockDeleter,
					    handle, priority);
	}
	...
	for (auto index_type :
		...
		for (bool block_based_filter : { true, false }) {
			for (bool partition_filter : { true, false }) {
				...
				for (bool index_and_filter_in_cache :
				     { true, false }) {
					for (bool pin_l0 : { true, false }) {
						...
						// Create a table
						Options opt;
						unique_ptr<InternalKeyComparator>
							ikc;
						ikc.reset(
						...
						const ImmutableCFOptions
							ioptions(opt);
						c.Finish(opt, ioptions,
							 table_options, *ikc,
							 &keys, &kvmap);

						// Doing a read to make index/filter loaded into the cache
						auto table_reader = dynamic_cast<
							BlockBasedTable *>(
							c.GetTableReader());
						PinnableSlice value;
						GetContext get_context(
							opt.comparator, nullptr,
							nullptr, nullptr,
							GetContext::kNotFound,
							user_key, &value,
							nullptr, nullptr,
							nullptr, nullptr);
						InternalKey ikey(user_key, 0,
								 kTypeValue);
						auto s = table_reader->Get(
							ReadOptions(), key,
							&get_context);
...
TEST_F(PlainTableTest, BasicPlainTableProperties)
{
	...
	std::unique_ptr<TableBuilder> builder(factory.NewTableBuilder(
		TableBuilderOptions(ioptions, ikc,
				    &int_tbl_prop_collector_factories,
	...
	for (char c = 'a'; c <= 'z'; ++c) {
		std::string key(8, c);
		key.append(
			"\1       "); // PlainTable expects internal key structure
...
TEST_F(GeneralTableTest, ApproximateOffsetOfPlain)
{
	...
	stl_wrappers::KVMap kvmap;
	Options options;
	test::PlainInternalKeyComparator internal_comparator(
		options.comparator);
	options.compression = kNoCompression;
	BlockBasedTableOptions table_options;
	table_options.block_size = 1024;
...
TEST_F(HarnessTest, RandomizedLongDB)
{
	...
	for (int level = 0; level < db()->NumberLevels(); level++) {
		std::string value;
		char name[100];
		snprintf(name, sizeof(name), "rocksdb.num-files-at-level%d",
			 level);
...
TEST_F(MemTableTest, Simple)
{
	...
	ASSERT_TRUE(WriteBatchInternal::InsertInto(&batch, &cf_mems_default,
						   nullptr)
			    .ok());
...
TEST_F(BlockBasedTableTest, TableWithGlobalSeqno)
{
	...
	int_tbl_prop_collector_factories.emplace_back(
		new SstFileWriterPropertiesCollectorFactory(
			2 /* version */, 0 /* global_seqno*/));
	std::string column_family_name;
	...
	std::function<void()> GetVersionAndGlobalSeqno = [&]() {
		unique_ptr<RandomAccessFileReader> file_reader(
			test::GetRandomAccessFileReader(new test::StringSource(
				ss_rw.contents(), 73342, true)));

		TableProperties *props = nullptr;
		ASSERT_OK(ReadTableProperties(
			file_reader.get(), ss_rw.contents().size(),
		...
		version = DecodeFixed32(
			user_props[ExternalSstFilePropertyNames::kVersion]
				.c_str());
		global_seqno = DecodeFixed64(
			user_props[ExternalSstFilePropertyNames::kGlobalSeqno]
				.c_str());
		global_seqno_offset =
			props->properties_offsets
	...
	std::function<InternalIterator *()> GetTableInternalIter = [&]() {
		...
		options.table_factory->NewTableReader(
			TableReaderOptions(ioptions, EnvOptions(), ikc),
			std::move(file_reader), ss_rw.contents().size(),
			&table_reader);
