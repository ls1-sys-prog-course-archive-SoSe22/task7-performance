...
class BlockPrefixIndex::Builder {
	...
	BlockPrefixIndex *Finish()
	{
		...
		uint32_t num_buckets =
			static_cast<uint32_t>(prefixes_.size()) + 1;
		...
		for (PrefixRecord *current : prefixes_) {
			uint32_t bucket =
				PrefixToBucket(current->prefix, num_buckets);
			...
			if (prev) {
				...
				if (distance <= 1) {
					...
					prev->num_blocks = prev->end_block -
							   prev->start_block +
							   1;
					num_blocks_per_bucket[bucket] +=
						(current->num_blocks +
						 distance - 1);
					continue;
				}
		...
		for (uint32_t i = 0; i < num_buckets; i++) {
			...
			if (num_blocks == 0) {
				...
				buckets[i] =
					prefixes_per_bucket[i]->start_block;
			} else {
		...
		return new BlockPrefixIndex(internal_prefix_extractor_,
					    num_buckets, buckets,
					    total_block_array_entries,
					    block_array_buffer);
	}
