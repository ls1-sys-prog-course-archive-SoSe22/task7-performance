...
CuckooTableReader::CuckooTableReader(
	...
	: file_(std::move(file)), ucomp_(comparator),
	  get_slice_hash_(get_slice_hash)
{
	...
	status_ =
		ReadTableProperties(file_.get(), file_size,
	...
	auto hash_funs =
		user_props.find(CuckooTablePropertyNames::kNumHashFunc);
	...
	num_hash_func_ =
		*reinterpret_cast<const uint32_t *>(hash_funs->second.data());
	...
	auto user_key_len =
		user_props.find(CuckooTablePropertyNames::kUserKeyLength);
	...
	auto value_length =
		user_props.find(CuckooTablePropertyNames::kValueLength);
	...
	auto is_last_level =
		user_props.find(CuckooTablePropertyNames::kIsLastLevel);
	if (is_last_level == user_props.end()) {
		status_ = Status::Corruption("Is last level not found");
		return;
	}
	is_last_level_ =
		*reinterpret_cast<const bool *>(is_last_level->second.data());
			      ...
			      GetContext *get_context, bool skip_filters)
{
	...
	for (uint32_t hash_cnt = 0; hash_cnt < num_hash_func_; ++hash_cnt) {
		...
		for (uint32_t block_idx = 0; block_idx < cuckoo_block_size_;
			...
			if (ucomp_->Equal(Slice(unused_key_.data(),
						user_key.size()),
			...
			if (ucomp_->Equal(user_key,
					  Slice(bucket, user_key.size()))) {
				Slice value(bucket + key_length_,
					    value_length_);
				if (is_last_level_) {
					...
					get_context->SaveValue(found_ikey,
							       value);
				}
...
void CuckooTableReader::Prepare(const Slice &key)
{
	...
	uint64_t addr =
		...
		bucket_length_ * CuckooHash(user_key, 0, use_module_hash_,
					    table_size_,
					    identity_as_first_hash_, nullptr);
	uint64_t end_addr = addr + cuckoo_block_bytes_minus_one_;
	for (addr &= CACHE_LINE_MASK; addr < end_addr;
...
class CuckooTableIterator : public InternalIterator {
	...
	struct BucketComparator {
		BucketComparator(const Slice &file_data,
				 const Comparator *ucomp, uint32_t bucket_len,
				 uint32_t user_key_len,
				 const Slice &target = Slice())
			: file_data_(file_data), ucomp_(ucomp),
			  bucket_len_(bucket_len), user_key_len_(user_key_len),
			  target_(target)
		{
		}
		bool operator()(const uint32_t first,
				const uint32_t second) const
		{
			...
			return ucomp_->Compare(
				       Slice(first_bucket, user_key_len_),
...
CuckooTableIterator::CuckooTableIterator(CuckooTableReader *reader)
	: bucket_comparator_(reader->file_data_, reader->ucomp_,
	  ...
	  reader_(reader), initialized_(false), curr_key_idx_(kInvalidIndex)
{
...
void CuckooTableIterator::InitIfNeeded()
{
	...
	uint64_t num_buckets =
		reader_->table_size_ + reader_->cuckoo_block_size_ - 1;
	...
	for (uint32_t bucket_id = 0; bucket_id < num_buckets; ++bucket_id) {
		if (Slice(bucket, reader_->key_length_) !=
		    Slice(reader_->unused_key_)) {
...
void CuckooTableIterator::Seek(const Slice &target)
{
	...
	auto seek_it = std::lower_bound(sorted_bucket_ids_.begin(),
					sorted_bucket_ids_.end(), kInvalidIndex,
					seek_comparator);
	curr_key_idx_ = static_cast<uint32_t>(
		std::distance(sorted_bucket_ids_.begin(), seek_it));
	PrepareKVAtCurrIdx();
}
...
void CuckooTableIterator::PrepareKVAtCurrIdx()
{
	...
	if (reader_->is_last_level_) {
		...
		curr_key_.SetInternalKey(Slice(offset,
					       reader_->user_key_length_),
					 0, kTypeValue);
	} else {
		curr_key_.SetInternalKey(Slice(offset, reader_->key_length_));
	}
	curr_value_ =
		Slice(offset + reader_->key_length_, reader_->value_length_);
}
...
void CuckooTableIterator::Prev()
{
	if (curr_key_idx_ == 0) {
		curr_key_idx_ =
			static_cast<uint32_t>(sorted_bucket_ids_.size());
	}
	if (!Valid()) {
		curr_value_.clear();
		curr_key_.Clear();
...
InternalIterator *
CuckooTableReader::NewIterator(const ReadOptions &read_options, Arena *arena,
			       ...
			       bool skip_filters)
{
	...
	if (arena == nullptr) {
		...
		auto iter_mem =
			arena->AllocateAligned(sizeof(CuckooTableIterator));
