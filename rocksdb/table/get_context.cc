...
void appendToReplayLog(std::string *replay_log, ValueType type, Slice value)
{
	...
	if (replay_log) {
		if (replay_log->empty()) {
			...
			replay_log->reserve(1 + VarintLength(value.size()) +
					    value.size());
		}
...
GetContext::GetContext(const Comparator *ucmp,
	...
	: ucmp_(ucmp), merge_operator_(merge_operator), logger_(logger),
	  ...
	  merge_context_(merge_context), range_del_agg_(_range_del_agg),
	  env_(env), seq_(seq), replay_log_(nullptr),
			   ...
			   const Slice &value, Cleanable *value_pinner)
{
	...
	if (ucmp_->Equal(parsed_key.user_key, user_key_)) {
		...
		case kTypeMerge:
			...
			if (pinned_iters_mgr() &&
			    pinned_iters_mgr()->PinningEnabled() &&
			    value_pinner != nullptr) {
				value_pinner->DelegateCleanupsTo(
					pinned_iters_mgr());
				merge_context_->PushOperand(
					value, true /*value_pinned*/);
			} else {
