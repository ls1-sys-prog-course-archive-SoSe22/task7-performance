...
class PlainTableIndex {
	...
	PlainTableIndex()
		: index_size_(0), sub_index_size_(0), num_prefixes_(0),
		  index_(nullptr), sub_index_(nullptr)
	{
	}
...
class PlainTableIndexBuilder {
	...
	PlainTableIndexBuilder(Arena *arena, const ImmutableCFOptions &ioptions,
		...
		: arena_(arena), ioptions_(ioptions),
		  record_list_(kRecordsPerGroup), is_first_record_(true),
		  due_index_(false), num_prefixes_(0), num_keys_per_prefix_(0),
		  prev_key_prefix_hash_(0), index_sparseness_(index_sparseness),
	...
	uint32_t GetTotalSize() const
	{
		return VarintLength(index_size_) + VarintLength(num_prefixes_) +
		       PlainTableIndex::kOffsetLen * index_size_ +
		       sub_index_size_;
	}
	...
	class IndexRecordList {
		...
		{
			IndexRecord *result =
				new IndexRecord[kNumRecordsPerGroup];
