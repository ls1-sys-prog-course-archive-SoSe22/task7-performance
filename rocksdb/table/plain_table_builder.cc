...
PlainTableBuilder::PlainTableBuilder(
	...
	uint32_t user_key_len, EncodingType encoding_type,
	size_t index_sparseness, uint32_t bloom_bits_per_key,
	const std::string &column_family_name, uint32_t num_probes,
	size_t huge_page_tlb_size, double hash_table_ratio,
	bool store_index_in_file)
	: ioptions_(ioptions), bloom_block_(num_probes), file_(file),
	  ...
	  prefix_extractor_(ioptions.prefix_extractor)
{
	...
	if (store_index_in_file_) {
		...
		index_builder_.reset(new PlainTableIndexBuilder(
			&arena_, ioptions, index_sparseness, hash_table_ratio,
			huge_page_tlb_size_));
	...
	properties_.prefix_extractor_name =
		ioptions_.prefix_extractor != nullptr ?
...
void PlainTableBuilder::Add(const Slice &key, const Slice &value)
{
	...
	if (store_index_in_file_) {
		if (ioptions_.prefix_extractor == nullptr) {
			keys_or_prefixes_hashes_.push_back(
				GetSliceHash(internal_key.user_key));
		} else {
			Slice prefix = ioptions_.prefix_extractor->Transform(
	...
	encoder_.AppendKey(key, file_, &offset_, meta_bytes_buf,
			   &meta_bytes_buf_size);
	if (SaveIndexInFile()) {
		index_builder_->AddKeyPrefix(GetPrefix(internal_key),
					     prev_offset);
	}

	// Write value length
	uint32_t value_size = static_cast<uint32_t>(value.size());
	char *end_ptr = EncodeVarint32(meta_bytes_buf + meta_bytes_buf_size,
				       value_size);
	...
	NotifyCollectTableCollectorsOnAdd(key, value, offset_,
...
Status PlainTableBuilder::Finish()
{
	...
	if (store_index_in_file_ && (properties_.num_entries > 0)) {
		assert(properties_.num_entries <=
		...
		if (bloom_bits_per_key_ > 0) {
			bloom_block_.SetTotalBits(
				&arena_,
				static_cast<uint32_t>(properties_.num_entries) *
					bloom_bits_per_key_,
			...
			s = WriteBlock(bloom_finish_result, file_, &offset_,
				       &bloom_block_handle);

			if (!s.ok()) {
				return s;
			}
			meta_index_builer.Add(BloomBlockBuilder::kBloomBlock,
					      bloom_block_handle);
		}
		...
		s = WriteBlock(index_finish_result, file_, &offset_,
			       &index_block_handle);
