...
PlainTableReader::PlainTableReader(const ImmutableCFOptions &ioptions,
	...
	: internal_comparator_(icomparator), encoding_type_(encoding_type),
	  full_scan_mode_(false),
	  user_key_len_(static_cast<uint32_t>(table_properties->fixed_key_len)),
	  prefix_extractor_(ioptions.prefix_extractor), enable_bloom_(false),
	  bloom_(6, nullptr),
	  file_info_(std::move(file), storage_options,
		     static_cast<uint32_t>(table_properties->data_size)),
	  ioptions_(ioptions), file_size_(file_size), table_properties_(nullptr)
{
}
			      ...
			      size_t huge_page_tlb_size, bool full_scan_mode)
{
	if (file_size > PlainTableIndex::kMaxFileSize) {
		return Status::NotSupported(
			"File is too large for PlainTableReader!");
	}
	...
	if (!full_scan_mode) {
		s = new_reader->PopulateIndex(props, bloom_bits_per_key,
					      hash_table_ratio,
					      index_sparseness,
					      huge_page_tlb_size);
		if (!s.ok()) {
...
Status
PlainTableReader::PopulateIndexRecordList(PlainTableIndexBuilder *index_builder,
					  vector<uint32_t> *prefix_hashes)
{
	...
	while (pos < file_info_.data_end_offset) {
		...
		Slice value_slice;
		bool seekable = false;
		Status s = Next(&decoder, &pos, &key, nullptr, &value_slice,
				&seekable);
		if (!s.ok()) {
			return s;
		}
		...
		if (enable_bloom_) {
			...
			if (is_first_record ||
				...
				if (!is_first_record) {
					prefix_hashes->push_back(GetSliceHash(
		...
		if (!seekable && is_first_record) {
			return Status::Corruption(
				"Key for a prefix is not seekable");
		}
					    ...
					    vector<uint32_t> *prefix_hashes)
{
	if (!IsTotalOrderMode()) {
		...
		if (bloom_total_bits > 0) {
			enable_bloom_ = true;
			bloom_.SetTotalBits(&arena_, bloom_total_bits,
					    ioptions_.bloom_locality,
					    huge_page_tlb_size,
					    ioptions_.info_log);
...
Status PlainTableReader::MmapDataIfNeeded()
{
	if (file_info_.is_mmap_mode) {
		// Get mmapped memory.
		return file_info_.file->Read(0, file_size_,
				       ...
				       size_t huge_page_tlb_size)
{
	...
	Status s = ReadMetaBlock(file_info_.file.get(), file_size_,
	...
	if (index_in_file) {
		s = ReadMetaBlock(file_info_.file.get(), file_size_,
				  kPlainTableMagicNumber, ioptions_,
	...
	if (!index_in_file) {
		...
		if (IsTotalOrderMode()) {
			...
			if (num_bloom_bits > 0) {
				enable_bloom_ = true;
				bloom_.SetTotalBits(&arena_, num_bloom_bits,
						    ioptions_.bloom_locality,
						    huge_page_tlb_size,
						    ioptions_.info_log);
			}
	...
	PlainTableIndexBuilder index_builder(&arena_, ioptions_,
	...
	if (!index_in_file) {
		...
		AllocateAndFillBloom(bloom_bits_per_key,
				     index_.GetNumPrefixes(),
				   ...
				   uint32_t *offset) const
{
	...
	const char *base_ptr = index_.GetSubIndexBasePtrAndUpperBound(
		prefix_index_offset, &upper_bound);
	uint32_t low = 0;
	...
	while (high - low > 1) {
		...
		uint32_t tmp;
		Status s = decoder->NextKeyNoValue(file_offset, &mid_key,
						   nullptr, &tmp);
		if (!s.ok()) {
			return s;
		}
		int cmp_result =
			internal_comparator_.Compare(mid_key, parsed_target);
		if (cmp_result < 0) {
			low = mid;
		} else {
			if (cmp_result == 0) {
	...
	Status s = decoder->NextKeyNoValue(low_key_offset, &low_key, nullptr,
					   &tmp);
	if (!s.ok()) {
		return s;
	}
			     ...
			     GetContext *get_context, bool skip_filters)
{
	...
	if (IsTotalOrderMode()) {
		if (full_scan_mode_) {
			status_ = Status::InvalidArgument(
				"Get() is not allowed in full scan mode.");
		}
	...
	while (offset < file_info_.data_end_offset) {
		...
		if (internal_comparator_.Compare(found_key, parsed_target) >=
		    0) {
...
void PlainTableIterator::SeekToLast()
{
	...
	status_ = Status::NotSupported(
		"SeekToLast() is not supported in PlainTable");
}
...
void PlainTableIterator::Seek(const Slice &target)
{
	...
	if (table_->IsTotalOrderMode()) {
		if (table_->full_scan_mode_) {
			status_ = Status::InvalidArgument(
				"Seek() is not allowed in full scan mode.");
			offset_ = next_offset_ =
				table_->file_info_.data_end_offset;
			return;
	...
	bool prefix_match;
	status_ = table_->GetOffset(&decoder_, target, prefix_slice,
				    prefix_hash, prefix_match, &next_offset_);
	if (!status_.ok()) {
		offset_ = next_offset_ = table_->file_info_.data_end_offset;
		return;
	}
	...
	if (next_offset_ < table_->file_info_.data_end_offset) {
		for (Next(); status_.ok() && Valid(); Next()) {
			...
			if (table_->internal_comparator_.Compare(key(),
								 target) >= 0) {
				break;
			}
		}
...
void PlainTableIterator::SeekForPrev(const Slice &target)
{
	assert(false);
	status_ = Status::NotSupported(
		"SeekForPrev() is not supported in PlainTable");
}
...
void PlainTableIterator::Next()
{
	...
	if (offset_ < table_->file_info_.data_end_offset) {
		Slice tmp_slice;
		ParsedInternalKey parsed_key;
		status_ = table_->Next(&decoder_, &next_offset_, &parsed_key,
				       &key_, &value_);
		if (!status_.ok()) {
			offset_ = next_offset_ =
				table_->file_info_.data_end_offset;
