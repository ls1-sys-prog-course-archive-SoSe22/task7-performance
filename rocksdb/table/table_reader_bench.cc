			  ...
			  bool through_db, bool measured_by_nanosecond)
{
	...
	std::string file_name =
		test::TmpDir() + "/rocksdb_table_reader_benchmark";
	...
	if (!through_db) {
		...
		s = opts.table_factory->NewTableReader(
			TableReaderOptions(ioptions, env_options, ikc),
		...
		for (int i = 0; i < num_keys1; i++) {
			for (int j = 0; j < num_keys2; j++) {
				...
				if (!for_iterator) {
					...
					std::string key =
						MakeKey(r1, r2, through_db);
					uint64_t start_time = Now(
						env, measured_by_nanosecond);
					if (!through_db) {
						PinnableSlice value;
					...
					std::string start_key =
						MakeKey(r1, r2, through_db);
					std::string end_key = MakeKey(
						r1, r2 + r2_len, through_db);
					...
					for (through_db ?
							   iter->Seek(start_key) :
							   iiter->Seek(start_key);
					     through_db ? iter->Valid() :
								iiter->Valid();
					     through_db ? iter->Next() :
								iiter->Next()) {
						if (if_query_empty_keys) {
							break;
						}
	...
	fprintf(stderr,
		...
		for_iterator ? "iterator" :
				     (if_query_empty_keys ? "empty" : "non_empty"),
...
int main(int argc, char **argv)
{
	...
	if (tf) {
		...
		rocksdb::TableReaderBenchmark(options, env_options, ro,
					      FLAGS_num_keys1, FLAGS_num_keys2,
					      FLAGS_iter, FLAGS_prefix_len,
