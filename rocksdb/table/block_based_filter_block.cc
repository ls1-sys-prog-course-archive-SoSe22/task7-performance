		...
		const std::string &value)
{
	...
	while (i < value.size()) {
		...
		value_str.append(&value[i], std::min(size_t(dataLength),
						     value.size() - i));
		i += dataLength;
	}
	...
	if (key.size() < (offLength - tabLength))
		result.append(size_t((offLength - tabLength)) - key.size(),
			      cspace);
	result.append(key);
...
BlockBasedFilterBlockBuilder::BlockBasedFilterBlockBuilder(
	...
	: policy_(table_opt.filter_policy.get()),
	  ...
	  prev_prefix_start_(0), prev_prefix_size_(0)
{
	assert(policy_);
}
...
inline void BlockBasedFilterBlockBuilder::AddPrefix(const Slice &key)
{
	...
	if (prev_prefix_size_ > 0) {
		prev = Slice(entries_.data() + prev_prefix_start_,
			     prev_prefix_size_);
	}
...
void BlockBasedFilterBlockBuilder::GenerateFilter()
{
	...
	if (num_entries == 0) {
		...
		filter_offsets_.push_back(
			static_cast<uint32_t>(result_.size()));
		return;
	}
...
BlockBasedFilterBlockReader::BlockBasedFilterBlockReader(
	...
	: FilterBlockReader(contents.data.size(), stats, _whole_key_filtering),
	  policy_(table_opt.filter_policy.get()),
	  prefix_extractor_(prefix_extractor), data_(nullptr), offset_(nullptr),
	  num_(0), base_lg_(0), contents_(std::move(contents))
{
	assert(policy_);
					   ...
					   uint64_t block_offset)
{
	...
	if (index < num_) {
		...
		if (start <= limit && limit <= (uint32_t)(offset_ - data_)) {
			...
			bool const may_match =
				policy_->KeyMayMatch(entry, filter);
			if (may_match) {
...
std::string BlockBasedFilterBlockReader::ToString() const
{
	...
	std::string s_bo("Block offset"), s_hd("Hex dump"),
		s_fb("# filter blocks");
