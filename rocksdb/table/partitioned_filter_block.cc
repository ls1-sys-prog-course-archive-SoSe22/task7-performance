	...
	const BlockHandle &last_partition_block_handle, Status *status)
{
	if (finishing_filters == true) {
		...
		index_on_filter_block_builder_.Add(last_entry.key,
						   handle_encoding);
		filters.pop_front();
	} else {
		MaybeCutAFilterBlock();
...
PartitionedFilterBlockReader::PartitionedFilterBlockReader(
	...
	: FilterBlockReader(contents.data.size(), stats, _whole_key_filtering),
	  prefix_extractor_(prefix_extractor), comparator_(comparator),
	  table_(table)
{
	idx_on_fltr_blk_.reset(
...
PartitionedFilterBlockReader::~PartitionedFilterBlockReader()
{
	{
		ReadLock rl(&mu_);
		for (auto it = handle_list_.begin(); it != handle_list_.end();
		     ++it) {
			table_->rep_->table_options.block_cache.get()->Release(
				*it);
		}
	}
	char cache_key[BlockBasedTable::kMaxCacheKeyPrefixSize +
		       kMaxVarint64Length];
	for (auto it = filter_block_set_.begin(); it != filter_block_set_.end();
	     ++it) {
		auto key = BlockBasedTable::GetCacheKey(
			table_->rep_->cache_key_prefix,
			table_->rep_->cache_key_prefix_size, *it, cache_key);
	...
	const Slice *const const_ikey_ptr)
{
	...
	auto filter_partition =
		GetFilterPartition(&filter_handle, no_io, &cached);
	if (UNLIKELY(!filter_partition.value)) {
		return true;
	}
	auto res =
		filter_partition.value->KeyMayMatch(key, block_offset, no_io);
	if (cached) {
		return res;
	}
	if (LIKELY(filter_partition.IsSet())) {
		filter_partition.Release(
			table_->rep_->table_options.block_cache.get());
	} else {
	...
	const Slice *const const_ikey_ptr)
{
	...
	auto res = filter_partition.value->PrefixMayMatch(prefix, kNotValid,
							  no_io);
	if (cached) {
		return res;
	}
...
Slice PartitionedFilterBlockReader::GetFilterPartitionHandle(const Slice &entry)
{
	BlockIter iter;
						 ...
						 const bool no_io, bool *cached)
{
	...
	if (LIKELY(block_cache != nullptr)) {
		...
		if (pin_cached_filters) {
			ReadLock rl(&mu_);
			auto iter =
				filter_cache_.find(fltr_blk_handle.offset());
		...
		auto filter = table_->GetFilter(fltr_blk_handle,
						is_a_filter_partition, no_io);
		if (filter.IsSet()) {
			WriteLock wl(&mu_);
			filter_block_set_.insert(fltr_blk_handle);
			if (pin_cached_filters) {
				std::pair<uint64_t, FilterBlockReader *> pair(
					fltr_blk_handle.offset(), filter.value);
				auto succ = filter_cache_.insert(pair).second;
				if (succ) {
					handle_list_.push_back(
						filter.cache_handle);
		...
		auto filter = table_->ReadFilter(fltr_blk_handle,
