...
class SstFileWriterPropertiesCollector : public IntTblPropCollector {
	...
	virtual Status Finish(UserCollectedProperties *properties) override
	{
		...
		properties->insert({ ExternalSstFilePropertyNames::kVersion,
				     version_val });

		// Global Sequence number
		std::string seqno_val;
		PutFixed64(&seqno_val, static_cast<uint64_t>(global_seqno_));
		properties->insert({ ExternalSstFilePropertyNames::kGlobalSeqno,
				     seqno_val });
	...
	{
		return { { ExternalSstFilePropertyNames::kVersion,
			   ToString(version_) } };
	}
...
class SstFileWriterPropertiesCollectorFactory
	...
	explicit SstFileWriterPropertiesCollectorFactory(
		int32_t version, SequenceNumber global_seqno)
		: version_(version), global_seqno_(global_seqno)
	{
	}
