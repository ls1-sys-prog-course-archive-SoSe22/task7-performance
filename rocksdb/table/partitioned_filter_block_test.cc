...
class MockedBlockBasedTable : public BlockBasedTable {
		  ...
		  const bool is_a_filter_partition, bool no_io) const override
	{
		...
		auto obj = new FullFilterBlockReader(
			nullptr, true,
			BlockContents(slice, false, kNoCompression),
			rep_->table_options.filter_policy->GetFilterBitsReader(
				slice),
			nullptr);
		return { obj, nullptr };
	}
...
class PartitionedFilterBlockTest : public testing::Test {
		...
		for (int i = 1; i < num_keys; i++) {
			max_key_size =
				std::max(max_key_size,
					 static_cast<uint64_t>(keys[i].size()));
		}
		uint64_t max_index_size =
			num_keys * (max_key_size + 8 /*handle*/);
	...
	PartitionedIndexBuilder *NewIndexBuilder()
	{
		return PartitionedIndexBuilder::CreateIndexBuilder(
			&icomp, table_options_);
	}

	PartitionedFilterBlockBuilder *
	NewBuilder(PartitionedIndexBuilder *const p_index_builder)
	{
	...
	NewReader(PartitionedFilterBlockBuilder *builder)
	{
		BlockHandle bh;
		Status status;
		Slice slice;
		...
		auto reader = new PartitionedFilterBlockReader(
			nullptr, true,
			BlockContents(slice, false, kNoCompression), nullptr,
			  ...
			  bool empty = false)
	{
		...
		for (auto key : keys) {
			...
			ASSERT_TRUE(reader->KeyMayMatch(key, kNotValid, !no_io,
							&ikey_slice));
		}
		{
			// querying a key twice
			auto ikey =
				InternalKey(keys[0], 0, ValueType::kTypeValue);
			const Slice ikey_slice = Slice(*ikey.rep());
			ASSERT_TRUE(reader->KeyMayMatch(keys[0], kNotValid,
							!no_io, &ikey_slice));
		}
		...
		for (auto key : missing_keys) {
			...
			if (empty) {
				ASSERT_TRUE(reader->KeyMayMatch(
					key, kNotValid, !no_io, &ikey_slice));
			} else {
		       ...
		       const std::string &user_key)
	{
		...
		std::string key = std::string(
			*InternalKey(user_key, 0, ValueType::kTypeValue).rep());
		       ...
		       const std::string &next_user_key)
	{
		...
		std::string next_key = std::string(
			*InternalKey(next_user_key, 0, ValueType::kTypeValue)
				 .rep());
		BlockHandle dont_care_block_handle(1, 1);
