...
// Return 0 for error
size_t EncodeSize(PlainTableEntryType type, uint32_t key_size, char *out_buffer)
{
	out_buffer[0] = type << 6;
	...
	if (key_size < static_cast<uint32_t>(kSizeInlineLimit)) {
		...
		char *ptr = EncodeVarint32(out_buffer + 1,
					       ...
					       uint32_t *bytes_read)
{
	...
	*entry_type = static_cast<PlainTableEntryType>(
		(static_cast<unsigned char>(next_byte_slice[0]) &
		 ~kSizeInlineLimit) >>
		6);
	...
	if (inline_key_size < kSizeInlineLimit) {
		...
		*bytes_read = 1;
		...
		success = file_reader_.ReadVarint32(
			start_offset + 1, &extra_size, &tmp_bytes_read);
		if (!success) {
			return file_reader_.status();
		}
				       ...
				       size_t *meta_bytes_buf_size)
{
	...
	if (encoding_type_ == kPlain) {
		...
		Slice prefix = prefix_extractor_->Transform(
			Slice(key.data(), user_key_size));
		if (key_count_for_prefix_ == 0 ||
		    prefix != pre_prefix_.GetUserKey() ||
		    key_count_for_prefix_ % index_sparseness_ == 0) {
			key_count_for_prefix_ = 1;
			pre_prefix_.SetUserKey(prefix);
			size_bytes_pos +=
				EncodeSize(kFullKey, user_key_size, size_bytes);
			Status s =
			...
			if (key_count_for_prefix_ == 2) {
				...
				size_bytes_pos += EncodeSize(
					...
					static_cast<uint32_t>(
						pre_prefix_.GetUserKey().size()),
					size_bytes + size_bytes_pos);
			}
			uint32_t prefix_len = static_cast<uint32_t>(
				pre_prefix_.GetUserKey().size());
			size_bytes_pos +=
				EncodeSize(kKeySuffix,
					   user_key_size - prefix_len,
					   size_bytes + size_bytes_pos);
			Status s =
				file->Append(Slice(size_bytes, size_bytes_pos));
			if (!s.ok()) {
			...
			*offset += size_bytes_pos;
			key_to_write = Slice(key.data() + prefix_len,
					     key.size() - prefix_len);
		}
	}
	...
	if (parsed_key.sequence == 0 && parsed_key.type == kTypeValue) {
		Status s = file->Append(
			Slice(key_to_write.data(), key_to_write.size() - 8));
		if (!s.ok()) {
			return s;
		}
		*offset += key_to_write.size() - 8;
		meta_bytes_buf[*meta_bytes_buf_size] =
			PlainTableFactory::kValueTypeSeqId0;
					  ...
					  uint32_t len)
{
	...
	return Slice(buffer->buf.get() +
			     (file_offset - buffer->buf_start_offset),
		     len);
}
				       ...
				       Slice *out)
{
	...
	for (uint32_t i = 0; i < num_buf_; i++) {
		...
		if (file_offset >= buffer->buf_start_offset &&
		    file_offset + len <=
			    buffer->buf_start_offset + buffer->buf_len) {
	...
	uint32_t size_to_read =
		std::min(file_info_->data_end_offset - file_offset,
	...
	Slice read_result;
	Status s = file_info_->file->Read(file_offset, size_to_read,
					       ...
					       uint32_t *bytes_read)
{
	if (file_info_->is_mmap_mode) {
		...
		const char *limit = file_info_->file_data.data() +
				    file_info_->data_end_offset;
					       ...
					       uint32_t *bytes_read)
{
	...
	uint32_t bytes_to_read = std::min(file_info_->data_end_offset - offset,
					  kMaxVarInt32Size);
	Slice bytes;
	...
	const char *key_ptr = GetVarint32Ptr(start, limit, out);
	*bytes_read = (key_ptr != nullptr) ?
				    static_cast<uint32_t>(key_ptr - start) :
				    0;
	return true;
}

Status PlainTableKeyDecoder::ReadInternalKey(uint32_t file_offset,
					     uint32_t user_key_size,
					     ParsedInternalKey *parsed_key,
					     uint32_t *bytes_read,
					     bool *internal_key_valid,
					     Slice *internal_key)
{
	Slice tmp_slice;
	bool success =
		file_reader_.Read(file_offset, user_key_size + 1, &tmp_slice);
	if (!success) {
		return file_reader_.status();
	}
	if (tmp_slice[user_key_size] == PlainTableFactory::kValueTypeSeqId0) {
		...
		*internal_key_valid = false;
		...
		success = file_reader_.Read(file_offset, user_key_size + 8,
					    internal_key);
		if (!success) {
			return file_reader_.status();
		}
		*internal_key_valid = true;
		if (!ParseInternalKey(*internal_key, parsed_key)) {
			return Status::Corruption(Slice(
				"Incorrect value type found when reading the next key"));
		}
						  ...
						  bool *seekable)
{
	...
	if (fixed_user_key_len_ != kPlainTableVariableLength) {
		...
		uint32_t tmp_size = 0;
		uint32_t tmp_read;
		bool success = file_reader_.ReadVarint32(start_offset,
							 &tmp_size, &tmp_read);
		if (!success) {
			return file_reader_.status();
		}
	...
	s = ReadInternalKey(start_offset + *bytes_read, user_key_size,
...
Status PlainTableKeyDecoder::NextPrefixEncodingKey(
	uint32_t start_offset, ParsedInternalKey *parsed_key,
	Slice *internal_key, uint32_t *bytes_read, bool *seekable)
{
	PlainTableEntryType entry_type;
	...
	do {
		...
		*bytes_read += my_bytes_read;
		...
		case kFullKey: {
			expect_suffix = false;
			Slice decoded_internal_key;
			s = ReadInternalKey(start_offset + *bytes_read, size,
					    parsed_key, bytes_read,
					    &decoded_internal_key_valid,
					    &decoded_internal_key);
			if (!s.ok()) {
				return s;
			}
			if (!file_reader_.file_info()->is_mmap_mode ||
			    (internal_key != nullptr &&
			     !decoded_internal_key_valid)) {
				...
				if (!file_reader_.file_info()->is_mmap_mode) {
					parsed_key->user_key = Slice(
						cur_key_.GetInternalKey().data(),
						size);
				}
				if (internal_key != nullptr) {
					*internal_key =
						cur_key_.GetInternalKey();
				}
			} else {
				if (internal_key != nullptr) {
					*internal_key = decoded_internal_key;
		...
		case kPrefixFromPreviousKey: {
			...
			assert(prefix_extractor_ == nullptr ||
			       prefix_extractor_->Transform(saved_user_key_)
					       .size() == prefix_len_);
		...
		case kKeySuffix: {
			...
			if (!file_reader_.file_info()->is_mmap_mode) {
				...
				std::string tmp = Slice(saved_user_key_.data(),
							prefix_len_)
							  .ToString();
				cur_key_.Reserve(prefix_len_ + size);
				cur_key_.SetInternalKey(tmp, *parsed_key);
				parsed_key->user_key =
					Slice(cur_key_.GetInternalKey().data(),
					      prefix_len_ + size);
				saved_user_key_ = cur_key_.GetUserKey();
			} else {
				cur_key_.Reserve(prefix_len_ + size);
				cur_key_.SetInternalKey(
					Slice(saved_user_key_.data(),
					      prefix_len_),
					*parsed_key);
			}
				     ...
				     uint32_t *bytes_read, bool *seekable)
{
	...
	Status s = NextKeyNoValue(start_offset, parsed_key, internal_key,
				  bytes_read, seekable);
	if (s.ok()) {
		...
		bool success =
			file_reader_.ReadVarint32(start_offset + *bytes_read,
						  &value_size,
						  &value_size_bytes);
		if (!success) {
			return file_reader_.status();
		...
		*bytes_read += value_size_bytes;
		success = file_reader_.Read(start_offset + *bytes_read,
					    value_size, value);
		if (!success) {
			return file_reader_.status();
		}
					    ...
					    bool *seekable)
{
	*bytes_read = 0;
	...
	Status s;
	if (encoding_type_ == kPlain) {
		return NextPlainEncodingKey(start_offset, parsed_key,
					    internal_key, bytes_read, seekable);
	} else {
		assert(encoding_type_ == kPrefix);
		return NextPrefixEncodingKey(start_offset, parsed_key,
					     internal_key, bytes_read,
					     seekable);
	}
}
