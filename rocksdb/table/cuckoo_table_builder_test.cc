...
uint64_t GetSliceHash(const Slice &s, uint32_t index, uint64_t max_num_buckets)
{
...
class CuckooBuilderTest : public testing::Test {
			       ...
			       uint32_t expected_cuckoo_block_size = 1)
	{
		...
		ASSERT_OK(env_->NewRandomAccessFile(fname, &read_file,
						    env_options_));
		...
		uint64_t value_len_found = *reinterpret_cast<const uint64_t *>(
			props->user_collected_properties
				[CuckooTablePropertyNames::kValueLength]
					.data());
		ASSERT_EQ(values.empty() ? 0 : values[0].size(),
			  value_len_found);
		ASSERT_EQ(props->raw_value_size,
			  values.size() * value_len_found);
		...
		const uint32_t cuckoo_block_size = *reinterpret_cast<
			const uint32_t *>(
			props->user_collected_properties
		...
		ASSERT_EQ(props->fixed_key_len,
			  keys.empty() ? 0 : keys[0].size());
		ASSERT_EQ(props->data_size,
			  expected_unused_bucket.size() *
				  (expected_table_size +
				   expected_cuckoo_block_size - 1));
		ASSERT_EQ(props->raw_key_size,
			  keys.size() * props->fixed_key_len);
		...
		for (uint32_t i = 0; i < table_size + cuckoo_block_size - 1;
		     ++i) {
			Slice read_slice;
			ASSERT_OK(file_reader->Read(i * bucket_size,
						    bucket_size, &read_slice,
						    nullptr));
			size_t key_idx =
				std::find(expected_locations.begin(),
					  expected_locations.end(), i) -
				expected_locations.begin();
			if (key_idx == keys.size()) {
				// i is not one of the expected locations. Empty bucket.
				ASSERT_EQ(read_slice.compare(
						  expected_unused_bucket),
					  0);
			} else {
				keys_found[key_idx] = true;
				ASSERT_EQ(read_slice.compare(keys[key_idx] +
							     values[key_idx]),
					  0);
			}
		}
	...
	{
		IterKey ikey;
		ikey.SetInternalKey(user_key, zero_seqno ? 0 : 1000,
				    kTypeValue);
...
TEST_F(CuckooBuilderTest, WriteSuccessNoCollisionFullKey)
{
	...
	unique_ptr<WritableFileWriter> file_writer(
		new WritableFileWriter(std::move(writable_file), EnvOptions()));
	CuckooTableBuilder builder(file_writer.get(), kHashTableRatio,
				   num_hash_fun, 100, BytewiseComparator(), 1,
	...
	CheckFileContents(keys, values, expected_locations,
			  expected_unused_bucket, expected_table_size, 2,
			  false);
}
...
TEST_F(CuckooBuilderTest, WriteSuccessWithCollisionFullKey)
{
	...
	expected_unused_bucket += std::string(values[0].size(), 'a');
	CheckFileContents(keys, values, expected_locations,
			  expected_unused_bucket, expected_table_size, 4,
			  false);
}
...
TEST_F(CuckooBuilderTest, WriteSuccessWithCollisionAndCuckooBlock)
{
	...
	expected_unused_bucket += std::string(values[0].size(), 'a');
	CheckFileContents(keys, values, expected_locations,
			  expected_unused_bucket, expected_table_size, 3, false,
			  cuckoo_block_size);
}
...
TEST_F(CuckooBuilderTest, WithCollisionPathFullKey)
{
	...
	// support operator= with initializer_list as a parameter
	std::unordered_map<std::string, std::vector<uint64_t> > hm = {
		{ user_keys[0], { 0, 1 } }, { user_keys[1], { 1, 2 } },
		{ user_keys[2], { 2, 3 } }, { user_keys[3], { 3, 4 } },
		{ user_keys[4], { 0, 2 } },
	};
	hash_map = std::move(hm);
...
TEST_F(CuckooBuilderTest, WithCollisionPathFullKeyAndCuckooBlock)
{
	...
	std::unordered_map<std::string, std::vector<uint64_t> > hm = {
		{ user_keys[0], { 0, 1 } }, { user_keys[1], { 1, 2 } },
		{ user_keys[2], { 3, 4 } }, { user_keys[3], { 4, 5 } },
		{ user_keys[4], { 0, 3 } },
	};
	hash_map = std::move(hm);
	...
	unique_ptr<WritableFileWriter> file_writer(
		new WritableFileWriter(std::move(writable_file), EnvOptions()));
	CuckooTableBuilder builder(file_writer.get(), kHashTableRatio,
				   num_hash_fun, 100, BytewiseComparator(), 2,
				   false, false, GetSliceHash,
				   0 /* column_family_id */,
				   kDefaultColumnFamilyName);
	ASSERT_OK(builder.status());
...
TEST_F(CuckooBuilderTest, WriteSuccessNoCollisionUserKey)
{
	...
	ASSERT_OK(builder.status());
	for (uint32_t i = 0; i < user_keys.size(); i++) {
		builder.Add(Slice(GetInternalKey(user_keys[i], true)),
			    Slice(values[i]));
		ASSERT_EQ(builder.NumEntries(), i + 1);
		ASSERT_OK(builder.status());
	}
...
TEST_F(CuckooBuilderTest, WithCollisionPathUserKey)
{
	...
	unique_ptr<WritableFileWriter> file_writer(
		new WritableFileWriter(std::move(writable_file), EnvOptions()));
	CuckooTableBuilder builder(file_writer.get(), kHashTableRatio,
				   num_hash_fun, 2, BytewiseComparator(), 1,
				   false, false, GetSliceHash,
				   0 /* column_family_id */,
				   kDefaultColumnFamilyName);
	ASSERT_OK(builder.status());
...
TEST_F(CuckooBuilderTest, FailWhenCollisionPathTooLong)
{
	...
	ASSERT_OK(builder.status());
	for (uint32_t i = 0; i < user_keys.size(); i++) {
		builder.Add(Slice(GetInternalKey(user_keys[i], false)),
			    Slice("value"));
		ASSERT_EQ(builder.NumEntries(), i + 1);
		ASSERT_OK(builder.status());
	}
