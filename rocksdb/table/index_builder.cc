	...
	const BlockBasedTableOptions &table_opt)
{
	...
	case BlockBasedTableOptions::kTwoLevelIndexSearch: {
		return PartitionedIndexBuilder::CreateIndexBuilder(comparator,
								   table_opt);
	}
	default: {
...
PartitionedIndexBuilder::PartitionedIndexBuilder(
	...
	: IndexBuilder(comparator),
	  ...
	  sub_index_builder_(nullptr), table_opt_(table_opt)
{
}
	...
	const Slice *first_key_in_next_block, const BlockHandle &block_handle)
{
	...
	if (UNLIKELY(first_key_in_next_block == nullptr)) { // no more keys
		...
		entries_.push_back({ sub_index_last_key_,
				     std::unique_ptr<ShortenedIndexBuilder>(
					     sub_index_builder_) });
		...
		if (sub_index_builder_ != nullptr) {
			...
			bool do_flush = flush_policy_->Update(
				*last_key_in_current_block, handle_encoding);
			if (do_flush) {
				entries_.push_back(
					{ sub_index_last_key_,
					  std::unique_ptr<ShortenedIndexBuilder>(
...
Status
PartitionedIndexBuilder::Finish(IndexBlocks *index_blocks,
...
size_t PartitionedIndexBuilder::EstimatedSize() const
{
	...
	total += sub_index_builder_ == nullptr ?
			       0 :
			       sub_index_builder_->EstimatedSize();
	return total;
}
} // namespace rocksdb
