...
CuckooTableBuilder::CuckooTableBuilder(
	...
	: num_hash_func_(2), file_(file),
	  ...
	  hash_table_size_(use_module_hash ? 0 : 2), is_last_level_file_(false),
	  has_seen_first_key_(false), has_seen_first_value_(false),
	  key_size_(0), value_size_(0), num_entries_(0), num_values_(0),
	  ucomp_(user_comparator), use_module_hash_(use_module_hash),
	  identity_as_first_hash_(identity_as_first_hash),
	  get_slice_hash_(get_slice_hash), closed_(false)
{
...
void CuckooTableBuilder::Add(const Slice &key, const Slice &value)
{
	...
	if (!ParseInternalKey(key, &ikey)) {
		status_ = Status::Corruption(
			"Unable to parse key into inernal key.");
		return;
	}
	...
	if (!has_seen_first_key_) {
		...
		smallest_user_key_.assign(ikey.user_key.data(),
					  ikey.user_key.size());
		largest_user_key_.assign(ikey.user_key.data(),
					 ikey.user_key.size());
		key_size_ =
	...
	if (ikey.type == kTypeValue) {
		...
		if (is_last_level_file_) {
			deleted_keys_.append(ikey.user_key.data(),
					     ikey.user_key.size());
		} else {
...
Slice CuckooTableBuilder::GetKey(uint64_t idx) const
{
	assert(closed_);
	if (IsDeletedKey(idx)) {
		return Slice(&deleted_keys_[(idx - num_values_) * key_size_],
			     key_size_);
	}
...
Status CuckooTableBuilder::MakeHashTable(std::vector<CuckooBucket> *buckets)
{
	...
	for (uint32_t vector_idx = 0; vector_idx < num_entries_; vector_idx++) {
		...
		for (uint32_t hash_cnt = 0;
		     hash_cnt < num_hash_func_ && !bucket_found; ++hash_cnt) {
			uint64_t hash_val =
				CuckooHash(user_key, hash_cnt, use_module_hash_,
					   hash_table_size_,
			...
			for (uint32_t block_idx = 0;
			     block_idx < cuckoo_block_size_;
			     ++block_idx, ++hash_val) {
				if ((*buckets)[hash_val].vector_idx ==
				    kMaxVectorIdx) {
					bucket_id = hash_val;
					bucket_found = true;
					break;
		...
		while (!bucket_found &&
		       !MakeSpaceForKey(hash_vals, ++make_space_for_key_call_id,
					buckets, &bucket_id)) {
			...
			uint64_t hash_val =
				CuckooHash(user_key, num_hash_func_,
					   use_module_hash_, hash_table_size_,
					   identity_as_first_hash_,
					   get_slice_hash_);
			++num_hash_func_;
			for (uint32_t block_idx = 0;
...
Status CuckooTableBuilder::Finish()
{
	...
	if (num_entries_ > 0) {
		...
		if (use_module_hash_) {
			hash_table_size_ = static_cast<uint64_t>(
		...
		while (curr_pos >= 0) {
			...
			if (Slice(unused_user_key).compare(smallest_user_key_) <
			    0) {
				break;
			}
			--curr_pos;
		...
		if (curr_pos < 0) {
			...
			while (curr_pos >= 0) {
				++unused_user_key[curr_pos];
				if (Slice(unused_user_key)
					    .compare(largest_user_key_) > 0) {
					break;
				}
				--curr_pos;
			}
	...
	properties_
		.user_collected_properties[CuckooTablePropertyNames::kValueLength]
		.assign(reinterpret_cast<const char *>(&value_size_),
			sizeof(value_size_));
	...
	properties_
		.user_collected_properties[CuckooTablePropertyNames::kEmptyKey] =
		unused_bucket;
	properties_
		.user_collected_properties[CuckooTablePropertyNames::kNumHashFunc]
		.assign(reinterpret_cast<char *>(&num_hash_func_),
	...
	properties_
		.user_collected_properties[CuckooTablePropertyNames::kIsLastLevel]
	...
	properties_
		...
		.assign(reinterpret_cast<const char *>(
				&identity_as_first_hash_),
	...
	uint32_t user_key_len =
		static_cast<uint32_t>(smallest_user_key_.size());
	properties_
		.user_collected_properties
...
uint64_t CuckooTableBuilder::FileSize() const
{
	...
	if (use_module_hash_) {
		...
		if (expected_hash_table_size <
		    (num_entries_ + 1) / max_hash_table_ratio_) {
	...
	std::vector<CuckooBucket> *buckets, uint64_t *bucket_id)
{
	struct CuckooNode {
		...
		uint32_t depth;
		uint32_t parent_pos;
		CuckooNode(uint64_t _bucket_id, uint32_t _depth,
			   int _parent_pos)
			: bucket_id(_bucket_id), depth(_depth),
	...
	for (uint32_t hash_cnt = 0; hash_cnt < num_hash_func_; ++hash_cnt) {
		...
		(*buckets)[bid].make_space_for_key_call_id =
	...
	while (!null_found && curr_pos < tree.size()) {
		...
		for (uint32_t hash_cnt = 0;
			...
			uint64_t child_bucket_id = CuckooHash(
				GetUserKey(curr_bucket.vector_idx), hash_cnt,
				use_module_hash_, hash_table_size_,
			...
			for (uint32_t block_idx = 0;
				...
				if ((*buckets)[child_bucket_id]
					    .make_space_for_key_call_id ==
				    make_space_for_key_call_id) {
					continue;
				}
				(*buckets)[child_bucket_id]
					.make_space_for_key_call_id =
					make_space_for_key_call_id;
				tree.push_back(CuckooNode(child_bucket_id,
							  curr_depth + 1,
							  curr_pos));
				if ((*buckets)[child_bucket_id].vector_idx ==
				    kMaxVectorIdx) {
					null_found = true;
					break;
				}
	...
	if (null_found) {
		...
		uint32_t bucket_to_replace_pos =
			static_cast<uint32_t>(tree.size()) - 1;
