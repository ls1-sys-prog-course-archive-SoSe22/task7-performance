		       ...
		       const int keys_share_prefix = 1)
{
	...
	for (int i = from; i < from + len; i += step) {
		...
		for (int j = 0; j < keys_share_prefix; ++j) {
			keys->emplace_back(
				GenerateKey(i, j, padding_size, &rnd));
...
TEST_F(BlockTest, SimpleIndexHash)
{
	...
	std::vector<std::string> values;
	GenerateRandomKVs(
		&keys, &values, 0 /* first key id */, kMaxKey /* last key id */,
		2 /* step */,
...
TEST_F(BlockTest, BlockReadAmpBitmap)
{
	...
	for (size_t block_size : block_sizes) {
		...
		std::sort(random_entry_offsets.begin(),
			  random_entry_offsets.end());
		auto it = std::unique(random_entry_offsets.begin(),
				      random_entry_offsets.end());
		...
		for (size_t i = 0; i < random_entries.size(); i++) {
			auto &current_entry =
				random_entries[rnd.Next() %
					       random_entries.size()];

			read_amp_bitmap.Mark(
				static_cast<uint32_t>(current_entry.first),
				static_cast<uint32_t>(current_entry.second));
			read_amp_slow_and_accurate.Mark(current_entry.first,
							current_entry.second);

			size_t total_bits = 0;
			for (size_t bit_idx = 0; bit_idx < needed_bits;
			     bit_idx++) {
				total_bits +=
					read_amp_slow_and_accurate.IsPinMarked(
						bit_idx * kBytesPerBit +
						pin_offset);
			}
			size_t expected_estimate_useful =
				total_bits * kBytesPerBit;
			size_t got_estimate_useful = stats->getTickerCount(
				READ_AMP_ESTIMATE_USEFUL_BYTES);
			ASSERT_EQ(expected_estimate_useful,
				  got_estimate_useful);
		}
	}
...
TEST_F(BlockTest, BlockWithReadAmpBitmap)
{
	...
	// Read the block sequentially using Next()
	{
		...
		BlockIter *iter = static_cast<BlockIter *>(reader.NewIterator(
			options.comparator, nullptr, true, stats.get()));
		for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
			...
			double semi_acc_read_amp =
				...
				rawblock.size();
			double read_amp =
				static_cast<double>(stats->getTickerCount(
					READ_AMP_ESTIMATE_USEFUL_BYTES)) /
			...
			// sequentially
			double error_pct =
				fabs(semi_acc_read_amp - read_amp) * 100;
...
TEST_F(BlockTest, ReadAmpBitmapPow2)
{
	...
	ASSERT_EQ(BlockReadAmpBitmap(100, 16, stats.get()).GetBytesPerBit(),
		  16);
	ASSERT_EQ(BlockReadAmpBitmap(100, 32, stats.get()).GetBytesPerBit(),
		  32);
	...
	ASSERT_EQ(BlockReadAmpBitmap(100, 17, stats.get()).GetBytesPerBit(),
		  16);
	ASSERT_EQ(BlockReadAmpBitmap(100, 33, stats.get()).GetBytesPerBit(),
		  32);
	ASSERT_EQ(BlockReadAmpBitmap(100, 35, stats.get()).GetBytesPerBit(),
		  32);
}
