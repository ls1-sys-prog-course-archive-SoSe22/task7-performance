			 ...
			 size_t read_amp_bytes_per_bit)
{
	...
	Status s = ReadBlockContents(file, footer, options, handle, &contents,
				     ioptions, do_uncompress, compression_dict,
				     cache_options);
	if (s.ok()) {
...
template <class ResourceType> void DeleteHeldResource(void *arg, void *ignored)
{
	delete reinterpret_cast<ResourceType *>(arg);
}

// Delete the entry resided in the cache.
template <class Entry> void DeleteCachedEntry(const Slice &key, void *value)
{
...
class PartitionIndexReader : public IndexReader, public Cleanable {
			     ...
			     const int level)
	{
		...
		auto s = ReadBlockFromFile(file, footer, ReadOptions(),
		...
		if (s.ok()) {
			*index_reader = new PartitionIndexReader(
				table, icomparator, std::move(index_block),
					      ...
					      bool dont_care = true) override
	{
		...
		return NewTwoLevelIterator(
			new BlockBasedTable::BlockEntryIteratorState(
				...
				skip_filters, is_index, block_cache_cleaner),
	...
	PartitionIndexReader(BlockBasedTable *table,
			     const InternalKeyComparator *icomparator,
			     std::unique_ptr<Block> &&index_block,
			     Statistics *stats, const int level)
		: IndexReader(icomparator, stats), table_(table),
		  index_block_(std::move(index_block)), level_(level)
	{
			     ...
			     const PersistentCacheOptions &cache_options)
	{
		...
		if (s.ok()) {
			*index_reader = new BinarySearchIndexReader(
				icomparator, std::move(index_block),
				ioptions.statistics);
		}
...
class HashIndexReader : public IndexReader {
			     ...
			     const PersistentCacheOptions &cache_options)
	{
		...
		*index_reader = new_index_reader;
		...
		s = FindMetaBlock(meta_index_iter,
		...
		s = ReadBlockContents(file, footer, ReadOptions(),
				      prefixes_handle, &prefixes_contents,
		...
		// TODO: log error
		if (s.ok()) {
			new_index_reader->index_block_->SetBlockPrefixIndex(
				prefix_index);
		}
	...
	NewIterator(BlockIter *iter = nullptr,
		    bool total_order_seek = true) override
	{
		return index_block_->NewIterator(icomparator_, iter,
						 total_order_seek);
	}
	...
	if (pos != props.end()) {
		if (pos->second == kPropFalse) {
			...
			ROCKS_LOG_WARN(info_log,
				       "Property %s has invalidate value %s",
				       user_prop_name.c_str(),
				       pos->second.c_str());
		}
	}
	return true;
			     ...
			     const bool skip_filters, const int level)
{
	...
	Status s = file->Prefetch(
		(file_size < 512 * 1024 ? 0 : file_size - 512 * 1024),
	...
	rep->hash_index_allow_collision =
		table_options.hash_index_allow_collision;
	...
	rep->persistent_cache_options = PersistentCacheOptions(
		rep->table_options.persistent_cache,
	...
	if (rep->filter_policy) {
		for (auto filter_type : { Rep::FilterType::kFullFilter,
					  Rep::FilterType::kPartitionedFilter,
			...
			if (FindMetaBlock(meta_iter.get(), filter_block_key,
					  &rep->filter_handle)
				    .ok()) {
	...
	s = SeekToCompressionDictBlock(meta_iter.get(),
				       &found_compression_dict);
	if (!s.ok()) {
		ROCKS_LOG_WARN(
			rep->ioptions.info_log,
		...
		// true.
		unique_ptr<BlockContents> compression_dict_block{
			new BlockContents()
		};
		...
		if (!s.ok()) {
			...
			rep->compression_dict_block =
				std::move(compression_dict_block);
		}
	}
	...
	if (!s.ok()) {
		...
		if (found_range_del_block && !rep->range_del_handle.IsNull()) {
			...
			s = MaybeLoadDataBlockToCache(
				rep, read_options, rep->range_del_handle,
	...
	if (rep->table_properties) {
		rep->whole_key_filtering &= IsFeatureSupported(
		...
		rep->global_seqno = GetGlobalSequenceNumber(
			*(rep->table_properties), rep->ioptions.info_log);
	}
	...
	if (table_options.cache_index_and_filter_blocks) {
		if (prefetch_index_and_filter_in_cache || level == 0) {
			...
			if (rep->table_options
				    .pin_l0_filter_and_index_blocks_in_cache &&
			    level == 0) {
				index_entry = &rep->index_entry;
			}
			unique_ptr<InternalIterator> iter(
				new_table->NewIndexIterator(
					ReadOptions(), nullptr, index_entry));
			s = iter->status();
			...
			if (s.ok()) {
				...
				if (rep->table_options
					...
					if (rep->filter_entry.value !=
					    nullptr) {
						rep->filter_entry.value
							->SetLevel(level);
					}
				} else {
					filter_entry.Release(
...
BlockBasedTable::GetTableProperties() const
{
	...
	bool is_index)
{
	...
	if (block_cache != nullptr) {
		block->cache_handle = GetEntryFromCache(
			...
			is_index ? BLOCK_CACHE_INDEX_MISS :
					 BLOCK_CACHE_DATA_MISS,
			is_index ? BLOCK_CACHE_INDEX_HIT : BLOCK_CACHE_DATA_HIT,
			statistics);
		if (block->cache_handle != nullptr) {
			block->value = reinterpret_cast<Block *>(
				block_cache->Value(block->cache_handle));
			return s;
		}
	}
	...
	if (s.ok()) {
		block->value = new Block(std::move(contents),
					 compressed_block->global_seqno(),
					 read_amp_bytes_per_bit,
		...
		if (block_cache != nullptr && block->value->cachable() &&
			...
			s = block_cache->Insert(block_cache_key, block->value,
						...
						&(block->cache_handle));
			block_cache->TEST_mark_as_data_block(
				block_cache_key, block->value->usable_size());
			if (s.ok()) {
...
Status BlockBasedTable::PutDataBlockToCache(
	...
	const Slice &compression_dict, size_t read_amp_bytes_per_bit,
	bool is_index, Cache::Priority priority)
{
	...
	if (raw_block->compression_type() != kNoCompression) {
		s = UncompressBlockContents(raw_block->data(),
	...
	if (block_cache_compressed != nullptr && raw_block != nullptr &&
	    raw_block->cachable()) {
		s = block_cache_compressed->Insert(compressed_block_cache_key,
						   raw_block,
	...
	if (block_cache != nullptr && block->value->cachable()) {
		...
		if (s.ok()) {
			...
			assert(reinterpret_cast<Block *>(block_cache->Value(
				       block->cache_handle)) == block->value);
		} else {
...
FilterBlockReader *
BlockBasedTable::ReadFilter(const BlockHandle &filter_handle,
			    const bool is_a_filter_partition) const
{
	...
	case Rep::FilterType::kPartitionedFilter: {
		return new PartitionedFilterBlockReader(
			rep->prefix_filtering ? rep->ioptions.prefix_extractor :
						      nullptr,
			rep->whole_key_filtering, std::move(block), nullptr,
			rep->ioptions.statistics, rep->internal_comparator,
			this);
	}
	...
	case Rep::FilterType::kBlockFilter:
		return new BlockBasedFilterBlockReader(
			rep->prefix_filtering ? rep->ioptions.prefix_extractor :
						      nullptr,
			rep->table_options, rep->whole_key_filtering,
			std::move(block), rep->ioptions.statistics);
	...
	case Rep::FilterType::kFullFilter: {
		...
		return new FullFilterBlockReader(
			rep->prefix_filtering ? rep->ioptions.prefix_extractor :
						      nullptr,
			rep->whole_key_filtering, std::move(block),
			filter_bits_reader, rep->ioptions.statistics);
	}
...
BlockBasedTable::CachableEntry<FilterBlockReader>
BlockBasedTable::GetFilter(const BlockHandle &filter_blk_handle,
...
InternalIterator *
BlockBasedTable::NewIndexIterator(const ReadOptions &read_options,
				  ...
				  CachableEntry<IndexReader> *index_entry)
{
	...
	IndexReader *index_reader = nullptr;
	if (cache_handle != nullptr) {
		index_reader = reinterpret_cast<IndexReader *>(
			block_cache->Value(cache_handle));
	} else {
...
InternalIterator *
BlockBasedTable::NewDataBlockIterator(Rep *rep, const ReadOptions &ro,
	...
	BlockIter *input_iter, bool is_index, Status s)
{
	...
	if (s.ok()) {
		...
		s = MaybeLoadDataBlockToCache(rep, ro, handle, compression_dict,
					      &block, is_index);
	}
	...
	if (s.ok() && block.value == nullptr) {
		...
		s = ReadBlockFromFile(
			rep->file.get(), rep->footer, ro, handle, &block_value,
			rep->ioptions, true /* compress */, compression_dict,
	...
	if (s.ok()) {
		...
		if (block.cache_handle != nullptr) {
			...
			iter->RegisterCleanup(&DeleteHeldResource<Block>,
					      block.value, nullptr);
		}
	} else {
		assert(block.value == nullptr);
	...
	bool is_index)
{
	...
	if (block_cache != nullptr || block_cache_compressed != nullptr) {
		...
		char cache_key[kMaxCacheKeyPrefixSize + kMaxVarint64Length];
		char compressed_cache_key[kMaxCacheKeyPrefixSize +
					  kMaxVarint64Length];
		...
		if (block_cache != nullptr) {
			key = GetCacheKey(rep->cache_key_prefix,
					  rep->cache_key_prefix_size, handle,
					  cache_key);
		}

		if (block_cache_compressed != nullptr) {
			ckey = GetCacheKey(
				rep->compressed_cache_key_prefix,
		...
		if (block_entry->value == nullptr && !no_io && ro.fill_cache) {
			...
			{
				StopWatch sw(rep->ioptions.env, statistics,
					     READ_BLOCK_GET_MICROS);
				s = ReadBlockFromFile(
					rep->file.get(), rep->footer, ro,
					handle, &raw_block, rep->ioptions,
					block_cache_compressed == nullptr,
					compression_dict,
					rep->persistent_cache_options,
					rep->global_seqno,
			...
			if (s.ok()) {
				s = PutDataBlockToCache(
					key, ckey, block_cache,
					block_cache_compressed, ro,
					rep->ioptions, block_entry,
					raw_block.release(),
...
BlockBasedTable::BlockEntryIteratorState::BlockEntryIteratorState(
	...
	: TwoLevelIteratorState(table->rep_->ioptions.prefix_extractor !=
				nullptr),
	  table_(table), read_options_(read_options), icomparator_(icomparator),
	  skip_filters_(skip_filters), is_index_(is_index),
	...
	const Slice &index_value)
{
	...
	Slice input = index_value;
	Status s = handle.DecodeFrom(&input);
	auto iter = NewDataBlockIterator(table_->rep_, read_options_, handle,
					 nullptr, is_index_, s);
	...
	if (filter != nullptr) {
		if (!filter->IsBlockBased()) {
			...
			may_match = filter->PrefixMayMatch(
				prefix, kNotValid, false, const_ikey_ptr);
		} else {
			InternalKey internal_key_prefix(
				prefix, kMaxSequenceNumber, kTypeValue);
...
InternalIterator *
BlockBasedTable::NewIterator(const ReadOptions &read_options, Arena *arena,
			     const InternalKeyComparator *icomp,
			     bool skip_filters)
{
	return NewTwoLevelIterator(
		new BlockEntryIteratorState(this, read_options, icomp,
					    skip_filters),
		NewIndexIterator(read_options), arena);
}

InternalIterator *
BlockBasedTable::NewRangeTombstoneIterator(const ReadOptions &read_options)
{
	...
	if (rep_->range_del_entry.cache_handle != nullptr) {
		...
		if (block_cache->Ref(rep_->range_del_entry.cache_handle)) {
			...
			iter->RegisterCleanup(
				&ReleaseCachedEntry, block_cache,
			    ...
			    GetContext *get_context, bool skip_filters)
{
	...
	if (!skip_filters) {
		filter_entry =
			GetFilter(read_options.read_tier == kBlockCacheTier);
	}
	FilterBlockReader *filter = filter_entry.value;
...
//  5. index_type
Status
BlockBasedTable::CreateIndexReader(IndexReader **index_reader,
				   ...
				   int level)
{
	...
	case BlockBasedTableOptions::kTwoLevelIndexSearch: {
		return PartitionIndexReader::Create(
			this, file, footer, footer.index_handle(),
			rep_->ioptions, icomparator, index_reader,
			rep_->persistent_cache_options, level);
	}
	case BlockBasedTableOptions::kBinarySearch: {
		return BinarySearchIndexReader::Create(
			file, footer, footer.index_handle(), rep_->ioptions,
			icomparator, index_reader,
	...
	case BlockBasedTableOptions::kHashSearch: {
		...
		if (meta_index_iter == nullptr) {
			auto s = ReadMetaBlock(rep_, &meta_guard,
					       &meta_iter_guard);
			if (!s.ok()) {
				...
				ROCKS_LOG_WARN(
					...
					" Fall back to binary search index.");
				return BinarySearchIndexReader::Create(
					file, footer, footer.index_handle(),
					rep_->ioptions, icomparator,
					index_reader,
					rep_->persistent_cache_options);
			}
		...
		return HashIndexReader::Create(
			rep_->internal_prefix_transform.get(), footer, file,
			rep_->ioptions, icomparator, footer.index_handle(),
			meta_index_iter, index_reader,
			rep_->hash_index_allow_collision,
			rep_->persistent_cache_options);
	}
	default: {
		std::string error_message = "Unrecognized index type: " +
					    ToString(rep_->index_type);
...
{
	unique_ptr<InternalIterator> index_iter(
	...
	std::vector<KVPairBlock> *kv_pair_blocks)
{
	...
	for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid();
		...
		for (datablock_iter->SeekToFirst(); datablock_iter->Valid();
			...
			kv_pair_block.push_back(std::make_pair(
...
{
	// Output Footer
	out_file->Append("Footer Details:\n"
			 "--------------------------------------\n"
			 "  ");
	...
	// Output MetaIndex
	out_file->Append("Metaindex Details:\n"
			 "--------------------------------------\n");
	std::unique_ptr<Block> meta;
	std::unique_ptr<InternalIterator> meta_iter;
	Status s = ReadMetaBlock(rep_, &meta, &meta_iter);
	if (s.ok()) {
		for (meta_iter->SeekToFirst(); meta_iter->Valid();
		     meta_iter->Next()) {
			s = meta_iter->status();
			if (!s.ok()) {
				return s;
			...
			if (meta_iter->key() == rocksdb::kPropertiesBlock) {
				...
				out_file->Append(meta_iter->value()
							 .ToString(true)
							 .c_str());
				out_file->Append("\n");
				   ...
				   rocksdb::kCompressionDictBlock) {
				out_file->Append(
					"  Compression dictionary block handle: ");
				out_file->Append(meta_iter->value()
							 .ToString(true)
							 .c_str());
				out_file->Append("\n");
				...
				out_file->Append("\n");
				   ...
				   rocksdb::kRangeDelBlock) {
				out_file->Append(
					"  Range deletion block handle: ");
				out_file->Append(meta_iter->value()
							 .ToString(true)
							 .c_str());
				out_file->Append("\n");
	...
	if (table_properties != nullptr) {
		out_file->Append("Table Properties:\n"
				 "--------------------------------------\n"
				 "  ");
		out_file->Append(
		...
		if (table_properties->filter_policy_name.compare(
			...
			if (FindMetaBlock(meta_iter.get(), filter_block_key,
				...
				if (ReadBlockContents(
					    rep_->file.get(), rep_->footer,
					    ReadOptions(), handle, &block,
					    rep_->ioptions,
					    false /*decompress*/,
					    Slice() /*compression dict*/,
					    rep_->persistent_cache_options)
	...
	if (rep_->filter) {
		out_file->Append("Filter Details:\n"
				 "--------------------------------------\n"
				 "  ");
	...
	if (rep_->compression_dict_block != nullptr) {
		...
		out_file->Append("Compression Dictionary:\n"
				 "--------------------------------------\n");
	...
	if (range_del_iter != nullptr) {
		...
		if (range_del_iter->Valid()) {
			out_file->Append(
				...
				"--------------------------------------\n"
				"  ");
			for (; range_del_iter->Valid();
			     range_del_iter->Next()) {
				DumpKeyValue(range_del_iter->key(),
					     range_del_iter->value(), out_file);
			}
			out_file->Append("\n");
		}
...
void BlockBasedTable::Close()
{
	...
	if (!rep_->table_options.no_block_cache) {
		...
		auto key = GetCacheKey(rep_->cache_key_prefix,
				       rep_->cache_key_prefix_size,
...
Status BlockBasedTable::DumpIndexBlock(WritableFile *out_file)
{
	...
	for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid();
		...
		out_file->Append(": ");
		out_file->Append(
			blockhandles_iter->value().ToString(true).c_str());
		out_file->Append("\n");
...
Status BlockBasedTable::DumpDataBlocks(WritableFile *out_file)
{
	...
	for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid();
		...
		datablock_size_min =
			std::min(datablock_size_min, datablock_size);
		datablock_size_max =
			std::max(datablock_size_max, datablock_size);
		...
		for (datablock_iter->SeekToFirst(); datablock_iter->Valid();
			...
			DumpKeyValue(datablock_iter->key(),
				     datablock_iter->value(), out_file);
		}
		out_file->Append("\n");
	}
	...
	if (num_datablocks) {
		double datablock_size_avg =
			static_cast<double>(datablock_size_sum) /
			num_datablocks;
...
{
	FilterBlockReader *filter =
		reinterpret_cast<FilterBlockReader *>(value);
...
void DeleteCachedIndexEntry(const Slice &key, void *value)
{
	...
	if (index_reader->statistics() != nullptr) {
		RecordTick(index_reader->statistics(),
