...
class BlockBasedTable : public TableReader {
	...
	InternalIterator *
	NewIterator(const ReadOptions &, Arena *arena = nullptr,
	...
	Status
	GetKVPairsFromDataBlocks(std::vector<KVPairBlock> *kv_pair_blocks);
    ...
    protected:
	template <class TValue> struct CachableEntry;
	struct Rep;
	Rep *rep_;
	...
	NewDataBlockIterator(Rep *rep, const ReadOptions &ro,
			     const BlockHandle &block_hanlde,
			     BlockIter *input_iter = nullptr,
			     bool is_index = false, Status s = Status());
	...
	virtual CachableEntry<FilterBlockReader>
	GetFilter(const BlockHandle &filter_blk_handle,
	...
	//     kBlockCacheTier
	InternalIterator *
	NewIndexIterator(const ReadOptions &read_options,
			 BlockIter *input_iter = nullptr,
	...
	static Status GetDataBlockFromCache(
		const Slice &block_cache_key,
		const Slice &compressed_block_cache_key, Cache *block_cache,
	...
	static Status PutDataBlockToCache(
		const Slice &block_cache_key,
		const Slice &compressed_block_cache_key, Cache *block_cache,
		Cache *block_cache_compressed, const ReadOptions &read_options,
		const ImmutableCFOptions &ioptions, CachableEntry<Block> *block,
	...
	Status
	CreateIndexReader(IndexReader **index_reader,
	...
	static Status ReadMetaBlock(Rep *rep,
	...
	static void GenerateCachePrefix(Cache *cc, WritableFile *file,
					char *buffer, size_t *size);
...
class BlockBasedTable::BlockEntryIteratorState : public TwoLevelIteratorState {
	...
	InternalIterator *
	NewSecondaryIterator(const Slice &index_value) override;
...
template <class TValue> struct BlockBasedTable::CachableEntry {
...
struct BlockBasedTable::Rep {
	Rep(const ImmutableCFOptions &_ioptions, const EnvOptions &_env_options,
	    ...
	    bool skip_filters)
		: ioptions(_ioptions), env_options(_env_options),
		  table_options(_table_opt),
		  filter_policy(skip_filters ? nullptr :
						     _table_opt.filter_policy.get()),
