...
PlainTableIndex::IndexSearchResult
PlainTableIndex::GetOffset(uint32_t prefix_hash, uint32_t *bucket_value) const
{
					  ...
					  uint32_t key_offset)
{
	...
	if (index_sparseness_ == 0 ||
	    num_keys_per_prefix_ % index_sparseness_ == 0) {
		due_index_ = true;
	}
...
void PlainTableIndexBuilder::AllocateIndex()
{
	if (prefix_extractor_ == nullptr || hash_table_ratio_ <= 0) {
		...
		index_size_ = static_cast<uint32_t>(num_prefixes_ *
	...
	const std::vector<uint32_t> &entries_per_bucket)
{
	...
	uint32_t *index = reinterpret_cast<uint32_t *>(
		EncodeVarint32(temp_ptr, num_prefixes_));
	...
	for (uint32_t i = 0; i < index_size_; i++) {
		...
		default:
			...
			char *cur_ptr =
				EncodeVarint32(prev_ptr, num_keys_for_bucket);
			sub_index_offset +=
				static_cast<uint32_t>(cur_ptr - prev_ptr);
			...
			for (j = num_keys_for_bucket - 1; j >= 0 && record;
				...
				EncodeFixed32(sub_index_pos +
						      j * sizeof(uint32_t),
					      record->offset);
			}
			assert(j == -1 && record == nullptr);
			sub_index_offset += PlainTableIndex::kOffsetLen *
					    num_keys_for_bucket;
