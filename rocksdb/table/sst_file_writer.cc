...
struct SstFileWriter::Rep {
	Rep(const EnvOptions &_env_options, const Options &options,
		...
		: env_options(_env_options), ioptions(options),
		  mutable_cf_options(options), io_priority(_io_priority),
		  internal_comparator(_user_comparator), cfh(_cfh),
		   ...
		   const ValueType value_type)
	{
		...
		if (file_info.num_entries == 0) {
			file_info.smallest_key.assign(user_key.data(),
						      user_key.size());
		} else {
	...
	void InvalidatePageCache(bool closing)
	{
		...
		if (bytes_since_last_fadvise > kFadviseTrigger || closing) {
			TEST_SYNC_POINT_CALLBACK(
				"SstFileWriter::Rep::InvalidatePageCache",
...
Status SstFileWriter::Open(const std::string &file_path)
{
	...
	s = r->ioptions.env->NewWritableFile(file_path, &sst_file,
					     r->env_options);
	if (!s.ok()) {
		return s;
	}
	...
	if (r->ioptions.bottommost_compression != kDisableCompressionOption) {
		...
		compression_type =
			*(r->ioptions.compression_per_level.rbegin());
	} else {
	...
	if (r->cfh != nullptr) {
		...
		cf_id = TablePropertiesCollectorFactory::Context::
	...
	TableBuilderOptions table_builder_options(
		...
		false /* skip_filters */, r->column_family_name, unknown_level);
	r->file_writer.reset(
