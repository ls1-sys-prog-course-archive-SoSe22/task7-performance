...
class BlockReadAmpBitmap {
	...
	void Mark(uint32_t start_offset, uint32_t end_offset)
	{
		...
		if (GetAndSet(start_bit) == 0) {
			uint32_t new_useful_bytes =
				(exclusive_end_bit - start_bit)
				<< bytes_per_bit_pow_;
			RecordTick(GetStatistics(),
	...
	uint32_t GetBytesPerBit()
	{
		return 1 << bytes_per_bit_pow_;
	}
	...
	inline bool GetAndSet(uint32_t bit_idx)
	{
		...
		return bitmap_[byte_idx].fetch_or(bit_mask,
...
class Block {
	...
	SequenceNumber global_seqno() const
	{
...
class BlockIter : public InternalIterator {
    public:
	BlockIter()
		: comparator_(nullptr), data_(nullptr), restarts_(0),
		  num_restarts_(0), current_(0), restart_index_(0),
		  status_(Status::OK()), prefix_index_(nullptr),
		  key_pinned_(false),
		  global_seqno_(kDisableGlobalSequenceNumber),
		  read_amp_bitmap_(nullptr), last_bitmap_offset_(0)
	{
	}
	...
	virtual Slice value() const override
	{
		...
		if (read_amp_bitmap_ && current_ < restarts_ &&
			...
			read_amp_bitmap_->Mark(
				current_ /* current entry offset */,
				NextEntryOffset() - 1);
	...
	{
		return NextEntryOffset() - current_;
	}
	...
	struct CachedPrevEntry {
		explicit CachedPrevEntry(uint32_t _offset, const char *_key_ptr,
					 ...
					 Slice _value)
			: offset(_offset), key_ptr(_key_ptr),
			  key_offset(_key_offset), key_size(_key_size),
			  value(_value)
		{
		}
